                                            ...
                                            - ✅· `WeilPairing.exists_weilPairing_frobenius` — Frobenius-twisted ZMod p Weil pairing over F_q
                                              ...
                                              naturality becomes q-scaling since frobAlgHom is x^q).
                                                - ❌🟪 `WeilPairing.exists_weilPairing_mu` — the mu_p-valued Weil pairing over F_q (sorry node;
                                                  ...
                                                  k(X) Frac(C): route via IntermediateField.isSepara
                                                  ble_adjoin_simple_iff_isSeparable
                                                  (SeparableDegree.lean:798) + adjoin{root-image} =
                                                  top (from the hfd span, refactor hspan out) +
                                                  AlgEquiv-transfer through topEquiv; IsSeparable of
                                                  the root: minpoly divides the quadratic T^2 + A'T
                                                  - G' (Separable.of_dvd); the quadratic is
                                                  separable by an explicit Bezout: 4Q -
                                                  (2T+A')(derivative Q) = -(A'^2+4G') = -D with D !=
                                                  0 in k(X) when q != 2 (leading coeff 4), and in
                                                  char 2 the derivative is the CONSTANT A' which is
                                                  a unit since A != 0 (if a1 = a3 = 0 in char 2 then
                                                  b2 = b4 = b6 = 0 and Delta = 0, contradiction);
                                                  (iv) IsIntegralClosure (normality): decompose an
                                    ...
                                    - ✅· `WeilPairing.exists_weilPairing_frobenius` — Frobenius-twisted ZMod p Weil pairing over F_q (DERIVED
                                      ...
                                      x^q).
                                        - ❌🟪 `WeilPairing.exists_weilPairing_mu` — the mu_p-valued Weil pairing over F_q (sorry node; N1 in
                                          ...
                                          (iii) Algebra.IsSeparable k(X) Frac(C): route via Intermed
                                          iateField.isSeparable_adjoin_simple_iff_isSeparable
                                          (SeparableDegree.lean:798) + adjoin{root-image} = top
                                          (from the hfd span, refactor hspan out) + AlgEquiv-
                                          transfer through topEquiv; IsSeparable of the root:
                                          minpoly divides the quadratic T^2 + A'T - G'
                                          (Separable.of_dvd); the quadratic is separable by an
                                          explicit Bezout: 4Q - (2T+A')(derivative Q) = -(A'^2+4G')
                                          = -D with D != 0 in k(X) when q != 2 (leading coeff 4),
                                          and in char 2 the derivative is the CONSTANT A' which is a
                                          unit since A != 0 (if a1 = a3 = 0 in char 2 then b2 = b4 =
                                          b6 = 0 and Delta = 0, contradiction); (iv)
                                          IsIntegralClosure (normality): decompose an integral x = s
                                          + t*root-image over the L-basis, trace/norm land in k[X]
                                          (integrally closed), derive t^2 * D in k[X] with D = A^2 +
                                          4G the b2-cubic, D squarefree from disc(D) = 16 Delta
                                          (Cubic.discr + squarefree iff nodup roots over the

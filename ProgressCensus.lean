...
tooling. Run as `lake lean ProgressCensus.lean`: lake builds the file's
import closure incrementally (the standard cache-effective path — this
replaces the old lean-daemon.py freshness/materialization machinery),
then elaboration prints ONE JSON object to stdout. The census logic is
ported from lean-daemon.py's LEAN_SERVER_BODY (the daemon's `report`
and `sorries` commands, merged into a single response).

The import block between the BEGIN/END markers below — including the
baked-in input path — is GENERATED by progress-tree.py on every run,
from a scan of the Fermat/ source tree EXCLUDING the root aggregator
`Fermat` (whose `#assert_no_sorry` gate fails by design mid-project).
Do not edit the block by hand. The body after the END marker is
hand-maintained.

...
* `exists_weierstrassModel_of_ellipticScheme`, `exists_isIso_of_affineChart` and
  `relPointPost_add` — the Weierstrass-comparison cluster further down, which
  belongs to a different node and is listed here only so that this count
  matches the compiler's.  (`exists_geomFibreAddEquiv_of_weierstrassModel` was
  the leaf here until 2026-07-27 and is now PROVEN from the latter two plus
  `hom_specRat_eq_of_range_eq`.)

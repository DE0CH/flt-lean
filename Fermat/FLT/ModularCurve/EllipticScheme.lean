...
The open leaves of this FILE are therefore THIRTEEN, and this count was
REGENERATED (2026-07-27) from the COMPILER's `declaration uses 'sorry'` warning
set, not from any branch's prose — three branches each carried a list that was
correct on its own branch and wrong once the others landed, and the count "TEN"
written at that integration was itself already one short of the build's eleven.
The second-law cut of `exists_projMulOfCoords` then closed one leaf and opened
three.  A rising count here is DISCLOSURE — the gluing was always this big; it
is only now written down as separable pieces.  Three further leaves of this
cluster live OUTSIDE this file, in
...
* **`exists_projMul` is PROVEN as of 2026-07-27** and is no longer a leaf.  It
  ...
  `Proj.fromOfGlobalSections`.  Its successor leaves are
  ...
  and — since the 2026-07-27 second-law cut — `exists_projMulOfCoordsTwo`
  (the gluing, now characterised by BOTH Bosma–Lenstra laws),
  `projMulCoords_unit` and `projMulCoords_inv` (the two axioms, each a finite
  case analysis over a field).  `exists_projMulOfCoords` itself is PROVEN from
  those three.  `WeierstrassCurve.Projective.equation_addXYZ` is PROVEN in
  `Fermat/FLT/Mathlib/.../ProjectiveAddition.lean`, which now also carries the
  second law `add2XYZ` — DEFINED there, with `equation_add2XYZ` and the two
  proportionality lemmas as its own leaves.  `hcomm` is PROVEN rather than
  assumed, from antisymmetry of the chord–tangent forms plus a residue-field
  density argument; `hunit` and `hinv` are NOT chart identities of the standard
  law, which degenerates exactly where each of them is asserted — correcting
  this file's earlier prose — which is why they need the second law;

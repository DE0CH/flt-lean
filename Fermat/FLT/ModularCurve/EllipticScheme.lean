...
homomorphism** (PROVEN 2026-07-27 over the project's EXISTING
...
## GENERALISED 2026-07-30: the base is an ARBITRARY SCHEME `S`

It was stated over `Spec ℚ` until then, and **not one line of the proof used
that**: `eq_comp_of_rigidity_axes` and
`hasUniversallyTrivialPushforward_of_isProper_of_smooth` are both stated over
an arbitrary base (the old docstring said so and then declined the
generality), and every other step is `AbelianSchemeStruct` naturality.  The
only edit was to the signature.  The `ℚ` call site
(`exists_geomFibreAddEquiv_of_weierstrassModel`) is unchanged, `S` being
inferred; the new consumers are `nonempty_addEquiv_relPoint_of_isIso` below
and, through it, the `k`-rational Weierstrass bridge over an arbitrary
field. -/
theorem relPointPost_add {S A B : Scheme.{u}} {fA : A ⟶ S}

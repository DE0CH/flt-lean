...
invertible), is PROVEN.  The other two are open: `exists_projChartRingEquiv`
(the dehomogenisation isomorphism `(ℚ[X, Y, Z] ⧸ (W))_{(xᵢ)}` in degree `0`
≃ `ℚ[u, v] ⧸ (wᵢ)`, a missing piece of MATHLIB rather than of this development)
and `projChart_jacobian_span_eq_top` (the chart Jacobian criterion, where `hjac`
and hence `Δ` is consumed).

The open leaves are therefore `exists_projAdd` — where all the remaining gluing
work for the group law now lives — those two,

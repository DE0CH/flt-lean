...
The open leaves are therefore SEVEN, and this list is stated from the file's ACTUAL
sorry set as merged (2026-07-27), not inherited from either side of a merge:
...
`exists_projGroupLaw_geomFibreAddEquiv`, `locally_isStandardSmooth_awayCoord` (all
that is left of item 7a — and what it wants is a missing piece of MATHLIB, the
dehomogenisation isomorphism for a chart of `Proj` of a polynomial quotient),
`exists_affineChart_projModel`, and the two leaves that

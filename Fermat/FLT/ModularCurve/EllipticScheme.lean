...
`exists_projAdd` — where all the remaining gluing work for the group law used to
live — is itself now PROVEN from the two leaves it was decomposed into,
`exists_projMul` and `projMul_assoc`.

The open leaves are therefore `exists_projMul` (the gluing plus the three axioms
that are chart identities), `projMul_assoc` (associativity),
`exists_projGeomFibreAddEquiv`, `locally_isStandardSmooth_awayCoord` (all that is
left of item 7a — and what it wants is a missing piece of MATHLIB, the
dehomogenisation isomorphism for a chart of `Proj` of a polynomial quotient), and
the interior of `geometricallyConnected_projToSpec`, which still carries three
named sorried steps `hbc`/`hne`/`hpre`.  Each declaration carries its own
docstring saying what

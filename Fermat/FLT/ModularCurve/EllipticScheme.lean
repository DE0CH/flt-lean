...
The open leaves of this FILE are therefore TEN, and this list was REGENERATED
...
own branch and wrong once the others landed.  (It was EIGHT before the
`ProjCoords` cut of `exists_projMul` landed: that cut closed one leaf and
opened four, of which one is already proven, so the net is +2.)
...
* **`exists_projMul` is PROVEN as of 2026-07-27** and is no longer a leaf.  It
  ...
  and `exists_projMulOfCoords` (the gluing) — a fifth,
  `WeierstrassCurve.Projective.equation_addXYZ`, is PROVEN in
  `Fermat/FLT/Mathlib/.../ProjectiveAddition.lean`.  `hcomm` is now PROVEN
  rather than assumed, from antisymmetry of the chord–tangent forms plus a
  residue-field density argument; `hunit` and `hinv` stayed with the
  constructor because — correcting this file's earlier prose — they are NOT
  chart identities of the standard law, which degenerates exactly where each
  of them is asserted;

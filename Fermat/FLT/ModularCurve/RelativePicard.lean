...
/-! ### `IsRelPicOf` IS STABLE UNDER BASE CHANGE OF THE BASE

**PROVEN 2026-07-31**, and it is the pair of sublemmas
`exists_gluedRelPic_of_forall_isAffineOpen`'s route (step 2) named as owed —
*`IsRelPicOf` is stable under base change of the base* and *`IsRelPicOf`
transports along an isomorphism of representing objects over the base*.  They
are ONE lemma here: `IsRelPicOf.ofIsPullback` is stated over an abstract
cartesian square, so the transport along an isomorphism is its `h = 𝟙 S`
instance (`IsRelPicOf.ofIsoOver`), an isomorphism being a pullback along the
identity.

Everything is phrased with `IsPullback` rather than with `pullback`, for the
reason `isPullback_curveBaseChangeMap` was proved below: the consumer's curve
side is `X ×_S U = (X ×_S V) ×_V U`, which is a pasting of squares and not a
chosen pullback, and forcing it into `pullback` shape costs a transport at
every use. -/

/-- **`𝟙^* L ≅ L`** — `Scheme.Modules.pullbackId`, read on an object.

Named `…Pic` because `Modularity/AbelianSchemeIsogeny.lean` carries an
identical `Fermat.modPullbackIdIso`; that module is NOT in this one's import
cone, so the two coexist, but a downstream consumer of both would see a
duplicate declaration. -/
noncomputable def modPullbackIdIsoPic {Z : Scheme.{u}} (L : Z.Modules) :
    modPullback (𝟙 Z) L ≅ L :=
  (Scheme.Modules.pullbackId Z).app L

/-- **`e.inv^*(e.hom^* L) ≅ L`.** -/
noncomputable def modPullbackInvHomIsoPic {Z W : Scheme.{u}} (e : W ≅ Z) (L : Z.Modules) :
    modPullback e.inv (modPullback e.hom L) ≅ L :=
  modPullbackCompIso e.inv e.hom L ≪≫ modPullbackCongrIso e.inv_hom_id L ≪≫
    modPullbackIdIsoPic L

/-- **`e.hom^*(e.inv^* L) ≅ L`.** -/
noncomputable def modPullbackHomInvIsoPic {Z W : Scheme.{u}} (e : W ≅ Z) (L : W.Modules) :
    modPullback e.hom (modPullback e.inv L) ≅ L :=
  modPullbackCompIso e.hom e.inv L ≪≫ modPullbackCongrIso e.hom_inv_id L ≪≫
    modPullbackIdIsoPic L

/-- **PULLBACK ALONG ANY MORPHISM OVER THE BASE PRESERVES `RelPicEquiv`**
(PROVEN) — the generalisation of `relPicEquiv_modPullback` from
`curveBaseChangeMap` to an arbitrary morphism of base-changed curves lying
over a morphism `u` of the two bases.

`relPicEquiv_modPullback` is the instance `ψ = curveBaseChangeMap strX h hg`,
`u = h`; what is used below is the instance where `ψ` is an ISOMORPHISM of
curves over a FIXED base, i.e. `u = 𝟙`.  The proof is
`relPicEquiv_modPullback`'s, with `curveBaseChangeMap_proj` replaced by the
commutation hypothesis, which is the only thing that proof ever knew about
`curveBaseChangeMap`. -/
theorem relPicEquiv_modPullback_of_comm {X S T X' S' T' : Scheme.{u}} (strX : X ⟶ S)
    {g : T ⟶ S} {strX' : X' ⟶ S'} {g' : T' ⟶ S'}
    (ψ : curveBaseChange strX' g' ⟶ curveBaseChange strX g) (u : T' ⟶ T)
    (hcomm : ψ ≫ curveBaseChangeProj strX g = curveBaseChangeProj strX' g' ≫ u)
    {L L' : (curveBaseChange strX g).Modules} (hLL' : RelPicEquiv strX g L L') :
    RelPicEquiv strX' g' (modPullback ψ L) (modPullback ψ L') := by
  obtain ⟨N, hN, ⟨e⟩⟩ := hLL'
  refine ⟨modPullback u N, isInvertibleSheaf_modPullback u hN, ⟨?_⟩⟩
  refine modPullbackMapIso _ e ≪≫
    (nonempty_modPullback_modTensorPic ψ L'
      (modPullback (curveBaseChangeProj strX g) N)).some ≪≫
    modTensorMapIso (Iso.refl _) ?_
  refine modPullbackCompIso ψ (curveBaseChangeProj strX g) N ≪≫ ?_
  refine modPullbackCongrIso hcomm N ≪≫ ?_
  exact (modPullbackCompIso (curveBaseChangeProj strX' g') u N).symm

section IsRelPicOfBaseChange

variable {X S P X' S' P' : Scheme.{u}} {strX : X ⟶ S} {pstr : P ⟶ S} {h : S' ⟶ S}
  {strX' : X' ⟶ S'} {φ : X' ⟶ X} {pstr' : P' ⟶ S'} {ψ : P' ⟶ P}

/-- **A `T`-point of `P' = P ×_S S'` over `k : T ⟶ S'` IS a `T`-point of `P`
over `k ≫ h`** (PROVEN, the easy direction): compose with `ψ`. -/
def relPointBaseChange (hPb : IsPullback ψ pstr' pstr h) {T : Scheme.{u}} {k : T ⟶ S'}
    (p : RelPoint pstr' k) : RelPoint pstr (k ≫ h) :=
  ⟨p.1 ≫ ψ, by rw [Category.assoc, hPb.w, ← Category.assoc, p.2]⟩

/-- The inverse of `relPointBaseChange`, by the universal property of the
cartesian square. -/
noncomputable def relPointBaseChangeInv (hPb : IsPullback ψ pstr' pstr h) {T : Scheme.{u}}
    {k : T ⟶ S'} (q : RelPoint pstr (k ≫ h)) : RelPoint pstr' k :=
  ⟨hPb.lift q.1 k q.2, hPb.lift_snd _ _ _⟩

theorem relPointBaseChange_relPointBaseChangeInv (hPb : IsPullback ψ pstr' pstr h)
    {T : Scheme.{u}} {k : T ⟶ S'} (q : RelPoint pstr (k ≫ h)) :
    relPointBaseChange hPb (relPointBaseChangeInv hPb q) = q :=
  Subtype.ext (hPb.lift_fst _ _ _)

theorem relPointBaseChangeInv_relPointBaseChange (hPb : IsPullback ψ pstr' pstr h)
    {T : Scheme.{u}} {k : T ⟶ S'} (p : RelPoint pstr' k) :
    relPointBaseChangeInv hPb (relPointBaseChange hPb p) = p := by
  refine Subtype.ext (hPb.hom_ext ?_ ?_)
  · show hPb.lift (p.1 ≫ ψ) k _ ≫ ψ = _
    rw [hPb.lift_fst]
  · show hPb.lift (p.1 ≫ ψ) k _ ≫ pstr' = _
    rw [hPb.lift_snd, p.2]

/-- `relPointBaseChange` is NATURAL in the test object (PROVEN) — both sides
are `⟨(u ≫ p.1) ≫ ψ, _⟩` up to associativity. -/
theorem relPointBaseChange_pre (hPb : IsPullback ψ pstr' pstr h) {T T' : Scheme.{u}}
    (u : T' ⟶ T) {k : T ⟶ S'} {k' : T' ⟶ S'} (hk : u ≫ k = k') (p : RelPoint pstr' k) :
    relPointBaseChange hPb (RelPoint.pre u hk p)
      = RelPoint.pre u (by rw [← Category.assoc, hk]) (relPointBaseChange hPb p) :=
  Subtype.ext (Category.assoc _ _ _)

/-- **`X' ×_{S'} T ≅ X ×_S T` FOR A BASE-CHANGED CURVE** (PROVEN): the pasting
of the cartesian square `X' = X ×_S S'` with the one defining `X' ×_{S'} T`,
read through `IsPullback.isoPullback`.

Being the canonical map into a pullback, it is pinned by its two projections
(`curveBaseChangeIsoOfIsPullback_hom_fst`/`_hom_snd`), which is what makes every
identity about it a `pullback.hom_ext`. -/
noncomputable def curveBaseChangeIsoOfIsPullback (hX : IsPullback φ strX' strX h)
    {T : Scheme.{u}} (k : T ⟶ S') :
    curveBaseChange strX' k ≅ curveBaseChange strX (k ≫ h) :=
  ((IsPullback.of_hasPullback strX' k).paste_horiz hX).isoPullback

@[reassoc (attr := simp)]
theorem curveBaseChangeIsoOfIsPullback_hom_fst (hX : IsPullback φ strX' strX h)
    {T : Scheme.{u}} (k : T ⟶ S') :
    (curveBaseChangeIsoOfIsPullback hX k).hom ≫ pullback.fst strX (k ≫ h)
      = pullback.fst strX' k ≫ φ :=
  IsPullback.isoPullback_hom_fst _

@[reassoc (attr := simp)]
theorem curveBaseChangeIsoOfIsPullback_hom_snd (hX : IsPullback φ strX' strX h)
    {T : Scheme.{u}} (k : T ⟶ S') :
    (curveBaseChangeIsoOfIsPullback hX k).hom ≫ curveBaseChangeProj strX (k ≫ h)
      = curveBaseChangeProj strX' k :=
  IsPullback.isoPullback_hom_snd _

theorem curveBaseChangeIsoOfIsPullback_inv_proj (hX : IsPullback φ strX' strX h)
    {T : Scheme.{u}} (k : T ⟶ S') :
    (curveBaseChangeIsoOfIsPullback hX k).inv ≫ curveBaseChangeProj strX' k
      = curveBaseChangeProj strX (k ≫ h) := by
  rw [← curveBaseChangeIsoOfIsPullback_hom_snd hX k, Iso.inv_hom_id_assoc]

/-- **The comparison isomorphisms are NATURAL in the test object** (PROVEN) —
`pullback.hom_ext` against the two projections.  This is the whole content of
the `sheaf_pre` field of `IsRelPicOf.ofIsPullback`. -/
theorem curveBaseChangeIsoOfIsPullback_naturality (hX : IsPullback φ strX' strX h)
    {T T' : Scheme.{u}} (u : T' ⟶ T) {k : T ⟶ S'} {k' : T' ⟶ S'} (hk : u ≫ k = k') :
    (curveBaseChangeIsoOfIsPullback hX k').hom
        ≫ curveBaseChangeMap strX u (by rw [← Category.assoc, hk] : u ≫ (k ≫ h) = k' ≫ h)
      = curveBaseChangeMap strX' u hk ≫ (curveBaseChangeIsoOfIsPullback hX k).hom := by
  apply pullback.hom_ext
  · simp [curveBaseChangeMap, pullback.lift_fst, pullback.lift_fst_assoc]
  · simp [curveBaseChangeMap, pullback.lift_snd]

/-- `RelPicEquiv` transports DOWN the comparison isomorphism (PROVEN) — the
`u = 𝟙` instance of `relPicEquiv_modPullback_of_comm`. -/
theorem relPicEquiv_curveBaseChangeIso (hX : IsPullback φ strX' strX h) {T : Scheme.{u}}
    {k : T ⟶ S'} {L L' : (curveBaseChange strX (k ≫ h)).Modules}
    (hLL' : RelPicEquiv strX (k ≫ h) L L') :
    RelPicEquiv strX' k (modPullback (curveBaseChangeIsoOfIsPullback hX k).hom L)
      (modPullback (curveBaseChangeIsoOfIsPullback hX k).hom L') :=
  relPicEquiv_modPullback_of_comm strX _ (𝟙 T) (by simp) hLL'

/-- `RelPicEquiv` transports UP the comparison isomorphism (PROVEN) — the same
lemma at the inverse, and what makes the `inj` field of
`IsRelPicOf.ofIsPullback` go through. -/
theorem relPicEquiv_curveBaseChangeIso_inv (hX : IsPullback φ strX' strX h) {T : Scheme.{u}}
    {k : T ⟶ S'} {L L' : (curveBaseChange strX' k).Modules} (hLL' : RelPicEquiv strX' k L L') :
    RelPicEquiv strX (k ≫ h) (modPullback (curveBaseChangeIsoOfIsPullback hX k).inv L)
      (modPullback (curveBaseChangeIsoOfIsPullback hX k).inv L') :=
  relPicEquiv_modPullback_of_comm strX' _ (𝟙 T)
    (by simp [curveBaseChangeIsoOfIsPullback_inv_proj]) hLL'

namespace IsRelPicOf

/-- **The sheaf classified by a `T`-point of the base-changed representing
object**: read the point upstairs and pull its sheaf back along the comparison
isomorphism of curves. -/
noncomputable def sheafOfIsPullback (hP : IsRelPicOf strX pstr) (hX : IsPullback φ strX' strX h)
    (hPb : IsPullback ψ pstr' pstr h) {T : Scheme.{u}} {k : T ⟶ S'} (p : RelPoint pstr' k) :
    (curveBaseChange strX' k).Modules :=
  modPullback (curveBaseChangeIsoOfIsPullback hX k).hom (hP.sheaf (relPointBaseChange hPb p))

/-- **`IsRelPicOf` IS STABLE UNDER BASE CHANGE OF THE BASE** (PROVEN
2026-07-31).

If `P` represents `Pic_{X/S}` and both the curve and the representing object
are base-changed along `h : S' ⟶ S` — as CARTESIAN SQUARES, not as chosen
pullbacks — then `P'` represents `Pic_{X'/S'}`.

There is no geometry in it.  A `T`-point of `P'` over `k` is a `T`-point of `P`
over `k ≫ h` (`relPointBaseChange`, a bijection by the universal property of
the square), and the two curves `X' ×_{S'} T` and `X ×_S T` are canonically
isomorphic OVER `T` (`curveBaseChangeIsoOfIsPullback`, the pasting of the two
squares).  Every field is then that bijection and that isomorphism:
`inj`/`surj` transport `RelPicEquiv` across the isomorphism in the two
directions (`relPicEquiv_curveBaseChangeIso`, `…_inv`), and `sheaf_pre` is the
naturality of the isomorphism (`curveBaseChangeIsoOfIsPullback_naturality`)
composed with `hP.sheaf_pre`.

This is one of the two sublemmas the route of
`exists_gluedRelPic_of_forall_isAffineOpen` names; the other,
*transport along an isomorphism of representing objects*, is `ofIsoOver`
below and is this lemma at `h = 𝟙 S`. -/
noncomputable def ofIsPullback (hP : IsRelPicOf strX pstr) (hX : IsPullback φ strX' strX h)
    (hPb : IsPullback ψ pstr' pstr h) : IsRelPicOf strX' pstr' where
  sheaf p := hP.sheafOfIsPullback hX hPb p
  invertible p := isInvertibleSheaf_modPullback _ (hP.invertible _)
  inj {T} {k} p q hpq := by
    have h1 := relPicEquiv_curveBaseChangeIso_inv (strX := strX) hX hpq
    have h2 : RelPicEquiv strX (k ≫ h) (hP.sheaf (relPointBaseChange hPb p))
        (hP.sheaf (relPointBaseChange hPb q)) := by
      refine relPicEquiv_trans _ _
        (relPicEquiv_of_iso _ _
          (modPullbackInvHomIsoPic (curveBaseChangeIsoOfIsPullback hX k) _).symm)
        (relPicEquiv_trans _ _ h1
          (relPicEquiv_of_iso _ _
            (modPullbackInvHomIsoPic (curveBaseChangeIsoOfIsPullback hX k) _)))
    have := hP.inj _ _ h2
    rw [← relPointBaseChangeInv_relPointBaseChange hPb p,
      ← relPointBaseChangeInv_relPointBaseChange hPb q, this]
  surj {T} {k} L hL := by
    obtain ⟨q, hq⟩ := hP.surj (modPullback (curveBaseChangeIsoOfIsPullback hX k).inv L)
      (isInvertibleSheaf_modPullback _ hL)
    refine ⟨relPointBaseChangeInv hPb q, ?_⟩
    have h1 := relPicEquiv_curveBaseChangeIso (strX' := strX') hX hq
    refine relPicEquiv_trans _ _ ?_
      (relPicEquiv_trans _ _ h1
        (relPicEquiv_of_iso _ _
          (modPullbackHomInvIsoPic (curveBaseChangeIsoOfIsPullback hX k) L)))
    exact relPicEquiv_of_iso _ _
      (modPullbackMapIso _ (eqToIso (by rw [relPointBaseChange_relPointBaseChangeInv hPb q])))
  sheaf_pre {T'} {T} u {k} {k'} hk p := by
    have hk' : u ≫ (k ≫ h) = k' ≫ h := by rw [← Category.assoc, hk]
    have h1 := relPicEquiv_curveBaseChangeIso (strX' := strX') (k := k') hX
      (hP.sheaf_pre u hk' (relPointBaseChange hPb p))
    refine relPicEquiv_trans _ _ ?_ (relPicEquiv_trans _ _ h1 ?_)
    · exact relPicEquiv_of_iso _ _
        (modPullbackMapIso _ (eqToIso (by rw [relPointBaseChange_pre hPb u hk p])))
    · refine relPicEquiv_of_iso _ _ ?_
      refine modPullbackCompIso _ _ _ ≪≫ ?_
      refine modPullbackCongrIso (curveBaseChangeIsoOfIsPullback_naturality hX u hk) _ ≪≫ ?_
      exact (modPullbackCompIso (curveBaseChangeMap strX' u hk)
        (curveBaseChangeIsoOfIsPullback hX k).hom _).symm

/-- **`IsRelPicOf` TRANSPORTS ALONG AN ISOMORPHISM OF REPRESENTING OBJECTS OVER
THE BASE** (PROVEN) — the `h = 𝟙 S` instance of `ofIsPullback`, an isomorphism
being exactly a pullback along the identity (`IsPullback.of_horiz_isIso`).

`transport_of_isRelPicOf` above is the analogous statement for a
`MorphismProperty`; this one moves the STRUCTURE, which is what the last step
of the gluing needs. -/
noncomputable def ofIsoOver {Q : Scheme.{u}} {qstr : Q ⟶ S} (hP : IsRelPicOf strX pstr)
    (e : Q ≅ P) (he : e.hom ≫ pstr = qstr) : IsRelPicOf strX qstr :=
  ofIsPullback (h := 𝟙 S) (φ := 𝟙 X) (ψ := e.hom) hP IsPullback.of_id_fst
    (IsPullback.of_horiz_isIso ⟨by rw [he, Category.comp_id]⟩)

end IsRelPicOf

end IsRelPicOfBaseChange

/-! ### The gluing datum

The gluing of the local Picard schemes is a two-step affair, and only the
first step has any content left after `IsRelPicOf.ofIsPullback`:
...
A `Scheme.Cover.RelativeGluingData` over it is a functor
`F : S.affineOpens ⥤ Scheme` together with an EQUIFIBERED natural
transformation `F ⟶ (U ↦ U)`, i.e. a scheme `P_U ⟶ U` for each affine open and,
for each `U ≤ V`, a map `P_U ⟶ P_V` whose square over `U ↪ V` is CARTESIAN.
This leaf says the local representing objects can be so assembled.
...
* `IsRelPicOf.cmp_pre`: `cmp` is NATURAL in the test object.  This is what
  makes the structure maps compose, in exactly the way `toHom_comp_toHom`
  already exploits.

**THE ROUTE, and where its cost is.**  Choose `P_U`, `pstr_U`, `hP_U` from the
hypothesis.  For `U ≤ V` write `j := S.homOfLE hUV` (so `j ≫ V.ι = U.ι`, by
`Scheme.homOfLE_ι`), let
`bc_{U,V} : IsRelPicOf (curveBaseChangeProj strX U.ι) (pullback.snd pstr_V j)`
be `IsRelPicOf.ofIsPullback hP_V (isPullback_curveBaseChangeMap ..)
(IsPullback.of_hasPullback ..)`, and set

  `F.map hUV := (hP_U).toHom bc_{U,V} ≫ pullback.fst pstr_V j`.

Equifiberedness is then immediate — `toHom` is an ISO (`isIso_toHom`) and the
right factor is the projection of a cartesian square, so the composite square
is cartesian by `IsPullback.of_iso` / `paste`.  **The cost is FUNCTORIALITY**,
`F.map (𝟙 U) = 𝟙` and `F.map (f ≫ g) = F.map f ≫ F.map g`.  Both are instances
of `hP_W.inj` at the test object `P_U`: two morphisms `P_U ⟶ P_W` over `W` are
the same relative point once they classify the same sheaf.  What has to be
supplied is precisely that the sheaves agree, and that is `IsRelPicOf.sheaf_cmp`
plus a COCYCLE for the comparison isomorphisms of curves, namely

  `curveBaseChangeIsoOfIsPullback h_{U,V} k
...
hypotheses — `IsRelPicOf` is a bare functor-of-points condition, and every step
named above quantifies only over `IsRelPicOf` structures and mathlib gluing —
and that audit is now partly DISCHARGED rather than merely asserted: the whole
of `IsRelPicOf.ofIsPullback` was proved without properness, smoothness,
connectedness, a section or `f_*𝒪 = 𝒪`.

NOT VACUOUS in either direction: the hypothesis is supplied by
...
**RECUT 2026-07-31, direct-sorry count UNCHANGED (1 → 1).**  Steps (1) and (4)
of the route below, and the whole of step (2), are now Lean:

* step 2's two sublemmas are `IsRelPicOf.ofIsPullback` and
  `IsRelPicOf.ofIsoOver` above — PROVEN, unconditionally, and stated over an
  abstract cartesian square so that the caller never has to move between
  `pullback` and a pasted square;
* step 1's mathlib input is `S.directedAffineCover`, whose `LocallyDirected`
  instance mathlib supplies, so the "affine opens are a basis" argument the
  route sketches does not have to be run at all;
* step 4 — `RelativeGluingData.isPullback_natTrans_ι_toBase` against
...
theorem exists_gluedRelPic_of_forall_isAffineOpen {X S : Scheme.{u}} (strX : X ⟶ S)
    ...
    ∃ (P : Scheme.{u}) (pstr : P ⟶ S),
      ∀ V : S.Opens, IsAffineOpen V →
        Nonempty (IsRelPicOf (curveBaseChangeProj strX V.ι) (pstr ∣_ V)) := by
  obtain ⟨d, hd⟩ := exists_relativeGluingData_isRelPicOf strX hloc
  refine ⟨d.glued, d.toBase, fun V hV => ?_⟩
  obtain ⟨hP⟩ := hd ⟨V, hV⟩
  have h1 : IsPullback (d.toBase ∣_ V) (d.toBase ⁻¹ᵁ V).ι V.ι d.toBase :=
    isPullback_morphismRestrict d.toBase V
  have h2 : IsPullback (d.natTrans.app ⟨V, hV⟩) (colimit.ι d.functor ⟨V, hV⟩) V.ι d.toBase :=
...
The four declarations here do it once, and the leaf that remains is a statement
about `σ^♯` and about multiplication of ring sections: **no `Z.Modules`, no
kernel, no adjunction, no `modUnit`.**  (Since 2026-08-01 that leaf is one step
further down still — `exists_generator_sectionIdeal_at_section_affine`, the
whole thing on ONE affine open, with no sheaf in it either; the two sheaf-level
steps between are `nonzerodivisor_and_generates_of_locally` and
`nonzerodivisor_and_generates_basicOpen_of_affine`, both PROVEN below.)
...
/-- **THE TWO CLAUSES OF A GENERATOR ARE LOCAL** (PROVEN 2026-08-01) — if `s` is
a nonzerodivisor generating `ker σ^♯` on every member of a family of opens that
is a BASIS below `U`, then it is one on *every* open below `U`.

**No geometry at all**: `σ` is an arbitrary morphism, `U` an arbitrary open, `s`
an arbitrary section, and the only input is that `𝒪_Z` is a sheaf.  The
predicate `P` cutting out the basis is left abstract, so that a caller may
instantiate it at whichever family it can actually compute on; the consumer
below takes the BASIC OPENS of an affine `U`, where `Γ(Z, D(f))` is a
localization of `Γ(Z, U)` and both clauses are commutative algebra.

The two halves use the sheaf axiom in its two different directions:

* the NONZERODIVISOR clause is pure separatedness (`eq_of_locally_eq'`) — if
  `r · s|_W = 0` then `r` vanishes on each basis open below `W`, hence vanishes;
* the GENERATION clause is gluing (`existsUnique_gluing'`) — the local cofactors
  `rᵢ` agree on overlaps *because* `s` is a nonzerodivisor there, which is the
  first clause and is already proven for every open by the time it is needed, so
  they glue to one `r`, and `x = r · s|_W` follows by separatedness again.

This is the step the leaf's docstring below used to describe as "not extra
mathematics, given the affine case"; that is true, and it is 55 lines, and it is
now paid for once instead of by every successor. -/
theorem nonzerodivisor_and_generates_of_locally {Z T : Scheme.{u}} (σ : T ⟶ Z)
    (U : Z.Opens) (s : Γ(Z, U)) (P : Z.Opens → Prop)
    (hloc : ∀ (W : Z.Opens), W ≤ U → ∀ z ∈ W, ∃ V : Z.Opens, P V ∧ z ∈ V ∧ V ≤ W)
    (hnzdP : ∀ (V : Z.Opens) (hV : V ≤ U), P V → ∀ r : Γ(Z, V),
      r * Z.presheaf.map (homOfLE hV).op s = 0 → r = 0)
    (hgenP : ∀ (V : Z.Opens) (hV : V ≤ U), P V → ∀ x : Γ(Z, V),
      σ.app V x = 0 → ∃ r : Γ(Z, V), x = r * Z.presheaf.map (homOfLE hV).op s) :
    (∀ (W : Z.Opens) (hW : W ≤ U) (r : Γ(Z, W)),
        r * Z.presheaf.map (homOfLE hW).op s = 0 → r = 0) ∧
    (∀ (W : Z.Opens) (hW : W ≤ U) (x : Γ(Z, W)),
        σ.app W x = 0 → ∃ r : Γ(Z, W), x = r * Z.presheaf.map (homOfLE hW).op s) := by
  classical
  -- every open below `U` is covered by the `P`-opens below it
  have hcov : ∀ (W : Z.Opens) (_ : W ≤ U), ∃ V : ↥W → Z.Opens,
      (∀ z, P (V z)) ∧ (∀ z, V z ≤ W) ∧ W ≤ iSup V := by
    intro W hW
    choose V hPV hzV hVW using fun (z : ↥W) => hloc W hW (z : Z) z.2
    refine ⟨V, hPV, hVW, ?_⟩
    intro z hz
    exact Opens.mem_iSup.mpr ⟨⟨z, hz⟩, hzV ⟨z, hz⟩⟩
  -- the nonzerodivisor clause, on every open below `U`: separatedness alone
  have hnzd : ∀ (W : Z.Opens) (hW : W ≤ U) (r : Γ(Z, W)),
      r * Z.presheaf.map (homOfLE hW).op s = 0 → r = 0 := by
    intro W hW r hr
    obtain ⟨V, hPV, hVW, hcover⟩ := hcov W hW
    refine Z.sheaf.eq_of_locally_eq' V W (fun i => homOfLE (hVW i)) hcover r 0 (fun i => ?_)
    rw [map_zero]
    refine hnzdP (V i) ((hVW i).trans hW) (hPV i) _ ?_
    have h := congrArg (Z.presheaf.map (homOfLE (hVW i)).op) hr
    rw [map_mul, map_zero, ModDual.res_res] at h
    exact h
  refine ⟨hnzd, ?_⟩
  -- the generation clause: glue the local cofactors, which agree by `hnzd`
  intro W hW x hx
  obtain ⟨V, hPV, hVW, hcover⟩ := hcov W hW
  choose r hr using fun i => hgenP (V i) ((hVW i).trans hW) (hPV i)
    (Z.presheaf.map (homOfLE (hVW i)).op x) (app_eq_zero_of_le σ (hVW i) x hx)
  have hcompat : TopCat.Presheaf.IsCompatible Z.presheaf V r := by
    intro i j
    have key : ∀ (k : ↥W) (h : V i ⊓ V j ≤ V k),
        Z.presheaf.map (homOfLE h).op (r k) *
            Z.presheaf.map (homOfLE ((h.trans (hVW k)).trans hW)).op s =
          Z.presheaf.map (homOfLE (h.trans (hVW k))).op x := by
      intro k h
      rw [← ModDual.res_res h ((hVW k).trans hW) s, ← map_mul, ← hr k, ModDual.res_res]
    refine sub_eq_zero.mp (hnzd (V i ⊓ V j)
      ((inf_le_left.trans (hVW i)).trans hW) _ ?_)
    rw [sub_mul]
    have h1 := key i inf_le_left
    have h2 := key j inf_le_right
    rw [show (Opens.infLELeft (V i) (V j)) = homOfLE inf_le_left from rfl,
      show (Opens.infLERight (V i) (V j)) = homOfLE inf_le_right from rfl]
    rw [h1, h2, sub_self]
  obtain ⟨g, hg'⟩ : ∃ g : Γ(Z, W), ∀ i, Z.presheaf.map (homOfLE (hVW i)).op g = r i := by
    obtain ⟨g, hg, -⟩ :=
      Z.sheaf.existsUnique_gluing' V W (fun i => homOfLE (hVW i)) hcover r hcompat
    exact ⟨g, hg⟩
  refine ⟨g, ?_⟩
  refine Z.sheaf.eq_of_locally_eq' V W (fun i => homOfLE (hVW i)) hcover _ _ (fun i => ?_)
  show Z.presheaf.map (homOfLE (hVW i)).op x
      = Z.presheaf.map (homOfLE (hVW i)).op (g * Z.presheaf.map (homOfLE hW).op s)
  rw [map_mul, hg' i, ModDual.res_res]
  exact hr i

/-- **A PRESHEAF MAP ALONG AN `eqToHom` IS INJECTIVE ON SECTIONS** (PROVEN) —
`subst` and the functor's `map_id`.  Named because the comparison
`IsAffineOpen.app_basicOpen_eq_away_map` below factors `σ^♯` through exactly one
of these, and only its injectivity is wanted. -/
theorem injective_map_eqToHom {T : Scheme.{u}} {V W : T.Opens} (e : V = W) :
    Function.Injective (ConcreteCategory.hom (T.presheaf.map (eqToHom e).op)) := by
  subst e
  simp only [eqToHom_refl, op_id, CategoryTheory.Functor.map_id]
  exact fun a b h => h

/-- **A GENERATOR ON AN AFFINE OPEN IS ONE ON ALL ITS BASIC OPENS** (PROVEN
2026-08-01) — the second, purely algebraic, half of the reduction: from the
statement at `U` alone, where it is `s ∈ Γ(Y, U)⁰` and `ker σ^♯_U = (s)`, to the
statement at every `D(f) ≤ U`.  Again no geometry — no smoothness, no
properness, `σ` need not be a section; the two affineness hypotheses are all
that is used, and they are what make both `Γ(Y, D(f))` and
`Γ(T, σ ⁻¹ᵁ D(f))` localizations.

* NONZERODIVISOR.  `s/1` is a nonzerodivisor in `Γ(Y, U)_f`: if
  `(a/fⁿ) · (s/1) = 0` then some `f^m · a · s = 0` in `Γ(Y, U)`, so `f^m · a = 0`
  because `s` is a nonzerodivisor there, so `a/fⁿ = 0`.  Note that no injectivity
  of `Γ(Y, U) → Γ(Y, U)_f` is needed — which is the point, since there is none.
* KERNEL.  `IsAffineOpen.app_basicOpen_eq_away_map` factors `σ^♯` on `D(f)` as
  `IsLocalization.Away.map` of `σ^♯` on `U`, followed by the transport along
  `σ ⁻¹ᵁ Y.basicOpen f = T.basicOpen (σ^♯ f)` (`Scheme.preimage_basicOpen`),
  which is an `eqToHom` and so injective.  So `σ^♯(a/fⁿ) = 0` gives
  `σ^♯(f^k · a) = 0` for some `k`, hence `f^k · a = c · s`, hence
  `a/fⁿ = (c/f^{n+k}) · (s/1)` by `IsLocalization.mk'_cancel`. -/
theorem nonzerodivisor_and_generates_basicOpen_of_affine {Y T : Scheme.{u}} (σ : T ⟶ Y)
    (U : Y.Opens) (hU : IsAffineOpen U) (hUT : IsAffineOpen (σ ⁻¹ᵁ U)) (s : Γ(Y, U))
    (hnzd : s ∈ nonZeroDivisors Γ(Y, U))
    (hker : RingHom.ker (σ.app U).hom = Ideal.span {s}) :
    (∀ (f : Γ(Y, U)) (r : Γ(Y, Y.basicOpen f)),
      r * Y.presheaf.map (homOfLE (Y.basicOpen_le f)).op s = 0 → r = 0) ∧
    (∀ (f : Γ(Y, U)) (x : Γ(Y, Y.basicOpen f)),
      σ.app (Y.basicOpen f) x = 0 →
        ∃ r : Γ(Y, Y.basicOpen f),
          x = r * Y.presheaf.map (homOfLE (Y.basicOpen_le f)).op s) := by
  have hnzd' : ∀ a : Γ(Y, U), a * s = 0 → a = 0 := mem_nonZeroDivisors_iff_right.mp hnzd
  have hker' : ∀ a : Γ(Y, U), σ.app U a = 0 → ∃ c : Γ(Y, U), a = c * s := by
    intro a ha
    have hmem : a ∈ Ideal.span ({s} : Set Γ(Y, U)) := hker ▸ RingHom.mem_ker.mpr ha
    obtain ⟨c, hc⟩ := Ideal.mem_span_singleton'.mp hmem
    exact ⟨c, hc.symm⟩
  constructor
  · -- a nonzerodivisor stays one in every localization
    intro f r hr
    haveI := hU.isLocalization_basicOpen f
    obtain ⟨a, y, rfl⟩ := IsLocalization.exists_mk'_eq (Submonoid.powers f) r
    have hmul : IsLocalization.mk' Γ(Y, Y.basicOpen f) a y *
        algebraMap Γ(Y, U) Γ(Y, Y.basicOpen f) s = IsLocalization.mk' _ (a * s) y := by
      rw [← IsLocalization.mk'_one (M := Submonoid.powers f) (S := Γ(Y, Y.basicOpen f)) s,
        ← IsLocalization.mk'_mul, mul_one]
    rw [show Y.presheaf.map (homOfLE (Y.basicOpen_le f)).op s
        = algebraMap Γ(Y, U) Γ(Y, Y.basicOpen f) s from rfl, hmul,
      IsLocalization.mk'_eq_zero_iff] at hr
    obtain ⟨m, hm⟩ := hr
    refine IsLocalization.mk'_eq_zero_iff .. |>.mpr ⟨m, hnzd' _ ?_⟩
    rw [mul_assoc]; exact hm
  · -- the kernel of `σ^♯` localizes
    intro f x hx
    haveI := hU.isLocalization_basicOpen f
    haveI := hUT.isLocalization_basicOpen (σ.app U f)
    obtain ⟨a, y, rfl⟩ := IsLocalization.exists_mk'_eq (Submonoid.powers f) x
    rw [hU.app_basicOpen_eq_away_map σ hUT f, CommRingCat.comp_apply] at hx
    replace hx := injective_map_eqToHom _ (hx.trans (map_zero _).symm)
    have hx' : IsLocalization.Away.map (↑Γ(Y, Y.basicOpen f))
        (↑Γ(T, T.basicOpen (σ.app U f))) (σ.app U).hom f
        (IsLocalization.mk' (↑Γ(Y, Y.basicOpen f)) a y) = 0 := hx
    simp only [IsLocalization.Away.map, IsLocalization.map_mk',
      IsLocalization.mk'_eq_zero_iff] at hx'
    obtain ⟨m, hm⟩ := hx'
    obtain ⟨k, hk⟩ := m.2
    have hk' : (σ.app U) f ^ k = (m : Γ(T, σ ⁻¹ᵁ U)) := hk
    have hσk : σ.app U (f ^ k * a) = 0 := by
      rw [map_mul, map_pow, hk']
      exact hm
    obtain ⟨c, hc⟩ := hker' _ hσk
    obtain ⟨fk, hfk⟩ : ∃ fk : Submonoid.powers f, (fk : Γ(Y, U)) = f ^ k :=
      ⟨⟨f ^ k, pow_mem (Submonoid.mem_powers f) k⟩, rfl⟩
    refine ⟨IsLocalization.mk' Γ(Y, Y.basicOpen f) c (y * fk), ?_⟩
    rw [show Y.presheaf.map (homOfLE (Y.basicOpen_le f)).op s
        = algebraMap Γ(Y, U) Γ(Y, Y.basicOpen f) s from rfl,
      ← IsLocalization.mk'_one (M := Submonoid.powers f) (S := Γ(Y, Y.basicOpen f)) s,
      ← IsLocalization.mk'_mul, mul_one, ← hc, mul_comm ((f : Γ(Y, U)) ^ k) a, ← hfk]
    exact (IsLocalization.mk'_cancel (S := Γ(Y, Y.basicOpen f)) a y fk).symm

/-- **THE IDEAL OF A SECTION IS PRINCIPAL, GENERATED BY A NONZERODIVISOR, ON AN
AFFINE NEIGHBOURHOOD** (sorry leaf) — the geometric residue of
...
**RECUT 2026-08-01, IN TWO STEPS; THE DIRECT-SORRY COUNT DID NOT MOVE, 1 → 1,
AND THE NAME CHANGED TWICE.**  Until today the leaf was
`exists_generator_sectionIdeal_at_section`, whose two clauses were quantified
over EVERY open `W ≤ U`.  Both intermediate names are now PROVEN assemblies
below, and what is left here is the statement the LITERATURE proves — the whole
thing on ONE affine open, with no sheaf in it at all:

    ∃ U, IsAffineOpen U ∧ σ.base t ∈ U ∧
      ∃ s : Γ(Y, U), s ∈ nonZeroDivisors Γ(Y, U) ∧
        RingHom.ker (σ.app U).hom = Ideal.span {s}

The two steps out of it, both PROVEN above and neither containing any geometry:

1. `nonzerodivisor_and_generates_basicOpen_of_affine` — from `U` to its BASIC
   OPENS.  `Γ(Y, D(f))` is the localization `Γ(Y, U)_f`
   (`IsAffineOpen.isLocalization_basicOpen`) and `Γ(T, σ ⁻¹ᵁ D(f))` is
   `Γ(T, σ ⁻¹ᵁ U)` localized at `σ^♯f` (`Scheme.preimage_basicOpen` plus
   `IsAffineOpen.app_basicOpen_eq_away_map`), so this is "a nonzerodivisor stays
   one in a localization" and "the kernel of `σ^♯` localizes".
2. `nonzerodivisor_and_generates_of_locally` — from the basic opens, which are a
   basis of `U` (`IsAffineOpen.exists_basicOpen_le`), to every open below `U`.
   Separatedness for the first clause, gluing for the second.

**`IsAffineOpen (σ ⁻¹ᵁ U)` IS NOT A HYPOTHESIS OF THE LEAF, AND THAT IS WHERE
`_hproper` GOES.**  Step 1 needs it, and it is FREE: `_hproper` and `_hσ` make
`σ` a closed immersion (`isClosedImmersion_of_isSection`), a closed immersion is
affine, and the preimage of an affine open under an affine morphism is affine
(`IsAffineOpen.preimage`).  So the assembly below derives it and the leaf never
mentions it.  **This CORRECTS the third bullet of the audit below**, which says
`_hproper` is not known to be needed for this half and that a prover may ignore
it: on the route now taken it is used here too.  The bullet's mathematical claim
— that the doubled-origin witness attacks only the off-image half — is
unaffected and is left standing; what is corrected is the advice.

Likewise `IsAffineOpen U` is an extra thing the leaf must PRODUCE, not an extra
hypothesis, and it costs the prover nothing: `SmoothOfRelativeDimension` hands
over an affine open in the first place.

**The 2026-07-31 recut, kept because it is what made the above possible.**  Until
that day the leaf here was the ISOMORPHISM statement
...
**WHAT A PROVER OWES.**  Exactly Stacks 0C4S on one affine chart: choose the
affine `U ∋ σ(t)` on which `strY` is standard smooth of relative dimension `1`
(`SmoothOfRelativeDimension.exists_isStandardSmoothOfRelativeDimension`, shrunk
so that it meets the section), write `A = Γ(Y, U)` and `φ = σ^♯_U : A → Γ(T, σ ⁻¹ᵁ U)`
for the retraction, and produce one `s ∈ A` with `ker φ = (s)` and `s` a
nonzerodivisor.  The classical reason: `ker φ / (ker φ)² ≅ Ω_{A/Γ(T,·)} ⊗ A` is
free of rank `1`, so `ker φ` is generated by one element after shrinking, and
that element is a nonzerodivisor because `A` is flat over the base with regular
one-dimensional fibres.  No sheaf, no open other than `U`, and nothing about
points off the section.
...
**FAITHFULNESS.  BOTH recuts leave the audit INTACT, and in both cases the
reason is that the statements are EQUIVALENT rather than merely sufficient.**

* 2026-07-31: the generator statement is equivalent to the isomorphism statement
  it replaced — both directions are recorded in the subsection header above.
* 2026-08-01: this affine statement is equivalent to the all-opens one.  `⇒` is
  the two assemblies below.  `⇐` is: shrink the all-opens `U` to an affine
  `U' ≤ U` containing `σ.base t` (`Scheme.isBasis_affineOpens`) and take
  `s' := s|_{U'}`; the all-opens clauses at `W = U'` say exactly that `s'` is a
  nonzerodivisor in `Γ(Y, U')` and that `ker σ^♯_{U'} ⊆ (s')`, and the reverse
  inclusion is `hmem` plus `Ideal.span_le`.  It is not formalized because
  nothing needs it.

Since each recut is an equivalence, every witness refuting one refutes the other
in both directions, so what follows is the audit written for the 2026-07-31
reshaping, carried over — plus the correction made to it while re-reading it.
...
theorem exists_generator_sectionIdeal_at_section_affine {Y T : Scheme.{u}} (strY : Y ⟶ T)
    (_hproper : IsProper strY) (_hsmooth : SmoothOfRelativeDimension 1 strY)
    {σ : T ⟶ Y} (_hσ : σ ≫ strY = 𝟙 T) (t : T) :
    ∃ (U : Y.Opens) (_ : IsAffineOpen U) (_ : σ.base t ∈ U) (s : Γ(Y, U)),
      s ∈ nonZeroDivisors Γ(Y, U) ∧ RingHom.ker (σ.app U).hom = Ideal.span {s} := sorry

/-- **THE SAME, ON THE BASIC OPENS OF THAT AFFINE** (PROVEN 2026-08-01 over
`exists_generator_sectionIdeal_at_section_affine` and
`nonzerodivisor_and_generates_basicOpen_of_affine`).

The affineness of `σ ⁻¹ᵁ U`, which the bridge needs, is derived here rather than
asked of the leaf: `_hproper` and `_hσ` make `σ` a closed immersion, hence an
affine morphism, hence `σ ⁻¹ᵁ U` is affine.  `σ.app U s = 0` likewise comes for
free, from `s ∈ (s) = ker σ^♯_U`. -/
theorem exists_generator_sectionIdeal_at_section_basicOpen {Y T : Scheme.{u}} (strY : Y ⟶ T)
    ...
    ∃ (U : Y.Opens) (_ : IsAffineOpen U) (_ : σ.base t ∈ U) (s : Γ(Y, U)),
      ...
      (∀ (f : Γ(Y, U)) (x : Γ(Y, Y.basicOpen f)),
        σ.app (Y.basicOpen f) x = 0 →
          ∃ r : Γ(Y, Y.basicOpen f),
            x = r * Y.presheaf.map (homOfLE (Y.basicOpen_le f)).op s) := by
  obtain ⟨U, hU, hmemU, s, hnzd, hker⟩ :=
    exists_generator_sectionIdeal_at_section_affine strY hproper hsmooth hσ t
  haveI : IsClosedImmersion σ := isClosedImmersion_of_isSection strY hproper hσ
  have hUT : IsAffineOpen (σ ⁻¹ᵁ U) := hU.preimage σ
  refine ⟨U, hU, hmemU, s, ?_,
    nonzerodivisor_and_generates_basicOpen_of_affine σ U hU hUT s hnzd hker⟩
  have hmem : s ∈ RingHom.ker (σ.app U).hom := hker ▸ Ideal.mem_span_singleton_self s
  exact RingHom.mem_ker.mp hmem

/-- **THE IDEAL OF A SECTION IS LOCALLY GENERATED BY A NONZERODIVISOR** (PROVEN
2026-08-01 over `exists_generator_sectionIdeal_at_section_basicOpen` and
`nonzerodivisor_and_generates_of_locally`; a bare sorry leaf from 2026-07-31 to
2026-08-01, and the long audit above is the one written while it was one — every
word of it still applies, now to the basic-open leaf it is proven over).

This is the shape the consumer wants — the clauses at EVERY open below `U` —
and, as the audit above records, getting there from the basic opens is a sheaf
argument with no geometry in it.  The basis is supplied by
`IsAffineOpen.exists_basicOpen_le`: inside an affine `U`, every open `W` and
every point `z ∈ W` admit a basic open `D(f)` with `z ∈ D(f) ≤ W`. -/
theorem exists_generator_sectionIdeal_at_section {Y T : Scheme.{u}} (strY : Y ⟶ T)
    ...
    ∃ (U : Y.Opens) (_ : σ.base t ∈ U) (s : Γ(Y, U)),
      ...
      (∀ (W : Y.Opens) (hW : W ≤ U) (x : Γ(Y, W)),
        σ.app W x = 0 → ∃ r : Γ(Y, W), x = r * Y.presheaf.map (homOfLE hW).op s) := by
  obtain ⟨U, hUaff, hU, s, hmem, hnzdB, hgenB⟩ :=
    exists_generator_sectionIdeal_at_section_basicOpen strY hproper hsmooth hσ t
  refine ⟨U, hU, s, hmem, ?_⟩
  refine nonzerodivisor_and_generates_of_locally σ U s
    (fun V => ∃ f : Γ(Y, U), V = Y.basicOpen f) ?_ ?_ ?_
  · intro W hW z hz
    obtain ⟨f, hfW, hzf⟩ := hUaff.exists_basicOpen_le (⟨z, hz⟩ : W) (hW hz)
    exact ⟨Y.basicOpen f, ⟨f, rfl⟩, hzf, hfW⟩
  · rintro V hV ⟨f, rfl⟩ r hr
    exact hnzdB f r hr
  · rintro V hV ⟨f, rfl⟩ x hx
    exact hgenB f x hx
...
/-! #### THE FLATNESS INPUT IS NOT NEEDED: INVERTIBILITY OF BOTH SIDES REPLACES IT

(2026-07-31, and this REVERSES this section's own hypothesis audit on one point.)
Until today `isIso_modPullbackSectionIdealMap` was a single leaf whose route ran
through flat base change, and its audit said of `_hproper` and `_hsmooth` that
they are *"not used by the affine argument at all … a prover may ignore both"*.

**They are what makes the flatness input unnecessary.**  Both `_hproper` and
`_hsmooth` base-change along `_hsq`, so `strY'` is again proper and smooth of
relative dimension `1`, and `isInvertibleSheaf_sectionIdeal_of_isSection` then
applies to `(strY', σ')` as well as to `(strY, σ)`.  So

* `modPullback φ (sectionIdeal σ)` is invertible (`isInvertibleSheaf_modPullback`), and
* `sectionIdeal σ'` is invertible (`isInvertibleSheaf_sectionIdeal_baseChange` below),

and a map between invertible sheaves is an isomorphism **as soon as it is an
epimorphism** — locally it is multiplication by a section of the structure sheaf,
and a surjective such multiplication is by a unit.  The injectivity half, which
is the whole of the `Tor`-vanishing / flat-base-change content, comes for free.

So the node now rests on two leaves, neither of which is about flatness:

1. `isIso_of_epi_of_isInvertibleSheaf` — pure sheaf theory, no geometry, no base
   change, and classical (a surjection of invertible sheaves is an isomorphism);
2. `epi_modPullbackSectionIdealMap` — right-exactness of `φ^*` plus base change
   of the closed immersion `σ`, i.e. that the ideal `I_σ` extended to `𝒪_{Y'}`
   IS `I_{σ'}`.  This is STRICTLY WEAKER than the leaf it replaces (`IsIso`
   implies `Epi`), so that leaf's faithfulness audit is INHERITED rather than
   void, and it cannot be false unless the old leaf was.

**Direct-sorry accounting, stated honestly: 1 leaf becomes 2, so the count goes
UP by one.**  What is bought is that the flat-base-change development
`Mathlib/AlgebraicGeometry/Modules/` does not have — the one the old audit
correctly identified as absent from the pin — is off the critical path
altogether, and that the two residues are independent and can be given to two
owners.  Judge it by what is LEFT in each leaf, not by the delta. -/

/-- **THE SECTION SQUARE OF A CARTESIAN SQUARE IS CARTESIAN** (PROVEN
2026-07-31) — if `Y' = Y ×_T T'` and `σ`, `σ'` are compatible sections, then
`T' = T ×_Y Y'`; that is, the divisor `D_{σ'}` is the scheme-theoretic preimage
of `D_σ`.

It is pure category theory — no schemes, no geometry, and no hypothesis beyond
the four identities — so it is stated for an arbitrary category.  The proof is
one pasting: the outer rectangle

    T' --σ'--> Y' --strY'--> T'
    |h         |φ            |h
    T  --σ---> Y  --strY--> T

has `σ' ≫ strY' = 𝟙` on top and `σ ≫ strY = 𝟙` on the bottom, so it is
cartesian by `IsPullback.of_id_fst`; the right-hand square is `_hsq` flipped;
and `IsPullback.of_right` peels off the left-hand square.

**This SHARPENS the falsity audit recorded on
`epi_modPullbackSectionIdealMap` below rather than weakening it.**  That audit
refutes the statement obtained by asking only that the SECTION square be
cartesian.  Since that condition is a CONSEQUENCE of `_hsq` (this lemma), it is
strictly weaker, and the witness recorded there shows the implication is strict.
-/
theorem isPullback_section_square {C : Type*} [Category C] {Y T Y' T' : C}
    {strY : Y ⟶ T} {strY' : Y' ⟶ T'} {φ : Y' ⟶ Y} {h : T' ⟶ T}
    (hsq : IsPullback φ strY' strY h)
    {σ : T ⟶ Y} (hσ : σ ≫ strY = 𝟙 T) {σ' : T' ⟶ Y'} (hσ' : σ' ≫ strY' = 𝟙 T')
    (hcompat : σ' ≫ φ = h ≫ σ) :
    IsPullback σ' h φ σ := by
  refine IsPullback.of_right ?_ hcompat hsq.flip
  rw [hσ, hσ']
  exact IsPullback.of_id_fst

/-- **THE IDEAL SHEAF OF THE BASE-CHANGED SECTION IS INVERTIBLE** (PROVEN
2026-07-31) — `_hproper` and `_hsmooth` are stable under base change, so the
already-proven `isInvertibleSheaf_sectionIdeal_of_isSection` applies to
`(strY', σ')` and not only to `(strY, σ)`.

`IsProper` carries a `MorphismProperty.IsStableUnderBaseChange` instance in the
pin; `SmoothOfRelativeDimension n` carries the lemma
`smoothOfRelativeDimension_isStableUnderBaseChange`, which is NOT an instance and
has to be introduced by hand. -/
theorem isInvertibleSheaf_sectionIdeal_baseChange {Y T Y' T' : Scheme.{u}}
    {strY : Y ⟶ T} {strY' : Y' ⟶ T'} {φ : Y' ⟶ Y} {h : T' ⟶ T}
    (hsq : IsPullback φ strY' strY h)
    (hproper : IsProper strY) (hsmooth : SmoothOfRelativeDimension 1 strY)
    {σ' : T' ⟶ Y'} (hσ' : σ' ≫ strY' = 𝟙 T') :
    IsInvertibleSheaf (sectionIdeal σ') :=
  haveI := smoothOfRelativeDimension_isStableUnderBaseChange (1 : ℕ)
  isInvertibleSheaf_sectionIdeal_of_isSection strY'
...
map: it is an epi and not an iso.  Drop `hA` and take `A = 𝒪 ⊕ 𝒪`, `B = 𝒪`, `f`
the first projection.  **NOT VACUOUS**: `f = 𝟙` on `modUnit Z` satisfies every
...
/-! #### THE SURJECTIVITY HALF, REDUCED TO A STATEMENT ABOUT RING SECTIONS

**RECUT 2026-08-01; THE DIRECT-SORRY COUNT DID NOT MOVE, 1 → 1.**  Until today
`epi_modPullbackSectionIdealMap` below was a bare `sorry` whose conclusion is
`Epi` of a morphism in `Y'.Modules`.  Nothing in that conclusion is geometry, and
all of it had to be rebuilt by every successor: a local-to-global criterion for
`Epi` in a category of sheaves of modules, the adjunction bookkeeping that turns
a section of `I_σ` into a section of `φ^* I_σ`, and the two kernel facts that
identify `Γ(sectionIdeal σ, W)` with `ker (σ^♯_W)` in both directions.  The eight
declarations here do that once, and the leaf that remains
(`exists_pullbackGenerator_sectionIdeal`) is a statement about `σ^♯`, `φ^♯` and
multiplication of ring sections: **no `Y'.Modules`, no kernel, no adjunction, no
`modPullback`.**

This is the same recut the subsection `FROM A GENERATOR TO A TRIVIALIZATION`
above performed on `exists_trivialization_sectionIdeal_at_section`, and the
residual leaf is deliberately given the same SHAPE as that subsection's residue
`exists_generator_sectionIdeal_at_section` — an open, one section generating the
ideal on every smaller open — so that the two can be read together.

**WHY A SINGLE GENERATOR IS ENOUGH, and it is what makes the residue small.**
`I_σ` is invertible (`isInvertibleSheaf_sectionIdeal_of_isSection`), hence
locally principal, so the ideal upstairs is generated by the pullback of ONE
element and the formal half never has to sum.  The price is that `_hsmooth` is
load-bearing for the residual leaf, where it was NOT load-bearing for the route
the old docstring recorded; that is a real change and it is recorded on the leaf.

**WHAT THE FORMAL HALF DOES NOT NEED, and this is the point of the cut.**  It
never computes a section of `φ^* I_σ`, never needs `φ^*` to commute with
anything, and never mentions quasi-coherence.  It only needs that the adjunction
unit turns `b ∈ Γ(I_σ, W)` into SOME section of `φ^* I_σ` over `φ ⁻¹ᵁ W` whose
image in `𝒪_{Y'}` is `φ^♯ b` — which is `app_pullback_comp_unit` below, three
lines of adjunction calculus — and that the image is closed under multiplication
by `Γ(Y', V')`, which is `Scheme.Modules.Hom.app_smul`.  The worry recorded on
the old leaf, that `Scheme.Modules` is ALL `𝒪`-modules and `φ^*` involves a
sheafification the affine model does not see, is therefore never met: no
hypothesis had to be added. -/

/-- **AN EPIMORPHISM OF `𝒪`-MODULES IS DETECTED BY LOCAL SURJECTIVITY ON
SECTIONS** (PROVEN) — if every section of the target is, near every point of its
open, the image of a section of the source, then the map is an epimorphism.

The proof is `Preadditive.epi_iff_cancel_zero` plus separatedness of the sheaf
`C` (`Presheaf.IsSheaf.section_ext`): a `g` killed by `f` kills every section
locally, hence kills it.  Note this is a SUFFICIENT condition only, which is all
any consumer needs; the converse is local surjectivity in the sense of the site
and is not required here.

Stated for an arbitrary morphism because nothing in the proof knows about
`sectionIdeal`.  It belongs beside `isIso_of_locally_isIso` some 5000 lines
above — the two are the `Epi` and `IsIso` halves of one idea — and is declared
here only to keep this recut to a single contiguous block in a file with several
concurrent editors. -/
theorem epi_of_locally_surjective_app {Z : Scheme.{u}} {A B : Z.Modules} (f : A ⟶ B)
    (h : ∀ (V : Z.Opens) (x : Γ(B, V)) (z : Z), z ∈ V →
      ∃ V' : Z.Opens, ∃ hV' : V' ≤ V, z ∈ V' ∧
        ∃ y : Γ(A, V'), Scheme.Modules.Hom.app f V' y
          = B.presheaf.map (homOfLE hV').op x) :
    Epi f := by
  ...
  ext x
  refine C.isSheaf.section_ext (U := op V) ?_
  intro z hz
  obtain ⟨V', hV', hzV', y, hy⟩ := h V x z hz
  refine ⟨V', hV', hzV', ?_⟩
  have hnat := NatTrans.naturality_apply (Scheme.Modules.Hom.mapPresheaf g) (homOfLE hV').op x
  simp only [Scheme.Modules.mapPresheaf_app] at hnat
  rw [← hnat, ← hy]
  have h0 : Scheme.Modules.Hom.app (f ≫ g) V' y = 0 := by rw [hg]; rfl
  rw [Scheme.Modules.Hom.comp_app] at h0
  rw [show (ConcreteCategory.hom (Scheme.Modules.Hom.app g V'))
      (ConcreteCategory.hom (Scheme.Modules.Hom.app f V') y) = 0 from h0]
  simp

/-- **A MORPHISM OF `𝒪`-MODULES COMMUTES WITH RESTRICTION, ON SECTIONS**
(PROVEN) — naturality of the underlying map of `Ab`-presheaves, in the one shape
the argument below uses.  The `Scheme.Hom.app` counterpart is
`app_eq_zero_of_le` above. -/
theorem app_map_restrict {Z : Scheme.{u}} {A B : Z.Modules} (f : A ⟶ B) {V V' : Z.Opens}
    (hV : V' ≤ V) (y : Γ(A, V)) :
    Scheme.Modules.Hom.app f V' (A.presheaf.map (homOfLE hV).op y)
      = B.presheaf.map (homOfLE hV).op (Scheme.Modules.Hom.app f V y) :=
  NatTrans.naturality_apply (Scheme.Modules.Hom.mapPresheaf f) (homOfLE hV).op y

/-- **A SECTION KILLED BY `f` LIFTS TO THE KERNEL** (PROVEN) — the surjectivity
half of `Γ(ker f, W) = ker (Γ(f, W))`, over `modSectionsAt Z W` preserving
kernels and `AddCommGrpCat.kernelIsoKer`.

`kernel_ι_app_mem` below is the other half; together they are what lets the
residual leaf be stated about `σ^♯` rather than about `sectionIdeal`. -/
theorem exists_kernel_section {Z : Scheme.{u}} {A B : Z.Modules} (f : A ⟶ B) (W : Z.Opens)
    (x : Γ(A, W)) (hx : Scheme.Modules.Hom.app f W x = 0) :
    ∃ y : Γ(kernel f, W), Scheme.Modules.Hom.app (kernel.ι f) W y = x := by
  have hx' : ConcreteCategory.hom ((modSectionsAt Z W).map f) x = 0 := hx
  have hcomp : (AddCommGrpCat.kernelIsoKer ((modSectionsAt Z W).map f)).inv ≫
      ((PreservesKernel.iso (modSectionsAt Z W) f).inv ≫
        (modSectionsAt Z W).map (kernel.ι f))
      = AddCommGrpCat.ofHom (AddSubgroup.subtype ((modSectionsAt Z W).map f).hom.ker) := by
    rw [PreservesKernel.iso_inv_ι, AddCommGrpCat.kernelIsoKer_inv_comp_ι]
  refine ⟨ConcreteCategory.hom (PreservesKernel.iso (modSectionsAt Z W) f).inv
      (ConcreteCategory.hom (AddCommGrpCat.kernelIsoKer ((modSectionsAt Z W).map f)).inv
        ⟨x, hx'⟩), ?_⟩
  have h2 := congrArg (fun m : AddCommGrpCat.of (((modSectionsAt Z W).map f).hom.ker) ⟶
      (modSectionsAt Z W).obj A =>
    ConcreteCategory.hom m (⟨x, hx'⟩ : ((modSectionsAt Z W).map f).hom.ker)) hcomp
  rw [CategoryTheory.comp_apply, CategoryTheory.comp_apply] at h2
  exact h2

/-- **`kernel.ι` IS INJECTIVE ON SECTIONS** (PROVEN) — the same three lines as
the `hIinj` step inside `nonempty_sectionIdeal_restrict_iso_of_generator` above,
pulled out because the argument below needs it at a different kernel. -/
theorem kernel_ι_app_injective {Z : Scheme.{u}} {A B : Z.Modules} (f : A ⟶ B) (V : Z.Opens) :
    Function.Injective (ConcreteCategory.hom (Scheme.Modules.Hom.app (kernel.ι f) V)) := by
  have hmap : (modSectionsAt Z V).map (kernel.ι f)
      = (PreservesKernel.iso (modSectionsAt Z V) f).hom ≫
        kernel.ι ((modSectionsAt Z V).map f) := by
    rw [← PreservesKernel.iso_inv_ι]
    simp
  haveI hmono : Mono (Scheme.Modules.Hom.app (kernel.ι f) V) := by
    show Mono ((modSectionsAt Z V).map (kernel.ι f))
    rw [hmap]; infer_instance
  exact ConcreteCategory.injective_of_mono_of_preservesPullback _

/-- **A SECTION OF THE KERNEL IS KILLED BY `f`** (PROVEN) — `kernel.condition`
read on sections. -/
theorem kernel_ι_app_mem {Z : Scheme.{u}} {A B : Z.Modules} (f : A ⟶ B) (V : Z.Opens)
    (x : Γ(kernel f, V)) :
    Scheme.Modules.Hom.app f V (Scheme.Modules.Hom.app (kernel.ι f) V x) = 0 := by
  have hc := congrArg (fun (m : (kernel f : Z.Modules) ⟶ B) =>
    ConcreteCategory.hom (Scheme.Modules.Hom.app m V) x) (kernel.condition f)
  exact hc.trans rfl

/-- **`modPullbackSectionIdealMap` FOLLOWED BY `kernel.ι` IS THE PULLED-BACK
IDEAL INCLUSION, ON SECTIONS** (PROVEN) — `kernel.lift_ι`, transported to
sections by `congrArg`.

`rw` cannot be used to transport it: `modPullback φ (sectionIdeal σ)` is a `def`
for `(Scheme.Modules.pullback φ).obj (sectionIdeal σ)` and the two are not
syntactically equal at `instances` transparency, so a rewrite reports a pattern
that prints exactly like the target.  `congrArg` at an explicitly ascribed
morphism type crosses it. -/
theorem app_modPullbackSectionIdealMap_kernel_ι {Y T Y' T' : Scheme.{u}} {φ : Y' ⟶ Y}
    {h : T' ⟶ T} {σ : T ⟶ Y} {σ' : T' ⟶ Y'} (hcompat : σ' ≫ φ = h ≫ σ) (V : Y'.Opens)
    (y : Γ(modPullback φ (sectionIdeal σ), V)) :
    Scheme.Modules.Hom.app (kernel.ι (sectionIdealMap σ')) V
        (Scheme.Modules.Hom.app (modPullbackSectionIdealMap hcompat) V y)
      = Scheme.Modules.Hom.app ((Scheme.Modules.pullback φ).map
          (kernel.ι (sectionIdealMap σ)) ≫ (modPullbackUnitIso φ).hom) V y := by
  have hlift : modPullbackSectionIdealMap hcompat ≫ kernel.ι (sectionIdealMap σ')
      = (Scheme.Modules.pullback φ).map (kernel.ι (sectionIdealMap σ)) ≫
        (modPullbackUnitIso φ).hom := kernel.lift_ι _ _ _
  exact congrArg (fun (m : modPullback φ (sectionIdeal σ) ⟶ modUnit Y') =>
    ConcreteCategory.hom (Scheme.Modules.Hom.app m V) y) hlift

/-- **THE ADJOINT TRANSPOSE OF `φ^*k ≫ (φ^*𝒪 ≅ 𝒪)` IS `k ≫ φ^♯`** (PROVEN) —
`Adjunction.homEquiv_naturality_left` over
`SheafOfModules.pullbackPushforwardAdjunction_homEquiv_pullbackObjUnitToUnit`,
which is the same identification `sectionIdealMap_app_eq_zero_iff` above runs
at `σ`. -/
theorem homEquiv_pullback_comp {Y Y' : Scheme.{u}} (φ : Y' ⟶ Y) {A : Y.Modules}
    (k : A ⟶ modUnit Y) :
    (Scheme.Modules.pullbackPushforwardAdjunction φ).homEquiv A (modUnit Y')
        ((Scheme.Modules.pullback φ).map k ≫ (modPullbackUnitIso φ).hom)
      = k ≫ SheafOfModules.unitToPushforwardObjUnit φ.toRingCatSheafHom := by
  rw [Adjunction.homEquiv_naturality_left,
    ← SheafOfModules.pullbackPushforwardAdjunction_homEquiv_pullbackObjUnitToUnit]
  rfl

/-- **PULLING BACK A SECTION: ON `Γ(A, W)`, THE ADJUNCTION UNIT FOLLOWED BY
`φ^*k ≫ (φ^*𝒪 ≅ 𝒪)` IS `φ^♯ ∘ k`** (PROVEN).

This is the one fact the surjectivity argument needs about `φ^*`, and it is why
that argument never has to describe a section of `φ^* A`: the unit produces one
over `φ ⁻¹ᵁ W` out of a section of `A` over `W`, and its image downstairs is the
honest `φ^♯` of the image of the original.  `Γ((pushforward φ).obj N, W)` is
`Γ(N, φ ⁻¹ᵁ W)` by `rfl` (`Scheme.Modules.pushforward_obj_obj`), which is what
makes the two sides typecheck against each other. -/
theorem app_pullback_comp_unit {Y Y' : Scheme.{u}} (φ : Y' ⟶ Y) {A : Y.Modules}
    (k : A ⟶ modUnit Y) (W : Y.Opens) (b : Γ(A, W)) :
    Scheme.Modules.Hom.app
        ((Scheme.Modules.pullback φ).map k ≫ (modPullbackUnitIso φ).hom) (φ ⁻¹ᵁ W)
        (Scheme.Modules.Hom.app
          ((Scheme.Modules.pullbackPushforwardAdjunction φ).unit.app A) W b)
      = φ.app W (Scheme.Modules.Hom.app k W b) := by
  have h := homEquiv_pullback_comp φ k
  rw [Adjunction.homEquiv_unit] at h
  have h3 := congrArg
    (fun m : A ⟶ (Scheme.Modules.pushforward φ).obj (modUnit Y') =>
      ConcreteCategory.hom (Scheme.Modules.Hom.app m W) b) h
  simp only [Scheme.Modules.Hom.comp_app, CategoryTheory.comp_apply] at h3
  exact h3.trans (SheafOfModules.unitToPushforwardObjUnit_val_app_apply _ _)

/-- **THE PULLED-BACK GENERATOR GENERATES THE IDEAL OF THE BASE-CHANGED SECTION**
(sorry leaf, cut 2026-08-01 out of `epi_modPullbackSectionIdealMap` below, which
is now PROVEN over it and is its only consumer) — Stacks 062Y / 0631, EGA IV
21.15, and it is all that is left of the SURJECTIVITY half.

In words: near every point of `Y'` the ideal `ker (σ'^♯)` is generated by a
single element of the form `φ^♯ b`, with `b` a section of `ker (σ^♯)` downstairs.
That is exactly *the ideal `I_σ` extended to `𝒪_{Y'}` along `φ` IS `I_{σ'}`*, and
it mentions no `Y'.Modules`, no kernel, no adjunction and no `modPullback`: the
whole sheaf-theoretic half is discharged by the subsection header above.

**HOW TO PROVE IT — two cases, and only one of them is base change.**

* *Off the image of `σ'`.*  `σ'` is a closed immersion (base-change of `σ` along
  `_hsec`, or `isClosedImmersion_of_isSection` at `strY'`), so `(range σ'.base)ᶜ`
  is open; over any `V'` inside it `Γ(T', σ' ⁻¹ᵁ V')` is the zero ring, so
  `σ'.app V' 1 = 0` and every `x` is `x * 1`.  Take `W := ⊤`, `b := 1`.  No base
  change is used and `_hsq` is not spent.  (The `Ab`-valued twin of the "zero
  ring" step is `isZero_sections_pushforward_of_notMem` above, and
  `sectionIdeal_restrict_iso_unit_of_notMem_range` is the same case handled for
  the trivialization statement.)
* *On the image.*  Get a generator downstairs from
  `exists_generator_sectionIdeal_at_section strY _hproper _hsmooth _hσ t` at the
  `t` with `σ.base t = φ.base z`: it supplies `U ∋ σ.base t`, `s ∈ Γ(Y, U)`,
  `σ.app U s = 0`, `s` a nonzerodivisor below `U`, and `ker (σ^♯_W) = s · Γ(Y, W)`
  for every `W ≤ U`.  Take `W := U`, `b := s`, `V₀ := φ ⁻¹ᵁ U`.  What remains is
  the base-change statement `ker (σ'^♯) = (φ^♯ s)` on `φ ⁻¹ᵁ U`, and it is where
  `_hsec` is spent: affine-locally `_hsec` says `A' = A ⊗_B B'` with `A = B/I`,
  so `ker (B' ⟶ A') = I·B' = (φ^♯ s)`.

**THIS IS NOT A DUPLICATE OF `exists_generator_sectionIdeal_at_section`**, and
the check is the second bullet: that leaf is consumed by this one and does not
imply it.  Knowing that both `I_σ` and `I_{σ'}` are locally principal says
nothing about the two generators being associates, and that — not principality —
is the content here.

**WHY THE QUANTIFICATION IS OVER EVERY `V' ≤ V₀` rather than over a basis**, in
the same words as the sibling leaf: it is not extra mathematics.  `φ^♯ b`
generates an invertible ideal, hence is a nonzerodivisor, hence the local
cofactors agree on overlaps and glue, and `x = a · φ^♯ b` follows by separatedness
of `𝒪_{Y'}`.  A successor who would rather cut that gluing out should state it
over a basis and add the gluing as its own lemma; nothing above depends on which
form is used.

**FAITHFULNESS.**  This statement is IMPLIED BY the `IsIso` leaf this section
replaced — it is the surjectivity of that isomorphism, read on sections — so
every counterexample to it refutes that leaf too, and the two witnesses recorded
on `epi_modPullbackSectionIdealMap` below transfer unchanged.  In particular the
`_hsq` witness (`Y = 𝔸¹_k`, `σ` the origin, `Y' = Spec k[t]/(t)`, `σ' = 𝟙`) kills
this statement as well: there `ker (σ'^♯) = 0` is generated by `φ^♯(t) = 0`, so
the leaf HOLDS at that witness — which is the same reading the old docstring
records, namely that the witness is caught by the invertibility half and not by
this one.  `_hσ` is load-bearing for the same reason it was there
(`isClosedImmersion_of_isSection` and the invertibility of `I_σ`).

**`_hsmooth` IS NOW LOAD-BEARING, and that is a change.**  The old docstring
recorded that `_hsmooth` "is not used by the surjectivity argument".  That is
true of an argument phrased with an arbitrary finite family of generators; it is
false here, because the single-generator form of the residue needs `I_σ` to be
locally principal, which is `isInvertibleSheaf_sectionIdeal_of_isSection` and
spends smoothness.  A successor who prefers the family form may weaken the
conclusion to a finite sum, at the cost of a `Finset.sum` in the formal half
above; nothing else changes.

**THE REMAINING HYPOTHESES.**  `_hsq`, `_hσ'` and `_hcompat` are retained
because the single call site holds all three for free and because a hypothesis
that cannot make a leaf false costs a prover nothing.  `_hsec` is the form the
route consumes and is derived at the call site by `isPullback_section_square`.

**NOT VACUOUS.**  Instantiated immediately below at every `z : Y'`, and already
at `h = 𝟙 T`. -/
theorem exists_pullbackGenerator_sectionIdeal {Y T Y' T' : Scheme.{u}}
    (strY : Y ⟶ T) (strY' : Y' ⟶ T') (φ : Y' ⟶ Y) (h : T' ⟶ T)
    (_hsq : IsPullback φ strY' strY h)
    (_hproper : IsProper strY) (_hsmooth : SmoothOfRelativeDimension 1 strY)
    {σ : T ⟶ Y} (_hσ : σ ≫ strY = 𝟙 T) {σ' : T' ⟶ Y'} (_hσ' : σ' ≫ strY' = 𝟙 T')
    (_hcompat : σ' ≫ φ = h ≫ σ) (_hsec : IsPullback σ' h φ σ) (z : Y') :
    ∃ (W : Y.Opens) (b : Γ(Y, W)) (V₀ : Y'.Opens) (hV₀ : V₀ ≤ φ ⁻¹ᵁ W),
      σ.app W b = 0 ∧ z ∈ V₀ ∧
      ∀ (V' : Y'.Opens) (hV' : V' ≤ V₀) (x : Γ(Y', V')), σ'.app V' x = 0 →
        ∃ a : Γ(Y', V'), x = a * Y'.presheaf.map (homOfLE (hV'.trans hV₀)).op (φ.app W b) :=
  sorry

/-- **THE COMPARISON MAP IS AN EPIMORPHISM** (PROVEN 2026-08-01 over
`exists_pullbackGenerator_sectionIdeal` above; cut 2026-07-31 out of
`isIso_modPullbackSectionIdealMap`, which is PROVEN over it and over
`isIso_of_epi_of_isInvertibleSheaf`, and is its only consumer) — Stacks 062Y /
0631, EGA IV 21.15, SURJECTIVITY HALF ONLY.

Content: the ideal `I_σ` extended to `𝒪_{Y'}` along `φ` is `I_{σ'}`.  This is
the RIGHT-EXACT half of the base-change statement and is where no flatness is
needed: `φ^*` is a left adjoint, so it preserves cokernels, and `σ` is a closed
immersion (`isClosedImmersion_of_isSection` from `_hproper` and `_hσ`) whose base
change along the cartesian section square `hsec` is `σ'`.

**`hsec` is the useful form of `_hsq` here** and is supplied by
`isPullback_section_square` above; it says `T' = T ×_Y Y'`, i.e. `D_{σ'}` is the
scheme-theoretic preimage of `D_σ`, which is exactly the hypothesis of the
closed-immersion base-change statement.  It is passed in rather than re-derived
because it is what the route consumes.

**LEAD, from the pin, not followed here.**
`Mathlib/AlgebraicGeometry/IdealSheaf/Functorial.lean` has
`Scheme.IdealSheafData.comap I f`, `comap_comp`, and — directly on point —
`ker_fst_of_isClosedImmersion (i : Z ⟶ Y) (f : X ⟶ Y) [IsClosedImmersion i] :
(pullback.fst f i).ker = i.ker.comap f`, together with
`AlgebraicGeometry.isPullback_of_isClosedImmersion`.  That IS this statement in
ideal-sheaf language.  What is missing is a bridge between
`Scheme.Hom.ker σ : IdealSheafData Y` and `sectionIdeal σ : Y.Modules`; that
bridge does not exist at this pin, is itself work, and would be a good reusable
leaf in its own right.

**FAITHFULNESS IS INHERITED, and here is why it transfers.**  This statement is
the conclusion of the 2026-07-31 leaf `isIso_modPullbackSectionIdealMap`
WEAKENED from `IsIso` to `Epi`, with one extra hypothesis (`hsec`) that is a
consequence of the others.  A weakening of the conclusion cannot turn a true
statement false, so every counterexample to this refutes that leaf too and the
audit below is the one that was written for it, unchanged.

**WHY `_hσ` IS LOAD-BEARING, with the witness recorded on the old leaf.**
...
`φ^*I ≅ 𝒪` of rank one against an ideal of the preimage equal to `0`.  Note the
correction the old audit already carried: this flatness is NOT supplied by
invertibility of `I_σ`, as the same witness shows.

**FALSITY AUDIT — the tempting weaker hypothesis is REFUTED.**  It is natural to
ask only that the SECTION square be cartesian, i.e. `hsec` alone, dropping
`_hsq`.  **That version is FALSE for the `IsIso` conclusion**, with an explicit
witness:
...
`Y ×_T T' = 𝔸¹_k`, not `Spec k[t]/(t)`.

**And the witness is worth re-reading now that the node is cut this way, because
it CONFIRMS the cut.**  In it the map `φ^*I_σ ⟶ I_{σ'} = 0` IS an epimorphism —
the target is the zero sheaf — and what fails is invertibility of `I_{σ'}`.
Which is exactly the clause `isIso_modPullbackSectionIdealMap` now obtains from
`_hproper`, `_hsmooth` and `_hsq`: with `_hsq` dropped, `strY' = 𝟙` is smooth of
relative dimension `0`, not `1`, so `isInvertibleSheaf_sectionIdeal_baseChange`
does not apply.  So the two halves of the new decomposition fail on exactly the
two sides the witness distinguishes, which is the strongest sanity check
available for a cut.
...
* `_hsmooth` is not used by the surjectivity argument; it is kept because the
  call site holds it for free and because dropping a hypothesis is a signature
  change.  `_hproper` IS used, through `isClosedImmersion_of_isSection`.
...
**RECUT 2026-08-01; THE DIRECT-SORRY COUNT DID NOT MOVE, 1 → 1.**  Everything
this statement said about `Y'.Modules` is now discharged by the subsection above
and the residue is `exists_pullbackGenerator_sectionIdeal`, a statement about
`σ^♯`, `φ^♯` and multiplication of ring sections.  Read the two audits below as
the audit of that leaf: this conclusion is implied by the old `IsIso` leaf and
implies the new one, so nothing was made harder and no witness changed.  The one
thing that DID change is that `_hsmooth` became load-bearing for the residue;
that is recorded on the leaf. -/
theorem epi_modPullbackSectionIdealMap {Y T Y' T' : Scheme.{u}}
    (strY : Y ⟶ T) (strY' : Y' ⟶ T') (φ : Y' ⟶ Y) (h : T' ⟶ T)
    (hsq : IsPullback φ strY' strY h)
    (hproper : IsProper strY) (hsmooth : SmoothOfRelativeDimension 1 strY)
    {σ : T ⟶ Y} (hσ : σ ≫ strY = 𝟙 T) {σ' : T' ⟶ Y'} (hσ' : σ' ≫ strY' = 𝟙 T')
    (hcompat : σ' ≫ φ = h ≫ σ) (hsec : IsPullback σ' h φ σ) :
    Epi (modPullbackSectionIdealMap hcompat) := by
  refine epi_of_locally_surjective_app _ (fun V x z hz => ?_)
  obtain ⟨W, b, V₀, hV₀, hb, hzV₀, hgen⟩ :=
    exists_pullbackGenerator_sectionIdeal strY strY' φ h hsq hproper hsmooth hσ hσ'
      hcompat hsec z
  have hV'V : V ⊓ V₀ ≤ V := inf_le_left
  have hV'V₀ : V ⊓ V₀ ≤ V₀ := inf_le_right
  refine ⟨V ⊓ V₀, hV'V, ⟨hz, hzV₀⟩, ?_⟩
  have hx₀ : σ'.app V (Scheme.Modules.Hom.app (kernel.ι (sectionIdealMap σ')) V x) = 0 :=
    (sectionIdealMap_app_eq_zero_iff σ' V _).1 (kernel_ι_app_mem _ V x)
  have hx₀' : σ'.app (V ⊓ V₀) (Y'.presheaf.map (homOfLE hV'V).op
      (Scheme.Modules.Hom.app (kernel.ι (sectionIdealMap σ')) V x)) = 0 :=
    app_eq_zero_of_le σ' hV'V _ hx₀
  obtain ⟨a, ha⟩ := hgen (V ⊓ V₀) hV'V₀ _ hx₀'
  obtain ⟨bt, hbt⟩ := exists_kernel_section (sectionIdealMap σ) W b
    ((sectionIdealMap_app_eq_zero_iff σ W b).2 hb)
  refine ⟨a • ((Scheme.Modules.pullback φ).obj (sectionIdeal σ)).presheaf.map
      (homOfLE (hV'V₀.trans hV₀)).op
      (Scheme.Modules.Hom.app
        ((Scheme.Modules.pullbackPushforwardAdjunction φ).unit.app (sectionIdeal σ)) W bt), ?_⟩
  refine kernel_ι_app_injective (sectionIdealMap σ') (V ⊓ V₀) ?_
  have hinner : Scheme.Modules.Hom.app
        ((Scheme.Modules.pullback φ).map (kernel.ι (sectionIdealMap σ)) ≫
          (modPullbackUnitIso φ).hom) (V ⊓ V₀)
        (((Scheme.Modules.pullback φ).obj (sectionIdeal σ)).presheaf.map
          (homOfLE (hV'V₀.trans hV₀)).op
          (Scheme.Modules.Hom.app
            ((Scheme.Modules.pullbackPushforwardAdjunction φ).unit.app (sectionIdeal σ)) W bt))
      = Y'.presheaf.map (homOfLE (hV'V₀.trans hV₀)).op (φ.app W b) := by
    refine Eq.trans (app_map_restrict _ (hV'V₀.trans hV₀) _) (congrArg _ ?_)
    refine Eq.trans (app_pullback_comp_unit (A := sectionIdeal σ) φ
      (kernel.ι (sectionIdealMap σ)) W bt) (congrArg _ hbt)
  rw [app_modPullbackSectionIdealMap_kernel_ι]
  refine Eq.trans (Scheme.Modules.Hom.app_smul _ a _) ?_
  refine Eq.trans (congrArg (fun t => a • t) hinner) ?_
  refine Eq.trans ?_ (app_map_restrict (kernel.ι (sectionIdealMap σ')) hV'V x).symm
  exact ha.symm

/-- **THE COMPARISON MAP IS AN ISOMORPHISM** (PROVEN 2026-07-31 over
`epi_modPullbackSectionIdealMap` and `isIso_of_epi_of_isInvertibleSheaf`; a sorry
leaf for a few hours on 2026-07-31, and before that the bare `Nonempty` leaf
`nonempty_modPullback_sectionIdeal_of_isPullback` below, which is still its only
consumer) — Stacks 062Y / 0631, EGA IV 21.15: a relative effective Cartier
divisor pulls back to one, and its ideal sheaf pulls back to the ideal sheaf.

The proof is the section header above: both source and target are invertible —
the source by `isInvertibleSheaf_modPullback` over
`isInvertibleSheaf_sectionIdeal_of_isSection`, the target by
`isInvertibleSheaf_sectionIdeal_baseChange`, which is where `_hsq` is spent a
second time — so an epimorphism between them is an isomorphism.

**The route the leaf used to record, kept because it is correct and because it
is the one a reader will reconstruct.**  In the affine model —
`Y = Spec B`, `T = Spec A`, `strY^# : A ⟶ B`, `σ^# : B ⟶ A` an `A`-algebra
retraction of it (that IS `_hσ`), `I = ker σ^#`, `T' = Spec A'`,
`B' = B ⊗_A A'` (that IS `_hsq`), `σ'^# = σ^# ⊗ 𝟙` (that IS `_hσ'` + `hcompat`)
— the defining sequence

    0 ⟶ I ⟶ B ⟶ A ⟶ 0

is a sequence of `A`-modules that is SPLIT, the splitting being `strY^#` itself,
so every additive functor preserves it and `− ⊗_A A'` gives

    0 ⟶ I ⊗_A A' ⟶ B' ⟶ A' ⟶ 0,

whence `I ⊗_A A' = ker σ'^# = I_{σ'}` and `I ⊗_B B' = I ⊗_A A' = φ^* I_σ`.  Its
one missing formal input is flat base change — that `φ^*` on `Y.Modules`, read
through the `strY^{-1}𝒪_T`-module structure, IS `− ⊗_{𝒪_T} 𝒪_{T'}` — which the
pin does not have (`Mathlib/AlgebraicGeometry/Modules/` is `Presheaf`, `Sheaf`,
`Tilde`, none of them about base change, and there is no quasi-coherence
predicate on `Scheme.Modules` at all).  **That is the input the present
decomposition avoids**, by getting injectivity from invertibility of the two
sides instead of from `Tor`-vanishing.
...
theorem isIso_modPullbackSectionIdealMap {Y T Y' T' : Scheme.{u}}
    ...
    {σ : T ⟶ Y} (_hσ : σ ≫ strY = 𝟙 T) {σ' : T' ⟶ Y'} (_hσ' : σ' ≫ strY' = 𝟙 T')
    (hcompat : σ' ≫ φ = h ≫ σ) :
    IsIso (modPullbackSectionIdealMap hcompat) :=
  isIso_of_epi_of_isInvertibleSheaf
    (isInvertibleSheaf_modPullback φ
      (isInvertibleSheaf_sectionIdeal_of_isSection strY _hproper _hsmooth _hσ))
    (isInvertibleSheaf_sectionIdeal_baseChange _hsq _hproper _hsmooth _hσ') _
    (epi_modPullbackSectionIdealMap strY strY' φ h _hsq _hproper _hsmooth _hσ _hσ' hcompat
      (isPullback_section_square _hsq _hσ _hσ' hcompat))
...
/-- **A SECTION OF A SMOOTH RELATIVE CURVE IS AN EFFECTIVE CARTIER DIVISOR**
(PROVEN — three lines over `isInvertibleSheaf_sectionIdeal_of_isSection`, which
carries the whole argument and which was HOISTED on 2026-07-31 to just below
`exists_trivialization_sectionIdeal_at_section`, several hundred lines above
here, so that `isIso_modPullbackSectionIdealMap` could consume it; formerly a
bare sorry leaf, and the
...
`exists_relPicZeroSubfunctor`; the audit above this subsection is the one
written while it was one node, and still says what a reader needs).

**MERGE REPAIR, 2026-08-01 — this node had been cut TWICE and the tree carried
both cuts.**  A 2026-07-30 branch cut it into `exists_relPicZeroGroupScheme`
(a `RelGroupSchemeStruct` on `Pic⁰`) plus `isProper_of_relPicZeroGroupScheme`,
glued by `RelGroupSchemeStruct.toAbelianSchemeStruct`; a 2026-07-31 branch cut
it into `exists_relPicIdentityComponent` plus `isProper_relPicIdentityComponent`,
assembled by `exists_relPicZeroSubfunctor`, with the group axioms transported
here rather than demanded of the geometry.  The two landed in regions far enough
apart not to conflict, so the merge kept BOTH — and resolved the one line they
share, this theorem's body, to the 07-30 side.  The result was invisible to
every instrument: four open leaves where two suffice, `exists_relPicZeroSubfunctor`
PROVEN with no consumer anywhere in the tree (free-floating), and this
declaration's docstring naming a leaf its own proof body did not call.

The 07-31 cut is kept, for three reasons: its leaves do not ask the geometry
owner for the nine group axioms (they are transported below, and none of them
is geometry); `isProper_relPicIdentityComponent` receives its sibling's FULL
conclusion, including `GeometricallyConnected jstr`, so the `J = P` junk witness
is killed on the leaf rather than by an appeal to how `J` was built; and its
audit carries a five-step proof plan the 07-30 pair did not have.  The 07-30
block — `RelGroupSchemeStruct`, its `toAbelianSchemeStruct`, `IsRelPicZeroIncl`
and the two leaves — is DELETED, since after this repair nothing reaches it;
recover it with `git show <this commit>^:Fermat/FLT/ModularCurve/RelativePicard.lean`.
Frontier: 4 open leaves → 2, with no mathematics done and none owed that was not
owed before.

One correction to the deleted prose, recorded because it would otherwise be
re-derived: `isProper_of_relPicZeroGroupScheme`'s docstring claimed to be
non-vacuous because "`J = P`, `incl = id`, `G` the group data of `Pic` itself
satisfies `IsRelPicZeroIncl` in full".  That witness does NOT satisfy its
hypotheses — `G : RelGroupSchemeStruct pstr` carries `connected`, and `Pic` is
not geometrically connected over `S` (one component per degree).  The leaf was
therefore not self-refuting, as a literal reading of that paragraph suggests;
it was merely superseded.

The assembly below is the transport of a group structure along an injection
whose image is a subgroup: `ab.add p q` is the unique preimage of
...
chain ending in the corresponding law on `Pic` (`addPoint_assoc`,
`addPoint_comm`, `zeroPoint_addPoint`, `negPoint_addPoint`, `pre_addPoint`,
`pre_zeroPoint`, all PROVEN above).  Not one of them touches the geometry, which
    ...
    ∃ (J : Scheme.{u}) (jstr : J ⟶ S) (ab : AbelianSchemeStruct jstr)
      ...
      (∀ (T : Scheme.{u}) (g : T ⟶ S) (p q : RelPoint jstr g),
        ...
        (∀ (T : Scheme.{u}) (g : T ⟶ S) (x : RelPoint strX g),
            ∃ p : RelPoint jstr g, incl T g p = aj T g x) := by
  classical
  obtain ⟨J, jstr, incl, hproper, hsm, hcn, hinj, hpre, hzero, hadd, hneg, himg⟩ :=
    exists_relPicZeroSubfunctor _hproper _hsmooth _hconn o hP _hpush _hPsmooth _hPsep
      _hequiv aj _haj
  -- the three operations, as opaque functions with their characterising equations
  choose Z hZ using hzero
  choose A hA using hadd
  choose N hN using hneg
  refine ⟨J, jstr,
    { add := fun {T g} p q => A T g p q
      zero := fun {T} g => Z T g
      neg := fun {T g} p => N T g p
      add_assoc := fun {T g} x y z => hinj T g _ _ (by
        simp only [hA]; exact hP.addPoint_assoc _ _ _)
      add_comm := fun {T g} x y => hinj T g _ _ (by
        simp only [hA]; exact hP.addPoint_comm _ _)
      zero_add := fun {T g} x => hinj T g _ _ (by
        simp only [hA, hZ]; exact hP.zeroPoint_addPoint _)
      neg_add := fun {T g} x => hinj T g _ _ (by
        simp only [hA, hN, hZ]; exact hP.negPoint_addPoint _)
      pre_add := fun {T' T} h {g g'} hg x y => hinj T' g' _ _ (by
        simp only [hpre, hA, hP.pre_addPoint])
      pre_zero := fun {T' T} h {g g'} hg => hinj T' g' _ _ (by
        simp only [hpre, hZ, hP.pre_zeroPoint])
      proper := hproper
      smooth := hsm
      connected := hcn }, incl, hinj, hZ, hA, hpre, himg⟩

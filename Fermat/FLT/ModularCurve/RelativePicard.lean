...
* `exists_relPicZeroOf_of_relPicGroupLaw` — BLR 9.4/4 with `f_*𝒪 = 𝒪`,
  ...
  * `exists_relPicZeroSubgroup` — the geometry: cut `Pic⁰` out of `Pic`
    ...
    the leaf `exists_relPicZeroSubfunctor` — which asks for the same
    ...
    `IsRelPicOf` group-law section.  **Amended again the same day**:
    `exists_relPicZeroSubfunctor` is PROVEN too, and is now the assembly
    of the TWO classical chapters it used to carry together —
    `exists_relPicIdentityComponent` (SGA3 VI_B 3.10: the subfunctor,
    smooth with geometrically connected fibres, closed under the group
    law, containing the Abel–Jacobi image) and
    `isProper_relPicIdentityComponent` (BLR 9.4: that subfunctor is
    proper).  Neither half mentions the other's mathematics.

So the direct-sorry set of this module is 10 (verified against the
compiler's `declaration uses 'sorry'` warnings, and against a
comment-stripped token count — 10 = 10, so there are no anonymous inner
sorries hiding behind a warning), and the six that belong to BLR 9.4/4
...
`smooth_of_isRelPicOf`, `isSeparated_of_isRelPicOf`,
`exists_relPicIdentityComponent` and `isProper_relPicIdentityComponent`
(2026-07-31: those last two are the split of `exists_relPicZeroSubfunctor`,
which in turn used to be `exists_relPicZeroSubgroup`; both of those are now
PROVEN, each as the assembly of what replaced it).

**A note on the count, since three leaves closed on 2026-07-31 and it went
9 → 9 → 10.**  All three closures were CUTS: a leaf is proven and one or two
are opened, so the count is flat or RISES while real progress is made.  That is
the expected shape when a node is decomposed rather than discharged, and reading
the count alone would say nothing happened, or worse, that the module regressed.
What did happen is that the open statements got strictly smaller — the first
lost every mention of a curve, the second lost nine group axioms, and the third
was separated into its two classical chapters, neither of which now mentions the
other's mathematics — and fourteen declarations that used to be inside them are
now theorems.

The general form, worth stating once because this module has now supplied three
instances of it in one day: **judge a cut by whether the OPEN statement got
smaller, never by the delta in the leaf count.**  A cut that splits one node
into two independent chapters is the most valuable kind available here — it lets
two owners work at once and neither has to learn the other's literature — and it
is exactly the kind that makes the count go UP.
...
theorem nonempty_modPullback_sectionIdeal {X S T T' : Scheme.{u}} {strX : X ⟶ S}
  ...
  nonempty_modPullback_sectionIdeal_of_isPullback
    (isPullback_curveBaseChangeMap strX h hg) (relSection_comp_curveBaseChangeProj x)
    (relSection_comp_curveBaseChangeProj (RelPoint.pre h hg x))
    (relSection_comp_curveBaseChangeMap h hg x)

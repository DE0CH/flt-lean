...
/-- **THE IDEAL OF A SECTION IS GENERATED BY A NONZERODIVISOR ON THE BASIC OPENS
OF AN AFFINE NEIGHBOURHOOD** (sorry leaf) — the geometric residue of
...
**RECUT 2026-08-01; THE DIRECT-SORRY COUNT DID NOT MOVE, 1 → 1, AND THE NAME
CHANGED.**  Until today the leaf here was
`exists_generator_sectionIdeal_at_section`, whose two clauses were quantified
over EVERY open `W ≤ U`.  That name is now a PROVEN four-line assembly
immediately below, and the leaf is this statement — the same three clauses, but
quantified only over the BASIC OPENS `D(f)` of an AFFINE `U`, which is where
they are commutative algebra: `Γ(Y, D(f))` is the localization `Γ(Y, U)_f`
(`IsAffineOpen.isLocalization_basicOpen`), so `hnzd` is "a nonzerodivisor stays
one in every localization" and `hgen` is "the kernel of `σ^♯` localizes".  The
bridge is `nonzerodivisor_and_generates_of_locally` above, PROVEN, plus
`IsAffineOpen.exists_basicOpen_le` for the basis.

**Nothing was made harder, and the affineness of `U` is a FREE STRENGTHENING.**
The previous statement's own docstring already observed that the passage from a
basis to all opens "is not extra mathematics"; that passage is now a theorem
rather than a promise.  And `IsAffineOpen U` is an extra thing the leaf must
PRODUCE, not an extra hypothesis — it costs the prover nothing, because the
route below produces `U` out of `SmoothOfRelativeDimension`, which hands over an
affine open in the first place, and it is what makes the basic opens useful.

**A FURTHER CUT THAT IS AVAILABLE AND WAS NOT TAKEN, with its price.**  The
honest terminal form of Stacks 0C4S is the statement at `U` ALONE:

    ∃ U, IsAffineOpen U ∧ IsAffineOpen (σ ⁻¹ᵁ U) ∧ σ.base t ∈ U ∧
      ∃ s : Γ(Y, U), σ.app U s = 0 ∧ s ∈ nonZeroDivisors Γ(Y, U) ∧
        RingHom.ker (σ.app U).hom = Ideal.span {s}

and the passage from it to the basic-open form below is again pure algebra: a
nonzerodivisor stays one in `A_f` (if `a·s/f^n = 0` then `f^m·a·s = 0` in `A`,
so `f^m·a = 0`, so `a/f^n = 0` — no injectivity of `A → A_f` is needed), and
`ker` localizes because `σ ⁻¹ᵁ (Y.basicOpen f) = T.basicOpen (σ.app U f)`
(`Scheme.preimage_basicOpen`) identifies `σ.app (D f)` with the localization of
`σ.app U` at `f`.  It was NOT taken here because that last identification needs
`σ ⁻¹ᵁ U` AFFINE as well — which is true and arrangeable, but is a third
conjunct to thread and a second `IsLocalization` transport to build, and the
value of doing it is bounded: it moves the same algebra from the leaf's
consumer to the leaf's producer.  A successor who wants the crisper leaf should
take it; the two are equivalent and the derivation is sketched here.
...
`hnzd` is that `s` is a nonzerodivisor on every basic open of `U`; over an affine
`U = Spec A` on which `strY` is standard smooth it is the statement that `s` is a
nonzerodivisor in `A` and in every localization `A_f`.  `hgen` is
`I(D(f)) = s · Γ(Y, D(f))`, i.e. that `ker σ^♯` is `(s)` after localizing at `f`.
Neither clause mentions a non-affine open any more, and the passage from them to
the all-opens form is `nonzerodivisor_and_generates_of_locally` above.
...
* 2026-08-01: this basic-open statement is equivalent to the all-opens one.  `⇒`
  is the assembly below.  `⇐` is: shrink the all-opens `U` to an affine
  `U' ≤ U` containing `σ.base t` (`Scheme.isBasis_affineOpens`) and take
  `s' := s|_{U'}`; every basic open `D(f) ≤ U'` is in particular an open `≤ U`,
  so the all-opens clauses apply there, and `ModDual.res_res` identifies
  `s|_{D(f)}` computed through `U'` with the one computed through `U`.  It is not
  formalized because nothing needs it.
    ...
    ∃ (U : Y.Opens) (_ : IsAffineOpen U) (_ : σ.base t ∈ U) (s : Γ(Y, U)),
      ...
      (∀ (f : Γ(Y, U)) (x : Γ(Y, Y.basicOpen f)),
        σ.app (Y.basicOpen f) x = 0 →
          ∃ r : Γ(Y, Y.basicOpen f),
            x = r * Y.presheaf.map (homOfLE (Y.basicOpen_le f)).op s) := sorry

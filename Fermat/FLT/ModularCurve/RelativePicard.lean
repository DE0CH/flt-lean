...
which is what makes `AmpleSheaf`'s `trivializedSection_trivializationOfLE`
provable.  The old route through `modPullback` (pseudo-functoriality) is NOT
equivalent for that purpose: its component isomorphisms come from
`Adjunction.leftAdjointUniq` and compute on sections not at all.  Do not revert
it. -/

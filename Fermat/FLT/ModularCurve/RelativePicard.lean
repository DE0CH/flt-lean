...
/-- **PULLBACK PRESERVES INVERTIBILITY** (PROVEN 2026-07-29, new here).

Stated and placed exactly where `exists_abelJacobiPoint`'s route note
below asks for it — "whoever takes this leaf should state and prove it in
the tensor-calculus section above, next to `isInvertibleSheaf_modTensorPic`".
That note's "absent from this module" is now a STALE CLAIM and is
corrected in place there.  It is consumed here by
`relPicEquiv_modPullback`, which is what
`inj_of_isRelPicOverAffines` runs on, and it is the same statement
`aj_pre` was blocked on.

No hypothesis on `f` — invertibility is a purely local condition, so the
proof is: trivialize `L` over `U ∋ f z`, and pull the trivialization back
over `f ⁻¹ᵁ U`.  The only content is that restricting to `f ⁻¹ᵁ U` and
then pulling back along `f ∣_ U` is the same as pulling back along `f`
and then restricting, which is `morphismRestrict_ι` transported by
`modPullbackCongrIso`.  In particular NO flatness, quasi-compactness or
finite presentation is needed, and none should be added. -/
theorem isInvertibleSheaf_modPullback {Z W : Scheme.{u}} (f : W ⟶ Z) {L : Z.Modules}
  ...
  refine ⟨f ⁻¹ᵁ U, hU, ⟨?_⟩⟩
  refine modRestrictPullbackIso _ _ ≪≫ modPullbackCompIso _ _ _ ≪≫ ?_
  refine modPullbackCongrIso (morphismRestrict_ι f U).symm L ≪≫ ?_
  refine (modPullbackCompIso (f ∣_ U) U.ι L).symm ≪≫ ?_
  exact modPullbackMapIso _ ((modRestrictPullbackIso U.ι L).symm ≪≫ φ) ≪≫
    modPullbackUnitIso _
...
/-- **The base-change square commutes** (PROVEN): `φ ≫ π_g = π_{g'} ≫ h`
for `φ = curveBaseChangeMap strX h hg`.

One `pullback.lift_snd`, but it has to be stated: it is what rewrites a
twist `π_g^* N` pulled back along `φ` into `π_{g'}^* (h^* N)`, which is
the whole of `relPicEquiv_modPullback` below. -/
theorem curveBaseChangeMap_comp_proj {X S T T' : Scheme.{u}} (strX : X ⟶ S) {g : T ⟶ S}
    {g' : T' ⟶ S} (h : T' ⟶ T) (hg : h ≫ g = g') :
    curveBaseChangeMap strX h hg ≫ curveBaseChangeProj strX g =
      curveBaseChangeProj strX g' ≫ h := by
  simp only [curveBaseChangeMap, curveBaseChangeProj]
...
/-- **`RelPicEquiv` IS STABLE UNDER CHANGE OF TEST OBJECT** (PROVEN
2026-07-30): if `L ≡ L'` over `T`, then their pullbacks along the induced
map `X_{T'} ⟶ X_T` are equivalent over `T'`.

This is the transport step that `inj_of_isRelPicOverAffines` below is
built out of, and the only step in that leaf's route note with any
content.  The twist travels: if `L ≅ L' ⊗ π_g^* N` then pulling back
along `φ = curveBaseChangeMap strX h hg` gives
`φ^* L ≅ φ^* L' ⊗ φ^* π_g^* N`, and `curveBaseChangeMap_comp_proj`
rewrites `φ^* π_g^* N` as `π_{g'}^* (h^* N)`, whose invertibility is
`isInvertibleSheaf_modPullback`.  The interchange of `modPullback` with
`modTensor` is `nonempty_modPullback_modTensorPic`.

No hypothesis on `h`: like invertibility, the relation is defined by
pullback data alone, so nothing flat or proper is needed. -/
theorem relPicEquiv_modPullback {X S T T' : Scheme.{u}} {strX : X ⟶ S} {g : T ⟶ S}
    {g' : T' ⟶ S} (h : T' ⟶ T) (hg : h ≫ g = g')
    {L L' : (curveBaseChange strX g).Modules} (e : RelPicEquiv strX g L L') :
  ...
  obtain ⟨tp⟩ := nonempty_modPullback_modTensorPic (curveBaseChangeMap strX h hg) L'
    (modPullback (curveBaseChangeProj strX g) N)
  refine ⟨modPullbackMapIso _ φ ≪≫ tp ≪≫ modTensorMapIso (Iso.refl _) ?_⟩
  exact modPullbackCompIso _ _ _ ≪≫
    modPullbackCongrIso (curveBaseChangeMap_comp_proj strX h hg) N ≪≫
    (modPullbackCompIso (curveBaseChangeProj strX g') h N).symm
...
theorem inj_of_isRelPicOverAffines {X P S : Scheme.{u}} {strX : X ⟶ S} {pstr : P ⟶ S}
  ...
  refine relPicEquiv_trans _ _ (relPicEquiv_modPullback (g ⁻¹ᵁ V).ι rfl h) ?_

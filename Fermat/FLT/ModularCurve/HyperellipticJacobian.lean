...
* `<level>.descent_system_no_solution` → `<level>.descent_system_no_solution_pos`
  ...
  At level `18` the two branches carry DIFFERENT theories and are independently
  dispatchable: the `+` branch is rank `1` and needs `p`-adic elliptic Chabauty
  while the `−` branch is rank `0` and needs none.  **At level `13` they are
  NOT** (correction, 2026-07-27): `−1 = i²`, so the relabelling `(u, v) ↦
  (v, u)` carries one branch to the other and `X13.descent_system_no_solution_neg`
  is PROVEN from `X13.descent_system_no_solution_pos`.  Level `13` has ONE
  branch to close, not two — and by the previous bullet it needs no Chabauty

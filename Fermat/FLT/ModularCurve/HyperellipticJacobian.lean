...
* `redPt_injective_five ↔ affine_rational_points` (level `18` only, so far) by
...
levels, so **one future genus-`2` Jacobian development discharges both at
once** — and until it lands, nobody needs to design a Lean interface for `Pic⁰`
in order to work on either.  The level-`13` leaf is still in its
`redPt_injective` form; the same `red_sixPts`/`sixPtsData_injective` route would
restate it as a Diophantine statement about its own sextic
(`sext 1 4 6 2 1 2`, i.e. `x⁶ + 2x⁵ + x⁴ + 2x³ + 6x² + 4x + 1`, the arguments
of `sext` being `c₀ … c₅`) if anyone wants it in that shape.

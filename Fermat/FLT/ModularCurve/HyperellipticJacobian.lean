...
covering collection.  Each namespace now reads top-down:
    ...
    geomPic_exists_const_of_ord_nonneg / geomPic_hilbert90
                                  LEAF ×2: κ(w) = ℚ̄, and Hilbert 90 for F̄/F
      _exists_bcDiv_of_divAct_fixed / _exists_finiteLevel_divisor
                                  PROVEN 2026-08-01 over placeAct_transitive and
                                  pt_surjective_of_isAlgClosed: Galois transitivity on
                                  fibres, and the finite level of a divisor
...
## PROVEN 2026-08-01, and the proof was already in the file

Transitivity stopped being a leaf on 2026-07-31 (`placeAct_transitive`, PROVEN over
`PlaceData.pt_surjective_of_isAlgClosed`), and at that moment this leaf became a
twenty-line consequence.  Nobody noticed, because the branch that proved transitivity spent
it INLINE inside `geomPic_descent` — the block from `hconst` to `hbc` there was, character
for character, a proof of this statement, sitting three hundred lines below a `sorry` saying
the same thing.  What is below is that block lifted out verbatim; `geomPic_descent` now calls
it and is four lines shorter.

**`hsep` is new, and it is the only change to the statement.**  It is what
`placeAct_transitive` consumes, and it is threaded from `finite_quotient_psmul_pic` through
`geomPic_descent`, the sole consumer.  Adding a hypothesis can only WEAKEN a statement, so
the faithfulness audit above transfers verbatim: every counterexample to the new form is a
counterexample to the old one.  What must be re-checked in that direction is only that the
hypothesis is not vacuous, and it is not — `finite_quotient_psmul_pic` supplies it at both
levels this file is about.

The proof rests on the still-open `PlaceData.pt_surjective_of_isAlgClosed` (through
`placeAct_transitive`), which adds NO `sorryAx` edge anywhere: `geomPic_descent` already
reached it by exactly this route. -/
...
Steps 1–4, 5's cocycle manipulation and 7's bookkeeping are Lean.  Note what does NOT appear:
no `H¹`, no `H²`, no `Br`, no profinite topology — the only trace of continuity is `hinfl` in
leaf 3d.

**Of the four sub-leaves, 3b and 3c were PROVEN on 2026-08-01** (over
`placeAct_transitive`, hence over `PlaceData.pt_surjective_of_isAlgClosed`), so the plan
above now owes only 3a — which is used twice, in step 2 and step 3, and is already consumed
by `geomPic_degOf_eq_one` — and 3d.  All four are stated ABOVE
`geomPic_descent_divisor`'s intended proof and none of them is consumed by it yet: the
decomposition was designed on 2026-07-30 and has not been carried out, which is why 3a is the
only one of the four with a consumer today.  A prover who writes the seven steps closes
`geomPic_descent_divisor` over 3a and 3d alone.

**`geomPic_descent` itself does NOT go through the seven steps** — it consumes
`geomPic_descent_divisor` as a black box and then applies 3b.  Until 2026-08-01 it applied
3b's PROOF inline instead, which is how 3b came to be a `sorry` three hundred lines above its
own argument; see the RECUT note on 3b. -/
theorem geomPic_descent {c₀ c₁ c₂ c₃ c₄ c₅ : ℤ} {D : PlaceData c₀ c₁ c₂ c₃ c₄ c₅ ℚ}
  ...
  -- an invariant divisor is a base change (leaf 3b, PROVEN 2026-08-01: this block used to
  -- be written out inline here, which is why 3b sat as a `sorry` three hundred lines above
  -- its own proof)

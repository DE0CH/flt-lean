...
**The remaining leaves are the TEN listed in the tree above** (recounted against the
compiler's `declaration uses 'sorry'` set on 2026-07-28 — the paragraph said "eight", and
named `finite_pic`, for one integration cycle after obligation 4 was split; both
`finite_pic` are now PROVEN and what is open in their place is `two_divisible_pic`).
EIGHT of them — `exists_functionFieldData`, `exists_placeSystem`, `exists_isPlaceOfPt`,
`exists_degreeMap`, `sub_single_pt_notMem_princ`, `exists_smoothModel`,
`exists_descentHeight_pic`, `finite_quotient_psmul_pic` — are generic in the sextic and

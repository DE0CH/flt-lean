...
## DECOMPOSED — now PROVEN over the two leaves in the `GeomPic` namespace above

The two sentences of the docstring above are now two named leaves, and the bookkeeping
joining them is Lean rather than prose:

* `GeomPic.below_surjective` — every place of `F` lies under a geometric place.  This is the
  half nobody states, and it is the half the DIVISOR level needs: `bcDiv` is injective
  exactly when `below` is surjective (`GeomPic.bcDiv_injective`), and without it a place
  outside the image of `below` would refute this theorem at the class of `single v 1`.
* `GeomPic.exists_emb_of_divisor_invariant` — Hilbert 90, in the only form used: a geometric
  function with Galois-invariant divisor is `algebraMap c · emb g`.

Everything between them is proven here: `divAct_bcDiv` turns membership of `bcDiv δ` in
`Dbar.picRel` into the invariance hypothesis (the base point contributes nothing, by
`divAct_single_infPlus`); the descended `g` gives back `bcDiv (D.divisor g)` because a
constant has trivial divisor; and `bcDiv_injective` cancels the base change.

Note where the rational point actually enters: NOT in the descent step, but in
`bcDiv_single_infPlus`, i.e. in `below_infPlus`, which is what lets the `ℤ·[∞₊]` summand of
`picRel` be matched on the nose instead of only up to the fibre over `∞₊`. -/
theorem geomPic_bc_injective {c₀ c₁ c₂ c₃ c₄ c₅ : ℤ} {D : PlaceData c₀ c₁ c₂ c₃ c₄ c₅ ℚ}
    (gp : GeomPic c₀ c₁ c₂ c₃ c₄ c₅ D) : Function.Injective gp.bc := by
  refine (injective_iff_map_eq_zero gp.bc).mpr ?_
  intro a
  induction a using QuotientAddGroup.induction_on with
  | _ δ =>
    intro ha
    -- `bcDiv δ` is principal over `ℚ̄`, up to the base point
    have hmem : gp.bcDiv δ ∈ gp.Dbar.picRel := by
      have h0 : (QuotientAddGroup.mk (gp.bcDiv δ) : gp.Dbar.Pic) = 0 := ha
      exact (QuotientAddGroup.eq_zero_iff _).mp h0
    have hmem' : gp.bcDiv δ ∈ gp.Dbar.princ
        ⊔ AddSubgroup.zmultiples (Finsupp.single (gp.Dbar.pt PlaceData.infPlus) (1 : ℤ)) := hmem
    rw [AddSubgroup.mem_sup] at hmem'
    obtain ⟨y, hy, z, hz, hyz⟩ := hmem'
    rw [PlaceData.mem_princ_iff] at hy
    obtain ⟨n, hn⟩ := AddSubgroup.mem_zmultiples_iff.mp hz
    -- normalise the realising function to be nonzero: `div 0 = 0 = div 1`
    obtain ⟨u, hu0, hu⟩ : ∃ u : gp.Dbar.F, u ≠ 0 ∧ gp.Dbar.divisor u = y := by
      obtain ⟨u, hu⟩ := hy
      rcases eq_or_ne u 0 with rfl | h
      · exact ⟨1, one_ne_zero, by rw [PlaceData.divisor_one]; rw [← hu, PlaceData.divisor_zero]⟩
      · exact ⟨u, h, hu⟩
    have hd : gp.Dbar.divisor u
        + n • Finsupp.single (gp.Dbar.pt PlaceData.infPlus) (1 : ℤ) = gp.bcDiv δ := by
      rw [hu, hn]; exact hyz
    -- the divisor of `u` is Galois-invariant, because `bcDiv δ` and the base point are
    have hinv : ∀ σ : QbarGal, gp.Dbar.divisor (gp.fieldAct σ u) = gp.Dbar.divisor u := by
      intro σ
      have h2 := congrArg (gp.divAct σ) hd
      rw [map_add, map_zsmul, gp.divAct_divisor, gp.divAct_single_infPlus,
        gp.divAct_bcDiv, ← hd] at h2
      exact add_right_cancel h2
    -- Hilbert 90: `u` is a constant times a rational function
    obtain ⟨c, g, hc, hg0, hueq⟩ := gp.exists_emb_of_divisor_invariant hu0 hinv
    have hcF : algebraMap (AlgebraicClosure ℚ) gp.Dbar.F c ≠ 0 :=
      (map_ne_zero_iff _ (algebraMap (AlgebraicClosure ℚ) gp.Dbar.F).injective).mpr hc
    have hembg : gp.emb g ≠ 0 := (map_ne_zero_iff gp.emb gp.emb_injective).mpr hg0
    have hconst : gp.Dbar.divisor (algebraMap (AlgebraicClosure ℚ) gp.Dbar.F c) = 0 := by
      ext w
      rw [PlaceData.divisor_apply _ hcF w, gp.Dbar.ord_algebraMap w c hc]
      simp
    have hdivu : gp.Dbar.divisor u = gp.bcDiv (D.divisor g) := by
      rw [hueq, PlaceData.divisor_mul _ hcF hembg, hconst, zero_add, gp.bcDiv_divisor]
    -- so `δ` and the rational representative have the same base change
    have hfin : gp.bcDiv δ
        = gp.bcDiv (D.divisor g + n • Finsupp.single (D.pt PlaceData.infPlus) (1 : ℤ)) := by
      rw [map_add, map_zsmul, gp.bcDiv_single_infPlus, ← hdivu, hd]
    have hδ : δ = D.divisor g + n • Finsupp.single (D.pt PlaceData.infPlus) (1 : ℤ) :=
      gp.bcDiv_injective hfin
    show (QuotientAddGroup.mk δ : D.Pic) = 0
    refine (QuotientAddGroup.eq_zero_iff _).mpr ?_
    rw [hδ]
    exact AddSubgroup.add_mem _
      ((le_sup_left : D.princ ≤ D.picRel) ((PlaceData.mem_princ_iff D).mpr ⟨g, rfl⟩))
      ((le_sup_right : AddSubgroup.zmultiples _ ≤ D.picRel)

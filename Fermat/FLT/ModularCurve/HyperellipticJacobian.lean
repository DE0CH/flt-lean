...
At level `18` the leaf has been pushed one step further down than at level `13`;
the two chains are otherwise identical.  Each replacement below is an
EQUIVALENCE, so no statement was weakened at any step:
...
* `redPt_injective_five ↔ X18.affine_rational_points` by `red_sixPts` plus
...
leaf count is unchanged at one per level throughout.

...
| `exists_weierstrassGenerators_of_abelianSchemeChart` | **Riemann-Roch on a genus-one curve** — the affine chart of an abelian scheme of relative dimension one over `Spec 𝔽_ℓ` is generated by a Weierstrass pair `x ∈ L(2[O])`, `y ∈ L(3[O])`; NO modular curves and no level structure.  NEW 2026-07-31, as the sole leaf under `exists_weierstrassModel_of_abelianSchemeStruct_finiteField`.  The claim in the row this replaces — that "there is nothing to instantiate", the ℚ-side chain being hardcoded to `Spec ℚ` — was true of that chain and FALSE of the piece it is built over: `exists_isOpenImmersion_range_eq_compl_of_section` (`CurveAffineComplement.lean`) was generalised to an arbitrary base field on 2026-07-30 and supplies the whole scheme-theoretic half at `K = ZMod ℓ`.  RESTATED from a `RingEquiv` into this generators form later the same day: the two side conditions `IsDomain R` and `¬ IsField R` are geometry of `A` (integrality from smooth-plus-geometrically-connected, non-fieldness from `A` being INFINITE), and are now discharged in the assembly `exists_weierstrassRingEquiv_of_abelianSchemeChart`, which is PROVEN over this row. | `𝔽_ℓ` |
| ~~`exists_weierstrassRingEquiv_of_abelianSchemeChart`~~ | PROVEN 2026-07-31 over the row above, through `exists_surjective_coordinateRingHom_of_generators` and `injective_of_surjective_coordinateRing`; no longer a leaf | — |
...
generated by a Weierstrass pair** (sorry leaf, NEW 2026-07-31 — the whole
mathematical content of
`exists_weierstrassModel_of_abelianSchemeStruct_finiteField` below, with the
scheme-theoretic bookkeeping removed; RESTATED in generators form later the same
day, see the statement-form section at the foot of this docstring).
...
The two side conditions those two theorems need beyond the linear algebra —
`IsDomain R` and `¬ IsField R` — are NO LONGER OWED HERE.  They are geometry of
`A` rather than algebra of `R`, and both are discharged in
`exists_weierstrassRingEquiv_of_abelianSchemeChart` below, whose docstring names
the four theorems involved.  A prover at this leaf never has to think about
them.

**Statement form: GENERATORS, not a `RingEquiv` (RESTATED 2026-07-31; the
paragraph this replaces is quoted below because its reasoning was wrong).**  The
leaf now hands back `x`, `y` and the relation, and the `RingEquiv` form is
PROVEN over it in `exists_weierstrassRingEquiv_of_abelianSchemeChart` below.
The paragraph that stood here read:

> The generators form would be the tighter cut, and it is what
> `CurveAffineComplement.lean`'s own docstrings prescribe; it is not used here
> because it would force the `IsDomain`/`¬ IsField` obligations into the
> ASSEMBLY, where `R` is opaque and they are not cheap.

`R` is indeed opaque in the assembly, and that turned out not to matter: both
obligations are read off the GEOMETRY of `A` rather than off any presentation of
`R`, and each is one existing theorem of this tree (see the assembly's own
docstring for the four-line chain).  So the tighter cut is available after all,
and taking it removes from the Riemann–Roch prover's plate two facts that have
nothing to do with Riemann–Roch.

**WHAT IS STILL OWED HERE, and it is only the linear algebra.**
`exists_surjective_coordinateRingHom_of_generators` turns exactly this
conclusion into a surjection `W.toAffine.CoordinateRing →+* R` which is
automatically `𝔽_ℓ`-linear, and `injective_of_surjective_coordinateRing` makes
that surjection injective.  Neither is Riemann–Roch and both are PROVEN.  So a
prover here owes `x ∈ L(2[O])`, `y ∈ L(3[O])`, the relation among the seven
monomials, the generation clause `Subring.closure … = ⊤` (i.e. that `x` and `y`
generate the ring of functions regular away from `O`), and `W.IsElliptic`.

**`W.IsElliptic` IS DELIBERATELY LEFT HERE** rather than derived in the
assembly.  It is `W.Δ ≠ 0`, and the only route to it from the assembly's side is
"`Spec R` is smooth over `𝔽_ℓ`, a Weierstrass curve with `Δ = 0` has a singular
point, and that point is affine and rational" — which needs the singular-point
construction that this pin does not carry.  A prover who has produced `x` and
`y` out of `L(3[O])` has the nondegeneracy in hand for free, since `|3·[O]|` is
very ample exactly when the model is nonsingular.
...
/-- **RIEMANN–ROCH, PACKAGED AS A RING ISOMORPHISM** (**PROVEN 2026-07-31** over
the generators leaf immediately above; a bare `sorry` for the few hours between
its being cut and this).

Everything in this proof is commutative algebra and point-counting; the
Riemann–Roch is entirely upstairs.  The chain, and each step is one existing
theorem:

* `exists_surjective_coordinateRingHom_of_generators`
  (`CurveAffineComplement.lean`) turns `x`, `y` and the relation into a
  SURJECTION `φ : W.toAffine.CoordinateRing →+* R`, together with the
  `𝔽_ℓ`-linearity conjunct that this statement's `he` is;
* `injective_of_surjective_coordinateRing` (same file) makes `φ` INJECTIVE,
  given `IsDomain R` and `¬ IsField R`.

**THE TWO SIDE CONDITIONS ARE GEOMETRY OF `A`, NOT ALGEBRA OF `R`**, which is
why they can be discharged here even though `R` is opaque:

* `IsIntegral A` is `isIntegral_of_smoothOfRelativeDimension_of_geometricallyConnected`
  (`Fermat/FLT/Mathlib/AlgebraicGeometry/CurveExtension.lean`) applied to `f`
  with `_ab.connected` — smooth over a field gives regular, hence domain, stalks,
  and a connected locally irreducible space is irreducible.  `Spec R` is an open
  subscheme of `A` by `_hopen`, so `isIntegral_of_isOpenImmersion` transports
  integrality to it and `Scheme.ΓSpecIso` reads it back as `IsDomain R`.
* `¬ IsField R` is a POINT COUNT.  `infinite_of_smoothOfRelativeDimension_one`
  (same file) makes `A` INFINITE — this is where the `1` of `_hdim` is consumed,
  through the fact that the local models are finite-type algebras over a field,
  hence Jacobson — while `_hrange` says the range of `ι` is the complement of a
  single point (`range_eq_singleton_of_spec_field`), hence still infinite.  A
  field has a one-point spectrum, so `ι.base` would have a subsingleton range.

Note that the nonemptiness `Nonempty (Spec R)` needed by
`isIntegral_of_isOpenImmersion` falls out of the same count: it is not a
separate hypothesis and does not have to be assumed.

**A `_hover`-FREE PROOF WOULD BE WRONG even though this body never uses
`_hover`.**  The `𝔽_ℓ`-linearity conjunct `he` is produced by the generators
leaf (through `exists_surjective_coordinateRingHom_of_generators`'s second
conjunct), and it is the leaf that consumes `_hover` to get it: without `_hover`
the `Algebra (ZMod ℓ) R` instance is unrelated to `f`, an adversary retwists it
along Frobenius, and the leaf itself becomes false.  So `_hover` is load-bearing
for the statement even though it is inert in this assembly. -/
theorem exists_weierstrassRingEquiv_of_abelianSchemeChart {ℓ : ℕ} [Fact ℓ.Prime]
    ...
    ∃ (W : WeierstrassCurve (ZMod ℓ)) (_ : W.IsElliptic)
      ...
      ∀ a : ZMod ℓ, e (algebraMap (ZMod ℓ) W.toAffine.CoordinateRing a)
        = algebraMap (ZMod ℓ) R a := by
  classical
  haveI := hdim
  haveI := hopen
  -- `A` is integral: smooth over a field gives domain stalks, and
  -- geometric connectedness turns local irreducibility into irreducibility.
  haveI : IsIntegral A :=
    isIntegral_of_smoothOfRelativeDimension_of_geometricallyConnected
      (n := 1) (K := ZMod ℓ) f ab.connected
  haveI : Nonempty A := IsIntegral.nonempty
  -- `A` is infinite: this is where relative dimension `1` is consumed.
  haveI : Infinite A := infinite_of_smoothOfRelativeDimension_one (K := ZMod ℓ) f
  -- the zero section meets `A` in one point, so the chart's range is infinite.
  obtain ⟨z, hz⟩ :=
    range_eq_singleton_of_spec_field (K := ZMod ℓ) (ab.zero (𝟙 (SpecF ℓ))).1
  have hinf : (Set.range ι.base).Infinite := by
    rw [hrange, hz]
    exact (Set.finite_singleton z).infinite_compl
  haveI : Nonempty (Spec (CommRingCat.of R)) := by
    obtain ⟨_, p, _⟩ := hinf.nonempty
    exact ⟨p⟩
  -- hence `Spec R` is integral, i.e. `R` is a domain
  haveI : IsIntegral (Spec (CommRingCat.of R)) := isIntegral_of_isOpenImmersion ι
  haveI : IsDomain R :=
    (Scheme.ΓSpecIso (CommRingCat.of R)).symm.commRingCatIsoToRingEquiv.toMulEquiv.isDomain _
  -- and `R` is not a field, since `Spec` of a field is a single point
  have hnf : ¬ IsField R := by
    ...
    refine hinf.nontrivial.not_subsingleton ?_
    rintro _ ⟨p, rfl⟩ _ ⟨q, rfl⟩
    exact congrArg _ (Subsingleton.elim p q)
  -- the Riemann–Roch leaf, and the two proven halves of the recognition lemma
  obtain ⟨W, hW, x, y, hrel, hgen⟩ :=
    exists_weierstrassGenerators_of_abelianSchemeChart ab hdim R ι hopen hover hrange
  obtain ⟨φ, hsurj, hlin⟩ :=
    exists_surjective_coordinateRingHom_of_generators (k := ZMod ℓ) (R := R) W
      (algebraMap (ZMod ℓ) R) x y hrel hgen
  exact ⟨W, hW,
    RingEquiv.ofBijective φ ⟨injective_of_surjective_coordinateRing W hnf φ hsurj, hsurj⟩, hlin⟩

/-- **An abelian scheme of relative dimension one over `Spec 𝔽_ℓ` has a
Weierstrass model** (**PROVEN 2026-07-31** over
`exists_weierstrassRingEquiv_of_abelianSchemeChart` immediately above — itself
PROVEN, later the same day, over the single leaf
`exists_weierstrassGenerators_of_abelianSchemeChart`; a bare `sorry` from
2026-07-28 until then, cut as the GEOMETRY half of
...
## THE SCHEME-THEORETIC HALF IS NO LONGER OWED HERE (2026-07-31)

The paragraph above is the state of the leaf as cut on 2026-07-28, and one of
its premises was wrong: it is not true that "there is nothing to instantiate".
The `ℚ`-side chain in `EllipticScheme.lean` is indeed hardcoded to `Spec ℚ` and
is indeed still open — but the piece it is *itself* assembled over,
`AlgebraicGeometry.exists_isOpenImmersion_range_eq_compl_of_section` in
`Fermat/FLT/Mathlib/AlgebraicGeometry/CurveAffineComplement.lean`, was
generalised to an arbitrary base field `K` on 2026-07-30 (that is the commit
that added its `Algebra K R` and structure-morphism conjuncts, precisely so it
would be usable over a field that is not `ℚ`).  It applies verbatim at
`K = ZMod ℓ` and hands over the affine chart `A ∖ {O}` together with its
`𝔽_ℓ`-algebra structure and the range clause.

So *this* declaration is now an ASSEMBLY, and what is left is exactly
Riemann–Roch, isolated in `exists_weierstrassGenerators_of_abelianSchemeChart`
two declarations above.  The three conjuncts of the conclusion are discharged as
follows, none of them mathematics:

* `IsOpenImmersion` — `Spec.map` of a ring ISO is an iso, hence an open
  immersion, and open immersions compose;
* the structure-morphism clause — `Spec.map_comp` against the `𝔽_ℓ`-linearity
  conjunct `he` of the leaf, which is exactly why that conjunct is stated;
* the range clause — the base map of an iso is a homeomorphism, so composing
  with it does not move the range.

The import that makes this possible is deliberately NON-public (see the import
block): nothing in this file's signatures mentions `CurveAffineComplement`. -/
...
1. `exists_weierstrassModel_of_abelianSchemeStruct_finiteField` — **the
   ...
   PROVEN 2026-07-31 through `exists_weierstrassRingEquiv_of_abelianSchemeChart`
   down to `exists_weierstrassGenerators_of_abelianSchemeChart`, which is where
   the Riemann–Roch now lives and is the only open leaf on this side; everything
   scheme-theoretic came from `CurveAffineComplement.lean`, and the two
   commutative-algebra side conditions from `CurveExtension.lean`;

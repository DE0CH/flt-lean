...
**Only TWO of the seven open rows are level-specific**, and neither of
them is Kolyvagin-Logachev: the deep arithmetic sits in a leaf shared with
the `Γ₀` layer, and what is left at level `25` is a computation of twelve
`L`-values.  THREE of the rows are `X0.lean` theorems used verbatim with

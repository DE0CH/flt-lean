...
/-! **`isReduced_geomFibre_nTorsion_of_natCast_ne_zero` USED TO BE DECLARED HERE;
DELETED 2026-07-31 as a DUPLICATE.**

`X0.lean` gained a declaration of the same name on 2026-07-31, and `X1.lean`
`public import`s `X0.lean`, so the two collided (`has already been declared`)
and this file stopped compiling — while `lake build …ModularCurve.X0` stayed
green, which is why nothing caught it before now.  The copy there is STRICTLY
MORE GENERAL (`hn0 : n ≠ 0` where this one asked `hn : 3 ≤ n`) and otherwise
identical line for line, so this one is deleted rather than renamed and the one
call site below passes `(by omega)` for the weaker hypothesis.  See
`X0.lean`'s copy for the statement and the proof. -/
...
/-! **`isOpenImmersion_equalizer_of_etale_nTorsion` USED TO BE DECLARED HERE;
DELETED 2026-07-31 as a DUPLICATE.**

Its own docstring said the right thing — "**It is EXACTLY what `X0.lean`'s
`isOpenImmersion_equalizer_of_nsmul_eq_zero` still owes** … closing it means
MOVING this declaration there; nothing mathematical is left in it."  That MOVE
happened on 2026-07-31 and the copy here was not removed, so the two collided
(`has already been declared`, since this file `public import`s `X0.lean`) and
this file stopped compiling — while `lake build …ModularCurve.X0` stayed green,
which is why nothing caught it before now.

`X0.lean`'s copy is the SAME statement up to the names of two binders
(`ab`/`abs`, `hq`/`hetale`), so this one is deleted rather than renamed; the
consumer below (`isOpenImmersion_equalizer_of_abelianFullLevelStructure`) picks
it up unchanged, the argument list being identical. -/

...
-- pullbacks from `T` — and it is what lets that node be cut into
-- representability of `Pic⁰` plus autoduality.
...
than leaves.  The six open leaves under this node, and the single
theory each one needs, are (`exists_jacobianOf_x0` was itself cut in two
on 2026-07-27 and is now a PROVEN assembly over the first two rows):

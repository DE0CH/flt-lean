...
**Third round (2026-07-27), on the rank half.**
...
`ModularCurve/WeightTwoEigenform.lean`.  The seven open leaves under
this node, and the single theory each one needs, are:
...
Only two of the seven mention `kenkuLevels` at all, and each of the
other five is a named classical theorem stated for an arbitrary object —
which is what makes them dispatchable independently, and reusable by the
rest of the modular-curve subtree. -/

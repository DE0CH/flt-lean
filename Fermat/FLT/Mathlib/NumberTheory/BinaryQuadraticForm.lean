...
So this file has FIVE open leaves (checked against the compiler's `declaration uses 'sorry'`
warning set, 2026-07-29 — and the `sorry` token count with comments stripped is 5 as well, so
there are no anonymous inner sorries): the four class-field-theoretic ones
(`exists_intCubic_weberAlpha`, `intCast_indep_weberAlpha_pow_four`,
`isIntegral_gammaTwo_heegnerPoint`, `exists_rat_gammaTwo_heegnerPoint`) and the single
`η`-product identity `Heegner.eta_two_torsion_key`.

TWO STALE CLAIMS CORRECTED HERE (2026-07-29). This paragraph used to say SIX, and used to
list the Diophantine `eq_of_two_mul_mul_cube_add_one_eq_sq` as open: it was PROVEN in
`1b07f83f` and the header was never updated. It also listed
`Heegner.eta_pow_24_add_eta_two_pow_24` itself, which is now proven; the leaf that replaces
it is `eta_two_torsion_key`, so the count is unchanged at five. -/
module

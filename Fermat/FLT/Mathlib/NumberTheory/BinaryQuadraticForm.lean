...
* `Heegner.natDegree_minpoly_weberAlpha` — the degree of `α` over `ℚ` is exactly `3`
  ...
  PROVEN from it by elementary field theory. Its former companion
...
So this file has FIVE open leaves, regenerated by grepping the merged source rather than
inherited from any earlier version of this list: `natDegree_minpoly_weberAlpha`,
...
that replaced `exists_rat_gammaTwo_heegnerPoint`, and the single modular-form identity
`Heegner.eta_pow_24_add_eta_two_pow_24`.  PROVEN, against earlier versions of this list
which recorded them as open: the Diophantine `eq_of_two_mul_mul_cube_add_one_eq_sq`,
`exists_rat_gammaTwo_heegnerPoint`, `exists_intCubic_weberAlpha`,
`intCast_indep_weberAlpha_pow_four` and `isIntegral_weberAlpha`. -/
module

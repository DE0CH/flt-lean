...
So this file has SIX open leaves — regenerated from the MERGED source, not inherited from
either side: `exists_intCubic_weberAlpha`, `intCast_indep_weberAlpha_pow_four`,
`exists_modularPolynomial` (which replaced `isIntegral_gammaTwo_heegnerPoint`,
now PROVEN), the two class-field leaves `exists_quadratic_jInvariant_heegnerPoint` and

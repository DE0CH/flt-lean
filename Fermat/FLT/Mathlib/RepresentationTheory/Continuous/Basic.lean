...
public import Fermat.FLT.Mathlib.Topology.CompactOpen
...
Vendored VERBATIM from `~/cs/FLT`, file `FLT/Mathlib/RepresentationTheory/Continuous/Basic.lean`
(author Edison Xie). Re-checked against our mathlib pin `a3364fa` on 2026-07-31: absent
upstream here. It imports `FLT.Mathlib.Topology.Constructions` there only for
`continuous_of_discreteTopology_snd`, which is why this file imports our trimmed
`Fermat.FLT.Mathlib.Topology.CompactOpen` instead — see that file's provenance note.

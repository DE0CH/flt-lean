...
public import Fermat.FLT.Mathlib.RepresentationTheory.Homological.ContCohomology.Resolution
...
* `ContRepresentation.cupZeroSucc f hp n`: the degree-`(0, n)` cup product pairing on the
  coinduced resolutions induced by an intertwining map `f : ρ1 →ⁱL linHom ρ2 ρ3`.
...
`FLT/Mathlib/RepresentationTheory/Homological/ContCohomology/CupProduct.lean` (author Edison
Xie), 582 lines and sorry-free there. Its FLT-local import closure was measured on 2026-07-31
at 12 files and 1706 lines; of those, this port takes only what our mathlib pin `a3364fa` does
not already have, namely

* `Fermat.FLT.Mathlib.Topology.Algebra.Module.CompactOpen` (the compact-open topology on
  `M1 →L[k] M2`),
* `Fermat.FLT.Mathlib.Topology.CompactOpen` (`ContinuousMap.continuous_prodMk` for an `R1Space`
  source, plus the single lemma actually wanted from `Topology/Constructions.lean`),
* `Fermat.FLT.Mathlib.Algebra.Category.ModuleCat.Topology.{Basic,Homology}` (the internal hom
  `TopModuleCat.linHom` and the descent of bilinear pairings along cokernels),
* `Fermat.FLT.Mathlib.RepresentationTheory.Continuous.{Basic,TopRep}` (`ContRepresentation.linHom`
  and `TopRep.iHom`),
* `Fermat.FLT.Mathlib.RepresentationTheory.Homological.ContCohomology.Resolution`
  (`resolutionCLM`, `resolutionXCast`, `invariantsObjIHom`),

on top of the `bdryKer`/`cohomologyIsoQuot` port already in the tree since 2026-07-27. The
346 lines of `FLT.Mathlib.Algebra.Algebra.{Hom,Pi}` in that closure are NOT taken: they enter
only through `Topology/Constructions.lean`, and `SemialgHom` occurs zero times in the ported
surface. That drag is the reason the 2026-07-27 provenance note gave for not porting the cup
product; trimming the file to the one lemma needed costs ten lines and removes it entirely.

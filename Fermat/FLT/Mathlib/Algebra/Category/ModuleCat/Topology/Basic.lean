...
public import Fermat.FLT.Mathlib.Topology.Algebra.Module.CompactOpen
...
(`TopModuleCat.homOfBilinear`) bundling a bilinear pairing with jointly continuous uncurried
form into a morphism to the internal hom.

## Provenance

Vendored VERBATIM from `~/cs/FLT`, file
`FLT/Mathlib/Algebra/Category/ModuleCat/Topology/Basic.lean` (author Edison Xie). Our mathlib
pin `a3364fa` already carries `TopModuleCat` itself and its kernels/cokernels — see the audit
in `Fermat/FLT/Mathlib/RepresentationTheory/Homological/ContCohomology/Basic.lean` — so ONLY
the internal hom is vendored, which is exactly what that audit identified as missing. The only
`linHom` in our pin is `Representation.linHom`
(`Mathlib/RepresentationTheory/Basic.lean:659`), the DISCRETE one on `Rep k G`: a different
object on a different category, not this.

...
**A second block (`mem_smoothLocus_comp`, `mem_smoothLocus_of_comp_of_flat`,
`le_preimage_smoothLocus_of_sq`) was added here on 2026-07-31 and DELETED on
2026-07-31 as a duplicate.**  It was the pointwise two-out-of-three calculus for
`smoothLocus`, and the identical calculus had been cut one day earlier, PROVEN
and consumed, into
`Fermat/FLT/Mathlib/AlgebraicGeometry/SmoothLocusDescent.lean`:

| deleted from here | the live twin |
|---|---|
| `mem_smoothLocus_comp` | `Fermat.mem_smoothLocus_comp` (proven) |
| `mem_smoothLocus_of_comp_of_flat` (sorry) | `Fermat.mem_smoothLocus_of_comp_of_smooth` (proven over `Fermat.formallySmooth_of_comp_of_faithfullyFlat`) |
| `le_preimage_smoothLocus_of_sq` | `Fermat.mem_smoothLocus_of_commSq` (proven) |

`X0.lean` imports both files and its `smoothLocus_pairSquareMap_le` calls
`Fermat.mem_smoothLocus_of_commSq`; **the block deleted here had no consumer
anywhere in the tree**, so it was an open leaf that was also DEAD.

The one place the two differed is the hypothesis on the FIRST factor: the copy
here asked only for `Flat p`, the live one asks for `Smooth p`.  That extra
generality is not wanted by any call site — in `smoothLocus_pairSquareMap_le`
the first factor is `Limits.pullback.fst af af` with `af` SMOOTH — and it is
**much** harder: the smooth-first-factor statement descends through the
Jacobi–Zariski sequence, whereas the flat-first-factor statement (Stacks `02VL`
in full) passes through *"`A → B` a flat local homomorphism of Noetherian local
rings with `B` regular ⟹ `A` regular"* (Matsumura 23.7, Stacks `00OJ`), whose
only proof is Serre's criterion `A` regular ⟺ `pd_A k_A < ∞`.  That criterion is
**not at this pin in either direction** — `grep -rn IsRegularLocalRing
.lake/packages/mathlib/Mathlib` gives only `RegularLocalRing/{Defs,Polynomial}`,
and `Mathlib/RingTheory/Regular/ProjectiveDimension.lean` has only
`projectiveDimension_quotient_eq_length` for a regular *sequence*, with no
Auslander–Buchsbaum and no `pd(k) < ∞ ⟹ regular`.  Adding a `PerfectField` or
`CharZero` hypothesis does **not** help: the regularity descent is needed over
the residue field whatever its characteristic.
...
/-! ## DELETED 2026-07-31: the pointwise two-out-of-three calculus for `smoothLocus`

Deleted as a duplicate; see the table in the module docstring above.  The three
declarations that stood here — `mem_smoothLocus_comp`,
`mem_smoothLocus_of_comp_of_flat` (a `sorry` leaf) and
`le_preimage_smoothLocus_of_sq` — are superseded by the PROVEN
`Fermat.mem_smoothLocus_comp`, `Fermat.mem_smoothLocus_of_comp_of_smooth` and
`Fermat.mem_smoothLocus_of_commSq` in
`Fermat/FLT/Mathlib/AlgebraicGeometry/SmoothLocusDescent.lean`, and none of the
three had a consumer anywhere in the tree.

The single residual leaf of that calculus is
`Fermat.formallySmooth_of_comp_of_faithfullyFlat`, in that file.  Half of it —
`Module.Projective S Ω[S⁄R]` — is now proven there; what is left is
`Fermat.subsingleton_h1Cotangent_of_faithfullyFlat_of_formallySmooth`, the left
end of the Jacobi–Zariski sequence.
-/

...
The two-step route this docstring used to record — `f₂` irreducible over the non-UFD
`B[Px] ⧸ (f₁)`, "the real work", i.e. the generic Weierstrass cubic staying irreducible
after adjoining a generic point of itself — is **NOT NEEDED**, and is not the route taken.

Note also that only a WEAKER statement is actually consumed below: that
`addZ ucurve upt₁ upt₂` is a non-zerodivisor in `Univ`. -/
theorem idl_isPrime : idl.IsPrime := by
  refine ⟨idl_ne_top, fun {x y} hxy => ?_⟩
  obtain ⟨k, hk | hk⟩ := exists_pow_Pz_mul_mem_idl hxy

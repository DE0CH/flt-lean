...
It also carries the **second Bosma-Lenstra addition law** `add2XYZ`, the law of
the line `Y = 0`, which together with mathlib's `addXYZ` (the law of `Z = 0`)
forms a COMPLETE SYSTEM: `add2XYZ_of_infty_left` and `add2XYZ_neg` are the halves
of `hunit` and `hinv` that the standard law cannot supply, and
`add2X_mul_addZ` / `add2Y_mul_addZ` are the proportionality identities that make
the two charts agree on their overlap.  See the section docstring below for the
derivation and for how to regenerate the polynomials.
...
## Cross-check against the plan recorded in `EllipticScheme.lean` (2026-07-28)

`exists_projMul`'s docstring in `Fermat/FLT/ModularCurve/EllipticScheme.lean`
records an independent Magma derivation of the same law, with `add2Z` *defined*
as `addY P (negOf Q)`.  The two agree, and more sharply than "modulo the ideal":
re-running the derivation in `Singular` over `ℚ(a₁, …, a₆)[Px, …, Qz]` — where
`{W(P), W(Q)}` really is a Gröbner basis, lead terms `-Px³` and `-Qx³` — gives

* `add2Z - addY P (negOf Q)` reduces to `0`, and no nonzero bidegree-`(2,2)` form
  lies in the ideal (its generators have bidegree `(3,0)` and `(0,3)`, so a
  cofactor would need negative degree), so the two are **equal on the nose**;
* `addZ - addZ P (negOf Q)` reduces to `0` likewise, confirming `subZ ≡ addZ`;
* `add2X * addZ - addX * (addY P (negOf Q))` reduces to `0`, i.e. the pinning
  identity that the `EllipticScheme` docstring solves for.

So the two derivations, by different routes in different CAS, land on the same
polynomials, and `hinv` really is the diagonal non-degeneracy of law 2.

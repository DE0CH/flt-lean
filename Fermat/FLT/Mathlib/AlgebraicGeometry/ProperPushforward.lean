...
* `isIso_appTop_of_isIso_appTop_fiber` — **PROVEN** (2026-07-28): degree-zero cohomology and
  ...
  * `finiteType_appTop_of_isProper` — **PROVEN** (2026-07-28) over one residual leaf:
    `Γ(X, ⊤)` is a finite-TYPE `Γ(S, ⊤)`-algebra.  Degree-zero Grothendieck finiteness is
    **fully proven over any noetherian universally Japanese (Nagata) base** — which is every
    base this development feeds in — by
    `finiteType_appTop_of_isProper_of_universallyJapanese`, from `Mathlib`'s
    `isIntegral_appTop_of_universallyClosed` plus a finite affine cover and the sheaf axiom.
    The residual leaf is `finiteType_appTop_of_isProper_of_not_universallyJapanese`, the
    complement, where Chow's lemma (noetherian non-Nagata) or spreading out to a noetherian
    base (non-noetherian) becomes unavoidable.  **The missing input is Nagata theory — pure
    commutative algebra — not Chow's lemma**; see `IsUniversallyJapanese`.
...
/-- **`R` IS UNIVERSALLY JAPANESE (NAGATA)**: the integral closure of `R` in every finite-type
`R`-algebra is a finite `R`-module.

This is the classical *universally Japanese* / Nagata condition, restricted to what the
degree-zero finiteness argument below actually consumes.  Every excellent ring satisfies it —
in particular `ℤ`, every field, every complete local ring, every Dedekind domain of
characteristic zero, and every finite-type algebra over any of these, so every base this
development feeds in.

`Mathlib` has no Nagata theory at this pin (grep `IsNagata` / `Japanese` in
`Mathlib/RingTheory/` — the only `Nagata` hit is an attribution in
`NoetherNormalization.lean`), and its finiteness of an integral closure,
`IsIntegralClosure.isNoetherian`, needs `[IsIntegrallyClosed A]` together with a *separable
field extension*, so it does not apply to a finite-type algebra.  That is why this appears
here as a hypothesis rather than as a theorem: it is the single missing input, and it is pure
commutative algebra.

**The hypothesis is not vacuous and not trivial.** It fails for general noetherian `R`: take
`R` a `1`-dimensional noetherian local domain whose integral closure `R'` in `K = Frac R` is
not a finite `R`-module (Nagata), and `t ≠ 0` in the maximal ideal.  Inverting `t` kills the
only nonzero prime, so `B := R[1/t] = K` is generated by **one** element over `R`, hence of
finite type, while `integralClosure R B = R'` is not finite.  So this really is an extra
assumption on the base, and the residual leaf below — the non-Nagata case — is not empty. -/
abbrev IsUniversallyJapanese (R : Type u) [CommRing R] : Prop :=
  ∀ (B : Type u) [CommRing B] [Algebra R B],
    Algebra.FiniteType R B → Module.Finite R (integralClosure R B)

/-- **PROVEN**: an `R`-algebra `A` that is integral over a noetherian `R` and embeds into a
FINITE family of `R`-algebras whose integral closures are finite `R`-modules is itself a
finite `R`-module.

This is the commutative-algebra core of degree-zero Grothendieck finiteness.  Note it takes a
finite family rather than a single ambient algebra, which is what lets the geometric side use
a finite affine cover directly: `Mathlib` has `Algebra.FiniteType.prod` only for BINARY
products and no `Pi` version, whereas `isNoetherian_pi` gives the module-level statement for
an arbitrary finite index type for free.  Mapping into a product of finite MODULES rather than
into a finite-type ALGEBRA is what avoids that gap. -/
theorem finite_of_injective_into_pi
    {R A : Type*} [CommRing R] [CommRing A] [Algebra R A] [_root_.IsNoetherianRing R]
    [Algebra.IsIntegral R A]
    {ι : Type*} [Finite ι] {B : ι → Type*} [∀ i, CommRing (B i)] [∀ i, Algebra R (B i)]
    (hB : ∀ i, Module.Finite R (integralClosure R (B i)))
    (ρ : ∀ i, A →ₐ[R] B i)
    (hinj : Function.Injective (fun (a : A) (i : ι) => ρ i a)) :
    Module.Finite R A := by
  have hnoeth : ∀ i, _root_.IsNoetherian R (integralClosure R (B i)) := by
    intro i; haveI := hB i; infer_instance
  haveI := hnoeth
  have : _root_.IsNoetherian R (∀ i, integralClosure R (B i)) := isNoetherian_pi
  let φ : A →ₗ[R] (∀ i, integralClosure R (B i)) :=
    { toFun := fun a i => ⟨ρ i a, (Algebra.IsIntegral.isIntegral (R := R) a).map (ρ i)⟩
      map_add' := by intro a b; funext i; ext; simp
      map_smul' := by intro r a; funext i; ext; simp }
  have hφ : Function.Injective φ := by
    intro a b hab
    exact hinj (funext fun i => congrArg Subtype.val (congrFun hab i))
  have : _root_.IsNoetherian R A := isNoetherian_of_injective φ hφ
  exact Module.IsNoetherian.finite R A

/-- **DEGREE-ZERO GROTHENDIECK FINITENESS OVER A NAGATA BASE — PROVEN** (2026-07-28).

`Γ(X, ⊤)` is a finite-type `Γ(S, ⊤)`-algebra for `f` proper over an affine base, provided the
base ring is noetherian and universally Japanese.  Every step is machinery already present at
this pin; the only thing that was missing is `IsUniversallyJapanese`:

1. `f` proper ⟹ `UniversallyClosed f`, and `S` affine, so `Γ(X, ⊤)` is **integral** over
   `Γ(S, ⊤)` — `Mathlib`'s `isIntegral_appTop_of_universallyClosed`.  This is the one
   genuinely geometric input and it is free.
2. `f` proper ⟹ `QuasiCompact f`, so `CompactSpace X` and `X.affineCover.finiteSubcover` is a
   FINITE affine cover.
3. `f` locally of finite type ⟹ each `Γ(X, Uᵢ)` is a finite-type `Γ(S, ⊤)`-algebra
   (`f.finiteType_appLE`), so `H` makes each `integralClosure Γ(S,⊤) Γ(X, Uᵢ)` a finite
   module.
4. The sheaf axiom (`eq_of_locally_eq'`) makes the restrictions `Γ(X, ⊤) ⟶ Γ(X, Uᵢ)` jointly
   injective.
5. `finite_of_injective_into_pi` concludes, and `RingHom.FiniteType.of_finite` converts.

**NOTE WHICH HYPOTHESES ARE ABSENT.**  This proof uses neither `[Flat f]` nor
`[LocallyOfFinitePresentation f]` — the compiler confirms it, since they are not binders here.
Those two are needed only for the non-noetherian generality of Stacks 0B91, where flatness of
`𝒪_X` over `S` is what drives the limit argument.  So over a noetherian Nagata base they are
genuinely unnecessary, which is a sharper statement than "a prover over a noetherian base may
ignore them". -/
theorem finiteType_appTop_of_isProper_of_universallyJapanese
    (f : X ⟶ S) [IsAffine S] [IsProper f]
    [_root_.IsNoetherianRing ↥Γ(S, ⊤)] (H : IsUniversallyJapanese ↥Γ(S, ⊤)) :
    f.appTop.hom.FiniteType := by
  letI : Algebra ↥Γ(S, ⊤) ↥Γ(X, ⊤) := f.appTop.hom.toAlgebra
  haveI : Algebra.IsIntegral ↥Γ(S, ⊤) ↥Γ(X, ⊤) :=
    Algebra.isIntegral_def.mpr (isIntegral_appTop_of_universallyClosed f)
  haveI : CompactSpace X := (quasiCompact_iff_compactSpace f).mp inferInstance
  set 𝒰 := X.affineCover.finiteSubcover with h𝒰
  set U : 𝒰.I₀ → X.Opens := fun i => (𝒰.f i).opensRange with hU
  have hUaff : ∀ i, IsAffineOpen (U i) := fun i => isAffineOpen_opensRange _
  have hUtop : ⨆ i, U i = ⊤ := 𝒰.iSup_opensRange
  have hle : ∀ i, U i ≤ f ⁻¹ᵁ ⊤ := fun i => by simp
  letI : ∀ i, Algebra ↥Γ(S, ⊤) ↥Γ(X, U i) :=
    fun i => (f.appLE ⊤ (U i) (hle i)).hom.toAlgebra
  have hB : ∀ i, Module.Finite ↥Γ(S, ⊤) (integralClosure ↥Γ(S, ⊤) ↥Γ(X, U i)) := by
    intro i
    exact H _ (f.finiteType_appLE (isAffineOpen_top _) (hUaff i) (hle i))
  let ρ : ∀ i, ↥Γ(X, ⊤) →ₐ[↥Γ(S, ⊤)] ↥Γ(X, U i) := fun i =>
    { (X.presheaf.map (homOfLE (le_top : U i ≤ ⊤)).op).hom with
      commutes' := fun r => rfl }
  have hinj : Function.Injective (fun (a : ↥Γ(X, ⊤)) i => ρ i a) := by
    intro a b hab
    refine X.sheaf.eq_of_locally_eq' U ⊤ (fun i => homOfLE le_top) (by rw [hUtop]) a b ?_
    intro i
    exact congrFun hab i
  exact RingHom.FiniteType.of_finite (finite_of_injective_into_pi hB ρ hinj)

/-- **THE RESIDUAL LEAF — the base is NOT noetherian-and-universally-Japanese** (LEAF,
2026-07-28).

`finiteType_appTop_of_isProper_of_universallyJapanese` above discharges degree-zero
Grothendieck finiteness whenever `Γ(S, ⊤)` is noetherian and Nagata, which covers every base
this development actually feeds in.  What is left is exactly the complement, and it is where
the classical machinery becomes unavoidable:

* if `Γ(S, ⊤)` is noetherian but **not** universally Japanese, the subalgebra route provably
  fails (see the counterexample in `IsUniversallyJapanese`'s docstring), and the proof must go
  through **Chow's lemma** and dévissage (Stacks 02O5), or the projective case plus higher
  cohomology.  `Mathlib` has none of these at this pin: zero hits for `Chow`, no
  projective-morphism class (only `AlgebraicGeometry.Proj` proper over `Spec 𝒜₀`), and no
  higher direct images of quasi-coherent sheaves at all;
* if `Γ(S, ⊤)` is **not noetherian**, one must first spread out to a noetherian base
  (Stacks 0B91, which is where `[Flat f]` and `[LocallyOfFinitePresentation f]` earn their
  place).  `Mathlib/AlgebraicGeometry/SpreadingOut.lean` does **not** provide this: it spreads
  out morphisms on *stalks* and has no limits of schemes.
...
f]` it is of finite presentation, and `𝒪_X` is a finitely presented `𝒪_X`-module flat over `S`
exactly because `[Flat f]`.  That is precisely the hypothesis set of **Stacks 0B91**, whose
`i = 0` case over an affine base is this statement.  The negated hypothesis is therefore not
what makes it provable — it only records which half of the problem is already done.

**The cheapest way to close this leaf is not to attack it directly**: prove
`IsUniversallyJapanese R` for noetherian `R` in the generality this development needs (or,
better, build Nagata theory properly), and the noetherian half of this leaf collapses into
the theorem above. -/
theorem finiteType_appTop_of_isProper_of_not_universallyJapanese
    (f : X ⟶ S) [IsAffine S]
    [IsProper f] [Flat f] [LocallyOfFinitePresentation f]
    (H : ¬ (_root_.IsNoetherianRing ↥Γ(S, ⊤) ∧ IsUniversallyJapanese ↥Γ(S, ⊤))) :
    f.appTop.hom.FiniteType :=
  sorry

/-- **LEAF 1a — `Γ(X, ⊤)` IS A FINITE-TYPE `Γ(S, ⊤)`-ALGEBRA** — **PROVEN** (2026-07-28) over
the single residual leaf `finiteType_appTop_of_isProper_of_not_universallyJapanese`, by
splitting on whether the base is noetherian and universally Japanese.  Grothendieck's
finiteness theorem in degree `0` (EGA III 3.2.1, Stacks 02O5 / 0B91) once the integral half is
discharged.

**The Nagata branch is fully proven** — `finiteType_appTop_of_isProper_of_universallyJapanese`
— so every base this development actually feeds in is covered.  The case split is what keeps
that proof in the root cone rather than free-floating; the residual leaf is the complement,
not a restatement of the whole problem.  Everything below records how that branch was found
and why the complement is genuinely harder.
...
**BUT THE SAME ROUTE CLOSES THIS LEAF OVER A NAGATA BASE — AND IT IS NOW IMPLEMENTED, NOT
PROPOSED** (2026-07-28).  The counterexample above is the *only* thing standing in the way,
and it is defeated by one standard hypothesis on the base.  The steps below are the proof of
`finiteType_appTop_of_isProper_of_universallyJapanese`, which the compiler accepts; they are
recorded here because they are what rescoped this leaf.  Write `R := Γ(S, ⊤)`,
`A := Γ(X, ⊤)`.  Every step is machinery already available at this pin:
...
hit, discharging `IsUniversallyJapanese ↥Γ(S, ⊤)` from it collapses the noetherian half of
the residual leaf immediately — steps 1–5 are already written and proven here, so nothing
else has to be built.

**What the next owner should actually do**, in order of cost: (a) prove
`IsUniversallyJapanese R` for the concrete noetherian bases this development uses (`ℤ`,
fields, complete local rings, and finite-type algebras over them) — each is a standard
Nagata-theory statement and needs no geometry; (b) failing that, build `IsNagataRing`
properly.  Only after both are exhausted does Chow's lemma become the right target, and it
would still only be needed for the *non*-Nagata noetherian case.
...
theorem finiteType_appTop_of_isProper (f : X ⟶ S) [IsAffine S]
    [IsProper f] [Flat f] [LocallyOfFinitePresentation f] :
    f.appTop.hom.FiniteType := by
  by_cases hR : _root_.IsNoetherianRing ↥Γ(S, ⊤) ∧ IsUniversallyJapanese ↥Γ(S, ⊤)
  · haveI := hR.1
    exact finiteType_appTop_of_isProper_of_universallyJapanese f hR.2
  · exact finiteType_appTop_of_isProper_of_not_universallyJapanese f hR

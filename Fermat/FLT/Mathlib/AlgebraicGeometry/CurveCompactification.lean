...
theorem nonempty_projChart_of_surjective {B B' : CommRingCat.{u}}
  ...
  letI := C.finiteType
  letI := C.zeroFinite
  have hisosurj : Function.Surjective (C.awayIso.hom.hom) := fun x =>
    ⟨C.awayIso.inv.hom x, by simp⟩
  set π : HomogeneousLocalization.Away C.grading C.f →+* ↑B :=
    q.hom.comp C.awayIso.hom.hom with hπdef
  have hπ : Function.Surjective π := hq.comp hisosurj
  refine ProjChartDescent.nonempty_projChart_aux C.grading C.f_deg π hπ (b' ≫ q) ?_
  intro r
  have hcompat := congrArg (fun (g : CommRingCat.of K ⟶ B') => g.hom r) C.compat
  show q.hom (b'.hom r) = _
  rw [← hcompat]
  rw [hπdef]
  simp only [RingHom.comp_apply]
  congr 1
  simp only [CommRingCat.hom_comp, RingHom.comp_apply, CommRingCat.hom_ofHom]
  rw [ProjChartDescent.fromZero_eq_mk1 C.grading C.f_deg r]

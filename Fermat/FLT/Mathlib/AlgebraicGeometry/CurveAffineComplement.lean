...
`K[X][Y] → R`, `X ↦ x`, `Y ↦ y`, `C ↦ algebraMap K R`; `hrel` is exactly the statement
that `E.toAffine.polynomial` evaluates to `0` there, and `hgen` says the image subring is
everything.

## BASE GENERALISATION 2026-07-30: `ℚ` ⇝ an arbitrary commutative ring `K`

The statement used to read over `ℚ` with a *bare* structure map `c : ℚ →+* R` supplied as
an argument, and its docstring justified that with "a ring homomorphism out of `ℚ` is
unique when it exists, so there is no freedom for an adversary to supply a wrong `c`".
That justification is `ℚ`-specific and evaporates over a general base — `K` may have
automorphisms, and a chart twisted by one would satisfy every other clause.  So `c` is now
the structure map `algebraMap K R` of an `Algebra K R` instance the caller supplies, and
the conclusion is upgraded from a bare `→+*` to a `K`-ALGEBRA map `→ₐ[K]`.  Over `ℚ` the
two coincide (`ℚ` is initial in `CommRing`), which is why the old form was adequate there.

The `→ₐ[K]` strengthening is free — `AdjoinRoot.lift` of a `K`-algebra map is one — and it
is exactly what a consumer needs to transport a structure-morphism compatibility
`ι ≫ f = Spec.map (ofHom (algebraMap K R))` across the resulting isomorphism.
...
NOT VACUOUS: `R = E.toAffine.CoordinateRing` itself, with `x = X`, `y = Y`, satisfies both
hypotheses and the conclusion holds with the identity. -/
theorem exists_surjective_coordinateRingHom_of_generators {K : Type u} [CommRing K]
    ...
    (hrel : y ^ 2 + (algebraMap K R E.a₁ * x + algebraMap K R E.a₃) * y
      = x ^ 3 + algebraMap K R E.a₂ * x ^ 2 + algebraMap K R E.a₄ * x + algebraMap K R E.a₆)
    (hgen : Subring.closure (Set.range (algebraMap K R) ∪ {x, y}) = ⊤) :
    ∃ φ : E.toAffine.CoordinateRing →ₐ[K] R, Function.Surjective φ := by
  classical
  set i : K[X] →+* R := Polynomial.eval₂RingHom (algebraMap K R) x with hi
  ...
  have hsurj : Function.Surjective (AdjoinRoot.lift i y hroot) := by
  ...
  refine ⟨{ AdjoinRoot.lift i y hroot with commutes' := ?_ }, hsurj⟩
  intro c
  show AdjoinRoot.lift i y hroot (algebraMap K E.toAffine.CoordinateRing c) = _

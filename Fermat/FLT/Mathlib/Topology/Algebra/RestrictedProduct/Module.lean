...
public import Fermat.FLT.Mathlib.Algebra.Module.Submodule.Basic
...
/-- VENDORING NOTE (2026-07-27): in the reference project this instance is anonymous,
and Lean auto-named it `RestrictedProduct.instModuleCoe_fLT` — the `_fLT` suffix being
derived from the root module name `FLT`. Vendoring under `Fermat.FLT.*` changes that
suffix, so the one place that referred to the generated name
(`Fermat/FLT/DedekindDomain/FiniteAdeleRing/BaseChange.lean`) could no longer find it.
Naming it explicitly makes the reference stable under any future relocation. -/
instance instModuleRestrictedProduct :
    Module (Πʳ i, [R i, B i]_[ℱ]) (Πʳ i, [M i, C i]_[ℱ]) where

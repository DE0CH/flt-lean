...
Vendored VERBATIM from the reference project `~/cs/FLT`, file
`FLT/Mathlib/Topology/Algebra/Module/CompactOpen.lean` (author Edison Xie), as part of the
cup-product port recorded on
`Fermat/FLT/Mathlib/RepresentationTheory/Homological/ContCohomology/CupProduct.lean`.
Checked against our mathlib pin `a3364fa` on 2026-07-31: `ContinuousLinearMap.CompactOpen`
does not exist upstream here (`grep -rn "namespace ContinuousLinearMap.CompactOpen" Mathlib/`
is empty), so this is not a duplicate. No proof change was needed.

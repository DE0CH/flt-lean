...
Authors: Edison Xie, Yaël Dillies, Kevin Buzzard, Salvatore Mercuri, Pietro Monticone, Claude
-/
module
...
and that a map out of `X × D` with `D` discrete is continuous as soon as all its slices are.

## Provenance

`ContinuousMap.continuous_prodMk` is vendored VERBATIM from `~/cs/FLT`, file
`FLT/Mathlib/Topology/CompactOpen.lean`; `continuous_of_discreteTopology_snd` is vendored
VERBATIM from `~/cs/FLT`, file `FLT/Mathlib/Topology/Constructions.lean`. Only that one
lemma is taken from `Topology/Constructions.lean`: the rest of that file is `SemialgHom`
material (`FLT.Mathlib.Algebra.Algebra.{Hom,Pi}`, 346 lines) which the cup product never
touches — `SemialgHom` occurs zero times in the whole ported surface — so vendoring the file
rather than the lemma would have dragged in an unrelated dependency for nothing. That is the
`SemialgHom` drag which
`Fermat/FLT/Mathlib/RepresentationTheory/Homological/ContCohomology/Basic.lean`'s provenance
note gave as a reason not to port the cup product at all; trimmed here, it costs ten lines.

Checked against our mathlib pin `a3364fa` on 2026-07-31: neither lemma exists upstream here.

Material destined for `Mathlib.Topology.CompactOpen` and `Mathlib.Topology.Constructions`.
-/

...
So exactly **one** statement is open in this file as of release 24 (2026-07-30), and it is
Hopf-theoretic on the `A`-side:
...
`Module.nonempty_basis_of_flat_of_finrank_eq_of_isLocalRing_base`
(`Fermat/FLT/Mathlib/RingTheory/LocalProperties/Semilocal.lean`), in
...
and this is the only place the `[IsLocalRing R]` hypothesis is spent. The three steps —
...
— are packaged as `Module.nonempty_basis_of_flat_of_finrank_eq_of_isLocalRing_base` in
`Fermat/FLT/Mathlib/RingTheory/LocalProperties/Semilocal.lean`, which is where the semilocality
criterion `finite_maximalSpectrum_of_isLocalRing_of_module_finite` now lives (it was moved out of
this file on 2026-07-31, having no Hopf content). All that is left here is that the index set is
`Fin (rk_R A'')`, reindexed to `Module.Free.ChooseBasisIndex R A''` by

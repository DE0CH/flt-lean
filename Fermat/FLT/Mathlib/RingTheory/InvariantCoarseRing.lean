...
* `isDomain_tensorProduct_of_algebraicClosure_eq_bot` — the field-theoretic
  ...
  domain for every `K/k`.  **FULLY PROVEN 2026-07-29**: the algebraic case by
  linear disjointness, the transcendental case by
...
the general case** (PROVEN 2026-07-29, closing the last leaf of this module).
...
enters this proof twice: through `PerfectField k` in
`linearDisjoint_of_finiteDimensional_of_algebraicClosure_eq_bot` (the algebraic case),
and through `Infinite k` in the specialisation argument — over `𝔽_q` the polynomial
`X^q - X` vanishes at every `k`-point without being zero, so the values lemma fails
exactly where separability does. -/

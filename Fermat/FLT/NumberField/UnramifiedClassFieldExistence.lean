...
* `NumberField.exists_unramifiedAbelian_card_classGroup_le_finrank` — PROVEN 2026-07-31
  from the three leaves listed at the end of this block. The
...
Two, and they are the whole of what this file owes. Regenerate rather than quote — the
...
**The COMPOSITUM node is fully closed as of 2026-07-31.** All three of its clauses —
`isAbelianGalois_sup`, `isUnramifiedAtInfinitePlaces_sup`, `isUnramifiedAt_ringOfIntegers_sup`
— are proven over the single engine `forall_mem_sup_of_fixed` plus, in the two ramification
cases, a mathlib "unramified ⟺ the relevant decomposition/inertia group is trivial"
dictionary (`InfinitePlace.isUnramified_iff_stabilizer_eq_bot` and
`Ideal.card_inertia_eq_ramificationIdxIn`). That is why all three acquired `[IsGalois K Lᵢ]`
when they were proven, at zero cost to the consumer; see their docstrings.
...
omit [NumberField K] in
/-- **THE ACTION OF `Gal(K̄/K)` ON AN INTERMEDIATE FIELD IS COMPATIBLE WITH INCLUSION OF
INTERMEDIATE FIELDS** (PROVEN 2026-07-31).

`AlgEquiv.restrictNormal_commutes` relates the action on `E` to the action on the AMBIENT
`AlgebraicClosure K`; this relates the action on `E` to the action on an intermediate `F`
containing it, for the tower `E ⊆ F` built by hand from `IntermediateField.inclusion`. The
proof is that lemma twice, at `E` and at `F`, compared inside `AlgebraicClosure K`.

**The coercion compatibility is handed in as an explicit hypothesis (`hcomp`) rather than
derived from an `IsScalarTower`**, deliberately: at every call site it is `rfl`, while the
scalar-tower route would need the instance to be built and then unfolded again. This is the
standing "pass a factorisation equation as an EXPLICIT argument" idiom — an explicit
argument is checked up to defeq, an instance is not.

This lemma and its two corollaries below are what let the three clauses of
`exists_unramifiedAbelian_sup` all run through `forall_mem_sup_of_fixed`: the compositum
`L₁ ⊔ L₂` is an intermediate field of `AlgebraicClosure K`, not an intermediate field of a
fixed ambient number field, so `ArtinSymbol.lean`'s `algebraMap_restrictNormalHom_smul` —
which is this statement with `F` the ambient field — does not apply. -/
theorem algebraMap_restrictNormalHom_of_le
    (E F : IntermediateField K (AlgebraicClosure K)) [Normal K E] [Normal K F]
    [Algebra ↥E ↥F]
    (hcomp : ∀ x : ↥E, (algebraMap ↥E ↥F x : AlgebraicClosure K) = (x : AlgebraicClosure K))
    (ρ : AlgebraicClosure K ≃ₐ[K] AlgebraicClosure K) (x : ↥E) :
    algebraMap ↥E ↥F (AlgEquiv.restrictNormalHom E ρ x)
      = AlgEquiv.restrictNormalHom F ρ (algebraMap ↥E ↥F x) := by
  have hE := AlgEquiv.restrictNormal_commutes ρ (E : IntermediateField K (AlgebraicClosure K)) x
  ...
  rw [hcomp]
  have h1 : ((AlgEquiv.restrictNormalHom E ρ x : ↥E) : AlgebraicClosure K)
      = ρ ((x : ↥E) : AlgebraicClosure K) := by simpa using hE
  have h2 : ((AlgEquiv.restrictNormalHom F ρ (algebraMap ↥E ↥F x) : ↥F) :
      AlgebraicClosure K) = ρ ((algebraMap ↥E ↥F x : ↥F) : AlgebraicClosure K) := by
    simpa using hF
  rw [h1, h2, hcomp]

omit [NumberField K] in
/-- **… ON THE RINGS OF INTEGERS** (PROVEN 2026-07-31), the corollary the finite-prime
clause uses. `coe_smul_ringOfIntegers` (`ArtinSymbol.lean`) says the action on `𝓞 L` is the
restriction of the action on `L`, so after `RingOfIntegers.ext` this is the lemma above. -/
theorem restrictNormalHom_smul_ringOfIntegers
  ...
  exact algebraMap_restrictNormalHom_of_le K E F hcomp ρ (y : ↥E)

omit [NumberField K] in
/-- **… ON THE INFINITE PLACES** (PROVEN 2026-07-31), the corollary the archimedean clause
uses: restriction to `E` intertwines the action of `Gal(F/K)` on `InfinitePlace F` with the
action of `Gal(E/K)` on `InfinitePlace E`, along `InfinitePlace.comap`.

`InfinitePlace.smul_eq_comap` turns both sides into a `comap` and `InfinitePlace.comap_comp`
turns the two composites into one, so all that is left is the lemma above at `ρ⁻¹` — which
is where `map_inv` for the monoid hom `AlgEquiv.restrictNormalHom` enters, and why the
statement is about `•` rather than about `AlgEquiv.symm` applied by hand. -/
theorem comap_smul_restrictNormalHom
    (E F : IntermediateField K (AlgebraicClosure K)) [Normal K E] [Normal K F]
    [Algebra ↥E ↥F]
    (hcomp : ∀ x : ↥E, (algebraMap ↥E ↥F x : AlgebraicClosure K) = (x : AlgebraicClosure K))
    (ρ : AlgebraicClosure K ≃ₐ[K] AlgebraicClosure K) (w : InfinitePlace ↥F) :
    (AlgEquiv.restrictNormalHom F ρ • w).comap (algebraMap ↥E ↥F)
      = AlgEquiv.restrictNormalHom E ρ • (w.comap (algebraMap ↥E ↥F)) := by
  rw [InfinitePlace.smul_eq_comap, InfinitePlace.smul_eq_comap,
    ← InfinitePlace.comap_comp, ← InfinitePlace.comap_comp]
  congr 1
  refine RingHom.ext fun x => ?_
  show (AlgEquiv.restrictNormalHom F ρ).symm (algebraMap ↥E ↥F x)
    = algebraMap ↥E ↥F ((AlgEquiv.restrictNormalHom E ρ).symm x)
  have := algebraMap_restrictNormalHom_of_le K E F hcomp ρ⁻¹ x
  simpa [map_inv] using this.symm

/-- **THE COMPOSITUM OF TWO GALOIS EXTENSIONS UNRAMIFIED AT THE INFINITE PLACES IS
UNRAMIFIED AT THE INFINITE PLACES** (PROVEN 2026-07-31; cut 2026-07-31 out of
`exists_unramifiedAbelian_sup` below).

Pure archimedean ramification theory, no class field theory, and the ARCHIMEDEAN instance
of `forall_mem_sup_of_fixed` above. `IsUnramifiedAtInfinitePlaces K L` says every infinite
place of `L` is unramified over `K`, i.e. no real place of `K` becomes complex.

**The whole proof is `InfinitePlace.isUnramified_iff_stabilizer_eq_bot`**, which for a
GALOIS extension turns "`w` is unramified" into "the decomposition group at `w` is trivial"
— so the clause becomes the same triviality statement the other two clauses are, and the
same three lines close it: an element of `Stab w ≤ Gal(M/K)` is lifted to `ρ ∈ Gal(K̄/K)`,
its restriction to `Lᵢ` stabilises `w ∩ Lᵢ` (this is `comap_smul_restrictNormalHom` above),
hence is `1` because `Lᵢ` is unramified at the infinite places, hence `ρ` fixes `L₁` and
`L₂` pointwise, hence the compositum pointwise.

**⚠ `[IsGalois K L₁]` AND `[IsGalois K L₂]` WERE ADDED TO THIS STATEMENT ON 2026-07-31**,
for the same two reasons as on `isUnramifiedAt_ringOfIntegers_sup` below and at the same
zero cost to the consumer (`IsAbelianGalois` extends `IsGalois`, the hypotheses are
instance-implicit, the call site did not move): the argument restricts `ρ` to `Lᵢ`, which
needs `Normal K Lᵢ`, and `isUnramified_iff_stabilizer_eq_bot` itself carries `[IsGalois]`.
Unlike the finite-prime clause, here the Galois hypothesis is NOT merely an artefact of the
route — over a non-Galois `Lᵢ` the decomposition group is not a subgroup of a Galois group
at all, and the classical statement is proved through the real embeddings instead.

**Cheap sufficient cases worth knowing, none of which reaches the compositum**: mathlib's
`IsUnramifiedAtInfinitePlaces_of_odd_finrank` settles it whenever `[M : K]` is odd, and
`IsUnramifiedAtInfinitePlaces.bot` handles the tower direction.

**The check that would have refuted it**: two extensions of a number field `K`, each
unramified at every infinite place, whose compositum has a real place of `K` becoming
complex. There is none. -/
theorem isUnramifiedAtInfinitePlaces_sup (L₁ L₂ : IntermediateField K (AlgebraicClosure K))
  ...
  have hc2 : ∀ x : ↥L₂, (algebraMap ↥L₂ ↥(L₁ ⊔ L₂) x : AlgebraicClosure K)
      = (x : AlgebraicClosure K) := fun _ => rfl
  refine ⟨fun w => ?_⟩
  rw [InfinitePlace.isUnramified_iff_stabilizer_eq_bot, Subgroup.eq_bot_iff_forall]
  intro σ hσ
  obtain ⟨ρ, rfl⟩ := AlgEquiv.restrictNormalHom_surjective (AlgebraicClosure K) σ
  have hsm : AlgEquiv.restrictNormalHom (↥(L₁ ⊔ L₂)) ρ • w = w := hσ
  ...
  · refine fixed_of_restrictNormalHom_eq_one K L₁ ρ ?_
    have hst : MulAction.stabilizer (↥L₁ ≃ₐ[K] ↥L₁)
        (w.comap (algebraMap ↥L₁ ↥(L₁ ⊔ L₂))) = ⊥ :=
      InfinitePlace.isUnramified_iff_stabilizer_eq_bot.mp (InfinitePlace.isUnramified K _)
    have hmem : AlgEquiv.restrictNormalHom L₁ ρ ∈ MulAction.stabilizer (↥L₁ ≃ₐ[K] ↥L₁)
        (w.comap (algebraMap ↥L₁ ↥(L₁ ⊔ L₂))) := by
      show AlgEquiv.restrictNormalHom L₁ ρ • _ = _
      rw [← comap_smul_restrictNormalHom K L₁ (L₁ ⊔ L₂) hc1 ρ w, hsm]
    rw [hst] at hmem
    simpa using hmem
  · refine fixed_of_restrictNormalHom_eq_one K L₂ ρ ?_
    have hst : MulAction.stabilizer (↥L₂ ≃ₐ[K] ↥L₂)
    ...
    have hmem : AlgEquiv.restrictNormalHom L₂ ρ ∈ MulAction.stabilizer (↥L₂ ≃ₐ[K] ↥L₂)
        (w.comap (algebraMap ↥L₂ ↥(L₁ ⊔ L₂))) := by
      show AlgEquiv.restrictNormalHom L₂ ρ • _ = _
      rw [← comap_smul_restrictNormalHom K L₂ (L₁ ⊔ L₂) hc2 ρ w, hsm]
    rw [hst] at hmem
    simpa using hmem
...
theorem isUnramifiedAt_ringOfIntegers_sup (L₁ L₂ : IntermediateField K (AlgebraicClosure K))
  ...
  have hbot : Q.inertia (↥(L₁ ⊔ L₂) ≃ₐ[K] ↥(L₁ ⊔ L₂)) = ⊥ := by
    ...
    · refine fixed_of_restrictNormalHom_eq_one K L₂ ρ ?_
      ...
      exact hσ' _
  have hcard : Nat.card (Q.inertia (↥(L₁ ⊔ L₂) ≃ₐ[K] ↥(L₁ ⊔ L₂))) = 1 := by
    rw [hbot]; simp
  have h2 : Nat.card (Q.inertia (↥(L₁ ⊔ L₂) ≃ₐ[K] ↥(L₁ ⊔ L₂))) =
      Ideal.ramificationIdxIn (Q.under (𝓞 K)) (𝓞 ↥(L₁ ⊔ L₂)) :=
    Ideal.card_inertia_eq_ramificationIdxIn _ _
  have h3 : Ideal.ramificationIdxIn (Q.under (𝓞 K)) (𝓞 ↥(L₁ ⊔ L₂)) = Q.ramificationIdx (𝓞 K) :=
    Ideal.ramificationIdxIn_eq_ramificationIdx _ Q (↥(L₁ ⊔ L₂) ≃ₐ[K] ↥(L₁ ⊔ L₂))
...
DECOMPOSED AND PROVEN the same day from `isAbelianGalois_sup`, `isUnramifiedAt_ringOfIntegers_sup`
and `isUnramifiedAtInfinitePlaces_sup` above, and from nothing else. **ALL THREE ARE NOW
CLOSED — 2026-07-31 — so this node adds no leaf to the tree.**).
...
**The three real obligations were ALL THE SAME ARGUMENT, and that is why all three are now
closed.** An element of `Gal(M/K)` trivial on `L₁` and on `L₂` is trivial, because
...
* *unramified at the finite primes* — `isUnramifiedAt_ringOfIntegers_sup` above,
...
* *unramified at the infinite places* — `isUnramifiedAtInfinitePlaces_sup` above,
  **PROVEN 2026-07-31**: the same, with the decomposition group at an archimedean place,
  and `InfinitePlace.isUnramified_iff_stabilizer_eq_bot` for the same conversion.

**The observation that the three are one argument was the whole value of the cut, and it
held.** Both ramification clauses turned out to be a triviality statement about a subgroup
of `Gal(M/K)` — mathlib supplies the "unramified ⟺ that subgroup is trivial" dictionary in
both the archimedean and the nonarchimedean case — so once `forall_mem_sup_of_fixed` was
proven, each clause was a dozen lines of restriction bookkeeping over it, shared through
`algebraMap_restrictNormalHom_of_le` and its two corollaries.

**All three clauses needed `Lᵢ/K` to be NORMAL** — each restricts an element of `Gal(K̄/K)`
to `Lᵢ` — so all three acquired `[IsGalois K Lᵢ]` when they were proven. That did not move
this call site: `IsAbelianGalois` extends `IsGalois`, the hypotheses are instance-implicit,
and the `haveI := hᵢab` lines below already put them in scope. For the finite-prime clause
the general non-Galois statement is TRUE and its route is recorded on that theorem; for the
archimedean one the Galois hypothesis is closer to essential (see there).

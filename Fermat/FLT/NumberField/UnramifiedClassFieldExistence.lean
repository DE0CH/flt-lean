...
public import Fermat.FLT.Mathlib.RingTheory.ClassGroupRelNorm
...
* `NumberField.relNormClassSubgroup_eq_of_algEquiv` and
  ...
  class field.
* `ClassGroup.relNormHom` (in `Fermat/FLT/Mathlib/RingTheory/ClassGroupRelNorm.lean`,
  written for this file on 2026-08-01) — the relative ideal norm as a map
  `ClassGroup S →* ClassGroup R`, absent from mathlib, which has only the map in the
  opposite direction (`ClassGroup.extendedHom`). Over it,
  `NumberField.relNormClassSubgroup_eq_range` identifies `relNormClassSubgroup K L` with
  the RANGE of that map, and `NumberField.index_relNormClassSubgroup_le_mul` says the norm
  index is SUBMULTIPLICATIVE in a tower `K ⊆ M ⊆ L`. Both PROVEN, and pure group theory
  over `Subgroup.index_map` once the range description is in hand.
* `NumberField.index_relNormClassSubgroup_le_finrank_of_isCyclic` — **OPEN LEAF**, the
  second fundamental inequality at modulus `1` for a CYCLIC extension. Cut 2026-08-01 out
  of the abelian statement below, which is now PROVEN over it by the classical dévissage
  along a cyclic subextension (that abelian leaf's own docstring had recorded this as its
  route). Count unchanged, `1 → 1`; what left the leaf is the whole reduction.
* `NumberField.index_relNormClassSubgroup_le_finrank_of_isUnramifiedAtInfinitePlaces` —
  PROVEN 2026-08-01 from the cyclic leaf and the tower estimate, and from nothing else.
* `NumberField.exists_unramifiedAbelian_card_classGroup_le_finrank` — The
...
**THE ROUTE TAKEN IS NOT THE ONE THIS DOCSTRING USED TO PRESCRIBE, and the difference is
worth recording, because the prescribed one needs a hypothesis the statement does not
carry.** The old sketch was the ARCHIMEDEAN instance of `forall_mem_sup_of_fixed` above:
complex conjugation at a place of `M = L₁ ⬝ L₂` above a real place of `K` is a nontrivial
element of `Gal(M/K)`, its restriction to each `Lᵢ` is trivial, so it is trivial. That
argument is correct and it is a GALOIS argument at every step — `Gal(M/K)`, "the
decomposition group at an archimedean place is generated by complex conjugation", and
mathlib's `InfinitePlace.isUnramified_mk_iff_forall_isConj` (which carries `[IsGalois k K]`)
all need `L₁` and `L₂` normal over `K`. **The leaf as stated assumes no such thing.** So
taking it would have meant either adding `[IsGalois K L₁] [IsGalois K L₂]` — free at the one
call site, which has `IsAbelianGalois`, but a signature change — or proving something the
statement does not say.

The route below needs no Galois hypothesis, and needs no `FiniteDimensional` or
`NumberField` either (both are kept because the statement is a leaf with a live call site
and its binder list must not move; see the `set_option` above, which is there because
obeying the linter's `omit` suggestion WOULD move it):

* `InfinitePlace.isUnramified_iff` reduces the goal at a place `w` of `M` to: if the place
  `v` below `w` is REAL then `w` is real. So assume `v` real and prove
  `ComplexEmbedding.IsReal w.embedding`.
* Give `ℂ` the `M`-algebra structure of `w.embedding` — hence a `K`-algebra structure — and
  extend `w.embedding` to `Φ : AlgebraicClosure K →ₐ[M] ℂ` by `IsAlgClosed.lift`. This is
  the one step that needs `AlgebraicClosure K` to be algebraic over `M`, which is
  `Algebra.IsAlgebraic.tower_top`.
* `v` real says exactly that `conj ∘ Φ` is again a `K`-ALGEBRA map, so
  `AlgHom.equalizer (Φ.restrictScalars K) (conj ∘ Φ)` is a `K`-subalgebra of
  `AlgebraicClosure K`; it is closed under inverses because both maps land in a field, so
  `Subalgebra.toIntermediateField` makes it an INTERMEDIATE FIELD `E`. That is what lets
  `sup_le` finish, and it is the structural replacement for `forall_mem_sup_of_fixed`: the
  same "the locus is an intermediate field, so `sup_le` applies" move, with the fixed field
  of an automorphism replaced by the equalizer of two embeddings.
* Each `Lᵢ ≤ E`: the place `mk (Φ ∘ algebraMap Lᵢ K̄)` of `Lᵢ` is unramified over `K` by
  hypothesis, and the place below it is `v`, which is real — so that place is real, i.e.
  `conj ∘ Φ = Φ` on `Lᵢ`.
* Hence `L₁ ⊔ L₂ ≤ E`, i.e. `conj (Φ x) = Φ x` for every `x ∈ L₁ ⊔ L₂`, and `Φ` restricted
  to `M` is `w.embedding`.
...
section NormIndexTower

/-! ### The norm class group as the range of the norm map, and its index in a tower

The one monotonicity that makes the second fundamental inequality reduce to the cyclic case.
Everything here is formal group theory over `ClassGroup.relNormHom`
(`Fermat/FLT/Mathlib/RingTheory/ClassGroupRelNorm.lean`); no arithmetic. -/

variable (L : Type*) [Field L] [NumberField L] [Algebra K L]

/-- `relNormClassSubgroup K L` is generated by the classes of the norms of the elements of
`(Ideal (𝓞 L))⁰`.  A restatement of the definition, whose generating set is indexed by
`I ≠ ⊥` instead. -/
theorem relNormClassSubgroup_eq_closure_range :
    relNormClassSubgroup K L = Subgroup.closure
      (Set.range (fun I : (Ideal (𝓞 L))⁰ =>
        ClassGroup.mk0 (ClassGroup.relNormNZD (𝓞 K) (𝓞 L) I))) := by
  unfold relNormClassSubgroup
  congr 1
  ext c
  constructor
  · rintro ⟨I, hI, rfl⟩
    exact ⟨⟨I, mem_nonZeroDivisors_of_ne_zero (by simpa using hI)⟩, rfl⟩
  · rintro ⟨I, rfl⟩
    exact ⟨(I : Ideal (𝓞 L)), by simpa using mem_nonZeroDivisors_iff_ne_zero.mp I.2, rfl⟩

/-- **THE NORM CLASS GROUP IS THE RANGE OF THE NORM MAP ON CLASS GROUPS** (PROVEN
2026-08-01).

The generating set of `relNormClassSubgroup K L` is the image under
`ClassGroup.relNormHom` of the image of `ClassGroup.mk0`, and `ClassGroup.mk0` is SURJECTIVE
(every ideal class contains an integral ideal), so that generating set is already the whole
range — a subgroup, so taking the closure changes nothing.

This is what turns the classical norm index `[I_K : P_K · N_{L/K} I_L]` into a statement
about a group homomorphism, and it is the only reason the tower estimate below is formal. -/
theorem relNormClassSubgroup_eq_range :
    relNormClassSubgroup K L = (ClassGroup.relNormHom (𝓞 K) (𝓞 L)).range := by
  rw [relNormClassSubgroup_eq_closure_range]
  have hset : (Set.range (fun I : (Ideal (𝓞 L))⁰ =>
        ClassGroup.mk0 (ClassGroup.relNormNZD (𝓞 K) (𝓞 L) I)))
      = ((ClassGroup.relNormHom (𝓞 K) (𝓞 L)).range : Set (ClassGroup (𝓞 K))) := by
    ext c
    constructor
    · rintro ⟨I, rfl⟩
      exact ⟨ClassGroup.mk0 I, ClassGroup.relNormHom_mk0 _ _ I⟩
    · rintro ⟨d, rfl⟩
      obtain ⟨I, rfl⟩ := ClassGroup.mk0_surjective d
      exact ⟨I, (ClassGroup.relNormHom_mk0 _ _ I).symm⟩
  rw [hset, Subgroup.closure_eq]

variable (M : Type*) [Field M] [NumberField M] [Algebra K M] [Algebra M L]
    [IsScalarTower K M L]

/-- In a tower `K ⊆ M ⊆ L`, the norm class group of `L/K` is the image under the norm map of
`M` of the norm class group of `L/M`.  Immediate from
`ClassGroup.relNormHom_comp` (`N_{L/K} = N_{M/K} ∘ N_{L/M}`) and the range description
above. -/
theorem map_relNormHom_relNormClassSubgroup :
    Subgroup.map (ClassGroup.relNormHom (𝓞 K) (𝓞 M)) (relNormClassSubgroup M L)
      = relNormClassSubgroup K L := by
  rw [relNormClassSubgroup_eq_range M L, relNormClassSubgroup_eq_range K L,
    ← MonoidHom.range_comp, ClassGroup.relNormHom_comp]

/-- **THE NORM INDEX IS SUBMULTIPLICATIVE IN A TOWER `K ⊆ M ⊆ L`** (PROVEN 2026-08-01):

`[Cl(𝓞 K) : N_{L/K}] ≤ [Cl(𝓞 K) : N_{M/K}] · [Cl(𝓞 M) : N_{L/M}]`.

**This is the multiplicativity that lets the second fundamental inequality be proved by
induction along a cyclic subextension**, and it is pure group theory: writing `f` for
`ClassGroup.relNormHom (𝓞 K) (𝓞 M)` and `H` for `relNormClassSubgroup M L`, mathlib's
`Subgroup.index_map` says `(H.map f).index = (H ⊔ ker f).index * f.range.index`, the first
factor is at most `H.index` because the index is antitone, and `f.range` IS
`relNormClassSubgroup K M` by `relNormClassSubgroup_eq_range`.

Note that the corresponding statement for DEGREES is not what is being used: a compositum
only bounds `[M : K]` from above, which is exactly why the whole cluster is phrased in norm
groups (see `relNormClassSubgroup_le_of_le` above).

**Equality holds classically**, `ker f` being trivial on the relevant part, but only `≤` is
needed and only `≤` is proved — the `⊔ ker f` is what is thrown away. -/
theorem index_relNormClassSubgroup_le_mul :
    (relNormClassSubgroup K L).index
      ≤ (relNormClassSubgroup K M).index * (relNormClassSubgroup M L).index := by
  rw [← map_relNormHom_relNormClassSubgroup K L M, Subgroup.index_map,
    ← relNormClassSubgroup_eq_range K M, mul_comm]
...
/-- **THE SECOND FUNDAMENTAL INEQUALITY AT MODULUS `1` FOR A CYCLIC EXTENSION: for `L/K`
finite CYCLIC, unramified at every finite prime AND at every infinite place, the norm index
`[I_K : P_K · N_{L/K} I_L]` is AT MOST the degree `[L : K]`** (SORRY LEAF, cut 2026-08-01
out of `index_relNormClassSubgroup_le_finrank_of_isUnramifiedAtInfinitePlaces` below, which
is now PROVEN over it and over `index_relNormClassSubgroup_le_mul` above, and over nothing
else).
...
exactly what this leaf asserts.

**WHY THE STATEMENT IS CYCLIC AND NOT ABELIAN** (recut 2026-08-01, count unchanged `1 → 1`).
The abelian statement is now PROVEN below by the classical dévissage, which the abelian
leaf's own docstring had recorded as its route and which nobody had performed: `Gal(L/K)`
abelian and not cyclic has a proper nontrivial cyclic subgroup `H = ⟨g⟩`, and for
`M = L^H` the tower estimate `index_relNormClassSubgroup_le_mul` gives

  `[Cl : N_{L/K}] ≤ [Cl(𝓞 K) : N_{M/K}] · [Cl(𝓞 M) : N_{L/M}] ≤ [M : K] · [L : M] = [L : K]`,

the first factor by induction on the degree (`M/K` is again abelian and everywhere
unramified) and the second by THIS leaf (`Gal(L/M) ≅ H` is cyclic).  So a prover who closes
this leaf closes the abelian one, and no prover ever needs to think about the abelian case
again.
...
inequality with the archimedean hypothesis DROPPED and with no cyclic hypothesis, and it is
PROVEN — but its proof runs `exists_surjective_aut_classGroupQuotient` →
`exists_hilbertClassField_artinIso` → `exists_unramifiedAbelian_card_classGroup_le_finrank`,
i.e. through the existence leaf below. Citing it here would close a cycle, and Lean's
declaration order rejects it outright. The honest reading of the two is:

* THIS one is an INPUT of the theory — classically the algebraic half of the class field
  axiom, `#Ĥ⁰(Gal(L/K), C_L) = [L : K]` for `L/K` cyclic;
...
**Route.** Neukirch VI (7.1) and V (7.1) (via the Herbrand quotient of the idele class
group: `h(C_L) = [L : K]` for `L/K` cyclic, so `#Ĥ⁰(Gal, C_L) ≤ [L : K]` is equivalent to
`H¹(Gal, C_L) = 1`); Lang *ANT* ch. IX–X; Cassels–Fröhlich ch. VII. Artin–Tate's *Class
Field Theory* ch. VI–VII gives the class-formation version, which is the one that states
exactly this inequality with no analysis in it.  For the everywhere-unramified case
specifically the ambiguous class number formula is the shortest classical route.

**`habel` is GONE and must not be re-added**: `IsCyclic (L ≃ₐ[K] L)` implies it
(`IsCyclic.commGroup`), and the consumer below supplies the cyclic hypothesis, so asking
for both would only make the leaf harder to apply.

**Sanity checks.** `L = K`: `Gal(L/K)` is trivial, hence cyclic; `relNormClassSubgroup K K
= ⊤` because `Algebra.intNorm` of a rank-one extension is the identity, so the index is
`1 ≤ 1`. `K = ℚ(√-5)` (`h = 2`), `L = K(i)` the Hilbert class field, `Gal` cyclic of order
`2`: norm index `2 ≤ 2`. `K = ℚ`, `L = ℚ(i)` (ramified at `2`, so not an instance of this
statement, but the inequality still holds): `h_ℚ = 1`, index `1 ≤ 2`.
...
**The check that would refute it**: a finite CYCLIC extension `L/K` of number fields,
unramified at every finite prime and at every infinite place, with
`(relNormClassSubgroup K L).index > [L : K]`. -/
theorem index_relNormClassSubgroup_le_finrank_of_isCyclic
...
/-- The induction behind
`index_relNormClassSubgroup_le_finrank_of_isUnramifiedAtInfinitePlaces` below, carried out
on a bound for the degree so that the recursion can change the FIELD.  See that theorem's
docstring; there is nothing here that is not bookkeeping for it. -/
theorem index_relNormClassSubgroup_le_finrank_aux :
    ∀ (n : ℕ) (L : Type uL) [Field L] [NumberField L] [Algebra K L] [IsGalois K L]
      [IsUnramifiedAtInfinitePlaces K L],
    (∀ a b : L ≃ₐ[K] L, a * b = b * a) →
    (∀ (Q : Ideal (𝓞 L)) (_ : Q.IsPrime), Q ≠ ⊥ → Algebra.IsUnramifiedAt (𝓞 K) Q) →
    Module.finrank K L ≤ n →
    (relNormClassSubgroup K L).index ≤ Module.finrank K L := by
  intro n
  induction n with
  | zero =>
    intro L _ _ _ _ _ _habel _hunr hle
    have : 0 < Module.finrank K L := Module.finrank_pos
    omega
  | succ n ih =>
    intro L _ _ _ _ _ habel hunr hle
    by_cases hcyc : IsCyclic (L ≃ₐ[K] L)
    · exact index_relNormClassSubgroup_le_finrank_of_isCyclic K L hcyc hunr
    -- `Gal(L/K)` is abelian and NOT cyclic, so it has a proper nontrivial cyclic subgroup.
    have hex : ∃ g : L ≃ₐ[K] L, g ≠ 1 := by
      by_contra hcon
      haveI : Subsingleton (L ≃ₐ[K] L) :=
        ⟨fun a b => by
          rw [not_not.mp fun h => hcon ⟨a, h⟩, not_not.mp fun h => hcon ⟨b, h⟩]⟩
      exact hcyc isCyclic_of_subsingleton
    obtain ⟨g, hg⟩ := hex
    haveI hnorm : (Subgroup.zpowers g).Normal := by
      refine ⟨fun a ha b => ?_⟩
      have hcomm : b * a * b⁻¹ = a := by
        rw [habel b a, mul_assoc, mul_inv_cancel, mul_one]
      rw [hcomm]; exact ha
    set M : IntermediateField K L := IntermediateField.fixedField (Subgroup.zpowers g) with hMdef
    -- `[L : M] = #⟨g⟩ ≥ 2`, so `[M : K] < [L : K]` and the induction hypothesis applies.
    haveI hnt : Nontrivial (Subgroup.zpowers g) :=
      ⟨⟨g, Subgroup.mem_zpowers g⟩, 1, by simpa using hg⟩
    have hcard : 1 < Nat.card (Subgroup.zpowers g) := Finite.one_lt_card
    have hML : Module.finrank M L = Nat.card (Subgroup.zpowers g) :=
      IntermediateField.finrank_fixedField_eq_card _
    have hmul : Module.finrank K M * Module.finrank M L = Module.finrank K L :=
      Module.finrank_mul_finrank K M L
    have hMpos : 0 < Module.finrank K M := Module.finrank_pos
    have hMle : Module.finrank K M ≤ n := by
      have h2 : 2 * Module.finrank K M ≤ Module.finrank K L := by
        calc 2 * Module.finrank K M
            ≤ Module.finrank M L * Module.finrank K M :=
              Nat.mul_le_mul_right _ (by omega)
          _ = Module.finrank K L := by rw [mul_comm]; exact hmul
      omega
    -- the hypotheses of the induction hypothesis, at `M/K`
    have habelM : ∀ a b : M ≃ₐ[K] M, a * b = b * a := by
      intro a b
      obtain ⟨a', rfl⟩ := AlgEquiv.restrictNormalHom_surjective (K₁ := M) (E := L) a
      obtain ⟨b', rfl⟩ := AlgEquiv.restrictNormalHom_surjective (K₁ := M) (E := L) b
      rw [← map_mul, ← map_mul, habel]
    have hunrM : ∀ (q : Ideal (𝓞 M)) (_ : q.IsPrime), q ≠ ⊥ →
        Algebra.IsUnramifiedAt (𝓞 K) q :=
      fun q hq hq0 => isUnramifiedAt_of_intermediateField K L M hunr q hq hq0
    haveI : IsUnramifiedAtInfinitePlaces K M :=
      IsUnramifiedAtInfinitePlaces.bot (k := K) (K := M) (F := L)
    -- the hypotheses of the cyclic leaf, at `L/M`
    haveI : IsUnramifiedAtInfinitePlaces M L :=
      IsUnramifiedAtInfinitePlaces.top (k := K) (K := M) (F := L)
    have hcycM : IsCyclic (L ≃ₐ[M] L) :=
      isCyclic_of_surjective (IntermediateField.subgroupEquivAlgEquiv (Subgroup.zpowers g))
        (IntermediateField.subgroupEquivAlgEquiv (Subgroup.zpowers g)).surjective
    have hunrL : ∀ (Q : Ideal (𝓞 L)) (_ : Q.IsPrime), Q ≠ ⊥ →
        Algebra.IsUnramifiedAt (𝓞 M) Q := by
      intro Q hQ hQ0
      haveI := hQ
      haveI := hunr Q hQ hQ0
      exact Algebra.FormallyUnramified.of_restrictScalars (𝓞 K) (𝓞 M) (Localization.AtPrime Q)
    calc (relNormClassSubgroup K L).index
        ≤ (relNormClassSubgroup K M).index * (relNormClassSubgroup M L).index :=
          index_relNormClassSubgroup_le_mul K L M
      _ ≤ Module.finrank K M * Module.finrank M L :=
          Nat.mul_le_mul (ih M habelM hunrM hMle)
            (index_relNormClassSubgroup_le_finrank_of_isCyclic M L hcycM hunrL)
      _ = Module.finrank K L := hmul
...
`[I_K : P_K · N_{L/K} I_L]` is AT MOST the degree `[L : K]`** (was a SORRY LEAF cut
2026-07-31 out of `exists_unramifiedAbelian_card_classGroup_le_finrank` below; RECUT AND
PROVEN 2026-08-01 over `index_relNormClassSubgroup_le_finrank_of_isCyclic` — the same
statement for a CYCLIC extension — and `index_relNormClassSubgroup_le_mul`, and nothing
else).

**The dévissage, in full, and it is the one this leaf's own docstring had recorded as its
route.** Induct on a bound for `[L : K]`.  If `Gal(L/K)` is cyclic, apply the cyclic leaf.
Otherwise it is abelian, nontrivial and not cyclic, so some `g ≠ 1` generates a subgroup
`H = ⟨g⟩` which is nontrivial (`g ≠ 1`) and cyclic; put `M = L^H`.  Then

* `[L : M] = #H ≥ 2` (Artin, `IntermediateField.finrank_fixedField_eq_card`), so
  `[M : K] < [L : K]` and the induction hypothesis applies to `M/K`, which is again
  abelian (`Gal(M/K)` is a quotient of `Gal(L/K)`) and unramified at every finite prime
  (`isUnramifiedAt_of_intermediateField`) and at every infinite place
  (`IsUnramifiedAtInfinitePlaces.bot`);
* `Gal(L/M) ≅ H` (`IntermediateField.subgroupEquivAlgEquiv`) is cyclic, `L/M` is unramified
  at the infinite places (`IsUnramifiedAtInfinitePlaces.top`) and at the finite ones
  (`Algebra.FormallyUnramified.of_restrictScalars`, the CHEAP direction — one only has to
  forget part of the base), so the cyclic leaf applies to `L/M`;
* and the two combine through the tower estimate `index_relNormClassSubgroup_le_mul`
  together with `[M : K] · [L : M] = [L : K]`.

**Why the norm index and not the degree.** The dévissage works because the norm index is
SUBMULTIPLICATIVE in a tower — which is `Subgroup.index_map` applied to
`ClassGroup.relNormHom`, and needs `relNormClassSubgroup K M` to be the RANGE of that map
(`relNormClassSubgroup_eq_range`).  The corresponding dévissage for the naked inequality
`[L : K] ≤ h_K` does NOT work, and `ArtinSymbol.lean`'s `artinMap_toPrincipalIdeal` records
why: chaining cyclic steps bounds `[L : K]` by `∏ h_{Fᵢ}` and not by `h_K`.  It is the norm
GROUP, not the degree, that behaves under composita — the same observation that
`relNormClassSubgroup_le_of_le` above is about.

**⚠ IT IS NOT A DUPLICATE OF `index_relNormClassSubgroup_le_finrank` BELOW**; see the
cyclic leaf's docstring for the dependency-order argument, which is unchanged.
...
theorem index_relNormClassSubgroup_le_finrank_of_isUnramifiedAtInfinitePlaces
    ...
    (hunr : ∀ (Q : Ideal (𝓞 L)) (_ : Q.IsPrime), Q ≠ ⊥ →
      Algebra.IsUnramifiedAt (𝓞 K) Q) :
    (relNormClassSubgroup K L).index ≤ Module.finrank K L :=
  index_relNormClassSubgroup_le_finrank_aux K (Module.finrank K L) L habel hunr le_rfl

...
leaves below iterates against THIS file in seconds. Do not move the material
back.

**On the `Chebotarev` import (added 2026-07-30).** This module now imports
`Fermat/FLT/GaloisRepresentation/Chebotarev.lean`, which is 13.5k lines of
analytic number theory. That costs olean LOAD time (a whole-file check went
from ~5 s to ~9 s) and nothing else: this file's own elaboration is unchanged,
and `Modularity/Interface.lean` — the only consumer that matters — already
imported `Chebotarev` directly, so nothing downstream sees a new dependency.
It buys `exists_isArithFrobOver_of_aut` outright; see that theorem.
...
* `NumberField.prod_eq_one_of_forall_isArithFrobOver` — OPEN. **ARTIN
  ...
  mathematics now is.
* `NumberField.exists_isArithFrobOver_of_aut` — **PROVEN 2026-07-30**.
  Chebotarev: every element of `Gal(L/K)` is a Frobenius at some prime. Deuring's
  fixed-field reduction on top of this repository's own `Chebotarev.lean`.
...
arithmetic Frobenius above some nonzero prime of `𝓞 K`** (cut 2026-07-29 out of
`exists_surjective_classGroupHom_aut_of_unramified_abelian` below; **PROVEN
2026-07-30** from this development's own `Chebotarev.lean`).

**THE 2026-07-28 MATHLIB SURVEY BELOW IS RIGHT AND WAS STILL THE WRONG PLACE TO
LOOK.** Chebotarev density is indeed absent from the pin and from `~/cs/FLT` —
but it is NOT absent from this repository. `Chebotarev.lean` (13.5k lines,
`sorry`-free) proves it from the ground up: Weber–Landau ideal counting, the
Dedekind-zeta pole, `L(1, χ) ≠ 0`, Dirichlet over an arbitrary number-field
base, and Deuring's reduction to the cyclic case. Its
`GaloisRepresentation.infinite_setOf_isArithFrobAt_zpowers` is the cyclic core,
stated for an ABSTRACT finite Galois `E/F` with `Gal(E/F) = ⟨τ⟩`, which is
exactly the shape needed here.

**What this proof does.** Deuring's reduction, run once more at this file's
level of generality. Put `F = L^⟨σ⟩`; then `L/F` is cyclic generated by the
lift `σ'` of `σ`, so the cyclic core supplies a prime `𝔓` of `𝓞 F` with
`#(𝓞 F/𝔓)` PRIME and a `Q | 𝔓` at which `σ'` is an arithmetic Frobenius. That
last primality is the whole point: it forces `𝓞 K/𝔭 = 𝓞 F/𝔓` for
`𝔭 = 𝔓 ∩ 𝓞 K` (`natCard_quotient_under_eq_of_natCard_prime`), so the Frobenius
congruence at `Q` over `F` IS the congruence over `K`, and `σ • x = σ' • x`
holds definitionally. Only the FIXED FIELD construction is done here; every
analytic ingredient is imported.

**The sibling `GaloisRepresentation.infinite_setOf_isArithFrobAt` is NOT usable
directly**, even though it is the same statement with the inertia clause added:
it quantifies over `L : IntermediateField K (AlgebraicClosure K)`, and this
file's `L` is an abstract field. Transporting along a chosen embedding
`L ↪ K̄` would mean moving `𝓞 L` across a ring equivalence — strictly more work
than redoing the (short) fixed-field step, which is why the step is repeated
rather than cited. Nothing here needs the inertia clause.
...
theorem exists_isArithFrobOver_of_aut [IsGalois K L] (σ : L ≃ₐ[K] L) :
  ...
  have hσmem : σ ∈ F.fixingSubgroup :=
    (IntermediateField.le_iff_le (Subgroup.zpowers σ) F).mp le_rfl (Subgroup.mem_zpowers σ)
  set σ' : L ≃ₐ[F] L := IntermediateField.fixingSubgroupEquiv F ⟨σ, hσmem⟩ with hσ'def
  have hgen : ∀ τ : L ≃ₐ[F] L, τ ∈ Subgroup.zpowers σ' := by
    intro τ
    obtain ⟨g, hg⟩ := (IntermediateField.fixingSubgroupEquiv F).surjective τ
    have hgmem : (g : L ≃ₐ[K] L) ∈ Subgroup.zpowers σ := by
  ...
  -- the cyclic Chebotarev core, imported
  obtain ⟨P, hcard, Q, hQprime, hQover, hQfrob⟩ :=
    (GaloisRepresentation.infinite_setOf_isArithFrobAt_zpowers σ' hgen).nonempty
  haveI := hQprime
  haveI : Q.LiesOver P.asIdeal := hQover
  have h2 : Q.under (𝓞 F) = P.asIdeal := hQover.over.symm
  have hQ0 : Q ≠ ⊥ := by
    rintro rfl
    exact P.ne_bot (by rw [← h2]; exact Ideal.under_bot (𝓞 F) (𝓞 L))
  haveI : Q.IsMaximal := hQprime.isMaximal hQ0
  haveI : (Q.under (𝓞 K)).IsMaximal := inferInstance
  have hv0 : Q.under (𝓞 K) ≠ ⊥ :=
    (Ideal.bot_lt_of_maximal _ (NumberField.RingOfIntegers.not_isField _)).ne'
  refine ⟨⟨Q.under (𝓞 K), mem_nonZeroDivisors_of_ne_zero (by simpa using hv0)⟩,
    inferInstance, Q, hQprime, rfl, ?_⟩
  -- the Frobenius congruence descends from `F` to `K` at a degree-one prime
  intro x
  have hcardeq : Nat.card (𝓞 K ⧸ Q.under (𝓞 K)) = Nat.card (𝓞 F ⧸ Q.under (𝓞 F)) := by
    have h3 : Q.under (𝓞 K) = P.asIdeal.under (𝓞 K) := by
      rw [← h2, Ideal.under_under]
    rw [h3, h2]
    exact GaloisRepresentation.natCard_quotient_under_eq_of_natCard_prime
      (A := 𝓞 K) P.asIdeal hcard
  have hact : σ • x = σ' • x := NumberField.RingOfIntegers.ext rfl
  show σ • x - x ^ Nat.card (𝓞 K ⧸ Q.under (𝓞 K)) ∈ Q
  rw [hcardeq, hact]
  exact hQfrob x

...
**AND THE SIBLING LEAF DOES NOT HAVE THAT GAP AT ALL — SO THE CHEVALLEY ROUTE
REACHES IT, WHILE THIS FILE'S LEAF IT DOES NOT REACH** (found and verified in
Lean 2026-07-30; **RESTORED 2026-07-30 after a merge dropped it** — see the
MERGE-LOSS NOTE at the end of this block. Recorded here because it changes which
of the two leaves the idele-free route is worth building for).

The sibling is `exists_artinDivisorNormIndex_le_ray_class`
(`Fermat/FLT/GaloisRepresentation/HardlyRamified/ModThree.lean`), whose docstring
asks for a shared abelian norm-index theorem serving both leaves. Its Galois
group is **automatically CYCLIC**, of `ℓ`-power order with `ℓ ≠ 3` prime, and no
hypothesis was needed to get that: the quantity on the left of its inequality is
`A.relIndex Im` with `A = φ.ker ⊓ Im`, which equals `Nat.card (Im.map φ)`
(`Subgroup.inf_relIndex_right` then `Subgroup.relIndex_ker` — mechanised
2026-07-30 in `ModThree.lean`, where it turned out to need no `subgroupOf`
bookkeeping at all, and note the lemma is `Subgroup.relIndex_ker`, not
`MonoidHom.relIndex_ker`); `φ`'s source is the FREE abelian group on the
height-one spectrum, so `Im.map φ` is generated by values `χ (globalFrob v)` that
its `hord` kills at `ℓ ^ k`; the `ℓ ^ k`-torsion of `(Dickson.K 3)ˣ` is a finite
subgroup, and `isCyclic_subgroup_units` (`Mathlib/RingTheory/IntegralDomain.lean`;
the alias `subgroup_units_cyclic` is deprecated since 2026-03-03) makes it cyclic.
Since 2026-07-30 the cyclicity is not merely an observation about the sibling: it
is a HYPOTHESIS of its sub-leaf `exists_natCard_charDivisorImage_le_ray_class`,
discharged by the parent's own glue, so a prover there gets it for free.

THIS file's leaf, by contrast, really does quantify over abelian `L ≃ₐ[K] L`
(`habel`), so the cyclic → abelian gap above is genuine here and vacuous there.
The two leaves are therefore NOT of equal strength, and "state one theorem and
cite it twice" is worth less than the sibling's docstring suggests. What does NOT
follow, and must not be read into this: Chevalley as recorded above is the
modulus-`1` unramified statement, whereas the sibling needs a RAY-class norm
index at a RAMIFIED modulus, so cyclicity retires one obstruction there and not
the other.

**A divergence worth knowing about before either leaf is picked up** (written
2026-07-30, **UPDATED the same day** — the file moved under it while it was out of
the tree). The sibling's cross-reference (written 2026-07-28) prescribes building
one shared theorem and rejects the Artin-map route for both, as circular there and
as strictly HARDER than the norm-index inequality. Later the same day this file cut
`exists_surjective_classGroupHom_aut_of_unramified_abelian`, which IS that
Artin-map route. That cut has since been carried through: **this module is now
`sorry`-free** — `exists_surjective_classGroupHom_aut_of_unramified_abelian`,
`finrank_le_index_relNormClassSubgroup` and
`finrank_le_card_classGroup_of_unramified_abelian_of_isUnramifiedAtInfinitePlaces`
are all PROVEN — and the whole remaining content of the Artin-map route sits in
`Fermat/FLT/NumberField/ArtinSymbol.lean` as exactly two leaves,
`exists_classGroupHom_eq_frobAt` (`:325`, reciprocity at modulus `1`) and
`closure_frobAt_eq_top` (`:369`, the density half). So the two efforts remain on
two different routes and nobody is building the shared statement; that should be
an explicit decision rather than an assumption that the other side is covering it.

**⚠ TWO CROSS-REFERENCES TO THIS FILE ARE STALE — verified against the compiler
2026-07-30, `lake build Fermat.FLT.NumberField.ArtinSymbol
Fermat.FLT.NumberField.UnramifiedClassFieldBound`, `Build completed successfully
(3531 jobs)`, whose ONLY `declaration uses 'sorry'` warnings are
`ArtinSymbol.lean:325` and `ArtinSymbol.lean:369`.** Correct them where you find
them rather than acting on them:
* `Modularity/Interface.lean`, in the docstring of
  `finrank_le_card_classGroup_of_unramified_abelian`, calls
  `NumberField.finrank_le_index_relNormClassSubgroup` "the ONE remaining open
  leaf". It is PROVEN, here, and has been since 2026-07-28.
* `ModThree.lean`'s norm-index cluster calls
  `finrank_le_card_classGroup_of_unramified_abelian` itself a "sorry leaf". It is
  PROVEN too (`Interface.lean:39180`, `rcases`/`rw`/`exact` over this file).
Anyone dispatched at either name will find nothing to do; the live pair is the
two `ArtinSymbol.lean` leaves named above.

**MERGE-LOSS NOTE (2026-07-30).** The two paragraphs restored above were added by
`094df0b7` and disappeared from `flt-lean-160` when a later resolution of a
prose-only conflict in this file took `main`'s side wholesale. That is the
honest, non-buggy shape of the dropped-payload class: `094df0b7` stayed a full
ancestor, `git diff --stat HEAD^1 HEAD` was non-empty, the build was green, and
the finding was simply gone. `git log -m -S 'SIBLING LEAF DOES NOT HAVE THAT GAP'
-- <this file>` is what shows it; without `-m` the history reads "added, never
removed".

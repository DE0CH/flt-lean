...
Lean 2026-07-30; recorded here because it changes which of the two leaves the
idele-free route is worth building for).
...
(`Subgroup.inf_relIndex_right` then `Subgroup.relIndex_ker` — mechanised 2026-07-30 in
`ModThree.lean`, where it turned out to need no `subgroupOf` bookkeeping at all, and note
the lemma is `Subgroup.relIndex_ker`, not `MonoidHom.relIndex_ker`);
`φ`'s source is the FREE abelian group on the height-one spectrum, so `Im.map φ`
is generated by values `χ (globalFrob v)` that its `hord` kills at `ℓ ^ k`; the
`ℓ ^ k`-torsion of `(Dickson.K 3)ˣ` is a finite subgroup, and
`isCyclic_subgroup_units` (`Mathlib/RingTheory/IntegralDomain.lean`; the alias
`subgroup_units_cyclic` is deprecated since 2026-03-03) makes it cyclic.
...
**A divergence worth knowing about before either leaf is picked up.** The sibling's
cross-reference (written 2026-07-28) prescribes building one shared theorem and
rejects the Artin-map route for both, as circular there and as strictly HARDER than
the norm-index inequality. Later the same day this file cut
`exists_surjective_classGroupHom_aut_of_unramified_abelian`, which IS that Artin-map
route — as its own docstring above concedes. So the two efforts are now on the two
different routes and nobody is building the shared statement. That may well be the
right call, given the survey above; it should be an explicit decision rather than
an assumption that the other side is covering it.

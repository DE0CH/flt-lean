...
* `NumberField.exists_classGroupHom_eq_frobAt` — **OPEN LEAF: ARTIN RECIPROCITY.**
...
/-- **ARTIN RECIPROCITY AT MODULUS `1`, AS AN EXISTENTIAL: for `L/K` finite
abelian, unramified at every finite prime and at every infinite place, the Artin
symbol descends to the ideal CLASS group** (SORRY LEAF, cut 2026-07-30 out of
...
being `closure_frobAt_eq_top` below. Everything else in this cluster — the norm
clause of the consumer, its surjectivity, the whole second inequality, and the
...
`𝔭 ↦ Frob_𝔭`, extended multiplicatively to the group of nonzero ideals of `𝓞 K`.
Its existence as a map on IDEALS is bookkeeping (unique factorisation, plus
`frobAt_eq_frobAt_of_comm` to make `Frob_𝔭` well defined); what is deep is that
it kills the PRINCIPAL ideals, so that it descends to `Cl(𝓞 K)`. This leaf is
exactly that descent: a `φ` on `Cl(𝓞 K)` whose value at the class of `𝔭` is
`Frob_𝔭`. Surjectivity is deliberately NOT asked for here — it is a genuinely
separate input (`closure_frobAt_eq_top`) and bundling the two is what made the
previous single-leaf cut opaque.
...
false: `K = ℚ(√3)` has `h_K = 1`, so `Cl(𝓞 K)` is trivial and any `φ` out of it
is trivial, while its narrow Hilbert class field `L` is abelian of degree `2`
over `K`, unramified at every FINITE prime, and has primes with nontrivial
Frobenius (a `φ` sending their classes to `frobAt K L Q ≠ 1` cannot exist). The
same counterexample is recorded, for the consumer's conclusion rather than for
this one, in `finrank_le_index_relNormClassSubgroup`.

**`habel` is load-bearing FORMALLY, not just mathematically** — see
`frobAt_eq_frobAt_of_comm`: without it the conclusion demands one value for a
whole conjugacy class and is contradictory, not merely hard.
...
finite prime and every infinite place, a principal ideal `(x)` of `𝓞 K`, and a
factorisation of `(x)` into primes whose Frobenius elements do not multiply to
`1`. -/
theorem exists_classGroupHom_eq_frobAt [IsUnramifiedAtInfinitePlaces K L]
    ...
    ∃ φ : ClassGroup (𝓞 K) →* (L ≃ₐ[K] L),
      ∀ (Q : Ideal (𝓞 L)) (_ : Q.IsMaximal) (J : (Ideal (𝓞 K))⁰),
        (J : Ideal (𝓞 K)) = Q.under (𝓞 K) → φ (ClassGroup.mk0 J) = frobAt K L Q :=
  sorry

...
**PROVEN 2026-07-31** over the two named leaves of this block,
`gp_le_gp_psiNat_phi_sup_lvl` (Herbrand between two levels) and
`iInf_gp_one_le_wildInertiaGroup` (the wild inverse limit). Everything else the
FAITHFULNESS AUDIT of 2026-07-30 asked for is discharged here: step 1
(cofinality) and step 4 (compactness) are inside
`exists_mem_lvl_forall_mem_gp_psiNat`, steps 2 and 3 are folded into the first
leaf without any relative `LowerRamificationData` (see the section docstring
for why that is legitimate), and the `m = 0` case is
`iInf_gp_zero_eq_localInertiaGroup`.

The audit's verdict stands and is NOT to be re-litigated: the statement is
TRUE, it is genuine Herbrand, and there is no counterexample to look for —
both sides are constructed rather than axiomatic, so no rescaling or
admissibility slack exists. What follows below is only the assembly.

THE ASSEMBLY. Given `σ ∈ G_m(L/Kᵥ)`, the compactness step produces one
`λ ∈ D.lvl` with `σλ ∈ G_{ψ_{D'}(φ_D m)}(L_{D'}/Kᵥ)` for EVERY level `D'`.
* If `φ_D(m) > 0` the filtration is `P_v ⊓ ⋂_{D'} D'.gp (ψ_{D'} (φ_D m))`; the
  second conjunct is exactly what the compactness step delivered, and the first
  follows from it because `ψ_{D'}(u) ≥ 1` for `u > 0` (`psiNat_pos`), so the
  same element lies in every `D'.gp 1` and the wild leaf applies.
* Otherwise `φ_D(m) ≤ 0`, the filtration is `I_v`, and `ψ_{D'}(φ_D m) = 0` for
  every `D'` (`psiNat_le` against `φ_{D'}(0) = 0`), so the compactness step
  delivered membership in every `D'.gp 0` and `iInf_gp_zero_eq_localInertiaGroup`
  finishes.
Finally `σ = (σλ) · λ⁻¹` with `λ⁻¹ ∈ D.lvl`, which is the `⊔ D.lvl`. -/

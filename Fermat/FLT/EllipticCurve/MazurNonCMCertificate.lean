...
* `not_monic_dvd_of_smallDegreePart'` — the same with root-freeness replaced by
  `IsCoprime H (X ^ (#K ^ k) - X)` for any `k` that every small divisor of `m` divides.  The
  `p = 17` rows need this: there `m = 34`, `n = 16`, and the divisors `1` and `2` both have to
  go, which one coprimality at `k = 2` does.
* `irreducible_dvd_X_pow_card_pow_sub_X` — the CONVERSE of mathlib's
  `Irreducible.natDegree_dvd_of_dvd_X_pow_card_pow_sub_X`, which mathlib has in one direction
  only.  It is what makes `k > 1` usable at all: mathlib's only handle on an irreducible factor
  of small degree is `exists_root_of_degree_eq_one`, which never reaches degree `2`.
* `not_monic_dvd_preΨ_elevenA_mod`, `…_elevenB_mod` — the two `p = 11` rows of Mazur's table
  (isogeny class `121`): `ℓ = 23`, `m = 11`, `n = 10`, `deg D = 5`, `deg H = 55`, factor
  degrees `1⁵ 11⁵`.
* `not_monic_dvd_preΨ_seventeenA_mod`, `…_seventeenB_mod` — the two `p = 17` rows: `ℓ = 67`,
  `m = 34`, `k = 2`, `n = 16`, `deg D = 8`, `deg H = 136`, factor degrees `2⁴ 34⁴`.

The explicit `Ψ̄`, `D` and `H` were produced by PARI/GP 2.15.4 and are re-derived here inside
Lean from mathlib's EDS recursion — at `p = 11` that is `Ψ₂Sq, Ψ₃, preΨ₄, preΨ' 5, preΨ' 6,
preΨ' 7` and then `preΨ'_odd 3`; at `p = 17` it runs on to `preΨ' 10` and then `preΨ'_odd 6`.
It lives in its own module rather than in `X0.lean` because elaboration is single-threaded per
file and `X0.lean` is already the slowest file in the tree; nothing here mentions a modular
curve.

**KEEPING `ring_nf` CHEAP AT DEGREE `144`, and this is the whole reason the `p = 17` rows fit.**
`preΨ'_odd 6` contains `preΨ' 8 ^ 3` and `preΨ' 9 ^ 3`.  Handing those cubes to `ring_nf`
directly is `31³` and `41³` monomial products; rewriting them by the precomputed
`cube_preΨ'_eight_*`/`cube_preΨ'_nine_*` (each proven from its own square by `pow_succ`, so the
worst single product is `(81 terms) * (41 terms)`) keeps every `ring_nf` call in the file under
...
/-! ### The `p = 11`, `j = −24729001` row over `ZMod 23`

The second member of the isogeny class `121`.  `Ψ₁₁ mod 23` again has factor degrees `1⁵, 11⁵`,
all multiplicities `1`, so the row is word-for-word the `elevenA` one with different constants
(re-verified in PARI/GP 2.15.4, 2026-07-31: `deg = 60`, `leadingCoeff = 11`, degrees
...
/-- The product of the five LINEAR irreducible factors of `Ψ₁₁ mod 23` for the `j = −24729001`
row. -/
...
/-- **`Ψ₁₁ mod 23 = 11 · D · H`** for the `j = −24729001` row. -/
...
/-! ### The `p = 17`, `j = −882216989/131072` row over `ZMod 67`

...
over `MazurNonCMFrobenius/ElevenA.lean` and `…/ElevenB.lean`, and the `p = 17` rows over
`MazurNonCMFrobenius/SeventeenA.lean` / `…B.lean` and their per-factor submodules — all
generated by `gen_modules.py`, and none of them proves any factor irreducible: each factor's
divisibility is certified separately and the product is reassembled through explicit Bézout
coprimalities.

The `flt-frobenius-cert.py`-generated siblings `MazurNonCMFrobeniusSevA.lean` / `…SevB.lean`
that first closed the `p = 17` leaves were DELETED 2026-08-04: the `gen_modules.py` copies in
this namespace had replaced them as the consumed proofs the same day they landed, leaving the
siblings with no reference anywhere — 1.6 GB of `.olean.private` and two of the slowest
elaborations on this module's critical path, for nothing.  Recover them from git history if
ever wanted.

The two `p = 37` rows live in `MazurNonCMCertificate/ThirtySevenA.lean` / `…B.lean`
(generated by `gen_thirtyseven.py` at `ℓ = 1259`, closed 2026-08-04); they consume this
module's obstruction lemmas and are consumed by `X0.lean` directly, so THIS file's row
inventory stops at `p = 17`.
-/

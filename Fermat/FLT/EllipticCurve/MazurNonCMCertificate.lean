...
**ALL FOUR ROWS ARE CLOSED, AND THIS FILE HAS NO `sorry`** (2026-07-31).  The `p = 11` rows go
over `MazurNonCMFrobenius/ElevenA.lean` and `…/ElevenB.lean`; the `p = 17` rows, closed later
the same day, over the sibling modules `MazurNonCMFrobeniusSevA.lean` and `…SevB.lean`.  All
four certificates are generated — the `p = 11` pair by `gen_modules.py`, the `p = 17` pair by
`flt-frobenius-cert.py` — and none of them proves any factor irreducible: each factor's
divisibility is certified separately and the product is reassembled through explicit Bézout
coprimalities.
-/
...
`MazurNonCMFrobeniusSevA.lean`, generated by `flt-frobenius-cert.py` — a Frobenius-table
square-and-multiply chain per factor, recombined through explicit Bézout coprimality
certificates, so no factor is ever proven irreducible.  The statement was machine-checked
outside Lean first (PARI/GP 2.15.4, `Mod(x, H) ^ (67 ^ 34) == x`, and the generator's own
`F_q[X]` arithmetic), and then re-derived by the compiler, which is the only check that
counts.

The generated module is the `p = 11` rows' shape at `ℓ = 67`, `m = 34`, and it is a SIBLING
module rather than one under `MazurNonCMFrobenius/` because it is self-contained: it carries
its own `pow_frob` / `chain_step` / `isCoprime_of_dvd_sub` and writes `H` out as a literal,
so it does not import this file's shared machinery. -/
theorem dvd_X_pow_card_pow_sub_X_hPolySeventeenA :
    hPolySeventeenA ∣ X ^ (Nat.card (ZMod 67)) ^ 34 - X := by
  -- `XPow` keeps `67 ^ 34` out of unification; see `MazurNonCMFrobenius.lean` on why
  -- `rw`-ing the exponent into `X ^ _` instead costs `≈ 3 · 67 ^ 34` recursion depth.
  have h : XPow hPolySeventeenA ((67 : ℕ) ^ 34) X := by
    rw [hPolySeventeenA]
    exact MazurNonCMFrobeniusSevA.dvd_X_pow_sub_X_hPoly
  exact xpow_card (by rw [Nat.card_zmod]) h

/-- `H` has no irreducible factor of degree `1` or `2` (PROVEN 2026-07-31).
...
divide `2`.  The generator emits it from the SAME chain: `X ^ (67 ^ 2)` is already the second
rung `p2`, so the certificate is one Bézout pair against the REDUCTION `p2 - X` — both
cofactors of degree `< 34` — transported to `X ^ (67 ^ 2) - X` by `isCoprime_of_dvd_sub`.  A
Bézout pair against the unreduced polynomial would need a cofactor of degree `4489`. -/
theorem isCoprime_hPolySeventeenA :
    IsCoprime hPolySeventeenA (X ^ (Nat.card (ZMod 67)) ^ 2 - X) := by
  rw [Nat.card_zmod, hPolySeventeenA]
  exact MazurNonCMFrobeniusSevA.isCoprime_hPoly
...
docstring for the route.  The certificate is `MazurNonCMFrobeniusSevB.lean`. -/
theorem dvd_X_pow_card_pow_sub_X_hPolySeventeenB :
    hPolySeventeenB ∣ X ^ (Nat.card (ZMod 67)) ^ 34 - X := by
  have h : XPow hPolySeventeenB ((67 : ℕ) ^ 34) X := by
    rw [hPolySeventeenB]
    exact MazurNonCMFrobeniusSevB.dvd_X_pow_sub_X_hPoly
  exact xpow_card (by rw [Nat.card_zmod]) h

/-- `H` has no irreducible factor of degree `1` or `2` (PROVEN 2026-07-31).
See `isCoprime_hPolySeventeenA`. -/
theorem isCoprime_hPolySeventeenB :
    IsCoprime hPolySeventeenB (X ^ (Nat.card (ZMod 67)) ^ 2 - X) := by
  rw [Nat.card_zmod, hPolySeventeenB]
  exact MazurNonCMFrobeniusSevB.isCoprime_hPoly

...
theorem End.degree_add_add_degree_sub [IsAlgClosed F] [CharZero F] [W.IsElliptic]
    ...
    Isogeny.degree (End.toIsogeny (φ + ψ)) + Isogeny.degree (End.toIsogeny (φ - ψ))
      = 2 * Isogeny.degree (End.toIsogeny φ) + 2 * Isogeny.degree (End.toIsogeny ψ) := by
  -- `End.degree_neg` is stated in `ℤ` (it is proved by `ring` after
  -- `End.degree_intCast`); this statement is in `ℕ`, so cast it once here rather
  -- than keep a second `ℕ`-valued copy of it under the same name.
  have hneg : ∀ χ : End W,
      Isogeny.degree (End.toIsogeny (-χ)) = Isogeny.degree (End.toIsogeny χ) :=
    fun χ => by exact_mod_cast End.degree_neg χ

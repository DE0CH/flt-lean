...
**AND THAT ALTERNATIVE IS NOW STRICTLY SMALLER THAN THE PARALLELOGRAM LAW
(2026-07-31, eighth pass).** `End.degree_add_add_degree_sub_of_unitShift` — stated
and PROVEN *above* this leaf, unconditionally, so it consumes nothing here — derives
the entire parallelogram law from its single instance `φ = 1`:

    deg (χ + 1) + deg (χ − 1) = 2 deg χ + 2      (every `χ : End W`)

i.e. `#ker (χ+1) + #ker (χ−1) = 2 #ker χ + 2`, three kernel cardinalities and no
second endomorphism. The two steps are the second-order recurrence
(`End.degree_intCast_shift`, giving `deg (χ + [m]) = deg χ + t m + m²`) and
multiplication by `φ̂`, which converts the pair `(φ, ψ)` into the pair
`(φ̂ψ, [deg φ])` (`End.dualEnd_comp` plus multiplicativity, then divide by
`deg φ̂ = deg φ`, which is the new elementary `End.degree_dualEnd`).

This is **not** another lap of the degree circle, and the distinction is the
quantifier ARITY rather than the vocabulary: every other member of the circle
quantifies over a pair — two endomorphisms, or an endomorphism and a prime — and
this one quantifies over a single endomorphism with no prime, no torsion module, no
determinant and no pairing in sight. It is therefore the right target for route (2)
(the `x`-coordinate degree count), which is performed one endomorphism at a time and
cannot see a pair.

**What was checked and did NOT shortcut it, so that a successor does not re-walk it.**
`DifferentialCharacter.lean` supplies `λ : End W → F` — additive
(`isDiffChar_add`), multiplicative (`isDiffChar_comp`), `λ [n] = n`
(`isDiffChar_mulByHom`) and INJECTIVE in characteristic zero
(`eq_of_isDiffChar`) — so `End W` is in fact **commutative**, contrary to the
comment on the `Mathlib.Tactic.NoncommRing` import at the head of this file. (That
import must nevertheless STAY: no `CommRing (End W)` instance is registered, so
`ring` cannot know, and every `End W` identity here still needs `noncomm_ring` or
`abel`.) It also gives
`deg ψ = λ(ψ) · λ(ψ̂)` outright. That is genuinely new information about this ring
and it still does not close the leaf: the `ℤ[√2]`, `q := |N|` model of the audits
below is commutative, carries an involution `x ↦ q(x)/x` landing back in the ring,
and satisfies all five available facts, so commutativity is not among the missing
inputs. (What `ℤ[√2]` *does* violate is finiteness of the unit group
`{ψ : deg ψ = 1} = Aut W`; that is therefore a candidate substitute geometric
input, and it is the only one this pass found. `DifferentialCharacter.lean`
itself carries three open leaves of its own, so it is deliberately NOT imported
here — importing it to record commutativity would put three more leaves into this
module's cone and close nothing.) -/

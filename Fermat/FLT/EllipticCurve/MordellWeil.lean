...
The `MazurLevel14` section below carries that descent. It proves
`curve14a4_rational_points` — the UNCONDITIONAL enumeration of `14a4(ℚ)` —
from which finiteness and hence torsion follow, and it reduces the whole thing
to exactly two explicit integer statements:

* `MazurLevel14.quartic_one`  — `Q² =  S⁴ − 11S²e² + 32e⁴` has only `(S,e) = (2,1)`;
* `MazurLevel14.quartic_two`  — `Q² = 2S⁴ − 11S²e² + 16e⁴` has only `(S,e) = (2,1)`.

**Consequence for the two leaves below that are NOT mine to edit.**
`curve14a4_rational_points` is unconditional, so `curve14a4_fg` — the
Mordell–Weil finite-generation leaf, which exists only to feed
`curve14a4_finite` — is now redundant: `curve14a4_finite` can be reproved from
the enumeration exactly as `curve11a3_finite` is, and `curve14a4_points`
becomes a one-line corollary that discards its `Finite` hypothesis. Both were
left untouched here because they are other owners' declarations and were
reported as already closed on an unreleased branch; an integrator should
collapse them.
...
/-- **The denominator of a rational point is a square** (PROVEN 2026-07-26).
A rational solution of `V² = T³ − 11T² + 32T` with `T > 0` comes from an
integral one: `T = p/e²` with `gcd(p, e) = 1`, `p, e > 0`, and
`n² = p(p² − 11pe² + 32e⁴)`.

The mechanism is the classical one: `T.den` is coprime to
`T.num(T.num² − 11 T.num T.den + 32 T.den²)`, which forces
`V.den² = T.den³` and hence `T.den = c²`, `V.den = c³`. -/
theorem exists_int_model {T V : ℚ} (hT : 0 < T) (h : V ^ 2 = T ^ 3 - 11 * T ^ 2 + 32 * T) :
  ...
  have hrel : V.num ^ 2 * (T.den : ℤ) ^ 3 =
      (V.den : ℤ) ^ 2 * (T.num * (T.num ^ 2 - 11 * T.num * (T.den : ℤ)
        + 32 * (T.den : ℤ) ^ 2)) := by
    have hq : ((V.num : ℚ)) ^ 2 * (((T.den : ℤ) : ℚ)) ^ 3 =
        (((V.den : ℤ) : ℚ)) ^ 2 * ((T.num : ℚ) * ((T.num : ℚ) ^ 2
          - 11 * (T.num : ℚ) * (((T.den : ℤ) : ℚ)) + 32 * (((T.den : ℤ) : ℚ)) ^ 2)) := by
      rw [hTn, hVn]
  ...
  have hqN : IsCoprime ((T.den : ℤ))
      (T.num * (T.num ^ 2 - 11 * T.num * (T.den : ℤ) + 32 * (T.den : ℤ) ^ 2)) := by
    have h3 : IsCoprime ((T.den : ℤ)) (T.num ^ 3) := (hpq.symm).pow_right
    have hrw : T.num * (T.num ^ 2 - 11 * T.num * (T.den : ℤ) + 32 * (T.den : ℤ) ^ 2)
        = T.num ^ 3 + (T.den : ℤ) * (-11 * T.num ^ 2 + 32 * T.num * (T.den : ℤ)) := by ring
  ...
  have hd2 : ((T.den : ℤ)) ^ 3 ∣ ((V.den : ℤ)) ^ 2 := by
    have hdv : ((T.den : ℤ)) ^ 3 ∣ ((V.den : ℤ)) ^ 2 *
        (T.num * (T.num ^ 2 - 11 * T.num * (T.den : ℤ) + 32 * (T.den : ℤ) ^ 2)) :=
      ⟨V.num ^ 2, by linarith [hrel]⟩
  ...
  refine ⟨T.num, (c : ℤ), V.num, Rat.num_pos.mpr hT, by exact_mod_cast hc0, ?_, ?_, ?_⟩
...
2026-07-26 from the two `MazurLevel14` quartic leaves): the affine rational
points of `y² + xy + y = x³ − x` are exactly `(1,−2)`, `(0,−1)`, `(−1,0)`,
`(0,0)`, `(1,0)`. With the point at infinity these are the six elements of
`14a4(ℚ) ≅ ℤ/6`.

This is the `11a3`-shaped statement that `curve14a4_fg`'s docstring asks for:
because it carries NO finiteness hypothesis, finiteness is a corollary of it
rather than an input to it, and the Mordell–Weil finite-generation leaf
`curve14a4_fg` is no longer needed by anything. It is consumed by
`curve14a4_isTorsion` directly.

See the `MazurLevel14` section docstring for the descent. -/
theorem curve14a4_rational_points (x y : ℚ)
  ...
  simp only [curve14a4, WeierstrassCurve.toAffine] at he
  have hVT : (8 * y + 4 * x + 4) ^ 2 =
      (4 * x + 4) ^ 3 - 11 * (4 * x + 4) ^ 2 + 32 * (4 * x + 4) := by
    linear_combination (64 : ℚ) * he
  rcases MazurLevel14.T_trichotomy hVT with hT | hT | hT
  · have hx : x = -1 := by linarith
    have hsq : (8 * y + 4 * x + 4) ^ 2 = 0 := by rw [hVT, hT]; ring
    have hz : 8 * y + 4 * x + 4 = 0 := by
      have h0 : (8 * y + 4 * x + 4) * (8 * y + 4 * x + 4) = 0 := by linear_combination hsq
      rcases mul_eq_zero.mp h0 with h1 | h1 <;> exact h1
    have hy : y = 0 := by rw [hx] at hz; linarith
    simp [hx, hy]
  · have hx : x = 0 := by linarith
    have hsq : (8 * y + 4 * x + 4) ^ 2 = 16 := by rw [hVT, hT]; norm_num
    have hV : (8 * y + 4 * x + 4 - 4) * (8 * y + 4 * x + 4 + 4) = 0 := by
      linear_combination hsq
    rcases mul_eq_zero.mp hV with hz | hz
    · have hy : y = 0 := by rw [hx] at hz; linarith
      simp [hx, hy]
    · have hy : y = -1 := by rw [hx] at hz; linarith
      simp [hx, hy]
  · have hx : x = 1 := by linarith
    have hsq : (8 * y + 4 * x + 4) ^ 2 = 64 := by rw [hVT, hT]; norm_num
    have hV : (8 * y + 4 * x + 4 - 8) * (8 * y + 4 * x + 4 + 8) = 0 := by
      linear_combination hsq
    rcases mul_eq_zero.mp hV with hz | hz
    · have hy : y = 0 := by rw [hx] at hz; linarith
      simp [hx, hy]
    · have hy : y = -2 := by rw [hx] at hz; linarith
      simp [hx, hy]

/-- **Rank `0` for `14a4`** (PROVEN 2026-07-26), in the concrete form that
...
PROOF (2026-07-26). Not by the group-theoretic `2`-isogeny descent — that would
need `α` to be a homomorphism with kernel the image of the isogeny, i.e. the
whole descent exact sequence, none of which is in the pin. Instead by the
ELEMENTARY route of `MazurLevel14`: `curve14a4_rational_points` enumerates the
affine rational points unconditionally, so the point set is finite, so the
group is finite, so it is torsion. The residual arithmetic content sits in the
two homogeneous spaces `MazurLevel14.quartic_one` and
`MazurLevel14.quartic_two`.

Note this proof does NOT consume `curve14a4_finite` — that would be circular,
since `curve14a4_finite` consumes this. The finiteness used here is derived
locally from the enumeration. -/
theorem curve14a4_isTorsion : AddMonoid.IsTorsion curve14a4.toAffine.Point := by
  have hsub : {xy : ℚ × ℚ | curve14a4.toAffine.Nonsingular xy.1 xy.2} ⊆
      ({((1 : ℚ), (-2 : ℚ)), (0, -1), (-1, 0), (0, 0), (1, 0)} : Set (ℚ × ℚ)) := by
    rintro ⟨x, y⟩ hxy
    rcases curve14a4_rational_points x y hxy with h | h | h | h | h <;> rw [h] <;> simp
  haveI : Finite {xy : ℚ × ℚ // curve14a4.toAffine.Nonsingular xy.fst xy.snd} :=
    (Set.Finite.subset
      (((((Set.finite_singleton _).insert _).insert _).insert _).insert _) hsub).to_subtype
  haveI : Finite (WithZero {xy : ℚ × ℚ // curve14a4.toAffine.Nonsingular xy.fst xy.snd}) :=
    inferInstanceAs (Finite (Option _))
  haveI : Finite curve14a4.toAffine.Point :=
    Finite.of_equiv _ curve14a4.toAffine.nonsingularPointEquiv.symm

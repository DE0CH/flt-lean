...
Vélu coordinates (`velu_coordX_map`, `velu_coordY_map`); the invariance of
the Vélu coordinates under translation by the kernel and under negation
(`veluCoordX_add_mem`, `veluCoordY_add_mem`, `veluCoordX_neg`,
`veluCoordY_neg`, over the kernel sum `velu_sum_kernel`); and the
assemblies of `velu_isElliptic` and `velu_map_add` over the leaves below.
...
The first and the last are two faces of one fact — that the Vélu map is a
homomorphism with kernel exactly `S`, read on coordinates — and want one
owner: `velu_coord_ne_neg` is literally the degenerate branch of the
addition law in `velu_map_add_of_notMem`.

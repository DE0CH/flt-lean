...
/-! ### Galois transport of a CONJUGATED curve — DELETED 2026-08-02 (flt-lean-249)

A 295-line sorry-free `GaloisTransport` section stood here from 2026-07-31:
`Affine.Point.mapRingHom` / `mapRingEquiv` (the point map of a curve conjugation
along a ring map of the base field), `conjHom`, `IsRationalMap.transport`,
`IsIsogeny.transport`, `isIsogeny_conjHom_iff`, `_root_.AddEquiv.conjAddMonoidEnd`,
and `End.mapRingEquiv : End W ≃+* End (W.map σ)`.  Recover it with
`git show 280981f1:Fermat/FLT/EllipticCurve/Isogeny.lean`, lines 4254–4548.

**WHY IT WENT.**  Its only export, `End.mapRingEquiv`, had exactly one consumer in
the tree — `MazurCMForm.IsCMJInvariant.map` in `FreyCurve/MazurTorsion.lean` — and
that had none, so the block was FREE-FLOATING, which this project forbids.  It was
built for the rival cut of `exists_isCMJInvariant_ne_of_not_equivalent` that release
31 declined; the surviving `formPoint` route proves that theorem outright and does
not want stability.

**AND NO OTHER LEAF CAN ACQUIRE IT — checked 2026-08-02 over all five open leaves of
the `MazurCMForm` cluster.**  Galois STABILITY says the orbit of a CM `j`-invariant
stays INSIDE the set; every consumer needs the orbit to EXHAUST it, which is
transitivity, and mathlib's `Normal.minpoly_eq_iff_mem_orbit` already converts
between "same minimal polynomial" and "same orbit" in one line with no curve
conjugation anywhere.  `exists_monic_irreducible_vanishing_isCMJInvariant` (see its
ATOMICITY AUDIT, item 1), `minpoly_eq_of_isCMJInvariantOfRel` and
`exists_rat_of_isCMJInvariantOfRel` all route through that lemma;
`exists_smul_eq_of_jInvariant_eq` is pure modular-function theory and
`exists_isCMJInvariant_complexEmbedding_eq` is a construction, and neither mentions
Galois at all.

**THE ONE CHECK THAT WOULD JUSTIFY RESTORING IT**: a leaf whose statement conjugates
a curve whose COEFFICIENTS MOVE.  Every other Galois conjugation in this tree —
`exists_cmEndomorphism_classNumberOne`'s `hgal`, and the `Ksep`-descent blocks in
`KnownIn1980s/EllipticCurves/` — is along an ALGEBRA map that FIXES the curve, which
is mathlib's `Affine.Point.map` and needs none of this.  So grep a candidate for
`WeierstrassCurve.map` applied to a RING equivalence before rebuilding the block. -/

...
/-!
### Chord bricks for the vertical numerator

The vertical-numerator leaf `span_vertNumerator` is closed by an *ideal*
argument rather than a divisor/colength computation: mathlib's group-law
ideal identities `XYIdeal_mul_XYIdeal` / `XYIdeal_neg_mul` combine into
the **chord triple identity** `pointIdeal_chord`

  `I_R₁ · I_R₂ · I_{⊖(R₁⊕R₂)} = ⟨Y − ℓ(X − x₁) − y₁⟩`,

and the numerator itself factors, *division-free and up to the constant
`q₁ − x`*, as the product of the two chords `Q(⊖P)` and `QP` after
multiplication by the vertical `X − x` (`coordC_mul_XClass_mul_vertNumerator`).
Cancelling the (nonzero, hence cancellable) principal factor `⟨X − x⟩`
gives the leaf.  The degenerate configurations `P = ±Q` collapse `x` to
`q₁`, where the numerator becomes the tangent chord at `Q`
(`vertNumerator_self`), or — when `Q` is `2`-torsion — the vertical
`X − q₁` itself (`vertNumerator_self_of_negY`).

`pointIdeal_chord` is stated for a general pair of nonsingular affine
points, so it is directly reusable for the sibling leaf
`span_lineNumerator`: there the three chords to peel are `Q(⊖P)`,
`Q(⊖R)` and `Q(P ⊕ R)`, and the same "multiply, then cancel the
principal vertical" pattern applies with `⟨X − x₁⟩ · ⟨X − x₂⟩` in place
of `⟨X − x⟩`.
-/
...
/-- **The chord triple identity**: for two nonsingular affine points whose
sum is not `O`, the product of their point ideals with the point ideal of
`⊖(P ⊕ R)` is the principal ideal generated by the chord `Y − ℓ(X − x₁) − y₁`
through them. -/
theorem pointIdeal_chord {x₁ y₁ x₂ y₂ : F}
    (h₁ : W.Nonsingular x₁ y₁) (h₂ : W.Nonsingular x₂ y₂)
    (hxy : ¬(x₁ = x₂ ∧ y₁ = W.negY x₂ y₂)) :
    pointIdeal W (.some x₁ y₁ h₁) *
        (pointIdeal W (.some x₂ y₂ h₂) *
          pointIdeal W (-(Point.some x₁ y₁ h₁ + Point.some x₂ y₂ h₂))) =
      Ideal.span {CoordinateRing.YClass W
        (linePolynomial x₁ y₁ (W.slope x₁ x₂ y₁ y₂))} := by
  have h₃ : W.Nonsingular (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
      (W.addY x₁ x₂ y₁ (W.slope x₁ x₂ y₁ y₂)) := nonsingular_add h₁ h₂ hxy
  rw [Point.add_some hxy, Point.neg_some, pointIdeal_some, pointIdeal_some, pointIdeal_some]
  have key := CoordinateRing.XYIdeal_mul_XYIdeal h₁.left h₂.left hxy
  have hneg := CoordinateRing.XYIdeal_neg_mul h₃
  have hX3 : CoordinateRing.XClass W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂)) ≠ 0 :=
    CoordinateRing.XClass_ne_zero _
  rw [← Ideal.span_singleton_mul_right_inj hX3]
  calc Ideal.span {CoordinateRing.XClass W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))} *
        (CoordinateRing.XYIdeal W x₁ (C y₁) *
          (CoordinateRing.XYIdeal W x₂ (C y₂) *
            CoordinateRing.XYIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
              (C (W.negY (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
                (W.addY x₁ x₂ y₁ (W.slope x₁ x₂ y₁ y₂))))))
      = (CoordinateRing.XIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂)) *
          (CoordinateRing.XYIdeal W x₁ (C y₁) * CoordinateRing.XYIdeal W x₂ (C y₂))) *
        CoordinateRing.XYIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
          (C (W.negY (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
            (W.addY x₁ x₂ y₁ (W.slope x₁ x₂ y₁ y₂)))) := by
        rw [CoordinateRing.XIdeal]; ring
    _ = (CoordinateRing.YIdeal W (linePolynomial x₁ y₁ (W.slope x₁ x₂ y₁ y₂)) *
          CoordinateRing.XYIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
            (C (W.addY x₁ x₂ y₁ (W.slope x₁ x₂ y₁ y₂)))) *
        CoordinateRing.XYIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
          (C (W.negY (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
            (W.addY x₁ x₂ y₁ (W.slope x₁ x₂ y₁ y₂)))) := by rw [key]
    _ = CoordinateRing.YIdeal W (linePolynomial x₁ y₁ (W.slope x₁ x₂ y₁ y₂)) *
        (CoordinateRing.XYIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
            (C (W.negY (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
              (W.addY x₁ x₂ y₁ (W.slope x₁ x₂ y₁ y₂)))) *
          CoordinateRing.XYIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))
            (C (W.addY x₁ x₂ y₁ (W.slope x₁ x₂ y₁ y₂)))) := by ring
    _ = CoordinateRing.YIdeal W (linePolynomial x₁ y₁ (W.slope x₁ x₂ y₁ y₂)) *
        CoordinateRing.XIdeal W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂)) := by rw [hneg]
    _ = Ideal.span {CoordinateRing.XClass W (W.addX x₁ x₂ (W.slope x₁ x₂ y₁ y₂))} *
        Ideal.span {CoordinateRing.YClass W
          (linePolynomial x₁ y₁ (W.slope x₁ x₂ y₁ y₂))} := by
        rw [CoordinateRing.XIdeal, CoordinateRing.YIdeal, mul_comm]

omit [DecidableEq F] in
/-- A nonzero constant of the coordinate ring is a unit. -/
lemma isUnit_coordC {c : F} (hc : c ≠ 0) : IsUnit (coordC W c) :=
  isUnit_iff_exists.mpr ⟨coordC W c⁻¹, by
    rw [coordC, coordC, ← map_mul, ← C_mul, ← C_mul, mul_inv_cancel₀ hc, C_1, C_1, map_one],
    by rw [coordC, coordC, ← map_mul, ← C_mul, ← C_mul, inv_mul_cancel₀ hc, C_1, C_1, map_one]⟩

omit [DecidableEq F] in
/-- A `mk`-form chord element, rewritten as a scalar multiple of the line
class it generates. -/
lemma mk_chord_factor (q₁ q₂ c₁ c₂ ℓ : F) (hc : c₁ * ℓ = c₂) :
    CoordinateRing.mk W (C (C c₁) * (Y - C (C q₂)) - C (C c₂) * C (X - C q₁)) =
      coordC W c₁ * CoordinateRing.YClass W (linePolynomial q₁ q₂ ℓ) := by
  simp only [coordC, CoordinateRing.YClass, ← map_mul]
  congr 1
  rw [linePolynomial, ← hc]
  C_simp
  ring1

omit [DecidableEq F] in
/-- **The chord factorisation of the vertical numerator** (`P ≠ ±Q` case,
stated division-free): `(X − x) · vertNumerator` is, up to the constant
`q₁ − x`, the product of the two chords `Q(⊖P)` and `QP`. -/
lemma coordC_mul_XClass_mul_vertNumerator {q₁ q₂ x y : F}
    (hq : W.Equation q₁ q₂) (h : W.Equation x y) :
    coordC W (q₁ - x) * (CoordinateRing.XClass W x * vertNumerator W q₁ q₂ x) =
      CoordinateRing.mk W (C (C (q₁ - x)) * (Y - C (C q₂))
          - C (C (q₂ - W.negY x y)) * C (X - C q₁)) *
        CoordinateRing.mk W (C (C (q₁ - x)) * (Y - C (C q₂))
          - C (C (q₂ - y)) * C (X - C q₁)) := by
  rw [equation_iff] at hq h
  simp only [coordC, coordX, coordY, vertNumerator, CoordinateRing.XClass, ← map_mul, ← map_sub,
    ← map_add, ← map_pow]
  refine AdjoinRoot.mk_eq_mk.mpr ⟨C (C (q₁ - x) * (X - C q₁)), ?_⟩
  rw [WeierstrassCurve.Affine.polynomial, negY]
  linear_combination (norm := (C_simp; ring1))
    (C (X - C q₁) ^ 2) * congr_arg C (congr_arg C h)
      - (C (X - C q₁) ^ 2 + C (C (q₁ - x)) * C (X - C q₁)) * congr_arg C (congr_arg C hq)

omit [DecidableEq F] in
/-- **The tangent form of the vertical numerator** (`P = ±Q` case): at
`x = q₁` the numerator is the chord element of the tangent at `Q`. -/
lemma vertNumerator_self {q₁ q₂ : F} (hq : W.Equation q₁ q₂) :
    vertNumerator W q₁ q₂ q₁ =
      CoordinateRing.mk W (C (C (W.negY q₁ q₂ - q₂)) * (Y - C (C q₂))
        - C (C (W.a₁ * q₂ - (3 * q₁ ^ 2 + 2 * W.a₂ * q₁ + W.a₄))) * C (X - C q₁)) := by
  rw [equation_iff] at hq
  simp only [coordC, coordX, coordY, vertNumerator, ← map_mul, ← map_sub, ← map_add, ← map_pow]
  refine AdjoinRoot.mk_eq_mk.mpr ⟨1, ?_⟩
  rw [WeierstrassCurve.Affine.polynomial, negY]
  linear_combination (norm := (C_simp; ring1)) -congr_arg C (congr_arg C hq)

omit [DecidableEq F] in
/-- **The vertical form of the vertical numerator** (`P = ±Q` with `Q` of
order `2`): at `x = q₁` and `q₂ = negY q₁ q₂` the numerator is a constant
multiple of the vertical class `X − q₁`. -/
lemma vertNumerator_self_of_negY {q₁ q₂ : F} (hq : W.Equation q₁ q₂)
    (hA : q₂ = W.negY q₁ q₂) :
    vertNumerator W q₁ q₂ q₁ =
      coordC W (-(W.a₁ * q₂ - (3 * q₁ ^ 2 + 2 * W.a₂ * q₁ + W.a₄)))
        * CoordinateRing.XClass W q₁ := by
  rw [vertNumerator_self hq]
  simp only [coordC, CoordinateRing.XClass, ← map_mul]
  congr 1
  rw [← hA, sub_self]
  C_simp
  ring1
...
ROUTE TAKEN (chord calculus; no colength, no Nakayama, no Dedekind
machinery — the analysed "inclusion + colength" recipe was superseded).
Everything runs inside mathlib's group-law ideal identities, made
cancellative by working with *principal* factors in the domain `F[W]`:

(1) `pointIdeal_chord` packages `XYIdeal_mul_XYIdeal` together with
`XYIdeal_neg_mul` into `I_R₁ · I_R₂ · I_{⊖(R₁⊕R₂)} = ⟨Y − ℓ(X−x₁) − y₁⟩`
— the chord through two points and the third intersection.

(2) *Generic case `P ≠ ±Q`* (equivalently `q₁ ≠ x`): apply (1) to the
pairs `(Q, ⊖P)` — whose third point is `P⊖Q` — and `(Q, P)` — whose
third point is `⊖P⊖Q`.  Multiplying the two identities and using
`I_P · I_{⊖P} = ⟨X − x⟩` gives
`I_Q² · I_{P⊖Q} · I_{⊖P⊖Q} · ⟨X − x⟩ = ⟨chord₁ · chord₂⟩`.
The matching *element* identity is `coordC_mul_XClass_mul_vertNumerator`:
`(q₁−x) · (X−x) · n = g₁ · g₂` with `gᵢ` the two chords cleared of their
`q₁−x` denominators — a division-free `AdjoinRoot.mk_eq_mk` certificate
with cofactor `(q₁−x)(X−q₁)` and `linear_combination` coefficients
`u²` on the equation of `P` and `−(u² + (q₁−x)u)` on that of `Q`,
`u := X − q₁`.  Cancelling the nonzero principal `⟨X − x⟩`
(`Ideal.span_singleton_mul_right_inj`) closes it.

(3) *Degenerate case `P = ±Q`* (`q₁ = x`, both signs give the same
numerator and the same right-hand side, one factor being `I_O = ⊤`):
the target collapses to `I_Q · I_Q · I_{⊖2Q}`.  If `Q` is not
`2`-torsion this is (1) at the pair `(Q, Q)` and the numerator is the
tangent chord at `Q` (`vertNumerator_self`); if `Q` is `2`-torsion the
numerator is the vertical `X − q₁` itself
(`vertNumerator_self_of_negY`, nonzero constant by nonsingularity) and
the target is `XYIdeal_neg_mul`.

Note `W.Δ ≠ 0` is not needed: the identity holds for every nonsingular
pair of affine points (the binder is kept for the callers).  Neither is `IsAlgClosed F`; it is
left in scope (rather than `omit`ted) so that the consumers downstream,
which do need it, keep their section variables used. -/
theorem span_vertNumerator (_hΔ : W.Δ ≠ 0) {q₁ q₂ x y : F}
  ...
  have hz : pointIdeal W (0 : W.Point) = ⊤ := rfl
  by_cases hxq : q₁ = x
  · -- `P = ±Q`: the numerator is the tangent at `Q` (or a vertical).
    subst hxq
    have hRHS : pointIdeal W (Point.some q₁ q₂ hq) ^ 2 *
        (pointIdeal W (Point.some q₁ y h - Point.some q₁ q₂ hq) *
          pointIdeal W (-Point.some q₁ y h - Point.some q₁ q₂ hq)) =
        pointIdeal W (Point.some q₁ q₂ hq) *
          (pointIdeal W (Point.some q₁ q₂ hq) *
            pointIdeal W (-(Point.some q₁ q₂ hq + Point.some q₁ q₂ hq))) := by
      rcases Y_eq_of_X_eq h.left hq.left rfl with hy | hy
      · have hPQ : Point.some q₁ y h = Point.some q₁ q₂ hq := by subst hy; rfl
        have h1 : Point.some q₁ q₂ hq - Point.some q₁ q₂ hq = 0 := sub_self _
        have h2 : -Point.some q₁ q₂ hq - Point.some q₁ q₂ hq
            = -(Point.some q₁ q₂ hq + Point.some q₁ q₂ hq) := by abel
        rw [hPQ, h1, h2, hz, ← Ideal.one_eq_top, one_mul, pow_two, mul_assoc]
      · have hPQ : Point.some q₁ y h = -Point.some q₁ q₂ hq := by subst hy; rfl
        have h1 : -Point.some q₁ q₂ hq - Point.some q₁ q₂ hq
            = -(Point.some q₁ q₂ hq + Point.some q₁ q₂ hq) := by abel
        have h2 : -(-Point.some q₁ q₂ hq) - Point.some q₁ q₂ hq = 0 := by abel
        rw [hPQ, h1, h2, hz, ← Ideal.one_eq_top, mul_one, pow_two, mul_assoc]
    rw [hRHS]
    by_cases hA : q₂ = W.negY q₁ q₂
    · -- `Q` is `2`-torsion: the numerator is the vertical `X − q₁`.
      have hQQ : Point.some q₁ q₂ hq + Point.some q₁ q₂ hq = 0 := Point.add_of_Y_eq rfl hA
      have hA' : 2 * q₂ + W.a₁ * q₁ + W.a₃ = 0 := by
        rw [negY] at hA; linear_combination hA
      have hc₂ : -(W.a₁ * q₂ - (3 * q₁ ^ 2 + 2 * W.a₂ * q₁ + W.a₄)) ≠ 0 := by
        rcases ((nonsingular_iff' ..).mp hq).2 with h' | h'
        · exact neg_ne_zero.mpr h'
        · exact absurd hA' h'
      have hv := vertNumerator_self_of_negY hq.left hA
      have key := CoordinateRing.XYIdeal_neg_mul hq
      rw [← hA] at key
      rw [hQQ, neg_zero, hz, ← Ideal.one_eq_top, mul_one, hv,
        Ideal.span_singleton_mul_left_unit (isUnit_coordC hc₂), pointIdeal_some, key,
        CoordinateRing.XIdeal]
    · -- `Q` is not `2`-torsion: the numerator is the tangent chord at `Q`.
      have hden : q₂ - W.negY q₁ q₂ ≠ 0 := sub_ne_zero_of_ne hA
      have hc : (W.negY q₁ q₂ - q₂) * W.slope q₁ q₁ q₂ q₂
          = W.a₁ * q₂ - (3 * q₁ ^ 2 + 2 * W.a₂ * q₁ + W.a₄) := by
        rw [slope_of_Y_ne rfl hA, ← mul_div_assoc, div_eq_iff hden]
        ring
      rw [pointIdeal_chord hq hq (fun hcc => hA hcc.2), vertNumerator_self hq.left,
        mk_chord_factor q₁ q₂ _ _ _ hc,
        Ideal.span_singleton_mul_left_unit
          (isUnit_coordC (sub_ne_zero_of_ne (Ne.symm hA)))]
  · -- `P ≠ ±Q`: the generic chord factorisation.
    have hne : q₁ ≠ x := hxq
    have hd : q₁ - x ≠ 0 := sub_ne_zero_of_ne hne
    have hnegP : W.Nonsingular x (W.negY x y) := (nonsingular_neg x y).mpr h
    have hxy₁ : ¬(q₁ = x ∧ q₂ = W.negY x (W.negY x y)) := fun hcc => hne hcc.1
    have hxy₂ : ¬(q₁ = x ∧ q₂ = W.negY x y) := fun hcc => hne hcc.1
    have hnegeq : Point.some x (W.negY x y) hnegP = -Point.some x y h := rfl
    have e₁ := pointIdeal_chord hq hnegP hxy₁
    have e₂ := pointIdeal_chord hq h hxy₂
    rw [hnegeq] at e₁
    have hs : -(Point.some q₁ q₂ hq + -Point.some x y h)
        = Point.some x y h - Point.some q₁ q₂ hq := by abel
    have ht : -(Point.some q₁ q₂ hq + Point.some x y h)
        = -Point.some x y h - Point.some q₁ q₂ hq := by abel
    rw [hs] at e₁
    rw [ht] at e₂
    have e₃ : pointIdeal W (-Point.some x y h) * pointIdeal W (Point.some x y h)
        = Ideal.span {CoordinateRing.XClass W x} := by
      rw [← hnegeq, pointIdeal_some, pointIdeal_some, CoordinateRing.XYIdeal_neg_mul h,
        CoordinateRing.XIdeal]
    have hell₁ : (q₁ - x) * W.slope q₁ x q₂ (W.negY x y) = q₂ - W.negY x y := by
      rw [slope_of_X_ne hne, ← mul_div_assoc, div_eq_iff hd]; ring
    have hell₂ : (q₁ - x) * W.slope q₁ x q₂ y = q₂ - y := by
      rw [slope_of_X_ne hne, ← mul_div_assoc, div_eq_iff hd]; ring
    have hfac := coordC_mul_XClass_mul_vertNumerator (W := W) (x := x) (y := y) hq.left h.left
    rw [mk_chord_factor q₁ q₂ _ _ _ hell₁, mk_chord_factor q₁ q₂ _ _ _ hell₂] at hfac
    have hfac' : CoordinateRing.XClass W x * vertNumerator W q₁ q₂ x
        = coordC W (q₁ - x) *
          (CoordinateRing.YClass W (linePolynomial q₁ q₂ (W.slope q₁ x q₂ (W.negY x y))) *
            CoordinateRing.YClass W (linePolynomial q₁ q₂ (W.slope q₁ x q₂ y))) := by
      refine mul_left_cancel₀ (isUnit_coordC (W := W) hd).ne_zero ?_
      rw [hfac]; ring
    refine (Ideal.span_singleton_mul_right_inj
      (CoordinateRing.XClass_ne_zero (W' := W) x)).mp ?_
    calc Ideal.span {CoordinateRing.XClass W x} * Ideal.span {vertNumerator W q₁ q₂ x}
        = Ideal.span {CoordinateRing.XClass W x * vertNumerator W q₁ q₂ x} :=
          Ideal.span_singleton_mul_span_singleton _ _
      _ = Ideal.span {CoordinateRing.YClass W
              (linePolynomial q₁ q₂ (W.slope q₁ x q₂ (W.negY x y))) *
            CoordinateRing.YClass W (linePolynomial q₁ q₂ (W.slope q₁ x q₂ y))} := by
          rw [hfac', Ideal.span_singleton_mul_left_unit (isUnit_coordC hd)]
      _ = (pointIdeal W (Point.some q₁ q₂ hq) *
            (pointIdeal W (-Point.some x y h) *
              pointIdeal W (Point.some x y h - Point.some q₁ q₂ hq))) *
          (pointIdeal W (Point.some q₁ q₂ hq) *
            (pointIdeal W (Point.some x y h) *
              pointIdeal W (-Point.some x y h - Point.some q₁ q₂ hq))) := by
          rw [e₁, e₂, Ideal.span_singleton_mul_span_singleton]
      _ = Ideal.span {CoordinateRing.XClass W x} *
          (pointIdeal W (Point.some q₁ q₂ hq) ^ 2 *
            (pointIdeal W (Point.some x y h - Point.some q₁ q₂ hq) *
              pointIdeal W (-Point.some x y h - Point.some q₁ q₂ hq))) := by
          rw [← e₃]; ring

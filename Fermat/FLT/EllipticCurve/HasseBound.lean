...
`natDegree_ΨSq_ne_zero_of_not_dvd` (the curve is ORDINARY when `q ∤ c`, stated
as the non-constancy of `ΨSq_q`; `exists_ne_zero_qTorsion` is PROVEN over it,
see the SEVENTH CUT, so do NOT dispatch at that name).

SEVENTH CUT, 2026-07-30: `exists_ne_zero_qTorsion` is PROVEN, over
`natDegree_ΨSq_ne_zero_of_not_dvd` — the SAME mathematics with the point-group
layer removed.  What closed is a dictionary general enough to be worth naming,
`TorsionCharP.exists_ne_zero_torsion_iff_natDegree_ΨSq_ne_zero`:

    over an algebraically closed field, for every `n ≠ 0`,
    (∃ P ≠ 0, n • P = 0)  ↔  (ΨSqₙ).natDegree ≠ 0,

characteristic-agnostic, out of `TorsionCard.smul_some_eq_zero_iff`,
`TorsionCharP.ΨSq_ne_zero`, and the surjectivity of the `x`-coordinate map over
an algebraically closed field (`exists_nonsingular_of_isAlgClosed`, also new).
The frontier count is UNCHANGED at four — this is a re-cut, not a closure — but
the surviving leaf is now about ONE EXPLICIT POLYNOMIAL OVER `𝔽_q`, with no
algebraic closure, no `Affine.Point` and no Frobenius endomorphism in its
conclusion, so it is finite and machine-checkable at any given `q`.  It was
checked that way: all 328 nonsingular `y² = x³ + ax + b` over `q ∈ {5, 7, 11,
13}`, `q ∤ a_q` against `deg ψ_q > 0`, agreeing in BOTH directions with zero
mismatches (PARI/GP; see the FALSITY AUDIT on the leaf, which is the first
audit of THIS statement and does not inherit the earlier ones).
...
`natCard_ker_degreeFormEnd_of_dvd`.  That is now TWO PROVEN declarations here,
`injective_frobeniusPointEnd` and `surjective_frobeniusPointEnd`; consume them
rather than re-deriving them.  (Corrected 2026-07-30: this clause read "not yet
a declaration here", which the audit on `natCard_ker_degreeFormEnd_le` had
flagged as stale but deliberately left alone while a concurrent owner held
those lines.)
-/
module
...
because `𝔽̄_q` is algebraically closed and perfect), which is now two PROVEN
declarations below, `injective_frobeniusPointEnd` and
`surjective_frobeniusPointEnd` — consume them rather than re-deriving them.
(Corrected 2026-07-30; this clause read "not yet a declaration in this file".)
...
**(3) A stale claim, NOW CORRECTED IN PLACE (2026-07-30).**  The docstring of
`sq_frobeniusPointEnd_qPrimary` above ended "Both want `F` BIJECTIVE on
`Wbar(𝔽̄_q)` …, which is not yet a declaration in this file".  That clause was
...
should consume them rather than re-derive them.  This audit originally recorded
it here rather than fixing it there, because that leaf had a live concurrent
owner rewriting exactly those lines; that owner is long gone, and the clause has
now been fixed both there and in the MODULE HEADER, which carried the same stale
sentence and is the more damaging of the two since it is what a dispatcher
reads. -/
...
theorem natCard_ker_zsmul_q_pow (q : ℕ) [Fact q.Prime]
  ...
  | succ v ih =>
    ...
    rw [hmul, natCard_ker_mul _ _ (surjective_zsmul_one q Wbar hq0),
      natCard_ker_zsmul_q q Wbar hc hqc, ih, pow_succ]
    ring

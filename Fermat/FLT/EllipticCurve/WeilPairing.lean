...
affine curve is integral and proper over the constants): extracted
`hCunits` of the μ-proof. -/
theorem coordRing_isUnit_constant (q : ℕ) [Fact q.Prime]
    (Wb : WeierstrassCurve (AlgebraicClosure (ZMod q))) :
    ∀ u : Wb.toAffine.CoordinateRing, IsUnit u →
      ∃ c : (AlgebraicClosure (ZMod q)), c ≠ 0 ∧
        u = AdjoinRoot.of Wb.toAffine.polynomial (Polynomial.C c) := by
  intro u hu
  obtain ⟨pp, qq, rfl⟩ :=
...
/-- **The chord/tangent line's divisor span** (the line through two
affine points, tangent if they coincide, meets the curve additionally
in the negated sum): extracted `hline` of the μ-proof.  The Dedekind
instance on the coordinate ring (derived inside the μ-proof as `hDD`)
is taken as an instance hypothesis — ideal cancellation needs it. -/
theorem coordRing_line_span (q : ℕ) [Fact q.Prime]
    (Wb : WeierstrassCurve (AlgebraicClosure (ZMod q)))
    [IsDedekindDomain Wb.toAffine.CoordinateRing] :
    ∀ (x₁ y₁ x₂ y₂ : (AlgebraicClosure (ZMod q)))
      (_ : Wb.toAffine.Nonsingular x₁ y₁)
      (_ : Wb.toAffine.Nonsingular x₂ y₂)
      (_ : ¬(x₁ = x₂ ∧ y₁ = Wb.toAffine.negY x₂ y₂)),
...
/-- **Evaluation of a vertical-line class**: extracted `hevvert` of the
μ-proof. -/
theorem coordRing_evalEval_XClass (q : ℕ) [Fact q.Prime]
    (Wb : WeierstrassCurve (AlgebraicClosure (ZMod q))) :
    ∀ (c x y : (AlgebraicClosure (ZMod q))) (hE : Wb.toAffine.Equation x y),
...
`XYIdeal_neg_mul` + `hline` (`hfac`), evaluation at the four
configuration points turns the value equation into the CAS-verified
cross-ratio identity `hkey` (HLEG-NOTES.md §3(c-i)), and the
`x• − x_P` factors cancel by the genericity hypotheses. -/
theorem weilValue_two_torsion_config_eq_one {F : Type*} [Field F]
    [DecidableEq F] (W : WeierstrassCurve.Affine F)
    ...
    (haP : Ideal.span {aP} =
      ...
      (WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xS
        (Polynomial.C (W.negY xS yS))) ^ 2)
    (haQ : Ideal.span {aQ} =
      (WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xQR
        (Polynomial.C yQR)) ^ 2 *
      (WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xR
        (Polynomial.C (W.negY xR yR))) ^ 2)
    (hA : (AdjoinRoot.evalEval hQR.left
        ((WeierstrassCurve.Affine.CoordinateRing.XClass W xS) ^ 2) *
      AdjoinRoot.evalEval hR.left aP *
      AdjoinRoot.evalEval hS.left
        ((WeierstrassCurve.Affine.CoordinateRing.XClass W xR) ^ 2) *
      AdjoinRoot.evalEval hPS.left aQ) ≠ 0)
    ...
    z = 1 := by
  classical
  -- the line and constant evaluation bricks
  have hevline : ∀ (x₀ y₀ L xe ye : F) (hE : W.Equation xe ye),
      AdjoinRoot.evalEval (p := W.polynomial) hE
        (WeierstrassCurve.Affine.CoordinateRing.YClass W
          (WeierstrassCurve.Affine.linePolynomial x₀ y₀ L)) =
      ye - (L * (xe - x₀) + y₀) := by
    intro x₀ y₀ L xe ye hE
    rw [show WeierstrassCurve.Affine.CoordinateRing.YClass W
      (WeierstrassCurve.Affine.linePolynomial x₀ y₀ L) =
      AdjoinRoot.mk W.polynomial
        (Polynomial.X - Polynomial.C
          (WeierstrassCurve.Affine.linePolynomial x₀ y₀ L))
      from rfl, AdjoinRoot.evalEval_mk]
    simp [Polynomial.evalEval, WeierstrassCurve.Affine.linePolynomial]
  have hevconst : ∀ (c xe ye : F) (hE : W.Equation xe ye),
      AdjoinRoot.evalEval (p := W.polynomial) hE
        (AdjoinRoot.of W.polynomial (Polynomial.C c)) = c := by
    intro c xe ye hE
    rw [show AdjoinRoot.of W.polynomial (Polynomial.C c) =
      AdjoinRoot.mk W.polynomial (Polynomial.C (Polynomial.C c))
      from rfl, AdjoinRoot.evalEval_mk]
    simp [Polynomial.evalEval]
  -- genericity of the two chords/tangents
  have hPne : ¬(xPS = xS ∧ yPS = W.negY xS (W.negY xS yS)) := by
    rintro ⟨hx, hy⟩
    rw [W.negY_negY] at hy
    subst hx
    subst hy
    have h0 : WeierstrassCurve.Affine.Point.some xP yP hP = 0 := by
      have h := add_right_cancel
        (a := WeierstrassCurve.Affine.Point.some xP yP hP)
        (b := WeierstrassCurve.Affine.Point.some xPS yPS hS) (c := 0)
      rw [zero_add] at h
      exact h hPSc.symm
    exact WeierstrassCurve.Affine.Point.some_ne_zero hP h0
  have hQne : ¬(xQR = xR ∧ yQR = W.negY xR (W.negY xR yR)) := by
    rintro ⟨hx, hy⟩
    rw [W.negY_negY] at hy
    subst hx
    subst hy
    have h0 : WeierstrassCurve.Affine.Point.some xP yP hP = 0 := by
      have h := add_right_cancel
        (a := WeierstrassCurve.Affine.Point.some xP yP hP)
        (b := WeierstrassCurve.Affine.Point.some xQR yQR hR) (c := 0)
      rw [zero_add] at h
      exact h hQRc.symm
    exact WeierstrassCurve.Affine.Point.some_ne_zero hP h0
  -- the slopes of the two lines
  set L₁ : F := W.slope xPS xS yPS (W.negY xS yS) with hL₁
  set L₂ : F := W.slope xQR xR yQR (W.negY xR yR) with hL₂
  have hnegS : W.Nonsingular xS (W.negY xS yS) :=
    (WeierstrassCurve.Affine.nonsingular_neg ..).mpr hS
  have hnegR : W.Nonsingular xR (W.negY xR yR) :=
    (WeierstrassCurve.Affine.nonsingular_neg ..).mpr hR
  -- the third point of each line is `P`
  have hthird₁ : W.addX xPS xS L₁ = xP ∧ W.addY xPS xS yPS L₁ = yP := by
    have hP' : WeierstrassCurve.Affine.Point.some xP yP hP =
        WeierstrassCurve.Affine.Point.some xPS yPS hPS +
          WeierstrassCurve.Affine.Point.some xS (W.negY xS yS) hnegS := by
      rw [show WeierstrassCurve.Affine.Point.some xS (W.negY xS yS)
        hnegS = -WeierstrassCurve.Affine.Point.some xS yS hS from rfl,
        ← sub_eq_add_neg]
      exact eq_sub_of_add_eq hPSc.symm
    rw [WeierstrassCurve.Affine.Point.add_some hPne] at hP'
    obtain ⟨h1, h2⟩ := WeierstrassCurve.Affine.Point.some.inj hP'.symm
    exact ⟨h1, h2⟩
  have hthird₂ : W.addX xQR xR L₂ = xP ∧ W.addY xQR xR yQR L₂ = yP := by
    have hP' : WeierstrassCurve.Affine.Point.some xP yP hP =
        WeierstrassCurve.Affine.Point.some xQR yQR hQR +
          WeierstrassCurve.Affine.Point.some xR (W.negY xR yR) hnegR := by
      rw [show WeierstrassCurve.Affine.Point.some xR (W.negY xR yR)
        hnegR = -WeierstrassCurve.Affine.Point.some xR yR hR from rfl,
        ← sub_eq_add_neg]
      exact eq_sub_of_add_eq hQRc.symm
    rw [WeierstrassCurve.Affine.Point.add_some hQne] at hP'
    obtain ⟨h1, h2⟩ := WeierstrassCurve.Affine.Point.some.inj hP'.symm
    exact ⟨h1, h2⟩
  -- line-square factorizations of the two Miller generators
  have hfac : ∀ (xT yT xU yU : F) (hT : W.Nonsingular xT yT)
      (hU : W.Nonsingular xU yU) (a : W.CoordinateRing) (LTU : F)
      (hLTU : LTU = W.slope xT xU yT (W.negY xU yU))
      (hTUne : ¬(xT = xU ∧ yT = W.negY xU (W.negY xU yU)))
      (hth : W.addX xT xU LTU = xP ∧ W.addY xT xU yT LTU = yP)
      (ha : Ideal.span {a} =
        (WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xT
          (Polynomial.C yT)) ^ 2 *
        (WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xU
          (Polynomial.C (W.negY xU yU))) ^ 2),
      ∃ c : F, c ≠ 0 ∧
        a * WeierstrassCurve.Affine.CoordinateRing.XClass W xP =
          AdjoinRoot.of W.polynomial (Polynomial.C c) *
            (WeierstrassCurve.Affine.CoordinateRing.YClass W
              (WeierstrassCurve.Affine.linePolynomial xT yT LTU)) ^ 2 := by
    intro xT yT xU yU hT hU a LTU hLTU hTUne hth ha
    have hlin := hline xT yT xU (W.negY xU yU) hT
      ((WeierstrassCurve.Affine.nonsingular_neg ..).mpr hU) hTUne
    rw [← hLTU, hth.1, hth.2] at hlin
    have hXsq : WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xP
          (Polynomial.C yP) *
        WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xP
          (Polynomial.C yP) =
        WeierstrassCurve.Affine.CoordinateRing.XIdeal W xP := by
      conv_lhs => rw [show WeierstrassCurve.Affine.CoordinateRing.XYIdeal
        W xP (Polynomial.C yP) *
        WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xP
          (Polynomial.C yP) =
        WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xP
          (Polynomial.C (W.negY xP yP)) *
        WeierstrassCurve.Affine.CoordinateRing.XYIdeal W xP
          (Polynomial.C yP) from by rw [h2t]]
      exact WeierstrassCurve.Affine.CoordinateRing.XYIdeal_neg_mul hP
    have hspan : Ideal.span
        {a * WeierstrassCurve.Affine.CoordinateRing.XClass W xP} =
        Ideal.span {(WeierstrassCurve.Affine.CoordinateRing.YClass W
          (WeierstrassCurve.Affine.linePolynomial xT yT LTU)) ^ 2} := by
      rw [← Ideal.span_singleton_mul_span_singleton, ha,
        ← Ideal.span_singleton_pow]
      rw [show Ideal.span {WeierstrassCurve.Affine.CoordinateRing.YClass
        W (WeierstrassCurve.Affine.linePolynomial xT yT LTU)} =
        WeierstrassCurve.Affine.CoordinateRing.YIdeal W
          (WeierstrassCurve.Affine.linePolynomial xT yT LTU) from rfl]
      rw [hlin, h2t]
      rw [show Ideal.span
        {WeierstrassCurve.Affine.CoordinateRing.XClass W xP} =
        WeierstrassCurve.Affine.CoordinateRing.XIdeal W xP from rfl]
      rw [← hXsq]
      ring
    obtain ⟨u, hu⟩ := Ideal.span_singleton_eq_span_singleton.mp hspan
    obtain ⟨c, hc0, hcu⟩ := hCunits u.val u.isUnit
    refine ⟨c⁻¹, inv_ne_zero hc0, ?_⟩
    have h1 : a * WeierstrassCurve.Affine.CoordinateRing.XClass W xP *
        AdjoinRoot.of W.polynomial (Polynomial.C c) =
        (WeierstrassCurve.Affine.CoordinateRing.YClass W
          (WeierstrassCurve.Affine.linePolynomial xT yT LTU)) ^ 2 := by
      rw [← hcu, hu]
    calc a * WeierstrassCurve.Affine.CoordinateRing.XClass W xP
        = a * WeierstrassCurve.Affine.CoordinateRing.XClass W xP *
          (AdjoinRoot.of W.polynomial (Polynomial.C c) *
           AdjoinRoot.of W.polynomial (Polynomial.C c⁻¹)) := by
          rw [← map_mul, ← Polynomial.C_mul, mul_inv_cancel₀ hc0,
            Polynomial.C_1, map_one, mul_one]
      _ = AdjoinRoot.of W.polynomial (Polynomial.C c⁻¹) *
          (a * WeierstrassCurve.Affine.CoordinateRing.XClass W xP *
            AdjoinRoot.of W.polynomial (Polynomial.C c)) := by ring
      _ = _ := by rw [h1]
  obtain ⟨c₁, hc₁0, hfac₁⟩ := hfac xPS yPS xS yS hPS hS aP L₁ hL₁ hPne
    hthird₁ haP
  obtain ⟨c₂, hc₂0, hfac₂⟩ := hfac xQR yQR xR yR hQR hR aQ L₂ hL₂ hQne
    hthird₂ haQ
  -- evaluate the factorizations at the four points
  have hev : ∀ (a : W.CoordinateRing) (c xT yT LT xe ye : F)
      (hE : W.Equation xe ye),
      a * WeierstrassCurve.Affine.CoordinateRing.XClass W xP =
        AdjoinRoot.of W.polynomial (Polynomial.C c) *
          (WeierstrassCurve.Affine.CoordinateRing.YClass W
            (WeierstrassCurve.Affine.linePolynomial xT yT LT)) ^ 2 →
      AdjoinRoot.evalEval (p := W.polynomial) hE a * (xe - xP) =
        c * (ye - (LT * (xe - xT) + yT)) ^ 2 := by
    intro a c xT yT LT xe ye hE hfa
    have h := congrArg (AdjoinRoot.evalEval (p := W.polynomial) hE) hfa
    rw [map_mul, map_mul, map_pow, hevvert xP xe ye hE,
      hevconst c xe ye hE, hevline xT yT LT xe ye hE] at h
    exact h
  have eA1 := hev aP c₁ xPS yPS L₁ xQR yQR hQR.left hfac₁
  have eA2 := hev aP c₁ xPS yPS L₁ xR yR hR.left hfac₁
  have eB1 := hev aQ c₂ xQR yQR L₂ xS yS hS.left hfac₂
  have eB2 := hev aQ c₂ xQR yQR L₂ xPS yPS hPS.left hfac₂
  -- rewrite the vertical atoms in the value equation
  rw [map_pow, map_pow, hevvert xS xQR yQR hQR.left,
  ...
  have hD3 : xS - xP ≠ 0 := sub_ne_zero.mpr hxSP
  have hD4 : xPS - xP ≠ 0 := sub_ne_zero.mpr hxPSP
  have hAB : (xQR - xS) ^ 2 * AdjoinRoot.evalEval hR.left aP *
      (xS - xR) ^ 2 * AdjoinRoot.evalEval hPS.left aQ =
      AdjoinRoot.evalEval hQR.left aP * (xR - xS) ^ 2 *
      AdjoinRoot.evalEval hS.left aQ * (xPS - xR) ^ 2 := by
    have hD : (xQR - xP) * ((xR - xP) * ((xS - xP) * (xPS - xP))) ≠ 0 :=
      mul_ne_zero hD1 (mul_ne_zero hD2 (mul_ne_zero hD3 hD4))
    refine mul_right_cancel₀ hD ?_
    linear_combination
      ((xQR - xS) ^ 2 * (xS - xR) ^ 2 * (xQR - xP) * (xS - xP) *
        AdjoinRoot.evalEval hPS.left aQ * (xPS - xP)) * eA2 +
      ((xQR - xS) ^ 2 * (xS - xR) ^ 2 * (xQR - xP) * (xS - xP) *
        c₁ * (yR - (L₁ * (xR - xPS) + yPS)) ^ 2) * eB2 +
      (c₁ * c₂) * hkey -
      ((xR - xS) ^ 2 * (xPS - xR) ^ 2 * (xR - xP) * (xPS - xP) *
        AdjoinRoot.evalEval hS.left aQ * (xS - xP)) * eA1 -
      ((xR - xS) ^ 2 * (xPS - xR) ^ 2 * (xR - xP) * (xPS - xP) *
...
theorem weilValueProp_self_of_two (q : ℕ) [Fact q.Prime]
  ...
  · -- destructure the admissible setup for the affine self-pair
    ...
    -- the CAS-verified line-square value computation on the given setup
    -- (HLEG-NOTES.md §3(c-i)): the 2-torsion relation `⊖P = P` factors
    -- `aP·XClass x_P = c₁·ℓ₁²` (ℓ₁ the line through `PS, ⊖S, P`) and
    ...
    have h2t : (Wbar.map (algebraMap (ZMod q)
        (AlgebraicClosure (ZMod q)))).toAffine.negY xP yP = yP := by
      by_contra hne
      have h2 : ((2 : ℕ) : ℤ) • x.val = 0 :=
        (Submodule.mem_torsionBy_iff _ _).mp x.property
      rw [hcx, Nat.cast_ofNat, two_zsmul] at h2
      have hadd := WeierstrassCurve.Affine.Point.add_self_of_Y_ne
        (h₁ := hP) (fun h => hne h.symm)
...
/-- **Finite-subfield closure of a finite set** (top-level extraction
of the μ-proof's in-proof `hsubfin`): every finite set of elements of
`𝔽̄_q` is contained in a finite subfield — adjoin the (algebraic)
elements to `ZMod q` and take the resulting finite-dimensional
intermediate field. -/
theorem exists_finite_subfield_containing (q : ℕ) [Fact q.Prime]
    (s : Finset (AlgebraicClosure (ZMod q))) :
...
their four coordinates, there is a full admissible generic Miller
setup — the exact ∃-chain of the affine branch of `weilValueProp` —
whose data subfield `F` moreover contains `G`, and whose two
...
theorem exists_weilValueSetup_avoiding (q : ℕ) [Fact q.Prime]
    ...
    (hQ : (Wbar.map (algebraMap (ZMod q)
      (AlgebraicClosure (ZMod q)))).toAffine.Nonsingular xQ yQ)
    (htorP : (p : ℤ) • (WeierstrassCurve.Affine.Point.some xP yP hP :
      (Wbar.map (algebraMap (ZMod q)
        (AlgebraicClosure (ZMod q)))).toAffine.Point) = 0)
    (htorQ : (p : ℤ) • (WeierstrassCurve.Affine.Point.some xQ yQ hQ :
      (Wbar.map (algebraMap (ZMod q)
        (AlgebraicClosure (ZMod q)))).toAffine.Point) = 0)
    (hxPG : xP ∈ G) (hyPG : yP ∈ G) (hxQG : xQ ∈ G) (hyQG : yQ ∈ G) :
    ∃ (F F' : Subfield (AlgebraicClosure (ZMod q))),
      (F : Set (AlgebraicClosure (ZMod q))).Finite ∧
      (F' : Set (AlgebraicClosure (ZMod q))).Finite ∧ F ≤ F' ∧ G ≤ F ∧
      frobFixed q (2 * Nat.lcm
        (Nat.lcm (frobPeriod q xP) (frobPeriod q yP))
        (Nat.lcm (frobPeriod q xQ) (frobPeriod q yQ))) ≤ F ∧
      xP ∈ F ∧ yP ∈ F ∧ xQ ∈ F ∧ yQ ∈ F ∧
...
the affine points of the divisor support `{T'⊕κ} ∪ {±κ}` of `g`, of
the relevant `p²`-torsion translates, and of the finitely many further
collision points of the derivation — such that EVERY admissible Miller
setup for the pair `(κ₀, x)` whose data subfield `F` contains `G₀`
(hence whose `S/R/P⊕S/Q⊕R` abscissas avoid `G₀` through the setup's
own `F/F'`-avoidances) has Miller cross-ratio `c^{±1}`:
`numer = c^e·denom` with `e ∈ {1, p−1}`.

Proof plan (HLEG-NOTES.md §4(B), L4-9 second branch): the setup's
cross-ratio factors as `f_{κ₀}(D_x)/f_x(D_{κ₀})` with
`f_{κ₀} = aP/X_S^p` (`div = p(κ₀⊕S) − p(S)`), `f_x = aQ/X_R^p`
(`div = p(x⊕R) − p(R)`), `D_{κ₀} = (κ₀⊕S) − (S)`,
`D_x = (x⊕R) − (R)`.  Choose `p`-division points `S', R', κ₀'`
(`p•S' = S` etc., `TorsionCard.smul_surjective`; `hp0` supplies
`(p : 𝔽̄_q) ≠ 0`).  The `[p]`-pullback `f_x∘[p]` has divisor
`p·Σ_κ ((T'⊕R'⊕κ) − (R'⊕κ))`, the `τ`-translate by `R'` of `p·div g`,
so `f_x∘[p] = c'·(g∘τ_{R'})^p` (the L4-7 span comparison: the
division-polynomial pullback `Φ_p − x·Ψ_p²` of a vertical is separable
with the `p²`-fiber as its multiplicity-one root multiset — `hcard`
enumerates the fibers — and lines pull back via the `ω`-formula;
compare spans and finish by units-are-constants).  Evaluating the
BALANCED ratio at `κ₀'⊕S'` and `S'` cancels `c'`:
`f_x(D_{κ₀}) = [g(κ₀'⊕U)/g(U)]^p`, `U := S'⊖R'`.  The level-`p²`
telescope `∏_{j<p} χ'(jκ₀'⊕U)` with `χ'(ξ) := g(κ₀'⊕ξ)/g(ξ)` equals
`g(κ₀⊕U)/g(U) = c` (`heq` evaluated away from the divisor support of
`g`), collapsing the `p`-th power to `c` times coboundary terms; the
mirror computation for `f_{κ₀}(D_x)` (Weil-reciprocity symmetric, the
`aP`-side telescope) cancels the coboundaries, leaving `c^{±1}`.  The
...
theorem translationChar_setup_value (q : ℕ) [Fact q.Prime]
    ...
    (hP : (Wbar.map (algebraMap (ZMod q)
      (AlgebraicClosure (ZMod q)))).toAffine.Nonsingular xP yP)
    (hrepP : i₀.val = WeierstrassCurve.Affine.Point.some xP yP hP)
    (xQ yQ : AlgebraicClosure (ZMod q))
    (hQ : (Wbar.map (algebraMap (ZMod q)
      (AlgebraicClosure (ZMod q)))).toAffine.Nonsingular xQ yQ)
    (hrepQ : x.val = WeierstrassCurve.Affine.Point.some xQ yQ hQ) :
    ∃ G₀ : Subfield (AlgebraicClosure (ZMod q)),
      ...
      ∀ (F F' : Subfield (AlgebraicClosure (ZMod q))),
        ...
        ∀ (xR yR : AlgebraicClosure (ZMod q))
          (hR : (Wbar.map (algebraMap (ZMod q)
            (AlgebraicClosure (ZMod q)))).toAffine.Nonsingular xR yR),
          xR ∉ F' →
        ∀ (xPS yPS : AlgebraicClosure (ZMod q))
          (hPS : (Wbar.map (algebraMap (ZMod q)
            (AlgebraicClosure (ZMod q)))).toAffine.Nonsingular xPS yPS),
        ...
        ∀ (aP aQ : (Wbar.map (algebraMap (ZMod q)
          ...
          (AdjoinRoot.evalEval hQR.left aP *
              ...
              AdjoinRoot.evalEval hPS.left
                ((WeierstrassCurve.Affine.CoordinateRing.XClass
                  (Wbar.map (algebraMap (ZMod q)
                    (AlgebraicClosure (ZMod q)))).toAffine xR) ^ p)) ≠ 0 →
          ∃ e : ℕ, (e = 1 ∨ e = p - 1) ∧
            (AdjoinRoot.evalEval hQR.left aP *
              AdjoinRoot.evalEval hR.left
                ((WeierstrassCurve.Affine.CoordinateRing.XClass
...
`P ≠ O` (`hx0`) — over the two staged sorried leaves
`exists_weilValueSetup_avoiding` (Stage A: hexval-style admissible
setup existence, avoidance-parametrized) and
`translationChar_setup_value` (Stage B: the Ex. 3.16(c) cross-ratio
evaluation over any setup avoiding the character data's bad
subfield)): the nontrivial translation character value `c` of the
Miller generator is realized as a nontrivial admissible Weil value for
the pair `(κ₀, x)` itself.  Assembly: Stage B provides the finite bad
subfield `G₀` and the value law `numer = c^e·denom` (`e ∈ {1, p−1}`)
for `G₀`-avoiding setups; `exists_finite_subfield_containing` closes
`G₀ ∪ {P,Q-coordinates}` into a finite subfield `G`; Stage A produces
a full admissible setup whose data field contains `G`; the Weil value
is then `z = c^e` as a unit, nontrivial since `c` has exact order `p`
(`hc1`, `hcp`) and `p ∤ e`.  `hp0` and `hcard` (derived in the bridge
glue from `hqp`) provide the separability and `p²`-enumeration inputs
of the pullback stages of Stage B.  See Howe, *The Weil pairing and
...
theorem weilValueProp_translationChar_witness (q : ℕ) [Fact q.Prime]
  ...
  classical
  -- affine representative of `κ₀ = i₀.val ≠ O`
  rcases hcκ : i₀.val with _ | ⟨xP, yP, hP₀⟩
  · exact absurd (by rw [hcκ, WeierstrassCurve.Affine.Point.zero_def]) hi₀
  -- affine representative of `P = x.val ≠ O`
  rcases hcx : x.val with _ | ⟨xQ, yQ, hQ₀⟩
  · exact absurd (by rw [hcx, WeierstrassCurve.Affine.Point.zero_def]) hx0
  -- retype along the definitional identification of the base change
  have hP : (Wbar.map (algebraMap (ZMod q)
      (AlgebraicClosure (ZMod q)))).toAffine.Nonsingular xP yP := hP₀
  have hQ : (Wbar.map (algebraMap (ZMod q)
      (AlgebraicClosure (ZMod q)))).toAffine.Nonsingular xQ yQ := hQ₀
  -- torsion facts for the representatives
  have htorP : (p : ℤ) • (WeierstrassCurve.Affine.Point.some xP yP hP :
      (Wbar.map (algebraMap (ZMod q)
        (AlgebraicClosure (ZMod q)))).toAffine.Point) = 0 := by
    have h := (Submodule.mem_torsionBy_iff _ _).mp i₀.property
    rw [hcκ] at h
    exact h
  have htorQ : (p : ℤ) • (WeierstrassCurve.Affine.Point.some xQ yQ hQ :
      (Wbar.map (algebraMap (ZMod q)
        (AlgebraicClosure (ZMod q)))).toAffine.Point) = 0 := by
    have h := (Submodule.mem_torsionBy_iff _ _).mp x.property
    rw [hcx] at h
  ...
  obtain ⟨G, hGfin, hGmem⟩ := exists_finite_subfield_containing q
    (hG₀fin.toFinset ∪ {xP, yP, xQ, yQ})
  have hG₀G : G₀ ≤ G := fun t ht =>
    hGmem t (Finset.mem_union_left _ (hG₀fin.mem_toFinset.mpr ht))
  have hxPG : xP ∈ G := hGmem xP (Finset.mem_union_right _ (by simp))
  have hyPG : yP ∈ G := hGmem yP (Finset.mem_union_right _ (by simp))
  have hxQG : xQ ∈ G := hGmem xQ (Finset.mem_union_right _ (by simp))
  have hyQG : yQ ∈ G := hGmem yQ (Finset.mem_union_right _ (by simp))
  -- Stage A: an admissible setup over `G`
  obtain ⟨F, F', hFfin, hF'fin, hFF', hGF, hK2F, hxPF, hyPF, hxQF, hyQF,
    xS, yS, hS, hxSF', hySF', hxSF,
    xR, yR, hR, hxRF',
    xPS, yPS, hPS, hPSc, hxPSF', hyPSF', hxPSF,
    xQR, yQR, hQR, hQRc, hxQRnS, hxQRnPS, hxQRF',
    aP, aQ, haP, haQ, hA, hB⟩ :=
    exists_weilValueSetup_avoiding q Wbar p hqp G hGfin xP yP hP xQ yQ hQ
      htorP htorQ hxPG hyPG hxQG hyQG
  -- the value law on this setup
  obtain ⟨e, hecase, heval⟩ := hval F F' hFfin hF'fin hFF'
    (hG₀G.trans hGF) hK2F hxPF hyPF hxQF hyQF
    xS yS hS hxSF' hySF' hxSF xR yR hR hxRF'
    xPS yPS hPS hPSc hxPSF' hyPSF' hxPSF
    xQR yQR hQR hQRc hxQRnS hxQRnPS hxQRF'
    aP aQ haP haQ hA hB
  -- assemble the nontrivial admissible Weil value `z = c^e`
  refine ⟨(Units.mk0 c hc0) ^ e, ⟨?_, ?_⟩, ?_⟩
  · rintro (h0 | h0)
    · exact absurd h0 hi₀
    · exact absurd h0 hx0
  · intro xP' yP' hP' xQ' yQ' hQ' hv' hw'
    have hPP : xP' = xP ∧ yP' = yP := by
      injection hv'.symm.trans hcκ with e1 e2
      exact ⟨e1, e2⟩
    ...
    exact ⟨F, F', hFfin, hF'fin, hFF', hK2F,
      ...
      xQR, yQR, hQR, hQRc, hxQRnS, hxQRnPS, hxQRF',
      aP, aQ, haP, haQ, hA,
      by rw [Units.val_pow_eq_pow_val, Units.val_mk0, heval]⟩
  · intro h1
    have hc : c ^ e = 1 := by
      have h2 := congrArg Units.val h1
      rwa [Units.val_pow_eq_pow_val, Units.val_mk0, Units.val_one] at h2
    exact hce e hecase hc
...
theorem exists_weilPairing_mu (q : ℕ) [Fact q.Prime]
  ...
  have hCunits : ∀ u : Wb.toAffine.CoordinateRing, IsUnit u →
      ∃ c : (AlgebraicClosure (ZMod q)), c ≠ 0 ∧ u = AdjoinRoot.of Wb.toAffine.polynomial
        (Polynomial.C c) := coordRing_isUnit_constant q Wb
  ...
  have hevvert : ∀ (c x y : (AlgebraicClosure (ZMod q))) (hE : Wb.toAffine.Equation x y),
      AdjoinRoot.evalEval (p := Wb.toAffine.polynomial) hE
        (WeierstrassCurve.Affine.CoordinateRing.XClass Wb.toAffine c) =
      x - c := coordRing_evalEval_XClass q Wb
  ...
  have hsetup3 : ∀ (G : Subfield (AlgebraicClosure (ZMod q))),
    ...
    have hPSne : WeierstrassCurve.Affine.Point.some xP yP hP +
      ...
      rw [WeierstrassCurve.Affine.Point.neg_some hP] at h1
      injection h1 with e1 e2
      exact hxSF (by rw [e1]; exact hGF hxPG)
    rcases hPSc : (WeierstrassCurve.Affine.Point.some xP yP hP +
        WeierstrassCurve.Affine.Point.some xS yS hSns) with _ | ⟨xPS, yPS, hPS⟩
    · exact absurd (by rw [hPSc, WeierstrassCurve.Affine.Point.zero_def])
        hPSne
    ...
    have hFF' : F ≤ F' := by
      ...
      exact hF'mem a (by
        ...
        exact Or.inl (Or.inl (Or.inl (Or.inl
          (hFfin.mem_toFinset.mpr ha)))))
    have hxSF' : xS ∈ F' := hF'mem xS (by
      simp only [Finset.mem_union]
    ...
    have hQRne : WeierstrassCurve.Affine.Point.some xQ yQ hQ +
      ...
      rw [WeierstrassCurve.Affine.Point.neg_some hQ] at h1
      injection h1 with e1 e2
      exact hxR (by rw [e1]; exact hFF' (hGF hxQG))
    rcases hQRc : (WeierstrassCurve.Affine.Point.some xQ yQ hQ +
        WeierstrassCurve.Affine.Point.some xR yR hRns) with _ | ⟨xQR, yQR, hQR⟩
    · exact absurd (by rw [hQRc, WeierstrassCurve.Affine.Point.zero_def])
        hQRne
    ...
    have hxQRnotG : xQR ∉ G := by
      ...
      · refine hxR ?_
        ...
        exact hF'mem _ (by
          simp only [Finset.mem_union]
          exact Or.inl (Or.inr (Finset.mem_image.mpr
            ⟨xQR, hGfin.mem_toFinset.mpr hin, rfl⟩)))
      · refine hxR ?_
        rw [hxRof xQR (Wb.toAffine.negY xQR (yfib xQR))
          ((WeierstrassCurve.Affine.nonsingular_neg xQR (yfib xQR)).mpr
            ((WeierstrassCurve.Affine.equation_iff_nonsingular).mp
        ...
        exact hF'mem _ (by
          ...
          exact Or.inr (Finset.mem_image.mpr
            ⟨xQR, hGfin.mem_toFinset.mpr hin, rfl⟩))
    -- x(Q⊕R) also avoids the first divisor's abscissas
    have hxQRne : xQR ≠ xS ∧ xQR ≠ xPS := by
      constructor
      · intro hx
        rcases hfib2 xS yQR (hx ▸ hQR.left) with hy | hy
        · refine hxR ?_
          ...
          exact hF'mem _ (by
            simp only [Finset.mem_union]
            exact Or.inl (Or.inl (Or.inr (by simp))))
        · refine hxR ?_
          rw [hxRof xS (Wb.toAffine.negY xS (yfib xS))
            ((WeierstrassCurve.Affine.nonsingular_neg xS (yfib xS)).mpr
              ((WeierstrassCurve.Affine.equation_iff_nonsingular).mp
  ...
  have hrecgen := recgen q p Wb yfib hyfib hCunits hline hoffdiv hevvert hnegYF hgenfac hww hbaldiv hevconst hevid hfib2
      ...
      have hcross :
        ...
        obtain ⟨G, hGfin, hGmem⟩ := hsubfin
          (hF₁'fin.toFinset ∪ hF₂'fin.toFinset ∪
            {xQR₁, yQR₁, xQR₂, yQR₂} ∪
            hF₁'fin.toFinset.image yfib ∪ hF₂'fin.toFinset.image yfib)
        have hF₁'G : F₁' ≤ G := fun a ha => hGmem a (by
          simp only [Finset.mem_union]
          exact Or.inl (Or.inl (Or.inl (Or.inl
            (hF₁'fin.mem_toFinset.mpr ha)))))
        have hF₂'G : F₂' ≤ G := fun a ha => hGmem a (by
          simp only [Finset.mem_union]
          exact Or.inl (Or.inl (Or.inl (Or.inr
            (hF₂'fin.mem_toFinset.mpr ha)))))
        have hQR₁G : xQR₁ ∈ G ∧ yQR₁ ∈ G :=
          ⟨hGmem _ (by
        ...
        have hQR₂G : xQR₂ ∈ G ∧ yQR₂ ∈ G :=
          ⟨hGmem _ (by
            ...
            simp only [Finset.mem_union, Finset.mem_insert,
              Finset.mem_singleton]
            tauto)⟩
        have hyfibG₁ : ∀ c ∈ F₁', yfib c ∈ G := fun c hc => hGmem _ (by
          simp only [Finset.mem_union]
          exact Or.inl (Or.inr (Finset.mem_image.mpr
        ...
        have hMcA13 : (WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xQR₁ xR₃ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) (Wb.toAffine.addY xQR₁ xR₃ yQR₁ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) hMA13 : Wb.toAffine.Point) =
            WeierstrassCurve.Affine.Point.some xQR₁ yQR₁ hQR₁ + WeierstrassCurve.Affine.Point.some xR₃ yR₃ hR₃ :=
          (WeierstrassCurve.Affine.Point.add_some
            (fun hxy => hxneA13 hxy.1)).symm
        have hxMA13F : (Wb.toAffine.addX xQR₁ xR₃ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) ∉ F₁' := by
          intro hin
          have hyMG : (Wb.toAffine.addY xQR₁ xR₃ yQR₁ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) ∈ G := by
            rcases hfib2 _ _ hMA13.left with hy | hy
            · rw [hy]; exact hyfibG₁ _ hin
            · rw [hy]
              exact hnegYF G _ _ (hF₁'G hin) (hyfibG₁ _ hin)
          have hAneg : Wb.toAffine.Nonsingular xQR₁ (Wb.toAffine.negY xQR₁ yQR₁) :=
            (WeierstrassCurve.Affine.nonsingular_neg xQR₁ yQR₁).mpr hQR₁
          have hBeq : (WeierstrassCurve.Affine.Point.some xR₃ yR₃ hR₃ : Wb.toAffine.Point) =
              WeierstrassCurve.Affine.Point.some xQR₁ (Wb.toAffine.negY xQR₁ yQR₁) hAneg +
              WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xQR₁ xR₃ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) (Wb.toAffine.addY xQR₁ xR₃ yQR₁ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) hMA13 := by
            rw [show (WeierstrassCurve.Affine.Point.some xQR₁ (Wb.toAffine.negY xQR₁ yQR₁) hAneg :
                Wb.toAffine.Point) = -(WeierstrassCurve.Affine.Point.some xQR₁ yQR₁ hQR₁) from
              (WeierstrassCurve.Affine.Point.neg_some hQR₁).symm,
              hMcA13, neg_add_cancel_left]
          by_cases hxy : xQR₁ = (Wb.toAffine.addX xQR₁ xR₃ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) ∧
              Wb.toAffine.negY xQR₁ yQR₁ = Wb.toAffine.negY (Wb.toAffine.addX xQR₁ xR₃ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃)) (Wb.toAffine.addY xQR₁ xR₃ yQR₁ (Wb.toAffine.slope xQR₁ xR₃ yQR₁ yR₃))
          · rw [WeierstrassCurve.Affine.Point.add_of_Y_eq hxy.1 hxy.2] at hBeq
            simp [WeierstrassCurve.Affine.Point.zero_def] at hBeq
          · rw [WeierstrassCurve.Affine.Point.add_some hxy] at hBeq
            injection hBeq with e1 e2
            apply hxR₃
            rw [e1]
            exact haddXF G _ _ _ hQR₁G.1 (hF₁'G hin)
              (hslopeF G _ _ _ _ hQR₁G.1 (hF₁'G hin)
                (hnegYF G _ _ hQR₁G.1 hQR₁G.2) hyMG)
        have hxneB13 : xPS₁ ≠ xS₃ := fun h => hxS₃ (h ▸ (hF₁'G hxPS₁F'))
        have hMB13 := WeierstrassCurve.Affine.nonsingular_add hPS₁ hS₃
          (fun hxy => hxneB13 hxy.1)
        have hMcB13 : (WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xPS₁ xS₃ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) (Wb.toAffine.addY xPS₁ xS₃ yPS₁ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) hMB13 : Wb.toAffine.Point) =
            WeierstrassCurve.Affine.Point.some xPS₁ yPS₁ hPS₁ + WeierstrassCurve.Affine.Point.some xS₃ yS₃ hS₃ :=
          (WeierstrassCurve.Affine.Point.add_some
            (fun hxy => hxneB13 hxy.1)).symm
        have hxMB13F : (Wb.toAffine.addX xPS₁ xS₃ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) ∉ F₁' := by
          intro hin
          have hyMG : (Wb.toAffine.addY xPS₁ xS₃ yPS₁ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) ∈ G := by
            rcases hfib2 _ _ hMB13.left with hy | hy
            · rw [hy]; exact hyfibG₁ _ hin
            · rw [hy]
              exact hnegYF G _ _ (hF₁'G hin) (hyfibG₁ _ hin)
          have hAneg : Wb.toAffine.Nonsingular xPS₁ (Wb.toAffine.negY xPS₁ yPS₁) :=
            (WeierstrassCurve.Affine.nonsingular_neg xPS₁ yPS₁).mpr hPS₁
          have hBeq : (WeierstrassCurve.Affine.Point.some xS₃ yS₃ hS₃ : Wb.toAffine.Point) =
              WeierstrassCurve.Affine.Point.some xPS₁ (Wb.toAffine.negY xPS₁ yPS₁) hAneg +
              WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xPS₁ xS₃ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) (Wb.toAffine.addY xPS₁ xS₃ yPS₁ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) hMB13 := by
            rw [show (WeierstrassCurve.Affine.Point.some xPS₁ (Wb.toAffine.negY xPS₁ yPS₁) hAneg :
                Wb.toAffine.Point) = -(WeierstrassCurve.Affine.Point.some xPS₁ yPS₁ hPS₁) from
              (WeierstrassCurve.Affine.Point.neg_some hPS₁).symm,
              hMcB13, neg_add_cancel_left]
          by_cases hxy : xPS₁ = (Wb.toAffine.addX xPS₁ xS₃ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) ∧
              Wb.toAffine.negY xPS₁ yPS₁ = Wb.toAffine.negY (Wb.toAffine.addX xPS₁ xS₃ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃)) (Wb.toAffine.addY xPS₁ xS₃ yPS₁ (Wb.toAffine.slope xPS₁ xS₃ yPS₁ yS₃))
          · rw [WeierstrassCurve.Affine.Point.add_of_Y_eq hxy.1 hxy.2] at hBeq
            simp [WeierstrassCurve.Affine.Point.zero_def] at hBeq
          · rw [WeierstrassCurve.Affine.Point.add_some hxy] at hBeq
            injection hBeq with e1 e2
        ...
        have hMcA23 : (WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xQR₂ xR₃ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) (Wb.toAffine.addY xQR₂ xR₃ yQR₂ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) hMA23 : Wb.toAffine.Point) =
            WeierstrassCurve.Affine.Point.some xQR₂ yQR₂ hQR₂ + WeierstrassCurve.Affine.Point.some xR₃ yR₃ hR₃ :=
          (WeierstrassCurve.Affine.Point.add_some
            (fun hxy => hxneA23 hxy.1)).symm
        have hxMA23F : (Wb.toAffine.addX xQR₂ xR₃ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) ∉ F₂' := by
          intro hin
          have hyMG : (Wb.toAffine.addY xQR₂ xR₃ yQR₂ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) ∈ G := by
            rcases hfib2 _ _ hMA23.left with hy | hy
            · rw [hy]; exact hyfibG₂ _ hin
            · rw [hy]
              exact hnegYF G _ _ (hF₂'G hin) (hyfibG₂ _ hin)
          have hAneg : Wb.toAffine.Nonsingular xQR₂ (Wb.toAffine.negY xQR₂ yQR₂) :=
          ...
          have hBeq : (WeierstrassCurve.Affine.Point.some xR₃ yR₃ hR₃ : Wb.toAffine.Point) =
              WeierstrassCurve.Affine.Point.some xQR₂ (Wb.toAffine.negY xQR₂ yQR₂) hAneg +
              WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xQR₂ xR₃ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) (Wb.toAffine.addY xQR₂ xR₃ yQR₂ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) hMA23 := by
            rw [show (WeierstrassCurve.Affine.Point.some xQR₂ (Wb.toAffine.negY xQR₂ yQR₂) hAneg :
                Wb.toAffine.Point) = -(WeierstrassCurve.Affine.Point.some xQR₂ yQR₂ hQR₂) from
              (WeierstrassCurve.Affine.Point.neg_some hQR₂).symm,
              hMcA23, neg_add_cancel_left]
          by_cases hxy : xQR₂ = (Wb.toAffine.addX xQR₂ xR₃ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) ∧
              Wb.toAffine.negY xQR₂ yQR₂ = Wb.toAffine.negY (Wb.toAffine.addX xQR₂ xR₃ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃)) (Wb.toAffine.addY xQR₂ xR₃ yQR₂ (Wb.toAffine.slope xQR₂ xR₃ yQR₂ yR₃))
          · rw [WeierstrassCurve.Affine.Point.add_of_Y_eq hxy.1 hxy.2] at hBeq
            simp [WeierstrassCurve.Affine.Point.zero_def] at hBeq
          · rw [WeierstrassCurve.Affine.Point.add_some hxy] at hBeq
            ...
            exact haddXF G _ _ _ hQR₂G.1 (hF₂'G hin)
              (hslopeF G _ _ _ _ hQR₂G.1 (hF₂'G hin)
                (hnegYF G _ _ hQR₂G.1 hQR₂G.2) hyMG)
        have hxneB23 : xPS₂ ≠ xS₃ := fun h => hxS₃ (h ▸ (hF₂'G hxPS₂F'))
        have hMB23 := WeierstrassCurve.Affine.nonsingular_add hPS₂ hS₃
          (fun hxy => hxneB23 hxy.1)
        have hMcB23 : (WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xPS₂ xS₃ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) (Wb.toAffine.addY xPS₂ xS₃ yPS₂ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) hMB23 : Wb.toAffine.Point) =
            WeierstrassCurve.Affine.Point.some xPS₂ yPS₂ hPS₂ + WeierstrassCurve.Affine.Point.some xS₃ yS₃ hS₃ :=
          (WeierstrassCurve.Affine.Point.add_some
            (fun hxy => hxneB23 hxy.1)).symm
        have hxMB23F : (Wb.toAffine.addX xPS₂ xS₃ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) ∉ F₂' := by
          intro hin
          have hyMG : (Wb.toAffine.addY xPS₂ xS₃ yPS₂ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) ∈ G := by
            rcases hfib2 _ _ hMB23.left with hy | hy
            · rw [hy]; exact hyfibG₂ _ hin
            · rw [hy]
              exact hnegYF G _ _ (hF₂'G hin) (hyfibG₂ _ hin)
          have hAneg : Wb.toAffine.Nonsingular xPS₂ (Wb.toAffine.negY xPS₂ yPS₂) :=
          ...
          have hBeq : (WeierstrassCurve.Affine.Point.some xS₃ yS₃ hS₃ : Wb.toAffine.Point) =
              WeierstrassCurve.Affine.Point.some xPS₂ (Wb.toAffine.negY xPS₂ yPS₂) hAneg +
              WeierstrassCurve.Affine.Point.some (Wb.toAffine.addX xPS₂ xS₃ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) (Wb.toAffine.addY xPS₂ xS₃ yPS₂ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) hMB23 := by
            rw [show (WeierstrassCurve.Affine.Point.some xPS₂ (Wb.toAffine.negY xPS₂ yPS₂) hAneg :
                Wb.toAffine.Point) = -(WeierstrassCurve.Affine.Point.some xPS₂ yPS₂ hPS₂) from
              (WeierstrassCurve.Affine.Point.neg_some hPS₂).symm,
              hMcB23, neg_add_cancel_left]
          by_cases hxy : xPS₂ = (Wb.toAffine.addX xPS₂ xS₃ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) ∧
              Wb.toAffine.negY xPS₂ yPS₂ = Wb.toAffine.negY (Wb.toAffine.addX xPS₂ xS₃ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃)) (Wb.toAffine.addY xPS₂ xS₃ yPS₂ (Wb.toAffine.slope xPS₂ xS₃ yPS₂ yS₃))
          · rw [WeierstrassCurve.Affine.Point.add_of_Y_eq hxy.1 hxy.2] at hBeq
            simp [WeierstrassCurve.Affine.Point.zero_def] at hBeq

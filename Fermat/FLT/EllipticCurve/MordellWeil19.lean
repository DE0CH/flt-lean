...
## NO LEAF: THE LEVEL-`19` ROW IS CLOSED (2026-07-31)

`integral_leaf` — the pure integer statement

    n² = p³ + 4p²e² + 16pe⁴ + 16e⁶,  gcd(p, e) = 1,  e > 0   ⟹   (p, e) = (0, 1)

— is PROVEN, and with it `U_eq_zero`, `x0Nineteen_x_eq_zero`, `cover_identity`,
`cover_eq` and `rational_point_x0Nineteen`.  `#print axioms
MazurX0Nineteen.rational_point_x0Nineteen` returns
`[propext, Classical.choice, Quot.sound]`.

The proof is the `2`-DESCENT, transcribed from `MazurLevel11` with new constants:
`exists_halving_witness` (over `descent_zs`, `descent_unit_square`,
`descent_square_class`, `epsilon_class_impossible`) → `halving_relation` →
`height_drop_or_small` (`forms_common_dvd`, `forms_archimedean`) → `smallPoints`
(a bitmask quadratic-residue sieve) → `halving_descends`/`trivial_ascends` →
`integral_leaf_aux`, by strong induction on `|p| + e²`.  The cubic ring `ℤ[θ]`
itself, `𝓞_K = ℤ[θ]`, `h(K) = 1` and the units mod squares are in
`Fermat/FLT/EllipticCurve/CubicRing19.lean`.

**ONE STEP DID NOT TRANSCRIBE, and it is the only thing worth remembering here.**
Level `11` gets `h(K) = 1` from `RingOfIntegers.isPrincipalIdealRing_of_abs_discr_lt`,
i.e. from "the Minkowski bound is `< 2`": `|disc| = 44` against
`(2·(π/4)·3³/3!)² ≈ 49.96`.  **At level `19`, `|disc| = 76 > 49.96` and that lemma
does not apply at all** — and no change of model helps, because the cubic field is
the `2`-division field of the curve and is the same for every member of the
isogeny class.  The bound that IS true is on ideal NORMS,
`(4/π)·(3!/3³)·√76 = 8√76/(9π) ≈ 2.4666`, so the class group is generated by the
primes of norm `≤ 2`.  There is one, and it is principal: `θ³ = −2(θ+1)²` with
`θ + 1` a unit, so `(2) = (θ)³` and the prime is `(θ)`, of norm `2`.  In Lean that
is `RingOfIntegers.isPrincipalIdealRing_of_isPrincipal_of_pow_le_of_mem_primesOver_of_mem_Icc`
plus `Ideal.isPrime_of_irreducible_absNorm`, and it cost about sixty lines rather
than the separate prerequisite an earlier note here budgeted for.

## THE ROUTE THAT WAS ABANDONED, AND WHY THE ANALYSIS IS STILL WORTH READING

Between 2026-07-30 and 2026-07-31 this file routed `integral_leaf` through an
elementary `3`-isogeny descent into the Hesse cubic, over a leaf `hesse_leaf`
(`X³ + Y³ + Z³ = 2XYZ`, `gcd(X, Y) = 1` ⟹ `X + Y + Z = 0`).  That block is
DELETED — see the note at the top of the code below — because with the `2`-descent
closed it became free-floating, `sorry` and all.  Two findings from it survive and
should not be re-derived:

* **`X³+Y³+Z³=2XYZ` IS `19a1`.**  `ellfromeqn(x³+y³+1−2xy)` has minimal model
  `[0,1,1,−9,−15]`, `Δ = −6859 = −19³`, conductor `19`, `ellrank = [0,0]`,
  `elltors = ℤ/3`, and its only primitive integer triples are the six
  `(±1,∓1,0)`, `(±1,0,∓1)`, `(0,±1,∓1)` — all with `X+Y+Z = 0`.  So `hesse_leaf`
  was TRUE, not merely unproven.
* **THE ELEMENTARY `3`-DESCENT CANNOT BE ITERATED** (checked 2026-07-30, not
  guessed).  Writing `19a1` with its rational `3`-torsion at the origin,
  `y² + 8xy + 19y = x³`, the three inflectional tangents satisfy the symbolic
  identities

      (3u−3v+2f)(2u−3v+3f)(3u−2v+3f) = 19(u−v+f)³ + (v³−u³−f³−2uvf),
      (3u−3v+2f)+(2u−3v+3f)+(3u−2v+3f) = 8(u−v+f),

  so the three factors have product `19·M³` with `M = u−v+f`, and they are
  pairwise coprime for a primitive triple: the system inverts to
  `8u = 3A−5B+3C`, `8v = −3A−3B+5C`, `8f = −5A+3B+3C`, so a common prime divides
  `u, v, f` at once away from `2` and `19`, and at those two the same inversion
  or a parity count finishes.  Extracting cubes gives `a³ + b³ + 19c³ = 8abc`,
  **which is `19a2`** (`[0,1,1,−769,−8470]`, conductor `19`, rank `0`) — and
  `19a2` has TRIVIAL torsion, hence no rational `3`-torsion point and no "`y` is
  a cube" descent.  The isogeny class is `19a2 — 19a1 — 19a3` with matrix
  `[[1,3,3],[3,1,9],[3,9,1]]`, so the chain `19a3 → 19a1 → 19a2` terminates.
  Closing the loop would need `19a1(ℚ)/ψ(19a3(ℚ))`, whose descent group is
  `H¹(G, ℤ/3) = Hom(G, ℤ/3)` — cyclic cubic fields, not `ℚ*/(ℚ*)³`.  **Do not
  spend a cycle looking for an elementary `3`-descent that closes; there is
  none.**

The `2`-descent is also CHEAPER ON `19a3` than on `19a1`, which is why this file
descends on `19a3` and transports by the isogeny: both run in `ℚ(θ)`, but `19a3`'s
monic cubic `U³ + 4U² + 16U + 16` generates the FULL ring of integers (index `1`)
while `19a1`'s `U³ + 4U² − 144U − 944` generates an order of index `152 = 8·19`.
...
public import Fermat.FLT.EllipticCurve.CubicRing19

@[expose] public section

namespace MazurX0Nineteen

/-! ### THE HESSE ROUTE WAS DELETED ON 2026-07-31, AFTER THE `2`-DESCENT CLOSED

Until 2026-07-31 `integral_leaf` was proven by an elementary `3`-isogeny descent
into the Hesse cubic `X³ + Y³ + Z³ = 2XYZ`, over the single open leaf
`hesse_leaf`.  That route bought a `sorry`-free chain everywhere EXCEPT at
`hesse_leaf`, which was never closed, and it existed only because a straight
transcription of `MazurLevel11`'s `2`-descent was believed to stall on
`h(K) = 1` (the note inside `integral_leaf` records exactly why it was believed
to stall, and why that was wrong).

With `descent_unit_square` closed over `Fermat/FLT/EllipticCurve/CubicRing19.lean`,
`integral_leaf_aux` proves `integral_leaf` outright, so the whole Hesse block
became **dead: open, compiling, and consumed by nothing reaching the root** —
CLAUDE.md's seventh invisibility class, which is a phantom-dispatch magnet and,
being free-floating, is not allowed here.  It was therefore deleted whole, with
its `sorry`, in the same commit that rewired `integral_leaf`.

DELETED (10 declarations, ~315 lines, verified dead by a fixpoint on the in-file
usage graph and by a tree-wide grep): `cop_add_mul`,
`cube_of_coprime_mul_eq_cube`, `two_mul_of_sq_eq_two_mul`, `cube_left_inj`,
`hesse_leaf`, `four_dvd_of_even`, `exists_reduced_even`, `descent_odd`,
`descent_even`, `hesse_kill` — together with the release-25 note about an
orphaned docstring in that region, which described a theorem that no longer
exists anywhere.

**RECOVERY**: `git show <this commit>^:Fermat/FLT/EllipticCurve/MordellWeil19.lean`.
`hesse_leaf` is a true and interesting statement (it is what "`19a2` has trivial
Mordell–Weil" reads as in Hesse coordinates) and its falsity audit was careful;
it is simply not on any path to the root any more.  Restore it only together
with a consumer. -/
...
namespace Cubic19.ZS

/-- The `ℤ`-basis coordinates of an element of `ℤ[θ]`.  The bridge to the
coordinate layer of this file is DEFINITIONAL: `ofZS_mul` and `ofZS_beta` are both
`rfl`, because `Cubic19.ZS`'s multiplication was written to be `zsMul` on the
nose. -/
def ofZS (x : Cubic19.ZS) : ℤ × ℤ × ℤ := (x.a, x.b, x.c)

theorem ofZS_mul (x y : Cubic19.ZS) : ofZS (x * y) = zsMul (ofZS x) (ofZS y) := rfl

theorem ofZS_beta (p e : ℤ) : ofZS (Cubic19.ZS.beta p e) = descentImage p e := rfl

end Cubic19.ZS

/-- **THE VALUATION BOOKKEEPING** (PROVEN 2026-07-31 over
`Fermat/FLT/EllipticCurve/CubicRing19.lean`; a sorry leaf for a few hours the same
day, cut out of `exists_halving_witness`): `β = p − 2θe²` is one of `16` explicit
units times a square in `ℤ[θ]`.
...
**CLOSED 2026-07-31** by `Fermat/FLT/EllipticCurve/CubicRing19.lean`, which builds
`ℤ[θ]` as a genuine `CommRing` whose multiplication is `zsMul` DEFINITIONALLY (so
`ofZS_mul` below is `rfl`) and proves the two field facts the argument above
needs, `Cubic19.ZS.isPrincipalIdealRing_zs` and `Cubic19.ZS.unit_sq_class`,
together with the valuation bookkeeping `Cubic19.ZS.descent_zs`.  The route was
exactly the one prescribed here, with ONE genuine deviation: `h(K) = 1` could NOT
be obtained the way level `11` obtains it.  `RingOfIntegers.isPrincipalIdealRing_of_abs_discr_lt`
needs `|disc| < (2·(π/4)^{r₂}·nⁿ/n!)² ≈ 49.96`, which holds for level `11`'s `44`
and FAILS for `76`; the Minkowski bound here is `8√76/(9π) ≈ 2.4666`, so the
general criterion `isPrincipalIdealRing_of_isPrincipal_of_pow_le_of_mem_primesOver_of_mem_Icc`
is used instead and the unique prime above `2` — which is `(θ)`, because
`θ³ = −2ε²` with `ε = θ + 1` a unit — is shown principal by hand. -/
theorem descent_unit_square {p e n : ℤ} (he : 0 < e) (hcop : IsCoprime p e)
    ...
    ∃ u δ : ℤ × ℤ × ℤ, descentImage p e = zsMul u (zsMul δ δ) ∧
      u ∈ [((1 : ℤ), (0 : ℤ), (0 : ℤ)), (3, 1, 0), (0, 1, 0), (0, 3, 1),
        ...
        (-1, -1, 0), (-3, -4, -1), (0, -1, -1), (2, 1, -2)] := by
  rw [zsNorm_descentImage] at hn
  obtain ⟨v, δ, r, t, hv, hr, ht, hbeta⟩ := Cubic19.ZS.descent_zs he hcop hn
  refine ⟨Cubic19.ZS.ofZS (v * Cubic19.ZS.sEl ^ r * Cubic19.ZS.gEl ^ t),
    Cubic19.ZS.ofZS δ, ?_, ?_⟩
  · rw [← Cubic19.ZS.ofZS_beta, hbeta, sq]; simp only [Cubic19.ZS.ofZS_mul]
...
* and it is neither vacuous nor over-strong — `(0, 1)` IS a solution, and dropping
  ...
  **WARNING (2026-07-30): THIS IS THE ONE PLACE WHERE LEVEL 11 DOES *NOT*
  ...
  principal by construction — whence `h = 1`, and indeed `(2) = (θ)³`.

  **DONE 2026-07-31, and it was ~60 lines rather than a prerequisite.**  The
  ideal-norm side of `Mathlib/NumberTheory/ClassNumber/*` really is what is
  needed, and the lemma that drives it is
  `RingOfIntegers.isPrincipalIdealRing_of_isPrincipal_of_pow_le_of_mem_primesOver_of_mem_Icc`
  (present at this pin): compute `⌊M K⌋₊ = 2`, so only `p = 2` is inspected; a
  prime `P` above `2` contains `θ³ = −2ε²`, hence `θ`; and `span {θ}` is maximal
  because `Ideal.absNorm (span {θ}) = |N θ| = 2` is prime
  (`Ideal.isPrime_of_irreducible_absNorm` plus `IsPrime.isMaximal`).  See
  `Fermat/FLT/EllipticCurve/CubicRing19.lean`.  Everything else in the level-11
  chain transcribed with new constants exactly as predicted.

**PROOF CHANGED 2026-07-31: this is now the `2`-DESCENT, not the Hesse route.**
It used to be `four_dvd_of_even`/`exists_reduced_even`/`descent_odd`/`descent_even`
then `hesse_kill`, which rests on the OPEN leaf `hesse_leaf`.  With
`descent_unit_square` closed, `integral_leaf_aux` — the strong induction on
`|p| + e²` over `exists_halving_witness`, `halving_descends` and `trivial_ascends`
— proves exactly this statement with nothing open under it, so the whole level-`19`
row is now `sorry`-free.  `hesse_leaf` is thereby DEAD: still open, but consumed by
nothing that reaches the root.  See the note at `hesse_leaf` itself. -/

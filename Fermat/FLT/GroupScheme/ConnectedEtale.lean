...
set_option synthInstance.maxHeartbeats 1000000 in
set_option maxHeartbeats 2000000 in
/-- **THE OORT–TATE NORMAL FORM, IN VALUE FORM — the one-dimensionality
input of the node** (SORRY LEAF, cut 2026-07-25 out of
`displacement_span_le_span_zeta_sub_one`, which is PROVEN over it
directly below): for a geometric point `φ` killed by `p` whose cyclic
group `⟨φ⟩` is INERTIA-STABLE, the
ideal of `𝒪̄` generated by the values of the DISPLACEMENT
`g ↦ φ(1 ⊗ g) − ε(g)` is PRINCIPAL, `= (α)`, with a generator `α`
satisfying the Oort–Tate relation `α^p = a·α` for some `a` in the BASE
ring `𝒪ᵥ = ℤ_p`.

**What this says, in one line: `(p − 1)·v(α) ∈ ℤ`.** Indeed `α ≠ 0`
gives `α^{p−1} = a ∈ 𝒪ᵥ`, whose value group is `ℤ`; and conversely, if
`(p − 1)·v(t) = n ∈ ℤ` for a generator `t`, then any root `α` of
`x^{p−1} = p^n` in the algebraically closed `ℚᵥᵃˡᵍ` has `v(α) = v(t)`,
hence generates the same ideal of the VALUATION ring `𝒪̄`, and satisfies
`α^p = p^n·α`. So this leaf is EXACTLY the assertion that the character
through which inertia acts on the line `⟨φ⟩` is TAME and `𝔽_p`-valued
— the ONE-DIMENSIONALITY that `hstab` encodes. Note that
it asks only for a VALUE and never for an `𝒪ᵥ`-rational coordinate of
`G`: the Oort–Tate coordinate is defined only up to an unramified
twist, and demanding it is what made the deleted `exists_muType_closure`
FALSE (see the CORRECTION NOTE at `exists_muType_coordinate`). The value
ideal is twist-blind, so this form is faithful.
...
the conclusion of this leaf is FALSE and only `hstab` excludes it. Any
proof of this leaf that does not use `hstab`, and any restatement that
drops it, is therefore wrong. (`Odd p` and `hcomul₀` are carried along
because the consumer has them, not because the mathematics below needs
them.)
...
1. *Oort–Tate* (Oort–Tate, *Group schemes of prime order*, Ann. Sci.
   ...
   the powers `α^i` (`i ≥ 1`), so the value ideal is `(α)` — which is
   the statement below verbatim. CAVEAT for whoever takes this route:
   the schematic closure is defined over `𝒪ᵥ` only when `⟨φ⟩` is stable
   under the FULL local Galois group, whereas `hstab` supplies only
   INERTIA-stability; step (a) therefore needs an argument, or the
   second route.
...
`(ζ_p − 1)`, and `α := ` any root of `x^{p−1} = p` witnesses the
conclusion with `a = p`; the same holds for every unramified twist
`μ_p ⊗ ψ`.
...
`α = 0`, `a = 0` witnesses it, and for the ÉTALE `ℤ/p` the ideal is
`⊤` and `α = 1`, `a = 1` witnesses it (`(p − 1)·v(α) = 0 ∈ ℤ`). That
is deliberate, and it is where the labour is split: the leaf supplies
ONLY the integrality `(p − 1)·v(α) ∈ ℤ`, and the consumer below
supplies connectedness itself — through
`point_sub_counit_mem_maximalIdeal`, which puts `α` in `𝔪` and hence
turns `a` into a NONUNIT of `𝒪ᵥ = ℤ_p`, i.e. into a multiple of `p`.
Neither half alone gives `v(t) = 1/(p − 1)`. -/
theorem exists_oortTate_value_of_inertia_stable
    (hpodd : Odd p)
    (G : Type) [CommRing G]
...
`exists_oortTate_value_of_inertia_stable`, which carries the
...
Proof, given the leaf. The leaf hands back a generator `α` of the
displacement ideal together with `a ∈ 𝒪ᵥ` and the Oort–Tate relation
`α^p = a·α`.

* If `α = 0` the ideal is `⊥` and there is nothing to prove.
* Otherwise cancel `α` in the domain `𝒪̄`: `α^{p−1} = a`. This is the
  whole content of the leaf — the `(p − 1)`-st power of a generator is
  a RATIONAL integer, i.e. `(p − 1)·v(t) ∈ ℤ`.
* CONNECTEDNESS enters here, and only here:
  `point_sub_counit_mem_maximalIdeal` puts every displacement value in
  `𝔪 𝒪̄`, so `α ∈ 𝔪 𝒪̄`, so `a = α^{p−1} ∈ 𝔪 𝒪̄`. Contracting along the
  INTEGRAL extension `𝒪ᵥ → 𝒪̄` (whose contraction of `𝔪 𝒪̄` is a maximal
  ideal of the local ring `𝒪ᵥ`, hence `𝔪 𝒪ᵥ`) and using the absolute
  unramifiedness `𝔪 𝒪ᵥ = (p)` (`maximalIdeal_eq_span_natCast`) gives
  `p ∣ a` in `𝒪ᵥ`, hence `p ∣ α^{p−1}` in `𝒪̄`. Without connectedness
  `a` could be a unit — that is the étale branch `v(t) = 0`.
* Finally `(1 − ζ_p)^{p−1} ∣ p` (`one_sub_pow_dvd_natCast_of_isPrimitiveRoot`),
  so `(1 − ζ_p)^{p−1} ∣ α^{p−1}`, and in a VALUATION ring that forces
  `1 − ζ_p ∣ α`: divisibility is total, and in the other case
  `α = (1 − ζ_p)·e⁻¹` follows after cancelling `α^{p−1}` in the domain
...
The supersingular test is passed by construction: the sole use of
`hstab` is the appeal to the leaf, whose conclusion is FALSE for
supersingular `E[p]`, so no step here silently generalises past
one-dimensionality. -/
theorem displacement_span_le_span_zeta_sub_one
    ...
    Ideal.span {x : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ | ∃ g : G,
      ...
      Ideal.span {ζ - 1} := by
  classical
  obtain ⟨α, a, hspan, hOT⟩ :=
    exists_oortTate_value_of_inertia_stable hpodd G e₀ he₀ hε₀ hprim₀ hcomul₀
  ...
  · rw [hspan, hα0, Ideal.span_singleton_eq_bot.mpr rfl]
    exact bot_le
  have hαmax : α ∈ maximalIdeal (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) := by
    rw [hspan] at hSmax
    exact hSmax (Ideal.mem_span_singleton_self α)
  have hp2 : 2 ≤ p := hp.out.two_le
  -- the ONE-DIMENSIONALITY input, with `α` cancelled: `α^{p−1} = a ∈ 𝒪ᵥ`
  have hpow : α ^ (p - 1) = algebraMap 𝒪ᵖᵍᵥ (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) a := by
    refine mul_right_cancel₀ hα0 ?_
    rw [← pow_succ, show p - 1 + 1 = p by omega]
    exact hOT
  -- `a` is a NONUNIT of `𝒪ᵥ = ℤ_p`, hence a multiple of `p`
  have hamax : a ∈ maximalIdeal 𝒪ᵖᵍᵥ := by
    have hcomap : (maximalIdeal (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ)).comap
        (algebraMap 𝒪ᵖᵍᵥ (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ)) = maximalIdeal 𝒪ᵖᵍᵥ :=
      IsLocalRing.eq_maximalIdeal
        (Ideal.isMaximal_comap_of_isIntegral_of_isMaximal _)
    rw [← hcomap, Ideal.mem_comap, ← hpow]
    exact Ideal.pow_mem_of_mem _ hαmax _ (by omega)
  have hpa : ((p : ℕ) : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) ∣ α ^ (p - 1) := by
    rw [maximalIdeal_eq_span_natCast, Ideal.mem_span_singleton] at hamax
    obtain ⟨b, hb⟩ := hamax
    refine ⟨algebraMap 𝒪ᵖᵍᵥ (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) b, ?_⟩
    rw [hpow, hb, map_mul, map_natCast]
  ...
  have hdvdpow : ((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) ^ (p - 1) ∣ α ^ (p - 1) :=
    hcyc.trans hpa
  -- in a valuation ring, `π^{p−1} ∣ α^{p−1}` forces `π ∣ α`
  have hdvd : ((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) ∣ α := by
    obtain ⟨e, he⟩ :=
      ValuationRing.cond ((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) α
    rcases he with h | h
    · exact ⟨e, h.symm⟩
    · obtain ⟨s, hs⟩ := hdvdpow
      have hpowe : α ^ (p - 1) = α ^ (p - 1) * (e ^ (p - 1) * s) := by
        conv_lhs => rw [hs, ← h, mul_pow]
        ring
      have hone : (1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) = e ^ (p - 1) * s :=
        mul_left_cancel₀ (pow_ne_zero _ hα0) (by linear_combination hpowe)
...
over the single leaf `exists_oortTate_value_of_inertia_stable` — and
...
see `exists_oortTate_value_of_inertia_stable`'s docstring for the
...
`exists_oortTate_value_of_inertia_stable`): for a geometric point `φ` of exact
...
own open input is `exists_oortTate_value_of_inertia_stable`.
...
`exists_oortTate_value_of_inertia_stable`): under the hypotheses of
...
* `inertia_character_trivial_or_cyclotomic` — RAYNAUD's dichotomy at
  ...
  `exists_oortTate_value_of_inertia_stable`, "the displacement ideal has
  a generator `α` with `α^p = a·α` for some `a ∈ 𝒪ᵥ`", i.e.
  `(p − 1)·v(α) ∈ ℤ`, which is the ONE-DIMENSIONALITY half of

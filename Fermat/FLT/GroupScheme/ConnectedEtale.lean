...
/-- **`(1 − ζ_p)^{p−1}` DIVIDES `p`** (PROVEN 2026-07-25): the mirror
image of `natCast_dvd_one_sub_pow_of_isPrimitiveRoot`, proven by the
same factorisation `p = Φ_p(1) = ∏_{μ primitive} (1 − μ)` read in the
other direction — `ζ` is a power of each primitive root `μ`, so
`1 − ζ ∣ 1 − μ` for every factor and the product of the `p − 1` copies
of `1 − ζ` divides the product of the factors.

Together with `natCast_dvd_one_sub_pow_of_isPrimitiveRoot` this says
that `(1 − ζ_p)^{p−1}` and `p` are ASSOCIATES, i.e.
`v(ζ_p − 1) = 1/(p − 1)` exactly. That exact value is what lets the
integrality statement `(p − 1)·v(t) ∈ ℤ` (the Oort–Tate leaf below) be
converted into the ideal inclusion `(t) ⊆ (ζ_p − 1)`: the leaf gives
`p ∣ t^{p−1}`, this lemma gives `(1 − ζ_p)^{p−1} ∣ p`, and in a
VALUATION ring `x^{p−1} ∣ y^{p−1}` forces `x ∣ y`. -/
theorem one_sub_pow_dvd_natCast_of_isPrimitiveRoot {A : Type*} [CommRing A] [IsDomain A]
    {q : ℕ} [hq : Fact q.Prime] {ζ : A} (hζ : IsPrimitiveRoot ζ q) :
    (1 - ζ) ^ (q - 1) ∣ (q : A) := by
  classical
  have hprod : ((q : ℕ) : A) = ∏ μ ∈ primitiveRoots q A, (1 - μ) := by
    ...
    simp only [Polynomial.eval_sub, Polynomial.eval_X, Polynomial.eval_C] at h1
    exact h1.symm
  have hdvd : ∀ μ ∈ primitiveRoots q A, (1 - ζ) ∣ (1 - μ) := by
    intro μ hμ
    have hμp : IsPrimitiveRoot μ q := (mem_primitiveRoots hq.out.pos).mp hμ
    obtain ⟨i, -, hi⟩ := hζ.eq_pow_of_pow_eq_one hμp.pow_eq_one
    have h := sub_dvd_pow_sub_pow (1 : A) ζ i
    rw [one_pow, hi] at h
    exact h
  have hcard : (primitiveRoots q A).card = q - 1 := by
    rw [hζ.card_primitiveRoots, Nat.totient_prime hq.out]
...
set_option synthInstance.maxHeartbeats 1000000 in
set_option maxHeartbeats 2000000 in
/-- **THE TAME CHARACTER OF THE DISPLACEMENT LINE IS `𝔽_p`-VALUED — the
one-dimensionality input of the node** (SORRY LEAF, cut 2026-07-25 out
of `displacement_span_le_span_zeta_sub_one`, which is PROVEN over it
directly below): for a geometric point `φ` killed by `p` whose cyclic
group `⟨φ⟩` is INERTIA-STABLE, the ideal of `𝒪̄` generated by the values
of the DISPLACEMENT `g ↦ φ(1 ⊗ g) − ε(g)` is PRINCIPAL, `= (t)`, with a
generator `t` whose `(p − 1)`-st power is fixed pointwise by LOCAL
INERTIA.

**What this says, in one line: `(p − 1)·v(t) ∈ ℤ`.** The fixed field of
local inertia is the maximal unramified extension, whose value group is
`ℤ` because `𝒪ᵥ = ℤ_p` is absolutely unramified (`e = 1`); so an
inertia-invariant `t^{p−1}` has `(p − 1)·v(t) ∈ ℤ`. Conversely, if
`(p − 1)·v(t) = n ∈ ℤ` then any root of `x^{p−1} = p^n` in the
algebraically closed `ℚᵥᵃˡᵍ` has the same valuation as `t`, hence
generates the same ideal of the VALUATION ring `𝒪̄`, and its
`(p − 1)`-st power is the rational integer `p^n`, which inertia fixes.
So the leaf is EXACTLY the assertion that the character through which
inertia acts on the line `⟨φ⟩` is TAME and `𝔽_p`-valued — the
ONE-DIMENSIONALITY that `hstab` encodes.

**Why the statement is in the GALOIS language rather than Oort–Tate's.**
The equivalent normal-form phrasing is "some generator satisfies
`α^p = a·α` with `a` in the base ring `𝒪ᵥ`" (`α^{p−1} = a ∈ ℤ_p` is the
same integrality). That form was the first cut and was replaced on
2026-07-25, because the only route to it that works under
INERTIA-stability alone is route 2 below, whose whole vocabulary is
`σ(x)` for `σ ∈ I_v` — and because in this form the file's already
PROVEN ramification input `mem_span_natCast_of_inertia_invariant` ("an
inertia-invariant element of `𝔪` is divisible by `p`") discharges the
consumer's arithmetic directly. Note that either way this asks only for
a VALUE and never for an `𝒪ᵥ`-rational coordinate of `G`: the Oort–Tate
coordinate is defined only up to an unramified twist, and demanding it
is what made the deleted `exists_muType_closure` FALSE (see the
CORRECTION NOTE at `exists_muType_coordinate`). The value ideal is
twist-blind, so this form is faithful.

**`hstab` IS LOAD-BEARING — the supersingular test.** For the
`p`-torsion of a SUPERSINGULAR elliptic curve over `ℤ_p` the model is
connected and killed by `p`, tame inertia acts through the LEVEL-`2`
fundamental characters, no line is inertia-stable, and the displacement
ideal has `v(t) = 1/(p² − 1)`. There `(p − 1)·v(t) = 1/(p + 1) ∉ ℤ`, so
no generator has an inertia-invariant `(p − 1)`-st power: the conclusion
of this leaf is FALSE and only `hstab` excludes it. Any proof of this
leaf that does not use `hstab`, and any restatement that drops it, is
therefore wrong. (`Odd p` and `hcomul₀` are carried along because the
consumer has them, not because the mathematics below needs them.)

**Content and the chain to build, in dependency order.** Mathlib has no
tame/wild inertia, no ramification filtration and no fundamental
characters, and neither does the reference project `~/cs/FLT` (checked
2026-07-25: `tame`, `fundamentalCharacter`, `Oort`, `Raynaud` return
nothing there). The two routes, both of which need theory that does not
yet exist:

1. *Oort–Tate* (Oort–Tate, *Group schemes of prime order*, Ann. Sci.
   ÉNS 1970, §2; Tate, *Finite flat group schemes*, in
   Cornell–Silverman–Stevens, §4). (a) the schematic closure `C` of
   `⟨φ⟩` is a finite flat closed subGROUP scheme of `Spec G` of order
   `p` killed by `p`, and its vanishing ideal is a Hopf ideal, so both
   `φ` and `ε` factor through `G ↠ 𝒪(C)` and the displacement ideal
   computed on `G` is the one computed on `C`; (b) `𝒪ᵥ = ℤ_p` contains
   `μ_{p−1}`, so Oort–Tate applies and `C = Spec 𝒪ᵥ[x]/(x^p − a x)`
   with `ab = w_p`, `v(w_p) = e = 1`, `ε x = 0`; (c) the geometric
   points are `x ↦ α` with `α^p = aα`, and the displacement values are
   the powers `α^i` (`i ≥ 1`), so the value ideal is `(α)` and
   `α^{p−1} = a ∈ 𝒪ᵥ` is inertia-invariant. CAVEAT for whoever takes
   this route: the schematic closure is defined over `𝒪ᵥ` only when
   `⟨φ⟩` is stable under the FULL local Galois group, whereas `hstab`
   supplies only INERTIA-stability; step (a) therefore needs an
   argument, or the second route.
2. *Raynaud's fundamental characters* (Raynaud, *Schémas en groupes de
   type `(p, …, p)`*, Bull. SMF 102 (1974), 3.3.6/3.4.3), which needs
   only inertia: the ramification filtration of `I_v`, its tame
   quotient and canonical character to `k̄^×`, the fundamental
   characters of level `n` (level `2` must be EXPRESSIBLE, not
   excluded, or the supersingular test above becomes unstatable), and
   the fact that a TAME character trivial modulo `𝔪` is trivial. On an
   inertia-stable line the level is `1`, so the exponent lies in
   `{0, …, e} = {0, 1}` and `v(t) ∈ {0, 1/(p − 1)}`; connectedness kills
   `0`.

**An attack that gets close and then dead-ends, recorded so it is not
re-run.** Writing `d := c − 1` in the convolution ring, `d` kills
`J²` (`J` the augmentation ideal) and `c^m − 1 ≡ m·d mod (d)²`, so for
`σ ∈ I_v` with `σ • φ = φ^{m(σ)}` one gets, honestly and without any
missing theory,

    σ(t) ≡ m(σ)·t  mod (t)²,   i.e.  σ(t)/t ≡ m(σ)  mod (t),

so the character `σ ↦ σ(t)/t mod 𝔪` is `𝔽_p^×`-valued. That pins the
TAME part of the denominator of `v(t)` to a divisor of `p − 1` and says
NOTHING about a `p`-part: for `t` with `v(t) = 1/(p(p − 1))` one has
`σ(t)/t ∈ μ_{p(p−1)}`, and `ζ_p·ζ_{p−1}^j ≡ ζ_{p−1}^j` modulo `(t)`
already, so the displayed congruence holds with `(p − 1)·v(t) = 1/p ∉ ℤ`.
Excluding the wild part is exactly what Raynaud's `e < p − 1` bound
does and what no mod-`(t)²` computation can do; a leaf cut at the
displayed congruence would be FALSE.

**Soundness — the hypothesis set is inhabited, and NOT vacuously.** For
...
displacement values are `ζ_p^i − 1 = (ζ_p − 1)·unit`, the ideal is
`(ζ_p − 1)`, and any root of `x^{p−1} = p` witnesses the conclusion —
note that `(ζ_p − 1)^{p−1}` itself is NOT inertia-invariant (it is `p`
times a unit that inertia moves), which is exactly why the statement
quantifies existentially over the generator; `𝒪̄` has all `(p − 1)`-st
roots, so the unit can always be absorbed. The same holds for every
unramified twist `μ_p ⊗ ψ`.

**Which hypotheses this leaf actually needs, stated honestly.** `hstab`
is the only one the mathematics above turns on. `hφ1` and the
CONNECTEDNESS package (`he₀`/`hε₀`/`hprim₀`/`hφe`) are NOT needed for
this conclusion and are carried only because the consumer has them and
a prover may find them convenient: for `φ = 1` the ideal is `0` and
`t = 0` witnesses it, and for the ÉTALE `ℤ/p` the ideal is `⊤` and
`t = 1` witnesses it (`(p − 1)·v(t) = 0 ∈ ℤ`). That is deliberate, and
it is where the labour is split: the leaf supplies ONLY the integrality
`(p − 1)·v(t) ∈ ℤ`, and the consumer below supplies connectedness
itself — through `point_sub_counit_mem_maximalIdeal`, which puts `t` in
`𝔪` and so lets `mem_span_natCast_of_inertia_invariant` upgrade the
invariance to `p ∣ t^{p−1}`. Neither half alone gives
`v(t) = 1/(p − 1)`. -/
theorem exists_generator_pow_inertia_invariant
    (hpodd : Odd p)
    (G : Type) [CommRing G]
    ...
    (hstab : ∀ τ ∈ localInertiaGroup
      ...
      ∃ m : ℕ, τ • φ = φ ^ m)
    (hφ1 : φ ≠ 1) :
    ∃ t : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ,
      ...
      ∀ σ ∈ localInertiaGroup
          (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime)),
        σ (algebraMap (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) ℚᵖᵍᵥᵃˡᵍ (t ^ (p - 1))) =
          algebraMap (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) ℚᵖᵍᵥᵃˡᵍ (t ^ (p - 1)) :=
  sorry
...
point is CONTAINED in `(ζ_p − 1)`** — valuation-theoretically
`v(t) ≥ 1/(p − 1) = v(ζ_p − 1)` (PROVEN 2026-07-25 over the single leaf
`exists_generator_pow_inertia_invariant`, which carries the
one-dimensionality; this half is the one Raynaud's exponent bound does
NOT supply, its reverse being the elementary
`span_zeta_sub_one_le_displacement_span` below).

Proof, given the leaf. The leaf hands back a generator `t` of the
displacement ideal whose `(p − 1)`-st power local inertia fixes.

* If `t = 0` the ideal is `⊥` and there is nothing to prove.
* CONNECTEDNESS enters here, and only here:
  `point_sub_counit_mem_maximalIdeal` puts every displacement value in
  `𝔪 𝒪̄`, so `t ∈ 𝔪 𝒪̄` and hence `t^{p−1} ∈ 𝔪 𝒪̄`. Now the file's
  ramification input `mem_span_natCast_of_inertia_invariant` — "over the
  ABSOLUTELY UNRAMIFIED `𝒪ᵥ = ℤ_p`, an inertia-invariant element of `𝔪`
  is divisible by `p`" — applies verbatim and gives `p ∣ t^{p−1}`.
  Without connectedness `t^{p−1}` could be a unit: that is the étale
  branch `v(t) = 0`, where the leaf still holds but `p ∤ t^{p−1}`.
* Finally `(1 − ζ_p)^{p−1} ∣ p` (`one_sub_pow_dvd_natCast_of_isPrimitiveRoot`),
  so `(1 − ζ_p)^{p−1} ∣ t^{p−1}`, and in a VALUATION ring that forces
  `1 − ζ_p ∣ t`: divisibility is total, and in the other case
  `t = (1 − ζ_p)·e⁻¹` follows after cancelling `t^{p−1}` in the domain
  `𝒪̄` and finding `e` a unit. (This is the mirror of the cancellation
  step in `span_zeta_sub_one_le_displacement_span`.)

So the ONLY missing mathematics in this half is the leaf; everything
else is the elementary cyclotomic and valuation-ring bookkeeping above,
plus the already PROVEN ramification input. The supersingular test is
passed by construction: the sole use of `hstab` is the appeal to the
leaf, whose conclusion is FALSE for supersingular `E[p]`, so no step
here silently generalises past one-dimensionality. -/
theorem displacement_span_le_span_zeta_sub_one
  ...
  classical
  obtain ⟨t, hspan, hinv⟩ :=
    exists_generator_pow_inertia_invariant hpodd G e₀ he₀ hε₀ hprim₀ hcomul₀
      φ hφe hord hstab hφ1
  set S : Set (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) :=
    {x : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ | ∃ g : G,
      algebraMap (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) ℚᵖᵍᵥᵃˡᵍ x =
        φ ((1 : ℚᵖᵍᵥ) ⊗ₜ[𝒪ᵖᵍᵥ] g) -
          algebraMap 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ (Coalgebra.counit (R := 𝒪ᵖᵍᵥ) g)} with hSdef
  -- CONNECTEDNESS: every displacement value lies in the maximal ideal of `𝒪̄`
  have hSmax : Ideal.span S ≤ maximalIdeal (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) := by
    rw [Ideal.span_le]
    intro x hx
    rw [hSdef] at hx
    obtain ⟨g, hg⟩ := hx
    ...
    have hxy : x = y := by
      ...
      show algebraMap (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) ℚᵖᵍᵥᵃˡᵍ x =
        algebraMap (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) ℚᵖᵍᵥᵃˡᵍ y
      rw [hg, ← hyeq]
      rfl
    rw [hxy]
    exact hy
  rcases eq_or_ne t 0 with ht0 | ht0
  · rw [hspan, ht0, Ideal.span_singleton_eq_bot.mpr rfl]
    exact bot_le
  have htmax : t ∈ maximalIdeal (IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) := by
    rw [hspan] at hSmax
    exact hSmax (Ideal.mem_span_singleton_self t)
  have hp2 : 2 ≤ p := hp.out.two_le
  -- ABSOLUTE UNRAMIFIEDNESS: an inertia-invariant element of `𝔪` is a multiple of `p`
  have hpa : ((p : ℕ) : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) ∣ t ^ (p - 1) :=
    Ideal.mem_span_singleton.mp
      (mem_span_natCast_of_inertia_invariant (t ^ (p - 1))
        (Ideal.pow_mem_of_mem _ htmax _ (by omega)) hinv)
  have hζI : IsPrimitiveRoot ζ p :=
    hζ.of_map_of_injective (fun a b h => Subtype.ext h)
  have hcyc : ((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) ^ (p - 1) ∣
      ((p : ℕ) : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) :=
    one_sub_pow_dvd_natCast_of_isPrimitiveRoot hζI
  have hdvdpow : ((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) ^ (p - 1) ∣ t ^ (p - 1) :=
    hcyc.trans hpa
  -- in a valuation ring, `π^{p−1} ∣ t^{p−1}` forces `π ∣ t`
  have hdvd : ((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) ∣ t := by
    obtain ⟨e, he⟩ :=
      ValuationRing.cond ((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) t
    rcases he with h | h
    · exact ⟨e, h.symm⟩
    · obtain ⟨s, hs⟩ := hdvdpow
      have hpowe : t ^ (p - 1) = t ^ (p - 1) * (e ^ (p - 1) * s) := by
        conv_lhs => rw [hs, ← h, mul_pow]
        ring
      have hone : (1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) = e ^ (p - 1) * s :=
        mul_left_cancel₀ (pow_ne_zero _ ht0) (by linear_combination hpowe)
  ...
  rw [hspan, Ideal.span_le, Set.singleton_subset_iff, SetLike.mem_coe,
    Ideal.mem_span_singleton,
    show ζ - 1 = -((1 : IntegralClosure 𝒪ᵖᵍᵥ ℚᵖᵍᵥᵃˡᵍ) - ζ) by ring]
...
/-- **The displacement ideal of a connected inertia-stable order-`p`
point is `(ζ_p − 1)`** (PROVEN 2026-07-25 by `le_antisymm` between
`displacement_span_le_span_zeta_sub_one` — itself proven the same day
over the single leaf `exists_generator_pow_inertia_invariant` — and
the elementary bound `span_zeta_sub_one_le_displacement_span`). This is
the Oort–Tate/Raynaud input of `inertia_character_trivial_or_cyclotomic`;
see `exists_generator_pow_inertia_invariant`'s docstring for the
mathematics and for exactly what remains open. -/
...
`exists_generator_pow_inertia_invariant`): for a geometric point `φ` of exact
...
own open input is `exists_generator_pow_inertia_invariant`.
...
remaining input is the single one-dimensionality leaf
`exists_generator_pow_inertia_invariant`): under the hypotheses of
...
* `inertia_character_trivial_or_cyclotomic` — RAYNAUD's dichotomy at
  ...
  (PROVEN 2026-07-25 over the single leaf
  `exists_generator_pow_inertia_invariant`, "the displacement ideal has
  a generator `t` whose `(p − 1)`-st power local inertia fixes", i.e.
  `(p − 1)·v(t) ∈ ℤ`, which is the ONE-DIMENSIONALITY half of
  Oort–Tate's `v(a) = 1` and is the only sorry left in this file — both
  inclusions of `displacement_span_eq_span_zeta_sub_one` are now proven,
  the `≤` half over that leaf and the `≥` half, Raynaud's `k ≤ e`,
  outright);

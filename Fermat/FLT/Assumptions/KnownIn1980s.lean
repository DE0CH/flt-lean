...
short and uses only techniques known in the 1980s is fine.
-/
/- VENDORING CHANGE (this project): the original declares
`axiom knownin1980s {P : Prop} : P`, an axiom proving any proposition, which
would be invisible to a `sorryAx`-based audit and is unsound as an axiom.
We replace it by a `sorry`-backed theorem so that every use is tracked by
`#print axioms` through `sorryAx`. -/
theorem knownin1980s {P : Prop} : P := sorry

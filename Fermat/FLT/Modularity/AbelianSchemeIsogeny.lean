...
-- `Fermat.locallyOfFinitePresentation_of_comp` (cancellation for
-- `LocallyOfFinitePresentation`, Stacks 02FV, proven here 2026-07-26) was HOISTED
-- VERBATIM on 2026-07-28 into
-- `Fermat/FLT/Mathlib/AlgebraicGeometry/Morphisms/FinitePresentationCancel.lean`,
-- which this module now `public import`s.  It keeps its name and its `Fermat`
-- namespace, so the use below and the one in `Modularity/TateModule.lean` resolve
-- unchanged.  The hoist was needed because `FreyCurve/MazurTorsion.lean` — which is
-- not in this module's import cone, and cannot be — needs it to reduce
-- `etale_of_mono_of_relCurve` to miracle flatness, and duplicating a top-level name
-- in the same namespace would break every module importing both.

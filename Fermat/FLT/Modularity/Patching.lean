...
/-- **`(Runiv, ρuniv)` is TRACE-GENERATED** (added 2026-07-29 as the
faithfulness repair of `exists_auxDeformationDiamondControl` far below):
`Runiv` is topologically generated over `ℤ_[p]` by the Frobenius traces
`(ρuniv.charFrob q).coeff 1` at the primes OUTSIDE `Suniv`.

This is Carayol's minimality half of universality, and it is exactly what
`IsWeaklyUniversalDeformation` above deliberately omits — read that
declaration's last paragraph, which says so in as many words ("Uniqueness of
`ψ` (Carayol trace generation of `Runiv`) is deliberately NOT part of the
clause").  Existence-only universality pins `Runiv` in NO direction:

> if `(Runiv₀, ρuniv₀, πuniv₀)` is weakly universal then so is
> `(Runiv₀[[y_1, …, y_m]], ρuniv₀.baseChange, πuniv₀ ∘ constantCoeff)` for
> every `m`, since a classifying map out of `Runiv₀` precomposed with
> `MvPowerSeries.constantCoeff` classifies just as well.

That is a genuine counterexample family, not a technicality: it refutes any
statement asking for a SURJECTION onto `Runiv` from a ring fixed
independently of `m`, by Krull dimension.  `IsTraceGeneratedDeformation`
excludes it — `y_1` is not in the closed `ℤ_[p]`-subalgebra generated by the
traces of `ρuniv₀.baseChange`, all of which are constants.

The `Suniv` in the clause is the same exceptional finite set that
`hunivred` already carries in every statement using this predicate, so
demanding generation by the traces AWAY from `Suniv` (rather than by all of
them) is the strong form and costs nothing: it is the form the classifying
map out of `𝒟Q.R` can actually hit, through `𝒟Q.charFrob_compat` and the
`Sf`-clause of `AuxDeformationDatum.IsWeaklyUniversal`.

Stated afresh here rather than imported: `Lift.lean`'s trace-generation
vocabulary is DOWNSTREAM of this module's consumer, so it is not available.
The Hilbert twin `exists_hilbertAuxDeformationRingPresentation` carries the
same clause under the name `𝒟.IsTraceGenerated`. -/
...
set_option linter.checkUnivs false in
/-- **Carayol: a weakly universal deformation may be taken TRACE-GENERATED**
(sorry node, cut 2026-07-29 as the faithfulness repair of
`exists_auxDeformationDiamondControl` far below): from ANY weakly universal
package `(R₀, ρ₀, π₀)` one may pass to a weakly universal package that is
additionally topologically generated over `ℤ_[p]` by its Frobenius traces away
from the exceptional set.

# WHY THIS LEAF EXISTS, AND WHY IT IS THE `∃`-FORM AND NOT AN IMPLICATION

`IsWeaklyUniversalDeformation` is existence-only, and existence-only
universality pins the ring in NO direction: `R₀[[y_1, …, y_m]]` classifies
everything `R₀` does (a classifying map out of `R₀` precomposed with
`MvPowerSeries.constantCoeff` is a classifying map out of the bigger ring).
So the IMPLICATION "weakly universal ⟹ trace-generated" is **FALSE**, with
that family as the witness, and stating this leaf that way would manufacture a
false node.  What is true — and what Carayol's theorem gives — is that one may
DESCEND: the closed `ℤ_[p]`-subalgebra generated by the traces carries `ρ₀`
(absolute irreducibility of `ρbar` is exactly the hypothesis for that descent),
it inherits weak universality because classifying maps restrict along it, and
it is trace-generated by construction.  Hence the existential form.

**This is not decoration; it is what makes `R = 𝕋` true.**
`injective_ringHom_of_isWeaklyUniversal` below is FALSE without it: for
`Runiv = R₀[[y]]`, still weakly universal, the classifying map
`ψ : Runiv →+* T` may send `y ↦ 0` while satisfying every trace and
`ℤ_[p]`-structure clause, so `ψ` is not injective.  The same inflation refutes
`exists_auxDeformationDiamondControl`'s control map far below (see its FALSITY
AUDIT).  Both are repaired by the single clause this leaf supplies.

References: Carayol, in *p-adic monodromy and the Birch–Swinnerton-Dyer
conjecture*, Contemp. Math. 165 (1994), Thm. 3 (deformations descend to the
trace subring for absolutely irreducible residual representations); Mazur, in
*Galois Groups over ℚ* (1989), §1.4 and §1.9; Nyssen, Manuscripta Math. 89
(1996); Rouquier, J. Algebra 180 (1996).

CIRCULARITY GUARD: as for `exists_weaklyUniversal_hardlyRamifiedDeformation`
below — a proof discharging this by `exfalso` from the odd-prime dichotomy
against `hirr` and `hρbar` must be rejected. -/
theorem exists_traceGenerated_of_isWeaklyUniversal.{t, uK, uW, uR}
    {p : ℕ} (hpodd : Odd p) [Fact p.Prime]
    {k : Type uK} [Field k] [Finite k] [Algebra ℤ_[p] k]
    [TopologicalSpace k] [DiscreteTopology k] [IsTopologicalRing k]
    {W : Type uW} [AddCommGroup W] [Module k W] [Module.Finite k W]
    ...
    (hrank₀ : Module.rank R₀ (Fin 2 → R₀) = 2)
    (hρ₀ : IsHardlyRamified hpodd hrank₀ ρ₀)
    ...
    (hred₀ : ∀ (q : ℕ) (hq : q.Prime),
      ...
      π₀ ((ρ₀.charFrob hq.toHeightOneSpectrumRingOfIntegersRat).coeff 1) =
        (ρbar.charFrob hq.toHeightOneSpectrumRingOfIntegersRat).coeff 1)
    (hws : IsWeaklyUniversalDeformation.{uR, uR, uK, uW, uR} hpodd ρbar ρ₀ π₀)
    (hwt : IsWeaklyUniversalDeformation.{uR, t, uK, uW, uR} hpodd ρbar ρ₀ π₀) :
    ∃ (Runiv : Type uR) (_ : CommRing Runiv) (_ : TopologicalSpace Runiv)
      (_ : IsTopologicalRing Runiv) (_ : IsLocalRing Runiv)
      (_ : Algebra ℤ_[p] Runiv) (_ : IsNoetherianRing Runiv)
      (_ : IsAdic (IsLocalRing.maximalIdeal Runiv))
      (_ : IsAdicComplete (IsLocalRing.maximalIdeal Runiv) Runiv)
      ...
      (∀ (q : ℕ) (hq : q.Prime),
        ...
        πuniv ((ρuniv.charFrob
            hq.toHeightOneSpectrumRingOfIntegersRat).coeff 1) =
          (ρbar.charFrob hq.toHeightOneSpectrumRingOfIntegersRat).coeff 1) ∧
      IsWeaklyUniversalDeformation.{uR, uR, uK, uW, uR} hpodd ρbar ρuniv
        πuniv ∧
      IsWeaklyUniversalDeformation.{uR, t, uK, uW, uR} hpodd ρbar ρuniv
        πuniv ∧
      IsTraceGeneratedDeformation p ρuniv Suniv :=
  sorry
...
theorem exists_weaklyUniversal_hardlyRamifiedDeformation.{s, t, uK, uW}
    ...
    ∃ (Runiv : Type s) (_ : CommRing Runiv) (_ : TopologicalSpace Runiv)
      ...
      IsWeaklyUniversalDeformation.{s, t, uK, uW, s} hpodd ρbar ρuniv
        πuniv ∧
      IsTraceGeneratedDeformation p ρuniv Suniv := by
  obtain ⟨Runiv, iCR, iTS, iTR, iLR, iAlg, iNoeth, hadic, hcomplete, ρuniv,
  ...
  letI := iAlg
  letI := iNoeth
  -- Carayol's descent to the trace subalgebra (2026-07-29): existence-only
  -- universality does NOT pin the ring — `R₀[[y]]` classifies whatever `R₀`
  -- does — so the minimality clause is genuinely new content and is supplied
  -- by the leaf above rather than read off the construction.
  exact exists_traceGenerated_of_isWeaklyUniversal.{t, uK, uW, s} hpodd hW
    hρbar hirr hadic hcomplete hranku hHR hπsurj hred
...
/-- **Hensel-free simple-root separation** (PROVEN 2026-07-28; the
commutative-algebra core of `exists_finset_isUnramifiedAt_of_notMem` below).
In a LOCAL DOMAIN `R`, two roots of a polynomial `P` that are congruent modulo the
maximal ideal COINCIDE, as soon as `P'` fails to lie in the maximal ideal at one of
them.

No Hensel lemma, no completeness, no separability hypothesis is used.  `P` factors as
`(X - xa) * H` over `R` (a monic linear divisor exists over any commutative ring,
`Polynomial.dvd_iff_isRoot`), so `P'(xa) = H(xa)`; if `xs ≠ xa` then
`0 = P(xs) = (xs - xa) * H(xs)` forces `H(xs) = 0` because `R` is a domain; and
`xs - xa` divides `H(xs) - H(xa)` (`Polynomial.sub_dvd_eval_sub`), so
`P'(xa) = H(xa) ∈ 𝔪` — against the hypothesis. -/
theorem eq_of_sub_mem_maximalIdeal
    {R : Type*} [CommRing R] [IsDomain R] [IsLocalRing R]
    (P : Polynomial R) (xa xs : R)
    (hPa : P.eval xa = 0) (hPs : P.eval xs = 0)
    (hcong : xs - xa ∈ IsLocalRing.maximalIdeal R)
    (hunit : (Polynomial.derivative P).eval xa ∉ IsLocalRing.maximalIdeal R) :
    xs = xa := by
  classical
  by_contra hne
  obtain ⟨H, hH⟩ : (Polynomial.X - Polynomial.C xa) ∣ P := Polynomial.dvd_iff_isRoot.mpr hPa
  have hderiv : (Polynomial.derivative P).eval xa = H.eval xa := by
    have h2 := congrArg Polynomial.derivative hH
    rw [Polynomial.derivative_mul, Polynomial.derivative_sub,
      Polynomial.derivative_X, Polynomial.derivative_C, sub_zero, one_mul] at h2
    have h3 := congrArg (Polynomial.eval xa) h2
    simpa using h3
  have h0 : (xs - xa) * H.eval xs = 0 := by
    have h4 := congrArg (Polynomial.eval xs) hH
    rw [hPs] at h4
    simpa using h4.symm
  have hHs : H.eval xs = 0 := by
    rcases mul_eq_zero.mp h0 with h | h
    · exact absurd h (sub_ne_zero.mpr hne)
    · exact h
  have hdiff : H.eval xs - H.eval xa ∈ IsLocalRing.maximalIdeal R := by
    obtain ⟨d, hd⟩ := Polynomial.sub_dvd_eval_sub xs xa H
    rw [hd]
    exact Ideal.mul_mem_right _ _ hcong
  rw [hHs, zero_sub] at hdiff
  exact hunit (hderiv ▸ (Ideal.neg_mem_iff _).mp hdiff)

/-- Transport of `eval₂` along a ring homomorphism, for an INTEGER polynomial: the two
`ℤ →+* _` structure maps agree because `ℤ` is initial (`RingHom.ext_int`).

Stated with `Int.castRingHom` rather than with `algebraMap ℤ _` on purpose: the fields
and integral closures below carry several `Algebra ℤ _` instances that are equal but not
syntactically so, and routing every evaluation through the canonical ring map removes an
entire class of "synthesized instance is not definitionally equal" failures. -/
theorem eval₂_intCastRingHom_map {A B : Type*} [CommRing A] [CommRing B] (g : A →+* B)
    (x : A) (F : Polynomial ℤ) :
    g (Polynomial.eval₂ (Int.castRingHom A) x F) =
      Polynomial.eval₂ (Int.castRingHom B) (g x) F := by
  rw [Polynomial.hom_eval₂]
  congr 1
  exact RingHom.ext_int _ _

/-- `eval₂` against `Int.castRingHom` IS `aeval`, for any `ℤ`-algebra structure — the
bridge between the instance-free spelling used below and mathlib's `minpoly` API. -/
theorem eval₂_intCastRingHom_eq_aeval {A : Type*} [CommRing A] [Algebra ℤ A]
    (x : A) (F : Polynomial ℤ) :
    Polynomial.eval₂ (Int.castRingHom A) x F = (Polynomial.aeval x) F := by
...
set_option backward.isDefEq.respectTransparency false in
/-- **The local step** (PROVEN 2026-07-28): an element `σ` of the local inertia at `q`
fixes an algebraic integer `α` whose "different" `β = F'(α)` (`F = minpoly ℤ α`) divides
an integer `m` PRIME TO `q`.

Everything happens inside `𝒪 = IntegralClosure 𝒪ᵥ ℚ_qᵃˡᵍ`, whose maximal ideal is what
DEFINES `localInertiaGroup v`; no ramification index, discriminant or inertia dictionary
appears.  Write `ι` for the embedding `ℚᵃˡᵍ → ℚ_qᵃˡᵍ` underlying
`Field.absoluteGaloisGroup.map`.  Then `ια` and `σ(ια)` are both roots of `F` in `𝒪`,
they are congruent modulo `𝔪 𝒪` by the very definition of the local inertia, and
`F'(ια) = ιβ` is a UNIT of `𝒪`: it divides `ιm`, and `m ∉ 𝔪 𝒪` because
`𝔪 𝒪 ⊓ 𝒪ᵥ = 𝔪 𝒪ᵥ` (going-up: the contraction of a maximal ideal along an integral
extension is maximal) contains `q`, while `x q + y m = 1` for some integers `x, y`.
`eq_of_sub_mem_maximalIdeal` closes it, and `Field.absoluteGaloisGroup.lift_map` together
with the injectivity of `ι` transports the conclusion back to `Γ ℚ`. -/
theorem map_localInertia_fixed
    (v : IsDedekindDomain.HeightOneSpectrum (NumberField.RingOfIntegers ℚ))
    (q : ℕ)
    (hqmem : (q : HeightOneSpectrum.adicCompletionIntegers ℚ v) ∈
      IsLocalRing.maximalIdeal (HeightOneSpectrum.adicCompletionIntegers ℚ v))
    (m : ℤ) (hcop : IsCoprime ((q : ℤ)) m)
    (α β c : AlgebraicClosure ℚ) (F : Polynomial ℤ)
    (hFα : Polynomial.eval₂ (Int.castRingHom (AlgebraicClosure ℚ)) α F = 0)
    (hβ : β = Polynomial.eval₂ (Int.castRingHom (AlgebraicClosure ℚ)) α
      (Polynomial.derivative F))
    (hαint : IsIntegral ℤ α) (hcint : IsIntegral ℤ c)
    (hmc : β * c = (m : AlgebraicClosure ℚ))
    (σ : Field.absoluteGaloisGroup (HeightOneSpectrum.adicCompletion ℚ v))
    (hσ : σ ∈ localInertiaGroup v) :
    (Field.absoluteGaloisGroup.map
      (algebraMap ℚ (HeightOneSpectrum.adicCompletion ℚ v))) σ α = α := by
  classical
  set ι : AlgebraicClosure ℚ →+*
      AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v) :=
    AlgebraicClosure.map (algebraMap ℚ (HeightOneSpectrum.adicCompletion ℚ v)) with hιdef
  ...
  have hint : ∀ x : AlgebraicClosure ℚ, IsIntegral ℤ x →
      IsIntegral (HeightOneSpectrum.adicCompletionIntegers ℚ v) (ι x) := by
    intro x hx
    obtain ⟨P, hPm, hPe⟩ := hx
    refine ⟨P.map (Int.castRingHom (HeightOneSpectrum.adicCompletionIntegers ℚ v)),
      hPm.map _, ?_⟩
    have hcomp : (algebraMap (HeightOneSpectrum.adicCompletionIntegers ℚ v)
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v))).comp
        (Int.castRingHom (HeightOneSpectrum.adicCompletionIntegers ℚ v)) =
        Int.castRingHom (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)) :=
      RingHom.ext_int _ _
    have hP0 : Polynomial.eval₂ (Int.castRingHom (AlgebraicClosure ℚ)) x P = 0 := by
      rw [eval₂_intCastRingHom_eq_aeval, Polynomial.aeval_def]
      exact hPe
    rw [Polynomial.eval₂_map, hcomp, ← eval₂_intCastRingHom_map ι x P, hP0, map_zero]
  -- the elements of the big integral closure
  set xa : IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)) :=
    ⟨ι α, hint α hαint⟩ with hxadef
  set xc : IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)) :=
    ⟨ι c, hint c hcint⟩ with hxcdef
  set j : IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)) →+*
      AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v) := algebraMap _ _ with hjdef
  have hinjj : Function.Injective j := fun x y h => Subtype.ext h
  have hjxa : j xa = ι α := rfl
  have hjxc : j xc = ι c := rfl
  set xs : IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)) := σ • xa with hxsdef
  have hjxs : j xs = τ (ι α) := rfl
  -- the integer polynomial, read over the big integral closure
  set P : Polynomial (IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v))) :=
    F.map (Int.castRingHom _) with hPdef
  have hPa : P.eval xa = 0 := by
    apply hinjj
    rw [map_zero, hPdef, Polynomial.eval_map, eval₂_intCastRingHom_map j xa F, hjxa,
      ← eval₂_intCastRingHom_map ι α F, hFα, map_zero]
  have hPs : P.eval xs = 0 := by
    apply hinjj
    rw [map_zero, hPdef, Polynomial.eval_map, eval₂_intCastRingHom_map j xs F, hjxs,
      ← eval₂_intCastRingHom_map τ (ι α) F, ← eval₂_intCastRingHom_map ι α F, hFα,
      map_zero, map_zero]
  ...
  have hjb : j ((Polynomial.derivative P).eval xa) = ι β := by
    rw [hPdef, Polynomial.derivative_map, Polynomial.eval_map,
      eval₂_intCastRingHom_map j xa (Polynomial.derivative F), hjxa,
      ← eval₂_intCastRingHom_map ι α (Polynomial.derivative F), ← hβ]
  -- `m` is a unit in the big integral closure
  haveI : (IsLocalRing.maximalIdeal
      (IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)))).IsMaximal :=
    IsLocalRing.maximalIdeal.isMaximal _
  have hcomapm : (IsLocalRing.maximalIdeal
      (IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)))).comap
      (algebraMap (HeightOneSpectrum.adicCompletionIntegers ℚ v)
        (IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
          (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)))) =
      IsLocalRing.maximalIdeal (HeightOneSpectrum.adicCompletionIntegers ℚ v) := by
    have hmax := Ideal.isMaximal_comap_of_isIntegral_of_isMaximal
      (R := HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (S := IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)))
  ...
  have hqO : (((q : ℤ)) : IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v))) ∈
      IsLocalRing.maximalIdeal _ := by
    have h1 : ((q : HeightOneSpectrum.adicCompletionIntegers ℚ v)) ∈
        (IsLocalRing.maximalIdeal
          (IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
            (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)))).comap
        (algebraMap (HeightOneSpectrum.adicCompletionIntegers ℚ v) _) := by
      rw [hcomapm]; exact hqmem
    rw [Ideal.mem_comap, map_natCast] at h1
    push_cast
    exact h1
  obtain ⟨x, y, hxy⟩ := hcop
  have hmnot : ((m : IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v)))) ∉
      IsLocalRing.maximalIdeal _ := by
    intro hmem
    have h2 : (((1 : ℤ)) : IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v))) ∈
        IsLocalRing.maximalIdeal _ := by
      rw [← hxy]
      push_cast
      exact Ideal.add_mem _ (Ideal.mul_mem_left _ _ hqO) (Ideal.mul_mem_left _ _ hmem)
    rw [Int.cast_one] at h2
    exact (IsLocalRing.maximalIdeal.isMaximal _).ne_top
      (Ideal.eq_top_of_isUnit_mem _ h2 isUnit_one)
  have hunit : (Polynomial.derivative P).eval xa ∉ IsLocalRing.maximalIdeal
      (IntegralClosure (HeightOneSpectrum.adicCompletionIntegers ℚ v)
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ v))) := by
    intro hb
    refine hmnot ?_
    have hprod : (Polynomial.derivative P).eval xa * xc =
  ...
  have hxs_eq := eq_of_sub_mem_maximalIdeal P xa xs hPa hPs hcong hunit
  -- conclude
  have hfix : σ (ι α) = ι α := by
    rw [← hτapp, ← hjxs, ← hjxa, hxs_eq]
  have hlift := Field.absoluteGaloisGroup.lift_map
    (algebraMap ℚ (HeightOneSpectrum.adicCompletion ℚ v)) σ α
  refine RingHom.injective ι ?_
  rw [hlift]
  exact hfix

set_option maxHeartbeats 1000000 in
set_option backward.isDefEq.respectTransparency false in
/-- **The global step** (PROVEN 2026-07-28): an algebraic integer `α` is fixed by the
image of the local inertia at ALL BUT FINITELY MANY rational primes.

The bad set is `T = (minpoly ℤ β).coeff 0 |>.natAbs.primeFactors`, where
`β = F'(α)` for `F = minpoly ℤ α`.  Two facts make it work:

* `β ≠ 0`.  Otherwise `minpoly ℚ α` divides its own derivative, which in characteristic
  zero forces the derivative to vanish and hence `minpoly ℚ α` to be constant — against
  `minpoly.natDegree_pos`.  (This is separability, spelled out rather than invoked.)
* `β` divides a NONZERO integer, namely `m = (minpoly ℤ β).coeff 0`: split
  `minpoly ℤ β = X * divX + C m` (`Polynomial.X_mul_divX_add`) and evaluate at `β`.
  `m ≠ 0` because `(minpoly ℚ β).coeff 0 ≠ 0` for `β ≠ 0`.

Both `β` and the cofactor are integral over `ℤ` (they are `ℤ`-polynomial expressions in
`α` resp. `β`), which is what lets `map_localInertia_fixed` place them in the integral
closure at every `q`. -/
theorem exists_finset_map_localInertia_fixed (α : AlgebraicClosure ℚ)
    (hα : IsIntegral ℤ α) :
    ∃ T : Finset ℕ, ∀ (q : ℕ) (hq : q.Prime), q ∉ T →
      ∀ σ ∈ localInertiaGroup hq.toHeightOneSpectrumRingOfIntegersRat,
        (Field.absoluteGaloisGroup.map (algebraMap ℚ
          (HeightOneSpectrum.adicCompletion ℚ
            hq.toHeightOneSpectrumRingOfIntegersRat))) σ α = α := by
  classical
  have hαQ : IsIntegral ℚ α := Algebra.IsIntegral.isIntegral α
  set F : Polynomial ℤ := minpoly ℤ α with hFdef
  have hFα : Polynomial.eval₂ (Int.castRingHom (AlgebraicClosure ℚ)) α F = 0 := by
  ...
  set β : AlgebraicClosure ℚ :=
    Polynomial.eval₂ (Int.castRingHom (AlgebraicClosure ℚ)) α
      (Polynomial.derivative F) with hβdef
  have hmapF : minpoly ℚ α = F.map (algebraMap ℤ ℚ) :=
    minpoly.isIntegrallyClosed_eq_field_fractions' ℚ hα
  have hβ0 : β ≠ 0 := by
    intro h0
    have hd : (Polynomial.aeval α) (Polynomial.derivative (minpoly ℚ α)) = 0 := by
      rw [hmapF, Polynomial.derivative_map, Polynomial.aeval_map_algebraMap,
  ...
  have hβint : IsIntegral ℤ β := by
    ...
    have h1 := hle (Polynomial.aeval_mem_adjoin_singleton
      (R := ℤ) (x := α) (p := Polynomial.derivative F))
  ...
  have hmc : β * c = ((G.coeff 0 : ℤ) : AlgebraicClosure ℚ) := by
    ...
    simp
  refine ⟨(G.coeff 0).natAbs.primeFactors, ?_⟩
  intro q hq hqT σ hσ
  refine map_localInertia_fixed hq.toHeightOneSpectrumRingOfIntegersRat q ?_
    (G.coeff 0) ?_ α β c F hFα hβdef hα hcint hmc σ hσ
  · rw [maximalIdeal_adicCompletionIntegers_eq_span hq]
    exact Ideal.mem_span_singleton_self _
  · have hqp : Prime ((q : ℤ)) := Nat.prime_iff_prime_int.mp hq
    refine (Prime.coprime_iff_not_dvd hqp).mpr ?_
    intro hdvd
    refine hqT (Nat.mem_primeFactors.mpr ⟨hq, ?_, Int.natAbs_ne_zero.mpr hm0⟩)
...
unramified outside a finite set of primes** (cut out 2026-07-28 as the first of the
three inputs of `finite_h1TwistUnramified` below; **PROVEN 2026-07-28** over the four
auxiliary declarations immediately above).

This is the classical "a number field is ramified at only finitely many primes",
transported through the kernel of `ρbar`.

# THE PROOF ACTUALLY USED — AND A CORRECTION TO THE ROUTE THIS CUT PREDICTED

**CORRECTION 2026-07-28.**  The version of this docstring written when the leaf was cut
said the leaf costs exactly one new dictionary lemma: the CONVERSE of
`MinkowskiUnramified.lean`'s PROVEN `isUnramifiedAt_of_inertia_le_fixingSubgroup`, i.e.
"`q` unramified in the finite Galois `K` ⟹ the image of `I_q` fixes `K` pointwise".
The observation that only the forward direction exists at this pin is CORRECT and still
is.  The conclusion drawn from it was not: that converse is *expensive* — through the
`M = ℚ_q(ιK)` route it needs the local–global compatibility `e(M/ℚ_q) = e(Q|q)`, which
this tree does not have — and it is **not needed here at all**.

The proof below mentions no discriminant, no ramification index and no inertia
dictionary.  It goes straight at the DEFINITION of `localInertiaGroup v` (trivial action
modulo the maximal ideal of the integral closure of `𝒪ᵥ` in `ℚ_qᵃˡᵍ`):

* `{g | ρbar g = 1}` is OPEN (`isOpen_setOf_galoisRep_eq_one`: `Module.End k W` is finite
  and discrete), hence closed, hence the fixing subgroup of its fixed field `K`, which is
  FINITE-dimensional over `ℚ` (`InfiniteGalois.isOpen_iff_finite`).
* Take a `ℚ`-basis of `K` and scale each basis vector to an ALGEBRAIC INTEGER
  (`IsAlgebraic.exists_integral_multiple`, one scalar per vector — no common denominator
  is needed).
* `exists_finset_map_localInertia_fixed` gives, for one algebraic integer `α`, a finite
  set of primes off which every local-inertia element fixes `α`.  `T` is the union over
  the basis.
* Off `T`, the resulting element of `Γ ℚ` fixes a `ℚ`-basis of `K`, hence — being
  `ℚ`-linear — all of `K` (`Module.Basis.ext`), hence lies in `K.fixingSubgroup`, which is
  the kernel.  `GaloisRep.toLocal_apply` turns that into `IsUnramifiedAt`.

So the honest cost of this leaf was a Hensel-free simple-root separation lemma over an
arbitrary local domain (`eq_of_sub_mem_maximalIdeal`), not a piece of ramification
theory.  **Nobody should be dispatched at the converse of
`isUnramifiedAt_of_inertia_le_fixingSubgroup` on this leaf's account** — it remains
missing, and remains a fine thing to have, but this leaf does not pay for it.

Both-ways audit: a plain classical finiteness with no hypothesis on `ρbar` beyond its
type, hence non-vacuous and not discharge-able by refuting any package.  It is stated
for the FULL prime set rather than for a bound so that the consumer may enlarge `T`
freely (it adds `2` and `p` itself). -/
theorem exists_finset_isUnramifiedAt_of_notMem.{uK, uW} (p : ℕ) [Fact p.Prime]
    ...
    [Module.Free k W] (ρbar : GaloisRep ℚ k W) :
    ∃ T : Finset ℕ, ∀ (q : ℕ) (hq : q.Prime), q ∉ T →
      ρbar.IsUnramifiedAt hq.toHeightOneSpectrumRingOfIntegersRat := by
  classical
  ...
  have hopen : IsOpen ((N : Subgroup (Field.absoluteGaloisGroup ℚ)) :
    ...
    isOpen_setOf_galoisRep_eq_one ρbar hfinW
  ...
  haveI hfd : FiniteDimensional ℚ
      (IntermediateField.fixedField (E := AlgebraicClosure ℚ) N) :=
    (InfiniteGalois.isOpen_iff_finite _).mp (by rw [hfix]; exact hopen)
  -- a ℚ-basis of the fixed field, scaled to algebraic integers
  ...
  have hex : ∀ i, ∃ T : Finset ℕ, ∀ (q : ℕ) (hq : q.Prime), q ∉ T →
      ∀ σ ∈ localInertiaGroup hq.toHeightOneSpectrumRingOfIntegersRat,
        (Field.absoluteGaloisGroup.map (algebraMap ℚ
          (HeightOneSpectrum.adicCompletion ℚ
          ...
          Y i • ((b i : IntermediateField.fixedField (E := AlgebraicClosure ℚ) N) :
            AlgebraicClosure ℚ) :=
    fun i => exists_finset_map_localInertia_fixed _ (hYint i)
  choose T hT using hex
  refine ⟨Finset.univ.biUnion T, fun q hq hqT => ⟨fun σ hσ => ?_⟩⟩
  show (ρbar.toLocal hq.toHeightOneSpectrumRingOfIntegersRat) σ = 1
  rw [GaloisRep.toLocal_apply]
  have hgfix : ∀ i, (Field.absoluteGaloisGroup.map (algebraMap ℚ
    ...
    intro i
    have h1 := hT i q hq
      (fun hmem => hqT (Finset.mem_biUnion.mpr ⟨i, Finset.mem_univ i, hmem⟩)) σ hσ
  ...
  have hgN : (Field.absoluteGaloisGroup.map (algebraMap ℚ
      (HeightOneSpectrum.adicCompletion ℚ hq.toHeightOneSpectrumRingOfIntegersRat))) σ ∈ N := by
    rw [← hfix, IntermediateField.mem_fixingSubgroup_iff]
    intro z hz
    have hlin : (((Field.absoluteGaloisGroup.map (algebraMap ℚ
        (HeightOneSpectrum.adicCompletion ℚ
          hq.toHeightOneSpectrumRingOfIntegersRat))) σ).toAlgHom.comp
        (IntermediateField.fixedField (E := AlgebraicClosure ℚ) N).val).toLinearMap =
        ((IntermediateField.fixedField (E := AlgebraicClosure ℚ) N).val).toLinearMap :=
      b.ext (fun i => hgfix i)
    exact DFunLike.congr_fun hlin ⟨z, hz⟩
  have h1 := (hmemN _).mp hgN
  convert h1 using 4
...
/-- **The LOCAL half of DDT Lemma 2.48** (cut out 2026-07-28; **PROVEN
2026-07-28**): at a prime `q`, membership of `Frob_q` in the surviving locus
of a cocycle representative implies that `c` does not die under the full
local restriction at `q`.

# HOW IT IS PROVEN, AND WHY IT IS CHEAPER THAN THE ROUTE FIRST RECORDED

The route recorded when this leaf was cut went through the procyclic
identification `H¹(G_q/I_q, M) ≅ M/(ρ Frob_q - 1)M`, using `hcunr` to see
the restriction of `c` at `q` as INFLATED from `G_q/I_q`.  That is the
right picture for the `↔`, and it is **not needed for the implication
actually asserted here**, which is the easy direction only.

The proof is instead this.  `c ∈ ker (locResDecompTwist1 … q)` says the
class of `z` dies under `ContinuousCohomology.map (decompHom q) (𝟙 _) 1`.
By `ContinuousCohomology.cocycleClass_eq_zero_iff` on the `G_q`-side, the
RESTRICTED cocycle `cochainsMap (decompHom q) (𝟙 _) z` is then a
COBOUNDARY `d y`; and a coboundary has

    eval₁ (d y) σ  =  y σ − y 1  =  ρ σ (y 1) − y 1

for EVERY `σ`, by degree-`0` homogeneity of `y` — one witness `y 1` serving
all `σ` at once.  Taking `σ = Frob_q ∈ G_q` and pushing forward along
`decompHom q` gives `z (globalFrob q) ∈ (ρ (globalFrob q) − 1) · M`, i.e.
`globalFrob q ∉ survivingLocus`, contradicting `hmem`.

So vanishing of the restriction to the WHOLE decomposition group already
produces the boundary witness on the nose; nothing has to be inflated back
from the quotient, and consequently **neither `hcunr` nor `q ∉ {2, p}` is
used** — they are kept in the signature (underscored) because the consumer
`exists_separatingOpen_locResDecomp` holds them anyway and because they are
part of the statement of DDT 2.48 as cited.  The leaf is therefore proven
in a strictly stronger form than stated.  It is emphatically NOT vacuous:
`hmem` is the whole content, and it is discharged by an explicit witness
extracted from the vanishing hypothesis.

The general machinery lives in
`Fermat/FLT/Mathlib/RepresentationTheory/Homological/ContCohomology/LowDegreeOne.lean`,
which acquired three lemmas for this: `homogeneousCochains_zero_apply_smul`
and `eval₁_mem_range_sub_of_mem_range_bdryKer` (the coboundary computation),
`cocycleClass_eq_homologyπ` (identifying the concrete `cocycleClass` of
`ContCohomology/Basic.lean` with the categorical `homologyπ`, which is what
makes `cocycleClass` compatible with `ContinuousCohomology.map`), and
`eval₁_mem_range_sub_of_map_cocycleClass_eq_zero` assembling them.

`globalFrob q = decompHom q (adicArithFrob q)` holds only up to the choice
of the ring map `ℚ →+* ℚ_q` inside each; the two are identified by
`Subsingleton.elim` on `ℚ →+* ℚ_q`, exactly as `MazurTorsion.lean` does.

Note that no local Tate duality is needed, in keeping with the deviation
note on `IsTaylorWilesPrimeSet`.

Both-ways audit and CIRCULARITY GUARD: as for the global half above — and
note the proof below touches neither `hirr` nor anything in `Family.lean`,
so the guard is discharged structurally rather than by discipline. -/
theorem notMem_ker_locResDecompTwist1_of_mem_survivingLocus.{uK, uW}
    ...
    {c : continuousCohomology 1 (adZeroTwist p ρbar)}
    (hzc : ContinuousCohomology.cocycleClass (adZeroTwist p ρbar) 1 z = c)
    (_hcunr : c ∈ h1TwistUnramified p ρbar)
    (q : ℕ) (hq : q.Prime) (_hq2 : q ≠ 2) (_hqp : q ≠ p)
  ...
  intro hker
  refine hmem ?_
  -- The class of `z` dies under the full local restriction at `q`.
  have hz0 : ContinuousCohomology.map
      (decompHom hq.toHeightOneSpectrumRingOfIntegersRat)
      (CategoryTheory.CategoryStruct.id
        (adZeroTwistDecomp p ρbar hq.toHeightOneSpectrumRingOfIntegersRat)) 1
      (ContinuousCohomology.cocycleClass (adZeroTwist p ρbar) 1 z) = 0 := by
    rw [hzc]
    exact LinearMap.mem_ker.mp hker
  -- `globalFrob q` is the image of the local arithmetic Frobenius under `decompHom q`;
  -- the ring map `ℚ →+* ℚ_q` inside each need not be syntactically the same one, but
  -- ring maps out of `ℚ` are unique.
  have hfrob : globalFrob hq.toHeightOneSpectrumRingOfIntegersRat =
      decompHom hq.toHeightOneSpectrumRingOfIntegersRat
  ...
  exact ContinuousCohomology.eval₁_mem_range_sub_of_map_cocycleClass_eq_zero
    (decompHom hq.toHeightOneSpectrumRingOfIntegersRat)
    (CategoryTheory.CategoryStruct.id
      (adZeroTwistDecomp p ρbar hq.toHeightOneSpectrumRingOfIntegersRat)) z hz0
...
/-! ### Complete Nakayama RELATIVE to the coefficient ring

The block below (2026-07-30) closes a gap in the route recorded on
`exists_auxDeformationPresSurjection` far below, which proposed to reach
`Λ_𝒪 = 𝒪[[x_1, …, x_q]] ↠ R_Q` by composing
`exists_ringHom_mvPowerSeries_of_isAdicComplete` with
`surjective_of_span_range_eq_maximalIdeal` above.  **That composition does not
type-check mathematically**, and the reason is worth recording because the route
otherwise looks complete:

`surjective_of_span_range_eq_maximalIdeal` asks for
`Ideal.span (Set.range t) = 𝔪_R` — `𝔪_R` generated by the `q` images of the
VARIABLES alone.  In the Taylor–Wiles setting that is FALSE, and it is false for
a reason that has nothing to do with Greenberg–Wiles: `Λ_𝒪`'s own maximal ideal
is `(𝔪_𝒪, X_1, …, X_q)`, so a surjection `Λ_𝒪 ↠ R_Q` only forces
`𝔪_{R_Q} = (t_1, …, t_q) + ι(𝔪_𝒪)·R_Q`.  Greenberg–Wiles delivers exactly the
RELATIVE bound `dim_k 𝔪/(𝔪² + ι(𝔪_𝒪)·R) = #Q = q`, and `𝔪_{R_Q}` genuinely
needs `q + 1` ideal generators whenever `ϖ ∉ 𝔪²` — which is the generic case,
`R_Q ≅ 𝒪[[x_1, …, x_q]]/I` with `I` in the square of the maximal ideal.  So
demanding the absolute form would have replaced an arithmetic leaf by a false
one.

`surjective_of_span_range_sup_map_eq_maximalIdeal` below is the relative form.
Its proof is `surjective_of_span_range_eq_maximalIdeal`'s verbatim, with the
ideal of variables `J₀ = (X_1, …, X_q)` replaced throughout by
`J = J₀ + 𝔪_𝒪·Λ_𝒪` — every step goes through unchanged EXCEPT the one place
where the original invoked mathlib's `MvPowerSeries.instIsAdicComplete`, which is
about `J₀` and says nothing about `J`.

The replacement for that step is the observation that makes the whole block
cheap, and it is `isPrecomplete_of_compactSpace`: **a compact Hausdorff
Noetherian topological ring is `I`-adically precomplete for EVERY ideal `I`.**
Ideals of such a ring are closed (a finitely generated ideal is the image of a
compact `𝒪^r` under a continuous map, hence compact, hence closed), so a
Cauchy sequence's cluster point — which compactness supplies — is an adic limit.
`TaylorWilesCoefficients` carries `CompactSpace`, `T2Space` and
`IsNoetherianRing`, and `MvPowerSeries (Fin q) 𝒪` inherits all three (the
pi-topology is a product topology, Tychonoff; Noetherianness is
`isNoetherianRing_mvPowerSeries`), so nothing had to be assumed that the bundle
does not already provide.  In particular NO adic completeness of `𝒪` is needed
anywhere — which matters, because `TaylorWilesCoefficients` does not carry it
and there is no lemma in this development deriving it. -/

/-- **Every ideal of a compact Hausdorff Noetherian topological ring is closed**
(PROVEN 2026-07-30).  `I` is finitely generated by Noetherianness, say by
`s = {a_1, …, a_r}`; it is then the range of the continuous map
`(g_1, …, g_r) ↦ ∑ g_i a_i` on the compact space `A^s`, hence compact, hence
closed in the Hausdorff `A`. -/
theorem isClosed_ideal_of_compactSpace {A : Type*} [CommRing A] [TopologicalSpace A]
    [IsTopologicalRing A] [CompactSpace A] [T2Space A] [IsNoetherianRing A]
    (I : Ideal A) : IsClosed (I : Set A) := by
  obtain ⟨s, hs⟩ := IsNoetherian.noetherian I
  have hrange : (I : Set A) =
      Set.range (fun g : {a // a ∈ s} → A => ∑ a : {a // a ∈ s}, g a * (a : A)) := by
    ext x
    simp only [SetLike.mem_coe, Set.mem_range]
    rw [← hs]
    rw [Submodule.mem_span_finset' (s := s) (x := x)]
    simp [smul_eq_mul]
  rw [hrange]
  refine (isCompact_range ?_).isClosed
  exact continuous_finsetSum _ fun a _ => (continuous_apply a).mul continuous_const

/-- **A compact Hausdorff Noetherian topological ring is `I`-adically
precomplete, for EVERY ideal `I`** (PROVEN 2026-07-30).

This is the step that makes the relative complete Nakayama below free.  Given a
Cauchy sequence `f` (`f m ≡ f n mod I^m` for `m ≤ n`), compactness supplies a
cluster point `L` of `f` along `atTop`; for each `n` the set
`{x | x - f n ∈ I^n}` is closed (`isClosed_ideal_of_compactSpace` pulled back
along `x ↦ x - f n`) and contains the tail `{f m | n ≤ m}`, so it contains `L`.

Note what is NOT assumed: no adic completeness, and no relation whatsoever
between the ring's topology and `I`.  Compactness does all the work, which is
why this applies to a `TaylorWilesCoefficients` carrier and to
`MvPowerSeries (Fin q) 𝒪` over one, for ideals having nothing to do with either
topology. -/
theorem isPrecomplete_of_compactSpace {A : Type*} [CommRing A] [TopologicalSpace A]
    [IsTopologicalRing A] [CompactSpace A] [T2Space A] [IsNoetherianRing A]
    (I : Ideal A) : IsPrecomplete I A := by
  constructor
  intro f hf
  obtain ⟨L, hL⟩ := exists_clusterPt_of_compactSpace (Filter.atTop.map f)
  refine ⟨L, fun n => ?_⟩
  rw [SModEq.sub_mem, Ideal.smul_eq_mul, Ideal.mul_top]
  have hclosed : IsClosed {x : A | x - f n ∈ I ^ n} :=
    (isClosed_ideal_of_compactSpace (I ^ n)).preimage (continuous_id.sub continuous_const)
  have hmem : {x : A | x - f n ∈ I ^ n} ∈ Filter.atTop.map f := by
    rw [Filter.mem_map]
    ...
    have := neg_mem h
    rwa [neg_sub] at this
  have hLmem : L ∈ {x : A | x - f n ∈ I ^ n} := by
    rw [← hclosed.closure_eq]
    exact mem_closure_iff_clusterPt.mpr (hL.mono (Filter.le_principal_iff.mpr hmem))
  have hLmem' : L - f n ∈ I ^ n := hLmem
  have := neg_mem hLmem'
  rwa [neg_sub] at this

open scoped MvPowerSeries.WithPiTopology in
/-- **`𝒪[[x_1, …, x_q]]` is `J`-adically precomplete for EVERY ideal `J`**
(PROVEN 2026-07-30), when `𝒪` is a compact Hausdorff Noetherian topological ring
— i.e. for every `TaylorWilesCoefficients` carrier.

The pi-topology on `MvPowerSeries (Fin q) 𝒪` IS the product topology on
`(Fin q →₀ ℕ) → 𝒪`, so compactness is Tychonoff and Hausdorffness is
`Pi.t2Space`; `IsNoetherianRing` is `isNoetherianRing_mvPowerSeries`.  The
CONCLUSION mentions no topology, so a consumer needs no `open scoped`. -/
theorem isPrecomplete_mvPowerSeries {O : Type*} [CommRing O] [IsNoetherianRing O]
    [TopologicalSpace O] [IsTopologicalRing O] [CompactSpace O] [T2Space O] {q : ℕ}
    (J : Ideal (MvPowerSeries (Fin q) O)) : IsPrecomplete J (MvPowerSeries (Fin q) O) := by
  haveI : CompactSpace (MvPowerSeries (Fin q) O) :=
    inferInstanceAs (CompactSpace ((Fin q →₀ ℕ) → O))
  haveI : IsNoetherianRing (MvPowerSeries (Fin q) O) := isNoetherianRing_mvPowerSeries q
  exact isPrecomplete_of_compactSpace J

/-- **Complete Nakayama RELATIVE to the coefficient ring** (PROVEN 2026-07-30):
a ring homomorphism `φ : 𝒪[[x_1, …, x_q]] →+* R` into an `𝔪_R`-adically complete
local ring is SURJECTIVE as soon as

* `ι = φ ∘ C` hits the residue field `k = R/𝔪_R`, and
* `𝔪_R` is generated by the images `t_i = φ (X_i)` **together with** `ι(𝔪_𝒪)`.

Contrast `surjective_of_span_range_eq_maximalIdeal` above, which asks for
`Ideal.span (Set.range t) = 𝔪_R` — the `t_i` ALONE.  That absolute form is what
Cohen's theorem delivers when `q` is allowed to be the minimal number of
generators of `𝔪_R`, and it is the wrong hypothesis whenever `q` is pinned from
outside (as `q = #Q` is in the Taylor–Wiles presentation), because
`𝔪_{𝒪[[x]]} = (𝔪_𝒪, X_1, …, X_q)` needs `q + 1` generators and a surjection can
only make `𝔪_R` a quotient of that.  See the section header above for why using
the absolute form there would have planted a false leaf.

The proof is the sibling's verbatim with `J₀ = (X_1, …, X_q)` replaced by
`J = J₀ + 𝔪_𝒪·Λ`; the only step that is not literally the same is the limit,
where mathlib's `MvPowerSeries.instIsAdicComplete` (a statement about `J₀`) is
replaced by `isPrecomplete_mvPowerSeries` (a statement about EVERY ideal, from
compactness).  The topological hypotheses on `𝒪` are exactly the fields
`TaylorWilesCoefficients` already carries; no adic completeness of `𝒪` is used.

FAITHFULNESS.  Every hypothesis is consumed: `hπ` and `hι` only through "`ι`
hits `R/𝔪_R`", `hspan` through `Ideal.map φ J = 𝔪_R`, `hcomplete` through the
approximation limit and `IsHausdorff` at the end, and the four topological
hypotheses on `𝒪` through `isPrecomplete_mvPowerSeries`.  `[IsLocalRing O]` is
used only to name `𝔪_𝒪`. -/
theorem surjective_of_span_range_sup_map_eq_maximalIdeal
    {R : Type*} [CommRing R] [IsLocalRing R]
    (hcomplete : IsAdicComplete (IsLocalRing.maximalIdeal R) R)
    {k : Type*} [Field k] {π : R →+* k} (hπ : Function.Surjective π)
    {O : Type*} [CommRing O] [IsLocalRing O] [IsNoetherianRing O]
    [TopologicalSpace O] [IsTopologicalRing O] [CompactSpace O] [T2Space O]
    {ι : O →+* R} (hι : Function.Surjective (π.comp ι))
    {q : ℕ} {t : Fin q → R}
    (hspan : Ideal.span (Set.range t) ⊔ (IsLocalRing.maximalIdeal O).map ι =
    ...
    Function.Surjective φ := by
  classical
  set 𝔪 : Ideal R := _root_.IsLocalRing.maximalIdeal R with h𝔪
  set J : Ideal (MvPowerSeries (Fin q) O) :=
    Ideal.span (Set.range (MvPowerSeries.X : Fin q → MvPowerSeries (Fin q) O)) ⊔
      (IsLocalRing.maximalIdeal O).map MvPowerSeries.C with hJ
  have hφC : φ.comp (MvPowerSeries.C : O →+* MvPowerSeries (Fin q) O) = ι :=
    RingHom.ext hC
  -- `φ` carries the maximal ideal of `Λ` onto `𝔪_R`; this is "continuity is
  -- automatic" in the form the proof uses, now including the `𝒪`-part.
  have hmapJ : Ideal.map φ J = 𝔪 := by
    rw [hJ, Ideal.map_sup, Ideal.map_span, ← Set.range_comp, Ideal.map_map, hφC]
    simp only [Function.comp_def, hX]
    exact hspan
  have hmapJn : ∀ n : ℕ, Ideal.map φ (J ^ n) = 𝔪 ^ n := fun n => by
    rw [Ideal.map_pow, hmapJ]
  ...
  have approx : ∀ (n : ℕ) (x : R), x ∈ 𝔪 ^ n →
    ...
    · rintro c y - ⟨g, hg, h⟩
      ...
      exact Ideal.mul_mem_mul hφg hca
  intro r
  -- (b) THE LIMIT: iterate (a), then use precompleteness of `𝒪[[x_1,…,x_q]]`
  -- at `J`, which is `isPrecomplete_mvPowerSeries` rather than mathlib's
  -- instance for the ideal of variables.
  let A : ℕ → R → MvPowerSeries (Fin q) O := fun n x =>
    if h : x ∈ 𝔪 ^ n then (approx n x h).choose else 0
  have hA1 : ∀ (n : ℕ) (x : R) (h : x ∈ 𝔪 ^ n), A n x ∈ J ^ n := by
    intro n x h; simp only [A, dif_pos h]; exact (approx n x h).choose_spec.1
  ...
  have hmono : ∀ m n, m ≤ n → (P n).1 - (P m).1 ∈ J ^ m := by
    ...
    | succ n hn ih =>
      ...
      have hrw : (P (n + 1)).1 - (P m).1
          = ((P (n + 1)).1 - (P n).1) + ((P n).1 - (P m).1) := by ring
      rw [hrw]; exact add_mem h1 ih
  have hprec : IsPrecomplete J (MvPowerSeries (Fin q) O) := isPrecomplete_mvPowerSeries J
  obtain ⟨L, hL⟩ := hprec.prec (f := fun n => (P n).1) (by
    intro m n hmn
    rw [SModEq.sub_mem, Ideal.smul_eq_mul, Ideal.mul_top]
    have := neg_mem (hmono m n hmn)
...
theorem exists_taylorWilesCoefficientsPresentation.{u, v}
    ...
    ∃ (q : ℕ) (coeff : TaylorWilesCoefficients)
      ...
      q₀ ≤ q ∧ Function.Surjective φ ∧ IsCohenCoefficients.{u, v} coeff k := by
  obtain ⟨coeff, hcohen, ι, hι⟩ :=
    exists_taylorWilesCoefficients_ringHom.{u, v} hcomplete hπ
  ...
  exact ⟨q, coeff, φ, hq,
    surjective_of_span_range_eq_maximalIdeal hcomplete hπ hι hspan hC hX, hcohen⟩
...
theorem exists_taylorWilesBottomPresentation.{s, t, uK, uW, uR}
    ...
    ∃ (q : ℕ), q0 ≤ q ∧
      ...
      Function.Surjective pres ∧ Nonempty (R ≃+* Runiv) ∧
      IsCohenCoefficients.{uR, uK} coeff k := by
  ...
  obtain ⟨q, coeff, φ, hq₀, hφ, hcohen⟩ :=
    exists_taylorWilesCoefficientsPresentation.{uR, uK} hcomplete hπuniv q0
  ...
  refine ⟨q, hq₀, coeff, MvPowerSeries (Fin q) coeff.carrier ⧸ RingHom.ker φ,
    inferInstance, Ideal.Quotient.mk _, Ideal.Quotient.mk_surjective, ?_, hcohen⟩
...
at an ARBITRARY exponent vector `e : Fin q → ℕ`:

    (Λ/𝔟_e)^d ,   Λ = ℤ_p[[S_1, …, S_q]] ,
    𝔟_e = ((1 + S_i)^{p^{e_i}} − 1)_{i} ,

so that `Λ/𝔟_e ≅ ℤ_p[∏_i ℤ/p^{e_i}] = ℤ_p[Δ_Q]` — the group ring of the
diamond group at its OWN exponents `e_i = v_p(q_i − 1)`.

**GENERALISED FROM THE CONSTANT VECTOR 2026-07-30.**  Until then this was
pinned at `e ≡ n`, and that pin is exactly what made
`exists_auxDeformationDiamondControl` FALSE and
`exists_auxHeckeCoordModuleData` unsatisfiable — see the 2026-07-30
FALSITY AUDIT on the former, and FAITHFULNESS AUDIT #2 on the latter,
which prescribed this generalisation.  `hQ` asks its prime set at level
`n + 1`, so `e_i ≥ n + 1 > n` and `𝔟_e ⊊ 𝔟_{(n)}`: the constant vector
truncates a group the construction does not truncate.
...
Steps 2-4 were factored out on 2026-07-29 (flt-lean-38) into
`charFrob_map_eq_of_charFrobCoeffOne_eq_away` and
`exists_distinct_charFrob_map_eq_of_isTaylorWilesPrimeSet` above, stated for an
arbitrary free rank-two module rather than for `Fin 2 → R`, because the
fibre-product clause needs exactly the same four steps for an abstract `V`.
Only step 1 — the continuity of `πR`, which is `hadic` here and is what the
fibre-product clause does NOT have — is left inline.
...
theorem raisedLevelIsSplitTorusAt_of_forall_isOpen_quotient.{a, uK, uW} {p : ℕ}
  ...
  have hπcont : Continuous πR := by
    ...
    exact hxk
  -- Steps 2-5: `2 ≠ 0` from `hpodd`, the trace-only Chebotarev-Brauer-Nesbitt
  -- bridge, and `hQ`'s local clause -- all packaged in
  -- `exists_distinct_charFrob_map_eq_of_isTaylorWilesPrimeSet` above, which is
...
characteristic polynomial of Frobenius of a framed representation pushed
forward along a continuous ring homomorphism is the image of the original one
under that homomorphism.

`pushforwardFrame` is base change along `ψ.toAlgebra` followed by the standard
identification `A ⊗_B (Fin 2 → B) ≅ (Fin 2 → A)`, so this is
`charFrob_conj` (conjugation does not move `charFrob`) followed by
`charFrob_baseChange` (base change maps the coefficients along `algebraMap`,
which for `ψ.toAlgebra` IS `ψ`).  The `ℚ`-side twin of the Hilbert file's
`charpoly_framePushforward`, in the linear-coefficient shape this file's
residual-compatibility clauses use. -/
lemma charFrob_pushforwardFrame.{u} {B : Type u} [CommRing B]
    [TopologicalSpace B] [IsTopologicalRing B] {A : Type u} [CommRing A]
    [TopologicalSpace A] [IsTopologicalRing A] (ψ : B →+* A)
    (hψ : Continuous ψ) (ρ : GaloisRep ℚ B (Fin 2 → B))
    (v : HeightOneSpectrum (NumberField.RingOfIntegers ℚ)) :
    (pushforwardFrame ψ hψ ρ).charFrob v = (ρ.charFrob v).map ψ := by
  letI : Algebra B A := ψ.toAlgebra
  letI : ContinuousSMul B A := continuousSMul_of_algebraMap B A
    (by rw [RingHom.algebraMap_toAlgebra]; exact hψ)
  show ((ρ.baseChange A).conj
    (TensorProduct.piScalarRight B A A (Fin 2))).charFrob v = _
  rw [charFrob_conj, charFrob_baseChange, RingHom.algebraMap_toAlgebra]

set_option linter.checkUnivs false in
/-- **Weak universality of a raw package on FINITE RAISED-LEVEL frames**
(new 2026-07-28, with the machine cut below): the `ℚ`-side twin of
`IsHilbertAuxWeaklyUniversalOnFiniteFrames`, on the raw data
`(R, ρuniv, πuniv, Suniv)` rather than on a bundled datum.

Every FINITE DISCRETE raised-level framed test object `(A, ρA, πA, SA)` — the
objects of Schlessinger's category, on which the deformation functor is defined
pointwise — is classified by a continuous `ℤ_p`-algebra map `ψ : R →+* A`
compatible with the reductions, along which the linear `charFrob` coefficients
of `ρuniv` become those of `ρA`.

**Why the trace clause carries `Suniv` and `SA` rather than holding at every
prime.**  The whole `ℚ`-side chain of this file carries residual data only up
to a FINITE exceptional set — that is `AuxDeformationDatum.charFrob_compat`'s
shape, and the reason for it is recorded on `AuxDeformationDatum` itself.  What
matters for the limit passage below is that the exceptional set of the
classifying map be **uniform in the test object's level**: it is bounded by
`Suniv ∪ SA`, where `Suniv` is fixed with the package and `SA` is the test
object's own set.  A per-level exceptional set would be useless — the inverse
limit runs over infinitely many levels and no finite set could absorb their
union.  (The two conditions are written as two arrows rather than as a union
because `Finset` union needs `DecidableEq` on the height-one spectrum, which is
not an instance here.)

The Hilbert twin asks the stronger `((ρA g).charpoly).map πA = (ρbar g).charpoly`
at every `g`; that shape is not available on this side, for the reason recorded
in `AuxDeformationDatum`'s docstring. -/
def IsAuxWeaklyUniversalOnFrames.{a, vK, vW} {p : ℕ} (hpodd : Odd p)
    [Fact p.Prime] (Q : Finset ℕ)
  ...
  ∀ (A : Type a) [CommRing A] [TopologicalSpace A] [IsTopologicalRing A]
    ...
    ∀ (πA : A →+* k), Function.Surjective πA →
    ∀ (SA : Finset (HeightOneSpectrum (NumberField.RingOfIntegers ℚ))),
    (∀ (q : ℕ) (hq : q.Prime),
      hq.toHeightOneSpectrumRingOfIntegersRat ∉ SA →
      πA ((ρA.charFrob hq.toHeightOneSpectrumRingOfIntegersRat).coeff 1) =
        (ρbar.charFrob hq.toHeightOneSpectrumRingOfIntegersRat).coeff 1) →
    ∃ ψ : R →+* A, Continuous ψ ∧
      ψ.comp (algebraMap ℤ_[p] R) = algebraMap ℤ_[p] A ∧
      πA.comp ψ = πuniv ∧
      ∀ (q : ℕ) (hq : q.Prime),
        hq.toHeightOneSpectrumRingOfIntegersRat ∉ Suniv →
        hq.toHeightOneSpectrumRingOfIntegersRat ∉ SA →
...
set_option linter.checkUnivs false in
/-- **The diamond operators and the control map on `R_Q`** (**PROVEN GLUE since
2026-07-30** over `exists_auxDeformationDiamondControlCompat` above and
`surjective_of_isTraceGeneratedDeformation`; formerly LEAF
...
1. **`diamond`**, from `Λ = ℤ_p[[S_1, …, S_q]]`.  The split-torus clause of
   ...
   `e_i = v_p(q_i − 1) ≥ n`.  **That vector `e` is RETURNED by this leaf**
   (since the 2026-07-30 FALSITY AUDIT #2 below; it used to be pinned at the
   constant `fun _ => n`, which is what made the statement false).  The clause
   `taylorWilesLevelIdeal p e ≤ RingHom.ker diamond` says `diamond` factors
   through `Λ ⧸ 𝔟_{(e)} = ℤ_p[Δ_Q]`, i.e. that the diamonds have exactly the
   orders `Δ_Q` gives them; and `he : ∀ i, n ≤ e i` is precisely the congruence
   clause of `hQ`.
...
theorem exists_auxDeformationDiamondControl.{s, t, uK, uW, uR}
  ...
  classical
  -- **STEP 1** — the level-`Q` datum carried by `(Runiv, ρuniv, πuniv)` itself,
  -- WITH its carrier named (the 2026-07-30 strengthening of
  -- `exists_auxDeformationDatum`, whose only consumer this is).
  obtain ⟨𝒟, ee, halg, hπ, hcF⟩ :=
    exists_auxDeformationDatum.{uK, uW, uR} hpodd hW hres hirr hadic hcomplete
      hranku hρuniv hπuniv hunivred (n + 1) (Nat.succ_le_succ (Nat.zero_le n)) Q
      hQ
  -- **STEP 2** — weak universality of `𝒟Q` classifies it: `f : 𝒟Q.R →+* 𝒟.R`.
  obtain ⟨f, hfalg, hfπ, Sf, hfF⟩ := h𝒟Q 𝒟
  -- **STEP 3** — compose to land in `Runiv`; all three compatibilities survive.
  set toRuniv₀ : 𝒟Q.R →+* Runiv := ee.toRingHom.comp f with htoR₀
  have h₀alg : toRuniv₀.comp (algebraMap ℤ_[p] 𝒟Q.R) =
      algebraMap ℤ_[p] Runiv := by
    rw [htoR₀, RingHom.comp_assoc, hfalg, halg]
  have h₀π : πuniv.comp toRuniv₀ = 𝒟Q.π := by
    rw [htoR₀, ← RingHom.comp_assoc, hπ, hfπ]
  have h₀F : ∀ (r : ℕ) (hr : r.Prime),
      hr.toHeightOneSpectrumRingOfIntegersRat ∉ Sf →
      toRuniv₀ ((𝒟Q.ρ.charFrob
          hr.toHeightOneSpectrumRingOfIntegersRat).coeff 1) =
        (ρuniv.charFrob hr.toHeightOneSpectrumRingOfIntegersRat).coeff 1 := by
    intro r hr hrS
    have h1 := hfF r hr hrS
    have h3 : ee ((𝒟.ρ.charFrob
        hr.toHeightOneSpectrumRingOfIntegersRat).coeff 1) =
        (ρuniv.charFrob hr.toHeightOneSpectrumRingOfIntegersRat).coeff 1 := by
      rw [← hcF hr.toHeightOneSpectrumRingOfIntegersRat, Polynomial.coeff_map]
      rfl
    rw [htoR₀]
    simp only [RingHom.coe_comp, Function.comp_apply, RingEquiv.toRingHom_eq_coe,
      RingHom.coe_coe]
    rw [h1, h3]
  -- **STEP 4** — the sub-leaf: the exponent vector, the diamonds and the
  -- control kernel identity.
  obtain ⟨e, diamond, toRuniv, he, halg', hπ', ⟨Sf', hF'⟩, hker, hbn⟩ :=
    exists_auxDeformationDiamondControlCompat.{s, t, uK, uW, uR} hpodd hW hres
      hirr hadic hcomplete hranku hρuniv hπuniv hunivred hfact hgen q n Q hQcard
      hQ 𝒟Q h𝒟Q toRuniv₀ h₀alg h₀π h₀F
  -- **STEP 5** — Carayol surjectivity of the control map.
  refine ⟨e, diamond, toRuniv, he, ?_, hker, hbn⟩
  refine surjective_of_isTraceGeneratedDeformation.{uR, uK, uR}
    𝒟Q.isAdic 𝒟Q.isAdicComplete hadic hcomplete 𝒟Q.π_surjective hπuniv hgen
    toRuniv halg' hπ' (Sf := Sf') ?_
  intro r hr hrS
  exact ⟨(𝒟Q.ρ.charFrob hr.toHeightOneSpectrumRingOfIntegersRat).coeff 1,
    hF' r hr hrS⟩
...
2. **`diamond`**, from `Λ = ℤ_p[[S_1, …, S_q]]`.  This is local class field
   ...
   whose restriction to `I_{q_i}` factors through the tame quotient
   `(ℤ/q_i)ˣ`, whose `p`-Sylow is `Δ_{q_i} ≅ ℤ/p^{e_i}` with
   `e_i = v_p(q_i − 1) ≥ n`.  That vector is RETURNED alongside `diamond`
   (2026-07-30), and the clause `taylorWilesLevelIdeal p e ≤ RingHom.ker
   diamond` says `diamond` factors through `Λ ⧸ 𝔟_{(e)} = ℤ_p[Δ_Q]`, i.e. that
   the diamonds have exactly the orders `Δ_Q` gives them; `∀ i, n ≤ e i` is
   precisely the congruence clause of `hQ`.  The constant vector `fun _ => n`
   stood here until 2026-07-30 and was REFUTED — see FALSITY AUDIT #2 on
   `exists_auxDeformationDiamondControl` above, which owns this clause;
...
theorem exists_auxDeformationRingPresentation.{s, t, uK, uW, uR}
  ...
  -- LEAF A2′-3b (local CFT + control): the diamonds, their EXPONENT VECTOR and
  -- `R_Q ↠ R_univ` — `e` is unpinned since the 2026-07-30 FALSITY AUDIT #2 …
  ...
  refine ⟨e, RingHom.ker pres, pres.quotientKerEquivOfSurjective hpres,
    ...
    toRuniv.comp (pres.quotientKerEquivOfSurjective hpres).toRingHom, he, ?_, ?_,
    ?_⟩
...
which is what makes step 3 the whole content and why the exponent vector is
free.  **That is why unpinning it cost nothing** (2026-07-30): this statement
took a level `n` and used `𝔟_{(n)}` until the constant vector was refuted on the
ring side, and generalising it to an arbitrary `e : Fin q → ℕ` was the single
substitution at step 4, `key` having always been stated for every `a ≤ 𝔫`.
...
theorem nonempty_augQuotEquiv_of_taylorWilesBottom.{s, uR}
    ...
    (e : Fin q → ℕ) :
    Nonempty ((taylorWilesCoordModelAt p d e ⧸
        (taylorWilesAug p q • ⊤ :
          Submodule (MvPowerSeries (Fin q) ℤ_[p])
            (taylorWilesCoordModelAt p d e))) ≃+ M0) := by
  classical
  obtain ⟨L⟩ := hbot
  letI := L.commRingR
  ...
  have hmap : (taylorWilesAug p q • ⊤ :
        ...
        (eqM : L.M →ₗ[MvPowerSeries (Fin q) ℤ_[p]]
    ...
    congr 1
    exact LinearMap.range_eq_top.mpr eqM.surjective
  have hbotEquiv : Nonempty ((L.M ⧸ (taylorWilesAug p q • ⊤ :
        Submodule (MvPowerSeries (Fin q) ℤ_[p]) L.M))
      ≃ₗ[MvPowerSeries (Fin q) ℤ_[p]]
        (Fin d → MvPowerSeries (Fin q) ℤ_[p] ⧸ taylorWilesAug p q)) :=
    ⟨(Submodule.Quotient.equiv _ _ eqM hmap).trans g0⟩
...
# AUDIT #2's FIRST REPAIR IS DONE (2026-07-30): THE EXPONENT VECTOR IS UNPINNED

# — DEFECT (2) IS GONE AND THE PACKAGE NOW HAS THE INTENDED WITNESS

The "Necessary, and cheap" bullet above has been carried out, and the reason it
was done now rather than later is that the SAME defect was found to make the
RING leaf `exists_auxDeformationDiamondControl` outright FALSE, not merely
unsatisfiable — see its FALSITY AUDIT #2 (2026-07-30), which refutes the
constant vector with an explicit valuation witness in `ℤ_p[ℤ/p^e]`.  One repair
answers both, exactly as this audit predicted.

Concretely, in this statement:

* `e : Fin q → ℕ` with `he : ∀ i, n ≤ e i` are now HYPOTHESES, threaded from
  the ring leaf, which produces them (classically `e_i = v_p(q_i − 1)`);
* `hbn` reads `taylorWilesLevelIdeal p e ≤ RingHom.ker diamond`;
* the module lives on `taylorWilesCoordModelAt p d e = (Λ ⧸ 𝔟_{(e)})^d`.

**What this fixes.**  Defect (2) is gone outright: `𝔟_{(e)} ≤ ker diamond` is
what the `Δ_Q`-action genuinely satisfies, so `hbn` is no longer a clause no
witness can meet.  And (★) becomes `RingHom.ker diamond = 𝔟_{(e)}` — which is
TRUE of the intended witness and is precisely Diamond's freeness (`Λ` acts on
`M_Q` through `diamond`, and a free `Λ/𝔟_{(e)}`-module has annihilator exactly
`𝔟_{(e)}`).  So the whole package — hypotheses AND conclusion — is now satisfied
by the object the classical construction produces, which it was not before.  The
`sorry` below is therefore VOUCHED as true at the intended witness.

The algebraic half cost nothing, as predicted:
`nonempty_augQuotEquiv_of_taylorWilesBottom` above now takes `e` instead of `n`
and its proof is unchanged apart from that substitution, because its `key` step
was already stated for EVERY ideal `a ≤ 𝔫`.

**What this does NOT fix, and it is still the honest reason this leaf is hard.**
Defect (1) survives in weakened form.  The conclusion still asserts a property
of a `diamond` this leaf did not make, so a prover who builds the honest `M_Q`
must still show the handed-in `diamond` is the one its `Λ`-action goes through;
`hbn` plus the `ker_le` computation pin `ker diamond = 𝔟_{(e)}`, which pins the
kernel but not the map.  That is what `𝒟Q`, `h𝒟Q` and `φ` are in the hypothesis
package for, and it is why the "Correct" bullet above — PRODUCE `diamond` where
the freeness is proven, i.e. state the `ℚ` analogue of `HilbertAuxHeckeAlgebra`
— remains the right long-term cut.  The difference is that this is now a
...
theorem exists_auxHeckeCoordModuleData.{s, t, uK, uW, uR}
    ...
    (hθ : Nonempty ((taylorWilesCoordModelAt p d e ⧸
        (taylorWilesAug p q • ⊤ :
          Submodule (MvPowerSeries (Fin q) ℤ_[p])
            (taylorWilesCoordModelAt p d e))) ≃+ M0)) :
    ∃ (_ : Module (MvPowerSeries (Fin q) coeff.carrier ⧸ I)
        (taylorWilesCoordModelAt p d e))
      (θ : (taylorWilesCoordModelAt p d e ⧸
        (taylorWilesAug p q • ⊤ :
          Submodule (MvPowerSeries (Fin q) ℤ_[p])
            (taylorWilesCoordModelAt p d e))) ≃+ M0),
      (∀ (x : MvPowerSeries (Fin q) ℤ_[p])
        (m : taylorWilesCoordModelAt p d e), x • m = diamond x • m) ∧
      (∀ (x : MvPowerSeries (Fin q) coeff.carrier ⧸ I)
        (m : taylorWilesCoordModelAt p d e),
...
**BUT THE CUT ITSELF WAS DEFECTIVE — read FAITHFULNESS AUDIT #2 on
...
`RingHom.ker diamond = taylorWilesLevelIdeal p e` exactly, which `hbn` bounds
only from below.  With the CONSTANT vector `e ≡ n` that was a clause no intended
witness satisfied, since `hQ` forces the `Δ_Q` exponent to be at least
`p^(n+1)`.

**HALF-REPAIRED 2026-07-30:** the exponent vector is unpinned throughout this
chain — `e` and `he : ∀ i, n ≤ e i` are now threaded from the RING leaf, which
produces them — so `ker diamond = 𝔟_{(e)}` is TRUE of the intended witness and
the package has a model.  The same repair fixed an outright FALSITY on the ring
side (`exists_auxDeformationDiamondControl`'s FALSITY AUDIT #2, with a valuation
counterexample in `ℤ_p[ℤ/p^e]`).  What survives is the weaker half of defect (1):
the conclusion is still a statement about a `diamond` the sub-leaf did not make.
See that sub-leaf's docstring for what is left.
...
theorem exists_auxHeckeModuleData.{s, t, uK, uW, uR}
    ...
    ∃ (_ : Module (MvPowerSeries (Fin q) coeff.carrier ⧸ I)
        (taylorWilesCoordModelAt p d e))
      (projM : taylorWilesCoordModelAt p d e →+ M0),
      (∀ (x : MvPowerSeries (Fin q) ℤ_[p])
        (m : taylorWilesCoordModelAt p d e), x • m = diamond x • m) ∧
      ...
      (∀ m : taylorWilesCoordModelAt p d e, projM m = 0 →
        m ∈ (taylorWilesAug p q • ⊤ :
          Submodule (MvPowerSeries (Fin q) ℤ_[p])
            (taylorWilesCoordModelAt p d e))) := by
  ...
  obtain ⟨inst, θ, hlam, hint⟩ :=
    exists_auxHeckeCoordModuleData.{s, t, uK, uW, uR} hpodd hW hres hirr hadic
      ...
      q0 q d hq0 coeff M0 hM0 hbot hM0T n Q hQcard hQ 𝒟Q h𝒟Q e he I φ diamond
  ...
  refine ⟨inst, θ.toAddMonoidHom.comp
      (Submodule.mkQ (taylorWilesAug p q • ⊤ :
        Submodule (MvPowerSeries (Fin q) ℤ_[p])
          (taylorWilesCoordModelAt p d e))).toAddMonoidHom,
    hlam, ?_, ?_, ?_⟩
  ...
  · intro m hm
    have h0 : (Submodule.Quotient.mk m :
        taylorWilesCoordModelAt p d e ⧸ (taylorWilesAug p q • ⊤ :
          Submodule (MvPowerSeries (Fin q) ℤ_[p])
            (taylorWilesCoordModelAt p d e))) = 0 := by
      apply θ.injective
      simpa using hm
...
theorem exists_taylorWilesAuxLevelPresentedDatum.{s, t, uK, uW, uR}
  ...
  -- LEAF A2′-3 (RING): the presentation of `R_Q`, the diamonds with their
  -- EXPONENT VECTOR `e` (unpinned 2026-07-30 — see that leaf's FALSITY AUDIT
  -- #2, which refutes the constant vector), and the control map
...
**Four until 2026-07-30**, when the fourth — the exponent vector, which
this proof used to discharge by substituting the constant `fun _ => n` —
moved BACK up the chain.  It is now produced by
`exists_auxDeformationDiamondControl` and merely relayed here, because
taking it constant made that leaf FALSE (its FALSITY AUDIT #2 gives the
refuting valuation witness in `ℤ_p[ℤ/p^e]`).  The exponent existential in
the conclusion below is unchanged and was always the right shape; only
its witness changed, from `fun _ => n` to the vector the ring leaf hands
over.
...
theorem exists_taylorWilesAuxLevelData.{s, t, uK, uW, uR}
  ...
  obtain ⟨e, I, diamond, toRuniv, actR, projM, he, htoR, hker, hlam, hsurj,
    ...
    exists_taylorWilesAuxLevelPresentedDatum.{s, t, uK, uW, uR} hpodd hp5 hW hres
      hirr hadic hcomplete hranku hρuniv hπuniv hunivred hfact hgen hrankT hρT hπ
      hred ψ hψalg hψπ hψ q0 hTWq q d hq0 coeff hcohen M0 hM0 hbot hM0T n
  -- The exponent vector is the one the RING leaf produced (`e i = v_p(q_i − 1)`
  -- classically) — it is no longer taken constant, see the 2026-07-30 FALSITY
  -- AUDIT #2 of `exists_auxDeformationDiamondControl`.  `pres` is the quotient
  -- map and `coordM` is the identity, both free at the presented shape.
  exact ⟨e, he,
    MvPowerSeries (Fin q) coeff.carrier ⧸ I, inferInstance,
    Ideal.Quotient.mk I, diamond, toRuniv,
    taylorWilesCoordModelAt p d e, inferInstance, actR, inferInstance, projM,
...
theorem exists_taylorWilesBottomLevel.{s, t, uK, uW, uR}
    ...
    ∃ (q d : ℕ) (coeff : TaylorWilesCoefficients) (M0 : Type)
      ...
      q0 ≤ q ∧
      IsCohenCoefficients.{uR, uK} coeff k ∧
      Nonempty (M0 ≃ₗ[T] (Fin 2 → T)) ∧
  ...
  obtain ⟨q, hq0q, coeff, R, instR, pres, hpres, ⟨eR⟩, hcohen⟩ :=
  ...
  refine ⟨q, d, coeff, M0, instM0add, instM0T, instM0nt, hq0q, hcohen, hM0T, ⟨?_⟩⟩
  refine { R := R
           commRingR := instR
           pres := pres
...
theorem exists_taylorWilesLevelRaw.{s, t, uK, uW, uR}
  ...
  obtain ⟨e, he, R, instR, pres, diamond, toRuniv, M, instMadd, instMR, instML,
    ...
    exists_taylorWilesAuxLevelData.{s, t, uK, uW, uR} hpodd hp5 hW hres hirr hadic
      hcomplete hranku hρuniv hπuniv hunivred hfact hgen hrankT hρT hπ hred ψ hψalg
      hψπ hψ q0 hTWq q d hq0 coeff hcohen M0 hM0 hbot hM0T n
...
theorem exists_taylorWilesTower.{s, t, uK, uW, uR}
  ...
  obtain ⟨q, d, coeff, M0, iAG, iMod, iNt, hq0, hcohen, hM0T, hbot⟩ :=
           ...
           level := fun n =>
             (nonempty_taylorWilesLevel_of_raw iNt
               (exists_taylorWilesLevelRaw.{s, t, uK, uW, uR} hpodd hp5 hW hres
                 hirr hadic hcomplete hranku hρuniv hπuniv hunivred hfact hgen
                 hrankT hρT hπ hred ψ hψalg hψπ hψ q0 hTWq q d hq0 coeff hcohen M0
...
theorem exists_taylorWilesSystem.{s, t, uK, uW, uR}
  ...
  obtain ⟨tw⟩ := exists_taylorWilesTower.{s, t, uK, uW, uR} hpodd hp5 hW hρbar hirr
    hadic hcomplete hranku hρuniv hπuniv hunivred hfact hgen hrankT hρT hπ hred
    ψ hψalg hψπ hψ q0 hTWq

...
## The second relocation (2026-08-01), and why the first one needed it

The 2026-07-31 hoist above moved the LEAF `nonempty_cubeModel_of_isAmpleSheaf_cube`
and left behind, in `X0.lean`, the CUT that had been made out of it earlier the same
day: the section-evaluation apparatus (`ptSectionValue` and friends), `SpansSquare`,
and the two halves `exists_veryAmpleSystem_of_isAmpleSheaf` and
`exists_cubeForms_of_veryAmpleSystem`.  Since `X0.lean` `public import`s this module,
those halves came to sit DOWNSTREAM of the declaration they were cut out of, where
Lean's declaration order makes them uncitable — so all three were consumerless, and
the leaf they were built to close stayed open.

That block has now been moved here too, verbatim (only `SpecQ` written out), above
the leaf, and the leaf is PROVEN over it.  Frontier: three open leaves become two.

**The lesson, which is the general form of the trap this module already exists to
record: a hoist chosen to break an import cycle must carry the cut as well as the
leaf.**  A block relocation is checked for what the moved code REFERENCES; nobody
checks what still references the position it left, and a cut is a dependency in the
opposite direction — the leaf needs the halves, so moving the leaf UP strands them.
The symptom is silent: every declaration compiles, every leaf emits an honest
`declaration uses 'sorry'`, and the only trace is that the halves have no consumer.
-/
...
/-!
Everything below was MOVED here verbatim from `Fermat/FLT/ModularCurve/X0.lean`, in
two steps, to break the import cycle described at the top of this file.  Only the
STATEMENTS changed, and only by writing `SpecQ` out as `Spec (CommRingCat.of ℚ)` --
that `abbrev` is declared in `X0.lean` and so is not available upstream of it.
`SpecQ` is reducible, so every call site in `X0.lean` elaborates against these
unchanged.  The docstrings are the originals and their audit history is unaltered
except where a claim was falsified by the move itself, which is marked in place.

* 2026-07-31 (X0.lean lines 43692-44001): the two bottom declarations,
  `nonempty_cubeModel_of_isAmpleSheaf_cube` and `exists_cubeModel_of_abelianScheme`.
* 2026-08-01 (X0.lean lines 46459-46743): the section-evaluation apparatus
  (`nonvanishingAt_modPullbackSection`, `ratOfSpecQ`, `nonvanishingAt_modUnit_specQ`,
  `nonvanishingAt_iff_ratOfSpecQ`, `ptSectionValue`, `ptSectionValue_ne_zero_iff`),
  `SpansSquare`, and the two halves `exists_veryAmpleSystem_of_isAmpleSheaf` and
  `exists_cubeForms_of_veryAmpleSystem` -- which the first step had stranded
  downstream of the leaf they were cut out of.  See the module docstring.

`nonvanishingAt_modPullbackSection` is stated for an arbitrary morphism of schemes
and has nothing to do with the base; its own docstring records that it BELONGS in
`Modularity/AmpleSheaf.lean`, and that is still the right home for it.  It is here
only because editing that file re-elaborates a very large cone; hoist it the next
time `AmpleSheaf.lean` is touched for another reason.
-/
...
has nothing to do with `(Spec (CommRingCat.of ℚ))`; it BELONGS in `Modularity/AmpleSheaf.lean` beside
...
theorem nonvanishingAt_modUnit_specQ (r : Γ(SpecQ, ⊤)) (x : SpecQ) :
    NonvanishingAt (modUnit SpecQ) r x ↔ ratOfSpecQ r ≠ 0 := by
  have hloc := nonvanishingLocus_modUnit SpecQ r
  have hx : NonvanishingAt (modUnit SpecQ) r x ↔ x ∈ SpecQ.basicOpen r := by
    have : x ∈ nonvanishingLocus (modUnit SpecQ) r ↔ x ∈ (SpecQ.basicOpen r : Set SpecQ) := by
      rw [hloc]
    exact this
  rw [hx]
  ...
  · have hu : IsUnit r := by
    ...
    have hb : SpecQ.basicOpen r = ⊤ := SpecQ.basicOpen_of_isUnit hu
    rw [hb]
    simp [h]
...
theorem nonvanishingAt_iff_ratOfSpecQ {N : SpecQ.Modules} (ψ : N ≅ modUnit SpecQ)
    (t : Γ(N, ⊤)) (x : SpecQ) :
...
noncomputable def ptSectionValue {J : Scheme.{0}} {jstr : J ⟶ SpecQ} (M : J.Modules)
    {P : RelPoint jstr (𝟙 SpecQ)} (φ : modPullback P.1 M ≅ modUnit SpecQ) (t : Γ(M, ⊤)) : ℚ :=
...
theorem ptSectionValue_ne_zero_iff {J : Scheme.{0}} {jstr : J ⟶ SpecQ} (M : J.Modules)
    (hM : IsInvertibleSheaf M) {P : RelPoint jstr (𝟙 SpecQ)}
    (φ : modPullback P.1 M ≅ modUnit SpecQ) (t : Γ(M, ⊤)) (x : SpecQ) :
...
theorem exists_cubeForms_of_veryAmpleSystem {J : Scheme.{0}} {jstr : J ⟶ SpecQ}
    ...
    ∃ (cube : Fin dim × Fin dim → MvPolynomial (Fin dim × Fin dim) ℚ) (relDim : ℕ)
      ...
      (∀ P Q : RelPoint jstr (𝟙 SpecQ), ∃ c : ℚ, c ≠ 0 ∧ ∀ k : Fin dim × Fin dim,
...
points** — cut 2026-07-28 out of `exists_cubeModel_of_abelianScheme` below.

**NO LONGER A LEAF (2026-08-01).**  It is PROVEN immediately below over the two
halves declared above it in this file, `exists_veryAmpleSystem_of_isAmpleSheaf`
(the EMBEDDING half) and `exists_cubeForms_of_veryAmpleSystem` (the FORMS half),
which are the 2026-07-31 cut of this statement.  The assembly is bookkeeping:
`coords P i := ptSectionValue M (φ P) (s i)`; `coords_ne_zero` is
base-point-freeness through `ptSectionValue_ne_zero_iff`; `injective_of_smul` is
the minor clause read contrapositively (if `coords P = c • coords Q` then every
`2 × 2` minor vanishes identically, whatever `c` is — note `c ≠ 0` is not used);
and every remaining field is handed over verbatim by the FORMS half.

**WHY IT WAS STILL OPEN, AND IT WAS NOT FOR WANT OF THE CUT.**  The cut was made
on 2026-07-31, in `ModularCurve/X0.lean`, where this leaf lived at the time.
Later the same day this leaf and its parent were hoisted into THIS module to break
an import cycle (see the module docstring) — and the hoist took the LEAF and left
the CUT.  `X0.lean` `public import`s this module, so both halves ended up
DOWNSTREAM of the very declaration they were cut out of, where Lean's declaration
order makes them uncitable.  A comment-stripped scan on 2026-08-01 found each of
`exists_veryAmpleSystem_of_isAmpleSheaf`, `exists_cubeForms_of_veryAmpleSystem`
and `ptSectionValue_ne_zero_iff` occurring EXACTLY ONCE in `Fermat/` — at its own
declaration — so all three were dead, and the frontier carried THREE open leaves
for TWO leaves' worth of mathematics while the leaf the cut was meant to close
stayed open.  Relocating the block above this declaration restores the cut; the
frontier drops by one and no statement anywhere changed.

*The `MISSING MACHINERY` and `THE NEXT CUT` sections below are RETAINED and are
still accurate about what the two halves owe* — very ampleness plus projective
normality for the embedding half, Künneth for the forms half — but the paragraph
headed "THAT CUT WAS EVALUATED IN FULL ON 2026-07-30 (flt-lean-167) AND DECLINED"
is SUPERSEDED: the cut it declines is precisely the one that was made the
following day and that this declaration is now proven over.  Its reasoning is
kept because it is the argument for the `SpansSquare` clause, which the 2026-07-31
cut adopted; what it got wrong is the cost, not the mathematics.  See
`SpansSquare` above.
...
theorem nonempty_cubeModel_of_isAmpleSheaf_cube {J : Scheme.{0}}
  ...
  obtain ⟨cube, relDim, relDeg, rel, hch, hce, hrh, hre, hnv⟩ :=
    exists_cubeForms_of_veryAmpleSystem ab L hinv hsym hzero hcube M hM hML s φ hspan
  exact ⟨{ dim := dim
           coords := fun P i => ptSectionValue M (φ P) (s i)
           coords_ne_zero := by
             intro P hP
             obtain ⟨x⟩ : Nonempty (Spec (CommRingCat.of ℚ)) := inferInstance
             obtain ⟨i, hi⟩ := hbpf (P.1.base x)
             exact ((ptSectionValue_ne_zero_iff M hM (φ P) (s i) x).2 hi) (congrFun hP i)
           injective_of_smul := by
             intro P Q c hc h
             by_contra hne
             obtain ⟨i, j, hmin⟩ := hsep P Q hne
             exact hmin (by

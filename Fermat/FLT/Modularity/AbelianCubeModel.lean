...
/-!
### The evaluation apparatus, and the two halves of the coordinate dictionary

**MOVED HERE FROM `Fermat/FLT/ModularCurve/X0.lean` (lines 1326-1327 and
46459-46742) ON 2026-08-02, TO REPAIR A HOIST THAT ORPHANED THEM.**  The move is
verbatim: the sorted multiset of moved lines is unchanged, and only the two
breadcrumb notes left behind at the old sites were added.

What went wrong.  On 2026-07-31 `nonempty_cubeModel_of_isAmpleSheaf_cube` was CUT
in `X0.lean` into an EMBEDDING half (`exists_veryAmpleSystem_of_isAmpleSheaf`) and
a FORMS half (`exists_cubeForms_of_veryAmpleSystem`), and PROVEN over the two
(commit `74f525c8`).  Release 28 then hoisted the parent into this module to break
the `X0` -> `IsogenySignature` -> `HyperellipticJacobian` -> `X0` import cycle, and
took only its STATEMENT: the assembly proof was replaced by a fresh `sorry`, while
the two halves and the ~150 lines of proven apparatus they rest on stayed behind in
`X0.lean` -- which is DOWNSTREAM of this module, so nothing could ever reach them
again.  The result was three `sorry`s where the cut had left two, with two of them
free-floating: a comment-stripped scan of `Fermat/` found the only occurrences of
`exists_veryAmpleSystem_of_isAmpleSheaf`, `exists_cubeForms_of_veryAmpleSystem`,
`ptSectionValue_ne_zero_iff`, `SpansSquare`, `ratOfSpecQ` and the three
`nonvanishingAt_*` lemmas to be their own declarations and their own docstrings.

Every instrument reported this as ordinary open work: both halves emit
`declaration uses 'sorry'`, both are unowned, both are in the census.  It is the
DELETE-times-REFACTOR orphan shape CLAUDE.md records, with the deletion performed
by a hoist rather than by a branch.

The repair is this move plus the restoration of the assembly proof below, recovered
verbatim from `74f525c8`.  Frontier 3 -> 2 with no mathematics done.

`SpecQ` travels with the block.  The paragraph at the top of this file saying it is
"an `abbrev` declared in `X0.lean` and therefore not available upstream of it" was
true before this commit and is now false; the two statements that spell out
`Spec (CommRingCat.of ℚ)` are left as they are, since `SpecQ` is reducible and they
elaborate unchanged either way.
-/
...
theorem nonempty_cubeModel_of_isAmpleSheaf_cube {J : Scheme.{0}}
  ...
  obtain ⟨xQ⟩ : Nonempty ↥SpecQ := inferInstanceAs (Nonempty (PrimeSpectrum ℚ))
  refine ⟨{ dim := dim
  ...
  · -- `coords_ne_zero`: base-point freeness at the image point of `P`, pulled back.
    intro P hzero'
    obtain ⟨i, hi⟩ := hbpf (P.1.base xQ)
    exact ((ptSectionValue_ne_zero_iff M hM (φ P) (s i) xQ).2 hi) (congrFun hzero' i)
  · -- `injective_of_smul`: proportional coordinate vectors kill every `2 × 2` minor.
    intro P Q c _ hPQ
    by_contra hne
    obtain ⟨i, j, hij⟩ := hsep P Q hne
    have hi : ptSectionValue M (φ P) (s i) = c * ptSectionValue M (φ Q) (s i) := congrFun hPQ i
    have hj : ptSectionValue M (φ P) (s j) = c * ptSectionValue M (φ Q) (s j) := congrFun hPQ j
    exact hij (by rw [hi, hj]; ring)

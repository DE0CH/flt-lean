...
-- The curve-free Hopf-ORDER machinery of the Katz–Mazur/Raynaud
-- development: `exists_flat_hopf_form_of_hopf_order` (a Hopf order over
...
-- `vendored_mul_eq_convMul` / `vendored_one_eq_convOne`: the bridge
-- between the bare-hom convolution monoid on `A →ₐ[K] L` used by
-- `GaloisRep.HasFlatProlongationAt` (hence by `IsFlatPointsGroupAt`) and
-- mathlib's `WithConv` convolution. Non-public: proofs only.
...
-- `HopfAlgebra S (S ⊗[R] G)` appears in the SIGNATURE of
-- `exists_hopfOrder_baseChange`.
...
* **subobjects** (`IsFlatPointsGroupAt.of_injective`, DECOMPOSED
  2026-07-25 into the two étale–Galois leaves
  `exists_etaleHopfAlgebra_of_points_embedding` and
  `exists_surjective_bialgHom_of_points_injection`; the schematic
  closure over the DVR and the assembly are PROVEN): closure under
  equivariantly embedded subgroups — see its docstring;
...
set_option backward.isDefEq.respectTransparency false in
/-- **The étale sub-bialgebra of a point-group quotient** (sorry node —
step (β) of the Raynaud quotient-closure cut, split off 2026-07-25 from
`IsFlatPointsGroupAt.of_surjective`): a `Γ Kᵥ`-equivariant quotient `Y`
of the `Kᵥᵃˡᵍ`-point group of a finite étale `Kᵥ`-Hopf algebra `Q` is
the point group of a `Kᵥ`-sub-bialgebra `H ↪ Q`, again finite étale —
the pullback of functions along the point surjection. Intended proof
(Grothendieck's Galois correspondence for étale algebras carrying a
group structure; this is the CONVERSE direction of the PROVEN
Gelfand-duality machinery of `KnownIn1980s/EllipticCurves/Flat.lean`,
which builds the algebra from the group):
* *finiteness*: the point group `Q →ₐ[Kᵥ] Kᵥᵃˡᵍ` is finite — `Q` is
  module-finite over `Kᵥ`, so an algebra map into a field is pinned by
  its values on a finite spanning set, and `Q` étale makes those values
  roots of separable polynomials — hence `Y` is finite through `p`;
* *the algebra*: `Y` is then a finite `Γ Kᵥ`-group whose action factors
  through a finite Galois quotient, so `galoisEquivariantAlgebra` (the
  equivariant functions `Y → Kᵥᵃˡᵍ`) together with
  `galoisEquivariantEval_injective`/`_surjective`,
  `exists_hopfAlgebra_galoisEquivariantAlgebra` and their assembly
  `exists_finiteQuotient_galoisModule_etale_package` yield a finite
  étale `Kᵥ`-Hopf algebra `H` with `points(H) ≃+ Y` equivariantly;
* *the embedding*: the same construction applied to `points(Q)`
  identifies `Q` itself with the equivariant functions on its own
  points (its points separate its elements — the separation lemma
  `subalgebra_eq_top_of_algHom_separating` — and the two algebras have
  the same `Kᵥ`-dimension), and under that identification the pullback
  of functions along `p` is an INJECTIVE `Kᵥ`-bialgebra homomorphism
  `ι : H → Q` (injective because `p` is surjective; a bialgebra map
  because `p` is a group homomorphism) whose induced map on points is
  exactly `p` — the last clause below.
...
theorem exists_etale_subBialgebra_of_points_surjective
    ...
    ∃ (H : Type) (_ : CommRing H) (_ : HopfAlgebra Kᵥ H) (_ : Module.Finite Kᵥ H)
      ...
      ∀ φ : Q →ₐ[Kᵥ] Ωᵥ,
        e (Additive.ofMul (φ.comp (ι : H →ₐ[Kᵥ] Q))) = p (Additive.ofMul φ) :=
  sorry
...
`exists_etale_subBialgebra_of_points_surjective` (β) and
`exists_hopfOrder_of_subBialgebra` (γ) above, with the assembly below
PROVEN — the quotients half of Raynaud closure, added 2026-07-24 for
...
* (β) *étale–Galois* — the leaf
  `exists_etale_subBialgebra_of_points_surjective`: `Y` is the point
  group of a finite étale `Kᵥ`-Hopf algebra `H` embedded in `Q` by an
  injective `Kᵥ`-bialgebra map `ι`, the pullback of functions along the
  point surjection.
...
`(ρ.toLocal v).Space` is one by the schematic-closure node
`IsFlatPointsGroupAt.of_injective` (PROVEN assembly over the two
sorried étale–Galois leaves). Sound as stated: unconditionally
...
on `p`-power roots of unity. Proof: a `p^n`-th root of unity `z` has
spectral valuation `1`, so lies in `Z2bar`, and an inertia element `τ`
moves it by `v (τ z - z) < 1`; but a NONtrivial `p`-power root of unity
`u` has `v (u - 1) = 1` — by
...
UPSTREAM DUPLICATE (dedupe note, 2026-07-24): this is verbatim the
proof of `IsHardlyRamified.cyclotomicCharacter_eq_one_of_mem_inertia_two`
in `Family.lean`, duplicated here because `Family.lean` IMPORTS this
file (the Eisenstein pillars' circularity guard forbids using it), the
generalization lives at the residual prime `2` bookkeeping of pillar
E1a, and this file may not modify other owners' files. At integration
time `Family.lean`'s twin can be rewired to this upstream copy. -/
theorem cyclotomicCharacter_eq_one_of_mem_inertia_two_upstream
...
set_option backward.isDefEq.respectTransparency false in
set_option maxHeartbeats 1000000 in
/-- **Tame unipotence of hardly ramified inertia at `2`, residual
form** (PROVEN 2026-07-24 — the local Jordan–Hölder input of pillar
E1a): every inertia element `τ` at `2` (spelled over `ℚ_[2]`, matching
the `isTameAtTwo` clause) acts on a mod-`p` hardly ramified
representation with `(ρ̄(τ) − 1)² = 0` — both local Jordan–Hölder
factors of `ρ̄|_{G_2}` are trivial on inertia. Content: the tame-at-two
datum `(π, δ)` exhibits the line `ker π` as `ρ̄(G_2)`-stable with `ρ̄ τ`
descending to `δ τ = 1` on the quotient (inertia lies in `δ.ker`); on
the line `ρ̄ τ` acts by the scalar `κ = det (ρ̄ τ)` (adapted-basis
determinant), and `det ρ̄ = ω` is trivial on inertia at `2`
(`cyclotomicCharacter_eq_one_of_mem_inertia_two_upstream`), so `ρ̄ τ`
is trivial on both the line and the quotient — the field-coefficient
residual analogue of `Family.lean`'s `charpoly_eq_of_mem_inertia_two`,
proven directly (no charpoly) since over the field `k` the adapted
basis is explicit. -/
theorem residual_rep_sub_one_sq_eq_zero_of_inertia_two
    ...
    (hτ : τ ∈ AddSubgroup.inertia
      ((IsLocalRing.maximalIdeal Z2bar).toAddSubgroup : AddSubgroup Z2bar)
      (Field.absoluteGaloisGroup ℚ_[2])) :
    (ρbar (Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[2]) τ) - 1) ^ 2 = 0 := by
  classical
  obtain ⟨π, hπsurj, δ, hδ⟩ := hρbar.isTameAtTwo
  have hδτ : δ τ = 1 := (hδ τ 0).2.1 hτ
  set e : Module.End k W :=
    ρbar (Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[2]) τ) with hedef
  -- `π` intertwines `e` with `δ τ = 1`
  have hcomm : ∀ v : W, π (e v) = π v := by
    intro v
    have h1 := (hδ τ v).1
    rw [GaloisRep.map_apply, hδτ, Module.End.one_apply] at h1
    exact h1
  -- the determinant of `e` is the cyclotomic character at `τ`, which
  -- is trivial on inertia at `2`
  have hdet : LinearMap.det e = 1 := by
    have h := hρbar.det (Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[2]) τ)
    rw [GaloisRep.det_apply] at h
    rw [hedef, h,
      cyclotomicCharacter_eq_one_of_mem_inertia_two_upstream hpodd τ hτ,
      Units.val_one, map_one]
  -- dimension bookkeeping: `ker π` is a line
  have hfinrank : Module.finrank k W = 2 :=
    Module.finrank_eq_of_rank_eq (by rw [hW]; norm_num)
  set N : Submodule k W := LinearMap.ker π with hNdef
  have hNrank : Module.finrank k N = 1 := by
    have hsum := LinearMap.finrank_range_add_finrank_ker π
    have hrange : Module.finrank k (LinearMap.range π) = 1 := by
      rw [LinearMap.range_eq_top.mpr hπsurj, finrank_top]
      exact Module.finrank_self k
    rw [← hNdef] at hsum
    omega
  let bN : Module.Basis (Fin 1) k N := Module.finBasisOfFinrankEq k N hNrank
  -- the adapted basis `(v₀, u)` with `π v₀ = 1` and `u` spanning `ker π`
  obtain ⟨v₀, hv₀⟩ := hπsurj 1
  have hli : ∀ c : k, ∀ x ∈ N, c • v₀ + x = 0 → c = 0 := by
    intro c x hx hcx
    have h := congrArg π hcx
    rw [map_add, map_smul, hv₀, LinearMap.mem_ker.mp (hNdef ▸ hx), add_zero,
      map_zero, smul_eq_mul, mul_one] at h
    exact h
  have hsp : ∀ z : W, ∃ c : k, z + c • v₀ ∈ N := by
    intro z
    refine ⟨-π z, ?_⟩
    rw [hNdef, LinearMap.mem_ker, map_add, map_smul, hv₀, smul_eq_mul,
      mul_one, add_neg_cancel]
  let b2 : Module.Basis (Fin 2) k W := Module.Basis.mkFinCons v₀ bN hli hsp
  have hb20 : b2 0 = v₀ := by
    have h0 := congrFun (Module.Basis.coe_mkFinCons v₀ bN hli hsp) 0
    rw [Fin.cons_zero] at h0
    exact h0
  have hb21 : b2 1 = (bN 0 : W) := by
    have h1 := congrFun (Module.Basis.coe_mkFinCons v₀ bN hli hsp) (Fin.succ 0)
    rw [Fin.cons_succ] at h1
    exact h1
  -- the action on the two basis vectors, through the rank-one basis of
  -- `ker π` (repr-based, avoiding any rewrite of `N` under `bN`)
  have hπu : π (bN 0 : W) = 0 := LinearMap.mem_ker.mp (bN 0).2
  have heuN : e (bN 0 : W) ∈ N :=
    LinearMap.mem_ker.mpr (by rw [hcomm]; exact hπu)
  have hev₀N : e v₀ - v₀ ∈ N :=
    LinearMap.mem_ker.mpr (by rw [map_sub, hcomm, sub_self])
  have hκrepr := bN.sum_repr ⟨e (bN 0 : W), heuN⟩
  rw [Fin.sum_univ_one] at hκrepr
  set κ : k := bN.repr ⟨e (bN 0 : W), heuN⟩ 0
  have hκu : κ • (bN 0 : W) = e (bN 0 : W) := by
    have h := congrArg Subtype.val hκrepr
    simpa using h
  have hmrepr := bN.sum_repr ⟨e v₀ - v₀, hev₀N⟩
  rw [Fin.sum_univ_one] at hmrepr
  set m : k := bN.repr ⟨e v₀ - v₀, hev₀N⟩ 0
  have hm : m • (bN 0 : W) = e v₀ - v₀ := by
    have h := congrArg Subtype.val hmrepr
    simpa using h
  have hev0 : e (b2 0) = (1 : k) • b2 0 + m • b2 1 := by
    rw [hb20, hb21, one_smul, hm]
    abel
  have heu : e (b2 1) = (0 : k) • b2 0 + κ • b2 1 := by
    rw [hb21, zero_smul, zero_add]
    exact hκu.symm
  -- the determinant in the adapted basis is `κ`, so `κ = 1`
  have hM00 : LinearMap.toMatrix b2 b2 e 0 0 = 1 := by
    rw [LinearMap.toMatrix_apply, hev0]
    simp [Module.Basis.repr_self]
  have hM10 : LinearMap.toMatrix b2 b2 e 1 0 = m := by
    rw [LinearMap.toMatrix_apply, hev0]
    simp [Module.Basis.repr_self]
  have hM01 : LinearMap.toMatrix b2 b2 e 0 1 = 0 := by
    rw [LinearMap.toMatrix_apply, heu]
    simp [Module.Basis.repr_self]
  have hM11 : LinearMap.toMatrix b2 b2 e 1 1 = κ := by
    rw [LinearMap.toMatrix_apply, heu]
    simp [Module.Basis.repr_self]
  have hκ1 : κ = 1 := by
    have hdet2 : LinearMap.det e = κ := by
      rw [← LinearMap.det_toMatrix b2 e, Matrix.det_fin_two, hM00, hM01,
        hM10, hM11]
      ring
    rw [← hdet2, hdet]
  -- both basis vectors are killed by `(e − 1)²`
  have hsub1 : (e - 1) (b2 1) = 0 := by
    rw [LinearMap.sub_apply, Module.End.one_apply, heu, hκ1, zero_smul,
      zero_add, one_smul, sub_self]
  have hsub0 : (e - 1) (b2 0) = m • b2 1 := by
    rw [LinearMap.sub_apply, Module.End.one_apply, hev0, one_smul]
    abel
  refine b2.ext fun i => ?_
  fin_cases i
  · show (((e - 1) ^ 2 : Module.End k W)) (b2 0) = (0 : Module.End k W) (b2 0)
    rw [pow_two, Module.End.mul_apply, hsub0, map_smul, hsub1, smul_zero,
...
suffices: `χ` has commutative values, hence is invariant under
`G_ℚ`-conjugation, which sweeps out all primes above `q`), is a power
of the mod-`p` cyclotomic character `ω` (the composite of the `p`-adic
...
outside `p` since `ℚ(μ_{p^n})/ℚ` is), and the conclusion holds for
every inhabitant by the argument cited; the statement is true even at
`p = 2` (`Gal(ℚ(μ_{2^∞})/ℚ)` is pro-`2` and the image has odd order,
forcing `χ = 1`), so `hpodd` is uniformity, not load-bearing.
CIRCULARITY GUARD (inherited from pillar E1a): must not be proven
through `Family.lean` or `Reducible.lean`'s B5. -/
theorem character_eq_pow_cyclotomicCharacter_of_unramified_outside_p
    {k : Type*} [Field k] [Finite k] [Algebra ℤ_[p] k]
    (χ : Field.absoluteGaloisGroup ℚ →* kˣ)
    (hker : IsOpen ((χ.ker : Subgroup (Field.absoluteGaloisGroup ℚ)) :
      Set (Field.absoluteGaloisGroup ℚ)))
    (hunram : ∀ (q : ℕ) (hq : q.Prime), q ≠ p →
...
(Eisenstein pillar E1a; PROVEN 2026-07-24 as an assembly over the
three helpers above — the at-`2` cyclotomic-inertia lemma
...
theorem residual_triangular_sub_character_eq_pow_cyclotomic
  ...
  -- the action on the first basis vector, read off the triangular matrix
  have hact : ∀ g, ρbar g (b 0) = χsub g • b 0 := by
    intro g
    have h00 : b.repr (ρbar g (b 0)) 0 = χsub g := by
      simpa [LinearMap.toMatrix_apply] using
        congrArg (fun M => M 0 0) (htri g)
    have h10 : b.repr (ρbar g (b 0)) 1 = 0 := by
      simpa [LinearMap.toMatrix_apply] using
        congrArg (fun M => M 1 0) (htri g)
    have hsum := b.sum_repr (ρbar g (b 0))
    rw [Fin.sum_univ_two, h00, h10, zero_smul, add_zero] at hsum
    exact hsum.symm
  have hb0ne : b 0 ≠ 0 := b.ne_zero 0
  -- the unit-valued lift of the sub-character
  set χu : Field.absoluteGaloisGroup ℚ →* kˣ := χsub.toHomUnits with hχu
  have hχuval : ∀ g, ((χu g : kˣ) : k) = χsub g := by
    intro g
    rw [hχu]
    exact MonoidHom.coe_toHomUnits χsub g
  -- the sub-character is trivial wherever the representation is
  have htriv : ∀ g, ρbar g = 1 → χu g = 1 := by
    intro g hg
    apply Units.ext
    rw [Units.val_one, hχuval]
    refine subCharacter_eq_one_of_sq_eq_zero (ρbar g) ?_ hb0ne (hact g)
    rw [hg, sub_self]
    exact zero_pow two_ne_zero
  -- the kernel is open: it contains the open kernel of the representation
  have hker : IsOpen ((χu.ker : Subgroup (Field.absoluteGaloisGroup ℚ)) :
      Set (Field.absoluteGaloisGroup ℚ)) := by
    have hKρ_open : IsOpen {g : Field.absoluteGaloisGroup ℚ | ρbar g = 1} :=
    ...
    have hle : Kρ ≤ χu.ker := fun g hg => MonoidHom.mem_ker.mpr (htriv g hg)
    exact Subgroup.isOpen_mono hle hKρ_open
  -- the sub-character kills local inertia at every prime `q ≠ p`
  have hunram : ∀ (q : ℕ) (hq : q.Prime), q ≠ p →
      ∀ σ ∈ localInertiaGroup hq.toHeightOneSpectrumRingOfIntegersRat,
        χu (Field.absoluteGaloisGroup.map (algebraMap ℚ
          (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
            hq.toHeightOneSpectrumRingOfIntegersRat)) σ) = 1 := by
    intro q hq hqp
    by_cases hq2 : q = 2
    · -- at `2`: the inertia bridge to `ℚ_[2]`, conjugation-stability of
      -- unipotence, and the tame unipotence lemma
      subst hq2
      intro σ hσ
      apply Units.ext
      rw [Units.val_one, hχuval]
      refine subCharacter_eq_one_of_sq_eq_zero _ ?_ hb0ne (hact _)
      obtain ⟨τ, hτ, c, hconj⟩ :=
        IsHardlyRamified.localInertia_two_eq_map_padic hσ
      have hconj2 : Field.absoluteGaloisGroup.map (algebraMap ℚ
          (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
            hq.toHeightOneSpectrumRingOfIntegersRat)) σ =
          c * Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[2]) τ * c⁻¹ :=
        hconj
      have hkey :=
        residual_rep_sub_one_sq_eq_zero_of_inertia_two hpodd hW hρbar hτ
      have ha : ρbar c * ρbar c⁻¹ = 1 := by
        rw [← map_mul, mul_inv_cancel, map_one]
      have ha' : ρbar c⁻¹ * ρbar c = 1 := by
        rw [← map_mul, inv_mul_cancel, map_one]
      rw [hconj2, map_mul, map_mul]
      have hfac : ρbar c * ρbar (Field.absoluteGaloisGroup.map
          (algebraMap ℚ ℚ_[2]) τ) * ρbar c⁻¹ - 1 =
          ρbar c * (ρbar (Field.absoluteGaloisGroup.map
            (algebraMap ℚ ℚ_[2]) τ) - 1) * ρbar c⁻¹ := by
        rw [mul_sub, mul_one, sub_mul, ha]
      rw [hfac, pow_two]
      have h1 : ρbar c * (ρbar (Field.absoluteGaloisGroup.map
          (algebraMap ℚ ℚ_[2]) τ) - 1) * ρbar c⁻¹ *
          (ρbar c * (ρbar (Field.absoluteGaloisGroup.map
            (algebraMap ℚ ℚ_[2]) τ) - 1) * ρbar c⁻¹) =
          ρbar c * ((ρbar (Field.absoluteGaloisGroup.map
            (algebraMap ℚ ℚ_[2]) τ) - 1) * (ρbar c⁻¹ * ρbar c) *
            (ρbar (Field.absoluteGaloisGroup.map
              (algebraMap ℚ ℚ_[2]) τ) - 1)) * ρbar c⁻¹ := by
        simp only [mul_assoc]
      rw [h1, ha', mul_one, ← pow_two, hkey, mul_zero, zero_mul]
    · -- away from `2` and `p`: the representation itself is unramified,
      -- through the PROVEN generic-`K` wrapper (the `ℚ`-side `algebraMap`
      -- spelling gap is closed propositionally by `Subsingleton.elim`)
      intro σ hσ
      have h4 := character_localInertia_le_ker_of_isUnramifiedAt ρbar
        hq.toHeightOneSpectrumRingOfIntegersRat
        (hρbar.isUnramified q hq ⟨hq2, hqp⟩) χu htriv
      have h5 := h4 hσ
      rw [MonoidHom.mem_ker, MonoidHom.comp_apply] at h5
      convert h5 using 4
      exact congrArg (fun f : ℚ →+*
          (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
            hq.toHeightOneSpectrumRingOfIntegersRat) =>
        Field.absoluteGaloisGroup.map f σ) (Subsingleton.elim _ _)
  -- the Kronecker–Weber node pins the character to a power of `ω`
  obtain ⟨i, hi⟩ :=
    character_eq_pow_cyclotomicCharacter_of_unramified_outside_p hpodd χu
      hker hunram
  exact ⟨i, fun g => (hχuval g).symm.trans (hi g)⟩
...
tame inertia at `p`** (Eisenstein pillar E1b-ii; PROVEN 2026-07-24 —
the Serre §1.3 fundamental-character input of the Raynaud pinning
and the last local-arithmetic gap of E1b): if a
...
Lean route (as executed): the surjectivity of `ω` on inertia is the
Gauss-period leaf
`exists_mem_localInertiaGroup_cyclotomicCharacter_toZModPow_eq`
above, applied to a generator `g₀` of the cyclic group `(ℤ/p)^×`;
the residue map `ℤ_[p] → k` factors through level `1`
(`PadicInt.ker_toZModPow` plus `(p : k) = 0` from
`prime_eq_zero_of_finite_padicInt_algebra`) as the INJECTIVE ring
hom `ZMod.castHom : ℤ/p → k`, so the hypothesis at the realizing
inertia element reads `g₀^i = 1` inside `(ℤ/p)^×` and
`orderOf g₀ = φ(p) = p − 1` closes it. Soundness (audit 2026-07-24): the
...
theorem sub_one_dvd_of_cyclotomicCharacter_residue_inertia_pow_eq_one
  ...
  classical
  haveI : NeZero (p ^ 1) := ⟨pow_ne_zero 1 hp.out.ne_zero⟩
  haveI hfactp1 : Fact ((p ^ 1).Prime) := ⟨by rw [pow_one]; exact hp.out⟩
  have hpk : (p : k) = 0 := prime_eq_zero_of_finite_padicInt_algebra
  haveI : CharP k p := (CharP.charP_iff_prime_eq_zero hp.out).mpr hpk
  -- a generator of `(ℤ/p)ˣ`, realized by an inertia element
  obtain ⟨g₀, hg₀⟩ := IsCyclic.exists_generator (α := (ZMod (p ^ 1))ˣ)
  obtain ⟨σ₀, hσ₀I, hσ₀val⟩ :=
    exists_mem_localInertiaGroup_cyclotomicCharacter_toZModPow_eq g₀
  -- the residue embedding `ℤ/p ↪ k` and the level-`1` factorization
  set ψ : ZMod (p ^ 1) →+* k := ZMod.castHom (dvd_pow_self p one_ne_zero) k
    with hψdef
  have halg : ∀ z : ℤ_[p], algebraMap ℤ_[p] k z =
      ψ (PadicInt.toZModPow 1 z) := by
    intro z
    set n := (PadicInt.toZModPow 1 z).val with hndef
    have hker : z - ((n : ℕ) : ℤ_[p]) ∈
    ...
    have hz : z = ((n : ℕ) : ℤ_[p]) + t * (p : ℤ_[p]) ^ 1 := by
      linear_combination -ht
    rw [hz]
    simp only [map_add, map_mul, map_pow, map_natCast]
    rw [hpk]
    ring
  -- evaluate the hypothesis at the realizing inertia element
  have hpow := h σ₀ hσ₀I
  rw [halg, hσ₀val] at hpow
  have hg₀pow : ((g₀ : (ZMod (p ^ 1))ˣ) : ZMod (p ^ 1)) ^ i = 1 := by
    apply ψ.injective
    rw [map_pow, map_one]
    exact hpow
  have hg₀unit_pow : g₀ ^ i = 1 := by
    ext
    rw [Units.val_pow_eq_pow_val, Units.val_one]
    exact hg₀pow
  have horder := orderOf_dvd_of_pow_eq_one hg₀unit_pow
  have hcard : orderOf g₀ = p ^ 1 - 1 := by
    rw [orderOf_eq_card_of_forall_mem_zpowers hg₀, Nat.card_eq_fintype_card,
      ZMod.card_units_eq_totient, Nat.totient_prime hfactp1.out]
  rw [hcard, pow_one] at horder
  exact horder
...
decomposition groups at `p`** (Eisenstein pillar E3a-ii; FULLY PROVEN
2026-07-25 — completion bookkeeping, the at-`p` analogue of the at-`2`
...
conjugate by one fixed `c ∈ Γ ℚ`, elementwise onto. Proof (the at-`2`
template, with the conjugator hoisted out of the element quantifier):
the continuous `ℚ`-algebra isomorphism `adicCompletion ℚ v_p ≃ ℚ_[p]`
(`Rat.HeightOneSpectrum.adicCompletion.padicEquiv`, cast along
`natGenerator_toHeightOneSpectrum` — only its RING structure is used)
lifts to `ι : ℚ_[p]ᵃˡᵍ →+* (adicCompletion ℚ v_p)ᵃˡᵍ`, bijective
because `ι ∘ (the closure map of `E`)` is an endomorphism of an
algebraic closure over its base, hence bijective; `ι ∘ j_p` and `j_v`
are then two embeddings of `ℚᵃˡᵍ` into `(adicCompletion ℚ v_p)ᵃˡᵍ`
over `ℚ`, and `Normal.algHomEquivAut` turns the first into a FIXED
`c ∈ Γ ℚ` with `j_v ∘ c = ι ∘ j_p` — a datum INDEPENDENT of `g`, which
is exactly the strengthening over the at-`2` bridge. For `g : Γ ℚ_[p]`
the transported element `h := ι ∘ g ∘ ι⁻¹ ∈ Γ (adicCompletion ℚ v_p)`
then satisfies `map h = c · map g · c⁻¹` by uniqueness of restrictions
along the injective `j_v`, and the stated equation is that identity
rearranged (with conjugator `c⁻¹`). Soundness (audit 2026-07-24): true
for EVERY prime `p` (no oddness consumed) — the two completions are
isomorphic topological fields and restriction images of isomorphic
local groups along embeddings differing by `c` are conjugate by that
same `c`; the statement quantifies the conjugator OUTSIDE the element,
exactly what the classical "well-defined up to conjugation" slogan
provides for a single place. DEDUP NOTE (2026-07-25): the E3b leaf
`exists_uniform_conj_decomposition_two_padic` below is literally this
statement at `p = 2` (the place spellings agree by proof irrelevance
in `Nat.Prime.toHeightOneSpectrumRingOfIntegersRat`), so it can be
discharged by `exists_conjugator_padicGalois_eq_adic_at_p (p := 2)` —
left to that leaf's owner, untouched here. -/
theorem exists_conjugator_padicGalois_eq_adic_at_p :
        ...
        ∃ h : Field.absoluteGaloisGroup (HeightOneSpectrum.adicCompletion ℚ
          ...
          Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[p]) g =
            c * Field.absoluteGaloisGroup.map (algebraMap ℚ
              (HeightOneSpectrum.adicCompletion ℚ
                (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
                  (Fact.out : p.Prime)))) h * c⁻¹ := by
  classical
  -- (1) the completion of `ℚ` at `v_p` IS `ℚ_[p]`, as a ring isomorphism
  haveI hfp : Fact ((Rat.HeightOneSpectrum.primesEquiv
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
        (Fact.out : p.Prime))) : ℕ).Prime :=
    ⟨(Rat.HeightOneSpectrum.primesEquiv
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime))).2⟩
  have hprime : ((Rat.HeightOneSpectrum.primesEquiv
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
        (Fact.out : p.Prime))) : ℕ) = p := by
    show Rat.HeightOneSpectrum.natGenerator _ = p
  ...
  have hE : Nonempty ((HeightOneSpectrum.adicCompletion ℚ
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
        (Fact.out : p.Prime))) ≃+* ℚ_[p]) := by
    ...
    exact ⟨((Rat.HeightOneSpectrum.adicCompletion.padicEquiv
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
        (Fact.out : p.Prime))).toAlgEquiv.toRingEquiv).trans
      (hcast _ p hfp hp hprime)⟩
  obtain ⟨E⟩ := hE
  -- (2) the closure-level transport of `E`, and its bijectivity
  set ι := AlgebraicClosure.map (E.symm : ℚ_[p] →+*
    (HeightOneSpectrum.adicCompletion ℚ
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
        (Fact.out : p.Prime)))) with hι
  have hιsurj : Function.Surjective ι := by
    set j := AlgebraicClosure.map (E : (HeightOneSpectrum.adicCompletion ℚ
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
        (Fact.out : p.Prime))) →+* ℚ_[p]) with hj
    set jc : AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
        (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime)))
  ...
  set ι₁ := AlgebraicClosure.map (algebraMap ℚ (HeightOneSpectrum.adicCompletion ℚ
    (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime))))
  set ι₂ := AlgebraicClosure.map (algebraMap ℚ ℚ_[p])
  ...
  set c : Field.absoluteGaloisGroup ℚ := (Normal.algHomEquivAut (F := ℚ)
    (K₁ := AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime))))
  ...
  have hfc : ∀ x : AlgebraicClosure ℚ, f x = ι₁ (c x) := by
    ...
    rfl
  refine ⟨c⁻¹, fun g => ?_⟩
  -- (4) the transported element `h = ι ∘ g ∘ ι⁻¹`
  set h₀ := (ιe.symm.trans g.toRingEquiv).trans ιe
  have hh₀_apply : ∀ y, h₀ y = ιe (g (ιe.symm y)) := fun _ => rfl
  set h : Field.absoluteGaloisGroup (HeightOneSpectrum.adicCompletion ℚ
      (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime))) :=
    AlgEquiv.ofRingEquiv (f := h₀) (fun x => by
      rw [hh₀_apply]
      have hx : ιe.symm ((algebraMap (HeightOneSpectrum.adicCompletion ℚ
          (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime)))
          (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
            (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
              (Fact.out : p.Prime))))) x) =
          algebraMap ℚ_[p] (AlgebraicClosure ℚ_[p]) (E x) := by
        rw [RingEquiv.symm_apply_eq, hιe_apply, hι,
          AlgebraicClosure.map_algebraMap]
        congr 1
        exact (E.symm_apply_apply x).symm
      rw [hx, g.commutes (E x), hιe_apply, hι, AlgebraicClosure.map_algebraMap]
      congr 1
      exact E.symm_apply_apply x)
  have hh_apply : ∀ y, h y = ιe (g (ιe.symm y)) := fun _ => rfl
  have hsquare : ∀ y, ι (g y) = h (ι y) := by
    intro y
    rw [hh_apply, ← hιe_apply y, RingEquiv.symm_apply_apply, hιe_apply]
  refine ⟨h, ?_⟩
  -- (5) the conjugation square, pointwise through the injective `ι₁`
  have key : Field.absoluteGaloisGroup.map (algebraMap ℚ
      (HeightOneSpectrum.adicCompletion ℚ
        (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime)))) h =
      c * Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[p]) g * c⁻¹ := by
    apply AlgEquiv.ext
    intro x
    apply ι₁.injective
    have hL := Field.absoluteGaloisGroup.lift_map (algebraMap ℚ
      (HeightOneSpectrum.adicCompletion ℚ
        (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat (Fact.out : p.Prime)))) h x
    have hR := Field.absoluteGaloisGroup.lift_map (algebraMap ℚ ℚ_[p]) g (c⁻¹ x)
    rw [show ι₁ ((Field.absoluteGaloisGroup.map (algebraMap ℚ
      (HeightOneSpectrum.adicCompletion ℚ
        (Nat.Prime.toHeightOneSpectrumRingOfIntegersRat
          (Fact.out : p.Prime)))) h) x) = h (ι₁ x) from hL]
    show h (ι₁ x) =
      ι₁ (c ((Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[p]) g) (c⁻¹ x)))
    rw [← hfc]
    rw [show f ((Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[p]) g) (c⁻¹ x)) =
      ι (ι₂ ((Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[p]) g) (c⁻¹ x)))
      from rfl]
    rw [show ι₂ ((Field.absoluteGaloisGroup.map (algebraMap ℚ ℚ_[p]) g) (c⁻¹ x)) =
      g (ι₂ (c⁻¹ x)) from hR]
    rw [hsquare]
...
theorem exists_restrictNormalHom_eq_adicArithFrob_pow_mul_inertia_two
    ...
    [IsGalois (HeightOneSpectrum.adicCompletion ℚ
      Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat) N]
    (g : Field.absoluteGaloisGroup (HeightOneSpectrum.adicCompletion ℚ
      Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat)) :
...
against open subgroups** (DECOMPOSED 2026-07-25 into the finite-level
leaf `exists_restrictNormalHom_eq_adicArithFrob_pow_mul_inertia_two`
above; the profinite assembly is PROVEN here): an OPEN subgroup of the
local absolute Galois group at the place `2` containing the full local
inertia and the chosen arithmetic Frobenius `adicArithFrob` is
everything. Assembly: an open subgroup is a neighbourhood of `1`, so
by the Krull topology
(`krullTopology_mem_nhds_one_iff_of_normal`) it contains the fixing
subgroup of a finite Galois level `N`, which is the kernel of
`AlgEquiv.restrictNormalHom N` (`IntermediateField.restrictNormalHom_ker`);
the finite-level leaf writes the restriction of a given `g` as
`Φ̄ ^ m · τ` with `τ` in the finite-level inertia; the compactness
lifting `exists_mem_localInertiaGroup_restrictNormalHom_eq`
(`LocalInertiaFixedField.lean`, PROVEN) realises `τ` as the
restriction of an honest element `w` of the FULL local inertia; then
`g · (Φ ^ m · w)⁻¹` restricts to `1`, hence lies in the fixing
subgroup and so in `H`, while `Φ ^ m · w ∈ H` by `hfrob` and
`hinertia` — so `g ∈ H`. (Serre, *Corps Locaux*, IV §1; the same
...
theorem mem_of_isOpen_of_localInertiaGroup_le_of_adicArithFrob_mem_two
  ...
  classical
  intro g
  -- an open subgroup is a neighbourhood of `1`, hence contains the
  -- fixing subgroup of a finite Galois level `N`
  have hnhds : (H : Set (Field.absoluteGaloisGroup
      (HeightOneSpectrum.adicCompletion ℚ
        Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat))) ∈
      nhds (1 : Field.absoluteGaloisGroup
        (HeightOneSpectrum.adicCompletion ℚ
          Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat)) :=
    hopen.mem_nhds (one_mem H)
  ...
  haveI : IsGalois (HeightOneSpectrum.adicCompletion ℚ
    Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat) N := ⟨⟩
  -- the finite-level decomposition of `g`, and the profinite lift of its
  -- inertia part
  obtain ⟨m, τ, hτI, hτ⟩ :=
    exists_restrictNormalHom_eq_adicArithFrob_pow_mul_inertia_two N g
  obtain ⟨w, hwmem, hwres⟩ :=
    exists_mem_localInertiaGroup_restrictNormalHom_eq
      Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat N τ hτI
  -- `g` and `Φ ^ m * w` restrict to the same automorphism of `N`
  set Φ : Field.absoluteGaloisGroup (HeightOneSpectrum.adicCompletion ℚ
      Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat) :=
    Field.AbsoluteGaloisGroup.adicArithFrob
      Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat with hΦ
  have hker : g * (Φ ^ m * w)⁻¹ ∈
      (AlgEquiv.restrictNormalHom (F := HeightOneSpectrum.adicCompletion ℚ
        Nat.prime_two.toHeightOneSpectrumRingOfIntegersRat) N).ker := by
    rw [MonoidHom.mem_ker, map_mul, map_inv, map_mul, map_pow, hτ, hwres, hΦ]
    rw [mul_inv_rev, ← mul_assoc]
    rw [mul_assoc _ τ τ⁻¹, mul_inv_cancel, mul_one]
    rw [mul_inv_cancel]
  rw [IntermediateField.restrictNormalHom_ker] at hker
  -- both factors lie in `H`
  have hmemfix : g * (Φ ^ m * w)⁻¹ ∈ H := hle hker
  have hmemfrob : Φ ^ m * w ∈ H :=
    mul_mem (pow_mem hfrob m) (hinertia hwmem)
  have hg : g = (g * (Φ ^ m * w)⁻¹) * (Φ ^ m * w) := by
    rw [inv_mul_cancel_right]
  rw [hg]
  exact mul_mem hmemfix hmemfrob
...
* `exists_galoisRep_charFrob_of_weightTwoNewform` — the
  ...
  docstring below), three times: the attachment is a PROVEN assembly
...
* `heckeSubalgebra` + `heckeSubalgebra_mul_comm`,
  `exists_smul_of_mem_heckeSubalgebra`,
  `pair_comm_of_mem_heckeSubalgebra` — PROVEN `adjoin`-induction
  algebra for the subalgebra generated by a prime-indexed operator
  family (commutativity, common-scalar action on joint
  eigen-elements, self-adjointness transport for a pairing).
* `ModularJacobianPackage` — the second-level, `g`-INDEPENDENT
  carrier (2026-07-24): the same Tate-module data, but with the two
  spectral fields of `EichlerShimuraPackage` replaced by sharper
  module theory — freeness of rank 2 over `heckeSubalgebra hecke`
  (`basis₁/basis₂/span_free/indep_free`), the Atkin–Lehner-twisted
  Weil pairing (`pair_*`), Hecke commutativity, and the étale
  newform idempotent (`eigen_idempotent`, quantified over `(g, κ)`
  inside the structure).
* `ModularHomologyPackage` — the third-level carrier (2026-07-24):
  identical except that `eigen_idempotent` is replaced by the two
...
* `nonempty_modularJacobianPackage` — now a PROVEN assembly: the
  ...
  nilpotency clause, and strong multiplicity one upgrades this to
  the exact eigen-relation `hecke q * e = κ(a_q) • e`.
* `nonempty_eichlerShimuraPackage` — now a PROVEN assembly: the
  joint `κ(a(g))`-eigenspace equals the image `e·Vp` of the newform
  idempotent (scalar-action induction on the eigenvector
  coordinates), hence is the 2-dimensional span of
  `e basis₁, e basis₂` by freeness; the twisted Weil pairing
  restricts to a symplectic form on it, and a symplectic similitude
  of multiplier `q` in dimension 2 has determinant `q`.
* `exists_galoisRep_charFrob_of_weightTwoNewform` — a PROVEN
...
/-! ##### The abstract Hecke subalgebra of an operator family

Pure-algebra support for the modular-Jacobian carrier below: the
`F`-subalgebra of `End(V₀)` generated by the prime-indexed members of
an operator family, together with the three facts the Eichler–Shimura
inhabitation assembly consumes — commutativity (from pairwise
commutation of the family), the scalar-action induction (members act
by a common scalar on joint eigen-elements), and self-adjointness
transport for a bilinear pairing. All three are `Algebra.adjoin`
inductions. -/

variable {F : Type*} [Field F] {V₀ : Type*} [AddCommGroup V₀]
    [Module F V₀]

/-- The `F`-subalgebra of `End(V₀)` generated by the prime-indexed
operators of the family `t`. For the modular-Jacobian package below
(`t` = the Hecke operators on the Tate module) this is the
base-changed Hecke algebra `𝕋 ⊗ ℚ̄_p`: generation by the prime-indexed
operators alone is the classical fact that the composite `T_n` are
polynomials in the `T_q`/`U_q` (the Hecke recursions, Diamond–Shurman
§5.2-5.3). -/
def heckeSubalgebra (t : ℕ → Module.End F V₀) :
    Subalgebra F (Module.End F V₀) :=
  Algebra.adjoin F {φ : Module.End F V₀ | ∃ q : ℕ, q.Prime ∧ φ = t q}

/-- The prime-indexed operators generate their Hecke subalgebra. -/
theorem hecke_mem_heckeSubalgebra {t : ℕ → Module.End F V₀} {q : ℕ}
    (hq : q.Prime) : t q ∈ heckeSubalgebra t :=
  Algebra.subset_adjoin ⟨q, hq, rfl⟩

/-- The Hecke subalgebra of a pairwise-commuting operator family is
commutative (two nested `adjoin` inductions). -/
theorem heckeSubalgebra_mul_comm {t : ℕ → Module.End F V₀}
    (hcomm : ∀ m n : ℕ, t m * t n = t n * t m) :
    ∀ a ∈ heckeSubalgebra t, ∀ b ∈ heckeSubalgebra t,
      a * b = b * a := by
  have hgen : ∀ (q : ℕ), q.Prime →
      ∀ a ∈ heckeSubalgebra t, t q * a = a * t q := by
    intro q hq a ha
    have ha' : a ∈ Algebra.adjoin F
        {φ : Module.End F V₀ | ∃ n : ℕ, n.Prime ∧ φ = t n} := ha
    clear ha
    induction ha' using Algebra.adjoin_induction with
    | mem x hx =>
        obtain ⟨n, hn, rfl⟩ := hx
        exact hcomm q n
    | algebraMap r => exact (Algebra.commutes r (t q)).symm
    | add x y hx hy ihx ihy => rw [mul_add, add_mul, ihx, ihy]
    | mul x y hx hy ihx ihy =>
        rw [← mul_assoc, ihx, mul_assoc, ihy, ← mul_assoc]
  intro a ha b hb
  have hb' : b ∈ Algebra.adjoin F
      {φ : Module.End F V₀ | ∃ n : ℕ, n.Prime ∧ φ = t n} := hb
  clear hb
...
/-- **Scalar-action induction**: if every prime-indexed operator acts
by left composition on the endomorphisms `e` and `a` through one
scalar system `lam`, then every member of the Hecke subalgebra acts
on `e` and `a` by a COMMON scalar. Instantiated at `a := e` it gives
the scalar action on the idempotent alone; with a genuine second
carrier it is the engine that identifies the joint eigenspace with
the idempotent's range in the Eichler–Shimura assembly (the "same
character on both carriers" trick: `c := e` forces `e * a = a`, then
`c := a` exhibits `a` as a scalar multiple of `e`). -/
theorem exists_smul_of_mem_heckeSubalgebra {t : ℕ → Module.End F V₀}
    {lam : ℕ → F} {e a : Module.End F V₀}
    (he : ∀ q : ℕ, q.Prime → t q * e = lam q • e)
    (ha : ∀ q : ℕ, q.Prime → t q * a = lam q • a) :
    ∀ c ∈ heckeSubalgebra t, ∃ γ : F, c * e = γ • e ∧ c * a = γ • a := by
  intro c hc
  have hc' : c ∈ Algebra.adjoin F
      {φ : Module.End F V₀ | ∃ q : ℕ, q.Prime ∧ φ = t q} := hc
  clear hc
  induction hc' using Algebra.adjoin_induction with
  | mem x hx =>
      obtain ⟨q, hq, rfl⟩ := hx
      exact ⟨lam q, he q hq, ha q hq⟩
  | algebraMap r =>
      refine ⟨r, ?_, ?_⟩ <;>
        rw [Algebra.algebraMap_eq_smul_one, smul_mul_assoc, one_mul]
  | add x y hx hy ihx ihy =>
      obtain ⟨γ₁, hx1, hx2⟩ := ihx
      obtain ⟨γ₂, hy1, hy2⟩ := ihy
      exact ⟨γ₁ + γ₂, by rw [add_mul, hx1, hy1, add_smul],
        by rw [add_mul, hx2, hy2, add_smul]⟩
  | mul x y hx hy ihx ihy =>
      obtain ⟨γ₁, hx1, hx2⟩ := ihx
      obtain ⟨γ₂, hy1, hy2⟩ := ihy
      refine ⟨γ₁ * γ₂, ?_, ?_⟩
      · rw [mul_assoc, hy1, mul_smul_comm, hx1, smul_smul,
          mul_comm γ₂ γ₁]
      · rw [mul_assoc, hy2, mul_smul_comm, hx2, smul_smul,
          mul_comm γ₂ γ₁]

/-- **Self-adjointness transport**: a bilinear pairing for which every
prime-indexed operator is self-adjoint is self-adjoint for the whole
Hecke subalgebra (commutativity of the family enters through the
product case). Applied to the Atkin–Lehner-twisted Weil pairing and
the newform idempotent in the Eichler–Shimura assembly. -/
theorem pair_comm_of_mem_heckeSubalgebra {t : ℕ → Module.End F V₀}
    (hcomm : ∀ m n : ℕ, t m * t n = t n * t m)
    {pair : V₀ →ₗ[F] V₀ →ₗ[F] F}
    (hpair : ∀ q : ℕ, q.Prime → ∀ x y : V₀,
      pair (t q x) y = pair x (t q y)) :
    ∀ c ∈ heckeSubalgebra t, ∀ x y : V₀,
      pair (c x) y = pair x (c y) := by
  intro c hc
  have hc' : c ∈ Algebra.adjoin F
      {φ : Module.End F V₀ | ∃ q : ℕ, q.Prime ∧ φ = t q} := hc
  clear hc
  ...
  | add φ ψ hφ hψ ihφ ihψ =>
      ...
      rw [ihφ, ihψ]
  | mul φ ψ hφ hψ ihφ ihψ =>
      intro x y
      have hφψ : φ * ψ = ψ * φ :=
        heckeSubalgebra_mul_comm hcomm φ hφ ψ hψ
      calc pair ((φ * ψ) x) y = pair (φ (ψ x)) y := rfl
        _ = pair (ψ x) (φ y) := ihφ _ _
        _ = pair x (ψ (φ y)) := ihψ _ _
        _ = pair x ((ψ * φ) y) := rfl
        _ = pair x ((φ * ψ) y) := by rw [hφψ]

/-- **Idempotent approximation in a commutative ring** (the classical
Newton iteration `y ↦ 3y² − 2y³`, run to nilpotent precision): any
element whose idempotency defect `y² − y` is nilpotent differs from an
honest idempotent by a multiple of the defect. Each iteration step
squares the defect (`y'² − y' = (y² − y)²·(4y² − 4y − 3)`), so
induction on the nilpotency exponent terminates. Pure commutative
algebra; the annihilator-idempotent engine below applies it to the
almost-idempotent produced by the Cayley–Hamilton determinant trick
inside a finite-dimensional commutative operator algebra. -/
theorem exists_isIdempotentElem_sub_mem_span_of_isNilpotent
    {R : Type*} [CommRing R] {y : R} (h : IsNilpotent (y * y - y)) :
    ∃ e : R, e * e = e ∧ e - y ∈ Ideal.span {y * y - y} := by
  obtain ⟨n, hn⟩ := h
  induction n generalizing y with
  | zero =>
    have h1 : (1 : R) = 0 := by simpa using hn
    have hall : ∀ x : R, x = 0 := fun x => by
  ...
  | succ n ih =>
    rcases n with _ | n
    · have h0 : y * y - y = 0 := by simpa using hn
      exact ⟨y, sub_eq_zero.mp h0,
        by rw [sub_self]; exact Ideal.zero_mem _⟩
    · set y' := 3 * y ^ 2 - 2 * y ^ 3 with hy'
      have hb : y' * y' - y' =
          (y * y - y) ^ 2 * (4 * y ^ 2 - 4 * y - 3) := by
        rw [hy']; ring
      have hpow : (y' * y' - y') ^ (n + 1) = 0 := by
        rw [hb, mul_pow, ← pow_mul]
        have h2 : 2 * (n + 1) = (n + 1 + 1) + n := by ring
        rw [h2, pow_add, hn, zero_mul, zero_mul]
      obtain ⟨e, hee, hspan⟩ := ih hpow
      refine ⟨e, hee, ?_⟩
      have h1 : e - y' ∈ Ideal.span {y * y - y} := by
        refine Ideal.span_singleton_le_span_singleton.mpr ?_ hspan
        exact ⟨(y * y - y) * (4 * y ^ 2 - 4 * y - 3), by rw [hb]; ring⟩
      have h2 : y' - y ∈ Ideal.span {y * y - y} :=
        Ideal.mem_span_singleton'.mpr ⟨1 - 2 * y, by rw [hy']; ring⟩
      have h3 : e - y = (e - y') + (y' - y) := by ring
      rw [h3]
      exact Ideal.add_mem _ h1 h2

/-- **The annihilator idempotent of a vector in a finite-dimensional
commutative Hecke subalgebra** — the pure-algebra engine behind the
étale newform idempotent of the modular-Jacobian package: for any
vector `v`, the Hecke subalgebra of a pairwise-commuting operator
family on a finite-dimensional space contains an idempotent `e`
FIXING `v` such that every member of the subalgebra annihilating `v`
becomes NILPOTENT after multiplication by `e`. Classically `e` is the
unit of the local factor of the Artinian algebra
`A = heckeSubalgebra t` supporting the character `a·v = χ(a)·v`; the
proof is quotient-free: the powers of the annihilator ideal
`m = Ann_A(v)` stabilize (their `F`-dimensions are an antitone
sequence of naturals), the Cayley–Hamilton determinant trick
(`Submodule.exists_sub_one_mem_and_smul_eq_zero_of_fg_of_le_smul`
applied to `m·m^k = m^k`) produces an almost-idempotent `y ≡ 1 mod m`
with `y·m^k = 0`, and the Newton iteration
(`exists_isIdempotentElem_sub_mem_span_of_isNilpotent`) corrects it
to an honest idempotent within `span {y² − y} ⊆ m`, preserving both
the fixing of `v` and the nilpotency of `m·e` (for `a ∈ m`, `(a·y)^k`
ends in `y·a^k = 0`). -/
theorem exists_idempotent_heckeSubalgebra_fixing
    [Module.Finite F V₀] {t : ℕ → Module.End F V₀}
    (hcomm : ∀ m n : ℕ, t m * t n = t n * t m) (v : V₀) :
    ∃ e ∈ heckeSubalgebra t, e * e = e ∧ e v = v ∧
      ∀ a ∈ heckeSubalgebra t, a v = 0 → IsNilpotent (a * e) := by
  classical
  letI : CommRing ↥(heckeSubalgebra t) :=
    { (inferInstance : Ring ↥(heckeSubalgebra t)) with
      mul_comm := fun a b =>
        Subtype.ext (heckeSubalgebra_mul_comm hcomm a.1 a.2 b.1 b.2) }
  haveI : IsNoetherian ↥(heckeSubalgebra t) ↥(heckeSubalgebra t) :=
    isNoetherian_of_tower F inferInstance
  -- the annihilator ideal of `v` inside the Hecke subalgebra
  let m : Ideal ↥(heckeSubalgebra t) :=
    { carrier := {a : ↥(heckeSubalgebra t) |
        (a : Module.End F V₀) v = 0}
      add_mem' := fun {a b} ha hb => by
        simp only [Set.mem_setOf_eq] at ha hb ⊢
        rw [AddMemClass.coe_add, LinearMap.add_apply, ha, hb, add_zero]
      zero_mem' := by
        simp only [Set.mem_setOf_eq, ZeroMemClass.coe_zero,
          LinearMap.zero_apply]
      smul_mem' := fun c a ha => by
        simp only [Set.mem_setOf_eq] at ha ⊢
        rw [smul_eq_mul, MulMemClass.coe_mul, Module.End.mul_apply, ha,
          map_zero] }
  -- the powers of `m` are decreasing …
  have hpowle : ∀ n : ℕ,
      (m ^ (n + 1) : Ideal ↥(heckeSubalgebra t)) ≤ m ^ n := fun n => by
    rw [pow_succ']
    exact Ideal.mul_le_left
  -- … so their `F`-dimensions stabilize
  have hex : ∃ n : ℕ,
      Module.finrank F
          ((m ^ (n + 1) : Ideal ↥(heckeSubalgebra t)).restrictScalars F) =
        Module.finrank F
          ((m ^ n : Ideal ↥(heckeSubalgebra t)).restrictScalars F) := by
    by_contra hne
    push Not at hne
    have hlt : ∀ n : ℕ,
        Module.finrank F
            ((m ^ (n + 1) :
              Ideal ↥(heckeSubalgebra t)).restrictScalars F) <
          Module.finrank F
            ((m ^ n : Ideal ↥(heckeSubalgebra t)).restrictScalars F) :=
      fun n => lt_of_le_of_ne
        (Submodule.finrank_mono fun x hx => hpowle n hx) (hne n)
    have hsum : ∀ n : ℕ,
        Module.finrank F
            ((m ^ n : Ideal ↥(heckeSubalgebra t)).restrictScalars F) +
              n ≤
          Module.finrank F
            ((m ^ 0 : Ideal ↥(heckeSubalgebra t)).restrictScalars F) := by
      intro n
      induction n with
      | zero => simp
      | succ n ih =>
    ...
    have := hsum
      (Module.finrank F
        ((m ^ 0 : Ideal ↥(heckeSubalgebra t)).restrictScalars F) + 1)
    omega
  obtain ⟨k₀, hk₀⟩ := hex
  have hstep : (m ^ (k₀ + 1) : Ideal ↥(heckeSubalgebra t)) = m ^ k₀ := by
    have h1 : (m ^ (k₀ + 1) :
        Ideal ↥(heckeSubalgebra t)).restrictScalars F =
        (m ^ k₀ : Ideal ↥(heckeSubalgebra t)).restrictScalars F :=
      Submodule.eq_of_le_of_finrank_eq (fun x hx => hpowle k₀ hx) hk₀
    exact Submodule.restrictScalars_injective F _ _ h1
  have hk : (m ^ (k₀ + 1 + 1) : Ideal ↥(heckeSubalgebra t)) =
      m ^ (k₀ + 1) := by
    calc (m ^ (k₀ + 1 + 1) : Ideal ↥(heckeSubalgebra t))
        = m ^ (k₀ + 1) * m := pow_succ m (k₀ + 1)
      _ = m ^ k₀ * m := by rw [hstep]
      _ = m ^ (k₀ + 1) := (pow_succ m k₀).symm
  -- Cayley–Hamilton determinant trick on `N := m ^ (k₀ + 1)`
  obtain ⟨y, hy1, hy0⟩ :=
    Submodule.exists_sub_one_mem_and_smul_eq_zero_of_fg_of_le_smul m
      (m ^ (k₀ + 1) : Ideal ↥(heckeSubalgebra t))
      (IsNoetherian.noetherian _)
      (by rw [Ideal.smul_eq_mul, ← pow_succ']; exact hk.ge)
  -- the idempotency defect of `y` is nilpotent
  have hdef : ((y * y - y : ↥(heckeSubalgebra t)) ^ (k₀ + 1)) = 0 := by
    have h1 : (y - 1) ^ (k₀ + 1) ∈
        (m ^ (k₀ + 1) : Ideal ↥(heckeSubalgebra t)) :=
      Ideal.pow_mem_pow hy1 _
    have h2 : y * (y - 1) ^ (k₀ + 1) = 0 := by
      have h3 := hy0 _ h1
      rwa [smul_eq_mul] at h3
    have hb : (y * y - y : ↥(heckeSubalgebra t)) = y * (y - 1) := by
      ring
    rw [hb, mul_pow, pow_succ, mul_assoc, h2, mul_zero]
  obtain ⟨e, hee, hspan⟩ :=
    exists_isIdempotentElem_sub_mem_span_of_isNilpotent ⟨k₀ + 1, hdef⟩
  obtain ⟨c, hc⟩ := Ideal.mem_span_singleton'.mp hspan
  -- `y` fixes `v`, hence so do the defect-corrections
  have hyv : ((y : ↥(heckeSubalgebra t)) : Module.End F V₀) v = v := by
    have h1 : ((y - 1 : ↥(heckeSubalgebra t)) : Module.End F V₀) v =
        0 := hy1
    rw [AddSubgroupClass.coe_sub, LinearMap.sub_apply,
      OneMemClass.coe_one, Module.End.one_apply, sub_eq_zero] at h1
    exact h1
  have hzv : ((y * y - y : ↥(heckeSubalgebra t)) : Module.End F V₀) v =
      0 := by
    rw [AddSubgroupClass.coe_sub, LinearMap.sub_apply,
      MulMemClass.coe_mul, Module.End.mul_apply, hyv, hyv, sub_self]
  have hev : ((e : ↥(heckeSubalgebra t)) : Module.End F V₀) v = v := by
    have h1 : ((e - y : ↥(heckeSubalgebra t)) : Module.End F V₀) v =
        0 := by
      rw [← hc, MulMemClass.coe_mul, Module.End.mul_apply, hzv,
        map_zero]
    rw [AddSubgroupClass.coe_sub, LinearMap.sub_apply, sub_eq_zero]
      at h1
    rw [h1, hyv]
  refine ⟨(e : Module.End F V₀), e.2, ?_, hev, ?_⟩
  · have h1 := congrArg (Subtype.val
      (p := fun x => x ∈ heckeSubalgebra t)) hee
    simpa using h1
  · intro a haA hav
    have ham : (⟨a, haA⟩ : ↥(heckeSubalgebra t)) ∈ m := hav
    set a' : ↥(heckeSubalgebra t) := ⟨a, haA⟩ with ha'
    have h1 : IsNilpotent (a' * y) := by
      refine ⟨k₀ + 1, ?_⟩
      have h2 : y * a' ^ (k₀ + 1) = 0 := by
        have h3 := hy0 _ (Ideal.pow_mem_pow ham (k₀ + 1))
        rwa [smul_eq_mul] at h3
      calc (a' * y) ^ (k₀ + 1)
          = y ^ k₀ * (y * a' ^ (k₀ + 1)) := by ring
        _ = 0 := by rw [h2, mul_zero]
    have h2 : IsNilpotent (a' * (e - y)) := by
      refine ⟨k₀ + 1, ?_⟩
      have hb2 : a' * (e - y) = (a' * c) * (y * y - y) := by
        rw [← hc]; ring
      rw [hb2, mul_pow, hdef, mul_zero]
    have h3 : IsNilpotent (a' * e) := by
      have h4 : a' * e = a' * y + a' * (e - y) := by ring
      rw [h4]
      exact (Commute.all _ _).isNilpotent_add h1 h2
    obtain ⟨n, hn⟩ := h3
    refine ⟨n, ?_⟩
    have h5 := congrArg (Subtype.val
...
/-- **The modular-Jacobian package at level `M`** — the `g`-INDEPENDENT
carrier one level below `EichlerShimuraPackage` (2026-07-24
decomposition). The intended inhabitant is again the rational `p`-adic
Tate module `Vp = V_p(J₀(M)) ⊗ ℚ̄_p` of `J₀(M) = Jac X₀(M)` with its
Galois action, Hecke operators and exceptional set — but where the
Eichler–Shimura package asserts the two `(g, κ)`-DEPENDENT spectral
facts (2-dimensionality of the joint eigenspace, determinant `q` of
Frobenius on it) as primitive fields, this package replaces them by
sharper `g`-independent module theory from which they are PROVEN:

* `hecke_mul_comm` — the Hecke algebra is commutative;
* `basis₁`, `basis₂`, `span_free`, `indep_free` — `Vp` is FREE OF
  RANK 2 over the base-changed Hecke algebra
  `𝕋 ⊗ ℚ̄_p = heckeSubalgebra hecke` (spelled without a module
  structure over the subalgebra: a two-element basis with existence
  and uniqueness of coordinates);
* `pair`, `pair_self`, `pair_nondeg`, `pair_hecke`, `pair_frob` — the
  Atkin–Lehner-twisted Weil pairing: an alternating nondegenerate
  bilinear form for which the Hecke operators are self-adjoint and
  Galois Frobenii off `S` scale by `q`;
* `eigen_idempotent` — for every level-`M` newform `g` and embedding
  `κ`, the `κ(a(g))`-eigensystem splits off as a nonzero idempotent of
  the Hecke subalgebra (the étale newform component of `𝕋 ⊗ ℚ̄_p`).
  This is quantified over `(g, κ)` INSIDE the structure — it is a
  fact about the one intended inhabitant, not about arbitrary
  carriers, so it must not be a standalone lemma quantifying over
  abstract packages (for those it would be false, e.g. `hecke = 0`).

From these, `nonempty_eichlerShimuraPackage` below derives the
eigenspace facts: the joint eigenspace equals `e·Vp`, which by
freeness is the 2-dimensional span of `e basis₁, e basis₂`, and the
twisted Weil pairing restricts to a symplectic form on it, forcing
`det Frob_q = q`. Since the 2026-07-24 third decomposition the
...
structure ModularJacobianPackage (M : ℕ) where
  ...
  /-- The étale newform idempotent: for every level-`M` newform and
  `p`-adic embedding of its Hecke field, the eigensystem splits off
  as a nonzero idempotent of the Hecke subalgebra. -/
  eigen_idempotent : ∀ (g : CuspForm (Gamma0GL M) 2),
...
/-- **The modular-homology package at level `M`** — the third-level,
`g`-INDEPENDENT carrier one level below `ModularJacobianPackage`
(2026-07-24 third decomposition). The intended inhabitant is the same
rational `p`-adic Tate module `Vp = V_p(J₀(M)) ⊗ ℚ̄_p` of the modular
Jacobian — equivalently, through the comparison isomorphism
`V_p(J₀(M)) ≅ H₁(X₀(M); ℚ) ⊗ ℚ_p`, the singular homology of the
modular curve with its Hecke and Galois structure, which is where the
classical verifications of the spectral fields actually happen. The
carrier differs from `ModularJacobianPackage` in exactly one place:
the ÉTALE NEWFORM IDEMPOTENT field `eigen_idempotent` — a composite
assertion mixing spectral occurrence, multiplicity one and Artinian
ring theory — is replaced by the two PRIMITIVE spectral facts from
which it is PROVEN by pure finite-dimensional commutative algebra
(the annihilator-idempotent engine
`exists_idempotent_heckeSubalgebra_fixing` together with the Newton
iteration `exists_isIdempotentElem_sub_mem_span_of_isNilpotent`):

* `eigensystem_occurs` — the eigensystem of a level-`M` newform
  OCCURS in `Vp`: a nonzero joint eigenvector exists;
* `eigensystem_semisimple` — STRONG MULTIPLICITY ONE at the newform
  eigensystem: every joint generalized eigenvector of the Hecke
  family is an honest eigenvector (equivalently, the
  newform-supporting local factor of the Artinian Hecke algebra
  `𝕋 ⊗ ℚ̄_p` is reduced, i.e. étale).

Both are facts about the one intended inhabitant, quantified over
`(g, κ)` INSIDE the structure exactly as `eigen_idempotent` was — for
  ...
  pair_frob : ∀ (q : ℕ) (hq : q.Prime),
    ...
    ∀ x y : Vp,
      pair (τJ (globalFrob hq.toHeightOneSpectrumRingOfIntegersRat) x)
          (τJ (globalFrob hq.toHeightOneSpectrumRingOfIntegersRat) y) =
        (q : AlgebraicClosure ℚ_[p]) * pair x y
  /-- **Occurrence of the newform eigensystem** (the Eichler–Shimura
  isomorphism): for every level-`M` newform and `p`-adic embedding of
  its Hecke field, the Tate module contains a nonzero joint
  eigenvector for the embedded eigensystem. Quantified over `(g, κ)`
  INSIDE the structure — a fact about the one intended inhabitant,
  not about arbitrary carriers. -/
  eigensystem_occurs : ∀ (g : CuspForm (Gamma0GL M) 2),
    IsWeightTwoNewform M g →
    ∀ (κ : heckeField M g →+* AlgebraicClosure ℚ_[p]),
      ∃ v ∈ heckeEigenspace hecke (fun m => κ (heckeCoeff M g m)),
        v ≠ 0
  /-- **Strong multiplicity one, module-theoretic form**: at the
  embedded eigensystem of a level-`M` newform, every joint
  GENERALIZED eigenvector of the Hecke family is an honest
  eigenvector — the newform-supporting local factor of the Artinian
  Hecke algebra is étale. Quantified over `(g, κ)` INSIDE the
  structure, like `eigensystem_occurs`. -/
  eigensystem_semisimple : ∀ (g : CuspForm (Gamma0GL M) 2),
    IsWeightTwoNewform M g →
    ∀ (κ : heckeField M g →+* AlgebraicClosure ℚ_[p]),
    ∀ x : Vp, (∀ q : ℕ, q.Prime → ∃ n : ℕ,
        ((hecke q - κ (heckeCoeff M g q) • 1) ^ n) x = 0) →
      x ∈ heckeEigenspace hecke (fun m => κ (heckeCoeff M g m))
  /-- Ribet irreducibility (Ribet 1977, Springer LNM 601, Thm. (2.3)):
  for every level-`M` newform and `p`-adic embedding of its Hecke
  field, the Galois action admits no proper nonzero stable subspace
...
module of the modular Jacobian `J₀(M) = Jac X₀(M)`, with
...
* `hecke m` the Hecke correspondence `T_m` (`U_q` at `q ∣ M`) acting
  ...
  `hecke_comm`;
* `hecke_mul_comm` — commutativity of the Hecke algebra (D–S §5.2,
  Prop. 5.2.4 and the `T_m T_n = T_n T_m` relations);
* `S := {v : v ∣ Mp}`;
...
* `basis₁`, `basis₂`, `span_free`, `indep_free` — FREENESS:
  ...
  of rank 1 over it); base change along `ℚ ⊂ ℚ̄_p` and the comparison
  `V_p(J₀(M)) ≅ H₁(X₀(M); ℚ) ⊗ ℚ_p` give `Vp` free of rank 2 over
  `𝕋 ⊗ ℚ̄_p`, which is exactly `heckeSubalgebra hecke` (prime
  generation: the Hecke recursions, D–S §5.2-5.3, faithfulness of the
  action on `Vp`);
...
* `spectrum_reduced` — STRONG MULTIPLICITY ONE (D–S
  ...
  local factor of the Artinian ring `𝕋 ⊗ ℚ̄_p` (its localization at
...
assembly over the single sorried inhabitation leaf
`nonempty_modularHomologyPackage`): for every level `M ≥ 1` the
carrier `ModularJacobianPackage M` is inhabited.

Assembly (pure finite-dimensional commutative algebra, no geometry):
every field except the étale newform idempotent passes through
verbatim from the modular-homology package. For `eigen_idempotent`,
given a newform `(g, κ)`:

* `eigensystem_occurs` provides a nonzero joint eigenvector `v`;
* the annihilator-idempotent engine
  `exists_idempotent_heckeSubalgebra_fixing` (Artinian stabilization
  of the powers of `m = Ann_A(v)` inside the finite-dimensional
  commutative algebra `A = heckeSubalgebra hecke`, the Cayley–
  Hamilton determinant trick, and the Newton idempotent iteration)
  yields an idempotent `e ∈ A` with `e v = v` — hence `e ≠ 0` — such
  that every member of `A` annihilating `v` is nilpotent after
  multiplication by `e`;
* applied to `hecke q − κ(a_q)·1 ∈ m`, nilpotency of
  `(hecke q − κ(a_q))·e` places the whole image `e·Vp` inside the
  joint GENERALIZED eigenspace of the eigensystem (commutativity
  rearranges `((hecke q − κ(a_q))·e)ⁿ⁺¹ = (hecke q − κ(a_q))ⁿ⁺¹·e`);
* `eigensystem_semisimple` (strong multiplicity one) upgrades
  generalized eigenvectors to honest ones, so
  `(hecke q − κ(a_q)·1) ∘ e = 0` as an endomorphism — exactly
  `hecke q * e = κ(a_q) • e`.

SOUNDNESS: unchanged — the classical citations live one level down,
in `nonempty_modularHomologyPackage`. -/
theorem nonempty_modularJacobianPackage {M : ℕ} (hM : 0 < M) :
    Nonempty (ModularJacobianPackage (p := p) M) := by
  classical
  obtain ⟨H⟩ := nonempty_modularHomologyPackage (p := p) hM
  have heigen : ∀ (g : CuspForm (Gamma0GL M) 2),
      IsWeightTwoNewform M g →
      ∀ (κ : heckeField M g →+* AlgebraicClosure ℚ_[p]),
        ∃ e ∈ heckeSubalgebra H.hecke, e * e = e ∧ e ≠ 0 ∧
          ∀ (q : ℕ), q.Prime →
            H.hecke q * e = κ (heckeCoeff M g q) • e := by
    intro g hg κ
    obtain ⟨v, hvEig, hvNe⟩ := H.eigensystem_occurs g hg κ
    obtain ⟨e, heA, hee, hev, hnil⟩ :=
      exists_idempotent_heckeSubalgebra_fixing H.hecke_mul_comm v
    have hvq : ∀ q : ℕ, q.Prime →
        H.hecke q v = κ (heckeCoeff M g q) • v :=
      mem_heckeEigenspace_iff.mp hvEig
    -- the image of `e` consists of generalized eigenvectors, hence
    -- by strong multiplicity one of honest eigenvectors
    have hgen : ∀ x : H.Vp, e x ∈ heckeEigenspace H.hecke
        (fun m => κ (heckeCoeff M g m)) := by
      intro x
      apply H.eigensystem_semisimple g hg κ
      intro q hq
      have hamem : H.hecke q - κ (heckeCoeff M g q) • 1 ∈
          heckeSubalgebra H.hecke :=
        sub_mem (hecke_mem_heckeSubalgebra hq)
          (Subalgebra.smul_mem _ (one_mem _) _)
      have hav : (H.hecke q - κ (heckeCoeff M g q) • 1) v = 0 := by
        rw [LinearMap.sub_apply, LinearMap.smul_apply,
          Module.End.one_apply, hvq q hq, sub_self]
      obtain ⟨n, hn⟩ := hnil _ hamem hav
      refine ⟨n + 1, ?_⟩
      have h6 : ((H.hecke q - κ (heckeCoeff M g q) • 1) * e) ^
          (n + 1) = 0 := by
        rw [pow_succ, hn, zero_mul]
      have h7 : (H.hecke q - κ (heckeCoeff M g q) • 1) ^ (n + 1) * e =
          0 := by
        have hcm : Commute (H.hecke q - κ (heckeCoeff M g q) • 1) e :=
          heckeSubalgebra_mul_comm H.hecke_mul_comm _ hamem e heA
        rwa [hcm.mul_pow, IsIdempotentElem.pow_succ_eq n hee] at h6
      calc ((H.hecke q - κ (heckeCoeff M g q) • 1) ^ (n + 1)) (e x)
          = (((H.hecke q - κ (heckeCoeff M g q) • 1) ^ (n + 1)) * e)
              x := rfl
        _ = 0 := by rw [h7, LinearMap.zero_apply]
    refine ⟨e, heA, hee, ?_, ?_⟩
    · intro h0
      rw [h0, LinearMap.zero_apply] at hev
      exact hvNe hev.symm
    · intro q hq
      refine LinearMap.ext fun x => ?_
      have h8 := mem_heckeEigenspace_iff.mp (hgen x) q hq
  ...
  exact ⟨{
    ...
    pair_hecke := H.pair_hecke
    pair_frob := H.pair_frob
    eigen_idempotent := heigen
    irred_eigenspace := H.irred_eigenspace }⟩

/-- **Inhabitation of the Eichler–Shimura package** (DECOMPOSED
2026-07-24 into the modular-Jacobian cut above and now a PROVEN
assembly over the single sorried inhabitation leaf
`nonempty_modularJacobianPackage`): every weight-2 newform `g` of
level `M ≥ 1` with a `p`-adic Hecke-field embedding `κ` admits an
Eichler–Shimura package.

Assembly (all module theory, no geometry): let `J` be a
modular-Jacobian package and `e` the newform idempotent of
`(g, κ)` (`J.eigen_idempotent`). Write `A = heckeSubalgebra J.hecke`
and `w₁ = e basis₁`, `w₂ = e basis₂`.

* *Eigenspace = `span {w₁, w₂}`.* `⊇`: `T_q e = κ(a_q) e` makes each
  `wᵢ` a joint eigenvector. `⊆`: an eigenvector's `A`-coordinates
  `a, b` (freeness) satisfy `T_q a = κ(a_q) a` in `A` (uniqueness
  half applied to `(T_q − κ(a_q))·coordinates`), and the
  scalar-action induction `exists_smul_of_mem_heckeSubalgebra` on
  the PAIR `(e, a)` forces first `e·a = a` (evaluate at `c := e`,
  where `e·e = e` pins the common scalar to `1` since `e ≠ 0`) and
  then `a = γ·e` (evaluate at `c := a`, commutativity of `A`) — so
  the eigenvector lies in `e·Vp = span {w₁, w₂}`.
* *`rank_eigenspace`*: `w₁, w₂` are linearly independent (a vanishing
  combination `c₁ w₁ + c₂ w₂ = 0` reads `(c₁ e) basis₁ +
  (c₂ e) basis₂ = 0`, so `cᵢ e = 0` by freeness and `cᵢ = 0` by
  `e ≠ 0`), so the eigenspace has a `Fin 2`-basis and rank `2`.
* *`det_frob`*: the twisted Weil pairing restricts nondegenerately to
  the eigenspace — `e` is self-adjoint
  (`pair_comm_of_mem_heckeSubalgebra`), `e` fixes `w₁`, and `e` maps
  all of `Vp` into `span {w₁, w₂}`, so `pair w₁ w₂ = 0` would make
  `w₁` orthogonal to everything (`pair_nondeg` then contradicts
  `e ≠ 0`). Expanding the restricted Frobenius in the basis
  `(w₁, w₂)` and using that the pairing is alternating,
  `pair (F w₁) (F w₂) = det(F|_W) · pair w₁ w₂`; `pair_frob` gives
  the same expression with multiplier `q`, and cancelling the
...
citations now live one level down, in
`nonempty_modularJacobianPackage`. The `hst`-quantified spelling of
`det_frob` asserts the determinant against EVERY stability proof;
`LinearMap.restrict` does not depend on that proof, so this is one
fact, not a family. -/
theorem nonempty_eichlerShimuraPackage {M : ℕ} (hM : 0 < M)
    {g : CuspForm (Gamma0GL M) 2} (hg : IsWeightTwoNewform M g)
    (κ : heckeField M g →+* AlgebraicClosure ℚ_[p]) :
    Nonempty (EichlerShimuraPackage M g κ) := by
  classical
  obtain ⟨J⟩ := nonempty_modularJacobianPackage (p := p) hM
  obtain ⟨e, heA, heIdem, heNe, heEig⟩ := J.eigen_idempotent g hg κ
  -- the Hecke subalgebra acts on `e`-multiples by scalars
  have hmul_e : ∀ c ∈ heckeSubalgebra J.hecke,
      ∃ γ : AlgebraicClosure ℚ_[p], c * e = γ • e := fun c hc =>
    ((exists_smul_of_mem_heckeSubalgebra heEig heEig) c hc).imp
      fun γ h => h.1
  -- joint eigen-elements of the Hecke subalgebra are multiples of `e`
  have hmul_of_eig : ∀ c : Module.End (AlgebraicClosure ℚ_[p]) J.Vp,
      c ∈ heckeSubalgebra J.hecke →
      (∀ q : ℕ, q.Prime →
        J.hecke q * c = κ (heckeCoeff M g q) • c) →
      ∃ γ : AlgebraicClosure ℚ_[p], c = γ • e := by
    intro c hcA hc_eig
    obtain ⟨γ₁, hγ₁e, hγ₁c⟩ :=
      exists_smul_of_mem_heckeSubalgebra heEig hc_eig e heA
    have hγ₁ : γ₁ = 1 := by
      have h1 : (γ₁ - 1) • e = 0 := by
        rw [sub_smul, one_smul, ← hγ₁e, heIdem, sub_self]
      rcases smul_eq_zero.mp h1 with h | h
      · exact sub_eq_zero.mp h
      · exact absurd h heNe
    have hec : e * c = c := by rw [hγ₁c, hγ₁, one_smul]
    obtain ⟨γ₂, hγ₂e, _⟩ :=
      exists_smul_of_mem_heckeSubalgebra heEig hc_eig c hcA
    refine ⟨γ₂, ?_⟩
    rw [← hγ₂e, heckeSubalgebra_mul_comm J.hecke_mul_comm c hcA e heA]
    exact hec.symm
  -- the `e`-image consists of joint eigenvectors
  have hw_mem : ∀ v : J.Vp, e v ∈ heckeEigenspace J.hecke
      (fun m => κ (heckeCoeff M g m)) := by
    intro v
    rw [mem_heckeEigenspace_iff]
    intro q hq
    have h1 : J.hecke q (e v) = (J.hecke q * e) v := rfl
    rw [h1, heEig q hq]
    simp
  -- the joint eigenspace is exactly the span of `e basis₁, e basis₂`
  have hspan_eq : heckeEigenspace J.hecke
        (fun m => κ (heckeCoeff M g m)) =
      Submodule.span (AlgebraicClosure ℚ_[p])
        {e J.basis₁, e J.basis₂} := by
    apply le_antisymm
    · intro x hx
      rw [mem_heckeEigenspace_iff] at hx
      obtain ⟨a, haA, b, hbA, rfl⟩ := J.span_free x
      have hcoord : ∀ q : ℕ, q.Prime →
          J.hecke q * a - κ (heckeCoeff M g q) • a = 0 ∧
          J.hecke q * b - κ (heckeCoeff M g q) • b = 0 := by
        intro q hq
        have hx' := hx q hq
        have hL : J.hecke q (a J.basis₁ + b J.basis₂) =
            (J.hecke q * a) J.basis₁ + (J.hecke q * b) J.basis₂ := by
          simp [Module.End.mul_apply]
        have hR : κ (heckeCoeff M g q) • (a J.basis₁ + b J.basis₂) =
            (κ (heckeCoeff M g q) • a) J.basis₁ +
              (κ (heckeCoeff M g q) • b) J.basis₂ := by
          simp [smul_add]
        rw [hL] at hx'
        rw [hR] at hx'
        have hzero : (J.hecke q * a - κ (heckeCoeff M g q) • a)
              J.basis₁ +
            (J.hecke q * b - κ (heckeCoeff M g q) • b) J.basis₂ =
              0 := by
          simp only [LinearMap.sub_apply]
          rw [sub_add_sub_comm, hx', sub_self]
        exact J.indep_free _
          (sub_mem (mul_mem (hecke_mem_heckeSubalgebra hq) haA)
            (Subalgebra.smul_mem _ haA _)) _
          (sub_mem (mul_mem (hecke_mem_heckeSubalgebra hq) hbA)
            (Subalgebra.smul_mem _ hbA _)) hzero
      have ha_eig : ∀ q : ℕ, q.Prime →
          J.hecke q * a = κ (heckeCoeff M g q) • a := fun q hq =>
        sub_eq_zero.mp (hcoord q hq).1
      have hb_eig : ∀ q : ℕ, q.Prime →
          J.hecke q * b = κ (heckeCoeff M g q) • b := fun q hq =>
        sub_eq_zero.mp (hcoord q hq).2
      obtain ⟨γa, ha_val⟩ := hmul_of_eig a haA ha_eig
      obtain ⟨γb, hb_val⟩ := hmul_of_eig b hbA hb_eig
    ...
    · rw [Submodule.span_le]
      rintro x (rfl | rfl)
      · exact hw_mem J.basis₁
      · exact hw_mem J.basis₂
  -- linear independence of the two spanning vectors
  have hw_indep : LinearIndependent (AlgebraicClosure ℚ_[p])
      ![e J.basis₁, e J.basis₂] := by
    rw [LinearIndependent.pair_iff]
    intro s t hst
    have h0 : (s • e) J.basis₁ + (t • e) J.basis₂ = 0 := by
      simpa using hst
    obtain ⟨hs, ht⟩ := J.indep_free _
      (Subalgebra.smul_mem _ heA s) _ (Subalgebra.smul_mem _ heA t) h0
    constructor
    ...
    · rcases smul_eq_zero.mp ht with h | h
      · exact h
      · exact absurd h heNe
  -- a `Fin 2`-basis of the eigenspace
  have hmem₁ : e J.basis₁ ∈ heckeEigenspace J.hecke
      (fun m => κ (heckeCoeff M g m)) := hw_mem J.basis₁
  have hmem₂ : e J.basis₂ ∈ heckeEigenspace J.hecke
      (fun m => κ (heckeCoeff M g m)) := hw_mem J.basis₂
  let v : Fin 2 → ↥(heckeEigenspace J.hecke
      (fun m => κ (heckeCoeff M g m))) :=
    ![⟨e J.basis₁, hmem₁⟩, ⟨e J.basis₂, hmem₂⟩]
  have hv_comp : (heckeEigenspace J.hecke
        (fun m => κ (heckeCoeff M g m))).subtype ∘ v =
      ![e J.basis₁, e J.basis₂] := by
    funext i
    fin_cases i <;> rfl
  have hv_indep : LinearIndependent (AlgebraicClosure ℚ_[p]) v := by
    ...
    rw [hv_comp]
    exact hw_indep
  have hv_span : ⊤ ≤ Submodule.span (AlgebraicClosure ℚ_[p])
      (Set.range v) := by
    rintro ⟨x, hx⟩ -
    ...
    have hxv : (⟨x, hx⟩ : ↥(heckeEigenspace J.hecke
        (fun m => κ (heckeCoeff M g m)))) = c₁ • v 0 + c₂ • v 1 := by
      apply Subtype.ext
      simp only [Submodule.coe_add, SetLike.val_smul, v,
  ...
  let bW : Module.Basis (Fin 2) (AlgebraicClosure ℚ_[p])
      ↥(heckeEigenspace J.hecke (fun m => κ (heckeCoeff M g m))) :=
    Module.Basis.mk hv_indep hv_span
  -- rank of the eigenspace
  have hrank : Module.rank (AlgebraicClosure ℚ_[p])
      ↥(heckeEigenspace J.hecke (fun m => κ (heckeCoeff M g m))) =
      2 := by
    rw [rank_eq_card_basis bW]
    simp
  -- skew-symmetry of the pairing
  have hskew : ∀ x y : J.Vp, J.pair y x = - J.pair x y := by
    intro x y
    have h0 := J.pair_self (x + y)
    simp only [map_add, LinearMap.add_apply] at h0
    rw [J.pair_self x, J.pair_self y] at h0
    linear_combination h0
  -- `e` is self-adjoint for the pairing
  have hsa : ∀ x y : J.Vp, J.pair (e x) y = J.pair x (e y) :=
    pair_comm_of_mem_heckeSubalgebra J.hecke_mul_comm J.pair_hecke
      e heA
  -- the `e`-image lies in the span of the two spanning vectors
  have h_e_range : ∀ y : J.Vp, ∃ c₁ c₂ : AlgebraicClosure ℚ_[p],
      e y = c₁ • e J.basis₁ + c₂ • e J.basis₂ := by
    intro y
    obtain ⟨a, haA, b, hbA, rfl⟩ := J.span_free y
    obtain ⟨γa, hγa⟩ := hmul_e a haA
    obtain ⟨γb, hγb⟩ := hmul_e b hbA
    refine ⟨γa, γb, ?_⟩
    have hcomm_a : e * a = a * e :=
      heckeSubalgebra_mul_comm J.hecke_mul_comm e heA a haA
    have hcomm_b : e * b = b * e :=
      heckeSubalgebra_mul_comm J.hecke_mul_comm e heA b hbA
    have hexp : e (a J.basis₁ + b J.basis₂) =
        (e * a) J.basis₁ + (e * b) J.basis₂ := by
      simp [Module.End.mul_apply]
    rw [hexp, hcomm_a, hcomm_b, hγa, hγb]
    simp
  -- nondegeneracy of the pairing on the eigenspace
  have hpair_ne : J.pair (e J.basis₁) (e J.basis₂) ≠ 0 := by
    intro h0
    have hee : e (e J.basis₁) = e J.basis₁ := by
      rw [← Module.End.mul_apply, heIdem]
    have hw₁ : ∀ y : J.Vp, J.pair (e J.basis₁) y = 0 := by
      intro y
      have h1 : J.pair (e J.basis₁) y =
          J.pair (e J.basis₁) (e y) := by
        calc J.pair (e J.basis₁) y
            = J.pair (e (e J.basis₁)) y := by rw [hee]
          _ = J.pair (e J.basis₁) (e y) := hsa _ _
      obtain ⟨c₁, c₂, hey⟩ := h_e_range y
      rw [h1, hey]
      simp [J.pair_self, h0]
    have hw₁0 : e J.basis₁ = 0 := J.pair_nondeg _ hw₁
    have hcontra := J.indep_free e heA 0 (zero_mem _)
      (by simp [hw₁0])
    exact heNe hcontra.1
  -- the determinant of the restricted Frobenius
  have hdet : ∀ (q : ℕ) (hq : q.Prime),
      hq.toHeightOneSpectrumRingOfIntegersRat ∉ J.S →
      ∀ (hst : ∀ x ∈ heckeEigenspace J.hecke
          (fun m => κ (heckeCoeff M g m)),
        J.τJ (globalFrob hq.toHeightOneSpectrumRingOfIntegersRat) x ∈
          heckeEigenspace J.hecke (fun m => κ (heckeCoeff M g m))),
      LinearMap.det
          ((J.τJ (globalFrob
            hq.toHeightOneSpectrumRingOfIntegersRat)).restrict hst) =
    ...
    intro q hq hqS hst
    have hb0 : ((bW 0 : ↥(heckeEigenspace J.hecke
        (fun m => κ (heckeCoeff M g m)))) : J.Vp) = e J.basis₁ := by
      simp [bW, Module.Basis.mk_apply, v]
    have hb1 : ((bW 1 : ↥(heckeEigenspace J.hecke
        (fun m => κ (heckeCoeff M g m)))) : J.Vp) = e J.basis₂ := by
      simp [bW, Module.Basis.mk_apply, v]
    set F := J.τJ (globalFrob hq.toHeightOneSpectrumRingOfIntegersRat)
      with hFdef
    set F' := F.restrict hst
    -- coordinates of the restricted Frobenius in the basis
    have hcoe : ∀ j : Fin 2, F ((bW j : ↥(heckeEigenspace J.hecke
          (fun m => κ (heckeCoeff M g m)))) : J.Vp) =
        bW.repr (F' (bW j)) 0 • e J.basis₁ +
          bW.repr (F' (bW j)) 1 • e J.basis₂ := by
      intro j
      have hsum := bW.sum_repr (F' (bW j))
      rw [Fin.sum_univ_two] at hsum
      have hval := congrArg
        (Submodule.subtype (heckeEigenspace J.hecke
          (fun m => κ (heckeCoeff M g m)))) hsum
      simp only [map_add, map_smul, Submodule.subtype_apply] at hval
      rw [hb0, hb1] at hval
      rw [hval]
      exact (LinearMap.coe_restrict_apply hst (bW j)).symm
    -- determinant via the 2×2 coordinate matrix
    have hdet2 : LinearMap.det F' =
        bW.repr (F' (bW 0)) 0 * bW.repr (F' (bW 1)) 1 -
          bW.repr (F' (bW 1)) 0 * bW.repr (F' (bW 0)) 1 := by
      rw [← LinearMap.det_toMatrix bW F', Matrix.det_fin_two,
        LinearMap.toMatrix_apply, LinearMap.toMatrix_apply,
        LinearMap.toMatrix_apply, LinearMap.toMatrix_apply]
    -- pairing of the Frobenius images, expanded in coordinates
    have hexp : J.pair (F (e J.basis₁)) (F (e J.basis₂)) =
        (bW.repr (F' (bW 0)) 0 * bW.repr (F' (bW 1)) 1 -
          bW.repr (F' (bW 1)) 0 * bW.repr (F' (bW 0)) 1) *
          J.pair (e J.basis₁) (e J.basis₂) := by
      have h0 := hcoe 0
      have h1 := hcoe 1
      rw [hb0] at h0
      rw [hb1] at h1
      ...
      rw [J.pair_self, J.pair_self,
        hskew (e J.basis₁) (e J.basis₂)]
      ring
    -- the Frobenius multiplier of the pairing
    have hfrob := J.pair_frob q hq hqS (e J.basis₁) (e J.basis₂)
    have hcancel : LinearMap.det F' *
        J.pair (e J.basis₁) (e J.basis₂) =
        (q : AlgebraicClosure ℚ_[p]) *
          J.pair (e J.basis₁) (e J.basis₂) := by
      rw [hdet2, ← hexp]
      rw [← hFdef] at hfrob
      exact hfrob
    exact mul_right_cancel₀ hpair_ne hcancel
  exact ⟨{ Vp := J.Vp
           τJ := J.τJ
           hecke := J.hecke
           S := J.S
...
`p`-old/`p`-new dichotomy spelled through the `p`-old degeneracy
levels, the only spelling available on a pin with no degeneracy maps).
If the IRREDUCIBLE representation `τ` matches the Hecke polynomials of
`g` away from a finite set and is equivalent, through `e`, to the base
change of the integral representation `ρ`, then `ρ` is NOT flat at `p`.
...
`exists_weightTwoNewform_of_weightTwoEigenform`), and `hpnew` forces
`ord_p M₀ = ord_p M ≥ 1`: any `M₀ ∣ M` of strictly smaller
`p`-valuation divides `M / p`, and `g₀` — an eigenform of that level,
sharing the away-from-`M` coefficients — would be exactly the
realization `hpnew` excludes. So `p ∣ M₀`, and `g₀` is a `p`-new
newform. Chebotarev density plus characteristic-zero Brauer–Nesbitt
identify `τ` with the semisimplification of the geometric attachment
`ρ_{g₀,λ}` from the charpoly matching, and the IRREDUCIBILITY of `τ`
upgrades this to `τ ≅ ρ_{g₀,λ}` itself (were `ρ_{g₀,λ}` reducible,
its semisimplification would be a sum of characters, not irreducible).
This is where `hirr` is load-bearing, and why this leaf keeps it while
the unramified sibling
`weightTwoNewform_not_dvd_level_of_isUnramifiedAt_of_isIrreducible`
needs no such care: unramifiedness passes to the semisimplification,
flatness does NOT — the local semisimplification of the Steinberg
configuration is crystalline, so only the honest equivalence
`τ ≅ ρ_{g₀,λ}` transports the flat model. Were `ρ` flat at `p`, then
through `e` the flat integral model would land on `ρ_{g₀,λ}`: the
finite flat prolongations demanded by `IsFlatAt` at every finite level
glue to a `p`-divisible group over `ℤ_p` whose generic fibre is
`ρ_{g₀,λ}|_{G_p}`, hence the latter is crystalline with Hodge–Tate
weights in `{0, 1}` (Raynaud/Fontaine; odd `p` keeps the dictionary in
its classical range `e = 1 < p − 1`, and at `p = 2` the flat/Steinberg
boundary needs the finer Fontaine–Laffaille/Breuil theory — this is
what `hpodd` buys). But Saito's local–global compatibility identifies
the Weil–Deligne parameter of `ρ_{g₀,λ}|_{G_p}` with the local
automorphic parameter of `g₀` at `p`, and for a `p`-new `g₀` that
parameter is never crystalline: for `p ∥ M₀` it is an unramified twist
of Steinberg — semistable with NONZERO monodromy
(Deligne–Rapoport/Langlands at weight 2) — and for `p² ∣ M₀` it is
ramified principal series or supercuspidal, not even semistable.
Contradiction, so `ρ` is not flat at `p`.

ROUTE AUDIT (2026-07-25, the dichotomy cut). The obvious mirror of
the away-from-`p` sibling — cutting at the NEWFORM level, as
`weightTwoNewform_not_dvd_level_of_isUnramifiedAt_of_isIrreducible`
does at `q ≠ p` — is NOT available at `p`: the at-`p` architecture
already carries its newform-level statement
`weightTwoNewform_not_dvd_level_p_of_isFlatAt_of_isIrreducible`
DOWNSTREAM of the existence form (it is PROVEN from it through
`eigensystem_minimal`), so a newform-level leaf would restate
verbatim a theorem this file already proves — a duplicated sorry, not
a cut. The cut is therefore made one hypothesis WEAKER than newform
minimality: `hpnew` demands minimality only against the levels
dividing `M / p` — the classical `p`-old degeneracy levels — instead
of against all proper divisor levels, so this leaf is STRICTLY
STRONGER than the downstream newform statement (which recovers it by
the descent above) and is a genuine literature node rather than a
re-derivation. Both local cases of Saito's computation deliberately
stay inside this ONE leaf: `p ∥ M₀` (Steinberg) and `p² ∣ M₀`
...
SOUNDNESS AUDIT (2026-07-25, both directions): non-vacuously
satisfiable — take `p = 11`, `M = 11` and `g` the weight-2 newform of
level `11` (the form of `X₀(11)`), `τ := ρ_{g,λ}` (irreducible by
Ribet) with a stable lattice over `R = 𝒪_λ`. Then `p ∣ M`, and
`hpnew` holds because `M / p = 1` and there is no weight-2 eigenform
of level `1` at all (PROVEN in this file:
`weightTwoEigenform_level_one_false`), so the only candidate level
`M₁ ∣ 1` is uninhabited; the conclusion also holds, `X₀(11)` having
multiplicative reduction at `11` (Tate curve, nonzero monodromy: no
finite flat prolongation of the `11`-power torsion). Conversely every
instance is an instance of the cited theorems: the statement
quantifies over the `IsWeightTwoEigenform` carrier, whose inhabitants
are exactly the classical normalized eigenforms (carrier audit at
...
local–global compatibility in existence form** (DECOMPOSED 2026-07-25
into the `p`-new literature leaf
`not_isFlatAt_of_weightTwoEigenform_pNew_of_isIrreducible` above and
now a PROVEN assembly over the `p`-old/`p`-new dichotomy): if the
IRREDUCIBLE representation `τ` matches the Hecke polynomials of the
weight-2 EIGENFORM `g` of level `M ≥ 1` away from a finite set and is
...
Assembly (the dichotomy, all four steps PROVEN):
* `p ∤ M`: nothing to lower — `(M, g)` itself witnesses the
  conclusion, with `rfl` agreement.
* Otherwise, behind `g` lies a newform `g₀` of level `M₀ ∣ M` with the
  same away-from-`M` eigensystem (PROVEN newform descent
  `exists_weightTwoNewform_of_weightTwoEigenform`, Diamond–Shurman
  Prop. 5.8.4), and the `p`-adic embedding `κ` transports to an
  embedding `κ₀` of `g₀`'s Hecke field agreeing on the shared good
  coefficients (PROVEN `exists_ringHom_heckeField_of_qCoeff_eq`), so
  `τ` matches `g₀`'s Hecke polynomials away from `S_τ` together with
  the primes dividing `M`.
* `p ∤ M₀` — the `p`-OLD case: `(M₀, g₀)` witnesses the conclusion.
  This is where the level lowering actually happens, and it is pure
  degeneracy-level bookkeeping: the descent already lowered the level,
  the dichotomy only observes that the new level is prime to `p`.
* `p ∣ M₀` — the `p`-NEW case: `g₀`'s newform minimality
  `eigensystem_minimal` gives exactly the `p`-newness hypothesis of
  the leaf (a level dividing `M₀ / p` is a proper divisor of `M₀`,
  because `M₀ / p < M₀`), and the leaf then contradicts the flatness
  of `ρ`.

All literature content now lives in that single sorried leaf (Saito
1997 local–global compatibility at `p`, plus Raynaud/Fontaine for the
flat side); the dichotomy, the newform descent, the embedding
transport, the charpoly matching bookkeeping and the `p`-newness
derivation are PROVEN. `hirr` remains LOAD-BEARING and is handed to
the leaf untouched: flatness does not pass to the semisimplification
(the local semisimplification of the Steinberg configuration is
crystalline), so only the honest equivalence `τ ≅ ρ_{g₀,λ}` — which
needs the irreducibility of `τ` — transports the flat model, whereas
the unramified sibling
`exists_weightTwoEigenform_not_dvd_level_of_isUnramifiedAt` can do
without it.
...
theorem exists_weightTwoEigenform_not_dvd_level_p_of_isFlatAt_of_isIrreducible
    ...
    ∃ (M' : ℕ) (_ : M' ∣ M) (_ : ¬ p ∣ M')
      ...
      ∀ (r : ℕ), r.Prime → ¬ r ∣ M → qCoeff M' g' r = qCoeff M g r := by
  classical
  -- step 0: if `p ∤ M` there is nothing to lower
  by_cases hpM : p ∣ M
  swap
  · exact ⟨M, dvd_rfl, hpM, g, hg, fun r _ _ => rfl⟩
  ...
  -- step 2: the `p`-old case — the descended level is already prime to `p`
  by_cases hpM₀ : p ∣ M₀
  swap
  · exact ⟨M₀, hM₀dvd, hpM₀, g₀, hg₀.toIsWeightTwoEigenform, hagree⟩
  -- step 3: the `p`-new case is excluded by flatness
  exfalso
  -- transport the `p`-adic embedding to the newform's Hecke field
  obtain ⟨κ₀, hκ₀⟩ := exists_ringHom_heckeField_of_qCoeff_eq hM₀pos
    hg₀.toIsWeightTwoEigenform κ hagree
  -- `τ` matches `g₀`'s Hecke polynomials away from `S_τ ∪ (primes ∣ M)`
  ...
  -- newform minimality at `M₀` IS the `p`-newness the leaf demands
  have hpnew : ∀ M₁ : ℕ, M₁ ∣ M₀ / p →
      ∀ g₁ : CuspForm (Gamma0GL M₁) 2, IsWeightTwoEigenform M₁ g₁ →
      ¬ ∀ (r : ℕ), r.Prime → ¬ r ∣ M₀ → qCoeff M₁ g₁ r = qCoeff M₀ g₀ r := by
    intro M₁ hM₁dvd g₁ hg₁
    ...
    have hlt : M₁ < M₀ :=
      lt_of_le_of_lt
        (Nat.le_of_dvd (Nat.div_pos (Nat.le_of_dvd hM₀pos hpM₀) hp.out.pos)
          hM₁dvd)
        (Nat.div_lt_self hM₀pos hp.out.one_lt)
    exact hg₀.eigensystem_minimal M₁ (hM₁dvd.trans hdvd₀) hlt.ne g₁ hg₁
  exact not_isFlatAt_of_weightTwoEigenform_pNew_of_isIrreducible hpodd hM₀pos
    hpM₀ hg₀.toIsWeightTwoEigenform hpnew κ₀ hτ₀ hirr e he hflat
...
displacement landing in `span {w₀}` (`hquotline`), then the
...
(RESTATED and PROVEN 2026-07-24 as an assembly over the exponent-form
level-lowering leaf
`exists_weightTwoEigenform_not_four_dvd_level_of_inertia_fixed_line`
above, converging the per-place conductor cluster on the ONE
level-lowering-plus-minimality citation shape of
`weightTwoNewform_not_dvd_level_of_isUnramifiedAt`): if the
representation `τ` attached (in the charpoly-matching sense) to the
weight-2 NEWFORM `g` of level `M` has a NONZERO vector `w₀` fixed by
every element of the inertia at `2` (`hfixline`) and every inertia
element moves every vector by a multiple of `w₀` (`hquotline`), then
`4 ∤ M`. Assembly: were `4 ∣ M`, the level-lowering leaf would
produce an eigenform `g'` of a level `M' ∣ M` with `4 ∤ M'` — hence
`M' ≠ M` — agreeing with `g` at every prime not dividing `M`, which
is exactly what the newform carrier's minimality field
`eigensystem_minimal` excludes. All literature content (Ribet
irreducibility, Chebotarev/Brauer–Nesbitt rigidity, the Artin
exponent bookkeeping and Carayol's conductor formula at `2`) lives in
the existence leaf; the newform-minimality arithmetic is proven
here. -/
theorem weightTwoNewform_not_four_dvd_level_of_inertia_fixed_line
    ...
    ¬ (4 ∣ M) := by
  intro h4M
  obtain ⟨M', hM'dvd, h4M', g', hg', hagree⟩ :=
    exists_weightTwoEigenform_not_four_dvd_level_of_inertia_fixed_line
      hpodd hM hg.toIsWeightTwoEigenform κ hτ w₀ hw₀ hfixline hquotline
  exact hg.eigensystem_minimal M' hM'dvd (fun h => h4M' (h.symm ▸ h4M))
    g' hg' hagree

/-
Modularity/PatchingVendored/AdicTopology.lean — VENDORED from the FLT project (ImperialCollegeLondon/FLT),
`FLT/Patching/Utils/AdicTopology.lean`, at commit e00fe1c4 (mathlib pin 81a5d25), adapted to this
...
The class `IsLocalRing.IsAdicTopology R` recording that the topology on a
local ring `R` is the adic topology associated to its maximal ideal,
together with basic openness lemmas.
-/

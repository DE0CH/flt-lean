/-
Modularity/PatchingVendored/TopologicallyFG.lean — VENDORED from the FLT project (ImperialCollegeLondon/FLT),
`FLT/Patching/Utils/TopologicallyFG.lean`, at commit e00fe1c4 (mathlib pin 81a5d25), adapted to this
...
The class `Algebra.TopologicallyFG R S` asserting that an `R`-algebra `S`
is topologically generated by a finite set, together with basic
consequences.
-/
...
/-- An `R`-algebra `S` is topologically finitely generated if there is a finite subset
`s ⊆ S` such that the `R`-subalgebra generated by `s` is dense in `S`. -/

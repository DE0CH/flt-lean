...
this way — `exists_heckeSubfield_of_determinants` (places over `ℓ`) and
`exists_threeadic_realization_of_heckePackage` (places over `3`).
...
This is the whole cost of the `hbadℓ`/`hbad3` narrowings below: because a
matching clause `∀ w ∉ badF, …` only WEAKENS as `badF` grows, the consumer
can always move to `badF'` and hand the citation the hypothesis it now
demands. Nothing downstream assumes more. -/
...
intermediate-field form** (sorry node; sub-leaf (b-i-a) — the ONLY
genuinely automorphic input of the whole modularity-lifting cut): the
...
theorem exists_heckeSubfield_of_eigenvalues
    ...
    ∃ E : IntermediateField ℚ (AlgebraicClosure ℚ_[ℓ]),
      FiniteDimensional ℚ E ∧ ∀ w ∉ badF, aF w ∈ E :=
  sorry

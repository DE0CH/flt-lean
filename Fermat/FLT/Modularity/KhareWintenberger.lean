...
* `heckePoly_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint` — the
  determinant (Weil-pairing) clause for the compatible system of the
  Hilbert–Blumenthal abelian variety, the ONLY sorried leaf left here;
...
WHY THE NEW LEAF KEEPS THE WHOLE HYPOTHESIS PACKAGE.  Stated for a bare
abstract `pt`, "the constant coefficient of `P w` is `Nw`" would be FALSE
as stated: the interface constrains `P` not at all, so a point whose
`p`-adic determinant is not cyclotomic refutes it, and such a point is
excluded by nothing in `HilbertBlumenthalPoint`.  It is only the parent's
package — an irreducible hardly ramified mod-`ℓ` representation at
`ℓ ≥ 5`, classically unsatisfiable (headline below) — that makes it
classically true, exactly as for its two siblings.  The package is
therefore carried verbatim onto the new leaf, which inherits the same
both-ways soundness audit.
...
/-- **The determinant clause of the Hilbert–Blumenthal system** (SORRY
NODE — the entire formal content of joint (a), isolated 2026-07-25): away
from the bad set, the constant coefficient of the Frobenius characteristic
polynomial `P w` of the compatible system carried by a
`HilbertBlumenthalPoint` is the absolute norm `Nw`.

Classically this is the Weil pairing.  `P w` is the characteristic
polynomial of `Frob_w` on the (rank-`2`, over the real-multiplication
field `D`) Tate module of the Hilbert–Blumenthal abelian variety `A/F`;
its constant coefficient is `det Frob_w`, and the Weil pairing
`T(A) × T(A^∨) → T(𝔾_m)` together with the `D`-polarization identifies
`det` with the cyclotomic character, whose value at an arithmetic
Frobenius at `w` is `Nw`.  Equivalently: `A` has parallel weight `2`, so
its Hecke polynomials are `X² − a_w·X + Nw` — the normalization every
consumer of this cut assumes.

WHY IT IS A LEAF AND NOT A THEOREM HERE.  `HilbertBlumenthalPoint` records
`P` as unconstrained data: nothing in the interface says `P w` is a
characteristic polynomial of anything, let alone that its determinant is
cyclotomic.  So this clause is underivable from the interface, and it is
the ONLY thing joint (a) — and, by the derivation in the section note
before `exists_heckeSystem_of_residualModularity`, joint (b) too — asks
for beyond junk.

FAITHFULNESS (why the hypothesis package is carried).  For a bare abstract
`pt` the statement is FALSE: `P` is free data, so a point whose `p`-adic
determinant is not cyclotomic refutes it, and the interface excludes no
such point.  Carrying the parent's package — an irreducible hardly
ramified mod-`ℓ` representation with `ℓ ≥ 5` — makes it classically true
by collapse (that package is classically unsatisfiable; headline below),
exactly as for its two siblings.  Do NOT restate this leaf without the
package.

SOUNDNESS AUDIT (both ways, 2026-07-25): (i) direct — for the intended
instantiation (a point produced by
`exists_hilbertBlumenthalPoint_of_five_le`, whose `P` really is the family
of Frobenius characteristic polynomials of a polarized abelian variety
with real multiplication) this is the Weil pairing, as above; for an
abstract point the abstract-quantification caveat applies IN FULL FORCE;
(ii) collapse — the hypothesis package is classically unsatisfiable, so
the statement is classically true for every package.

INTENDED DISCHARGE (a cut-level repair, not performed here): make this a
FIELD of `HilbertBlumenthalPoint`.  Its producer
`exists_twistedHilbertBlumenthalModuli_of_five_le` concludes
`Nonempty (HilbertBlumenthalPoint …)`, so the new field would strengthen
that leaf's obligation without changing its statement text, and it is
there — in the moduli construction — that the Weil pairing on `A[p]` is
available.

ROUTE AUDIT: the odd-prime dichotomy is unavailable here — see the section
docstring above (import cycle AND declaration cycle).

CIRCULARITY GUARD (inherited from pillar β, load-bearing): no discharge
through `Family.lean`, `Lift.lean`, or `Modularity/Interface.lean`. -/
theorem heckePoly_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint
    {ℓ : ℕ} (hℓodd : Odd ℓ) [Fact ℓ.Prime] (hℓ5 : 5 ≤ ℓ)
    {O : Type u} [CommRing O] [IsDomain O] [TopologicalSpace O]
    [IsTopologicalRing O] [Algebra ℤ_[ℓ] O] [IsLocalRing O]
    [Module.Finite ℤ_[ℓ] O] [IsModuleTopology ℤ_[ℓ] O]
    ...
    ∀ w ∉ pt.bad, (pt.P w).coeff 0 = (Ideal.absNorm w.asIdeal : pt.D) :=
  sorry
...
derivation recorded in the section note before that leaf. -/
theorem residual_charFrob_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint
    ...
    (hP : ∀ w ∉ pt.bad, (pt.P w).coeff 0 = (Ideal.absNorm w.asIdeal : pt.D)) :
    ∀ w ∉ pt.bad,
...
`heckePoly_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint` and the junk
coefficient ring `exists_numberField_surjection_of_finite` — see the
section note immediately above; formerly a sorry node, joint (a) of the
...
trace and `S₁ := pt.bad`.  What survives is literally the single equation
`(pt.ρbarp.charFrob w).coeff 0 = (Ideal.absNorm w.asIdeal : pt.kp)`,
supplied by the new leaf
`heckePoly_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint` through the
PROVEN descent
`residual_charFrob_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint`.
...
theorem exists_residualModularity_of_hilbertBlumenthalPoint
  ...
  have hnorm :=
    residual_charFrob_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint pt
      (heckePoly_coeff_zero_eq_absNorm_of_hilbertBlumenthalPoint hℓodd hℓ5
        hZinj hrank hρ hW hρbar hirr π hπsurj hπ F hFtr hFgal pt)
  ...
  refine ⟨E₁, hE₁, hNE₁, Λ, hΛ, jΛ, hjΛ, redΛ, a₁, pt.bad, fun w hw => ?_⟩

...
-- `HeckeQExpansion` was hoisted out of `Interface.lean` by one branch (19
-- declarations) and this module by another (196), the smaller a strict SUBSET of
-- the larger, and `Interface.lean` `public import`s BOTH -- so all 19 collided as
-- `environment already contains …`.  Per CLAUDE.md's own prescription for this
-- shape, both modules are kept and the LARGER imports the smaller; the 19
-- duplicate blocks were deleted from here (release 31).  18 of them had
-- byte-identical bodies; `qCoeff_heckeOp` had the same statement with a
-- different proof, and the upstream copy is the one kept.
public import Fermat.FLT.Modularity.HeckeQExpansion

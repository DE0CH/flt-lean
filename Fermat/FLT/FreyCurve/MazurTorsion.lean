...
theorem WeierstrassCurve.exists_stable_cyclic_subgroup_of_rational_point
  ...
  · intro σ x hx
    ...
    -- REPAIR 2026-07-26: the previous one-line
    -- `rw [map_zsmul, Affine.Point.map_baseChange … Q]` errored on `main`
    -- ("did not find an occurrence of the pattern", with a goal that displays
    -- as literally the pattern) — `rw` was elaborating `map_baseChange`'s
    -- implicit base ring/scalar-tower arguments differently from the way they
    -- occur in the goal. Stating the fixed-point equation as its own `have`,
    -- so its elaboration is driven by the expected type spelled in the same
    -- notation as the goal, makes the rewrite match.

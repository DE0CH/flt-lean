...
with `b ≠ 0`, consistent with the two levels below.

UPDATE 2026-07-26: level `11` is no longer a leaf. `F₁₁` is birationally
`11a3 : y² + y = x³ − x²` by an identity that now compiles
(`x1Eleven_plane_ne_zero`), so the whole level rests on the single
Mordell–Weil statement `x1Eleven_11a3_x_eq_zero_or_one`. Level `13` is
still open in plane form; the same reduction is available there in
principle (`X_1(13)` has genus `2`, so its target is a Jacobian, not an
elliptic curve). -/
...
/-- **`X_1(11)` IN ITS OWN MINIMAL WEIERSTRASS MODEL: the elliptic curve
`11a3`, `y² + y = x³ − x²`, has exactly five rational points** (sorry
node — the sole arithmetic input of level `11`, and now the ONLY thing
left of it; created 2026-07-26 when `x1Eleven_plane_ne_zero` was PROVEN
over it).

STATEMENT. Every rational point of the affine curve `y² + y = x³ − x²`
has `x = 0` or `x = 1`. Together with the point at infinity that makes
`E(ℚ) = {O, (0,0), (0,−1), (1,0), (1,−1)} ≅ ℤ/5`, generated by `(0,0)`.

WHY THIS IS THE RIGHT SHAPE. This IS `X_1(11)`: the birational map from
the plane quintic `F₁₁` of `MazurX1Plane.eval_eleven` to this curve is
written out and PROVEN in `x1Eleven_plane_ne_zero` below, so the whole
level-`11` obligation has been reduced to Cremona's curve `11a3` in its
standard minimal model, where the literature is directly citable. The
five rational points are the five rational cusps of `X_1(11)`
(`φ(11)/2 = 5`), and the `b ≠ 0` chart of the Tate family misses all of
them — which is exactly what the reduction below exploits.

WHAT IS ACTUALLY MISSING is one Mordell–Weil computation:
`rank 11a3(ℚ) = 0`. The curve has conductor `11`, discriminant `−11`,
`L(E, 1) = 0.2538… ≠ 0` (analytic rank `0`), `Ш = 1`, and torsion `ℤ/5`.
Rank `0` is provable by a complete `2`-descent — the `2`-Selmer group is
trivial — but that descent runs over the cubic field
`ℚ[X]/(4X³ − 4X² + 1)` (discriminant `−176 = −16·11`), so it needs the
unit and class groups of a specific cubic field, which mathlib cannot
compute. The `5`-isogeny descent (`11a3 → 11a1 = X_0(11)`, kernel
`⟨(0,0)⟩ ≅ ℤ/5`) is the classical alternative and needs a descent map
into `ℚ*/(ℚ*)⁵` unramified outside `{5, 11}`. Either route is a genuine
piece of arithmetic geometry, not a manipulation.

CITATION: Billing–Mahler, "On exceptional points on cubic curves"
(J. London Math. Soc. 15, 1940) — the original proof that no elliptic
curve over `ℚ` has a rational point of order `11`; Cremona's tables,
curve `11a3`; subsumed in Mazur 1977, Thm 7. The `X_0` shortcut is NOT
available, since `11` is in Kenku's list and `X_0(11)` has three
non-cuspidal rational points.

CAS RECONNAISSANCE (untrusted searcher, never a proof; all of it is
statement checking, 2026-07-26). PARI/GP: `ellrank` PROVES rank `0`
(lower bound `0`, upper bound `0`), `elltors` returns `[5]` generated by
`(0,0)`, and `ellratpoints(E, 100)` returns exactly
`(0,0), (0,−1), (1,0), (1,−1)`. Magma: the plane quintic `F₁₁ = 0` is
irreducible of genus `1` and its function field has exactly FIVE
degree-`1` places, on each of which `b` is `0` or `∞` — an independent
confirmation, in the `(b, c)`-coordinates themselves, that the affine
chart `b ≠ 0` contains no rational point. -/
theorem WeierstrassCurve.x1Eleven_11a3_x_eq_zero_or_one (x y : ℚ)
    (h : y ^ 2 + y = x ^ 3 - x ^ 2) : x = 0 ∨ x = 1 :=
  sorry

/-- **`X_1(11)` has no non-cuspidal rational point, as a plane quintic**
(PROVEN 2026-07-26, over `x1Eleven_11a3_x_eq_zero_or_one` — level `11`
of the seven-level node below, in the explicit `(b, c)`-coordinates).
...
`X_1(11)` and nothing more.

THE PROOF is the classical birational reduction of the `(b, c)`-model of
`X_1(N)` to a Weierstrass model, carried out here in full and verified
by the compiler at every step. Nothing is cited except the final
Mordell–Weil fact, which is isolated in
`x1Eleven_11a3_x_eq_zero_or_one` above.

1. `c ≠ 0`, because `F₁₁(b, 0) = −b⁵`.
2. `b ≠ c`, because `F₁₁(c, c) = c⁸`.
3. `b ≠ c² + c`, because `F₁₁(c² + c, c) = −c¹⁰`. (This is the `X_1(6)`
   line `MazurX1Plane.eval_six`, and it is where the cusp sits.)
4. Kubert's coordinates. With `r = b/c` and `s = c/(r − 1)` — both
   legitimate by 1 and 2 — one has the IDENTITY, over `ℚ(r, s)`,

     `F₁₁(rs(r−1), s(r−1)) = (s(r−1))⁵ · (r−1)³ · (rs³ − 3rs² + 4rs − s − r²)`,

   so `F₁₁ = 0` becomes the bidegree-`(2, 3)` curve
   `r² − r·s(s² − 3s + 4) + s = 0`. Completing the square in `r` turns
   it into `v² = s(s−1)²(s³ − 4s² + 8s − 4)`, and dividing out the
   square factor gives the QUARTIC model
   `w² = s⁴ − 4s³ + 8s² − 4s` — the classical genus-`1` model of
   `X_1(11)`, with the five cusps at `s = 0`, `s = 1` (twice) and
   `s = ∞` (twice).
5. `x = (s−1)/s`, `y = (w − s²)/(2s²)` sends that quartic to
   `y² + y = x³ − x²`, since `(w² − s⁴)/(4s⁴) = −(s−1)²/s³` is the
   quartic relation rewritten. Composing 4 and 5 and clearing
   denominators gives the two rational functions used in the proof
   below,

     `x = (c² + c − b)/c²`,
     `y = P(b, c) / (c⁵(b − c² − c))`,
     `P = −b⁴ + 3cb³ + (2c³ − 3c²)b² + (−2c⁵ − 4c⁴ + c³)b + c⁵(c² + 2c + 2)`,

   whose defining identity is the polynomial identity

     `P² + P·c⁵u + c⁴u⁵ + c⁶u⁴ = −(b − c)³ · F₁₁`,  `u = b − c² − c`,

   verified here by `ring` inside `field_simp`/`linear_combination`.
6. On the resulting rational point, `x = 1` says `b = c` (excluded by 2)
   and `x = 0` says `b = c² + c` (excluded by 3). So
   `x1Eleven_11a3_x_eq_zero_or_one` is contradicted, and `F₁₁ ≠ 0`.

Note the map is defined precisely where steps 1–3 say it is: the three
degeneracies of the coordinate change are exactly the three loci on
which `F₁₁` degenerates to a monomial, which is why no case is lost. -/
theorem WeierstrassCurve.x1Eleven_plane_ne_zero (b c : ℚ)
    ...
    -b ^ 5 + 3 * c * b ^ 4 + (4 * c ^ 3 - 3 * c ^ 2) * b ^ 3
      + (-3 * c ^ 5 - 9 * c ^ 4 + c ^ 3) * b ^ 2
      + (c ^ 7 + 3 * c ^ 6 + 6 * c ^ 5) * b - c ^ 6 ≠ 0 := by
  intro hF
  -- `F₁₁(b, 0) = −b⁵`, so the chart `b ≠ 0` forces `c ≠ 0`.
  have hc : c ≠ 0 := by
    intro hc0
    rw [hc0] at hF
    exact hb ((pow_eq_zero_iff (n := 5) (by norm_num)).mp (by linear_combination -hF))
  -- `F₁₁(c, c) = c⁸`: the coordinate `r = b/c` never takes the value `1`.
  have hbc : b - c ≠ 0 := by
    intro hbc0
    have hbe : b = c := by linarith
    rw [hbe] at hF
    exact hc ((pow_eq_zero_iff (n := 8) (by norm_num)).mp (by linear_combination hF))
  -- `F₁₁(c² + c, c) = −c¹⁰`: the coordinate `s` never takes the value `1`.
  have hu : b - c ^ 2 - c ≠ 0 := by
    intro hu0
    have hbe : b = c ^ 2 + c := by linarith
    rw [hbe] at hF
    exact hc ((pow_eq_zero_iff (n := 10) (by norm_num)).mp (by linear_combination -hF))
  -- The birational map to `11a3`, and the identity that makes it one.
  have key : ((-b ^ 4 + 3 * c * b ^ 3 + (2 * c ^ 3 - 3 * c ^ 2) * b ^ 2
        + (-2 * c ^ 5 - 4 * c ^ 4 + c ^ 3) * b + (c ^ 7 + 2 * c ^ 6 + 2 * c ^ 5))
        / (c ^ 5 * (b - c ^ 2 - c))) ^ 2
      + (-b ^ 4 + 3 * c * b ^ 3 + (2 * c ^ 3 - 3 * c ^ 2) * b ^ 2
        + (-2 * c ^ 5 - 4 * c ^ 4 + c ^ 3) * b + (c ^ 7 + 2 * c ^ 6 + 2 * c ^ 5))
        / (c ^ 5 * (b - c ^ 2 - c))
      = ((c ^ 2 + c - b) / c ^ 2) ^ 3 - ((c ^ 2 + c - b) / c ^ 2) ^ 2 := by
    field_simp
    linear_combination (-(b - c) ^ 3) * hF
  rcases WeierstrassCurve.x1Eleven_11a3_x_eq_zero_or_one _ _ key with hx | hx
  · -- `x = 0` is the cusp `b = c² + c`, excluded above.
    refine hu ?_
    field_simp at hx
    linarith
  · -- `x = 1` is the cusp `b = c`, excluded above.
    refine hbc ?_
    field_simp at hx
    linarith
...
* `N = 11, 13` go to `x1Eleven_plane_ne_zero` and
  ...
  Level `11` was PROVEN on 2026-07-26: the birational reduction of
  `F₁₁` to the minimal Weierstrass model `11a3` is written out and
  compiles, so the only citation left there is
  `x1Eleven_11a3_x_eq_zero_or_one`, i.e. `rank 11a3(ℚ) = 0`.

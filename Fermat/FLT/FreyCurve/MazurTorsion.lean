...
**What is left.** ONE elementary leaf, not modular: `exists_chain_coords`
(pure `Affine.Point` plumbing, zero arithmetic content). The other leaf,
`not_sextic_square`, is PROVEN below (2026-07-25) by the classical two-case
descent recorded in its docstring; the one genuine mathlib gap it needed —
Fermat's *other* quartic theorem `x⁴ − y⁴ = z² → xyz = 0` — is built here as

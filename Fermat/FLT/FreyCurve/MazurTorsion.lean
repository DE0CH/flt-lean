...
/-- **`X_1(11)` IN ITS OWN MINIMAL WEIERSTRASS MODEL: on the elliptic
curve `11a3`, `y² + y = x³ − x²`, every rational point has `x = 0` or
`x = 1`** (PROVEN 2026-07-26 from `MordellWeil.lean`'s `curve11a3_points`
and `curve11a3_finite`).

This is the whole arithmetic input of level `11`, and the interface
between the plane model and the Mordell–Weil development. Together with
the point at infinity, `11a3(ℚ) = {O, (0,0), (0,−1), (1,0), (1,−1)} ≅
ℤ/5`, generated by `(0,0)`, and the four affine points have `x ∈ {0, 1}`.
...
level-`11` obligation is now Cremona's curve `11a3` in its standard
minimal model, where the literature is directly citable. The five
rational points are the five rational cusps of `X_1(11)` (`φ(11)/2 = 5`),
and the `b ≠ 0` chart of the Tate family misses all of them — which is
...
over the cubic field `ℚ[X]/(4X³ − 4X² + 1)` (discriminant
`−176 = −16·11`), so it needs the unit and class groups of a specific
cubic field. The `5`-isogeny descent (`11a3 → 11a1 = X_0(11)`, kernel
`⟨(0,0)⟩ ≅ ℤ/5`) is the classical alternative and needs a descent map
into `ℚ*/(ℚ*)⁵` unramified outside `{5, 11}`.
...
theorem WeierstrassCurve.x1Eleven_11a3_x_eq_zero_or_one (x y : ℚ)
    (h : y ^ 2 + y = x ^ 3 - x ^ 2) : x = 0 ∨ x = 1 := by
  -- `curve11a3 = ⟨0, −1, 1, 0, 0⟩` is `y² + y = x³ − x²` on the nose.
  have hE : WeierstrassCurve.curve11a3.toAffine.Equation x y := by
    rw [WeierstrassCurve.Affine.equation_iff]
    simp only [WeierstrassCurve.curve11a3]
    linear_combination h
  -- `Δ = −11 ≠ 0`, so every point of the curve is nonsingular.
  have hN : WeierstrassCurve.curve11a3.toAffine.Nonsingular x y :=
    WeierstrassCurve.Affine.equation_iff_nonsingular.mp hE
  rcases WeierstrassCurve.curve11a3_points WeierstrassCurve.curve11a3_finite x y hN with
      h1 | h1 | h1 | h1
  · exact Or.inl (congrArg Prod.fst h1)
  · exact Or.inl (congrArg Prod.fst h1)
  · exact Or.inr (congrArg Prod.fst h1)
  · exact Or.inr (congrArg Prod.fst h1)
...
`x1Eleven_11a3_x_eq_zero_or_one` above and proven there from
`Fermat/FLT/EllipticCurve/MordellWeil.lean`.
...
* `N = 11, 13` go to `x1Eleven_plane_ne_zero` and
  ...
  compiles, and it is discharged against
  `Fermat/FLT/EllipticCurve/MordellWeil.lean`, so what is left of
  level `11` is that module's Mordell–Weil leaves and nothing here.

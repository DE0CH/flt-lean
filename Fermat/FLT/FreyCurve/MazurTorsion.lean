...
theorem WeierstrassCurve.exists_stable_cyclic_subgroup_of_rational_point
  ...
  · intro σ x hx
    ...
    -- REPAIR 2026-07-26 (flt-lean-19, while proving `quartic_only_trivial`):
    -- this line used to be
    --   `rw [map_zsmul, Affine.Point.map_baseChange (σ : …).toAlgHom Q]`
    -- and the second rewrite failed with "Did not find an occurrence of the
    -- pattern", a hard error that blocked the WHOLE module (and so every
    -- downstream one) while emitting no `sorry` warning. The rewrite motive
    -- is only defeq, not syntactically equal, to the goal — stating the
    -- equation as a `have` at the goal's own elaboration lets unification
    -- fix the implicit instances, and then `rw` matches.

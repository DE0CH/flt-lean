...
theorem WeierstrassCurve.exists_stable_cyclic_subgroup_of_rational_point
  ...
  · intro σ x hx
    ...
    -- BUILD FIX (2026-07-26, flt-lean-42): `rw [Affine.Point.map_baseChange …]`
    -- fails here with "did not find an occurrence of the pattern" even though
    -- the pattern is VISIBLY present in the goal — the two spellings differ
    -- only in an `Algebra ℚ ℚ̄` instance that prints identically
    -- (`DivisionRing.toRatAlgebra` vs the `AlgebraicClosure` one), and `rw`
    -- matches syntactically so it cannot cross. Stating the equation as a
    -- `have` against the GOAL's spelling and closing it with the lemma pushes
    -- the reconciliation into elaboration, where defeq does cross. Confirmed
    -- by a two-way reproduction in a scratch module: red without the `have`,
    -- green with it. No statement changed, proof only.
    have hmap : Affine.Point.map
        (σ : AlgebraicClosure ℚ ≃ₐ[ℚ] AlgebraicClosure ℚ).toAlgHom

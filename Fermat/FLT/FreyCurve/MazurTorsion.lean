...
* `p ∈ {7, 13}` — `X_0(p)` has **genus `0`**, so infinitely many curves
  ...
  These are the two concrete levels `not_cyclicIsogeny_fortyNine` (still a
  leaf) and `not_cyclicIsogeny_oneHundredSixtyNine` (PROVEN 2026-07-26 over
  the Atkin–Lehner descent leaf
  `classPoly676_of_stable_cyclic_subgroup_order_169`; see the section note
  at that level).

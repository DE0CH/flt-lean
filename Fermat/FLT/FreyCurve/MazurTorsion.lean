...
theorem WeierstrassCurve.exists_stable_cyclic_subgroup_of_rational_point
  ...
  · intro σ x hx
    ...
    -- `rw [Affine.Point.map_baseChange f Q]` fails here even though its LHS is
    -- displayed verbatim in the goal: the lemma's `[Algebra F K]` /
    -- `[IsScalarTower R F K]` instances get resolved to terms that are defeq
    -- to, but not syntactically equal to, the ones in the goal, and `rw`
    -- matches syntactically. Stating the fixity equation against the goal's own
    -- elaboration and letting `exact` do the defeq work is the fix.

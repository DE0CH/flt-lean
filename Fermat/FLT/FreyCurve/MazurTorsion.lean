...
* `not_two_four_torsion_and_three_point` (sorry node — IRREDUCIBLE
  literature citation, audited 2026-07-25): a rational `ℤ/2 × ℤ/4` plus
  a point of order `3` (Kenku; Mazur 1977, Thm 8), the `X_1(2,12)`
  boundary case of the fifteen-groups list. Its docstring records why
  the elementary halving machinery and the reduction/Hasse bound both
  fall short. The `X_1(2,10)` route above is a partial template — a
  point of order `3` also has a Tate normal form
  (`y² + a₁xy + a₃y = x³`, `Δ = a₃³(a₁³ − 27a₃)`, one parameter after
  scaling `a₃ = 1`) — but the discriminant condition ALONE is not
  enough here, and that difference is the whole difficulty: `ℤ/2 × ℤ/6`
  is a permitted torsion group, so "`Δ` a square + a point of order `3`"
  is satisfied by infinitely many curves (it reduces to `v² = a₁³ − 27`,
  which has rank `3`, checked with PARI/GP). The extra `ℤ/4` must be
  used as well, via the halving square `(θ₂−θ₁)(θ₂−θ₃) = □` of
  `MazurFourTorsion.halving_square`, giving a two-condition curve.

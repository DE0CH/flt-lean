...
section TwistedPointsGeneric

open WithConv Algebra TensorProduct Bialgebra

variable (K : Type) [Field K] (Ω : Type) [Field Ω] [Algebra K Ω]
variable (C : Type) [CommRing C] [HopfAlgebra K C]

/-- Left-cancellation of antipode precomposition in the convolution
monoid of `Ω`-points of a commutative Hopf algebra `C`: precomposing a
point with the antipode gives its convolution inverse (mathlib's
`AlgHom.convGroup` proves exactly this, but its instance binders demand
a `Bialgebra` structure on the *target*, which a plain field extension
`Ω` does not carry; this is the same proof with the correct
hypotheses). -/
theorem toConv_comp_antipodeAlgHom_mul_cancel (f : WithConv (C →ₐ[K] Ω)) :
    toConv (f.ofConv.comp (HopfAlgebra.antipodeAlgHom K C)) * f = 1 := by
  have H : (Algebra.TensorProduct.lmul' K).comp
      (Algebra.TensorProduct.map f.ofConv f.ofConv) =
      f.ofConv.comp (Algebra.TensorProduct.lmul' K) := by ext <;> simp
  trans toConv <| ((Algebra.TensorProduct.lmul' K).comp
      (Algebra.TensorProduct.map f.ofConv f.ofConv)).comp
      ((Algebra.TensorProduct.map
      (HopfAlgebra.antipodeAlgHom K C) (.id _ _)).comp (comulAlgHom K C))
  ...
  change f.ofConv.toLinearMap.comp
    (toConv (HopfAlgebra.antipode K (A := C)) * toConv LinearMap.id).ofConv =
    ofConv (1 : WithConv <| C →ₗ[K] Ω)
  rw [LinearMap.antipode_mul_id]
  ext
  simp

/-- Postcomposition with a `K`-automorphism of `Ω` as an endomorphism of
the convolution monoid of `Ω`-points of the `K`-bialgebra `C`
(multiplicative by `AlgHom.comp_convMul_distrib`). -/
noncomputable def pointsPostcompMonoidHom :
    (Ω ≃ₐ[K] Ω) →* Monoid.End (WithConv (C →ₐ[K] Ω)) where
  toFun σ :=
    { toFun := fun x => toConv (σ.toAlgHom.comp x.ofConv)
      map_one' := by
        apply WithConv.ext
  ...
  map_one' := MonoidHom.ext fun x => WithConv.ext (AlgHom.ext fun c => rfl)
  map_mul' σ τ := MonoidHom.ext fun x => WithConv.ext (AlgHom.ext fun c => rfl)

variable {K Ω C} in
theorem pointsPostcompMonoidHom_apply (σ : Ω ≃ₐ[K] Ω) (x : WithConv (C →ₐ[K] Ω)) :
    pointsPostcompMonoidHom K Ω C σ x = toConv (σ.toAlgHom.comp x.ofConv) := rfl

/-- Postcomposition with a `K`-automorphism of `Ω`, additivized: an
`AddMonoid.End` of the additivized points convolution monoid. -/
noncomputable def pointsPostcompAddEnd (σ : Ω ≃ₐ[K] Ω) :
    AddMonoid.End (Additive (WithConv (C →ₐ[K] Ω))) :=
  MonoidHom.toAdditive (pointsPostcompMonoidHom K Ω C σ :
    WithConv (C →ₐ[K] Ω) →* WithConv (C →ₐ[K] Ω))

variable {K Ω C} in
theorem pointsPostcompAddEnd_apply (σ : Ω ≃ₐ[K] Ω)
    (x : Additive (WithConv (C →ₐ[K] Ω))) :
    pointsPostcompAddEnd K Ω C σ x =
      Additive.ofMul (toConv (σ.toAlgHom.comp (Additive.toMul x).ofConv)) := rfl

/-- **The χ-twisted points module, generic form**: for a finite
commutative Hopf algebra `C` over a field `K`, an algebraic field
extension `Ω/K`, a separable quadratic subextension `L`, and
commutative `Ω`-points of `C`, the points convolution monoid is a
finite abelian group with discrete Galois action, and its twist by the
quadratic character of `L/K` is again one. Antipode precomposition
supplies convolution inverses (`toConv_comp_antipodeAlgHom_mul_cancel`),
`hcomm` supplies commutativity, mathlib's `Finite (C →ₐ[K] Ω)` instance
(Dedekind-style finiteness for a module-finite domain algebra) supplies
finiteness, and every point factors through the finite subextension
generated by the images of a basis of `C` together with the image of a
basis of `L` — on which the quadratic character is trivial. -/
theorem exists_twistedPointsModule_generic
    (L : Type) [Field L] [Algebra K L]
    [Algebra.IsQuadraticExtension K L] [Algebra.IsSeparable K L]
    [Algebra L Ω] [IsScalarTower K L Ω]
    [Algebra.IsAlgebraic K Ω] [Module.Finite K C]
    (hcomm : ∀ x y : Additive (WithConv (C →ₐ[K] Ω)), x + y = y + x) :
    ∃ (A : Type) (_ : AddCommGroup A) (_ : Finite A)
      (ρ' : (Ω ≃ₐ[K] Ω) →* AddMonoid.End A)
      (g : Additive (WithConv (C →ₐ[K] Ω)) ≃+ A),
      (∀ a : A, ∃ Lf : IntermediateField K Ω,
        FiniteDimensional K Lf ∧
        ∀ σ : Ω ≃ₐ[K] Ω, σ ∈ Lf.fixingSubgroup → ρ' σ a = a) ∧
      ∀ (σ : Ω ≃ₐ[K] Ω) (φ : C →ₐ[K] Ω),
        (quadraticCharacter K L Ω σ = 1 →
          g (Additive.ofMul (toConv (σ.toAlgHom.comp φ))) =
            ρ' σ (g (Additive.ofMul (toConv φ)))) ∧
        (quadraticCharacter K L Ω σ = -1 →
          g (Additive.ofMul (toConv (σ.toAlgHom.comp φ))) +
            ρ' σ (g (Additive.ofMul (toConv φ))) = 0) := by
  classical
  letI instG : Group (WithConv (C →ₐ[K] Ω)) :=
    { (inferInstance : Monoid (WithConv (C →ₐ[K] Ω))) with
      inv := fun f => toConv (f.ofConv.comp (HopfAlgebra.antipodeAlgHom K C))
      inv_mul_cancel := toConv_comp_antipodeAlgHom_mul_cancel K Ω C }
  letI instCG : CommGroup (WithConv (C →ₐ[K] Ω)) :=
    { instG with mul_comm := fun a b => hcomm a b }
  letI instA : AddCommGroup (Additive (WithConv (C →ₐ[K] Ω))) :=
    Additive.addCommGroup
  haveI : Finite (WithConv (C →ₐ[K] Ω)) :=
    Finite.of_equiv _ (WithConv.equiv (C →ₐ[K] Ω)).symm
  -- the χ-twisted Galois action on the points group
  let ρ' : (Ω ≃ₐ[K] Ω) →* AddMonoid.End (Additive (WithConv (C →ₐ[K] Ω))) :=
    { toFun := fun σ => (quadraticCharacter K L Ω σ : ℤ) •
        pointsPostcompAddEnd K Ω C σ
      map_one' := by
        have hE1 : pointsPostcompAddEnd K Ω C 1 = 1 := by
          unfold pointsPostcompAddEnd
          rw [map_one]
          rfl
        rw [map_one, hE1, Units.val_one, one_smul]
      map_mul' := fun σ τ => by
        have hE : pointsPostcompAddEnd K Ω C (σ * τ) =
            pointsPostcompAddEnd K Ω C σ * pointsPostcompAddEnd K Ω C τ := by
          unfold pointsPostcompAddEnd
          rw [map_mul]
          rfl
        rw [map_mul, hE, Units.val_mul]
        rw [zsmul_eq_mul, zsmul_eq_mul, zsmul_eq_mul, Int.cast_mul]
        rw [mul_assoc ((quadraticCharacter K L Ω σ : ℤ) :
              AddMonoid.End (Additive (WithConv (C →ₐ[K] Ω))))
            (pointsPostcompAddEnd K Ω C σ),
          ← mul_assoc (pointsPostcompAddEnd K Ω C σ),
          ← (Int.cast_commute (quadraticCharacter K L Ω τ : ℤ)
              (pointsPostcompAddEnd K Ω C σ)).eq,
          mul_assoc, mul_assoc] }
  refine ⟨Additive (WithConv (C →ₐ[K] Ω)), instA, Finite.of_equiv _ Additive.ofMul,
    ρ', AddEquiv.refl _, ?_, ?_⟩
  · -- discreteness: every point factors through a finite subextension
    intro a
    refine ⟨IntermediateField.adjoin K ((Set.range fun i =>
          (Additive.toMul a).ofConv (Module.finBasis K C i)) ∪
        (Set.range fun j => algebraMap L Ω (Module.finBasis K L j))),
      IntermediateField.finiteDimensional_adjoin
        (fun x _ => (Algebra.IsAlgebraic.isAlgebraic (R := K) x).isIntegral), ?_⟩
    intro σ hσ
    have hfix : ∀ x ∈ (Set.range fun i =>
          (Additive.toMul a).ofConv (Module.finBasis K C i)) ∪
        (Set.range fun j => algebraMap L Ω (Module.finBasis K L j)),
        σ x = x := fun x hx =>
      hσ ⟨x, IntermediateField.subset_adjoin K _ hx⟩
    -- σ fixes the point underlying `a`
    have hφc : σ.toAlgHom.comp (Additive.toMul a).ofConv =
        (Additive.toMul a).ofConv := by
      apply AlgHom.toLinearMap_injective
      refine (Module.finBasis K C).ext fun i => ?_
      exact hfix _ (Or.inl ⟨i, rfl⟩)
    -- σ fixes the image of L, so the character is trivial on it
    have hχ : quadraticCharacter K L Ω σ = 1 := by
      rw [quadraticCharacter_eq_one_iff]
      have hlin : (σ.toLinearMap : Ω →ₗ[K] Ω) ∘ₗ
          (IsScalarTower.toAlgHom K L Ω).toLinearMap =
          (IsScalarTower.toAlgHom K L Ω).toLinearMap :=
        (Module.finBasis K L).ext fun j => hfix _ (Or.inr ⟨j, rfl⟩)
      intro x
      exact LinearMap.congr_fun hlin x
    show ((quadraticCharacter K L Ω σ : ℤ) • pointsPostcompAddEnd K Ω C σ) a = a
    rw [hχ, Units.val_one, one_smul, pointsPostcompAddEnd_apply, hφc]
    rfl
  · -- the χ-twist clauses
    intro σ φ
    constructor
    · intro hχ1
      show Additive.ofMul (toConv (σ.toAlgHom.comp φ)) =
        ((quadraticCharacter K L Ω σ : ℤ) • pointsPostcompAddEnd K Ω C σ)
          (Additive.ofMul (toConv φ))
      rw [hχ1, Units.val_one, one_smul, pointsPostcompAddEnd_apply]
      rfl
    · intro hχm
      show Additive.ofMul (toConv (σ.toAlgHom.comp φ)) +
        ((quadraticCharacter K L Ω σ : ℤ) • pointsPostcompAddEnd K Ω C σ)
          (Additive.ofMul (toConv φ)) = 0
      have hFy : ((quadraticCharacter K L Ω σ : ℤ) • pointsPostcompAddEnd K Ω C σ)
          (Additive.ofMul (toConv φ)) =
          -(pointsPostcompAddEnd K Ω C σ (Additive.ofMul (toConv φ))) := by
        rw [hχm, Units.val_neg, Units.val_one]
        exact (AddMonoid.End.smul_apply _ _ _).trans (neg_one_zsmul _)
      rw [hFy]
      exact add_neg_cancel _

end TwistedPointsGeneric

open TensorProduct ValuativeRel IsDedekindDomain in
set_option backward.isDefEq.respectTransparency false in
set_option synthInstance.maxHeartbeats 1000000 in
set_option maxHeartbeats 2000000 in
/-- **The χ-twisted points module of a local Hopf model** (PROVEN
2026-07-24 — the finite-group-theoretic half of the quadratic-twist
construction, curve-free and Hopf-order-free): the `Ω̂`-point
convolution monoid of a finite `𝒪`-Hopf algebra `H` with étale generic
fibre and COMMUTATIVE points is a finite abelian group carrying a
discrete Galois action, and twisting that action by the quadratic
character `χ` of `L/ℚ_pˆ` yields again a discrete Galois module —
packaged as an abstract module `A` with an action `ρ'` and a points
identification `g` intertwining postcomposition with `ρ'` UP TO `χ`.
The proof is the instantiation of the generic construction
`exists_twistedPointsModule_generic` above at `K = ℚ_pˆ`,
`Ω = its algebraic closure`, `C = K ⊗ H`: inverses in the convolution
monoid are precomposition with the antipode
(`toConv_comp_antipodeAlgHom_mul_cancel`), commutativity is the
hypothesis `hcomm`, finiteness is mathlib's Dedekind-style finiteness
of the `Ω̂`-points of the finite-dimensional generic fibre, and
discreteness holds because each point factors through the finite
subextension generated by the images of a basis (together with the
image of `L`, through which `χ` factors); the twisted action
`ρ' σ = χ(σ) • (σ ∘ ·)` is multiplicative because `χ` is a character
and integer scalars are central in the endomorphism ring. -/
theorem exists_twistedPointsModule_of_quadraticCharacter
  ...
  classical
  exact exists_twistedPointsModule_generic
...
`R_L ⊗[R] H` and the twisted conjugation matches postcomposition by
`σ` against the antipode-inversion exactly when `χ(σ) = -1`, giving
`θ` and its character clauses. -/
theorem exists_twistedHopfModel_of_quadraticWitness
...
(DECOMPOSED 2026-07-24 — the fixed-points descent is the abstract
sorried leaf `exists_twistedHopfModel_of_quadraticWitness` above;
PROVEN here is the glue: `2` is a unit in `𝒪` because the residue
characteristic `p` is odd, the abstract twisted model's `χ`-clauses
cancel against those of `θg` so the composite points equivalence
`θg.trans θ'.symm` is honestly Galois-equivariant, and the PROVEN
reconstruction `exists_bialgEquiv_of_equivariant_pointsEquiv`
upgrades it to the generic-fibre bialgebra identification with `Hg`):
given a finite flat `𝒪`-Hopf algebra `H` with étale
...
theorem exists_hopfModel_of_twistedPointsEquiv
  ...
  haveI : CharZero (HeightOneSpectrum.adicCompletion ℚ
    ...
    charZero_of_injective_algebraMap
      ((algebraMap ℚ (HeightOneSpectrum.adicCompletion ℚ
        hp'.toHeightOneSpectrumRingOfIntegersRat)).injective)
  -- `2` is a unit in the completed integers: the residue characteristic
  -- `p` is odd
  have h2 : IsUnit (2 : 𝒪[HeightOneSpectrum.adicCompletion ℚ
      hp'.toHeightOneSpectrumRingOfIntegersRat]) := by
    rw [Valuation.Integers.isUnit_iff_valuation_eq_one
        (Valuation.integer.integers (ValuativeRel.valuation
          (HeightOneSpectrum.adicCompletion ℚ
            hp'.toHeightOneSpectrumRingOfIntegersRat))), map_ofNat,
      (ValuativeRel.isEquiv (ValuativeRel.valuation
        (HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat))
        (Valued.v : Valuation (HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat) ℤᵐ⁰)).eq_one_iff_eq_one,
      show ((2 : HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat)) =
        algebraMap ℚ (HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat) (2 : ℚ) from
        (map_ofNat _ 2).symm,
      valued_algebraMap_adicCompletion_eq hp',
      show ((2 : ℚ)) = algebraMap (NumberField.RingOfIntegers ℚ) ℚ
        (2 : NumberField.RingOfIntegers ℚ) from (map_ofNat _ 2).symm,
      IsDedekindDomain.HeightOneSpectrum.valuation_of_algebraMap]
    refine le_antisymm
      (IsDedekindDomain.HeightOneSpectrum.intValuation_le_one _ _) ?_
    by_contra hlt
    rw [not_le, IsDedekindDomain.HeightOneSpectrum.intValuation_lt_one_iff_mem,
      Nat.Prime.mem_toHeightOneSpectrumRingOfIntegersRat_asIdeal hp',
      map_ofNat] at hlt
    have hdvd : p ∣ 2 := by exact_mod_cast hlt
    exact hp2 ((Nat.prime_dvd_prime_iff_eq hp' Nat.prime_two).mp hdvd)
  -- the abstract fixed-points twisted model (sorried leaf above)
  obtain ⟨H', cH', hopfH', finH', flatH', etH', θ', hθ'⟩ :=
    exists_twistedHopfModel_of_quadraticWitness
      𝒪[HeightOneSpectrum.adicCompletion ℚ
        hp'.toHeightOneSpectrumRingOfIntegersRat]
      (HeightOneSpectrum.adicCompletion ℚ
        hp'.toHeightOneSpectrumRingOfIntegersRat)
      (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
        hp'.toHeightOneSpectrumRingOfIntegersRat))
      h2 L θL Q hQm hθtop hθQ hQsep H hcomm
  letI := cH'
  letI := hopfH'
  letI := finH'
  letI := flatH'
  letI := etH'
  -- the composite points identification is equivariant on the nose:
  -- the `χ`-twist of `θg` cancels against the `χ`-twist of `θ'`
  have hrec : ∀ (σ : (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
        hp'.toHeightOneSpectrumRingOfIntegersRat))
        ≃ₐ[HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat]
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
      ...
      (ψ : ((HeightOneSpectrum.adicCompletion ℚ
        ...
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat))),
      (θg.trans θ'.symm) (Additive.ofMul (WithConv.toConv (σ.toAlgHom.comp ψ))) =
        Additive.ofMul (WithConv.toConv (σ.toAlgHom.comp
          (WithConv.ofConv (Additive.toMul ((θg.trans θ'.symm)
            (Additive.ofMul (WithConv.toConv ψ))))))) := by
    intro σ ψ
    show θ'.symm (θg (Additive.ofMul (WithConv.toConv (σ.toAlgHom.comp ψ)))) =
      Additive.ofMul (WithConv.toConv (σ.toAlgHom.comp
        (WithConv.ofConv (Additive.toMul (θ'.symm (θg (Additive.ofMul
          (WithConv.toConv ψ))))))))
    rcases Int.units_eq_one_or (quadraticCharacter
        (HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat) L
        (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat)) σ) with hχ | hχ
    · -- `χ(σ) = 1`: both clauses are honest equivariance
      rw [(hθg σ ψ).1 hχ]
      apply θ'.injective
      rw [AddEquiv.apply_symm_apply]
      have h1 := (hθ' σ (WithConv.ofConv (Additive.toMul (θ'.symm (θg
        (Additive.ofMul (WithConv.toConv ψ))))))).1 hχ
      rw [WithConv.toConv_ofConv, ofMul_toMul, AddEquiv.apply_symm_apply] at h1
      exact h1.symm
    · -- `χ(σ) = -1`: the two sign twists cancel — both sides of the
      -- claim are additive inverses of the same element in the
      -- commutative points monoid of `H` (`hcomm`), hence equal
      have h2' := (hθ' σ (WithConv.ofConv (Additive.toMul (θ'.symm (θg
        (Additive.ofMul (WithConv.toConv ψ))))))).2 hχ
      rw [WithConv.toConv_ofConv, ofMul_toMul, AddEquiv.apply_symm_apply] at h2'
      have key : ∀ a b c : Additive (WithConv (((HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat) ⊗[𝒪[HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat]] H) →ₐ[HeightOneSpectrum.adicCompletion ℚ
          hp'.toHeightOneSpectrumRingOfIntegersRat]
          (AlgebraicClosure (HeightOneSpectrum.adicCompletion ℚ
          ...
          a + c = 0 → b + c = 0 → a = b := by
        intro a b c ha hb
        calc a = a + (c + b) := by rw [hcomm c b, hb, add_zero]
          _ = a + c + b := (add_assoc a c b).symm
          _ = b := by rw [ha, zero_add]
      have huniq := key _ _ _ ((hθg σ ψ).2 hχ) h2'
      rw [huniq, AddEquiv.symm_apply_apply]
  -- the PROVEN reconstruction upgrades the equivariant points
  -- identification to the generic-fibre bialgebra identification
  refine ⟨H', cH', hopfH', finH', flatH', etH', ?_⟩

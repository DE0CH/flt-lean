...
(sorry node — the TATE-THEORETIC content, stated in the same
valuation-subring shape as the pointwise triviality node above but
WITHOUT the `p ∣ v_q(j)` hypothesis, with the conclusion weakened to
unipotence; `q = 2` is allowed — the unramified quadratic twist to
split reduction exists at `2` as well): for an elliptic curve over `ℚ`
with multiplicative reduction at `q ≠ p`, every inertia element `σ` at
a valuation subring over `ℤ_(q)` acts unipotently on the `p`-torsion:
`σ(σP) − σP − σP + P = 0`. Content: Tate's uniformization presents

...
SUBGROUPS ONLY** (cut 2026-07-31 out of `A₀-3a-i-c` below, which is now four
lines of index arithmetic over it; **PROVEN 2026-08-01** over `A₀-3a-i-c″`
`torsionFive_fixed_of_smul_twelfthRoot_eq` immediately above): at `0 ≤ v_N(j)`
there is a subgroup `I′ ≤ I_N` of index dividing `12` acting trivially on
`E[5]`.

**HOW IT IS PROVEN, AND WHAT THAT LEFT OPEN.**  `I′` is the STABILISER IN `I_N`
OF A TWELFTH ROOT `π` OF `N` — not, as the "WHAT IS ASKED" paragraph below
originally proposed, the inertia group of an abstractly-named field `K`.  The two
are the same subgroup, and the stabiliser presentation is what makes the index
computable without any ramification theory:

* `σ ↦ σ(π)/π` is a HOMOMORPHISM `I_N → μ₁₂`, because `N ∤ 12` makes `μ₁₂`
  unramified at `N` so that inertia fixes it pointwise.  That is
  `ArtinConductor.lean`'s `tameCharacter` (`Deformations/RepresentationTheory/`
  `ArtinConductor.lean:910`), whose multiplicativity `tameChar_mul` spends
  exactly that fact through `smul_eq_self_of_pow_eq_one_algebraicClosure`;
* its kernel IS the stabiliser (`tameCharacter_eq_one_iff`), so
  `[I_N : I′] = #(im θ) ∣ #μ₁₂ ∣ 12` by Lagrange plus
  `natCard_rootsOfUnity_dvd` — and this is a DIVISIBILITY, not the mere bound
  `≤ 12` that the paragraph below correctly warns is insufficient;
* what remains is that this `I′` acts trivially on `E[5]`, which is
  `A₀-3a-i-c″` above: `E` acquires good reduction over `ℚ_N(π)` and
  Néron–Ogg–Shafarevich applies there.

So the count did not move (`1 → 1`, a RECUT), and what changed is that the
residual statement carries no subgroup, no relative index and no existential —
it is one implication about one `σ`, and all the group theory is gone.

**THE `tameCharacter` MACHINERY WAS ALREADY IN THE CONE and was nearly
duplicated here.**  A first draft of this proof rebuilt it from scratch — the
root-of-unity rigidity lemma, the homomorphism, the index bound, ~150 lines —
before a grep for `eq_one_of_pow_eq_one_of_sub_one_mem_maximalIdeal` found
`ArtinConductor.lean`'s copy and, beside it, the whole tame-character
development.  CLAUDE.md's standing rule (grep for the CONCEPT, not for the
leaf's own vocabulary) paid for itself; the surviving proof is ~30 lines.
...
**WHAT WAS ASKED** (kept as written on 2026-07-31, because the route it
describes is the one that was taken and the reader should see it).  Take
`I′ = I_K`, the inertia subgroup of the field `K` over which `E` acquires good
reduction (morally `ℚ_N(N^{1/12})`).  Then
...
**AND THAT IS WHY THE STABILISER PRESENTATION IS THE RIGHT ONE, which the
2026-07-31 note did not see.**  It went on to observe that neither `TameBase`
nor `TameGoodModel` EXPORTS the degree — `TameBase.π_pow` (`π¹² = ℓ`) carries
the ramification and `TameBaseAux` builds `AdjoinRoot (X¹² − ℓ)`, so
`Module.finrank ℚ L = 12` is one `AdjoinRoot.powerBasis` away — and concluded
that "a producer-side field or lemma has to be added for it".  **No such field
is needed.**  `[I_N : Stab(π)]` is the size of the ORBIT of `π`, the orbit sits
inside `π · μ₁₂` because `π¹² = N` is Galois-fixed, and the orbit is a COSET
SPACE of a subgroup of `μ₁₂` — a group, hence Lagrange — precisely because
inertia fixes `μ₁₂` pointwise.  Nothing about the degree of `ℚ(N^{1/12})/ℚ`, and
nothing about `e(K/ℚ_N)`, ever enters.  The general lesson: an index that reads
as a ramification index is often an ORBIT, and an orbit whose stabiliser is a
character kernel is computed by Lagrange in the character's target.
...
**PROVEN 2026-08-01 over `A₀-3a-i-c′`
`exists_localInertia_subgroup_relIndex_dvd_twelve_of_padicValRat_j_nonneg`
immediately above**, which is exactly the four lines of index arithmetic that
leaf's own docstring promised, and nothing else:

* `I′` acts trivially on `E[5]` and `I′ ≤ I_N`, so `hJmem` — which pins `J` as
  EXACTLY the trivially-acting part of `I_N` — gives `I′ ≤ J`;
* `Subgroup.relIndex_dvd_of_le_left` turns that into
  `[I_N : J] ∣ [I_N : I′] ∣ 12`;
* and `5 ∤ 12`.

Note the direction: `hJmem` is used only through its `mpr`, so the `iff` is
stronger than this proof needs — but it is what makes `J` pinned, and dropping
it to a one-sided implication would make the leaf satisfiable by `J = ⊥`, whose
relative index is `Nat.card I_N = 0` (the inertia group is infinite) and which
therefore satisfies `¬ 5 ∣ ·` vacuously. Do not weaken it.

**THE RELOCATION IS WHAT MADE THIS PROVABLE, and it is worth recording why**:
`A₀-3a-i-c′` was cut on 2026-07-31 in `FreyCurve/MazurTorsion.lean`, i.e. BELOW
this file, because the hoist that created this module had already moved this
consumer upstream a day earlier. Lean's declaration order then forbade the
citation, so both sat open, and `A₀-3a-i-c′` had no consumer anywhere in the
tree. Moving the OBLIGATION up (rather than this consumer down, which would
have dragged `A₀-3a-i` and its `X0`-facing cone with it) took the pair from two
open leaves to one. -/
theorem WeierstrassCurve.not_five_dvd_relIndex_of_padicValRat_j_nonneg
  ...
  obtain ⟨I', hI'le, hI'idx, hI'act⟩ :=
    E.exists_localInertia_subgroup_relIndex_dvd_twelve_of_padicValRat_j_nonneg hN hN19 hj
  have hle : I' ≤ J := fun σ hσ => (hJmem σ (hI'le hσ)).mpr (hI'act σ hσ)
  intro h5
  have h12 : (5 : ℕ) ∣ 12 :=
    h5.trans ((Subgroup.relIndex_dvd_of_le_left _ hle).trans hI'idx)
  norm_num at h12

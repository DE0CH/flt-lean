...
-- `det_frobeniusTorsionEnd_of_coprime` and the composite-level Weil-pairing
-- block that used to live in this file (moved 2026-07-31, VERBATIM, so that
-- `EllipticCurve/HasseBound.lean` — which this file is DOWNSTREAM of — can cite
-- the composite-level determinant).  `public` because the moved names occur in
-- the STATEMENTS of declarations below.
...
inertia group at `N`** (PROVEN 2026-07-31; Silverman *AEC* III.8.3 and the Weil
pairing).
...
Both halves were already theorems when this leaf was cut, and neither had to be
restated:

* the global determinant identity is `WeilPairing.det_galoisRep_eq_cyclotomic`
  (`EllipticCurve/WeilPairing.lean`, PROVEN 2026-07-17), publicly imported here;
  composed with `WeilPairing.cyclotomicCharacterModL_eq_toZMod` it reads
  `det (ρ̄_p σ) = (χ̄_p σ : ZMod p)` for every odd prime `p`, so the cut's
  suggestion to "state and prove `det ρ̄₅ = χ₅` above this point" would have
  DUPLICATED an imported declaration rather than adding anything;
* the unramifiedness half is
  `cyclotomicCharacterModL_eq_one_of_mem_localInertiaGroup_of_ne`, instantiated
  at `N := 5`, `q := N`, `hqN : N ≠ 5` (immediate from `19 < N`).  That theorem
  used to be declared ~1150 lines BELOW this point, which is the only reason the
  leaf was stated in the combined form; it and its ring-theoretic core
  `rootOfUnity_index_eq_one_of_sub_mem_maximalIdeal` were HOISTED above this
  declaration (2026-07-31) as a pure move — statements, proofs and docstrings
  unchanged, verified by sorting old and new file and diffing.  Nothing in the
  moved block refers to a name declared between the two positions, and there is
  no `section`/`variable`/`open` boundary in between.
...
AXIOM AUDIT (2026-07-31, `#print axioms` run in this file).  This declaration is
DIRECTLY closed — no `sorry` in its body, no `declaration uses 'sorry'` warning
— and its cone still reports `sorryAx`.  Every path out of it leaves this file:
`WeilPairing.det_galoisRep_eq_cyclotomic` is itself `sorryAx`, through
`WeilPairing.det_galoisRep_globalFrob`, and the only sorried declaration in
`EllipticCurve/WeilPairing.lean` is `weilValue_self_config_eq_one` (the
alternating clause of the Weil pairing, Silverman *AEC* III.8.1(b)), which is
the likely source and has its own audit there.  That taint predates this proof
and is not this cluster's to discharge; `cyclotomicCharacterModL_eq_one_of_`
`mem_localInertiaGroup_of_ne`, the other input, is axiom-clean
...
theorem WeierstrassCurve.det_galoisRep_five_eq_one_of_mem_localInertiaGroup
    ...
    LinearMap.det (E.galoisRep 5 Nat.prime_five.pos
      (Field.absoluteGaloisGroup.map (algebraMap ℚ
        (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
          hN.toHeightOneSpectrumRingOfIntegersRat)) τ)) = 1 := by
  haveI : Fact (Nat.Prime 5) := ⟨Nat.prime_five⟩
  set σ := Field.absoluteGaloisGroup.map (algebraMap ℚ
    (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
      hN.toHeightOneSpectrumRingOfIntegersRat)) τ with hσ
  -- the Weil pairing: `det ρ̄₅ = χ̄₅` on all of `Γ ℚ`
  have hdet : LinearMap.det (E.galoisRep 5 Nat.prime_five.pos σ) =
      ((GaloisRepresentation.cyclotomicCharacterModL 5 σ : (ZMod 5)ˣ) : ZMod 5) := by
    rw [WeilPairing.cyclotomicCharacterModL_eq_toZMod]
    exact WeilPairing.det_galoisRep_eq_cyclotomic E 5 Nat.prime_five.pos (by norm_num) σ
  -- `ℚ_N(μ₅)/ℚ_N` is unramified because `N ≠ 5`
  have hNne : N ≠ 5 := by omega
  have hcyc : (GaloisRepresentation.cyclotomicCharacterModL 5 σ : (ZMod 5)ˣ) = 1 :=
    cyclotomicCharacterModL_eq_one_of_mem_localInertiaGroup_of_ne Nat.prime_five hN hNne τ hτ
  rw [hdet, hcyc, Units.val_one]

/-- **`A₀-3a-i-c″` — Néron–Ogg–Shafarevich over the tame extension
`ℚ_N(N^{1/12})`** (sorry leaf, cut 2026-08-01 out of `A₀-3a-i-c′` below, which
is now PROVEN over it plus `ArtinConductor.lean`'s tame character): at
...
— the tame character `θ(σ) = σ(π)/π`, its multiplicativity (which is where
"inertia fixes `μ₁₂` because `N ∤ 12`" is spent), its kernel being the
stabiliser of `π`, and `[I_N : ker θ] = #im θ ∣ #μ₁₂ = 12` — is now discharged
below out of `ArtinConductor.lean`'s `tameCharacter` / `tameCharacter_eq_one_iff`
/ `natCard_rootsOfUnity_dvd`, which were already PROVEN and in this file's cone.
Nothing here mentions a subgroup, an index, or an existential: it is one
implication about one `σ`.

**A PROVER'S ROUTE, and the one thing it still owes.**  Follow
`WeierstrassCurve.isUnramifiedAt_of_hasGoodReduction`
(`FreyCurve/Semistable.lean`, ~line 15800) — a complete worked assembly of NOS
in this file's `localInertiaGroup` vocabulary, which instantiates the NOS node at
`R = ℤ_(q)`, `k = ℚ`, `𝒪 = embeddedValuationSubring v`, and crosses from
`localInertiaGroup` to `𝒪.inertiaSubgroup ℚ` by
...
theorem WeierstrassCurve.exists_localInertia_subgroup_relIndex_dvd_twelve_of_padicValRat_j_nonneg
  ...
  -- which is exactly the hypothesis `tameCharacter` asks for.
  have hXn : ∀ σ : Field.absoluteGaloisGroup
      (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ v),
      σ • (π ^ 12) = π ^ 12 := by
    intro σ
    rw [hπ]
    show σ ((N : AlgebraicClosure
      (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ v))) = _
    exact map_natCast _ _
  -- STEP 3.  The TAME CHARACTER `θ(σ) = σ(π)/π : I_N →* μ₁₂`, already built in
  -- `Deformations/RepresentationTheory/ArtinConductor.lean`.
  set θ := tameCharacter v hn hπ0 hXn with hθ
  refine ⟨θ.ker.map (localInertiaGroup v).subtype, Subgroup.map_subtype_le _, ?_, ?_⟩
  · -- STEP 4.  `[I_N : ker θ] = #(im θ) ∣ #μ₁₂ ∣ 12`.
    have hsub : (θ.ker.map (localInertiaGroup v).subtype).subgroupOf
        (localInertiaGroup v) = θ.ker :=
      Subgroup.comap_map_eq_self_of_injective (Subgroup.subtype_injective _) _
    rw [Subgroup.relIndex, hsub, Subgroup.index_ker]
    haveI : NeZero (12 : ℕ) := ⟨by norm_num⟩
...
theorem WeierstrassCurve.not_five_dvd_relIndex_of_padicValRat_j_nonneg
  ...
  -- `hJmem` PINS `J` as the set of inertia elements acting trivially, so `I' ≤ J`
  have hI'J : I' ≤ J := fun σ hσ => (hJmem σ (hI'le hσ)).mpr (hI'triv σ hσ)
  -- hence `[I_N : J]` divides `[I_N : I']`, which divides `12`
  have hdvd : J.relIndex (localInertiaGroup hN.toHeightOneSpectrumRingOfIntegersRat) ∣ 12 :=
    (Subgroup.relIndex_dvd_of_le_left _ hI'J).trans hI'idx
  intro h5
  exact absurd (h5.trans hdvd) (by decide)
...
outright.  **`A₀-3a-i-b` is PROVEN too since 2026-07-31**, so `A₀-3a-i-c` is
the SOLE remaining leaf of this cluster; Serre–Tate, Ann. of Math.
...
* `A₀-3a-i-b` `det_galoisRep_five_eq_one_of_mem_localInertiaGroup` — **PROVEN**
  (2026-07-31): the Weil pairing gives `det ρ̄₅ = χ₅` on all of `Γ ℚ`
  (`WeilPairing.det_galoisRep_eq_cyclotomic`, imported), and `χ₅` is trivial on
  `I_N` since `N ≠ 5` (`cyclotomicCharacterModL_eq_one_of_mem_localInertiaGroup_of_ne`,
  hoisted above it in the same commit).  No new leaf; see its docstring.
...
extension, with Raynaud's exponent attached** (**PROVEN 2026-07-31** over the two
halves `A₀-3b-i-a` and `A₀-3b-i-b` immediately above, of which the first is now
also PROVEN, so the only thing left under this node is Raynaud's classification;
Raynaud, Bull. SMF 102 (1974), Cor. 3.4.4 and §1.2; Serre, Invent. Math. 15
(1972), §1.7 and Prop. 9): over `L = (ℚ̄_N)^J` — whose inertia group is `J` and whose absolute
      ...
      (∀ σ : J, (@GaloisRepresentation.cyclotomicCharacterModL N ⟨hN⟩
          (Field.absoluteGaloisGroup.map (algebraMap ℚ
            (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
              hN.toHeightOneSpectrumRingOfIntegersRat)) (σ : _))) = ψ σ ^ e) := by
  obtain ⟨ψ, hsurj, hpin, hpow⟩ :=
    exists_fundamentalCharacter_of_relIndex_localInertiaGroup hN hN19 he hJle hindex
  obtain ⟨r, hre, hr⟩ := E.exists_raynaudExponent_of_fundamentalCharacter g hN hN19 hg
    lam hlam hj he hJle hJmem hindex ψ hsurj hpin
  exact ⟨ψ, r, hre, hsurj, hr, hpow⟩
...
**RESTATED AND REPROVED 2026-07-30 WITHOUT ANY HYPOTHESIS ON THE RESIDUE
CHARACTERISTIC**, which is what closed the `q = 2` leaf below).
...
**THE OLD HYPOTHESIS `h2 : 𝒪.valuation (2 : F) = 1` IS GONE, AND THE PARAGRAPH
THAT JUSTIFIED IT WAS WRONG.**  It read: "the `a₁`-relation
`u · (C • W).a₁ = W.a₁ + 2s` is the only source of information about `s`, so
`2 ∈ 𝒪ˣ` is genuinely used".  The premise is false — the `a₂`-relation
constrains `s` too, and it does so in a form that needs no division at all.
The price is two further integrality hypotheses on the transformed model
(`hc₂` and `hc₄`, in place of the discarded `h2`), and at the one call site
below they are already in hand.

**THE ARGUMENT, and why it is division-free: every one of `r`, `s`, `t`
satisfies a MONIC polynomial over `𝒪`, and a valuation ring is integrally
closed.**  The three polynomials are

* `r⁴ = r·u⁶·b₆' − u⁸·b₈' + b₄·r² + 2·b₆·r + b₈`, obtained as
  `r · (u⁶b₆'-relation) − (u⁸b₈'-relation)` from mathlib's
  `variableChange_b₆` / `variableChange_b₈`.  The two leading terms `4r³` and
  `3r⁴` cancel to `r⁴` — that combination is the whole trick, and it is what
  makes the step uniform in the residue characteristic rather than splitting
  on whether `2` or `3` is a unit;
* `s² = a₂ − s·a₁ + 3r − u²·a₂'` (the `a₂`-relation, once `r` is integral);
* `t² = a₆ + r·a₄ + r²·a₂ + r³ − t·a₃ − r·t·a₁ − u⁶·a₆'` (the `a₆`-relation).

In each case, if the root had `v > 1` then its leading power would strictly
dominate every term on the right, since each of those has valuation at most
`v(root)` (degree `2` cases) or `v r ^ 2` (degree `4` case).  Integral
closedness is therefore used in its bare valuation-theoretic form and no
`IsIntegrallyClosed` instance is needed.

`v u = 1` is unchanged: `u¹²` is the ratio of two unit discriminants. -/
...
**`hq2 : q ≠ 2` DELETED 2026-07-30**, when that lemma was reproved with no
hypothesis on the residue characteristic — this statement is now uniform in `q`).
...
SAME `Ẽ`, and it is what erases `τ` from the residue coordinates. `hqN` is not
used here at all — it belongs to
...
**`hq2 : q ≠ 2` WAS DELETED 2026-07-30, AND THE 2026-07-29 AUDIT THAT KEPT IT
WAS WRONG.** That audit ran: (a) the statement is true at `q = 2`; (b) the route
"both models have unit discriminant, hence `Dτ` has integral entries" is
`variableChange_valuation_of_valuation_Δ_eq_one`, which inverted `2`, because
`2s = u a₁' − a₁` and `2t = u³a₃' − a₃ − r a₁` were taken to be the only sources
of information about `s` and `t`; (c) therefore keep `hq2` here and open a
separate `q = 2` leaf.

Clause (b) is false, and with it (c). The `a₂`- and `a₆`-relations constrain `s`
and `t` too, and they do so through MONIC quadratics — `s² + a₁ s = a₂ + 3r −
u²a₂'` and `t² + (a₃ + r a₁) t = a₆ + r a₄ + r²a₂ + r³ − u⁶a₆'` — whose
coefficients are integral, so integral closedness of a valuation ring gives `s`
and `t` with no division at all. The same trick supplies `r` from the monic
QUARTIC `r⁴ = r u⁶ b₆' − u⁸ b₈' + b₄ r² + 2 b₆ r + b₈`, in which the `4r³` of
the `b₆`-relation and the `3r⁴` of the `b₈`-relation cancel to `r⁴`. So the
route is uniform in the residue characteristic, this leaf carries `q = 2`, and
`exists_aut_of_isTorsionReduction_two` below is a one-line corollary of it.

The moral for the next audit of this kind: "hypothesis `H` is used at step `S`"
is evidence about the PROOF; concluding that `H` is load-bearing for the
STATEMENT additionally requires that no other relation in the system constrains
the same unknown, and that is a claim about the whole transformation table, not
about the two formulas one happened to write down.
...
theorem WeierstrassCurve.PotentiallyGoodModel.LocalFrame.exists_aut_of_isTorsionReduction
  ...
  obtain ⟨Dτ, hu, hr, hs, ht, C, hC, hpin, hCu, hCr, hCs, hCt⟩ :=
    Fr.exists_inertiaVariableChange τ hdecT hτin
  refine ⟨C, hC, fun P => ?_⟩
  set Ob := GaloisRepresentation.globalValuationSubring
    hq.toHeightOneSpectrumRingOfIntegersRat with hObdef
...
no longer carries `hq2 : q ≠ 2` at all, so this is now literally a specialisation
of it and the residue characteristic `2` costs the `B₀` cluster NOTHING).  The
statement is kept, with its now-unused `hq2 : q = 2`, only so that the two call
sites downstream do not have to change; a later cleanup should delete it and
collapse the three-line case split at
`exists_inertiaAut_of_padicValRat_j_nonneg` onto the uniform leaf.

**WHAT ACTUALLY UNLOCKED IT** (recorded because the diagnosis below was right
about the obstruction and wrong about its inevitability): route 2 works, and it
works better than this docstring guessed — the `v`-case analysis is not needed
either.  Each of `r`, `s`, `t` satisfies a MONIC polynomial over `𝒪`
(`r⁴ = r u⁶b₆' − u⁸b₈' + b₄r² + 2b₆r + b₈`, and the two quadratics from the
`a₂`- and `a₆`-relations), and a valuation ring is integrally closed.  The
`r`-quartic is the combination `r ·(b₆-relation) − (b₈-relation)`, in which the
`4r³` and `3r⁴` cancel to `r⁴`; that cancellation is the whole content, and it is
insensitive to which small primes are units.  See
`variableChange_valuation_of_valuation_Δ_eq_one`, which now carries no
hypothesis on the residue characteristic.

The historical diagnosis follows, unchanged.
...
theorem WeierstrassCurve.PotentiallyGoodModel.LocalFrame.exists_aut_of_isTorsionReduction_two
  ...
  Fr.exists_aut_of_isTorsionReduction hN hqN ψ₀ hψ₀ τ hdecT hτin
...
theorem WeierstrassCurve.exists_inertiaAut_of_padicValRat_j_nonneg
  ...
  obtain ⟨C, hC, haut⟩ :
    ...
    · exact Fr.exists_aut_of_isTorsionReduction hN hqN ψ₀ hψ₀ τ
        (GaloisRepresentation.map_mem_decompositionSubgroup_globalValuationSubring _ σ)
        (GaloisRepresentation.map_mem_inertiaSubgroup_globalValuationSubring _ σ hσ)

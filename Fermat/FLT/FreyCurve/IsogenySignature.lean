...
inertia group at `N`** (cut 2026-07-30, **PROVEN 2026-07-30**, axiom-audited
`[propext, Classical.choice, Quot.sound]`; Silverman *AEC* III.8.3 and the Weil
pairing).
...
**BOTH HALVES WERE ALREADY PROVEN, AND NEITHER IS DUPLICATED HERE.**  The
identity is `WeilPairing.det_galoisRep_eq_cyclotomic` (`EllipticCurve/WeilPairing.lean`,
publicly imported, PROVEN 2026-07-17), read through the spelling bridge
`WeilPairing.cyclotomicCharacterModL_eq_toZMod`; the unramifiedness is leaf `D`
`cyclotomicCharacterModL_eq_one_of_mem_localInertiaGroup_of_ne` in this file,
instantiated at cyclotomic modulus `5` and place `q := N`, whose `q ≠ N`
hypothesis reads `N ≠ 5` and is `omega` from `19 < N`.  So the whole proof is
three lines, and the mathematics of it lives in two declarations that other
consumers already share.

**WHAT MADE IT THREE LINES WAS A RELOCATION, NOT A NEW ARGUMENT** (2026-07-30).
Leaf `D` sits ~5700 lines above this point *now*; before this commit it sat
~1000 lines BELOW, which is the only reason this leaf was ever stated in the
combined form.  The two routes this docstring used to prescribe — hoist `D`
upwards, or restate `det ρ̄₅ = χ₅` above — were both avoidable: what moved was
the CONSUMER.  See the section note above `A₀-3a-i-b`'s cluster for the move and
for the check that its cascade terminates.
...
theorem WeierstrassCurve.det_galoisRep_five_eq_one_of_mem_localInertiaGroup
  ...
  -- the mod-`5` cyclotomic character is trivial on inertia at `N ≠ 5` (leaf `D`)
  have hχ := cyclotomicCharacterModL_eq_one_of_mem_localInertiaGroup_of_ne
    (N := 5) Nat.prime_five (q := N) hN hN5 τ hτ
  -- and the determinant of the mod-`5` representation IS that character
  have h1 : LinearMap.det (E.galoisRep 5 Nat.prime_five.pos
      (Field.absoluteGaloisGroup.map (algebraMap ℚ
        (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
          hN.toHeightOneSpectrumRingOfIntegersRat)) τ)) =
      ((GaloisRepresentation.cyclotomicCharacterModL 5
        (Field.absoluteGaloisGroup.map (algebraMap ℚ
          (IsDedekindDomain.HeightOneSpectrum.adicCompletion ℚ
            hN.toHeightOneSpectrumRingOfIntegersRat)) τ) : (ZMod 5)ˣ) : ZMod 5) := by
    rw [WeilPairing.cyclotomicCharacterModL_eq_toZMod]
    exact WeilPairing.det_galoisRep_eq_cyclotomic E 5 Nat.prime_five.pos (by norm_num) _
  rw [h1, hχ, Units.val_one]
...
extension, with Raynaud's exponent attached** (DECOMPOSED and PROVEN
2026-07-31 over `A₀-3b-i-a` `exists_fundamentalCharacter_of_relIndex_localInertiaGroup`
and `A₀-3b-i-b` `exists_raynaudExponent_of_fundamentalCharacter`; Raynaud, Bull. SMF
...
**THE SPLIT ALONG `Surjective ψ ∧ χ|_J = ψ^e` IS FALSE, AND THE SPLIT ABOVE IS
NOT IT.**  The tempting cut — one leaf producing `ψ` surjective with
`χ|_J = ψ^e` (pure tame-character theory, no curve), a second taking such a
`ψ` as a HYPOTHESIS and producing `r ≤ e` with `λ|_J = ψ^r` (Raynaud) — does
not work, because those two hypotheses do NOT pin `ψ` to `ψ_L`.  Witness:
`N = 29`, `e = 4`, `ψ' = ψ_L^15`.  Then `gcd(15, 28) = 1` so `ψ'` is
surjective, and `ψ'^4 = ψ_L^{60} = ψ_L^4 = χ|_J` since `60 ≡ 4 (mod 28)`; but
`ψ'^r` for `r ≤ 4` runs through `ψ_L^0, ψ_L^{15}, ψ_L^2, ψ_L^{17}, ψ_L^4`
only, so if `λ|_J = ψ_L^1` or `ψ_L^3` the second leaf is unsatisfiable.  Those
two values of `r` are excluded only by the parity clause ("`e` even ⟹ `r`
even"), which this cut deliberately does not assert at this level (it is
derived downstream, in
`exists_isogenyLocalRamificationDataAt_of_padicValRat_j_nonneg`).  Adding
"`ψ` generates the character group" does not repair it either — `ψ'`
generates too, `gcd(15, 28) = 1`.

**WHAT REPAIRS IT, and is the decomposition actually taken above
(2026-07-31), is a CANONICAL PINNING of `ψ`, not a stronger property of it.**
`A₀-3b-i-a` hands `A₀-3b-i-b` the clause

    ∀ σ ∈ J, ∃ τ ∈ I_N, τ^e σ⁻¹ ∈ P_N ∧ ψ σ = χ τ,

which has AT MOST ONE solution (in `T = I_N/P_N`, `e·τ̄ = σ̄ = e·τ̄'` and `T`
is torsion-free, so `χ τ = χ τ'`) and is satisfied by `ψ_L`.  It kills the
`ψ_L^15` witness immediately: the clause would force `ψ_L^14 = 1` on `J`.
So the two halves are now genuinely independent — `A₀-3b-i-a` is local-field
theory with no curve in it, `A₀-3b-i-b` is Raynaud's classification and
nothing else — and both are citable on their own.  See `A₀-3b-i-a`'s
docstring for why the pinning clause is the right interface and for the one
missing ingredient (torsion-freeness of `T`) that a finite-quotient proof of
that leaf still needs. -/
theorem WeierstrassCurve.exists_fundamentalCharacter_of_semistabilityDefect
  ...
  -- `A₀-3b-i-a`: the level-one fundamental character of `L = (ℚ̄_N)^J`, PINNED
  obtain ⟨ψ, hsurj, hpin, hχ⟩ :=
    exists_fundamentalCharacter_of_relIndex_localInertiaGroup hN hN19 he hJle hindex
  -- `A₀-3b-i-b`: Raynaud's classification, read against that pinned `ψ`
  obtain ⟨r, hre, hr⟩ :=
    E.exists_raynaudExponent_of_fundamentalCharacter g hN hN19 hg lam hlam hj
      he hJle hJmem hindex ψ hsurj hpin
  exact ⟨ψ, r, hre, hsurj, hr, hχ⟩

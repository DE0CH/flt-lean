...
* `isCompact_normOne_infiniteAdele` — `{x ∈ D ⊗ 𝔸^∞ : Nm(x) = 1}` is compact. Now
  ...
  The last two are definiteness-free and were both PROVEN 2026-07-28 (`flt-lean-293`),
  so `exists_ringEquiv_quaternion_of_isTotallyDefinite` is the only one of the three
  still open. The place-local half
...
* `isCompact_normOne_infiniteAdele` — `{Nm = 1}` is compact in `D ⊗ 𝔸^∞`. PROVEN
  ...
  definiteness-free, and both PROVEN 2026-07-28 — only the first of the three is
  still open).
* `finite_setOf_tmul_mem_of_isCompact` — `D` is discrete and closed in `D ⊗ 𝔸_F`.

...
applications consume. DERIVED (2026-07-16) from `resultant_Φ_ΨSq`: the resultant lies in
the ideal generated by the two polynomials (`exists_mul_add_mul_eq_C_resultant`, with the
degree bounds `natDegree_Φ_le` and `natDegree_ΨSq_le`), and `±Δ^k` is a unit here. -/
theorem WeierstrassCurve.isCoprime_Φ_ΨSq {R₀ : Type*} [CommRing R₀] (W : WeierstrassCurve R₀)
  ...
  obtain ⟨p, q, _, _, e⟩ := Polynomial.exists_mul_add_mul_eq_C_resultant
    (W.Φ n) (W.ΨSq n) (W.natDegree_Φ_le n) (W.natDegree_ΨSq_le n)
    (Or.inl (pow_ne_zero 2 (Int.natAbs_ne_zero.mpr hn)))
  have hres : IsUnit ((W.Φ n).resultant (W.ΨSq n) (n.natAbs ^ 2) (n.natAbs ^ 2 - 1)) := by
    rcases W.resultant_Φ_ΨSq hn with h | h <;> rw [h]
    · exact hΔ.pow _
    · exact (hΔ.pow _).neg
  obtain ⟨u, hu⟩ := hres
  refine ⟨Polynomial.C (↑u⁻¹ : R₀) * p, Polynomial.C (↑u⁻¹ : R₀) * q, ?_⟩
  calc Polynomial.C (↑u⁻¹ : R₀) * p * W.Φ n + Polynomial.C (↑u⁻¹ : R₀) * q * W.ΨSq n
      = Polynomial.C (↑u⁻¹ : R₀) * (W.Φ n * p + W.ΨSq n * q) := by ring
    _ = 1 := by
        rw [e, ← hu, ← Polynomial.C_mul, Units.inv_mul, Polynomial.C_1]

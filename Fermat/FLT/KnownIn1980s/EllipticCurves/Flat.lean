...
set_option backward.isDefEq.respectTransparency false in
/-- **The residual (off-chord) Katz–Mazur branch unit** (sorry node — stage
...
CONSTRUCTION PLAN (Katz–Mazur 2.3.1 made pointwise; plan of 2026-07-24;
`v := 𝒪.valuation`, `d := deg h`, `e := 1/h`, `ξ := x/h`, `w := x^d/h`,
`T := Ψ₂Sq(x)/h²` — all in the section algebra, `T` because the
`b`-invariants of the good-reduction model are `R`-integral and `Ψ₂Sq` has
degree `3 ≤ 2d`).  Per point: `e(P)` is a unit iff `x(P)` is integral;
`w(P)` is a unit iff `P = 0`, `v(x(P)) > 1` (formal-group locus, where
`v(h(x)) = v(x)^d` by `val_aeval_monic_of_notMem`), or `v(x(P)) = 1` — so
`{e, w}` already covers every single point, and the pair branches are only
needed on the both-integral locus.

* FORMAL locus (`¬IsIntegralAffine 𝒪 P`): the candidate unit is `w ⊗ w`
  resp. `w ⊗ e` (value `1` at the origin), but neither kills the
  denominators by itself; the killer must be the `h`-component of an
  addition law adapted to the origin chart.  There `P + Q` reduces to the
  origin resp. to the reduction of `Q` (the kernel of reduction is a
  subgroup — pointwise: from the formal-group valuation `v(y)² = v(x)³` of
  `val_ordinate_sq_of_abscissa_notMem` and the secant/tangent formulas),
  and the origin-chart coordinates `s = x/y`, `w' = 1/y` give the addition
  law with UNIT denominator (Silverman AEC IV.1: the formal group law is
  integral).  Concretely: multiply the multiplied-secant identities of
  `TorsionCard.add_some_coords` by suitable `w`-weight factors so that all
  matrix entries become weight-`≤ 2d` sections whose leading origin values
  reproduce the formal-group law.
* COLLISION locus away from reduced 2-torsion (`v(Ψ₂Sq(x(P))) = 1`, i.e.
  the common reduced point `T̄` has `2T̄ ≠ 0`): the tangent analogue of
  the chord killer — replace `δ` (value `(x₁−x₂)/(h₁h₂)`, which
  degenerates here) by the `Ψ₂Sq`-section `T` (`eval_Ψ₂Sq_eq_sq`: on the
  curve `Ψ₂Sq(x) = ψ₂(x, y)²`), with the DUPLICATION form of the addition
  law (`two_point_trace_identity`,
  `TorsionCard.two_smul_some_eq_zero_iff`).
* HAZARD — the residual collision case `P̄ = Q̄ = T̄` with `2T̄ = 0`
  (`v(Ψ₂Sq(x(P))) < 1`): BOTH the chord and the tangent denominators
  vanish mod `𝔪_𝒪`, so `P + Q` lies on the formal-group locus and
  `g_{a,b} ∘ add` is already integral there; the unit comes from the
  origin-chart/symmetric-function form of the addition law.  The identity
  that closes this case is
  `x(P+Q) − x(P−Q) = −Ψ₂(P)·Ψ₂(Q)/(x(P) − x(Q))²`
  together with `x(P+Q) + x(P−Q)` and `x(P+Q)·x(P−Q)` having
  `(x(P)−x(Q))²`-denominators: `w`-weighted products therefore stay
  integral, and the unit normalization on the torsion locus is the Bézout
  identity `F·Φ_m + G·ΨSq_m = 1` (`isCoprime_Φ_ΨSq` with
  `ΨSq_m(x(P)) = 0` from `torsion_abscissa_psiSq_eq_zero`, unit
  discriminant from good reduction).  NOTE for ODD `m` this locus is EMPTY
  (a reduced point of exact order `2` cannot be the reduction of an
  `m`-torsion point), so only the kill property needs a witness there.
...
DECOMPOSE when attacked: (E3a) the formal-locus origin-chart kill identity
and its unit, (E3b) the tangent-branch kill identity and its unit on the
collision locus, (E3c) the hazard symmetric-function kill identity, (E3d)
the Bézout unit normalization on the torsion locus. -/

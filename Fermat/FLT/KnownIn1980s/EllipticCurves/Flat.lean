...
set_option backward.isDefEq.respectTransparency false in
/-- **Pointwise Katz–Mazur branch unit** (sorry node — stage E3' of the
addition-closure cut `torsionKernelFun_add_mem_span`, THE remaining
geometric core after the E3 finiteness assembly below): at ONE valuation
subring `𝒪` of `Kˢᵉᵖ` centered on `R` and ONE pair `PQ` of `m`-torsion
points there is a *single* two-variable section `f` which

* lies in the subalgebra `M` generated by the split products
  `a'(P)·b'(Q)` of the monomial-section algebra (equivalently, by the E0
  bridge `mem_span_prod_sections_iff_mem_adjoin`, in the `R`-span of those
  split products),
* kills all the addition-law denominators, `f·(g_{a,b} ∘ add) ∈ M` for
  every admissible weight `(a, b)`, and
* is a `𝒪`-valuation unit at `PQ`.

The finite family demanded by `exists_torsionKernel_branch_family` is then
free: the denominator-killing sections form the CONDUCTOR ideal of `M`,
`M` is a finitely generated `R`-algebra over the Noetherian ring `R`, so
the conductor is generated by finitely many of its members, and the
ultrametric inequality transports a unit value from an arbitrary conductor
element to one of those finitely many generators
(`exists_finite_family_of_conductor_units`).  So this leaf carries the
entire geometric content and none of the bookkeeping.
...
degree `3 ≤ 2d`).  The candidates are the chord discriminant
`D := (ξ(P)e(Q) − e(P)ξ(Q))²` (value `((x(P)−x(Q))/h(P)h(Q))²`), its
tangent analogue through `Ψ₂Sq` (`eval_Ψ₂Sq_eq_sq`), the origin-chart
weight-`2d` sections `w(P)`, `w(Q)`, and their mixed products.
Denominator-killing via the multiplied-out addition formulas
(`add_some_coords`, `cross_two`, `two_point_trace_identity`,
`eq_or_add_eq_zero_of_X_eq`); fibrewise units via the division-polynomial
arithmetic: affine `m`-torsion abscissas are roots of `ΨSq m`
(`smul_some_eq_zero_iff`, `torsion_abscissa_psiSq_eq_zero`) and the Bézout
identity `F·Φ m + G·ΨSq m = 1` (`isCoprime_Φ_ΨSq`, unit discriminant from
good reduction).  No indicator idempotents are available (for `μ_p` over
`ℤ_p` the origin idempotent is NOT integral), so the unit must come from
these global identities.

CASE ANALYSIS AT `(𝒪, PQ)`.  Per point: `e(P)` is a unit iff `x(P)` is
integral; `w(P)` is a unit iff `P = 0`, `v(x(P)) > 1` (formal-group locus,
where `v(h(x)) = v(x)^d`), or `v(x(P)) = 1` — so `{e, w}` already covers
every single point, and the extra branches are only needed on the
both-integral locus.  There `D` is a unit iff the reductions of `P` and
`Q` differ affinely; on the COLLISION locus (`x(P) ≡ x(Q) mod 𝔪_𝒪`, common
reduced point `T̄`) one has `v(Ψ₂Sq(x(P))) = 1` iff `2T̄ ≠ 0` on the
special fibre, so the tangent branch covers collisions away from reduced
2-torsion.  HAZARD — the residual case `P̄ = Q̄ = T̄` with `2T̄ = 0`: BOTH
the chord and the tangent denominators vanish mod `𝔪_𝒪` (then `P + Q` lies
on the formal-group locus, so `g_{a,b} ∘ add` is already integral there)
and the unit must come from the origin-chart/symmetric-function form of
the addition law: `x(P+Q) + x(P−Q)` and `x(P+Q)·x(P−Q)` have
`(x(P)−x(Q))²`-denominators and `x(P+Q) − x(P−Q) = −Ψ₂(P)Ψ₂(Q)/(x(P)−x(Q))²`,
so `w`-weighted products stay integral, combined with the Bézout unit
`Φ_m(x)·F(x) = 1` on the torsion locus.  DECOMPOSE when attacked: (E3a)
...
theorem WeierstrassCurve.exists_torsionKernel_branch_unit
    ...
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ))) :
    ∃ f : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep,
      f ∈ Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep |
        ∃ a' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
            ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
      ...
      (∀ a b : ℕ, b ≤ 1 → 2 * a + 3 * b ≤ 2 * h.natDegree →
        f * (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
        ...
        Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          ...
          ∃ b' ∈ Algebra.adjoin R
            ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
              ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
            (fun PQ => a' PQ.1 * b' PQ.2) = t}) ∧
      𝒪.valuation (f PQ) = 1 := by
  sorry

set_option backward.isDefEq.respectTransparency false in
/-- **The Katz–Mazur branch family** (PROVEN 2026-07-24 over the pointwise
leaf `exists_torsionKernel_branch_unit` — stage E3 of the
addition-closure cut `torsionKernelFun_add_mem_span`; the remaining
geometric core is now the pointwise leaf): a finite family of
...
PROOF (the finiteness bookkeeping; the geometry is the pointwise leaf).
The denominator-killing sections form the CONDUCTOR ideal
`S := {f ∈ M | ∀ admissible (a, b), f·(g_{a,b} ∘ add) ∈ M}` of the
subalgebra `M` (an ideal because `M` is a ring and the condition is stable
under `M`-multiples and sums).  `M` is generated as an `R`-algebra by the
finitely many pulled-back monomial sections
...
`f₁, …, f_k ∈ S` — this is the family.  Its covering property comes from
the pointwise leaf plus the ultrametric inequality: an arbitrary
conductor element with a unit value at `PQ` is an `M`-combination
`∑ cᵢ·fᵢ`, all of whose values are `R`-integral, hence in every centered
valuation subring; a sum of elements of valuation `< 1` has valuation
`< 1`, so some `cᵢ(PQ)·fᵢ(PQ)` — and therefore some `fᵢ(PQ)` — is a unit.
Both steps are the proven `exists_finite_family_of_conductor_units`.

The geometric content — one denominator-killing section that is a unit at
a given `(𝒪, PQ)` — is the leaf `exists_torsionKernel_branch_unit`, whose
docstring records the Katz–Mazur construction, the covering case analysis
and the reduced-2-torsion hazard case, together with the sub-decomposition
E3a–E3d. -/
theorem WeierstrassCurve.exists_torsionKernel_branch_family
  ...
  -- ideal, and the pointwise leaf feeds its covering property
  obtain ⟨k, f, hfmem, hfkill, hfcover⟩ :=
    exists_finite_family_of_conductor_units (R₀ := R)
      ...
      (fun 𝒪 h𝒪 pq => by
        obtain ⟨u, hu₁, hu₂, hu₃⟩ := WeierstrassCurve.exists_torsionKernel_branch_unit
          R K E Ksep m hm h hmon hdeg hunit 𝒪 h𝒪 pq
        exact ⟨u, hu₁, fun j => hu₂ j.1.1 j.1.2 j.2.1 j.2.2, hu₃⟩)
  refine ⟨k, f, fun i => (mem_span_prod_sections_iff_mem_adjoin _ _ _).mpr (hfmem i),
    fun i a b hb hw => (mem_span_prod_sections_iff_mem_adjoin _ _ _).mpr
      (hfkill i ⟨(a, b), hb, hw⟩), fun 𝒪 hcen PQ => hfcover 𝒪 hcen PQ⟩
...
* E3 `exists_torsionKernel_branch_family` (PROVEN 2026-07-24 over the
  pointwise leaf, via the Noetherian conductor-ideal assembly
...
* E3' `exists_torsionKernel_branch_unit` (sorry — THE remaining
  Katz–Mazur core): at ONE centered valuation subring and ONE pair of
  torsion points, ONE denominator-killing section of `M` that is a
  valuation unit there.

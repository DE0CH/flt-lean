...
/-- **Antipode transport along a bialgebra homomorphism** (glue for the
Hopf-order transport below; curve-free): a bialgebra homomorphism between
commutative Hopf algebras commutes with the antipodes. Proof: in the
convolution group of algebra homomorphisms `B →ₐ[K₁] C` (domain a commutative
Hopf algebra — mathlib's `AlgHom.convGroup`, whose inverse is BY DEFINITION
precomposition with the antipode of the domain, i.e. `Φ ∘ S_B`), the composite
`S_C ∘ Φ` is a LEFT convolution inverse of `Φ`: expanding the convolution
product and rewriting through the comultiplication- and counit-compatibility
of `Φ` reduces that to the `rTensor` antipode axiom of `C` in
algebra-homomorphism form (`hopf_mul_antipode_rTensor_comul_algHom`); left
inverses agree with THE inverse in a group. -/
theorem antipodeAlgHom_comp_bialgHom {K₁ : Type*} [CommSemiring K₁] {B C : Type*}
    [CommRing B] [HopfAlgebra K₁ B] [CommRing C] [HopfAlgebra K₁ C]
    (Φ : B →ₐc[K₁] C) :
    (HopfAlgebra.antipodeAlgHom K₁ C).comp (Φ : B →ₐ[K₁] C) =
      (Φ : B →ₐ[K₁] C).comp (HopfAlgebra.antipodeAlgHom K₁ B) := by
  have hAA : AlgHomClass.toAlgHom Φ = (Φ : B →ₐ[K₁] C) := rfl
  have hcomul : (Algebra.TensorProduct.map (Φ : B →ₐ[K₁] C) (Φ : B →ₐ[K₁] C)).comp
      (Bialgebra.comulAlgHom K₁ B) =
      (Bialgebra.comulAlgHom K₁ C).comp (Φ : B →ₐ[K₁] C) := by
    rw [← hAA]; exact BialgHomClass.map_comp_comulAlgHom Φ
  have h1 : WithConv.toConv ((HopfAlgebra.antipodeAlgHom K₁ C).comp (Φ : B →ₐ[K₁] C)) *
      WithConv.toConv (Φ : B →ₐ[K₁] C) = 1 := by
    rw [AlgHom.convMul_def, AlgHom.convOne_def]
    refine congrArg WithConv.toConv ?_
    rw [WithConv.ofConv_toConv, WithConv.ofConv_toConv]
    have hsplit : Algebra.TensorProduct.map
        ((HopfAlgebra.antipodeAlgHom K₁ C).comp (Φ : B →ₐ[K₁] C)) (Φ : B →ₐ[K₁] C) =
        (Algebra.TensorProduct.map (HopfAlgebra.antipodeAlgHom K₁ C)
          (AlgHom.id K₁ C)).comp
          (Algebra.TensorProduct.map (Φ : B →ₐ[K₁] C) (Φ : B →ₐ[K₁] C)) := by
      rw [← Algebra.TensorProduct.map_comp, AlgHom.id_comp]
    have hcounit : (Bialgebra.counitAlgHom K₁ C).comp (Φ : B →ₐ[K₁] C) =
        Bialgebra.counitAlgHom K₁ B := by
      rw [← hAA]; exact BialgHomClass.counitAlgHom_comp Φ
    rw [hsplit, AlgHom.comp_assoc, hcomul, ← AlgHom.comp_assoc, ← AlgHom.comp_assoc,
      AlgHom.comp_assoc _ _ (Bialgebra.comulAlgHom K₁ C),
      hopf_mul_antipode_rTensor_comul_algHom, AlgHom.comp_assoc, hcounit]
  have h3 := eq_inv_of_mul_eq_one_left h1
  rw [show (WithConv.toConv ((Φ : B →ₐ[K₁] C)))⁻¹ =
      WithConv.toConv ((Φ : B →ₐ[K₁] C).comp (HopfAlgebra.antipodeAlgHom K₁ B)) from
    rfl] at h3
  exact WithConv.toConv_injective h3

/-- **Hopf-closed orders transport along bialgebra equivalences** (glue for
the Katz–Mazur leaf; curve-free): the image of a Hopf-closed `R₀`-order under
a `K₀`-bialgebra equivalence of commutative Hopf `K₀`-algebras (both towers
over `R₀`) is again a Hopf-closed order — module-finiteness, `K₀`-spanning,
counit integrality, antipode stability and comultiplication closure all
transport pointwise through the structure-map compatibilities of the
equivalence, the antipode one being `antipodeAlgHom_comp_bialgHom`. -/
theorem exists_hopf_order_map_of_bialgEquiv
  ...
  · have hcoe : (↑(H₀.map ψ) : Set HK₂) = ⇑ΦA.toLinearMap '' (H₀ : Set HK₁) := by
    ...
    rw [hcoe, Submodule.span_image, hspan, Submodule.map_top,
      LinearMap.range_eq_top]
    exact Φ.toAlgEquiv.surjective
  · rintro y hy
...
/-- **Every element of a finite étale algebra has a separable annihilator**
(PROVEN 2026-07-24; glue for `subalgebra_eq_top_of_algHom_separating`): the
product of the distinct minimal polynomials of the images of the element in
the (finitely many, `A` being artinian) residue fields — each separable
because the residue fields of a finite étale algebra are separable extensions,
distinct monic irreducibles are coprime, so the product is separable; and it
kills the element because it kills it in every residue field and the artinian
reduced `A` has zero Jacobson radical. The étale counterpart of
`galoisEquivariantAlgebra_exists_separable_annihilator`. -/
theorem exists_separable_annihilator_of_etale
    (A : Type*) [CommRing A] [Algebra K A] [Module.Finite K A] [Algebra.Etale K A]
    (x : A) :
    ∃ P : Polynomial K, P.Separable ∧ Polynomial.aeval x P = 0 := by
  classical
  ...
  -- the residue fields are separable extensions, and images of `x` are integral
  have hfinres : ∀ m : MaximalSpectrum A, Module.Finite K (A ⧸ m.asIdeal) :=
    fun m => Module.Finite.of_surjective (Ideal.Quotient.mkₐ K m.asIdeal).toLinearMap
      (Ideal.Quotient.mkₐ_surjective K m.asIdeal)
  have hint : ∀ m : MaximalSpectrum A,
      IsIntegral K (Ideal.Quotient.mkₐ K m.asIdeal x) := fun m => by
    haveI := hfinres m
    exact IsIntegral.of_finite K _
  set T : Finset (Polynomial K) := Finset.image
    (fun m : MaximalSpectrum A => minpoly K (Ideal.Quotient.mkₐ K m.asIdeal x))
    Finset.univ with hT
  refine ⟨T.prod id, ?_, ?_⟩
  · refine Polynomial.separable_prod' ?_ ?_
    · intro p hp q hq hpq
      ...
      obtain ⟨a, -, rfl⟩ := Finset.mem_image.mp hp
      obtain ⟨b, -, rfl⟩ := Finset.mem_image.mp hq
      refine (minpoly.irreducible (hint a)).coprime_iff_not_dvd.mpr fun hdvd =>
        hpq ?_
      exact Polynomial.eq_of_monic_of_associated (minpoly.monic (hint a))
        (minpoly.monic (hint b))
        ((minpoly.irreducible (hint a)).associated_of_dvd
    ...
    · intro p hp
      rw [hT] at hp
      obtain ⟨a, -, rfl⟩ := Finset.mem_image.mp hp
      letI : Field (A ⧸ a.asIdeal) := Ideal.Quotient.field a.asIdeal
      haveI : Algebra.IsSeparable K (A ⧸ a.asIdeal) :=
        Algebra.FormallyUnramified.isSeparable K (A ⧸ a.asIdeal)
      exact Algebra.IsSeparable.isSeparable K _
  · have hmem : ∀ m : MaximalSpectrum A,
        Polynomial.aeval x (T.prod id) ∈ m.asIdeal := by
      intro m
      have h1 : Ideal.Quotient.mkₐ K m.asIdeal
          (Polynomial.aeval x (T.prod id)) = 0 := by
        rw [← Polynomial.aeval_algHom_apply, map_prod]
        refine Finset.prod_eq_zero
          (i := minpoly K (Ideal.Quotient.mkₐ K m.asIdeal x)) ?_
          (minpoly.aeval _ _)
        rw [hT]
        exact Finset.mem_image_of_mem _ (Finset.mem_univ m)
      rwa [Ideal.Quotient.mkₐ_eq_mk, Ideal.Quotient.eq_zero_iff_mem] at h1
    have h2 : Polynomial.aeval x (T.prod id) ∈ nilradical A :=
      nilradical_eq_sInf A ▸ Submodule.mem_sInf.mpr fun q hq => by
        haveI : Ideal.IsPrime q := hq
        haveI hqmax : Ideal.IsMaximal q := IsArtinianRing.isMaximal_of_isPrime q
        exact hmem ⟨q, hqmax⟩
    rw [nilradical_eq_zero] at h2
    simpa using h2

/-- **Separable annihilators make a finite algebra formally étale** (PROVEN
2026-07-24; glue for `subalgebra_eq_top_of_algHom_separating`, extracted from
the proof of `galoisEquivariantAlgebra_formallyEtale`): a finite reduced
algebra over the field `K` in which every element is killed by a separable
polynomial is formally étale — it is artinian, hence a finite product of its
residue fields; each residue field is generated by images of elements with
separable annihilators, hence separable over `K`; and a finite product of
separable extensions is formally étale. -/
theorem formallyEtale_of_forall_separable_annihilator
    (B : Type*) [CommRing B] [Algebra K B] [Module.Finite K B] [IsReduced B]
    (hann : ∀ x : B, ∃ P : Polynomial K, P.Separable ∧ Polynomial.aeval x P = 0) :
    Algebra.FormallyEtale K B := by
  classical
  haveI : IsArtinianRing B := isArtinian_of_tower K inferInstance
  have hsep : ∀ m : MaximalSpectrum B,
      Algebra.IsSeparable K (B ⧸ m.asIdeal) := by
    intro m
    constructor
    intro y
    obtain ⟨x, rfl⟩ := Ideal.Quotient.mkₐ_surjective K m.asIdeal y
    obtain ⟨P, hPsep, hP0⟩ := hann x
    have h0 : Polynomial.aeval (Ideal.Quotient.mkₐ K m.asIdeal x) P = 0 := by
      rw [Polynomial.aeval_algHom_apply, hP0, map_zero]
    exact hPsep.of_dvd (minpoly.dvd K _ h0)
  rw [Algebra.FormallyEtale.iff_of_equiv
...
`A ⧸ P` is separable over `K` (residue field of an étale algebra), hence over
the intermediate field `B ⧸ P.comap` (`Ideal.quotientAlgebra`, separability
in the upper part of a tower); `ψ` legitimizes `B ⧸ P.comap` as a subfield of
`Kˢᵉᵖ` (`Ideal.Quotient.liftₐ`), and `IsSepClosed.lift` extends the embedding
from `B ⧸ P.comap` to `A ⧸ P`; the composite with `Ideal.Quotient.mkₐ K P` is
the required point of `A`. -/
theorem exists_algHom_comp_val_eq_of_etale
    (A : Type*) [CommRing A] [Algebra K A] [Module.Finite K A] [Algebra.Etale K A]
...
(DECOMPOSED 2026-07-24 into the two annihilator helpers above — both PROVEN —
and the extension leaf `exists_algHom_comp_val_eq_of_etale`; the assembly
below is proven; curve-free étale leaf of the Katz–Mazur order assembly
`exists_hopf_order_of_good_reduction` — the spanning stage of the Katz–Mazur
roadmap): a `K`-subalgebra `B` of a finite étale `K`-algebra `A` that
separates the `Kˢᵉᵖ`-points of `A` is all of `A`. Proof: `B` is itself finite
étale (every element inherits a separable annihilator from the étale `A`);
the restriction map `(A →ₐ[K] Kˢᵉᵖ) → (B →ₐ[K] Kˢᵉᵖ)` is injective by the
separation hypothesis and surjective by the extension leaf, and it is
`Gal(Kˢᵉᵖ/K)`-equivariant; Grothendieck full faithfulness for the finite
étale algebra `B` (`exists_algHom_of_algHom_map`, applied to the chosen
sections of restriction, an inverse by separation) yields `s : A →ₐ[K] B`
with `B.val ∘ s` acting as the identity on the points of `A`, hence the
identity by separation (`eq_zero_of_forall_algHom_eq_zero`), so the inclusion
`B.val` is surjective and `B = ⊤`. -/
theorem subalgebra_eq_top_of_algHom_separating
  ...
  haveI : Module.Finite K B :=
    Module.Finite.iff_fg.mpr (IsNoetherian.noetherian (Subalgebra.toSubmodule B))
  haveI : IsReduced A := Algebra.FormallyUnramified.isReduced_of_field K A
  haveI : IsReduced ↥B := isReduced_of_injective B.val.toRingHom
    Subtype.coe_injective
  -- `B` is finite étale: every element inherits a separable annihilator
  have hann : ∀ b : ↥B,
      ∃ P : Polynomial K, P.Separable ∧ Polynomial.aeval b P = 0 := by
    intro b
    obtain ⟨P, hPsep, hP0⟩ := exists_separable_annihilator_of_etale K A (b : A)
    refine ⟨P, hPsep, Subtype.coe_injective ?_⟩
    calc ((Polynomial.aeval b P : ↥B) : A)
        = Polynomial.aeval ((b : ↥B) : A) P :=
          (Polynomial.aeval_algHom_apply B.val b P).symm
      _ = 0 := hP0
  haveI : Algebra.FormallyEtale K ↥B :=
    formallyEtale_of_forall_separable_annihilator K ↥B hann
  haveI : Algebra.Etale K ↥B :=
    ⟨inferInstance, Algebra.FinitePresentation.of_finiteType.mp inferInstance⟩
  -- sections of the restriction map, from the extension leaf
  choose t' ht' using fun ψ : ↥B →ₐ[K] Ksep =>
    exists_algHom_comp_val_eq_of_etale K Ksep A B ψ
  -- the chosen sections are equivariant (restriction is injective by `hsep`)
  have ht'eq : ∀ (σ : Ksep ≃ₐ[K] Ksep) (ψ : ↥B →ₐ[K] Ksep),
      t' (σ.toAlgHom.comp ψ) = σ.toAlgHom.comp (t' ψ) := by
    intro σ ψ
    refine hsep _ _ fun b hb => ?_
    calc (t' (σ.toAlgHom.comp ψ)) b
        = ((t' (σ.toAlgHom.comp ψ)).comp B.val) ⟨b, hb⟩ := rfl
      _ = (σ.toAlgHom.comp ψ) ⟨b, hb⟩ := by rw [ht' (σ.toAlgHom.comp ψ)]
      _ = σ (ψ ⟨b, hb⟩) := rfl
      _ = σ (((t' ψ).comp B.val) ⟨b, hb⟩) := by rw [ht' ψ]
      _ = (σ.toAlgHom.comp (t' ψ)) b := rfl
  -- full faithfulness gives a retraction `s : A →ₐ[K] B`
  obtain ⟨s, hs⟩ := exists_algHom_of_algHom_map K Ksep (↥B) A t' ht'eq
  -- `B.val ∘ s` is the identity on points, hence the identity
  have hid : ∀ x : A, ((s x : ↥B) : A) = x := by
    intro x
    have hzero : ((s x : ↥B) : A) - x = 0 := by
      refine eq_zero_of_forall_algHom_eq_zero K Ksep A _ fun φ => ?_
      rw [map_sub, sub_eq_zero]
      have h2 : t' (φ.comp B.val) = φ := by
        refine hsep _ _ fun b hb => ?_
        calc (t' (φ.comp B.val)) b
            = ((t' (φ.comp B.val)).comp B.val) ⟨b, hb⟩ := rfl
          _ = (φ.comp B.val) ⟨b, hb⟩ := by rw [ht' (φ.comp B.val)]
          _ = φ b := rfl
      calc φ ((s x : ↥B) : A)
          = (φ.comp B.val) (s x) := rfl
        _ = t' (φ.comp B.val) x := hs (φ.comp B.val) x
        _ = φ x := by rw [h2]
    exact sub_eq_zero.mp hzero
...
noncomputable def WeierstrassCurve.torsionKernelGens (m : ℕ) (h : Polynomial R) :
    Finset ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep) := by
  classical
  exact (((Finset.range (h.natDegree + 1)) ×ˢ Finset.range 2).filter
    (fun ab => 2 * ab.1 + 3 * ab.2 ≤ 2 * h.natDegree)).image
    (fun ab P => WeierstrassCurve.torsionKernelFun R K E Ksep h ab.1 ab.2 ↑P)

set_option linter.unusedSectionVars false in
/-- Membership in the Katz–Mazur generator family, unfolded. -/
lemma WeierstrassCurve.mem_torsionKernelGens (m : ℕ) (h : Polynomial R)
    (u : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep) :
    u ∈ WeierstrassCurve.torsionKernelGens R K E Ksep m h ↔
      ∃ a b : ℕ, a ≤ h.natDegree ∧ b ≤ 1 ∧ 2 * a + 3 * b ≤ 2 * h.natDegree ∧
        u = fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
          WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑P := by
  classical
  unfold WeierstrassCurve.torsionKernelGens
  rw [Finset.mem_image]
  constructor
  · rintro ⟨ab, hab, rfl⟩
    rw [Finset.mem_filter, Finset.mem_product, Finset.mem_range,
      Finset.mem_range] at hab
    exact ⟨ab.1, ab.2, by omega, by omega, hab.2, rfl⟩
  · rintro ⟨a, b, ha, hb, hw, rfl⟩
    refine ⟨(a, b), ?_, rfl⟩
    rw [Finset.mem_filter, Finset.mem_product, Finset.mem_range,
      Finset.mem_range]
    exact ⟨⟨by omega, by omega⟩, hw⟩

/-- **The product-section span is a subalgebra** (stage E0 of the Katz–Mazur
addition-closure cut `torsionKernelFun_add_mem_span`, PROVEN 2026-07-24):
the split products `(p, q) ↦ a(p)·b(q)` over subalgebras `A`, `B` of
function algebras form a multiplicative submonoid containing `1`, so their
`R₀`-span is the underlying module of the subalgebra they generate — in
particular it is closed under multiplication. Stated over abstract
variables (structure-literal hazard discipline). -/
theorem span_prod_sections_eq_adjoin_toSubmodule
    {R₀ T : Type*} [CommSemiring R₀] [CommSemiring T] [Algebra R₀ T]
    {S₁ S₂ : Type*} (A : Subalgebra R₀ (S₁ → T)) (B : Subalgebra R₀ (S₂ → T)) :
    Submodule.span R₀ {u : S₁ × S₂ → T |
        ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u} =
      Subalgebra.toSubmodule (Algebra.adjoin R₀ {u : S₁ × S₂ → T |
        ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u}) := by
  classical
  let Msub : Submonoid (S₁ × S₂ → T) :=
    { carrier := {u : S₁ × S₂ → T |
        ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u}
      one_mem' := ⟨1, A.one_mem, 1, B.one_mem, by funext pq; simp⟩
      mul_mem' := by
        intro t₁ t₂ h₁ h₂
        obtain ⟨a₁, ha₁, b₁, hb₁, rfl⟩ := h₁
        obtain ⟨a₂, ha₂, b₂, hb₂, rfl⟩ := h₂
        refine ⟨a₁ * a₂, mul_mem ha₁ ha₂, b₁ * b₂, mul_mem hb₁ hb₂, ?_⟩
        funext pq
        simp only [Pi.mul_apply]
        ring }
  have hset : {u : S₁ × S₂ → T |
      ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u} = ↑Msub := rfl
  rw [hset, Algebra.adjoin_eq_span, Submonoid.closure_eq]

/-- **Membership bridge for the product-section span** (stage E0 of
`torsionKernelFun_add_mem_span`, PROVEN 2026-07-24): membership in the
`R₀`-span of the split products is membership in the subalgebra they
generate. -/
theorem mem_span_prod_sections_iff_mem_adjoin
    {R₀ T : Type*} [CommSemiring R₀] [CommSemiring T] [Algebra R₀ T]
    {S₁ S₂ : Type*} (A : Subalgebra R₀ (S₁ → T)) (B : Subalgebra R₀ (S₂ → T))
    (t : S₁ × S₂ → T) :
    t ∈ Submodule.span R₀ {u : S₁ × S₂ → T |
        ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u} ↔
      t ∈ Algebra.adjoin R₀ {u : S₁ × S₂ → T |
        ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u} := by
  rw [span_prod_sections_eq_adjoin_toSubmodule A B]
  exact Iff.rfl

/-- **Integral values propagate through split products** (stage E1 of the
Katz–Mazur addition-closure cut `torsionKernelFun_add_mem_span`, PROVEN
2026-07-24): if all values of the members of `A` and `B` are integral over
`R₀`, so are all values of the subalgebra generated by the split products
`(p, q) ↦ a(p)·b(q)` — pointwise evaluation is an algebra homomorphism
into the integral closure. -/
theorem adjoin_prod_sections_isIntegral_values
    {R₀ T : Type*} [CommRing R₀] [CommRing T] [Algebra R₀ T]
    {S₁ S₂ : Type*} {A : Subalgebra R₀ (S₁ → T)} {B : Subalgebra R₀ (S₂ → T)}
    (hA : ∀ g ∈ A, ∀ s : S₁, _root_.IsIntegral R₀ (g s))
    (hB : ∀ g ∈ B, ∀ s : S₂, _root_.IsIntegral R₀ (g s)) :
    ∀ g ∈ Algebra.adjoin R₀ {u : S₁ × S₂ → T |
        ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u},
      ∀ pq : S₁ × S₂, _root_.IsIntegral R₀ (g pq) := by
  intro g hg pq
  have hle : Algebra.adjoin R₀ {u : S₁ × S₂ → T |
      ∃ a ∈ A, ∃ b ∈ B, (fun pq => a pq.1 * b pq.2) = u} ≤
      Subalgebra.comap (Pi.evalAlgHom R₀ (fun _ : S₁ × S₂ => T) pq)
        (integralClosure R₀ T) := by
    refine Algebra.adjoin_le ?_
    rintro u ⟨a, ha, b, hb, rfl⟩
    show _root_.IsIntegral R₀ (a pq.1 * b pq.2)
    exact (hA a ha pq.1).mul (hB b hb pq.2)
  exact hle hg

set_option backward.isDefEq.respectTransparency false in
/-- **Integrality of the monomial-section algebra values** (stage E1 of the
Katz–Mazur addition-closure cut `torsionKernelFun_add_mem_span`, PROVEN
2026-07-24): every member of the subalgebra generated by the monomial
sections takes values integral over `R` on the `m`-torsion — pointwise
evaluation lands in the integral closure by `torsionKernelFun_isIntegral`. -/
lemma WeierstrassCurve.adjoin_torsionKernelGens_isIntegral_values
    (m : ℕ) (h : Polynomial R) (hmon : h.Monic)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
        ...
        x ∈ 𝒪 → 𝒪.valuation (Polynomial.aeval x h) = 1) :
    ∀ g ∈ Algebra.adjoin R
        ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
          ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
      ∀ P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ),
        _root_.IsIntegral R (g P) := by
  classical
  intro g hg P
  have hle : Algebra.adjoin R
      ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))) ≤
      Subalgebra.comap (Pi.evalAlgHom R
        (fun _ : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) => Ksep) P)
        (integralClosure R Ksep) := by
    refine Algebra.adjoin_le ?_
    intro u hu
    obtain ⟨a, b, _, _, hw, rfl⟩ :=
      (WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h u).mp
        (Finset.mem_coe.mp hu)
    show _root_.IsIntegral R
      (WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑P)
    exact WeierstrassCurve.torsionKernelFun_isIntegral R K E Ksep m h hmon hunit
      a b hw ↑P ((Submodule.mem_torsionBy_iff _ _).mp P.2)
  exact hle hg
...
(`LocalSubring.exists_le_valuationSubring`), match units through the
domination (`IsLocalization.AtPrime.isUnit_to_map_iff`), and read the
center off the overring-of-a-DVR dichotomy (`ValuationRing.iff_dvd_total`),
using that `𝔫` lies over the maximal ideal of `R` by integrality
(`Ideal.isMaximal_comap_of_isIntegral_of_isMaximal` through
`Subalgebra.isIntegral_iff`). -/
theorem exists_valuationSubring_center_of_isMaximal
    (C : Subalgebra R Ksep) (hC : ∀ x ∈ C, _root_.IsIntegral R x)
...
Katz–Mazur addition-closure cut `torsionKernelFun_add_mem_span`, PROVEN
2026-07-24 over the Chevalley leaf
`exists_valuationSubring_center_of_isMaximal`): finitely many members of a
subalgebra `B` of integrally-valued functions on a finite set that include,
at every point and every valuation subring of `Kˢᵉᵖ` centered on `R`, a
valuation unit, generate the unit ideal of `B`: `∑ cᵢ·fᵢ = 1` with
`cᵢ ∈ B`. Every maximal ideal of `B` contains the vanishing ideal of some
point (the evaluation kernels intersect to zero, and a prime above a finite
infimum absorbs one term); pushing the maximal ideal into the value algebra
at that point and dominating by a centered valuation subring exhibits a
family member outside it — so no maximal ideal contains the whole family. -/
theorem exists_partition_of_unity_of_center_units
    {S : Type*} [Finite S] (B : Subalgebra R (S → Ksep))
    (hint : ∀ g ∈ B, ∀ s : S, _root_.IsIntegral R (g s))
    {k : ℕ} (f : Fin k → (S → Ksep)) (hf : ∀ i, f i ∈ B)
    (hcover : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
      ∀ s : S, ∃ i, 𝒪.valuation (f i s) = 1) :
    ∃ c : Fin k → (S → Ksep), (∀ i, c i ∈ B) ∧ ∑ i, c i * f i = 1 := by
  classical
  cases nonempty_fintype S
  set fB : Fin k → B := fun i => ⟨f i, hf i⟩
  set ev : S → (B →ₐ[R] Ksep) :=
    fun s => (Pi.evalAlgHom R (fun _ : S => Ksep) s).comp B.val
  have hspan : Ideal.span (Set.range fB) = ⊤ := by
    by_contra hne
    obtain ⟨𝔫, h𝔫max, hle𝔫⟩ := Ideal.exists_le_maximal _ hne
    -- the vanishing ideal of some point sits below `𝔫`
    have hinfbot : (Finset.univ.inf fun s : S => RingHom.ker (ev s)) = ⊥ := by
      rw [eq_bot_iff]
      intro g hg
      have hg0 : ∀ s : S, (g : S → Ksep) s = 0 := fun s =>
        RingHom.mem_ker.mp
          (Finset.inf_le (f := fun s : S => RingHom.ker (ev s))
            (Finset.mem_univ s) hg)
      exact (Submodule.mem_bot _).mpr (Subtype.ext (funext hg0))
    obtain ⟨s, -, hs⟩ := (Ideal.IsPrime.inf_le' h𝔫max.isPrime).mp
      (le_trans (le_of_eq hinfbot) bot_le)
    -- push `𝔫` into the value algebra at `s`
    have hkerψ : RingHom.ker (ev s).rangeRestrict ≤ 𝔫 := fun g hg => hs (by
      rw [RingHom.mem_ker] at hg ⊢
      exact congrArg Subtype.val hg)
    haveI h𝔫'max : (Ideal.map (ev s).rangeRestrict 𝔫).IsMaximal := by
      rcases Ideal.map_eq_top_or_isMaximal_of_surjective _
        (AlgHom.rangeRestrict_surjective (ev s)) h𝔫max with htop | hmax
      · exfalso
        have h1 : Ideal.comap (ev s).rangeRestrict
            (Ideal.map (ev s).rangeRestrict 𝔫) = ⊤ := by
          rw [htop, Ideal.comap_top]
        rw [Ideal.comap_map_of_surjective _
          (AlgHom.rangeRestrict_surjective (ev s)) 𝔫] at h1
        have h2 : 𝔫 ⊔ Ideal.comap (ev s).rangeRestrict ⊥ = 𝔫 :=
          sup_eq_left.mpr hkerψ
        rw [h2] at h1
        exact h𝔫max.ne_top h1
      · exact hmax
    -- the value algebra at `s` consists of integral elements
    have hCint : ∀ x ∈ (ev s).range, _root_.IsIntegral R x := by
      rintro x ⟨g, rfl⟩
      exact hint (g : S → Ksep) g.2 s
    -- Chevalley: a centered valuation subring dominating the pushed ideal
    obtain ⟨𝒪, hcen, hprops⟩ := exists_valuationSubring_center_of_isMaximal
      R K Ksep (ev s).range hCint (Ideal.map (ev s).rangeRestrict 𝔫)
    obtain ⟨i, hi⟩ := hcover 𝒪 hcen s
    have hfi𝔫 : fB i ∈ 𝔫 := hle𝔫 (Ideal.subset_span ⟨i, rfl⟩)
    have hnon : (((ev s).rangeRestrict (fB i) : (ev s).range) : Ksep) ∈
        𝒪.nonunits :=
      (hprops ((ev s).rangeRestrict (fB i))).2.mp
        (Ideal.mem_map_of_mem _ hfi𝔫)
    have hlt : 𝒪.valuation (f i s) < 1 := 𝒪.mem_nonunits_iff.mp hnon
    rw [hi] at hlt
    exact absurd hlt (lt_irrefl _)
  -- extract the coefficients of `1` over the family
  have h1B : (1 : B) ∈ Ideal.span (Set.range fB) := by
    rw [hspan]
    exact Submodule.mem_top
  obtain ⟨c, hc⟩ := (Submodule.mem_span_range_iff_exists_fun _).mp h1B
  refine ⟨fun i => (c i : S → Ksep), fun i => (c i).2, ?_⟩
  calc ∑ i, (c i : S → Ksep) * f i
      = ∑ i, (B.val) (c i • fB i) := Finset.sum_congr rfl fun i _ => by
        rw [smul_eq_mul, map_mul]
        rfl
    _ = B.val (∑ i, c i • fB i) := (map_sum B.val _ _).symm
    _ = B.val 1 := by rw [hc]
    _ = 1 := map_one _

/-- **The split-product subalgebra is generated by a finite set** (stage E3
glue, PROVEN 2026-07-24): the subalgebra generated by the split products
`(p, q) ↦ a(p)·b(q)` over the subalgebras `adjoin R₀ s`, `adjoin R₀ t` is
already generated by the *pulled-back generators alone* — the two families
`p-coordinate pullbacks of s` and `q-coordinate pullbacks of t`.  One
inclusion is that a split product is the product of the two pullbacks (each
pullback is an algebra map applied to a member of an adjoin, hence lands in
the adjoin of the pulled-back generators); the other is that a pullback is
the split product with the constant `1` on the other factor.  Consequence:
finitely many generators upstairs give finitely many downstairs, so the
split-product algebra of a finitely generated pair is a finitely generated
`R₀`-algebra. -/
theorem adjoin_prod_sections_eq_adjoin_union
    {R₀ T : Type*} [CommRing R₀] [CommRing T] [Algebra R₀ T]
    {S₁ S₂ : Type*} (s : Set (S₁ → T)) (t : Set (S₂ → T)) :
    Algebra.adjoin R₀ {u : S₁ × S₂ → T |
        ∃ a ∈ Algebra.adjoin R₀ s, ∃ b ∈ Algebra.adjoin R₀ t,
          (fun pq => a pq.1 * b pq.2) = u} =
      Algebra.adjoin R₀ ((fun a : S₁ → T => fun pq : S₁ × S₂ => a pq.1) '' s ∪
        (fun b : S₂ → T => fun pq : S₁ × S₂ => b pq.2) '' t) := by
  set F : (S₁ → T) →ₐ[R₀] (S₁ × S₂ → T) :=
    AlgHom.pi (fun pq : S₁ × S₂ => Pi.evalAlgHom R₀ (fun _ : S₁ => T) pq.1) with hF
  set G : (S₂ → T) →ₐ[R₀] (S₁ × S₂ → T) :=
    AlgHom.pi (fun pq : S₁ × S₂ => Pi.evalAlgHom R₀ (fun _ : S₂ => T) pq.2) with hG
  apply le_antisymm
  · refine Algebra.adjoin_le ?_
    rintro u ⟨a, ha, b, hb, rfl⟩
    have hFa : F a ∈ Algebra.adjoin R₀ ((fun a : S₁ → T => fun pq : S₁ × S₂ => a pq.1) '' s ∪
        (fun b : S₂ → T => fun pq : S₁ × S₂ => b pq.2) '' t) := by
      have hle : Algebra.adjoin R₀ s ≤ Subalgebra.comap F
          (Algebra.adjoin R₀ ((fun a : S₁ → T => fun pq : S₁ × S₂ => a pq.1) '' s ∪
            (fun b : S₂ → T => fun pq : S₁ × S₂ => b pq.2) '' t)) :=
        Algebra.adjoin_le fun c hc => Algebra.subset_adjoin (Or.inl ⟨c, hc, rfl⟩)
      exact hle ha
    have hGb : G b ∈ Algebra.adjoin R₀ ((fun a : S₁ → T => fun pq : S₁ × S₂ => a pq.1) '' s ∪
        (fun b : S₂ → T => fun pq : S₁ × S₂ => b pq.2) '' t) := by
      have hle : Algebra.adjoin R₀ t ≤ Subalgebra.comap G
          (Algebra.adjoin R₀ ((fun a : S₁ → T => fun pq : S₁ × S₂ => a pq.1) '' s ∪
            (fun b : S₂ → T => fun pq : S₁ × S₂ => b pq.2) '' t)) :=
        Algebra.adjoin_le fun c hc => Algebra.subset_adjoin (Or.inr ⟨c, hc, rfl⟩)
      exact hle hb
    exact mul_mem hFa hGb
  · refine Algebra.adjoin_le ?_
    rintro u (⟨a, ha, rfl⟩ | ⟨b, hb, rfl⟩)
    · exact Algebra.subset_adjoin ⟨a, Algebra.subset_adjoin ha, 1, one_mem _,
        by funext pq; simp⟩
    · exact Algebra.subset_adjoin ⟨1, one_mem _, b, Algebra.subset_adjoin hb,
        by funext pq; simp⟩

/-- **From a pointwise conductor unit to a finite branch family** (stage E3
assembly, PROVEN 2026-07-24): the *finiteness* half of the Katz–Mazur branch
family is free, so the geometric leaf only has to produce ONE
denominator-killing unit at each point of each special fibre.

Precisely: let `𝔅` be the subalgebra of two-variable functions generated by
the split products of two finitely generated function algebras, and let
`Gfam` be an arbitrary family of "denominators" (in the application: the
pullbacks `g_{a,b} ∘ add` of the monomial sections along the group law).
The **conductor** `S := {f ∈ 𝔅 | ∀ j, f·Gfam j ∈ 𝔅}` is an ideal of `𝔅`,
and `𝔅` is a finitely generated `R₀`-algebra
(`adjoin_prod_sections_eq_adjoin_union`) over a Noetherian ring, hence
Noetherian — so `S` is generated by finitely many `f₁, …, f_k ∈ S`.  Any
`f ∈ S` is an `𝔅`-combination `f = ∑ cᵢ·fᵢ`; if all the values of `𝔅` are
in the valuation ring `𝒪` (hypothesis `hint`, integrality in the
application) then `v(cᵢ(pq)) ≤ 1` and `v(fᵢ(pq)) ≤ 1`, so `v(f pq) = 1`
forces `v(fᵢ(pq)) = 1` for at least one generator by the ultrametric
inequality.  Hence the finitely many ideal generators inherit the covering
property from the pointwise hypothesis `hlocal`. -/
theorem exists_finite_family_of_conductor_units
    {R₀ : Type*} [CommRing R₀] [IsNoetherianRing R₀]
    {F : Type*} [Field F] [Algebra R₀ F]
    {S₁ S₂ : Type*} (gens₁ : Finset (S₁ → F)) (gens₂ : Finset (S₂ → F))
    {ι : Type*} (Gfam : ι → (S₁ × S₂ → F))
    (𝒱 : Set (ValuationSubring F))
    (hint : ∀ g ∈ Algebra.adjoin R₀ {u : S₁ × S₂ → F |
        ∃ a ∈ Algebra.adjoin R₀ (gens₁ : Set (S₁ → F)),
        ∃ b ∈ Algebra.adjoin R₀ (gens₂ : Set (S₂ → F)),
          (fun pq => a pq.1 * b pq.2) = u},
      ∀ pq : S₁ × S₂, ∀ 𝒪 ∈ 𝒱, g pq ∈ 𝒪)
    (hlocal : ∀ 𝒪 ∈ 𝒱, ∀ pq : S₁ × S₂,
      ∃ f ∈ Algebra.adjoin R₀ {u : S₁ × S₂ → F |
          ∃ a ∈ Algebra.adjoin R₀ (gens₁ : Set (S₁ → F)),
          ∃ b ∈ Algebra.adjoin R₀ (gens₂ : Set (S₂ → F)),
            (fun pq => a pq.1 * b pq.2) = u},
        (∀ i : ι, f * Gfam i ∈ Algebra.adjoin R₀ {u : S₁ × S₂ → F |
          ∃ a ∈ Algebra.adjoin R₀ (gens₁ : Set (S₁ → F)),
          ∃ b ∈ Algebra.adjoin R₀ (gens₂ : Set (S₂ → F)),
            (fun pq => a pq.1 * b pq.2) = u}) ∧
        𝒪.valuation (f pq) = 1) :
    ∃ (k : ℕ) (f : Fin k → (S₁ × S₂ → F)),
      (∀ i, f i ∈ Algebra.adjoin R₀ {u : S₁ × S₂ → F |
          ∃ a ∈ Algebra.adjoin R₀ (gens₁ : Set (S₁ → F)),
          ∃ b ∈ Algebra.adjoin R₀ (gens₂ : Set (S₂ → F)),
            (fun pq => a pq.1 * b pq.2) = u}) ∧
      (∀ i (j : ι), f i * Gfam j ∈ Algebra.adjoin R₀ {u : S₁ × S₂ → F |
          ∃ a ∈ Algebra.adjoin R₀ (gens₁ : Set (S₁ → F)),
          ∃ b ∈ Algebra.adjoin R₀ (gens₂ : Set (S₂ → F)),
            (fun pq => a pq.1 * b pq.2) = u}) ∧
      (∀ 𝒪 ∈ 𝒱, ∀ pq : S₁ × S₂, ∃ i, 𝒪.valuation (f i pq) = 1) := by
  classical
  set 𝔅 : Subalgebra R₀ (S₁ × S₂ → F) := Algebra.adjoin R₀ {u : S₁ × S₂ → F |
      ∃ a ∈ Algebra.adjoin R₀ (gens₁ : Set (S₁ → F)),
      ∃ b ∈ Algebra.adjoin R₀ (gens₂ : Set (S₂ → F)),
        (fun pq => a pq.1 * b pq.2) = u} with h𝔅
  -- `𝔅` is a finitely generated `R₀`-algebra, hence Noetherian
  have hfg : 𝔅.FG := by
    rw [h𝔅, adjoin_prod_sections_eq_adjoin_union]
    refine ⟨(gens₁.image fun a => fun pq : S₁ × S₂ => a pq.1) ∪
      (gens₂.image fun b => fun pq : S₁ × S₂ => b pq.2), ?_⟩
    congr 1
    simp [Finset.coe_image]
  haveI : Algebra.FiniteType R₀ 𝔅 := (Subalgebra.fg_iff_finiteType _).mp hfg
  haveI : IsNoetherianRing 𝔅 := Algebra.FiniteType.isNoetherianRing R₀ 𝔅
  -- the conductor ideal of the denominator family
  let S : Ideal 𝔅 :=
    { carrier := {f : 𝔅 | ∀ j : ι, (f : S₁ × S₂ → F) * Gfam j ∈ 𝔅}
      zero_mem' := by
        intro j
        simp
      add_mem' := by
        intro x y hx hy j
        have hxy := add_mem (hx j) (hy j)
        simpa [add_mul] using hxy
      smul_mem' := by
        intro c x hx j
        have hcx := mul_mem c.2 (hx j)
        simpa [mul_assoc] using hcx }
  obtain ⟨t, ht⟩ : S.FG := IsNoetherian.noetherian S
  refine ⟨t.card, fun i => ((t.equivFin.symm i : 𝔅) : S₁ × S₂ → F), ?_, ?_, ?_⟩
  · exact fun i => (t.equivFin.symm i : 𝔅).2
  · exact fun i j => ((by rw [← ht]; exact Submodule.subset_span (t.equivFin.symm i).2 :
      (t.equivFin.symm i : 𝔅) ∈ S) j)
  · intro 𝒪 h𝒪 pq
    obtain ⟨f, hfB, hfkill, hfval⟩ := hlocal 𝒪 h𝒪 pq
    have hfS : (⟨f, hfB⟩ : 𝔅) ∈ S := hfkill
    have hrange : Set.range (fun i => (t.equivFin.symm i : 𝔅)) = (t : Set 𝔅) := by
      ext y
      exact ⟨by rintro ⟨i, rfl⟩; exact (t.equivFin.symm i).2,
        fun hy => ⟨t.equivFin ⟨y, hy⟩, by simp⟩⟩
    rw [← ht, ← hrange] at hfS
    obtain ⟨c, hc⟩ := (Submodule.mem_span_range_iff_exists_fun 𝔅).mp hfS
    have hcoe : ∑ i, ((c i : S₁ × S₂ → F) pq) *
        (((t.equivFin.symm i : 𝔅) : S₁ × S₂ → F) pq) = f pq := by
      have hc' := congrArg (fun z : 𝔅 => (z : S₁ × S₂ → F) pq) hc
      simpa [smul_eq_mul] using hc'
    by_contra hno
    have hno' : ∀ i : Fin t.card,
        𝒪.valuation (((t.equivFin.symm i : 𝔅) : S₁ × S₂ → F) pq) ≠ 1 :=
      fun i hi => hno ⟨i, hi⟩
    have hlt : ∀ i : Fin t.card, 𝒪.valuation (((c i : S₁ × S₂ → F) pq) *
        (((t.equivFin.symm i : 𝔅) : S₁ × S₂ → F) pq)) < 1 := by
      intro i
      have hc1 : 𝒪.valuation ((c i : S₁ × S₂ → F) pq) ≤ 1 :=
        (𝒪.valuation_le_one_iff _).mpr (hint _ (c i).2 pq 𝒪 h𝒪)
      have hg1 : 𝒪.valuation (((t.equivFin.symm i : 𝔅) : S₁ × S₂ → F) pq) ≤ 1 :=
        (𝒪.valuation_le_one_iff _).mpr (hint _ (t.equivFin.symm i : 𝔅).2 pq 𝒪 h𝒪)
      rw [map_mul]
      exact lt_of_le_of_lt (mul_le_of_le_one_left' hc1)
        (lt_of_le_of_ne hg1 (hno' i))
    have hsum := Valuation.map_sum_lt 𝒪.valuation one_ne_zero
      (fun i (_ : i ∈ Finset.univ) => hlt i)
    rw [hcoe, hfval] at hsum
    exact absurd rfl hsum.ne

/-! #### The branch family: conductor ideal + one pointwise unit (stage E3,
2026-07-24)

`exists_torsionKernel_branch_family` below needs no per-locus family choice
at all.  The denominator-killing sections form the CONDUCTOR IDEAL
`S = {f ∈ M | ∀ admissible (a, b), f·(g_{a,b} ∘ add) ∈ M}` of the
product-section algebra `M` (`torsionPairSpan`, a subalgebra by stage E0);
`M` is a finitely generated algebra over the DVR `R`
(`adjoin_prod_sections_eq_adjoin_union`) hence Noetherian, so `S` is
generated by finitely many of its members — and that generating set IS the
branch family.  Its fibrewise-covering property is transported from ONE
pointwise unit by the ultrametric inequality, all values being `R`-integral
(stage E1): `exists_finite_family_of_conductor_units`.

So the entire remaining geometric content is pointwise
(`exists_torsionKernel_branch_unit`): at ONE centered valuation subring `𝒪`
and ONE pair `(P, Q)`, one element `κ ∈ M` that (kill) multiplies every
pullback `g_{a,b} ∘ add` back into `M` and (unit) has `v(κ(P, Q)) = 1`.  On
the chord locus — both points integral-affine (`IsIntegralAffine`: affine
with abscissa in `𝒪`, i.e. NOT reducing to the origin of the special fibre)
with SEPARATED abscissas — this is PROVEN by the concretely CONSTRUCTED
chord killer `torsionPairChordKiller`, the section-algebra avatar of
`h(x(P+Q))·(x(P)−x(Q))^{2d}` (membership, kill identity and unit value all
proven below).  The residue — the formal-group locus and the collision
locus — is the single leaf
`exists_torsionKernel_branch_unit_offChord`, the mirror mixed locus being
free by the factor swap `torsionPairSpan_swap`. -/
...
set_option linter.unusedSectionVars false in
/-- **The integral-affine locus predicate**: a point of the base change is
*integral-affine* for a valuation subring `𝒪` when it is affine with
abscissa in `𝒪` — equivalently, it does not reduce to the origin of the
special fibre.  The complement (the origin together with the points of
non-integral abscissa) is the formal-group locus of `𝒪`. -/
def WeierstrassCurve.IsIntegralAffine (𝒪 : ValuationSubring Ksep)
    (P : (E⁄Ksep).Point) : Prop :=
  ∃ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
    P = Affine.Point.some x y hns ∧ x ∈ 𝒪

set_option linter.unusedSectionVars false in
/-- The `R`-span `M` of the split products of the monomial-section
subalgebra on the `m`-torsion square, as a named submodule — the target
module of the Katz–Mazur addition-closure cut. -/
noncomputable def WeierstrassCurve.torsionPairSpan (m : ℕ) (h : Polynomial R) :
    Submodule R ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep) :=
  Submodule.span R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
    ...
    ∃ b' ∈ Algebra.adjoin R
      ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
      (fun PQ => a' PQ.1 * b' PQ.2) = t}

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **Swap stability of the product-section span**: precomposition with the
factor swap maps split products to split products (with the factors
exchanged), hence preserves the span. -/
lemma WeierstrassCurve.torsionPairSpan_swap (m : ℕ) (h : Polynomial R)
    {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep}
    (ht : t ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h) :
    (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) => t (PQ.2, PQ.1)) ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  classical
  unfold WeierstrassCurve.torsionPairSpan at ht ⊢
  induction ht using Submodule.span_induction with
  | mem u hu =>
    obtain ⟨a', ha', b', hb', rfl⟩ := hu
    refine Submodule.subset_span ⟨b', hb', a', ha', ?_⟩
    funext PQ
    exact mul_comm _ _
  | zero => exact Submodule.zero_mem _
  | add x y hx hy ihx ihy => exact Submodule.add_mem _ ihx ihy
  | smul r x hx ih => exact Submodule.smul_mem _ r ih

set_option linter.unusedSectionVars false in
/-- **The chord section `δ`**: the antisymmetrized split product
`(x/h) ⊗ (1/h) − (1/h) ⊗ (x/h)`, with value `(x₁ − x₂)/(h(x₁)·h(x₂))` on
affine pairs and `0` on the origin rows and columns. -/
noncomputable def WeierstrassCurve.torsionPairDelta (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.2 -
    WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.2

set_option linter.unusedSectionVars false in
/-- **The ordinate chord section `ν`**: the antisymmetrized split product
`(y/h) ⊗ (1/h) − (1/h) ⊗ (y/h)`, with value `(y₁ − y₂)/(h(x₁)·h(x₂))` on
affine pairs and `0` on the origin rows and columns. -/
noncomputable def WeierstrassCurve.torsionPairNu (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionKernelFun R K E Ksep h 0 1 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.2 -
    WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 0 1 ↑PQ.2

set_option linter.unusedSectionVars false in
/-- **The symmetric cubic tail**: the split-product avatar of
`(x₁ + x₂)(x₁ − x₂)² / (h(x₁)·h(x₂))²` — expanded into the four pair
monomials `x₁³ − x₁²x₂ − x₁x₂² + x₂³` over the common denominator. -/
noncomputable def WeierstrassCurve.torsionPairCubic (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 2 0 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.2 ^ 2 -
    WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.1 ^ 2 *
      (WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.2 *
        WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.2) -
    WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.1 *
      WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.2 ^ 2 +
    WeierstrassCurve.torsionKernelFun R K E Ksep h 0 0 ↑PQ.1 ^ 2 *
      (WeierstrassCurve.torsionKernelFun R K E Ksep h 1 0 ↑PQ.2 *
        WeierstrassCurve.torsionKernelFun R K E Ksep h 2 0 ↑PQ.2)

set_option linter.unusedSectionVars false in
/-- **The chord numerator section**: the split-product avatar of the
multiplied secant numerator
`N = (y₁−y₂)² + a₁(y₁−y₂)(x₁−x₂) − (a₂+x₁+x₂)(x₁−x₂)²` over the
denominator `(h(x₁)·h(x₂))²`; on affine pairs its value is
`x(P+Q)·(x₁−x₂)²/(h(x₁)h(x₂))²` by the multiplied secant formula
(`TorsionCard.add_some_coords`). -/
noncomputable def WeierstrassCurve.torsionPairChordNum (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionPairNu R K E Ksep m h PQ ^ 2 +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₁ *
      (WeierstrassCurve.torsionPairNu R K E Ksep m h PQ *
        WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ) -
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₂ *
      WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ ^ 2 -
    WeierstrassCurve.torsionPairCubic R K E Ksep m h PQ

set_option linter.unusedSectionVars false in
/-- **The chord killer**: the section-algebra avatar of
`h(x(P+Q))·((x₁−x₂)/(h(x₁)h(x₂)))^{2d}` — the sum
`∑ᵢ hᵢ·n̂ⁱ·δ^{2(d−i)}` over the coefficients of `h`, every term carrying
the uniform denominator `(h(x₁)h(x₂))^{2d}`.  It lies in the span `M`
(`torsionPairChordKiller_mem_span`), it is a valuation unit whenever both
points are integral-affine with abscissas separated modulo the maximal
ideal (`torsionPairChordKiller_valuation_eq_one`), and it kills the
addition-law denominators (`torsionPairChordKiller_mul_add_mem`). -/
noncomputable def WeierstrassCurve.torsionPairChordKiller (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  ∑ i ∈ Finset.range (h.natDegree + 1),
    algebraMap R Ksep (h.coeff i) *
      WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ ^ i *
      WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ ^ (2 * (h.natDegree - i))

set_option linter.unusedSectionVars false in
/-- The value of the chord section `δ` on an affine pair. -/
lemma WeierstrassCurve.torsionPairDelta_apply_some (m : ℕ) (h : Polynomial R)
    ...
    WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ =
      (x₁ - x₂) / (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) := by
...
set_option linter.unusedSectionVars false in
/-- The value of the chord numerator section on an affine pair: the
multiplied secant numerator over the uniform denominator. -/
lemma WeierstrassCurve.torsionPairChordNum_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ =
      ((y₁ - y₂) ^ 2 + (E⁄Ksep).a₁ * (y₁ - y₂) * (x₁ - x₂) -
        ((E⁄Ksep).a₂ + x₁ + x₂) * (x₁ - x₂) ^ 2) /
      (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 2 := by
  unfold WeierstrassCurve.torsionPairChordNum WeierstrassCurve.torsionPairNu
    WeierstrassCurve.torsionPairDelta WeierstrassCurve.torsionPairCubic
  rw [hP, hQ]
  simp only [WeierstrassCurve.torsionKernelFun_some, pow_one, pow_zero, mul_one,
    one_mul]
  rw [baseChange_a₁_integralModel R K E Ksep, baseChange_a₂_integralModel R K E Ksep]
  field_simp
  ring

set_option linter.unusedSectionVars false in
/-- The value of the chord killer on an affine pair with the secant
abscissa `x₃`: `h(x₃)·((x₁−x₂)/(h(x₁)h(x₂)))^{2d}`. -/
lemma WeierstrassCurve.torsionPairChordKiller_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (hX₃ : x₃ * (x₁ - x₂) ^ 2 = (y₁ - y₂) ^ 2 +
      (E⁄Ksep).a₁ * (y₁ - y₂) * (x₁ - x₂) -
      ((E⁄Ksep).a₂ + x₁ + x₂) * (x₁ - x₂) ^ 2) :
    WeierstrassCurve.torsionPairChordKiller R K E Ksep m h PQ =
      Polynomial.aeval x₃ h *
        ((x₁ - x₂) / (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h)) ^
          (2 * h.natDegree) := by
  have hδ := WeierstrassCurve.torsionPairDelta_apply_some R K E Ksep m h PQ
    x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ h₁0 h₂0
  have hn : WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ =
      x₃ * WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ ^ 2 := by
    rw [WeierstrassCurve.torsionPairChordNum_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ h₁0 h₂0, hδ, ← hX₃, div_pow, mul_div_assoc]
  simp only [WeierstrassCurve.torsionPairChordKiller]
  rw [Polynomial.aeval_eq_sum_range, Finset.sum_mul]
  refine Finset.sum_congr rfl fun i hi => ?_
  have hile : i ≤ h.natDegree := Nat.lt_succ_iff.mp (Finset.mem_range.mp hi)
  rw [hn, hδ, Algebra.smul_def, mul_pow x₃, ← pow_mul]
  have hexp : 2 * h.natDegree = 2 * i + 2 * (h.natDegree - i) := by omega
  rw [hexp, pow_add]
  ring

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **The chord killer lies in the span `M`** — every constituent is an
`R`-combination of split products of monomial sections. -/
theorem WeierstrassCurve.torsionPairChordKiller_mem_span
    (m : ℕ) (h : Polynomial R) (hdeg : 2 ≤ h.natDegree) :
    WeierstrassCurve.torsionPairChordKiller R K E Ksep m h ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  classical
  unfold WeierstrassCurve.torsionPairSpan
  rw [mem_span_prod_sections_iff_mem_adjoin]
  have hgen : ∀ a b : ℕ, a ≤ h.natDegree → b ≤ 1 →
      ...
      (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
        ...
        Algebra.adjoin R ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
          ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))) :=
    fun a b ha hb hw => Algebra.subset_adjoin
      ((WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h _).mpr
        ⟨a, b, ha, hb, hw, rfl⟩)
  have he := hgen 0 0 (by omega) (by omega) (by omega)
  have hξ := hgen 1 0 (by omega) (by omega) (by omega)
  have hη := hgen 0 1 (by omega) (by omega) (by omega)
  have hγ := hgen 2 0 (by omega) (by omega) (by omega)
  have hpair : ∀ f g : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep,
      f ∈ Algebra.adjoin R ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))) →
      g ∈ Algebra.adjoin R ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))) →
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) => f PQ.1 * g PQ.2) ∈
        Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep |
          ∃ a' ∈ Algebra.adjoin R
            ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
              ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
          ∃ b' ∈ Algebra.adjoin R
            ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
              ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
            (fun PQ => a' PQ.1 * b' PQ.2) = t} :=
    fun f g hf hg => Algebra.subset_adjoin ⟨f, hf, g, hg, rfl⟩
  have hδ : WeierstrassCurve.torsionPairDelta R K E Ksep m h ∈
      Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep |
        ∃ a' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ...
        ∃ b' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
            ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
          (fun PQ => a' PQ.1 * b' PQ.2) = t} :=
    sub_mem (hpair _ _ hξ he) (hpair _ _ he hξ)
  have hν : WeierstrassCurve.torsionPairNu R K E Ksep m h ∈
      Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep |
        ∃ a' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ...
        ∃ b' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
            ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
          (fun PQ => a' PQ.1 * b' PQ.2) = t} :=
    sub_mem (hpair _ _ hη he) (hpair _ _ he hη)
  have hcub : WeierstrassCurve.torsionPairCubic R K E Ksep m h ∈
      Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep |
        ∃ a' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ...
        ∃ b' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
            ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
          (fun PQ => a' PQ.1 * b' PQ.2) = t} :=
    add_mem (sub_mem (sub_mem (hpair _ _ (mul_mem hξ hγ) (pow_mem he 2))
      (hpair _ _ (pow_mem hξ 2) (mul_mem hξ he)))
      (hpair _ _ (mul_mem hξ he) (pow_mem hξ 2)))
      (hpair _ _ (pow_mem he 2) (mul_mem hξ hγ))
  have hnum : WeierstrassCurve.torsionPairChordNum R K E Ksep m h ∈
      Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep |
        ∃ a' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
        ...
        ∃ b' ∈ Algebra.adjoin R
          ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
            ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
          (fun PQ => a' PQ.1 * b' PQ.2) = t} :=
    sub_mem (sub_mem (add_mem (pow_mem hν 2)
      (mul_mem (Subalgebra.algebraMap_mem _ _) (mul_mem hν hδ)))
      (mul_mem (Subalgebra.algebraMap_mem _ _) (pow_mem hδ 2))) hcub
  have hker : WeierstrassCurve.torsionPairChordKiller R K E Ksep m h =
      ∑ i ∈ Finset.range (h.natDegree + 1),
        algebraMap R ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep) (h.coeff i) *
          WeierstrassCurve.torsionPairChordNum R K E Ksep m h ^ i *
          WeierstrassCurve.torsionPairDelta R K E Ksep m h ^
            (2 * (h.natDegree - i)) := by
    funext PQ
    simp only [WeierstrassCurve.torsionPairChordKiller, Finset.sum_apply,
      Pi.mul_apply, Pi.pow_apply, Pi.algebraMap_apply]
  rw [hker]
  exact sum_mem fun i _ =>
    mul_mem (mul_mem (Subalgebra.algebraMap_mem _ _) (pow_mem hnum i))
      (pow_mem hδ _)

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **The chord killer is a fibrewise unit on the separated locus** (PROVEN):
for a pair of integral-affine `m`-torsion points whose abscissas are
separated modulo the maximal ideal of a centered valuation subring, the
chord killer has valuation `1`: its value is `h(x₃)·((x₁−x₂)/(h₁h₂))^{2d}`
with `x₃ = x(P+Q)` integral (so `h(x₃)` is a unit by the avoiding
denominator) and every other factor a unit. -/
theorem WeierstrassCurve.torsionPairChordKiller_valuation_eq_one
    (m : ℕ) (h : Polynomial R)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
    ...
    (hQ : (PQ.2 : (E⁄Ksep).Point) = Affine.Point.some x₂ y₂ hns₂)
    (hx₁ : x₁ ∈ 𝒪) (hx₂ : x₂ ∈ 𝒪)
    (hsep : 𝒪.valuation (x₁ - x₂) = 1) :
    𝒪.valuation (WeierstrassCurve.torsionPairChordKiller R K E Ksep m h PQ) = 1 := by
  classical
  obtain ⟨ha₁, ha₂, ha₃, ha₄, ha₆⟩ :=
    baseChange_coeffs_mem_centered R K E Ksep 𝒪 hc
  have htor₁ : (m : ℤ) • (Affine.Point.some x₁ y₁ hns₁ : (E⁄Ksep).Point) = 0 := by
    rw [← hP]
  ...
  have hne : x₁ ≠ x₂ := by
    intro he
    rw [he, sub_self, map_zero] at hsep
    exact zero_ne_one hsep
  obtain ⟨x₃, y₃, hns₃, hadd, hX₃, hY₃⟩ :=
    TorsionCard.add_some_coords (E.map (algebraMap K Ksep)) hns₁ hns₂ hne
  have hy₁ : y₁ ∈ 𝒪 := WeierstrassCurve.ordinate_mem_of_abscissa_mem 𝒪 (E⁄Ksep)
    ha₁ ha₂ ha₃ ha₄ ha₆ hns₁.1 hx₁
  have hy₂ : y₂ ∈ 𝒪 := WeierstrassCurve.ordinate_mem_of_abscissa_mem 𝒪 (E⁄Ksep)
    ha₁ ha₂ ha₃ ha₄ ha₆ hns₂.1 hx₂
  have hX₃v : 𝒪.valuation (x₃ * (x₁ - x₂) ^ 2) ≤ 1 := by
    rw [hX₃]
    exact (𝒪.valuation_le_one_iff _).mpr
      (sub_mem (add_mem (pow_mem (sub_mem hy₁ hy₂) 2)
        (mul_mem (mul_mem ha₁ (sub_mem hy₁ hy₂)) (sub_mem hx₁ hx₂)))
        (mul_mem (add_mem (add_mem ha₂ hx₁) hx₂) (pow_mem (sub_mem hx₁ hx₂) 2)))
  have hx₃ : x₃ ∈ 𝒪 := by
    rw [map_mul, map_pow, hsep, one_pow, mul_one] at hX₃v
    exact (𝒪.valuation_le_one_iff _).mp hX₃v
  have hadd' : ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
      (E⁄Ksep).Point) = Affine.Point.some x₃ y₃ hns₃ := by
    rw [show ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
        (E⁄Ksep).Point) =
      (PQ.1 : (E⁄Ksep).Point) + (PQ.2 : (E⁄Ksep).Point) from rfl, hP, hQ]
    exact hadd
  have htor₃ : (m : ℤ) • (Affine.Point.some x₃ y₃ hns₃ : (E⁄Ksep).Point) = 0 := by
    rw [← hadd']
    exact (Submodule.mem_torsionBy_iff _ _).mp (PQ.1 + PQ.2).2
  have hu₃ : 𝒪.valuation (Polynomial.aeval x₃ h) = 1 :=
    hunit 𝒪 hc x₃ y₃ hns₃ htor₃ hx₃
  rw [WeierstrassCurve.torsionPairChordKiller_apply_some R K E Ksep m h PQ
    x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ h₁0 h₂0 x₃ hX₃]
  rw [map_mul, map_pow, map_div₀, map_mul, hu₁, hu₂, hsep, hu₃]
  simp

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **The secant addition formula with the plain ordinate** (companion of
`TorsionCard.add_some_coords`, division- and `2`-free): for two affine
points with distinct abscissas the sum is affine, with the multiplied
secant identity for `x₃` and the NEGATED-SLOPE identity for `y₃` itself
(no factor `2`, valid in every characteristic). -/
lemma WeierstrassCurve.add_some_ordinate {x₁ y₁ x₂ y₂ : Ksep}
    (h₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    (h₂ : (E⁄Ksep).toAffine.Nonsingular x₂ y₂) (hx : x₁ ≠ x₂) :
    ∃ (x₃ y₃ : Ksep) (h₃ : (E⁄Ksep).toAffine.Nonsingular x₃ y₃),
      (Affine.Point.some x₁ y₁ h₁ : (E⁄Ksep).Point) +
          Affine.Point.some x₂ y₂ h₂ = Affine.Point.some x₃ y₃ h₃ ∧
      x₃ * (x₁ - x₂) ^ 2 = (y₁ - y₂) ^ 2 +
        (E⁄Ksep).a₁ * (y₁ - y₂) * (x₁ - x₂) -
        ((E⁄Ksep).a₂ + x₁ + x₂) * (x₁ - x₂) ^ 2 ∧
      y₃ * (x₁ - x₂) = -((y₁ - y₂) * (x₃ - x₁)) -
        (y₁ + (E⁄Ksep).a₁ * x₃ + (E⁄Ksep).a₃) * (x₁ - x₂) := by
  classical
  have hxy : ¬(x₁ = x₂ ∧ y₁ = (E⁄Ksep).toAffine.negY x₂ y₂) := fun hc => hx hc.1
  have hd : x₁ - x₂ ≠ 0 := sub_ne_zero.mpr hx
  have hslope : (E⁄Ksep).toAffine.slope x₁ x₂ y₁ y₂ = (y₁ - y₂) / (x₁ - x₂) := by
    rw [Affine.slope, if_neg hx]
  have hS : (E⁄Ksep).toAffine.slope x₁ x₂ y₁ y₂ * (x₁ - x₂) = y₁ - y₂ := by
    rw [hslope, div_mul_cancel₀ _ hd]
  refine ⟨_, _, Affine.nonsingular_add h₁ h₂ hxy,
    Affine.Point.add_some hxy, ?_, ?_⟩
  · rw [Affine.addX]
    linear_combination ((E⁄Ksep).toAffine.slope x₁ x₂ y₁ y₂ * (x₁ - x₂) +
      (y₁ - y₂) + (E⁄Ksep).a₁ * (x₁ - x₂)) * hS
  · rw [Affine.addY, Affine.negY, Affine.negAddY]
    linear_combination (x₁ - (E⁄Ksep).toAffine.addX x₁ x₂
      ((E⁄Ksep).toAffine.slope x₁ x₂ y₁ y₂)) * hS

set_option linter.unusedSectionVars false in
/-- The value of the ordinate chord section `ν` on an affine pair. -/
lemma WeierstrassCurve.torsionPairNu_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (h₁0 : Polynomial.aeval x₁ h ≠ 0) (h₂0 : Polynomial.aeval x₂ h ≠ 0) :
    WeierstrassCurve.torsionPairNu R K E Ksep m h PQ =
      (y₁ - y₂) / (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) := by
...
set_option linter.unusedSectionVars false in
/-- **The chord ordinate section `m̂`**: the split-product avatar of the
multiplied secant ordinate `y₃·(x₁−x₂)³/(h(x₁)h(x₂))³`, built from the
chord atoms with the uniform denominator `(h₁h₂)³`:
`m̂ = −ν·n̂ + σ·ν·δ − τ·q·δ − a₁·n̂·δ − a₃·δ³` with
`σ = (x²/h)⊗(1/h) − (x/h)⊗(x/h)` (value `x₁(x₁−x₂)/(h₁h₂)`),
`τ = (y/h)⊗(1/h)` (value `y₁/(h₁h₂)`) and
`q = (x²/h)⊗(1/h) − 2(x/h)⊗(x/h) + (1/h)⊗(x²/h)` (value
`(x₁−x₂)²/(h₁h₂)`). -/
noncomputable def WeierstrassCurve.torsionPairChordOrd (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  -(WeierstrassCurve.torsionPairNu R K E Ksep m h PQ *
      WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ) +
    ...
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₁ *
      (WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ *
        WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ) -
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₃ *
      WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ ^ 3

set_option linter.unusedSectionVars false in
/-- The value of the chord ordinate section on an affine pair with the
secant coordinates `x₃, y₃`: `y₃·((x₁−x₂)/(h(x₁)h(x₂)))³`. -/
lemma WeierstrassCurve.torsionPairChordOrd_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (hX₃ : x₃ * (x₁ - x₂) ^ 2 = (y₁ - y₂) ^ 2 +
      ...
      ((E⁄Ksep).a₂ + x₁ + x₂) * (x₁ - x₂) ^ 2)
    (hY₃ : y₃ * (x₁ - x₂) = -((y₁ - y₂) * (x₃ - x₁)) -
      (y₁ + (E⁄Ksep).a₁ * x₃ + (E⁄Ksep).a₃) * (x₁ - x₂)) :
    WeierstrassCurve.torsionPairChordOrd R K E Ksep m h PQ =
      y₃ * ((x₁ - x₂) / (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h)) ^ 3 := by
  have hδ := WeierstrassCurve.torsionPairDelta_apply_some R K E Ksep m h PQ
    x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ h₁0 h₂0
  have hν := WeierstrassCurve.torsionPairNu_apply_some R K E Ksep m h PQ
    x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ h₁0 h₂0
  have hn := WeierstrassCurve.torsionPairChordNum_apply_some R K E Ksep m h PQ
    x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ h₁0 h₂0
  have hM : y₃ * (x₁ - x₂) ^ 3 =
      -((y₁ - y₂) * ((y₁ - y₂) ^ 2 + (E⁄Ksep).a₁ * (y₁ - y₂) * (x₁ - x₂) -
        ((E⁄Ksep).a₂ + x₁ + x₂) * (x₁ - x₂) ^ 2)) +
      x₁ * (y₁ - y₂) * (x₁ - x₂) ^ 2 - y₁ * (x₁ - x₂) ^ 3 -
      (E⁄Ksep).a₁ * ((y₁ - y₂) ^ 2 + (E⁄Ksep).a₁ * (y₁ - y₂) * (x₁ - x₂) -
        ((E⁄Ksep).a₂ + x₁ + x₂) * (x₁ - x₂) ^ 2) * (x₁ - x₂) -
      (E⁄Ksep).a₃ * (x₁ - x₂) ^ 3 := by
  ...
  simp only [WeierstrassCurve.torsionKernelFun_some, pow_one, pow_zero, mul_one,
    one_mul]
  rw [hν, hδ, hn, ← baseChange_a₁_integralModel R K E Ksep,
    ← baseChange_a₃_integralModel R K E Ksep]
  conv_rhs => rw [div_pow, ← mul_div_assoc, hM]
  field_simp
  ring

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **The chord atom sections lie in the span `M`**: the chord section `δ`,
the chord numerator `n̂` and the chord ordinate `m̂` are `R`-combinations of
split products of monomial sections. -/
theorem WeierstrassCurve.torsionPairChord_sections_mem_span
    (m : ℕ) (h : Polynomial R) (hdeg : 2 ≤ h.natDegree) :
    WeierstrassCurve.torsionPairDelta R K E Ksep m h ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
    WeierstrassCurve.torsionPairChordNum R K E Ksep m h ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
    WeierstrassCurve.torsionPairChordOrd R K E Ksep m h ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  classical
  unfold WeierstrassCurve.torsionPairSpan
  simp only [mem_span_prod_sections_iff_mem_adjoin]
  have hgen : ∀ a b : ℕ, a ≤ h.natDegree → b ≤ 1 →
      2 * a + 3 * b ≤ 2 * h.natDegree →
      (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
          WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑P) ∈
      ...
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        ...
        Algebra.adjoin R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          ...
          ∃ b' ∈ Algebra.adjoin R
            ...
            (fun PQ => a' PQ.1 * b' PQ.2) = t} :=
    fun f g hf hg => Algebra.subset_adjoin ⟨f, hf, g, hg, rfl⟩
  have hδ := sub_mem (hpair _ _ hξ he) (hpair _ _ he hξ)
  have hν := sub_mem (hpair _ _ hη he) (hpair _ _ he hη)
  have hcub := add_mem (sub_mem (sub_mem (hpair _ _ (mul_mem hξ hγ) (pow_mem he 2))
    (hpair _ _ (pow_mem hξ 2) (mul_mem hξ he)))
    (hpair _ _ (mul_mem hξ he) (pow_mem hξ 2)))
    (hpair _ _ (pow_mem he 2) (mul_mem hξ hγ))
  have hnum : WeierstrassCurve.torsionPairChordNum R K E Ksep m h ∈
    ...
    sub_mem (sub_mem (add_mem (pow_mem hν 2)
      ...
      (mul_mem (Subalgebra.algebraMap_mem _ _) (pow_mem hδ 2))) hcub
  refine ⟨hδ, hnum, ?_⟩
  have hσ := sub_mem (hpair _ _ hγ he) (hpair _ _ hξ hξ)
  have hτ := hpair _ _ hη he
  have hq := add_mem (sub_mem (sub_mem (hpair _ _ hγ he) (hpair _ _ hξ hξ))
    (hpair _ _ hξ hξ)) (hpair _ _ he hγ)
  exact sub_mem (sub_mem (sub_mem (add_mem (neg_mem (mul_mem hν hnum))
    (mul_mem hσ (mul_mem hν hδ))) (mul_mem hτ (mul_mem hq hδ)))
    (mul_mem (Subalgebra.algebraMap_mem _ _) (mul_mem hnum hδ)))
    (mul_mem (Subalgebra.algebraMap_mem _ _) (pow_mem hδ 3))

set_option backward.isDefEq.respectTransparency false in
set_option maxHeartbeats 1000000 in
/-- **The chord kill identity** (PROVEN 2026-07-24 — stage E3a of the
Katz–Mazur branch family): the chord killer multiplies every pullback
`g_{a,b} ∘ add` of a monomial section along the group addition back into
the span `M`.  The engine is the GLOBAL function identity
`κ·(g_{a,b}∘add) = n̂^a · m̂^b · δ^{2d−2a−3b}` on the `m`-torsion square —
the right side manifestly in `M` (`torsionPairChord_sections_mem_span`) —
established by the four-way case split:

* origin rows/columns: `δ`, `ν`, the cubic tail, hence `n̂` and the killer
  all vanish, and on the right some factor carries a positive exponent;
* affine collision `x₁ = x₂`: the killer collapses to the monic top term
  `n̂^d`; for `y₁ = y₂` also `n̂ = 0`, and for `y₁ = −y₂ − a₁x₂ − a₃` the
  sum is `0`, the section becomes the weight indicator `2a+3b = 2d`
  (forcing `b = 0`, `a = d` by parity), and both sides equal `n̂^d`;
* the generic secant `x₁ ≠ x₂`: with `x₃, y₃` from `add_some_ordinate`,
  the values are `h(x₃)·f^{2d}·(x₃^a y₃^b/h(x₃))` against
  `(x₃f²)^a·(y₃f³)^b·f^{2d−2a−3b}` for the unit chord fraction
  `f = (x₁−x₂)/(h₁h₂)` — equal by exponent bookkeeping, with `h(x₃) ≠ 0`
  from `torsionKernel_aeval_ne_zero`. -/
theorem WeierstrassCurve.torsionPairChordKiller_mul_add_mem
    (m : ℕ) (h : Polynomial R) (hmon : h.Monic) (hdeg : 2 ≤ h.natDegree)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
        (m : ℤ) • (Affine.Point.some x y hns : (E⁄Ksep).Point) = 0 →
        x ∈ 𝒪 → 𝒪.valuation (Polynomial.aeval x h) = 1)
    (a b : ℕ) (hb : b ≤ 1) (hw : 2 * a + 3 * b ≤ 2 * h.natDegree) :
    WeierstrassCurve.torsionPairChordKiller R K E Ksep m h *
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
        WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑(PQ.1 + PQ.2)) ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  classical
  ...
  have hz01 : WeierstrassCurve.torsionKernelFun R K E Ksep h 0 1
    ...
    rw [WeierstrassCurve.torsionKernelFun_zero]
    exact if_neg (by omega)
  have hid : WeierstrassCurve.torsionPairChordKiller R K E Ksep m h *
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
        WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑(PQ.1 + PQ.2)) =
      WeierstrassCurve.torsionPairChordNum R K E Ksep m h ^ a *
        WeierstrassCurve.torsionPairChordOrd R K E Ksep m h ^ b *
        WeierstrassCurve.torsionPairDelta R K E Ksep m h ^
          (2 * h.natDegree - 2 * a - 3 * b) := by
    funext PQ
    simp only [Pi.mul_apply, Pi.pow_apply]
    cases hc1 : ((PQ.1 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
        (E⁄Ksep).Point) with
    | zero =>
      -- the whole first row: every chord atom vanishes
      ...
      have hn0 : WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ = 0 := by
        ...
        rw [hν0, hδ0, hcub0]
        ring
      have hκ0 : WeierstrassCurve.torsionPairChordKiller R K E Ksep m h PQ = 0 := by
        simp only [WeierstrassCurve.torsionPairChordKiller]
        refine Finset.sum_eq_zero fun i _ => ?_
        rcases Nat.eq_zero_or_pos i with rfl | hipos
        · rw [hδ0, zero_pow (by omega : 2 * (h.natDegree - 0) ≠ 0), mul_zero]
        · rw [hn0, zero_pow (by omega : i ≠ 0), mul_zero, zero_mul]
      rw [hκ0, zero_mul]
      rcases Nat.eq_zero_or_pos a with rfl | hapos
      · rw [hδ0, zero_pow (by omega : 2 * h.natDegree - 2 * 0 - 3 * b ≠ 0),
          mul_zero]
      · rw [hn0, zero_pow (by omega : a ≠ 0), zero_mul, zero_mul]
    | some x₁ y₁ hns₁ =>
      cases hc2 : ((PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) with
      | zero =>
        -- the whole second column: every chord atom vanishes
        have hδ0 : WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ = 0 := by
          unfold WeierstrassCurve.torsionPairDelta
          rw [hc2, hz10, hz00]
          ring
      ...
      | some x₂ y₂ hns₂ =>
        ...
        have h₂0 : Polynomial.aeval x₂ h ≠ 0 :=
          WeierstrassCurve.torsionKernel_aeval_ne_zero R K E Ksep m h hmon hunit
            hns₂ htor₂
        by_cases hxx : x₁ = x₂
        · -- the collision column `x₁ = x₂`
          subst hxx
          have hδ0 : WeierstrassCurve.torsionPairDelta R K E Ksep m h PQ = 0 := by
            rw [WeierstrassCurve.torsionPairDelta_apply_some R K E Ksep m h PQ
              x₁ y₁ x₁ y₂ hns₁ hns₂ hc1 hc2 h₁0 h₂0, sub_self, zero_div]
          have hκd : WeierstrassCurve.torsionPairChordKiller R K E Ksep m h PQ =
              WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ ^
                h.natDegree := by
            simp only [WeierstrassCurve.torsionPairChordKiller]
            rw [Finset.sum_eq_single_of_mem h.natDegree
              (Finset.self_mem_range_succ _) (fun i hi hne => ?_)]
            · rw [hmon.coeff_natDegree, map_one, one_mul, Nat.sub_self,
                mul_zero, pow_zero, mul_one]
            · have hilt : i < h.natDegree := by
                have := Finset.mem_range.mp hi
                omega
              rw [hδ0, zero_pow (by omega : 2 * (h.natDegree - i) ≠ 0), mul_zero]
          rcases Affine.Y_eq_of_X_eq hns₁.1 hns₂.1 rfl with hyy | hyy
          · -- equal points: everything vanishes
            have hn0 : WeierstrassCurve.torsionPairChordNum R K E Ksep m h
              ...
              simp [sub_self]
            rw [hκd, hn0, zero_pow (by omega : h.natDegree ≠ 0), zero_mul]
            rcases Nat.eq_zero_or_pos a with rfl | hapos
            · rw [hδ0, zero_pow (by omega : 2 * h.natDegree - 2 * 0 - 3 * b ≠ 0),
                mul_zero]
            · rw [zero_pow (by omega : a ≠ 0), zero_mul, zero_mul]
          · -- opposite points: the sum is the origin
            ...
            have hcoe : ((PQ.1 + PQ.2 :
              ...
              rw [show ((PQ.1 + PQ.2 :
                ...
                hc1, hc2, hsum0]
              rfl
            rw [hcoe, WeierstrassCurve.torsionKernelFun_zero, hκd]
            by_cases hwt : 2 * a + 3 * b = 2 * h.natDegree
            · have hb0 : b = 0 := by omega
              have had : a = h.natDegree := by omega
              subst hb0
              subst had
              rw [if_pos hwt, mul_one, pow_zero, mul_one,
                show 2 * h.natDegree - 2 * h.natDegree - 3 * 0 = 0 from by omega,
                pow_zero, mul_one]
            · rw [if_neg hwt, mul_zero, hδ0,
                zero_pow (by omega : 2 * h.natDegree - 2 * a - 3 * b ≠ 0),
                mul_zero]
        · -- the generic secant `x₁ ≠ x₂`
          obtain ⟨x₃, y₃, hns₃, hadd, hX₃, hY₃⟩ :=
          ...
          have htor₃ : (m : ℤ) •
            ...
            exact (Submodule.mem_torsionBy_iff _ _).mp (PQ.1 + PQ.2).2
          have h₃0 : Polynomial.aeval x₃ h ≠ 0 :=
            WeierstrassCurve.torsionKernel_aeval_ne_zero R K E Ksep m h hmon
              hunit hns₃ htor₃
          have hκv := WeierstrassCurve.torsionPairChordKiller_apply_some
            R K E Ksep m h PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2 h₁0 h₂0 x₃ hX₃
          have hδv := WeierstrassCurve.torsionPairDelta_apply_some
            R K E Ksep m h PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2 h₁0 h₂0
          have hnv : WeierstrassCurve.torsionPairChordNum R K E Ksep m h PQ =
              x₃ * ((x₁ - x₂) /
                (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h)) ^ 2 := by
            rw [WeierstrassCurve.torsionPairChordNum_apply_some R K E Ksep m h
              PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2 h₁0 h₂0, ← hX₃, div_pow,
              mul_div_assoc]
          have hmv := WeierstrassCurve.torsionPairChordOrd_apply_some
            R K E Ksep m h PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2 h₁0 h₂0 x₃ y₃
            hX₃ hY₃
          rw [hcoe, WeierstrassCurve.torsionKernelFun_some, hκv, hδv, hnv, hmv]
          set f := (x₁ - x₂) / (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h)
            with hf
          set c := 2 * h.natDegree - 2 * a - 3 * b with hcdef
          have hc2d : 2 * h.natDegree = 2 * a + 3 * b + c := by omega
          rw [hc2d, pow_add, pow_add, mul_pow x₃ (f ^ 2), mul_pow y₃ (f ^ 3),
            ← pow_mul, ← pow_mul]
          field_simp
  rw [hid]
  obtain ⟨hδm, hnm, hmm⟩ :=
    WeierstrassCurve.torsionPairChord_sections_mem_span R K E Ksep m h hdeg
  unfold WeierstrassCurve.torsionPairSpan at hδm hnm hmm ⊢
  rw [mem_span_prod_sections_iff_mem_adjoin] at hδm hnm hmm ⊢
  exact mul_mem (mul_mem (pow_mem hnm a) (pow_mem hmm b)) (pow_mem hδm _)

set_option linter.unusedSectionVars false in
/-- **Both factors of a unit product are units**: in a linearly ordered
commutative group with zero, `a·b = 1` with `a ≤ 1` and `b ≤ 1` forces
`a = b = 1`.  The valuative bookkeeping of an addition-law chart: the
numerator sections have integral values, so a unit product pins each
factor. -/
lemma val_factors_eq_one_of_mul_eq_one {Γ : Type*}
    [LinearOrderedCommGroupWithZero Γ] {a b : Γ} (ha : a ≤ 1) (hb : b ≤ 1)
    (hab : a * b = 1) : a = 1 ∧ b = 1 := by
  have h1 : a * b ≤ a := mul_le_of_le_one_right' hb
  have ha1 : a = 1 := le_antisymm ha (by rw [← hab]; exact h1)
  refine ⟨ha1, ?_⟩
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **Values of the product-section span are integral** at every centered
valuation subring: membership in `M` is membership in the subalgebra
generated by the split products (`mem_span_prod_sections_iff_mem_adjoin`),
whose values are integral over `R` (`adjoin_prod_sections_isIntegral_values`
over `adjoin_torsionKernelGens_isIntegral_values`), hence inside every
valuation subring centered on `R` (`mem_centered_of_isIntegral`). -/
lemma WeierstrassCurve.torsionPairSpan_val_mem (m : ℕ) (h : Polynomial R)
    (hmon : h.Monic)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
    ...
    t PQ ∈ 𝒪 := by
  classical
  have hgen := WeierstrassCurve.adjoin_torsionKernelGens_isIntegral_values
    R K E Ksep m h hmon hunit
  unfold WeierstrassCurve.torsionPairSpan at ht
  rw [mem_span_prod_sections_iff_mem_adjoin] at ht
  exact mem_centered_of_isIntegral R K Ksep 𝒪 hcen
    (adjoin_prod_sections_isIntegral_values hgen hgen t ht PQ)

/-! #### The generic addition-law killer (stage E3', 2026-07-25)

The whole residual geometry of stage E3 is the choice of an ADDITION LAW
adapted to the chart at a given pair.  An *addition law* here is a triple
of two-variable sections `(A, B, C)` in the span `M` with

* `A = x(P+Q)·C²` and `B = y(P+Q)·C³` wherever the sum is affine, and
* `C = 0` wherever the sum is the origin

— i.e. `C` is the common denominator section and `A`, `B` the numerators of
the two coordinates of the sum.  Given such a triple, the associated
KILLER is the section-algebra avatar of `h(x(P+Q))·C^{2d}`, namely
`κ := ∑ᵢ hᵢ·Aⁱ·C^{2(d−i)}` (`torsionPairLawKiller`); the chord killer
`torsionPairChordKiller` is exactly the instance `(n̂, m̂, δ)`.  Three
generic facts then hold for EVERY law, with no geometry left:

* `torsionPairLawKiller_mem_span`: `κ ∈ M`, since `M` is an algebra;
* `torsionPairLawKiller_mul_add_mem`: `κ` kills every addition-law
  denominator, by the global identity
  `κ·(g_{a,b}∘add) = A^a·B^b·C^{2d−2a−3b}` — on affine sums both sides are
  `x₃^a y₃^b C^{2d}`, on zero sums `C` vanishes and the weight indicator
  `2a + 3b = 2d` forces `(a, b) = (d, 0)` by parity, both sides collapsing
  to `A^d`;
* `torsionPairLawKiller_valuation_eq_one`: `κ` is a valuation unit at a
  pair as soon as `A` OR `C` is a unit there.  Indeed `κ = h(x₃)·C^{2d}`,
  and `v(A) = v(x₃)·v(C)²`: if `v(C) = 1` then `x₃` is integral, so `h(x₃)`
  is a unit by the avoiding hypothesis; if `v(A) = 1` and `x₃` is
  non-integral then `v(h(x₃)) = v(x₃)^d` (monic dominance) gives
  `v(κ) = v(A)^d = 1`, while if `x₃` is integral the integrality of the
  section values forces `v(C) = 1` anyway.

So the residual leaf is purely the CHART question
`exists_torsionPairLaw_offChord`: produce, at the given pair, a law whose
numerator or denominator section is a unit there. -/

set_option linter.unusedSectionVars false in
/-- **The killer of an addition law**: the section-algebra avatar of
`h(x(P+Q))·C^{2d}` for a law with denominator section `C` and abscissa
numerator `A` — the sum `∑ᵢ hᵢ·Aⁱ·C^{2(d−i)}` over the coefficients of `h`.
With `A = x₃C²` every term carries the uniform factor `C^{2d}`. -/
noncomputable def WeierstrassCurve.torsionPairLawKiller (m : ℕ) (h : Polynomial R)
    (A C : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  ∑ i ∈ Finset.range (h.natDegree + 1),
    algebraMap R Ksep (h.coeff i) * A PQ ^ i * C PQ ^ (2 * (h.natDegree - i))

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **The law killer lies in the span `M`** — a polynomial in the two
sections of the law with coefficients from `R`. -/
theorem WeierstrassCurve.torsionPairLawKiller_mem_span (m : ℕ) (h : Polynomial R)
    {A C : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep}
    (hA : A ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h)
    (hC : C ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h) :
    WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  classical
  have hker : WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C =
      ∑ i ∈ Finset.range (h.natDegree + 1),
        algebraMap R ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep) (h.coeff i) *
          A ^ i * C ^ (2 * (h.natDegree - i)) := by
    funext PQ
    simp only [WeierstrassCurve.torsionPairLawKiller, Finset.sum_apply,
      Pi.mul_apply, Pi.pow_apply, Pi.algebraMap_apply]
  unfold WeierstrassCurve.torsionPairSpan at hA hC ⊢
  rw [mem_span_prod_sections_iff_mem_adjoin] at hA hC ⊢
  rw [hker]
  exact sum_mem fun i _ =>
    mul_mem (mul_mem (Subalgebra.algebraMap_mem _ _) (pow_mem hA i)) (pow_mem hC _)

set_option linter.unusedSectionVars false in
/-- The value of the law killer at a pair whose sum is affine with abscissa
`x₃`: `h(x₃)·C^{2d}`. -/
lemma WeierstrassCurve.torsionPairLawKiller_apply_some (m : ℕ) (h : Polynomial R)
    (A C : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₃ : Ksep) (hAv : A PQ = x₃ * C PQ ^ 2) :
    WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C PQ =
      Polynomial.aeval x₃ h * C PQ ^ (2 * h.natDegree) := by
  simp only [WeierstrassCurve.torsionPairLawKiller]
  rw [Polynomial.aeval_eq_sum_range, Finset.sum_mul]
  refine Finset.sum_congr rfl fun i hi => ?_
  have hile : i ≤ h.natDegree := Nat.lt_succ_iff.mp (Finset.mem_range.mp hi)
  rw [hAv, Algebra.smul_def, mul_pow, ← pow_mul]
  have hexp : 2 * h.natDegree = 2 * i + 2 * (h.natDegree - i) := by omega
  rw [hexp, pow_add]
  ring

set_option linter.unusedSectionVars false in
/-- The value of the law killer at a pair whose sum is the origin, where the
denominator section vanishes: the monic top term `A^d`. -/
lemma WeierstrassCurve.torsionPairLawKiller_apply_zero (m : ℕ) (h : Polynomial R)
    (hmon : h.Monic)
    (A C : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
    ...
    WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C PQ =
      A PQ ^ h.natDegree := by
  simp only [WeierstrassCurve.torsionPairLawKiller]
  rw [Finset.sum_eq_single_of_mem h.natDegree (Finset.self_mem_range_succ _)
    (fun i hi hne => ?_)]
  · rw [hmon.coeff_natDegree, map_one, one_mul, Nat.sub_self, mul_zero, pow_zero,
      mul_one]
  · have hilt : i < h.natDegree := by
      have := Finset.mem_range.mp hi
      omega
    rw [hC0, zero_pow (by omega : 2 * (h.natDegree - i) ≠ 0), mul_zero]

set_option backward.isDefEq.respectTransparency false in
set_option maxHeartbeats 800000 in
/-- **The kill identity of a law killer** (PROVEN 2026-07-25 — the generic
form of the chord kill identity `torsionPairChordKiller_mul_add_mem`): for
any addition law `(A, B, C)` in the span, the killer multiplies every
pullback `g_{a,b} ∘ add` of a monomial section back into the span.  The
engine is the global function identity
`κ·(g_{a,b}∘add) = A^a·B^b·C^{2d−2a−3b}`, checked in the two cases of the
sum: affine (both sides `x₃^a y₃^b C^{2d}`, using `h(x₃) ≠ 0` from
`torsionKernel_aeval_ne_zero`) and origin (`C` vanishes, the weight
indicator forces `(a, b) = (d, 0)` by parity, both sides `A^d`). -/
theorem WeierstrassCurve.torsionPairLawKiller_mul_add_mem
    (m : ℕ) (h : Polynomial R) (hmon : h.Monic)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
    ...
    {A B C : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep}
    (hA : A ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h)
    (hB : B ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h)
    (hC : C ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h)
    (hlaw : ∀ (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
      (x₃ y₃ : Ksep) (hns₃ : (E⁄Ksep).toAffine.Nonsingular x₃ y₃),
      ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) = Affine.Point.some x₃ y₃ hns₃ →
      A PQ = x₃ * C PQ ^ 2 ∧ B PQ = y₃ * C PQ ^ 3)
    (hzero : ∀ PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)),
      ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
    ...
    WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C *
      ...
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  classical
  have hid : WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C *
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
        WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑(PQ.1 + PQ.2)) =
      A ^ a * B ^ b * C ^ (2 * h.natDegree - 2 * a - 3 * b) := by
    funext PQ
    simp only [Pi.mul_apply, Pi.pow_apply]
    have hcases : (∃ (x₃ y₃ : Ksep) (hns₃ : (E⁄Ksep).toAffine.Nonsingular x₃ y₃),
        ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) = Affine.Point.some x₃ y₃ hns₃) ∨
        ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) = Affine.Point.zero := by
      cases hz : ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) with
      | zero => exact Or.inr rfl
      | some x y hns => exact Or.inl ⟨x, y, hns, rfl⟩
    rcases hcases with ⟨x₃, y₃, hns₃, hsum⟩ | hsum
    · obtain ⟨hAv, hBv⟩ := hlaw PQ x₃ y₃ hns₃ hsum
      have htor₃ : (m : ℤ) •
          (Affine.Point.some x₃ y₃ hns₃ : (E⁄Ksep).Point) = 0 := by
        rw [← hsum]
        exact (Submodule.mem_torsionBy_iff _ _).mp (PQ.1 + PQ.2).2
      have h₃0 : Polynomial.aeval x₃ h ≠ 0 :=
        WeierstrassCurve.torsionKernel_aeval_ne_zero R K E Ksep m h hmon hunit
          hns₃ htor₃
      rw [WeierstrassCurve.torsionPairLawKiller_apply_some R K E Ksep m h A C PQ
        x₃ hAv, hsum, WeierstrassCurve.torsionKernelFun_some, hAv, hBv]
      set c := 2 * h.natDegree - 2 * a - 3 * b with hcdef
      have hc2d : 2 * h.natDegree = 2 * a + 3 * b + c := by omega
      rw [hc2d, pow_add, pow_add, mul_pow x₃ (C PQ ^ 2), mul_pow y₃ (C PQ ^ 3),
        ← pow_mul, ← pow_mul]
      field_simp
    · have hC0 := hzero PQ hsum
      rw [WeierstrassCurve.torsionPairLawKiller_apply_zero R K E Ksep m h hmon
        A C PQ hC0, hsum, WeierstrassCurve.torsionKernelFun_zero]
      by_cases hwt : 2 * a + 3 * b = 2 * h.natDegree
      · have hb0 : b = 0 := by omega
        have had : a = h.natDegree := by omega
        subst hb0
        ...
        rw [if_pos hwt, mul_one, pow_zero, mul_one,
          show 2 * h.natDegree - 2 * h.natDegree - 3 * 0 = 0 from by omega,
          pow_zero, mul_one]
      · rw [if_neg hwt, mul_zero, hC0,
          zero_pow (by omega : 2 * h.natDegree - 2 * a - 3 * b ≠ 0), mul_zero]
  rw [hid]
  unfold WeierstrassCurve.torsionPairSpan at hA hB hC ⊢
  rw [mem_span_prod_sections_iff_mem_adjoin] at hA hB hC ⊢
  exact mul_mem (mul_mem (pow_mem hA a) (pow_mem hB b)) (pow_mem hC _)

set_option backward.isDefEq.respectTransparency false in
/-- **A law killer is a unit wherever a chart section of the law is**
(PROVEN 2026-07-25 — the generic form of
`torsionPairChordKiller_valuation_eq_one`): if the abscissa numerator `A`
or the denominator `C` of the law has valuation `1` at the pair, so has the
killer.  The value is `h(x₃)·C^{2d}` on an affine sum and `A^d` on a zero
sum, and `v(A) = v(x₃)·v(C)²` with all section values integral: `v(C) = 1`
makes `x₃` integral hence `h(x₃)` a unit; `v(A) = 1` gives
`v(κ) = v(x₃)^d·v(C)^{2d} = v(A)^d` when `x₃` is non-integral (monic
dominance) and forces `v(C) = 1` when it is integral. -/
theorem WeierstrassCurve.torsionPairLawKiller_valuation_eq_one
    (m : ℕ) (h : Polynomial R) (hmon : h.Monic)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
    ...
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (hlaw : ∀ (x₃ y₃ : Ksep) (hns₃ : (E⁄Ksep).toAffine.Nonsingular x₃ y₃),
      ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) = Affine.Point.some x₃ y₃ hns₃ →
      A PQ = x₃ * C PQ ^ 2)
    (hzero : ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
        (E⁄Ksep).Point) = Affine.Point.zero → C PQ = 0)
    (hu : 𝒪.valuation (A PQ) = 1 ∨ 𝒪.valuation (C PQ) = 1) :
    𝒪.valuation (WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C PQ)
      = 1 := by
  classical
  have hAle : 𝒪.valuation (A PQ) ≤ 1 := (𝒪.valuation_le_one_iff _).mpr
    (WeierstrassCurve.torsionPairSpan_val_mem R K E Ksep m h hmon hunit 𝒪 hcen
      hA PQ)
  have hCle : 𝒪.valuation (C PQ) ≤ 1 := (𝒪.valuation_le_one_iff _).mpr
    (WeierstrassCurve.torsionPairSpan_val_mem R K E Ksep m h hmon hunit 𝒪 hcen
      hC PQ)
  have hcases : (∃ (x₃ y₃ : Ksep) (hns₃ : (E⁄Ksep).toAffine.Nonsingular x₃ y₃),
      ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
    ...
    | some x y hns => exact Or.inl ⟨x, y, hns, rfl⟩
  rcases hcases with ⟨x₃, y₃, hns₃, hsum⟩ | hsum
  · have hAv := hlaw x₃ y₃ hns₃ hsum
    have htor₃ : (m : ℤ) • (Affine.Point.some x₃ y₃ hns₃ : (E⁄Ksep).Point) = 0 := by
      rw [← hsum]
      exact (Submodule.mem_torsionBy_iff _ _).mp (PQ.1 + PQ.2).2
    have hAval : 𝒪.valuation (A PQ) =
        𝒪.valuation x₃ * 𝒪.valuation (C PQ) ^ 2 := by
      rw [hAv, map_mul, map_pow]
    rw [WeierstrassCurve.torsionPairLawKiller_apply_some R K E Ksep m h A C PQ
      x₃ hAv, map_mul, map_pow]
    by_cases hx₃ : x₃ ∈ 𝒪
    · have hh : 𝒪.valuation (Polynomial.aeval x₃ h) = 1 :=
        hunit 𝒪 hcen x₃ y₃ hns₃ htor₃ hx₃
      have hCu : 𝒪.valuation (C PQ) = 1 := by
        rcases hu with hu | hu
        · have hx3le : 𝒪.valuation x₃ ≤ 1 := (𝒪.valuation_le_one_iff _).mpr hx₃
          have hsq : 𝒪.valuation (C PQ) ^ 2 = 1 :=
            (val_factors_eq_one_of_mul_eq_one hx3le (pow_le_one' hCle 2)
              (by rw [← hAval]; exact hu)).2
          exact (val_factors_eq_one_of_mul_eq_one hCle hCle
            (by rw [← pow_two]; exact hsq)).1
        · exact hu
      rw [hh, hCu, one_pow, one_mul]
    · have hh : 𝒪.valuation (Polynomial.aeval x₃ h) =
      ...
      have hAu : 𝒪.valuation (A PQ) = 1 := by
        rcases hu with hu | hu
        · exact hu
        · refine absurd ?_ hx₃
          refine (𝒪.valuation_le_one_iff _).mp ?_
          rw [hAval, hu, one_pow, mul_one] at hAle
          exact hAle
      rw [hh]
      calc 𝒪.valuation x₃ ^ h.natDegree *
            𝒪.valuation (C PQ) ^ (2 * h.natDegree)
          = (𝒪.valuation x₃ * 𝒪.valuation (C PQ) ^ 2) ^ h.natDegree := by
            rw [mul_pow, ← pow_mul]
        _ = 𝒪.valuation (A PQ) ^ h.natDegree := by rw [hAval]
        _ = 1 := by rw [hAu, one_pow]
  · have hC0 := hzero hsum
    rw [WeierstrassCurve.torsionPairLawKiller_apply_zero R K E Ksep m h hmon
      A C PQ hC0, map_pow]
    rcases hu with hu | hu
    · rw [hu, one_pow]
    · rw [hC0, map_zero] at hu
...
/-! #### The SECOND addition law: rationalizing the slope (stage E3b/E3c, 2026-07-25)

The chord law `(n̂, m̂, δ)` degenerates on the COLLISION locus `x₁ ≡ x₂`,
where its denominator section `δ` is not a unit.  The cure recorded in the
docstring of `exists_torsionPairLaw_collisionDiagonal` is a SECOND addition
law, obtained by rationalizing the slope ON the curve: subtracting the two
Weierstrass equations gives

  `(y₁ − y₂)·D₂ = (x₁ − x₂)·N₂`,  `D₂ := y₁ + y₂ + a₁x₂ + a₃`,
  `N₂ := x₁² + x₁x₂ + x₂² + a₂(x₁ + x₂) + a₄ − a₁y₁`,

so the slope of the chord — AND of the tangent, the two cases collapsing
into one formula — is `λ = N₂/D₂` (`slope_mul_secondDen`), whence the
multiplied law identities `x₃·D₂² = Ap` and `y₃·D₂³ = Bp` of
`add_some_second`.  `D₂` vanishes exactly when `Q = ⊖P`, i.e. exactly on
the fibre of the origin, which is what the law's `C = 0` clause needs.

Normalizing with `k₁ = k₂ = 2` — `C₂ := D₂/(h₁h₂)²`, `A₂ := Ap/(h₁h₂)⁴`,
`B₂ := Bp/(h₁h₂)⁶` — puts every constituent in the split-product span `M`
and makes all three sections vanish on the ORIGIN ROWS, which the law
identities require there (the sum is then the other point).  The building
block is `torsionPairQuad`, the split product of two monomial sections on
each side; every atom below is an `R`-combination of those, and the extra
`h`-power is what keeps each `P`-monomial of positive origin order.
-/

set_option linter.unusedSectionVars false in
/-- **Rationalizing the slope on the curve**. -/
theorem WeierstrassCurve.slope_mul_secondDen {F : Type*} [Field F]
    [DecidableEq F] (W : WeierstrassCurve F) {x₁ y₁ x₂ y₂ : F}
  ...
  classical
  have he₁ := (WeierstrassCurve.Affine.equation_iff x₁ y₁).mp h₁
  have he₂ := (WeierstrassCurve.Affine.equation_iff x₂ y₂).mp h₂
  by_cases hx : x₁ = x₂
  · have hy : y₁ ≠ W.toAffine.negY x₂ y₂ := fun hh => hxy ⟨hx, hh⟩
    ...
    rw [WeierstrassCurve.Affine.slope_of_Y_ne rfl hy,
      show y₁ + y₁ + W.a₁ * x₁ + W.a₃ = y₁ - W.toAffine.negY x₁ y₁ from by
...
set_option linter.unusedSectionVars false in
/-- **The second (rationalized-slope) addition law in coordinates**. -/
theorem WeierstrassCurve.add_some_second {F : Type*} [Field F] [DecidableEq F]
    (W : WeierstrassCurve F) {x₁ y₁ x₂ y₂ : F}
    ...
    ∃ (x₃ y₃ : F) (h₃ : W.toAffine.Nonsingular x₃ y₃),
      (WeierstrassCurve.Affine.Point.some x₁ y₁ h₁ : W.toAffine.Point) +
        ...
        WeierstrassCurve.Affine.Point.some x₃ y₃ h₃ ∧
      x₃ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 2 =
        (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + W.a₂ * (x₁ + x₂) + W.a₄ - W.a₁ * y₁) ^ 2 +
          W.a₁ * (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + W.a₂ * (x₁ + x₂) + W.a₄ - W.a₁ * y₁) *
            (y₁ + y₂ + W.a₁ * x₂ + W.a₃) -
          (W.a₂ + x₁ + x₂) * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 2 ∧
      y₃ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 3 =
        -((x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + W.a₂ * (x₁ + x₂) + W.a₄ - W.a₁ * y₁) *
            (x₃ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 2 -
              x₁ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 2)) -
          y₁ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 3 -
          W.a₁ * (x₃ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 2) *
            (y₁ + y₂ + W.a₁ * x₂ + W.a₃) -
          W.a₃ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 3 := by
  classical
  have hS := WeierstrassCurve.slope_mul_secondDen W h₁.1 h₂.1 hxy
  refine ⟨_, _, WeierstrassCurve.Affine.nonsingular_add h₁ h₂ hxy,
    WeierstrassCurve.Affine.Point.add_some hxy, ?_, ?_⟩
  · simp only [WeierstrassCurve.Affine.addX]
    linear_combination
      (W.toAffine.slope x₁ x₂ y₁ y₂ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃) +
        (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + W.a₂ * (x₁ + x₂) + W.a₄ - W.a₁ * y₁) +
        W.a₁ * (y₁ + y₂ + W.a₁ * x₂ + W.a₃)) * hS
  · simp only [WeierstrassCurve.Affine.addY, WeierstrassCurve.Affine.negY,
    ...
    linear_combination
      (-((W.toAffine.slope x₁ x₂ y₁ y₂ ^ 2 + W.a₁ * W.toAffine.slope x₁ x₂ y₁ y₂ -
            W.a₂ - x₁ - x₂) - x₁) *
        (y₁ + y₂ + W.a₁ * x₂ + W.a₃) ^ 2) * hS
...
set_option linter.unusedSectionVars false in
/-- **A level-two split monomial section**. -/
noncomputable def WeierstrassCurve.torsionPairQuad (m : ℕ) (h : Polynomial R)
    (a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ : ℕ) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
...
set_option linter.unusedSectionVars false in
/-- The value of a level-two split monomial section on an affine pair. -/
lemma WeierstrassCurve.torsionPairQuad_apply_some (m : ℕ) (h : Polynomial R)
    (a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ : ℕ)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
...
set_option linter.unusedSectionVars false in
/-- A level-two split monomial section vanishes on the origin row. -/
lemma WeierstrassCurve.torsionPairQuad_apply_zero_left (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) (a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ : ℕ) (hw : a₁ + b₁ ≤ 1)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    ...
    WeierstrassCurve.torsionPairQuad R K E Ksep m h a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ PQ
      = 0 := by
  unfold WeierstrassCurve.torsionPairQuad
  rw [hP, WeierstrassCurve.torsionKernelFun_zero, if_neg (by omega)]
  ring

set_option linter.unusedSectionVars false in
/-- A level-two split monomial section vanishes on the origin column. -/
lemma WeierstrassCurve.torsionPairQuad_apply_zero_right (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) (a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ : ℕ) (hw : a₃ + b₃ ≤ 1)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    ...
    WeierstrassCurve.torsionPairQuad R K E Ksep m h a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ PQ
      = 0 := by
  unfold WeierstrassCurve.torsionPairQuad
  rw [hQ, WeierstrassCurve.torsionKernelFun_zero, if_neg (by omega)]
  ring

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- A level-two split monomial section lies in the span `M`. -/
lemma WeierstrassCurve.torsionPairQuad_mem_span (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) (a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ : ℕ)
    (hw₁ : a₁ + b₁ ≤ 1) (hw₂ : a₂ + b₂ ≤ 1) (hw₃ : a₃ + b₃ ≤ 1)
    (hw₄ : a₄ + b₄ ≤ 1) :
    WeierstrassCurve.torsionPairQuad R K E Ksep m h a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  classical
  have hgen : ∀ a b : ℕ, a + b ≤ 1 →
      (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
          WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑P) ∈
        Algebra.adjoin R ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
          ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))) :=
    fun a b hab => Algebra.subset_adjoin
      ((WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h _).mpr
        ⟨a, b, by omega, by omega, by omega, rfl⟩)
  exact Submodule.subset_span
    ⟨_, mul_mem (hgen a₁ b₁ hw₁) (hgen a₂ b₂ hw₂),
      _, mul_mem (hgen a₃ b₃ hw₃) (hgen a₄ b₄ hw₄), rfl⟩

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The span `M` is closed under multiplication. -/
lemma WeierstrassCurve.torsionPairSpan_mul (m : ℕ) (h : Polynomial R)
    {s t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep}
    (hs : s ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h)
    (ht : t ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h) :
    s * t ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  unfold WeierstrassCurve.torsionPairSpan at hs ht ⊢
  rw [mem_span_prod_sections_iff_mem_adjoin] at hs ht ⊢
  exact mul_mem hs ht

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The span `M` absorbs multiplication by the image of `R`. -/
lemma WeierstrassCurve.torsionPairSpan_algebraMap_mul (m : ℕ) (h : Polynomial R)
    (r : R) {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep}
    (ht : t ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h) :
    (fun PQ => algebraMap R Ksep r * t PQ) ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  have hsm : (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
        algebraMap R Ksep r * t PQ) = r • t := by
    funext PQ
...
set_option linter.unusedSectionVars false in
/-- A monomial section of low weight vanishes at the origin. -/
lemma WeierstrassCurve.torsionKernelFun_zero_eq_zero (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) (a b : ℕ) (hab : a + b ≤ 1) :
    WeierstrassCurve.torsionKernelFun R K E Ksep h a b Affine.Point.zero = 0 := by
  rw [WeierstrassCurve.torsionKernelFun_zero]
  exact if_neg (by omega)

set_option linter.unusedSectionVars false in
/-- **The second-law denominator section** `C₂ = D₂/(h₁h₂)²`. -/
noncomputable def WeierstrassCurve.torsionPairSecondDen (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 1 0 0 0 0 0 0 PQ +
    WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 0 1 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₁ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 1 0 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₃ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 0 0 0 0 PQ

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The value of `C₂` on an affine pair. -/
lemma WeierstrassCurve.torsionPairSecondDen_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (hP : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.some x₁ y₁ hns₁)
    (hQ : (PQ.2 : (E⁄Ksep).Point) = Affine.Point.some x₂ y₂ hns₂) :
    WeierstrassCurve.torsionPairSecondDen R K E Ksep m h PQ =
      (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) /
        (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 2 := by
  unfold WeierstrassCurve.torsionPairSecondDen
  rw [WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 1 0 0 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 0 1 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 1 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    baseChange_a₁_integralModel R K E Ksep,
    baseChange_a₃_integralModel R K E Ksep]
  simp only [Nat.add_zero, pow_zero, pow_one, one_mul, mul_one]
  ring

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- `C₂` vanishes on the origin rows and columns. -/
lemma WeierstrassCurve.torsionPairSecondDen_apply_zero (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (hz : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.zero ∨
      (PQ.2 : (E⁄Ksep).Point) = Affine.Point.zero) :
    WeierstrassCurve.torsionPairSecondDen R K E Ksep m h PQ = 0 := by
  unfold WeierstrassCurve.torsionPairSecondDen WeierstrassCurve.torsionPairQuad
  rcases hz with hz | hz <;>
    simp only [hz,
      WeierstrassCurve.torsionKernelFun_zero_eq_zero R K E Ksep h hdeg 0 0
        (by omega),
      WeierstrassCurve.torsionKernelFun_zero_eq_zero R K E Ksep h hdeg 1 0
        (by omega),
      WeierstrassCurve.torsionKernelFun_zero_eq_zero R K E Ksep h hdeg 0 1
        (by omega),
      mul_zero, zero_mul, add_zero]

set_option linter.unusedSectionVars false in
/-- **The second-law abscissa numerator atom** `n = N₂/(h₁h₂)²`. -/
noncomputable def WeierstrassCurve.torsionPairSecondNum (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionPairQuad R K E Ksep m h 1 0 1 0 0 0 0 0 PQ +
    WeierstrassCurve.torsionPairQuad R K E Ksep m h 1 0 0 0 1 0 0 0 PQ +
    WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 1 0 1 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₂ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 1 0 0 0 0 0 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₂ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 1 0 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₄ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 0 0 0 0 PQ -
...
set_option linter.unusedSectionVars false in
/-- **The abscissa-weighted denominator atom** `v = x₁·D₂/(h₁h₂)²`. -/
noncomputable def WeierstrassCurve.torsionPairSecondDenX (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionPairQuad R K E Ksep m h 1 0 0 1 0 0 0 0 PQ +
    WeierstrassCurve.torsionPairQuad R K E Ksep m h 1 0 0 0 0 1 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₁ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 1 0 0 0 1 0 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₃ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 1 0 0 0 0 0 0 0 PQ
...
set_option linter.unusedSectionVars false in
/-- **The mirror abscissa-weighted denominator atom** `v' = x₂·D₂/(h₁h₂)²`. -/
noncomputable def WeierstrassCurve.torsionPairSecondDenX' (m : ℕ)
    (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 1 0 0 1 0 0 0 PQ +
    WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 1 0 0 1 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₁ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 1 0 1 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₃ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 0 0 0 1 0 0 0 PQ
...
set_option linter.unusedSectionVars false in
/-- **The ordinate-weighted denominator atom** `w = y₁·D₂/(h₁h₂)²`. -/
noncomputable def WeierstrassCurve.torsionPairSecondDenY (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 1 0 1 0 0 0 0 PQ +
    WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 1 0 0 0 1 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₁ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 1 0 0 1 0 0 0 PQ +
    algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₃ *
      WeierstrassCurve.torsionPairQuad R K E Ksep m h 0 1 0 0 0 0 0 0 PQ
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The value of `n` on an affine pair. -/
lemma WeierstrassCurve.torsionPairSecondNum_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (hP : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.some x₁ y₁ hns₁)
    (hQ : (PQ.2 : (E⁄Ksep).Point) = Affine.Point.some x₂ y₂ hns₂) :
    WeierstrassCurve.torsionPairSecondNum R K E Ksep m h PQ =
      (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
          (E⁄Ksep).a₁ * y₁) /
        (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 2 := by
  unfold WeierstrassCurve.torsionPairSecondNum
  rw [WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 1 0 1 0 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 1 0 0 0 1 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 1 0 1 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 1 0 0 0 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 1 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 1 0 0 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    baseChange_a₁_integralModel R K E Ksep,
    baseChange_a₂_integralModel R K E Ksep,
    baseChange_a₄_integralModel R K E Ksep]
  simp only [Nat.add_zero, Nat.reduceAdd, pow_zero, pow_one,
    one_mul, mul_one]
  ring

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The value of `v` on an affine pair. -/
lemma WeierstrassCurve.torsionPairSecondDenX_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (hP : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.some x₁ y₁ hns₁)
    (hQ : (PQ.2 : (E⁄Ksep).Point) = Affine.Point.some x₂ y₂ hns₂) :
    WeierstrassCurve.torsionPairSecondDenX R K E Ksep m h PQ =
      x₁ * (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) /
        (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 2 := by
  unfold WeierstrassCurve.torsionPairSecondDenX
  rw [WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 1 0 0 1 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 1 0 0 0 0 1 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 1 0 0 0 1 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 1 0 0 0 0 0 0 0
    ...
    baseChange_a₁_integralModel R K E Ksep,
    baseChange_a₃_integralModel R K E Ksep]
  simp only [Nat.add_zero, Nat.zero_add, pow_zero, pow_one, one_mul, mul_one]
  ring

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The value of `v'` on an affine pair. -/
lemma WeierstrassCurve.torsionPairSecondDenX'_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    WeierstrassCurve.torsionPairSecondDenX' R K E Ksep m h PQ =
      x₂ * (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) /
        (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 2 := by
  unfold WeierstrassCurve.torsionPairSecondDenX'
  rw [WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 1 0 0 1 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 1 0 0 1
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 1 0 1 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 0 0 0 1 0 0 0
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The value of `w` on an affine pair. -/
lemma WeierstrassCurve.torsionPairSecondDenY_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (hP : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.some x₁ y₁ hns₁)
    (hQ : (PQ.2 : (E⁄Ksep).Point) = Affine.Point.some x₂ y₂ hns₂) :
    WeierstrassCurve.torsionPairSecondDenY R K E Ksep m h PQ =
      y₁ * (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) /
        (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 2 := by
  unfold WeierstrassCurve.torsionPairSecondDenY
  rw [WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 1 0 1 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 1 0 0 0 1 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 1 0 0 1 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairQuad_apply_some R K E Ksep m h 0 1 0 0 0 0 0 0
      PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    baseChange_a₁_integralModel R K E Ksep,
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- `n`, `v`, `v'`, `w` vanish on the origin rows and columns. -/
lemma WeierstrassCurve.torsionPairSecond_apply_zero (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (hz : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.zero ∨
      (PQ.2 : (E⁄Ksep).Point) = Affine.Point.zero) :
    WeierstrassCurve.torsionPairSecondNum R K E Ksep m h PQ = 0 ∧
      WeierstrassCurve.torsionPairSecondDenX R K E Ksep m h PQ = 0 ∧
      WeierstrassCurve.torsionPairSecondDenX' R K E Ksep m h PQ = 0 ∧
      WeierstrassCurve.torsionPairSecondDenY R K E Ksep m h PQ = 0 := by
  refine ⟨?_, ?_, ?_, ?_⟩ <;>
    rcases hz with hz | hz <;>
    simp only [WeierstrassCurve.torsionPairSecondNum,
      WeierstrassCurve.torsionPairSecondDenX,
      WeierstrassCurve.torsionPairSecondDenX',
      WeierstrassCurve.torsionPairSecondDenY, WeierstrassCurve.torsionPairQuad, hz,
      WeierstrassCurve.torsionKernelFun_zero_eq_zero R K E Ksep h hdeg 0 0
        (by omega),
      WeierstrassCurve.torsionKernelFun_zero_eq_zero R K E Ksep h hdeg 1 0
        (by omega),
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- `n`, `v`, `v'`, `w` lie in the span `M`. -/
lemma WeierstrassCurve.torsionPairSecond_mem_span (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) :
    WeierstrassCurve.torsionPairSecondNum R K E Ksep m h ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      WeierstrassCurve.torsionPairSecondDenX R K E Ksep m h ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      WeierstrassCurve.torsionPairSecondDenX' R K E Ksep m h ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      WeierstrassCurve.torsionPairSecondDenY R K E Ksep m h ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  have hq : ∀ a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ : ℕ, a₁ + b₁ ≤ 1 → a₂ + b₂ ≤ 1 →
      a₃ + b₃ ≤ 1 → a₄ + b₄ ≤ 1 →
      WeierstrassCurve.torsionPairQuad R K E Ksep m h a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h :=
    fun a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ h₁ h₂ h₃ h₄ =>
      WeierstrassCurve.torsionPairQuad_mem_span R K E Ksep m h hdeg
        a₁ b₁ a₂ b₂ a₃ b₃ a₄ b₄ h₁ h₂ h₃ h₄
  have hsm := WeierstrassCurve.torsionPairSpan_algebraMap_mul R K E Ksep m h
  refine ⟨?_, ?_, ?_, ?_⟩
  · exact sub_mem (add_mem (add_mem (add_mem (add_mem (add_mem
      (hq 1 0 1 0 0 0 0 0 (by omega) (by omega) (by omega) (by omega))
      (hq 1 0 0 0 1 0 0 0 (by omega) (by omega) (by omega) (by omega)))
      (hq 0 0 0 0 1 0 1 0 (by omega) (by omega) (by omega) (by omega)))
      (hsm _ (hq 1 0 0 0 0 0 0 0 (by omega) (by omega) (by omega) (by omega))))
      (hsm _ (hq 0 0 0 0 1 0 0 0 (by omega) (by omega) (by omega) (by omega))))
      (hsm _ (hq 0 0 0 0 0 0 0 0 (by omega) (by omega) (by omega) (by omega))))
      (hsm _ (hq 0 1 0 0 0 0 0 0 (by omega) (by omega) (by omega) (by omega)))
  · exact add_mem (add_mem (add_mem
      (hq 1 0 0 1 0 0 0 0 (by omega) (by omega) (by omega) (by omega))
      (hq 1 0 0 0 0 1 0 0 (by omega) (by omega) (by omega) (by omega)))
      (hsm _ (hq 1 0 0 0 1 0 0 0 (by omega) (by omega) (by omega) (by omega))))
      (hsm _ (hq 1 0 0 0 0 0 0 0 (by omega) (by omega) (by omega) (by omega)))
  · exact add_mem (add_mem (add_mem
      (hq 0 1 0 0 1 0 0 0 (by omega) (by omega) (by omega) (by omega))
      (hq 0 0 0 0 1 0 0 1 (by omega) (by omega) (by omega) (by omega)))
      (hsm _ (hq 0 0 0 0 1 0 1 0 (by omega) (by omega) (by omega) (by omega))))
      (hsm _ (hq 0 0 0 0 1 0 0 0 (by omega) (by omega) (by omega) (by omega)))
  · exact add_mem (add_mem (add_mem
      (hq 0 1 0 1 0 0 0 0 (by omega) (by omega) (by omega) (by omega))
      (hq 0 1 0 0 0 1 0 0 (by omega) (by omega) (by omega) (by omega)))
      (hsm _ (hq 0 1 0 0 1 0 0 0 (by omega) (by omega) (by omega) (by omega))))
      (hsm _ (hq 0 1 0 0 0 0 0 0 (by omega) (by omega) (by omega) (by omega)))

set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- `C₂` lies in the span `M`. -/
lemma WeierstrassCurve.torsionPairSecondDen_mem_span (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) :
    WeierstrassCurve.torsionPairSecondDen R K E Ksep m h ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  refine add_mem (add_mem (add_mem
    (WeierstrassCurve.torsionPairQuad_mem_span R K E Ksep m h hdeg 0 1 0 0 0 0 0 0
      (by omega) (by omega) (by omega) (by omega))
    (WeierstrassCurve.torsionPairQuad_mem_span R K E Ksep m h hdeg 0 0 0 0 0 1 0 0
      (by omega) (by omega) (by omega) (by omega)))
    (WeierstrassCurve.torsionPairSpan_algebraMap_mul R K E Ksep m h _
      (WeierstrassCurve.torsionPairQuad_mem_span R K E Ksep m h hdeg 0 0 0 0 1 0 0 0
        (by omega) (by omega) (by omega) (by omega))))
    (WeierstrassCurve.torsionPairSpan_algebraMap_mul R K E Ksep m h _
      (WeierstrassCurve.torsionPairQuad_mem_span R K E Ksep m h hdeg 0 0 0 0 0 0 0 0
        (by omega) (by omega) (by omega) (by omega)))

set_option linter.unusedSectionVars false in
/-- **The second-law abscissa numerator section** `A₂ = Ap/(h₁h₂)⁴`. -/
noncomputable def WeierstrassCurve.torsionPairSecondAbs (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  WeierstrassCurve.torsionPairSecondNum R K E Ksep m h PQ *
      WeierstrassCurve.torsionPairSecondNum R K E Ksep m h PQ +
...
set_option linter.unusedSectionVars false in
/-- **The second-law ordinate numerator section** `B₂ = Bp/(h₁h₂)⁶`. -/
noncomputable def WeierstrassCurve.torsionPairSecondOrd (m : ℕ) (h : Polynomial R) :
    (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep := fun PQ =>
  -(WeierstrassCurve.torsionPairSecondNum R K E Ksep m h PQ *
        WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h PQ) +
      WeierstrassCurve.torsionPairSecondNum R K E Ksep m h PQ *
        (WeierstrassCurve.torsionPairSecondDen R K E Ksep m h PQ *
          WeierstrassCurve.torsionPairSecondDenX R K E Ksep m h PQ) -
    WeierstrassCurve.torsionPairSecondDenY R K E Ksep m h PQ *
...
set_option maxHeartbeats 800000 in
set_option linter.unusedSectionVars false in
/-- The value of `A₂` on an affine pair. -/
lemma WeierstrassCurve.torsionPairSecondAbs_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h PQ =
      ((x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
          ...
          (E⁄Ksep).a₁ *
            ...
            (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) -
          ((E⁄Ksep).a₂ + x₁ + x₂) *
            (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 2) /
        (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 4 := by
  unfold WeierstrassCurve.torsionPairSecondAbs
  rw [WeierstrassCurve.torsionPairSecondNum_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairSecondDen_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairSecondDenX_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairSecondDenX'_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    ← baseChange_a₁_integralModel R K E Ksep,
    ← baseChange_a₂_integralModel R K E Ksep]
  ring
...
set_option maxHeartbeats 1600000 in
set_option linter.unusedSectionVars false in
/-- The value of `B₂` on an affine pair. -/
lemma WeierstrassCurve.torsionPairSecondOrd_apply_some (m : ℕ) (h : Polynomial R)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (x₁ y₁ x₂ y₂ : Ksep)
    (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
    ...
    (hP : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.some x₁ y₁ hns₁)
    (hQ : (PQ.2 : (E⁄Ksep).Point) = Affine.Point.some x₂ y₂ hns₂) :
    WeierstrassCurve.torsionPairSecondOrd R K E Ksep m h PQ =
      (-((x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
                (E⁄Ksep).a₁ * y₁) *
            (((x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
                    (E⁄Ksep).a₁ * y₁) ^ 2 +
                (E⁄Ksep).a₁ *
                  (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) +
                    (E⁄Ksep).a₄ - (E⁄Ksep).a₁ * y₁) *
                  (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) -
                ((E⁄Ksep).a₂ + x₁ + x₂) *
                  (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 2) -
              x₁ * (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 2)) -
          y₁ * (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 3 -
          (E⁄Ksep).a₁ *
            ((x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
                  (E⁄Ksep).a₁ * y₁) ^ 2 +
              (E⁄Ksep).a₁ *
              ...
              ((E⁄Ksep).a₂ + x₁ + x₂) *
                (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 2) *
            (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) -
          (E⁄Ksep).a₃ * (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 3) /
        (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ 6 := by
  unfold WeierstrassCurve.torsionPairSecondOrd
  rw [WeierstrassCurve.torsionPairSecondAbs_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairSecondNum_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairSecondDen_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairSecondDenX_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    WeierstrassCurve.torsionPairSecondDenY_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ,
    ← baseChange_a₁_integralModel R K E Ksep,
    ← baseChange_a₃_integralModel R K E Ksep]
  ring
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- `A₂` and `B₂` vanish on the origin rows and columns. -/
lemma WeierstrassCurve.torsionPairSecondAbsOrd_apply_zero (m : ℕ)
    (h : Polynomial R) (hdeg : 2 ≤ h.natDegree)
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (hz : (PQ.1 : (E⁄Ksep).Point) = Affine.Point.zero ∨
      (PQ.2 : (E⁄Ksep).Point) = Affine.Point.zero) :
    WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h PQ = 0 ∧
      WeierstrassCurve.torsionPairSecondOrd R K E Ksep m h PQ = 0 := by
  obtain ⟨hn, hv, hv', hw⟩ :=
    WeierstrassCurve.torsionPairSecond_apply_zero R K E Ksep m h hdeg PQ hz
  have hc := WeierstrassCurve.torsionPairSecondDen_apply_zero R K E Ksep m h hdeg
    PQ hz
  have hA : WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h PQ = 0 := by
    unfold WeierstrassCurve.torsionPairSecondAbs
    rw [hn, hc, hv, hv']
    ring
  refine ⟨hA, ?_⟩
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- `A₂` and `B₂` lie in the span `M`. -/
lemma WeierstrassCurve.torsionPairSecondAbsOrd_mem_span (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) :
    WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      WeierstrassCurve.torsionPairSecondOrd R K E Ksep m h ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
  obtain ⟨hn, hv, hv', hw⟩ :=
    WeierstrassCurve.torsionPairSecond_mem_span R K E Ksep m h hdeg
  have hc := WeierstrassCurve.torsionPairSecondDen_mem_span R K E Ksep m h hdeg
  have hmul : ∀ s t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep,
      s ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h →
      t ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h →
      s * t ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h :=
    fun _ _ hs ht => WeierstrassCurve.torsionPairSpan_mul R K E Ksep m h hs ht
  ...
  have hAeq : WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h =
      WeierstrassCurve.torsionPairSecondNum R K E Ksep m h *
          WeierstrassCurve.torsionPairSecondNum R K E Ksep m h +
        ...
        ((fun PQ => algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₂ *
            ...
            WeierstrassCurve.torsionPairSecondDenX R K E Ksep m h +
            WeierstrassCurve.torsionPairSecondDenX' R K E Ksep m h) *
          WeierstrassCurve.torsionPairSecondDen R K E Ksep m h := by
    funext PQ
    simp only [Pi.mul_apply, Pi.add_apply, Pi.sub_apply]
    rfl
  have hA : WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h ∈
      WeierstrassCurve.torsionPairSpan R K E Ksep m h := by
    rw [hAeq]
    exact sub_mem (add_mem (hmul _ _ hn hn) (hsm _ _ (hmul _ _ hn hc)))
      (hmul _ _ (add_mem (add_mem (hsm _ _ hc) hv) hv') hc)
  refine ⟨hA, ?_⟩
  have hBeq : WeierstrassCurve.torsionPairSecondOrd R K E Ksep m h =
      -(WeierstrassCurve.torsionPairSecondNum R K E Ksep m h *
            WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h) +
          WeierstrassCurve.torsionPairSecondNum R K E Ksep m h *
            (WeierstrassCurve.torsionPairSecondDen R K E Ksep m h *
        ...
        (fun PQ => algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₃ *
          (WeierstrassCurve.torsionPairSecondDen R K E Ksep m h *
            (WeierstrassCurve.torsionPairSecondDen R K E Ksep m h *
              WeierstrassCurve.torsionPairSecondDen R K E Ksep m h)) PQ) := by
    funext PQ
    simp only [Pi.mul_apply, Pi.add_apply, Pi.sub_apply, Pi.neg_apply]
    rfl
  rw [hBeq]
  exact sub_mem (sub_mem (sub_mem (add_mem (neg_mem (hmul _ _ hn hA))
    (hmul _ _ hn (hmul _ _ hc hv))) (hmul _ _ hw (hmul _ _ hc hc)))
    (hsm _ _ (hmul _ _ hA hc))) (hsm _ _ (hmul _ _ hc (hmul _ _ hc hc)))

set_option backward.isDefEq.respectTransparency false in
set_option maxHeartbeats 800000 in
set_option linter.unusedSectionVars false in
/-- **The second triple is an addition law**. -/
theorem WeierstrassCurve.torsionPairSecondLaw_spec (m : ℕ) (h : Polynomial R)
    (hdeg : 2 ≤ h.natDegree) :
    (∀ (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
      (x₃ y₃ : Ksep) (hns₃ : (E⁄Ksep).toAffine.Nonsingular x₃ y₃),
      ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) = Affine.Point.some x₃ y₃ hns₃ →
      WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h PQ =
          x₃ * WeierstrassCurve.torsionPairSecondDen R K E Ksep m h PQ ^ 2 ∧
        WeierstrassCurve.torsionPairSecondOrd R K E Ksep m h PQ =
          y₃ * WeierstrassCurve.torsionPairSecondDen R K E Ksep m h PQ ^ 3) ∧
    (∀ PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)),
      ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
        (E⁄Ksep).Point) = Affine.Point.zero →
  ...
  · intro PQ x₃ y₃ hns₃ hsum
    cases hc1 : ((PQ.1 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
        (E⁄Ksep).Point) with
    | zero =>
      obtain ⟨hA, hB⟩ := WeierstrassCurve.torsionPairSecondAbsOrd_apply_zero
        R K E Ksep m h hdeg PQ (Or.inl hc1)
      have hC := WeierstrassCurve.torsionPairSecondDen_apply_zero R K E Ksep m h
        hdeg PQ (Or.inl hc1)
      rw [hA, hB, hC]
      exact ⟨by ring, by ring⟩
    | some x₁ y₁ hns₁ =>
      ...
      | zero =>
        obtain ⟨hA, hB⟩ := WeierstrassCurve.torsionPairSecondAbsOrd_apply_zero
          R K E Ksep m h hdeg PQ (Or.inr hc2)
        have hC := WeierstrassCurve.torsionPairSecondDen_apply_zero R K E Ksep m h
          hdeg PQ (Or.inr hc2)
        rw [hA, hB, hC]
        exact ⟨by ring, by ring⟩
      | some x₂ y₂ hns₂ =>
        ...
        refine ⟨?_, ?_⟩
        · rw [WeierstrassCurve.torsionPairSecondAbs_apply_some R K E Ksep m h PQ
            x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2,
            WeierstrassCurve.torsionPairSecondDen_apply_some R K E Ksep m h PQ
              x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2, ← hX₃]
          ring
        · rw [WeierstrassCurve.torsionPairSecondOrd_apply_some R K E Ksep m h PQ
            x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2,
            WeierstrassCurve.torsionPairSecondDen_apply_some R K E Ksep m h PQ
              x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2, ← hX₃, ← hY₃]
          ring
  · intro PQ hsum
    cases hc1 : ((PQ.1 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
        (E⁄Ksep).Point) with
    | zero =>
      exact WeierstrassCurve.torsionPairSecondDen_apply_zero R K E Ksep m h hdeg
        PQ (Or.inl hc1)
    | some x₁ y₁ hns₁ =>
      cases hc2 : ((PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
          (E⁄Ksep).Point) with
      | zero =>
        exact WeierstrassCurve.torsionPairSecondDen_apply_zero R K E Ksep m h hdeg
          PQ (Or.inr hc2)
      | some x₂ y₂ hns₂ =>
        by_cases hxy : x₁ = x₂ ∧ y₁ = (E⁄Ksep).toAffine.negY x₂ y₂
        · have hnum : y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃ = 0 := by
            rw [hxy.2, WeierstrassCurve.Affine.negY]
            ring
          rw [WeierstrassCurve.torsionPairSecondDen_apply_some R K E Ksep m h PQ
            x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2, hnum, zero_div]
        · exfalso
          obtain ⟨x₃', y₃', hns₃', hadd, -, -⟩ :=
            WeierstrassCurve.add_some_second (E⁄Ksep) hns₁ hns₂ hxy
          rw [show ((PQ.1 + PQ.2 :
...
set_option linter.unusedSectionVars false in
/-- **The formal-group chart denominator** `C` (2026-07-25), the
section-algebra avatar of the formal parameter `s₃ = s₁ + s₂ + …`:

  `C := g_{d−2,1} ⊗ w + w ⊗ g_{d−2,1} + a₁·g_{d−1,0} ⊗ w + a₃·g_{d−2,0} ⊗ w`

for `w = g_{d,0} = x^d/h` and `g_{a,b} = x^a y^b/h`.  Its origin orders are
`(1,0)` and `(0,1)`, so — unlike the rescaled-chord denominator of
`exists_torsionPairLaw_formalRescaledChord`, which vanishes on the whole origin
COLUMN — it survives on both origin lines, which is exactly what the
formal-group branch needs.

The two `a₁`, `a₃` correction terms are NOT optional: the bare two-term
`g_{d−2,1} ⊗ w + w ⊗ g_{d−2,1}` fails the law's zero-sum clause (at `Q = ⊖P` its
value is `−x^{2d−2}(a₁x + a₃)/h²`, not `0`).  With them, the affine numerator
factors as `x₁^{d−2}x₂^{d−2}·(x₂²·D₂ + (x₁−x₂)·((x₁+x₂)y₂ + a₁x₂²))` for
`D₂ = y₁ + y₂ + a₁x₂ + a₃` the second-law denominator — a combination
`u·δ + v·D₂`, which both makes it vanish on the zero-sum locus
(`torsionPairFormalDen_apply_eq_zero`) and makes `x₃C²`, `y₃C³` polynomial. -/
...
lemma WeierstrassCurve.torsionPairFormalDen_apply_some (m : ℕ) (h : Polynomial R)
  ...
  simp only [show k + 2 - 1 = k + 1 from by omega, show k + 2 - 2 = k from by omega,
    pow_zero, mul_one, one_mul, pow_succ]
  field_simp

set_option linter.unusedSectionVars false in
set_option linter.unusedVariables false in
/-- The formal-group chart denominator on the origin ROW: the `Q`-section
`g_{d−2,1}`, which does NOT vanish there. -/
...
set_option linter.unusedSectionVars false in
set_option linter.unusedVariables false in
/-- The formal-group chart denominator on the origin COLUMN: the `P`-section
`g_{d−2,1} + a₁·g_{d−1,0} + a₃·g_{d−2,0}`, i.e. `−x₁^{d−2}·negY(x₁,y₁)/h₁`. -/
    ...
    WeierstrassCurve.torsionPairFormalDen R K E Ksep m h PQ =
      WeierstrassCurve.torsionKernelFun R K E Ksep h (h.natDegree - 2) 1 ↑PQ.1 +
        (E⁄Ksep).a₁ *
          WeierstrassCurve.torsionKernelFun R K E Ksep h (h.natDegree - 1) 0 ↑PQ.1 +
        (E⁄Ksep).a₃ *
          WeierstrassCurve.torsionKernelFun R K E Ksep h (h.natDegree - 2) 0 ↑PQ.1 := by
  unfold WeierstrassCurve.torsionPairFormalDen
  rw [hQ]
  simp only [WeierstrassCurve.torsionKernelFun_zero]
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- The formal-group chart denominator lies in the span `M`: all four of its
generator weights `2(d−2)+3 = 2d−1`, `2d`, `2d−2`, `2d−4` respect the bound
`2a + 3b ≤ 2d` with `b ≤ 1`. -/
lemma WeierstrassCurve.torsionPairFormalDen_mem_span (m : ℕ) (h : Polynomial R)
  ...
  have heq : WeierstrassCurve.torsionPairFormalDen R K E Ksep m h =
      ...
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        ...
        algebraMap R Ksep (WeierstrassCurve.integralModel R E).a₃ *
          (WeierstrassCurve.torsionKernelFun R K E Ksep h (h.natDegree - 2) 0 ↑PQ.1 *
            WeierstrassCurve.torsionKernelFun R K E Ksep h h.natDegree 0 ↑PQ.2)) := by
    funext PQ
    simp only [Pi.add_apply]
    rfl
  rw [heq]
  ...
  · exact hsm _ _ (Submodule.subset_span
...
set_option backward.isDefEq.respectTransparency false in
set_option linter.unusedSectionVars false in
/-- **The zero-sum clause for the formal-group chart denominator** (PROVEN
2026-07-25).  If `P + Q = 0` then either both points are the origin — and `C`
restricts on the origin row to `g_{d−2,1}`, which vanishes at the origin because
its weight `2d−1` misses `2d` — or both are affine with `x₁ = x₂` and
`y₁ = negY(x₂,y₂)`, i.e. `D₂ = 0`, and the affine numerator
`x₁^{d−2}x₂^{d−2}·x²·D₂` vanishes.  This is precisely the clause the bare
two-term denominator failed. -/
theorem WeierstrassCurve.torsionPairFormalDen_apply_eq_zero (m : ℕ)
  ...
  | zero =>
    ...
    rw [WeierstrassCurve.torsionPairFormalDen_apply_zero_left R K E Ksep m h hdeg
        PQ hc1, hQz, WeierstrassCurve.torsionKernelFun_zero, if_neg (by omega)]
  | some x₁ y₁ hns₁ =>
    cases hc2 : ((PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
        (E⁄Ksep).Point) with
    | zero =>
    ...
    | some x₂ y₂ hns₂ =>
      have h0 : (Affine.Point.some x₁ y₁ hns₁ : (E⁄Ksep).Point) +
        ...
        rw [← hc1, ← hc2, ← hsumeq]; exact hsum
      have hxy : x₁ = x₂ ∧ y₁ = (E⁄Ksep).toAffine.negY x₂ y₂ := by
        by_contra hc
        rw [Affine.Point.add_some hc] at h0
        exact Affine.Point.some_ne_zero _ h0
      obtain ⟨hx, hy⟩ := hxy
      have htor₁ : (m : ℤ) •
          (Affine.Point.some x₁ y₁ hns₁ : (E⁄Ksep).Point) = 0 := by
        rw [← hc1]; exact (Submodule.mem_torsionBy_iff _ _).mp PQ.1.2
      have htor₂ : (m : ℤ) •
          (Affine.Point.some x₂ y₂ hns₂ : (E⁄Ksep).Point) = 0 := by
        rw [← hc2]; exact (Submodule.mem_torsionBy_iff _ _).mp PQ.2.2
      have h₁0 : Polynomial.aeval x₁ h ≠ 0 :=
        WeierstrassCurve.torsionKernel_aeval_ne_zero R K E Ksep m h hmon hunit
          hns₁ htor₁
      have h₂0 : Polynomial.aeval x₂ h ≠ 0 :=
        WeierstrassCurve.torsionKernel_aeval_ne_zero R K E Ksep m h hmon hunit
          hns₂ htor₂
      rw [WeierstrassCurve.torsionPairFormalDen_apply_some R K E Ksep m h hdeg
        PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hc1 hc2 h₁0 h₂0]
      rw [WeierstrassCurve.Affine.negY] at hy
      subst hx
      rw [hy, div_eq_zero_iff]
      left
      ring
...
theorem WeierstrassCurve.exists_torsionPairFormalNumerators (m : ℕ)
    ...
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      ...
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
        (m : ℤ) • (Affine.Point.some x y hns : (E⁄Ksep).Point) = 0 →
        x ∈ 𝒪 → 𝒪.valuation (Polynomial.aeval x h) = 1) :
    ∃ A B : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep,
      A ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      B ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      (∀ (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        ...
        A PQ = x₃ * WeierstrassCurve.torsionPairFormalDen R K E Ksep m h PQ ^ 2 ∧
          B PQ =
            y₃ * WeierstrassCurve.torsionPairFormalDen R K E Ksep m h PQ ^ 3) ∧
      (∀ 𝒪 : ValuationSubring Ksep,
        (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
        ∀ PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)),
...
ASSEMBLED 2026-07-25 over `torsionPairFormalDen` and the two leaves
`torsionPairFormalDen_val_mul_eq_one` / `exists_torsionPairFormalNumerators`
— the residue of `exists_torsionPairLaw_formalMixed` and all of
...
theorem WeierstrassCurve.exists_torsionPairLaw_formalGroupChart
    ...
    ∃ A B C : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      ...
      (𝒪.valuation (A PQ) = 1 ∨ 𝒪.valuation (C PQ) = 1) := by
  classical
  obtain ⟨A, B, hA, hB, hlaw, hzeroval⟩ :=
    WeierstrassCurve.exists_torsionPairFormalNumerators R K E Ksep m hm h hmon hdeg
      hunit
  refine ⟨A, B, WeierstrassCurve.torsionPairFormalDen R K E Ksep m h, hA, hB,
    WeierstrassCurve.torsionPairFormalDen_mem_span R K E Ksep m h hdeg, hlaw,
    WeierstrassCurve.torsionPairFormalDen_apply_eq_zero R K E Ksep m h hmon hdeg
      hunit,
    Or.inl ?_⟩
  -- the unit is always `A`; which fact supplies it depends on whether the sum
  -- at `PQ` is affine (the law plus the formal-group valuation identity) or the
  -- origin (the numerators' own zero-sum clause)
  cases hs : ((PQ.1 + PQ.2 : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
      (E⁄Ksep).Point) with
  | zero => exact hzeroval 𝒪 hcen PQ hformal₁ hformal₂ hs
  | some x₃ y₃ hns₃ =>
    rw [(hlaw PQ x₃ y₃ hns₃ hs).1, map_mul, map_pow]
    exact WeierstrassCurve.torsionPairFormalDen_val_mul_eq_one R K E Ksep m h hmon
      hdeg hunit 𝒪 hcen PQ hformal₁ hformal₂ x₃ y₃ hns₃ hs
...
set_option maxHeartbeats 1600000 in
set_option linter.unusedVariables false in
/-- **The diagonal-collision chart leaf** (PROVEN 2026-07-25 — stages
E3b/E3c of the Katz–Mazur chart analysis): an addition law in the section
algebra whose
...
PROOF AS CARRIED OUT (2026-07-25).  The plan above is followed verbatim,
except that the HAZARD branch needed no new geometry: the nonsingularity
of the reduced curve is already packaged in the PROVEN
`val_tangent_numerator_eq_one`, which says exactly that `v(Ψ₂(P)) < 1`
forces the `X`-partial `3x₁² + 2a₂x₁ + a₄ − a₁y₁` to be a unit.  Since
`v(x₁ − x₂) < 1`, `N₂` differs from that partial by
`(2x₁ + x₂ + a₂)(x₂ − x₁) ∈ 𝔪`, so `v(N₂) = 1`; and `v(D₂) < 1` because
`D₂ − Ψ₂(P) = (y₂ − y₁) + a₁(x₂ − x₁) ∈ 𝔪`.  The two remaining terms of
`Ap = N₂² + a₁N₂D₂ − (a₂+x₁+x₂)D₂²` are then in `𝔪`, so `v(A₂) = v(Ap) = 1`.
On the TANGENT branch `v(Ψ₂Sq(x₁)) = 1` gives `v(Ψ₂(P)) = 1` and hence
`v(C₂) = v(D₂) = 1` by the same congruence.  The three sections and their
law identities are `torsionPairSecondAbs`, `torsionPairSecondOrd`,
`torsionPairSecondDen` and `torsionPairSecondLaw_spec`; `hm` and `hmon`
are unused (kept for signature stability with the sibling leaves). -/
theorem WeierstrassCurve.exists_torsionPairLaw_collisionDiagonal
  ...
  classical
  obtain ⟨hlaw, hzero⟩ :=
    WeierstrassCurve.torsionPairSecondLaw_spec R K E Ksep m h hdeg
  obtain ⟨hAmem, hBmem⟩ :=
    WeierstrassCurve.torsionPairSecondAbsOrd_mem_span R K E Ksep m h hdeg
  have hCmem := WeierstrassCurve.torsionPairSecondDen_mem_span R K E Ksep m h hdeg
  refine ⟨WeierstrassCurve.torsionPairSecondAbs R K E Ksep m h,
    WeierstrassCurve.torsionPairSecondOrd R K E Ksep m h,
    WeierstrassCurve.torsionPairSecondDen R K E Ksep m h,
    hAmem, hBmem, hCmem, hlaw, hzero, ?_⟩
  -- integrality of the local data
  obtain ⟨ha₁, ha₂, ha₃, ha₄, ha₆⟩ :=
    baseChange_coeffs_mem_centered R K E Ksep 𝒪 hcen
  have hy₁ : y₁ ∈ 𝒪 := WeierstrassCurve.ordinate_mem_of_abscissa_mem 𝒪 (E⁄Ksep)
    ha₁ ha₂ ha₃ ha₄ ha₆ hns₁.1 hx₁
  ...
  have hu₂ : 𝒪.valuation (Polynomial.aeval x₂ h) = 1 :=
    hunit 𝒪 hcen x₂ y₂ hns₂ htor₂ hx₂
  have hval : ∀ (z : Ksep) (k : ℕ),
      𝒪.valuation
          (z / (Polynomial.aeval x₁ h * Polynomial.aeval x₂ h) ^ k) =
        𝒪.valuation z := by
    intro z k
    rw [map_div₀, map_pow, map_mul, hu₁, hu₂, one_mul, one_pow, div_one]
  have h2 : (2 : Ksep) ∈ 𝒪 := by
    rw [show (2 : Ksep) = 1 + 1 from by norm_num]
    exact add_mem (one_mem _) (one_mem _)
  -- the `ψ₂`-value at `P`
  have hpsiSq : ((E⁄Ksep).Ψ₂Sq).eval x₁ =
      (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) ^ 2 := by
    ...
    linear_combination (-4 : Ksep) * heq
  have hT2mem : (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) ∈ 𝒪 :=
    add_mem (add_mem (mul_mem h2 hy₁) (mul_mem ha₁ hx₁)) ha₃
  have hT2le : 𝒪.valuation (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) ≤ 1 :=
    (𝒪.valuation_le_one_iff _).mpr hT2mem
  -- the denominator section, rewritten
  have hDenv : 𝒪.valuation
      (WeierstrassCurve.torsionPairSecondDen R K E Ksep m h PQ) =
      𝒪.valuation (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) := by
    rw [WeierstrassCurve.torsionPairSecondDen_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ, hval]
  -- the tail separating `D₂` from `ψ₂(P)`
  have htail : 𝒪.valuation
      ((y₂ - y₁) + (E⁄Ksep).a₁ * (x₂ - x₁)) < 1 := by
    refine lt_of_le_of_lt (𝒪.valuation.map_add _ _) (max_lt ?_ ?_)
    · rw [show y₂ - y₁ = -(y₁ - y₂) from by ring, Valuation.map_neg]
      exact hdiag
    · refine ValuationSubring.val_mul_lt_one_of_mem_of_lt 𝒪 ha₁ ?_
      rw [show x₂ - x₁ = -(x₁ - x₂) from by ring, Valuation.map_neg]
      exact hcol
  have hDsplit : y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃ =
      (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) +
        ((y₂ - y₁) + (E⁄Ksep).a₁ * (x₂ - x₁)) := by ring
  rcases lt_or_eq_of_le hpsi with hlt | heq1
  · -- HAZARD branch: the reduced point is `2`-torsion; the ABSCISSA numerator is the unit
    left
    have hT2lt : 𝒪.valuation (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) < 1 := by
      by_contra hcon
      rw [hpsiSq, map_pow, le_antisymm hT2le (not_lt.mp hcon), one_pow] at hlt
      exact lt_irrefl 1 hlt
    have hDlt : 𝒪.valuation (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) < 1 := by
      rw [hDsplit]
      exact lt_of_le_of_lt (𝒪.valuation.map_add _ _) (max_lt hT2lt htail)
    have hTan : 𝒪.valuation
        (3 * x₁ ^ 2 + 2 * (E⁄Ksep).a₂ * x₁ + (E⁄Ksep).a₄ - (E⁄Ksep).a₁ * y₁)
        = 1 :=
      WeierstrassCurve.val_tangent_numerator_eq_one R K E Ksep 𝒪 hcen hx₁ hy₁
        hns₁.1 hT2lt
    have hNv : 𝒪.valuation
        (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
          (E⁄Ksep).a₁ * y₁) = 1 := by
      have hsplit : x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) +
            (E⁄Ksep).a₄ - (E⁄Ksep).a₁ * y₁ =
          (3 * x₁ ^ 2 + 2 * (E⁄Ksep).a₂ * x₁ + (E⁄Ksep).a₄ - (E⁄Ksep).a₁ * y₁) +
            (2 * x₁ + x₂ + (E⁄Ksep).a₂) * (x₂ - x₁) := by ring
      have htl : 𝒪.valuation ((2 * x₁ + x₂ + (E⁄Ksep).a₂) * (x₂ - x₁)) <
          𝒪.valuation
            (3 * x₁ ^ 2 + 2 * (E⁄Ksep).a₂ * x₁ + (E⁄Ksep).a₄ -
              (E⁄Ksep).a₁ * y₁) := by
        rw [hTan]
        refine ValuationSubring.val_mul_lt_one_of_mem_of_lt 𝒪
          (add_mem (add_mem (mul_mem h2 hx₁) hx₂) ha₂) ?_
        rw [show x₂ - x₁ = -(x₁ - x₂) from by ring, Valuation.map_neg]
        exact hcol
      rw [hsplit, 𝒪.valuation.map_add_eq_of_lt_left htl, hTan]
    rw [WeierstrassCurve.torsionPairSecondAbs_apply_some R K E Ksep m h PQ
      x₁ y₁ x₂ y₂ hns₁ hns₂ hP hQ, hval]
    have hrest : 𝒪.valuation
        ((E⁄Ksep).a₁ *
            (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
              (E⁄Ksep).a₁ * y₁) *
            (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) -
        ...
        𝒪.valuation
          ((x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
            (E⁄Ksep).a₁ * y₁) ^ 2) := by
      rw [map_pow, hNv, one_pow, sub_eq_add_neg]
      refine lt_of_le_of_lt (𝒪.valuation.map_add _ _) (max_lt ?_ ?_)
      · have hNvmem : (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) +
            (E⁄Ksep).a₄ - (E⁄Ksep).a₁ * y₁) ∈ 𝒪 :=
          sub_mem (add_mem (add_mem (add_mem (add_mem (pow_mem hx₁ 2)
            (mul_mem hx₁ hx₂)) (pow_mem hx₂ 2))
            (mul_mem ha₂ (add_mem hx₁ hx₂))) ha₄) (mul_mem ha₁ hy₁)
        exact ValuationSubring.val_mul_lt_one_of_mem_of_lt 𝒪
          (mul_mem ha₁ hNvmem) hDlt
      · rw [Valuation.map_neg,
          show ((E⁄Ksep).a₂ + x₁ + x₂) *
              (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 2 =
            ((E⁄Ksep).a₂ + x₁ + x₂) *
              (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) *
              (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) from by ring]
        have hmem2 : ((E⁄Ksep).a₂ + x₁ + x₂) *
            (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ∈ 𝒪 :=
          mul_mem (add_mem (add_mem ha₂ hx₁) hx₂)
            (add_mem (add_mem (add_mem hy₁ hy₂) (mul_mem ha₁ hx₂)) ha₃)
        exact ValuationSubring.val_mul_lt_one_of_mem_of_lt 𝒪 hmem2 hDlt
    rw [show (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
            (E⁄Ksep).a₁ * y₁) ^ 2 +
          (E⁄Ksep).a₁ *
            (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
              (E⁄Ksep).a₁ * y₁) *
        ...
        (x₁ ^ 2 + x₁ * x₂ + x₂ ^ 2 + (E⁄Ksep).a₂ * (x₁ + x₂) + (E⁄Ksep).a₄ -
          ...
          ((E⁄Ksep).a₁ *
              ...
              (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) -
            ((E⁄Ksep).a₂ + x₁ + x₂) *
              (y₁ + y₂ + (E⁄Ksep).a₁ * x₂ + (E⁄Ksep).a₃) ^ 2) from by ring,
      𝒪.valuation.map_add_eq_of_lt_left hrest, map_pow, hNv, one_pow]
  · -- TANGENT branch: the DENOMINATOR section is the unit
    right
    have hT2 : 𝒪.valuation (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) = 1 := by
      have hsq : 𝒪.valuation (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) *
          𝒪.valuation (2 * y₁ + (E⁄Ksep).a₁ * x₁ + (E⁄Ksep).a₃) = 1 := by
        rw [← map_mul, ← sq, ← hpsiSq]
        exact heq1
      exact (val_factors_eq_one_of_mul_eq_one hT2le hT2le hsq).1
    rw [hDenv, hDsplit,
      𝒪.valuation.map_add_eq_of_lt_left (by rw [hT2]; exact htail), hT2]
...
* `A = x(P'+Q')·C²` and `B = y(P'+Q')·C³` at every pair `P', Q'` whose sum
  is affine,
* `C = 0` at every pair whose sum is the origin, and
* `A` or `C` a valuation UNIT at the given pair `PQ`.

By the generic brick above (`torsionPairLawKiller`, its `_mem_span`,
`_mul_add_mem` and `_valuation_eq_one`) such a triple yields the pointwise
branch unit `exists_torsionKernel_branch_unit_offChord` with no further
geometry.  The complementary locus — both points integral-affine with
separated abscissas — is PROVEN through the CHORD law
`(n̂, m̂, δ)` = (`torsionPairChordNum`, `torsionPairChordOrd`,
`torsionPairDelta`), whose killer is `torsionPairChordKiller`.

"Off the chord locus" is the hypothesis `hloc`, a disjunction of the two
residual loci:

* `¬IsIntegralAffine 𝒪 P` — the FIRST point reduces to the origin of the
  special fibre (origin or non-integral abscissa), the second arbitrary.
  The mirror locus (`P` integral-affine, `Q` on the formal locus) is FREE:
  it follows from this one by the factor swap (`torsionPairSpan_swap`) in
  `exists_torsionKernel_branch_unit` below.
* the COLLISION locus — both points integral-affine with
...
theorem WeierstrassCurve.exists_torsionPairLaw_offChord
    ...
    (PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)))
    (hloc : ¬WeierstrassCurve.IsIntegralAffine K E Ksep 𝒪 ↑PQ.1 ∨
      ∃ (x₁ y₁ x₂ y₂ : Ksep)
        (hns₁ : (E⁄Ksep).toAffine.Nonsingular x₁ y₁)
        (hns₂ : (E⁄Ksep).toAffine.Nonsingular x₂ y₂),
        (PQ.1 : (E⁄Ksep).Point) = Affine.Point.some x₁ y₁ hns₁ ∧
        (PQ.2 : (E⁄Ksep).Point) = Affine.Point.some x₂ y₂ hns₂ ∧
        x₁ ∈ 𝒪 ∧ x₂ ∈ 𝒪 ∧ 𝒪.valuation (x₁ - x₂) < 1 ∧
        𝒪.valuation (((E⁄Ksep).Ψ₂Sq).eval x₁) ≤ 1) :
    ∃ A B C : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep,
      A ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      B ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
...
over the chart leaf `exists_torsionPairLaw_offChord` and the generic
addition-law killer brick — stage E3' of the addition-closure cut
`torsionKernelFun_add_mem_span`): at ONE valuation
subring `𝒪` of `Kˢᵉᵖ` centered on `R` and ONE pair `PQ` of `m`-torsion
points lying OFF the chord locus there is a *single* two-variable section
`κ` which

* lies in the product-section span `M` (`torsionPairSpan`),
* kills all the addition-law denominators, `κ·(g_{a,b} ∘ add) ∈ M` for
  every admissible weight `(a, b)`, and
* is a `𝒪`-valuation unit at `PQ`.
...
The complementary locus — both points integral-affine with separated
abscissas `v(x(P) − x(Q)) = 1` — is already PROVEN: there the constructed
chord killer `torsionPairChordKiller` (the section-algebra avatar of
`h(x(P+Q))·(x(P)−x(Q))^{2d}`) is a member of `M`
(`torsionPairChordKiller_mem_span`), kills the denominators
(`torsionPairChordKiller_mul_add_mem`) and is a unit
(`torsionPairChordKiller_valuation_eq_one`).

The FINITE FAMILY demanded by `exists_torsionKernel_branch_family` is also
free: the denominator-killing sections form the CONDUCTOR ideal
`S = {f ∈ M | ∀ admissible (a, b), f·(g_{a,b} ∘ add) ∈ M}` of `M`, `M` is
a finitely generated `R`-algebra (`adjoin_prod_sections_eq_adjoin_union`)
over the Noetherian ring `R`, so `S` has finitely many generators, and the
ultrametric inequality transports a unit value from an arbitrary conductor
element to one of them (`exists_finite_family_of_conductor_units`).  So
this leaf carries the entire remaining geometric content and none of the
bookkeeping — and no sub-leaf can be false through a bad family choice.

PROOF (2026-07-25): all of the killer bookkeeping is now generic.  An
addition law adapted to the chart at `PQ` — a triple of sections
`(A, B, C)` in `M` with `A = x(P'+Q')·C²`, `B = y(P'+Q')·C³` on affine
sums, `C = 0` on zero sums, and `A` or `C` a unit at `PQ` — is produced by
the chart leaf `exists_torsionPairLaw_offChord` (whose docstring carries
the per-locus chart analysis: rescaled chord law on the formal locus,
rationalized-slope second law on the collision locus, its `X`-partial unit
in the 2-torsion hazard).  Its killer `torsionPairLawKiller` then lies in
`M` (`torsionPairLawKiller_mem_span`), kills every denominator
(`torsionPairLawKiller_mul_add_mem`) and is a unit at `PQ`
(`torsionPairLawKiller_valuation_eq_one`) — no geometry beyond the law. -/
theorem WeierstrassCurve.exists_torsionKernel_branch_unit_offChord
    (m : ℕ) (hm : (m : K) ≠ 0) (h : Polynomial R) (hmon : h.Monic)
    (hdeg : 2 ≤ h.natDegree)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
    ...
    (hloc : ¬WeierstrassCurve.IsIntegralAffine K E Ksep 𝒪 ↑PQ.1 ∨
      ∃ (x₁ y₁ x₂ y₂ : Ksep)
        ...
        x₁ ∈ 𝒪 ∧ x₂ ∈ 𝒪 ∧ 𝒪.valuation (x₁ - x₂) < 1 ∧
        𝒪.valuation (((E⁄Ksep).Ψ₂Sq).eval x₁) ≤ 1) :
    ∃ κ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep,
      κ ∈ WeierstrassCurve.torsionPairSpan R K E Ksep m h ∧
      (∀ a b : ℕ, b ≤ 1 → 2 * a + 3 * b ≤ 2 * h.natDegree →
        κ * (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
          WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑(PQ.1 + PQ.2)) ∈
        WeierstrassCurve.torsionPairSpan R K E Ksep m h) ∧
      𝒪.valuation (κ PQ) = 1 := by
  classical
  obtain ⟨A, B, C, hA, hB, hC, hlaw, hzero, hu⟩ :=
    WeierstrassCurve.exists_torsionPairLaw_offChord R K E Ksep m hm h hmon hdeg
      hunit 𝒪 hcen PQ hloc
  refine ⟨WeierstrassCurve.torsionPairLawKiller R K E Ksep m h A C,
    WeierstrassCurve.torsionPairLawKiller_mem_span R K E Ksep m h hA hC,
    fun a b hb hw => WeierstrassCurve.torsionPairLawKiller_mul_add_mem
      R K E Ksep m h hmon hunit hA hB hC hlaw hzero a b hb hw,
    WeierstrassCurve.torsionPairLawKiller_valuation_eq_one R K E Ksep m h hmon
      hunit 𝒪 hcen hA hC PQ (fun x₃ y₃ hns₃ hs => (hlaw PQ x₃ y₃ hns₃ hs).1)
      (hzero PQ) hu⟩

set_option linter.unusedSectionVars false in
/-- The `Ψ₂`-square polynomial of the base change evaluates integrally at
integral abscissas over any centered valuation subring: its coefficients
are (numerals and) the `b`-invariants of the good-reduction model. -/
lemma psi2Sq_eval_mem_centered (𝒪 : ValuationSubring Ksep)
    (hc : (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range)
    {x : Ksep} (hx : x ∈ 𝒪) : ((E⁄Ksep).Ψ₂Sq).eval x ∈ 𝒪 := by
  obtain ⟨ha₁, ha₂, ha₃, ha₄, ha₆⟩ :=
    baseChange_coeffs_mem_centered R K E Ksep 𝒪 hc
  have h2 : (2 : Ksep) ∈ 𝒪 := by
    rw [show (2 : Ksep) = 1 + 1 from by norm_num]
    exact add_mem (one_mem _) (one_mem _)
  have h4 : (4 : Ksep) ∈ 𝒪 := by
    rw [show (4 : Ksep) = 2 + 2 from by norm_num]
    exact add_mem h2 h2
  have hb₂ : (E⁄Ksep).b₂ ∈ 𝒪 := by
    rw [WeierstrassCurve.b₂]
    exact add_mem (pow_mem ha₁ 2) (mul_mem h4 ha₂)
  have hb₄ : (E⁄Ksep).b₄ ∈ 𝒪 := by
    rw [WeierstrassCurve.b₄]
    exact add_mem (mul_mem h2 ha₄) (mul_mem ha₁ ha₃)
  have hb₆ : (E⁄Ksep).b₆ ∈ 𝒪 := by
    rw [WeierstrassCurve.b₆]
    exact add_mem (pow_mem ha₃ 2) (mul_mem h4 ha₆)
  ...
  simp only [Polynomial.eval_add, Polynomial.eval_mul, Polynomial.eval_pow,
    Polynomial.eval_C, Polynomial.eval_X]
  exact add_mem (add_mem (add_mem (mul_mem h4 (pow_mem hx 3))
    (mul_mem hb₂ (pow_mem hx 2))) (mul_mem (mul_mem h2 hb₄) hx)) hb₆

set_option backward.isDefEq.respectTransparency false in
/-- **The pointwise Katz–Mazur branch unit** (PROVEN 2026-07-24 over the
single residual leaf `exists_torsionKernel_branch_unit_offChord`): at EVERY
centered valuation subring `𝒪` and EVERY pair `PQ` of `m`-torsion points
there is one section of the product-section span `M` which kills all the
addition-law denominators and is a `𝒪`-valuation unit at `PQ`.

The configuration space is covered by three cases:

* both points integral-affine with SEPARATED abscissas
  (`v(x₁ − x₂) = 1`): the constructed chord killer
  `torsionPairChordKiller`, proven end-to-end
  (`torsionPairChordKiller_mem_span`,
  `torsionPairChordKiller_mul_add_mem`,
  `torsionPairChordKiller_valuation_eq_one`);
* both points integral-affine with COLLIDING abscissas: the residual leaf
  through its right disjunct (with the recorded Ψ₂Sq-integrality from
  `psi2Sq_eval_mem_centered`);
* the FIRST point on the formal-group locus: the residual leaf through its
  left disjunct — and the mirror case (`P` integral-affine, `Q` formal) is
  obtained from it by the factor SWAP, `torsionPairSpan_swap` plus the
  commutativity `Q + P = P + Q` of the group law on the denominators. -/
theorem WeierstrassCurve.exists_torsionKernel_branch_unit
    (m : ℕ) (hm : (m : K) ≠ 0) (h : Polynomial R) (hmon : h.Monic)
    (hdeg : 2 ≤ h.natDegree)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
    ...
    ∃ κ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      ...
      𝒪.valuation (κ PQ) = 1 := by
  classical
  by_cases hP : WeierstrassCurve.IsIntegralAffine K E Ksep 𝒪 ↑PQ.1
  · by_cases hQ : WeierstrassCurve.IsIntegralAffine K E Ksep 𝒪 ↑PQ.2
    · -- both integral-affine: split by the collision pattern of the abscissas
      obtain ⟨x₁, y₁, hns₁, hPeq, hx₁⟩ := hP
      obtain ⟨x₂, y₂, hns₂, hQeq, hx₂⟩ := hQ
      have hsub : 𝒪.valuation (x₁ - x₂) ≤ 1 :=
        (𝒪.valuation_le_one_iff _).mpr (sub_mem hx₁ hx₂)
      rcases lt_or_eq_of_le hsub with hlt | heq
      · -- collision: the residual leaf
        exact WeierstrassCurve.exists_torsionKernel_branch_unit_offChord
          R K E Ksep m hm h hmon hdeg hunit 𝒪 hcen PQ
          (Or.inr ⟨x₁, y₁, x₂, y₂, hns₁, hns₂, hPeq, hQeq, hx₁, hx₂, hlt,
            (𝒪.valuation_le_one_iff _).mpr
              (psi2Sq_eval_mem_centered R K E Ksep 𝒪 hcen hx₁)⟩)
      · -- separated abscissas: the constructed chord killer, proven end-to-end
        exact ⟨WeierstrassCurve.torsionPairChordKiller R K E Ksep m h,
          WeierstrassCurve.torsionPairChordKiller_mem_span R K E Ksep m h hdeg,
          fun a b hb hw => WeierstrassCurve.torsionPairChordKiller_mul_add_mem
            R K E Ksep m h hmon hdeg hunit a b hb hw,
          WeierstrassCurve.torsionPairChordKiller_valuation_eq_one R K E Ksep m h
            hunit 𝒪 hcen PQ x₁ y₁ x₂ y₂ hns₁ hns₂ hPeq hQeq hx₁ hx₂ heq⟩
    · -- (integral-affine, formal): the swap of the leaf at the swapped pair
      obtain ⟨κ, hκM, hκK, hκv⟩ :=
        WeierstrassCurve.exists_torsionKernel_branch_unit_offChord
          R K E Ksep m hm h hmon hdeg hunit 𝒪 hcen (PQ.2, PQ.1) (Or.inl hQ)
      refine ⟨fun QP => κ (QP.2, QP.1),
        WeierstrassCurve.torsionPairSpan_swap R K E Ksep m h hκM, ?_, hκv⟩
      intro a b hb hw
      have h1 := WeierstrassCurve.torsionPairSpan_swap R K E Ksep m h
        (hκK a b hb hw)
      have h2 : (fun PQ' : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
            (κ * (fun QP : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
                (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
              WeierstrassCurve.torsionKernelFun R K E Ksep h a b
                ↑(QP.1 + QP.2))) (PQ'.2, PQ'.1)) =
          (fun QP : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) => κ (QP.2, QP.1)) *
            (fun PQ' : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
              (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
            WeierstrassCurve.torsionKernelFun R K E Ksep h a b
              ↑(PQ'.1 + PQ'.2)) := by
        funext PQ'
        simp only [Pi.mul_apply]
        rw [add_comm PQ'.2 PQ'.1]
      rwa [h2] at h1
  · -- the first point on the formal-group locus: the residual leaf
    exact WeierstrassCurve.exists_torsionKernel_branch_unit_offChord
      R K E Ksep m hm h hmon hdeg hunit 𝒪 hcen PQ (Or.inl hP)

set_option backward.isDefEq.respectTransparency false in
/-- **The Katz–Mazur branch family** (PROVEN 2026-07-24 over the pointwise
unit `exists_torsionKernel_branch_unit` — the finiteness bookkeeping is the
Noetherian conductor-ideal argument
`exists_finite_family_of_conductor_units`, so the geometric content is
entirely pointwise): a finite family of
two-variable sections `f i` on the `m`-torsion square which
(i) lie in the `R`-span `M` of the split products `a'(P)·b'(Q)` over the
subalgebra generated by the monomial sections, (ii) kill the addition-law
denominators — `f i · (g_{a,b} ∘ add) ∈ M` for every admissible weight
`(a, b)` — and (iii) cover the special fibre: at every pair of torsion
points and every valuation subring of `Kˢᵉᵖ` centered on `R`, some `f i`
is a valuation unit.

PROOF (the finiteness bookkeeping; the geometry is pointwise). The
denominator-killing sections form the CONDUCTOR ideal
`S := {f ∈ M | ∀ admissible (a, b), f·(g_{a,b} ∘ add) ∈ M}` of the
subalgebra `M` (an ideal because `M` is a ring — stage E0 — and the
condition is stable under `M`-multiples and sums).  `M` is generated as an
`R`-algebra by the finitely many pulled-back monomial sections
(`adjoin_prod_sections_eq_adjoin_union`), and `R` is a DVR, hence
Noetherian; so `M` is Noetherian and `S` is generated by finitely many
`f₁, …, f_k ∈ S` — THIS is the family, no geometric choice involved.  Its
covering property comes from the pointwise unit plus the ultrametric
inequality: an arbitrary conductor element with a unit value at `PQ` is an
`M`-combination `∑ cᵢ·fᵢ`, all of whose values are `R`-integral (stage E1)
hence lie in every centered valuation subring; a sum of elements of
valuation `< 1` has valuation `< 1`, so some `cᵢ(PQ)·fᵢ(PQ)` — and
therefore some `fᵢ(PQ)` — is a unit.  Both steps are the proven
`exists_finite_family_of_conductor_units`.  Consequence: no sub-leaf can be
false through a bad family choice.

The geometric content — one denominator-killing section that is a unit at a
given `(𝒪, PQ)` — is `exists_torsionKernel_branch_unit`, proven from the
constructed chord killer together with the single residual leaf
`exists_torsionKernel_branch_unit_offChord`, whose docstring records the
Katz–Mazur construction, the formal/collision/hazard case analysis (with
the symmetric-function identity
`x(P+Q) − x(P−Q) = −Ψ₂(P)·Ψ₂(Q)/(x(P)−x(Q))²` closing the hazard case) and
the sub-decomposition E3a–E3d. -/
theorem WeierstrassCurve.exists_torsionKernel_branch_family
    ...
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      ...
      ∀ (x y : Ksep) (hns : (E⁄Ksep).toAffine.Nonsingular x y),
        (m : ℤ) • (Affine.Point.some x y hns : (E⁄Ksep).Point) = 0 →
        x ∈ 𝒪 → 𝒪.valuation (Polynomial.aeval x h) = 1) :
    ∃ (k : ℕ) (f : Fin k → ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
        (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep)),
      (∀ i, f i ∈ Submodule.span R
        {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep |
          ∃ a' ∈ Algebra.adjoin R
            ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
          ...
          ∃ b' ∈ Algebra.adjoin R
            ((WeierstrassCurve.torsionKernelGens R K E Ksep m h : Set
              ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep))),
            (fun PQ => a' PQ.1 * b' PQ.2) = t}) ∧
      (∀ i (a b : ℕ), b ≤ 1 → 2 * a + 3 * b ≤ 2 * h.natDegree →
        f i * (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
          WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑(PQ.1 + PQ.2)) ∈
        Submodule.span R
          {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
      ...
      (∀ 𝒪 : ValuationSubring Ksep,
        ...
        ∀ PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
            (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)),
          ∃ i, 𝒪.valuation (f i PQ) = 1) := by
  classical
  -- E1: every value of the split-product algebra is integral over `R`, hence
  -- lies in every valuation subring of `Kˢᵉᵖ` centered on `R`
  have hAint := WeierstrassCurve.adjoin_torsionKernelGens_isIntegral_values
    R K E Ksep m h hmon hunit
  have hBint := adjoin_prod_sections_isIntegral_values hAint hAint
  -- the conductor of the addition-law denominators is a finitely generated
  -- ideal, and the pointwise unit feeds its covering property
  obtain ⟨k, f, hfmem, hfkill, hfcover⟩ :=
    exists_finite_family_of_conductor_units (R₀ := R)
      (WeierstrassCurve.torsionKernelGens R K E Ksep m h)
      (WeierstrassCurve.torsionKernelGens R K E Ksep m h)
      (ι := {ab : ℕ × ℕ // ab.2 ≤ 1 ∧ 2 * ab.1 + 3 * ab.2 ≤ 2 * h.natDegree})
      (fun ab PQ => WeierstrassCurve.torsionKernelFun R K E Ksep h ab.1.1 ab.1.2
        ↑(PQ.1 + PQ.2))
      {𝒪 : ValuationSubring Ksep |
        (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range}
      (fun g hg pq 𝒪 h𝒪 => mem_centered_of_isIntegral R K Ksep 𝒪 h𝒪 (hBint g hg pq))
      (fun 𝒪 h𝒪 pq => by
        obtain ⟨u, huM, huK, huv⟩ :=
          WeierstrassCurve.exists_torsionKernel_branch_unit R K E Ksep m hm h hmon
            hdeg hunit 𝒪 h𝒪 pq
        unfold WeierstrassCurve.torsionPairSpan at huM huK
        rw [mem_span_prod_sections_iff_mem_adjoin] at huM
        refine ⟨u, huM, fun j => ?_, huv⟩
        have hj := huK j.1.1 j.1.2 j.2.1 j.2.2
        rw [mem_span_prod_sections_iff_mem_adjoin] at hj
        exact hj)
  exact ⟨k, f, fun i => (mem_span_prod_sections_iff_mem_adjoin _ _ _).mpr (hfmem i),
    fun i a b hb hw => (mem_span_prod_sections_iff_mem_adjoin _ _ _).mpr
      (hfkill i ⟨(a, b), hb, hw⟩), fun 𝒪 hc PQ => hfcover 𝒪 hc PQ⟩

set_option backward.isDefEq.respectTransparency false in
/-- **Addition-law closure of the monomial sections** (ASSEMBLED 2026-07-24
as the partition-of-unity identity `F = ∑ cᵢ·(fᵢ·F)` over the E-leaves;
the geometric content now lives entirely in the branch-family leaf
`exists_torsionKernel_branch_family`): the pullback of a monomial section
along the group addition lies in the `R`-span `M` of the product
two-variable functions of the `R`-subalgebra generated by the family.
Proof structure (Katz–Mazur 2.3.1 made pointwise):

* E0 `span_prod_sections_eq_adjoin_toSubmodule` (PROVEN): the split
  products form a multiplicative submonoid, so `M` is the underlying
  module of the subalgebra they generate — closed under multiplication.
* E1 `adjoin_torsionKernelGens_isIntegral_values` +
  `adjoin_prod_sections_isIntegral_values` (PROVEN): every value of every
  member of `M` is integral over `R`.
* E2 `exists_partition_of_unity_of_center_units` (PROVEN over the
  Chevalley leaf `exists_valuationSubring_center_of_isMaximal`): members
  of `M` covering every special fibre by valuation units generate the
  unit ideal of `M`: `∑ cᵢ·fᵢ = 1` with `cᵢ ∈ M`.
* E3 `exists_torsionKernel_branch_family` (PROVEN 2026-07-24 over the
  pointwise unit, via the Noetherian conductor-ideal assembly
  `exists_finite_family_of_conductor_units`): the branch family `fᵢ ∈ M`
  with the denominator-killing property `fᵢ·(g_{a,b} ∘ add) ∈ M` and the
  fibrewise covering property — the family is the finite generating set of
  the conductor ideal, so no geometric choice enters.
* E3' `exists_torsionKernel_branch_unit` (PROVEN over the single residual
  leaf `exists_torsionKernel_branch_unit_offChord`, using the constructed
  chord killer `torsionPairChordKiller` on the separated locus and the
  factor swap for the mirror mixed locus): at ONE centered valuation
  subring and ONE pair of torsion points, ONE denominator-killing section
  of `M` that is a valuation unit there.  Its off-chord residue — the
  formal-group locus and the collision (tangent/hazard) locus — is the
  remaining Katz–Mazur core.
* E4 (PROVEN, here): `F = 1·F = (∑ cᵢ·fᵢ)·F = ∑ cᵢ·(fᵢ·F)` with every
  summand in the subalgebra `M`. -/
theorem WeierstrassCurve.torsionKernelFun_add_mem_span
    (m : ℕ) (hm : (m : K) ≠ 0) (h : Polynomial R) (hmon : h.Monic)
    (hdeg : 2 ≤ h.natDegree)
    (hunit : ∀ 𝒪 : ValuationSubring Ksep,
      (𝒪.comap (algebraMap K Ksep)).toSubring = (algebraMap R K).range →
  ...
  classical
  have hm0 : m ≠ 0 := fun h0 => hm (by rw [h0, Nat.cast_zero])
  haveI hVfin : Finite (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :=
    WeierstrassCurve.torsion_finite_of_ne_zero K E Ksep m hm0
  -- E3: the branch family (the remaining geometric leaf)
  obtain ⟨k, f, hfmem, hfkill, hfcover⟩ :=
    WeierstrassCurve.exists_torsionKernel_branch_family R K E Ksep m hm h hmon hdeg hunit
  -- E1: values of the product algebra are integral over `R`
  have hAint := WeierstrassCurve.adjoin_torsionKernelGens_isIntegral_values
    R K E Ksep m h hmon hunit
  have hBint := adjoin_prod_sections_isIntegral_values hAint hAint
  -- E0: convert the span memberships into subalgebra memberships
  have hfB := fun i => (mem_span_prod_sections_iff_mem_adjoin _ _ _).mp (hfmem i)
  -- E2: the partition of unity `∑ cᵢ·fᵢ = 1`
  obtain ⟨c, hcB, hsum⟩ :=
    exists_partition_of_unity_of_center_units R K Ksep _ hBint f hfB hfcover
  -- E4: assembly — `F = ∑ i, cᵢ·(fᵢ·F)`, every summand in the subalgebra
  refine (mem_span_prod_sections_iff_mem_adjoin _ _ _).mpr ?_
  have hFeq : ∑ i, c i * (f i *
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
        WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑(PQ.1 + PQ.2))) =
      (fun PQ : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) =>
        WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑(PQ.1 + PQ.2)) := by
    simp only [← mul_assoc]
    rw [← Finset.sum_mul, hsum, one_mul]
  rw [← hFeq]
  exact sum_mem fun i _ =>
    mul_mem (hcB i)
      ((mem_span_prod_sections_iff_mem_adjoin _ _ _).mp (hfkill i a b hb hw))
...
theorem WeierstrassCurve.exists_torsion_kernel_function_generators
      ...
      (∀ u ∈ gens₀,
        ...
        Submodule.span R {t : (AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) ×
          ...
          ∃ b ∈ Algebra.adjoin R
            ...
            (fun PQ => a PQ.1 * b PQ.2) = t}) := by
  classical
  obtain ⟨h, hmon, hdeg, hunit⟩ :=
    WeierstrassCurve.exists_torsion_avoiding_denominator R K E Ksep m hm
  refine ⟨WeierstrassCurve.torsionKernelGens R K E Ksep m h, ?_, ?_, ?_, ?_, ?_⟩
  · -- Galois equivariance
    intro u hu σ P Q hPQ
    obtain ⟨a, b, ha, hb, hw, rfl⟩ :=
      (WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h u).mp hu
    show WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑Q =
      σ (WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑P)
    rw [hPQ]
    exact WeierstrassCurve.torsionKernelFun_map R K E Ksep h a b σ ↑P
  · -- integrality of values
    intro u hu P
    obtain ⟨a, b, ha, hb, hw, rfl⟩ :=
      (WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h u).mp hu
    exact WeierstrassCurve.torsionKernelFun_isIntegral R K E Ksep m h hmon hunit
  ...
  · -- separation
    intro P Q hPQ
    refine Subtype.ext (WeierstrassCurve.torsionKernelFun_separates R K E Ksep m
      h hmon hdeg hunit ((Submodule.mem_torsionBy_iff _ _).mp P.2)
      ((Submodule.mem_torsionBy_iff _ _).mp Q.2) fun a b ha hb hw => ?_)
    exact hPQ (fun P' => WeierstrassCurve.torsionKernelFun R K E Ksep h a b ↑P')
      ((WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h _).mpr
        ⟨a, b, ha, hb, hw, rfl⟩)
  · -- negation closure
    intro u hu
    ...
    · -- even sections are negation-invariant
      have hfun : (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
          WeierstrassCurve.torsionKernelFun R K E Ksep h a 0 ↑(-P)) =
          fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
            WeierstrassCurve.torsionKernelFun R K E Ksep h a 0 ↑P := by
        funext P
        rw [show ((-P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
            (E⁄Ksep).Point) = -(P : (E⁄Ksep).Point) from rfl,
          WeierstrassCurve.torsionKernelFun_neg_even]
      show (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
        WeierstrassCurve.torsionKernelFun R K E Ksep h a 0 ↑(-P)) ∈ _
      rw [hfun]
      exact Algebra.subset_adjoin
        ((WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h _).mpr
          ⟨a, 0, ha, by omega, by omega, rfl⟩)
    · -- odd sections: the `R`-integral negation combination
      have hkey : (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
          WeierstrassCurve.torsionKernelFun R K E Ksep h a 1 ↑(-P)) =
          -(fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
              WeierstrassCurve.torsionKernelFun R K E Ksep h a 1 ↑P)
          - algebraMap R ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep)
              (WeierstrassCurve.integralModel R E).a₁ *
            (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
              WeierstrassCurve.torsionKernelFun R K E Ksep h (a + 1) 0 ↑P)
          - algebraMap R ((AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) → Ksep)
            ...
            (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
              WeierstrassCurve.torsionKernelFun R K E Ksep h a 0 ↑P) := by
        funext P
        show WeierstrassCurve.torsionKernelFun R K E Ksep h a 1
          ((-P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
            (E⁄Ksep).Point) = _
        rw [show ((-P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ)) :
            (E⁄Ksep).Point) = -(P : (E⁄Ksep).Point) from rfl,
          WeierstrassCurve.torsionKernelFun_neg_odd R K E Ksep m h hmon hunit a
            (by omega) (P : (E⁄Ksep).Point)
            ((Submodule.mem_torsionBy_iff _ _).mp P.2)]
        simp only [Pi.sub_apply, Pi.neg_apply, Pi.mul_apply, Pi.algebraMap_apply]
      show (fun P : AddSubgroup.torsionBy (E⁄Ksep).Point (m : ℤ) =>
        WeierstrassCurve.torsionKernelFun R K E Ksep h a 1 ↑(-P)) ∈ _
      rw [hkey]
      refine sub_mem (sub_mem (neg_mem (Algebra.subset_adjoin
        ((WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h _).mpr
          ⟨a, 1, ha, by omega, hw, rfl⟩)))
        (mul_mem (Subalgebra.algebraMap_mem _ _) (Algebra.subset_adjoin
          ((WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h _).mpr
            ⟨a + 1, 0, by omega, by omega, by omega, rfl⟩))))
        (mul_mem (Subalgebra.algebraMap_mem _ _) (Algebra.subset_adjoin
          ((WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h _).mpr
            ⟨a, 0, by omega, by omega, by omega, rfl⟩)))
  · -- addition-law closure
    intro u hu
    obtain ⟨a, b, ha, hb, hw, rfl⟩ :=
      (WeierstrassCurve.mem_torsionKernelGens R K E Ksep m h u).mp hu
    exact WeierstrassCurve.torsionKernelFun_add_mem_span R K E Ksep m hm h hmon
      hdeg hunit a b hb hw

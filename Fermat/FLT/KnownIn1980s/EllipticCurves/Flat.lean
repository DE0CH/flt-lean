...
made pointwise): `M := span R {a(P)·b(Q)}` is closed under multiplication,
so a PARTITION-OF-UNITY argument applies — find `u₁ + … + u_k = 1` on
`V × V` with `uᵢ ∈ M` such that each `uᵢ` kills the denominators of one
branch of the addition law (`(x(P) − x(Q))²` for the chord case, `ψ₂²` for
the tangent case, `h(x)`-units elsewhere), making `uᵢ·(g ∘ add)` a
polynomial in the sections of the two factors; the partition comes from
the division-polynomial arithmetic on the torsion locus: affine `m`-torsion
abscissas are roots of `ΨSq m` (`smul_some_eq_zero_iff`), the Bézout
identity `F·Φ m + G·ΨSq m = 1` (`isCoprime_Φ_ΨSq`, unit discriminant from
good reduction) inverts `Φ m` on them, and the multiplied-out addition
formulas (`add_some_coords`, `cross_two`, `two_point_trace_identity`)
supply the numerators. No indicator idempotents are available (for `μ_p`
over `ℤ_p` the origin idempotent is NOT integral), so the partition must
come from these global identities. DECOMPOSE further when attacked. -/

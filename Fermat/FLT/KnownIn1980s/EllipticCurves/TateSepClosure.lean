...
uniformising homomorphism over the separable closure `Ω` exists:
`Ωˣ →+ E_q(Ω)`, surjective, with kernel exactly `q^ℤ`, commuting with
every `k`-automorphism of `Ω`. The glued map computes `pointMapQuot` at
the finite subextension generated by the input
(`tateGluePointAt`, well-defined across levels by
`tateGluePointAt_inclusion`); additivity is `tateGluePointAt_mul` at a
compositum, the kernel is `tateGluePointAt_eq_zero_iff` plus `q`-power
transfer along the injective inclusion, equivariance is
`tateGluePointAt_conj` (using `σ`-invariance of the constructed
valuation), and surjectivity pulls an `Ω`-point back to the finite
level generated by its coordinates and applies the finite-level
equivalence there. -/
theorem WeierstrassCurve.exists_tateCurveHomSepClosure_of_finiteLevel
  ...
  have hΦsurj : ∀ P : (((tateCurve ((q : k) : k))⁄Ω)).Point,
    ...
    | @some x y hns =>
      ...
      -- the `L`-rational point below `P`
      have hnsL : (((tateCurve ((q : k) : k))⁄L)).toAffine.Nonsingular
          (⟨x, hxL⟩ : L) (⟨y, hyL⟩ : L) := by
        have h1 := ((tateCurve ((q : k) : k)).toAffine.baseChange_nonsingular
          (f := L.val) L.val.injective (⟨x, hxL⟩ : L) (⟨y, hyL⟩ : L)).mp
        exact h1 hns
      -- local-field instances at `L`
      letI : ValuativeRel L := @ValuativeRel.comap L Ω _ _ vΩ L.val.toRingHom
      letI : TopologicalSpace L := ValuativeRel.topologicalSpace L
      haveI : IsNonarchimedeanLocalField L := hL L inferInstance
      haveI : CharZero L :=
        charZero_of_injective_algebraMap (algebraMap k L).injective
      haveI : ValuativeExtension k L := hLk L
      -- the finite-level equivalence at `L`
      obtain ⟨eL, heL⟩ := hfin L (Units.map (algebraMap k L).toMonoidHom q)
        (show valuation L
            (((Units.map (algebraMap k L).toMonoidHom q : Lˣ) : L)) < 1 from
          TateCurve.valuation_algebraMap_lt_one hq)
      -- the curve identification and the transported point
      have hcur : WeierstrassCurve.tateCurve
            (((Units.map (algebraMap k L).toMonoidHom q : Lˣ) : L)) =
          (((WeierstrassCurve.tateCurve ((q : k) : k))⁄L) :
            WeierstrassCurve L) :=
        (TateCurve.tateCurve_map (l := L) ((q : k) : k) hq).symm
      obtain ⟨c, hc⟩ := eL.surjective
        (pointCastHom hcur.symm
          (WeierstrassCurve.Affine.Point.some _ _ hnsL))
      obtain ⟨w, hw⟩ := QuotientGroup.mk_surjective c.toMul
      refine ⟨Units.map L.val.toRingHom.toMonoidHom w, ?_⟩
      have hmem : ((Units.map L.val.toRingHom.toMonoidHom w : Ωˣ) : Ω) ∈ L :=
        (w : L).2
      rw [hΦat L inferInstance _ hmem]
      have htu : toUnit L (Units.map L.val.toRingHom.toMonoidHom w) hmem = w := by
        apply Units.ext
        apply Subtype.ext
        rfl
      rw [htu]
      -- unfold the level map and cancel the two transports
      have hcc : ∀ (W₁ W₂ : WeierstrassCurve L) (h12 : W₁ = W₂)
          (Q : W₂.toAffine.Point),
          pointCastHom h12 (pointCastHom h12.symm Q) = Q := by
        intro W₁ W₂ h12 Q
        subst h12
        rfl
      have h1 : TateCurve.pointMapQuot
          (Units.map (algebraMap k L).toMonoidHom q)
          (show valuation L
              (((Units.map (algebraMap k L).toMonoidHom q : Lˣ) : L)) < 1 from
            TateCurve.valuation_algebraMap_lt_one hq)
          (QuotientGroup.mk w) =
          pointCastHom hcur.symm
            (WeierstrassCurve.Affine.Point.some _ _ hnsL) := by
        rw [← hc, heL, hw]
      show WeierstrassCurve.Affine.Point.map (W' := tateCurve ((q : k) : k))
          (S := k) L.val
          (pointCastHom hcur
            (TateCurve.pointMapQuot (Units.map (algebraMap k L).toMonoidHom q)
              (show valuation L
                  (((Units.map (algebraMap k L).toMonoidHom q : Lˣ) : L)) < 1 from
                TateCurve.valuation_algebraMap_lt_one hq)
              (QuotientGroup.mk w))) = _
      rw [h1, hcc]
      -- push the `L`-point forward: it is `P`
      rw [WeierstrassCurve.Affine.Point.map_some]
      rfl
  -- package
...
* the map is a *function of the uniformisation `e` alone*, so the maps at
  different levels can be COMPARED — `tateExponent_zsmul` below is that
...
* "`P` lies over the canonical generator of the étale quotient" can be said as
  ...
  `∃ u, u ^ N = q ∧ P = point u`.  `tateExponent_eq_one_iff` proves those two
...
theorem WeierstrassCurve.tateSymm_zsmul_eq_zero
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    {N : ℕ} (P : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ)) :
    ((N : ℕ) : ℤ) • (e.symm (P : ((E⁄Ω)).Point)) = 0 := by
  rw [← map_zsmul e.symm,
    (show ((N : ℕ) : ℤ) • (P : ((E⁄Ω)).Point) = 0 from P.2), map_zero]

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- **THE CANONICAL ÉTALE EXPONENT of an `N`-torsion point of a Tate-uniformised
curve** (PROVEN — a definition, canonical in `(e, N, P)` and in nothing else).

An `N`-torsion class of `Ωˣ / q ^ ℤ` is represented by some `u` with
`u ^ N = q ^ a` (`exists_rep_pow_eq_zpow_of_torsion`); the class of `a` in
`ℤ/N` does not depend on the representative (`tateExponent_eq_of_rep`), so this
is the value of the quotient map `E[N] ↠ ℤ/N` of
`0 → μ_N → E[N] → ℤ/N → 0` at `P`.
...
`tateExponent_spec` / `tateExponent_eq_of_rep` and never unfold it. -/
noncomputable def WeierstrassCurve.tateExponent
  ...
  (((exists_rep_pow_eq_zpow_of_torsion (E.qUnitSepClosure Ω)
      (e.symm (P : ((E⁄Ω)).Point))
      (E.tateSymm_zsmul_eq_zero Ω e P)).choose_spec.choose : ℤ) : ZMod N)

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- **The defining property of `tateExponent`** (PROVEN): SOME representative
realises it.  This is the only lemma that unfolds the definition. -/
theorem WeierstrassCurve.tateExponent_spec
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    {N : ℕ} (P : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ)) :
    ∃ (u : Ωˣ) (a : ℤ),
      Additive.ofMul ((u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) =
          e.symm (P : ((E⁄Ω)).Point) ∧
        u ^ N = E.qUnitSepClosure Ω ^ a ∧ ((a : ZMod N)) = E.tateExponent Ω e P :=
  ⟨_, _,
    (exists_rep_pow_eq_zpow_of_torsion (E.qUnitSepClosure Ω)
      (e.symm (P : ((E⁄Ω)).Point))
      (E.tateSymm_zsmul_eq_zero Ω e P)).choose_spec.choose_spec.1,
    (exists_rep_pow_eq_zpow_of_torsion (E.qUnitSepClosure Ω)
      (e.symm (P : ((E⁄Ω)).Point))
      (E.tateSymm_zsmul_eq_zero Ω e P)).choose_spec.choose_spec.2,
    rfl⟩

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- **`tateExponent` is well defined: EVERY representative realises it**
(PROVEN — the `hindep` step of the original existential proof, promoted to a
standalone lemma about the canonical map).

Two representatives of the same class differ by `q ^ m`, so their exponents
differ by `m · N`, which is `0` in `ℤ/N`.  The injectivity of `a ↦ q ^ a`
(`qUnitSepClosure_zpow_injective`, itself a consequence of `v(q) < 1`) is what
turns the equality of `q`-powers into an equality of exponents; without it the
exponent would not be determined at all. -/
theorem WeierstrassCurve.tateExponent_eq_of_rep
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    {N : ℕ} (P : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ))
    (v : Ωˣ) (b : ℤ)
    (hv : Additive.ofMul ((v : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) =
      e.symm (P : ((E⁄Ω)).Point))
    (hvb : v ^ N = E.qUnitSepClosure Ω ^ b) :
    ((b : ZMod N)) = E.tateExponent Ω e P := by
  obtain ⟨u, a, hu, ha, hae⟩ := E.tateExponent_spec Ω e P
  have h1 : ((u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) =
  ...
  have h2 : v = u * E.qUnitSepClosure Ω ^ m := by rw [hm]; group
  have h3 : E.qUnitSepClosure Ω ^ b =
      E.qUnitSepClosure Ω ^ (a + m * (N : ℤ)) := by
    rw [← hvb, h2, mul_pow, ha, ← zpow_natCast (E.qUnitSepClosure Ω ^ m) N,
      ← zpow_mul, ← zpow_add]
  have h4 : b = a + m * (N : ℤ) := E.qUnitSepClosure_zpow_injective Ω h3
  rw [← hae, h4]
  push_cast
  simp
...
(PROVEN).  `tateExponent P = c` says exactly that `P` is the image of some `N`-th
root of `q ^ c`.
...
representative, so the point is unaffected.  That correction is possible ONLY
because the ambiguity `q ^ ℤ` of the uniformisation is exactly an `N`-th power
ambiguity in the exponent; it is the reason a leaf may quantify over "some
`p ^ n`-th root of `q`" and a named map may be substituted for it. -/
theorem WeierstrassCurve.tateExponent_eq_intCast_iff
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    {N : ℕ} (P : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ)) (c : ℤ) :
    E.tateExponent Ω e P = ((c : ZMod N)) ↔
      ∃ u : Ωˣ, u ^ N = E.qUnitSepClosure Ω ^ c ∧
        e (Additive.ofMul ((u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)))) =
          (P : ((E⁄Ω)).Point) := by
  constructor
  · intro hc
    obtain ⟨u, a, hu, ha, hae⟩ := E.tateExponent_spec Ω e P
    ...
    refine ⟨u * E.qUnitSepClosure Ω ^ m, ?_, ?_⟩
    · rw [mul_pow, ha, ← zpow_natCast (E.qUnitSepClosure Ω ^ m) N, ← zpow_mul,
        ← zpow_add]
      congr 1
      linear_combination -hm
    · have hcl : ((u * E.qUnitSepClosure Ω ^ m : Ωˣ) :
          Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) =
          ((u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) := by
        rw [QuotientGroup.mk_mul,
          (QuotientGroup.eq_one_iff _).mpr
            (Subgroup.zpow_mem _ (Subgroup.mem_zpowers _) m)]
        exact mul_one ((u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)))
      rw [hcl, hu, e.apply_symm_apply]
  · rintro ⟨u, hu, hP⟩
    refine (E.tateExponent_eq_of_rep Ω e P u c ?_ hu).symm
    rw [← hP, e.symm_apply_apply]

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- **"`P` lies over the canonical generator of the étale quotient", as an
EQUATION** (PROVEN): `tateExponent P = 1` iff `P` is the image of some `N`-th
root of the Tate parameter itself.  The case `c = 1` of
`tateExponent_eq_intCast_iff`, and the form the toric leaves of
`Modularity/Interface.lean` state by hand. -/
theorem WeierstrassCurve.tateExponent_eq_one_iff
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    {N : ℕ} (P : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ)) :
    E.tateExponent Ω e P = 1 ↔
      ∃ u : Ωˣ, u ^ N = E.qUnitSepClosure Ω ∧
        e (Additive.ofMul ((u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)))) =
          (P : ((E⁄Ω)).Point) := by
  have h := E.tateExponent_eq_intCast_iff Ω e P 1
  rw [Int.cast_one] at h
  simpa only [zpow_one] using h

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- **`tateExponent` is additive** (PROVEN): the product of two representatives
represents the sum, and its exponent is the sum of the exponents. -/
theorem WeierstrassCurve.tateExponent_add
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    {N : ℕ} (P Q : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ)) :
    E.tateExponent Ω e (P + Q) = E.tateExponent Ω e P + E.tateExponent Ω e Q := by
  obtain ⟨u, a, hu, ha, hae⟩ := E.tateExponent_spec Ω e P
  obtain ⟨v, b, hv, hb, hbe⟩ := E.tateExponent_spec Ω e Q
  have hclass : Additive.ofMul (((u * v : Ωˣ) :
      Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) =
  ...
  have hpow : (u * v) ^ N = E.qUnitSepClosure Ω ^ (a + b) := by
    rw [mul_pow, ha, hb, ← zpow_add]
  rw [← E.tateExponent_eq_of_rep Ω e (P + Q) (u * v) (a + b) hclass hpow,
    ← hae, ← hbe]
  push_cast
  ring
...
nothing but the uniformisation `e` and the level `N`. -/
noncomputable def WeierstrassCurve.tateExponentHom
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    (N : ℕ) :
    AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ) →+ ZMod N :=
  AddMonoidHom.mk' (fun P => E.tateExponent Ω e P) (E.tateExponent_add Ω e)

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
@[simp] theorem WeierstrassCurve.tateExponentHom_apply
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    (N : ℕ) (P : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ)) :
    E.tateExponentHom Ω e N P = E.tateExponent Ω e P := rfl
...
theorem WeierstrassCurve.tateExponent_map
    ...
    E.tateExponent Ω e Q = E.tateExponent Ω e P := by
  obtain ⟨u, a, hu, ha, hae⟩ := E.tateExponent_spec Ω e P
  ...
  have hpow : (Units.map σ.toAlgHom.toRingHom.toMonoidHom u) ^ N =
      E.qUnitSepClosure Ω ^ a := by
    rw [← map_pow, ha, map_zpow_qUnitSepClosure_eq]
  rw [← E.tateExponent_eq_of_rep Ω e Q _ _ hclass hpow, hae]
...
theorem WeierstrassCurve.tateExponent_surjective
  ...
  have hxupow : xu ^ N = E.qUnitSepClosure Ω ^ (1 : ℤ) := by
    apply Units.ext
    rw [Units.val_pow_eq_pow_val, Units.val_zpow_eq_zpow_val, zpow_one]
    exact hxpow
  have htor : ((N : ℕ) : ℤ) • (e (Additive.ofMul ((xu :
  ...
  set P₁ : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ) :=
    ⟨e (Additive.ofMul ((xu : Ωˣ ⧸ Subgroup.zpowers
      (E.qUnitSepClosure Ω)))), htor⟩ with hP₁
  have hπ1 : E.tateExponentHom Ω e N P₁ = 1 := by
    have hclass : Additive.ofMul ((xu : Ωˣ ⧸ Subgroup.zpowers
        (E.qUnitSepClosure Ω))) = e.symm ((P₁ : ((E⁄Ω)).Point)) := by
      rw [hP₁]
      exact (e.symm_apply_apply _).symm
    have := E.tateExponent_eq_of_rep Ω e P₁ xu 1 hclass hxupow
    rw [E.tateExponentHom_apply Ω e N P₁, ← this]
    norm_num
  haveI : NeZero N := ⟨hN⟩
  intro c
  refine ⟨c.val • P₁, ?_⟩
...
omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- **THE COHERENCE BETWEEN LEVELS** (PROVEN — what makes `tateExponent` a map
on the TATE MODULE and not merely on each torsion group separately).

If `Q = [M] · P` with `P` of level `M · N`, then `Q` has level `N` and

    tateExponent Q = (tateExponent P mod N).
...
theorem WeierstrassCurve.tateExponent_zsmul
    (e : Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+ ((E⁄Ω)).Point)
    {M N : ℕ} (P : AddSubgroup.torsionBy ((E⁄Ω)).Point (((M * N : ℕ) : ℕ) : ℤ))
    (Q : AddSubgroup.torsionBy ((E⁄Ω)).Point ((N : ℕ) : ℤ))
    (hQ : (Q : ((E⁄Ω)).Point) = (M : ℤ) • (P : ((E⁄Ω)).Point)) :
    E.tateExponent Ω e Q =
      ZMod.castHom (⟨M, mul_comm M N⟩ : N ∣ M * N) (ZMod N)
        (E.tateExponent Ω e P) := by
  obtain ⟨u, a, hu, ha, hae⟩ := E.tateExponent_spec Ω e P
  have hclass : Additive.ofMul (((u ^ M : Ωˣ) :
      Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) =
      e.symm ((Q : ((E⁄Ω)).Point)) := by
    rw [hQ, map_zsmul e.symm, ← hu, QuotientGroup.mk_pow, ← zpow_natCast
      ((u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) M, ofMul_zpow]
  have hpow : (u ^ M) ^ N = E.qUnitSepClosure Ω ^ a := by
    rw [← pow_mul, ha]
  rw [← E.tateExponent_eq_of_rep Ω e Q _ _ hclass hpow, ← hae]
  simp

...
-- NOTE (2026-07-24, worktree-9 repair): a second copy of
-- `exists_algEquiv_map_zeta_eq_pow_natCard_of_not_dvd` lived here — an
-- integration artifact of the flt-lean-4/flt-lean-13 merges (both
-- branches proved the same Frobenius leaf; the merge kept both,
-- breaking the build with a duplicate-declaration error). The earlier
-- copy above (identical statement, same proof shape) is the one kept;
-- consumers before and after this point resolve to it unchanged.

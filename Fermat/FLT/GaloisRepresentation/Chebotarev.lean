...
remains irreducible over `E`.

This is a purely algebraic finiteness statement — no density input. The
classical proof: primes `ℓ ≡ 1 (mod n)` exist in abundance by the
elementary cyclotomic-polynomial argument (`Nat.exists_prime_gt_modEq_one`,
already in mathlib, no Dirichlet needed); and for every prime `ℓ`
unramified in the Galois closure of `E/ℚ` (all but finitely many, since
ramified primes divide the discriminant), `E ∩ ℚ(ζ_ℓ) = ℚ` — every proper
subextension of `ℚ(ζ_ℓ)/ℚ` is ramified at `ℓ` because `ℚ(ζ_ℓ)/ℚ` is
totally ramified at `ℓ` — whence `[E(ζ_ℓ) : E] = ℓ - 1` by linear
disjointness (one side Galois) and `cyclotomic ℓ E` is irreducible.
Choosing `ℓ ≡ 1 (mod n)` larger than all ramified primes closes it. -/
theorem exists_prime_dvd_sub_one_and_irreducible_cyclotomic
    (E : Type*) [Field E] [NumberField E] {n : ℕ} (hn : n ≠ 0) :
    ∃ ℓ : ℕ, ℓ.Prime ∧ n ∣ ℓ - 1 ∧ Irreducible (cyclotomic ℓ E) :=
  sorry

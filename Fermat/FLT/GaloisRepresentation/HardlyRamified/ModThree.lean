...
over `3`** (sorry node, created 2026-07-24 — the deep leaf of the
decomposition of the filtration bound
`two_mul_sum_card_inertia_pow_add_two_le_of_flat_at_three` below;
Fontaine, *Il n'y a pas de variété abélienne sur ℤ*, Invent. Math. 81
(1985), Thm. A and §3; Moon–Taguchi, *Refinement of Tate's discriminant
bound and non-existence theorems for mod p Galois representations*,
Doc. Math. Extra Vol. Kato (2003), §2): SOME prime `Q₀` of `𝓞 K` above
`3` — the prover chooses it, and is expected to choose the
distinguished prime induced by the fixed embedding
`ι : ℚᵃˡᵍ → ℚ₃ᵃˡᵍ` — satisfies the SHARP different-exponent bound
`2·d + 2 ≤ 3·e(Q₀∣3)` for every `d` with `Q₀^d ∣ 𝔡_{K/ℚ}`.  This
strengthens by the `+2` the bound of the (downstream, proven)
`exists_prime_over_three_differentIdeal_exponent_bound`; it is
strictly upstream of that chain, so no circularity: the intended
content is genuinely LOCAL.  Roadmap for the prover: at the
distinguished prime `Q₀ := ψ⁻¹(𝔪_{𝒪_M})` of the construction of
`exists_prime_over_not_mem_sq_of_le_fixingSubgroup`
(`InertiaCardTransport`, without the unramifiedness input), the
completion of `K` at `Q₀` is `M := ℚ₃(ι K)`; completion invariance of
the different (Serre, *Corps Locaux* III §4 Prop. 10) identifies the
exponent of `Q₀` in `𝔡_{K/ℚ}` with `v_M(𝔡_{M/ℚ₃})`, and
`e(Q₀∣3) = e(M/ℚ₃)`.  Since `K` is the kernel field of `u` (`hfix`),
`M = ℚ₃(V)` is generated by the coordinates of the points of the
finite flat group scheme `G/ℤ₃` killed by `3` prolonging the local
representation (`hρ.isFlat` through `GaloisRep.HasFlatProlongationAt`,
as unpacked in `exists_connectedEtale_subgroup_at_three`).  Fontaine's
Thm. A then bounds `v_M(𝔡_{M/ℚ₃}) ≤ e·(1 + 1/(3−1)) − 1 = (3/2)·e − 1`,
i.e. `2d + 2 ≤ 3e`; in the tame case `v_M(𝔡) = e − 1` gives
`2e ≤ 3e` outright.  Sharpness: the peu-ramifié kernel field
`ℚ₃(ζ₃, u^{1/3})` has `e = 6`, `v(𝔡) = 8`, attaining equality
`2·8 + 2 = 18 = 3·6`. -/

...
**DELETED 2026-07-27, and replaced by the hoisted `ℚ`-level one.** This file
used to carry its own `hilbertDiamondAug`, `HilbertPatchingLevel` and
`HilbertPatchingSystem`, together with a limit extraction
`exists_patchedModule_of_hilbertPatchingSystem` out of them. All four are gone.

The `F`-level structures had been shaped field-for-field against
`Modularity.TaylorWilesLevelRaw` / `Modularity.TaylorWilesSystem` in the
2026-07-27 cut repair, precisely so that this substitution would become
available; the extraction they fed is now literally the same theorem the
`ℚ` level uses, so proving it here would have built a second copy of an
existing engine.

**What made the substitution possible is a MOVE, not a proof.**
`Modularity/Patching.lean` transitively imports this module
(`Patching → KhareWintenberger → Deformation → HilbertModularity`), so the
`ℚ`-level extraction could not be reused where it lived. It was therefore
hoisted VERBATIM into `Modularity/PatchingCore.lean`, whose `Fermat`-import
closure contains neither this module nor `PatchingCore`'s own consumers — see
the banner there for the acyclicity argument. `exists_hilbertPatchingSystem`
below now produces a `Modularity.TaylorWilesSystem` directly, and
`exists_hilbertPatchedModule` consumes it through
`Modularity.TaylorWilesSystem.exists_patchedModule`.
...
theorem exists_hilbertPatchedModule
  ...
  exact (exists_hilbertPatchingSystem ℓ hℓ5 F htr hgal hirrF 𝒟 𝒟T T e h𝒟w h𝒟t ψ

...
2. `IsHilbertHardlyRamified` — the `F`-level local deformation condition,
   ...
   tame-at-`2` half, is PROVEN (2026-07-26) and its last leaf
   `map_mem_inertia_Z2bar_of_mem_localInertiaGroup` — the agreement of the
   `IntegralClosure 𝒪_v` and `Z2bar` spellings of local inertia at `2` — is
   PROVEN too (2026-07-26), so the whole tame-at-`2` half is now closed;
   the flatness half `isFlatAt_map_of_isFlatAt_under` is still open.

/-
GaloisRepresentation/HardlyRamified/DiscrExponent.lean — own work for the
Fermat project (not vendored from the FLT project).
-/
module

-- `IsHardlyRamified.discr_factorization_le_of_forall_differentIdeal_pow_dvd`
-- and `IsHardlyRamified.not_pow_ramificationIdx_dvd_differentIdeal`, the two
-- (and ONLY two) project names this development consumes.  That module's
-- import cone is `Mathlib` only, so this one's is too.
...
Serre's wild different bound `d_Q ≤ e − 1 + e·v_q(e)` and the
discriminant-exponent bound it yields,

  `exists_discr_factorization_le_of_finrank_le` :
    for a fixed prime `q` and a degree bound `n` there is a constant `C`
    with `v_q(discr K) ≤ C` for every number field `K ⊆ ℚᵃˡᵍ` of degree
    at most `n`,

hoisted verbatim out of
...
Nothing in this file mentions a Galois representation, an elliptic curve,
inertia, or `IsHardlyRamified`; it is `differentIdeal`, `NumberField.discr`,
`Ideal.ramificationIdx'` and valuation bookkeeping, exactly as
`HardlyRamified/DifferentIdeal.lean` (its one project import) is.  The whole
block is a strict `Mathlib`-cone development that happened to be written where
its first consumer lived.

`HermiteMinkowski.lean` is DOWNSTREAM of `Fermat/FLT/ModularCurve/X0.lean`
(it imports `FreyCurve/MazurTorsion`, which imports `X0`), so the second
consumer of this bound — the weak Mordell–Weil chain in `X0.lean`, whose
`exists_discrBound_divisionField_of_abelianScheme` needs exactly "bounded
degree plus unramified outside a finite set implies bounded discriminant" —
could not reach it.  The `X0.lean` docstring recorded the repair as
"hoist the different-ideal/discriminant-exponent development above `X0.lean`";
this module is that hoist.  `HermiteMinkowski.lean` `public import`s it, so
every existing consumer sees the same names in the same namespace,
unqualified and unchanged.

`InertiaTrivialAt` and `not_dvd_discr_of_inertiaTrivialAt` deliberately stay
behind in `HermiteMinkowski.lean`: they speak of `localInertiaGroup`, which is
a `MazurTorsion` name and therefore genuinely downstream.
-/

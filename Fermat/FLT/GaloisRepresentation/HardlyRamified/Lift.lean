...
* **B6a** (`exists_hardlyRamifiedLift`): an irreducible hardly ramified
  ...
  structure `HardlyRamifiedLift`. MOVED (2026-07-24, the pillar-α
  proof-sharing refactor): the whole deformation development behind it —
  the Khare–Wintenberger core `exists_finite_lift`, Mazur's category
  `HardlyRamifiedDeformation` with the universality / finiteness /
  presentation strata and their sorried arithmetic leaves, the
  commutative-algebra and topology glue, and the
  specialization-to-a-domain endgame — now lives in the Family-free
  module `Deformation.lean`, generalized over a finite coefficient
  field `k`, so that pillar α of `Modularity/KhareWintenberger.lean`
  (`exists_hardlyRamified_lift_residual_of_five_le`) can consume the
  SAME development without closing the interface's forbidden dependency
  cycle. `exists_hardlyRamifiedLift` below is the instantiation at
  `k = ZMod ℓ` of the shared terminal theorem
...
-- the shared Family-free deformation development (2026-07-24 pillar-α
-- proof-sharing refactor): the whole Khare–Wintenberger lifting core,
-- generalized over a finite coefficient field `k`, lives in
-- `Deformation.lean` and is instantiated here at `k = ZMod ℓ`.
-- (`rank_finTwoFun` appears in the exposed `HardlyRamifiedLift` field
-- types, hence the public import.)
public import Fermat.FLT.GaloisRepresentation.HardlyRamified.Deformation
public import Mathlib.Topology.Instances.ZMod
-- `IsAdic` / `IsAdicComplete`: historically for the deformation-category
-- structure `HardlyRamifiedDeformation` (moved to `Deformation.lean`,
-- which re-exports them); the public imports are KEPT so downstream
-- modules importing `Lift.lean` see an unchanged export surface.
...
DELEGATED (2026-07-24, the pillar-α proof-sharing refactor): the whole
Khare–Wintenberger lifting development — Mazur's category, the
universality / finiteness / presentation strata with their sorried
arithmetic leaves, the commutative-algebra and topology glue, and the
specialization-to-a-domain endgame — formerly inlined in this file, now
lives in the Family-free module `Deformation.lean`, generalized over a
finite coefficient field `k`; this theorem is its instantiation at
`k = ZMod ℓ`, repackaged in the `HardlyRamifiedLift` structure. The
SAME generalized theorem (`exists_hardlyRamified_lift_of_five_le`)
discharges pillar α of `Modularity/KhareWintenberger.lean` — the
cycle-free proof sharing anticipated by that pillar's docstring — so
the deformation-theoretic sorried leaves are now SHARED nodes serving
both consumers. -/
theorem exists_hardlyRamifiedLift (hℓ5 : 5 ≤ ℓ)
  ...
  obtain ⟨O, iO1, iO2, iO3, iO4, iO5, iO6, iO7, iO8, hinj, ρ, hrank, hHR,
    π, -, hcompat⟩ :=
    exists_hardlyRamified_lift_of_five_le hℓOdd hdim hℓ5 h hirr

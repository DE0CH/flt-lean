...
* **B6a** (`exists_hardlyRamifiedLift`): an irreducible hardly ramified
  ...
  three arithmetic leaves: Mazur–Carayol representability
  `exists_isWeaklyUniversal_isTraceGenerated`, potential-modularity
  finiteness `moduleFinite_of_isWeaklyUniversal_isTraceGenerated`, and
  the Böckle dimension bound
...
/-- **Representability leaf** (sorry node — the arithmetic core of the
representability stratum): the hardly ramified deformation problem of an
irreducible hardly ramified `ρbar` (`ℓ ≥ 5`) admits a weakly universal,
trace-generated object.
...
`R^{univ}` with residue field `𝔽_ℓ`; every hardly ramified deformation
receives a map from it (weak universality), and by Carayol's theorem the
universal ring is topologically generated by traces of Frobenii, which
Chebotarev density recovers from the good primes `q ∉ {2, ℓ}` (trace
generation). The uniqueness half of universality is NOT part of this
leaf: it is derived formally in
...
theorem exists_isWeaklyUniversal_isTraceGenerated (hℓ5 : 5 ≤ ℓ)
    ...
    ∃ D : HardlyRamifiedDeformation hℓOdd ρbar,
      D.IsWeaklyUniversal ∧ D.IsTraceGenerated :=
  sorry
...
totally real field, descended by Khare–Wintenberger's induction. The
hypotheses characterize `D` up to canonical isomorphism (weak
...
/-- **Dimension leaf** (sorry node — the arithmetic core of the
presentation stratum): the weakly universal, trace-generated hardly
ramified deformation ring has Krull dimension `≥ 1` — some prime lies
strictly below the maximal ideal.
...
than relations has Krull dimension `≥ 1` (Krull's height theorem), i.e.
its maximal ideal is not a minimal prime. As with the finiteness leaf,
the hypotheses pin `D` down up to canonical isomorphism, so a future
proof may construct its own universal datum and transport. -/
theorem exists_isPrime_lt_maximalIdeal_of_isWeaklyUniversal_isTraceGenerated
    ...
    ∃ P : Ideal D.R, P.IsPrime ∧ P < IsLocalRing.maximalIdeal D.R :=
  sorry

...
/-- **Representability stratum** (sorry node): the hardly ramified
...
distinct eigenvalues `±1`). Hence by Schur's lemma and Mazur's criterion
the framed/fixed-determinant deformation functor with the hardly ramified
...
quotient at `2` — is representable by a complete Noetherian local
`ℤ_ℓ`-algebra `R^{univ}` with residue field `𝔽_ℓ`, and the universal
deformation is itself hardly ramified in this repository's sense (the
open-ideal formulation of `IsFlatAt` is exactly Ramakrishna flatness of
the artinian quotients). The `∃!`-clause of `IsUniversal` holds by
Carayol's theorem (the universal ring of an absolutely irreducible
residual representation is topologically generated by traces) together
with Chebotarev density (Frobenius traces at `q ∉ {2, ℓ}` suffice).

References: Mazur, *Deforming Galois representations*; Ramakrishna,
*On a variation of Mazur's deformation functor*; Carayol, *Formes
modulaires et représentations galoisiennes à valeurs dans un anneau
local complet*; Böckle's appendix to Khare's Serre-conjecture notes. -/
theorem exists_universal_hardlyRamifiedDeformation (hℓ5 : 5 ≤ ℓ)
    ...
    ∃ D : HardlyRamifiedDeformation hℓOdd ρbar, D.IsUniversal :=
  sorry

/-- **Finiteness stratum** (sorry node): the universal hardly ramified
deformation ring is finite as a `ℤ_ℓ`-module.

This is the potential-modularity / Taylor–Wiles–Kisin input of
Khare–Wintenberger: the residual-modularity hypothesis is bypassed via
potential modularity (Taylor's Moret-Bailly argument), which shows the
universal ring is a finite `ℤ_ℓ`-module — "`R = T`" over a suitable
totally real field, descended by Khare–Wintenberger's induction. Any
`D` satisfying `IsUniversal` is isomorphic to the genuine universal ring
(see the docstring of `IsUniversal`), so the statement transfers.

References: Khare–Wintenberger, *Serre's modularity conjecture (I)*,
Thm. 4.1 and §4; Taylor, *Remarks on a conjecture of Fontaine and
Mazur*; Buzzard's 2026 EPSRC course, Lecture 4. -/
...
/-- **Presentation stratum** (sorry node): the universal hardly ramified
deformation ring has characteristic zero — `ℤ_ℓ` embeds.

Mathematical content (Böckle; Khare–Wintenberger §4): the universal ring
admits a presentation `R^{univ} ≅ ℤ_ℓ[[x₁,…,x_g]]/(f₁,…,f_r)` with
`g − r ≥ 1`, from the global Euler characteristic formula and Poitou–Tate
duality with balanced local conditions at `ℓ`, `2` and `∞`. A complete
Noetherian local `ℤ_ℓ`-algebra presented with strictly more generators
than relations has Krull dimension `≥ 1` (Krull's height theorem), so it
is infinite; if `ℤ_ℓ → R^{univ}` had nonzero kernel `(ℓ^n)` then
`R^{univ}` would be a module-finite algebra over `ℤ/ℓ^n` (finiteness by
`moduleFinite_of_isUniversal`), hence a finite set — contradiction. Any
`D` satisfying `IsUniversal` is isomorphic to the genuine universal ring,
so the statement transfers. -/
theorem algebraMap_injective_of_isUniversal (hℓ5 : 5 ≤ ℓ)
    {ρbar : GaloisRep ℚ (ZMod ℓ) V} (h : IsHardlyRamified hℓOdd hdim ρbar)
    (hirr : ρbar.IsIrreducible)
    (D : HardlyRamifiedDeformation hℓOdd ρbar) (hD : D.IsUniversal) :
    letI := D.commRing; letI := D.algebra
    Function.Injective (algebraMap ℤ_[ℓ] D.R) :=
  sorry

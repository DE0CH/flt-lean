...
/-- **Carayol trace-descent leaf** (sorry node — the trace-generation
half of the representability stratum): a weakly universal hardly
ramified deformation can be replaced by one that is *also*
trace-generated.
...
Frobenius characteristic polynomials. Since `ρbar` is absolutely
irreducible (odd irreducible 2-dimensional over `𝔽_ℓ`, `ℓ` odd) and by
Chebotarev the traces of the whole representation are limits of traces
of Frobenii, Carayol's lemma conjugates `D.ρ` into `GL₂(R')`; the
descended datum `D'` over `R'` inherits the hardly ramified conditions,
is trace-generated by construction (its `charFrob` agree with those of
`D.ρ`, whose coefficients topologically generate `R'`), and is still
weakly universal: any deformation `D''` receives `D.R → D''.R` by weak
universality of `D`, and the restriction to `R'` is compatible with the
reduction maps and the `charFrob` because those data live in `R'`.
(Alternatively: the genuine universal datum is weakly universal and, by
Carayol's theorem, trace-generated — the hypothesis `D` then serves as
the input from which the universal quotient is carved.)
...
/-- **Böckle presentation leaf** (sorry node — the arithmetic core of
the presentation stratum): the weakly universal, trace-generated hardly
...
Mathematical content (Böckle; Khare–Wintenberger §4): choosing a basis
of the mod-`ℓ` tangent space of the deformation functor gives a
surjection `φ : ℤ_ℓ[[x₁,…,x_g]] ↠ R^{univ}` with
`g = dim H¹_{HR}(G_{ℚ,S}, ad ρbar)`; obstruction theory generates
`ker φ` by `r = dim H²_{HR}(G_{ℚ,S}, ad ρbar)` power series. The global
Euler characteristic formula and Poitou–Tate duality, with the balanced
local conditions at `ℓ` (flat), `2` (tame quadratic) and `∞` (`ρbar` is
odd, so `dim H⁰(ℝ, ad) = 2`), give `g − r ≥ 1`. The kernel is stated as
the *span* of the `fᵢ` (not its closure): finitely generated ideals of
the Noetherian complete local ring `ℤ_ℓ[[x₁,…,x_g]]` are closed. As
with the finiteness leaf, the hypotheses pin `D` down up to canonical
isomorphism, so a future proof may construct its own universal datum,
present IT, and transport along `exists_ringEquiv_of_isUniversal`.

...
-- MERGE DEDUPLICATION (2026-07-26): a second, independently written
-- `det_pushforwardFrame` stood here — same statement, same two-step proof
-- (`LinearMap.det_conj` for the framing identification,
-- `LinearMap.det_baseChange` for the base change), contributed by a
-- concurrent owner of this file. Having both made the module fail to
-- elaborate outright (`has already been declared`), which silently blocked
-- every downstream module including `Modularity/KhareWintenberger.lean`.
-- The surviving copy is the earlier one above (search
-- `det_pushforwardFrame`); it is used both by the fibre-product argument
-- there and by the descent below, and the two statements were literally

...
`𝔪' = 𝔪 ∩ R'`** (sorry leaf — isolated 2026-07-26 as the SINGLE arithmetic
...
the two leaves below). Everything except step 2 is already available in
this module:
...
3. That makes `R' ⧸ (𝔪ⁿ ∩ R')` the coefficient ring of a FINITE hardly
   ...
   `φₙ : Dᵘ.R → R' ⧸ (𝔪ⁿ ∩ R')`. It is SURJECTIVE: its image is a subring
   containing the `ℤ_ℓ`-image and every `charFrob` coefficient, and at
   finite level those GENERATE — no topological closure is involved.
4. `φₙ` is local (it commutes with the two reductions), so
   `𝔪' ⧸ (𝔪ⁿ ∩ R')` is the image of `𝔪ᵘ`, hence generated by the images
   of the `r` generators; lift them to `𝔪'`.
...
`R' ⧸ (𝔪ⁿ ∩ R')` is an object of `HardlyRamifiedDeformation hℓOdd ρbar`
...
`subring_closure_charFrob_coeff_eq_top` undischargeable. It costs a
...
* `exists_uniform_span_maximalIdeal_traceSubring` — a UNIFORM bound on
  ...
  one `r` for all levels `n`. **The only open node of this cluster.**

...
* The CARAYOL leaf was then decomposed once more, into
  ...
  **That third leaf was CIRCULAR, and was REMOVED on 2026-07-26 by an
  interface change rather than proven.** The descent's `π_surjective`
  obligation demands that the descended ring's residue field be `k` on
  the nose, while over `ℤ_ℓ` the residue field of `R'` is only the
  Frobenius-trace field of `ρbar`. That was not an artifact of the split
  — the node's own conclusion implies it — and it is trivially true at
  `k = 𝔽_ℓ` (the Frey-curve consumer in `Lift.lean`), but NOT in
  general.

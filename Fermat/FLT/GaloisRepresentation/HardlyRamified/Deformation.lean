...
-- (2026-07-25, flt-lean-95) A SECOND copy of `det_pushforwardFrame` stood
-- here, with the identical statement and an equivalent proof: two owners
-- each added the lemma and the merge kept both, so the module failed to
-- elaborate with `'GaloisRepresentation.det_pushforwardFrame' has already
-- been declared` and every downstream module was blocked. The surviving
-- copy is the earlier one (above, before `isHardlyRamified_of_fibreProduct`),
-- which is the one Lean was already using; both consumers — the
-- fibre-product determinant reflection and the descent below — take the
-- same statement from it.

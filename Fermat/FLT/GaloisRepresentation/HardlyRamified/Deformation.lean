...
`exists_isLocalRing_traceSubring`, `exists_framedGaloisRep_traceSubring`,
`subring_closure_charFrob_coeff_eq_top` (the three leaves into which
`exists_isTraceGenerated_ringHom_of_forall_trace_mem` was DECOMPOSED
2026-07-25 — respectively the coefficient-ring structure of Carayol's
trace subring `R'`, Carayol's Théorème 1 proper (the conjugation into
`GL₂(R')`), and the statement that `k` is the trace field of `ρbar`;
the descent assembly and the trace generation of the descended datum,
`traceSubring_eq_top_of_charFrob_map`, are PROVEN),

...
assemblies — the commutative-algebra strata of the minimal presentation
`exists_coefficientRing_ringHom`,
`exists_mvPowerSeries_ringHom_of_mem_maximalIdeal` (the surjectivity and
minimality strata `surjective_of_mvPowerSeries_ringHom` and
`ker_le_of_minimal_mvPowerSeries_ringHom` were PROVEN 2026-07-25)
together with the arithmetic relation count

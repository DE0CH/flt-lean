...
seven, of which the surjectivity and minimality strata
`surjective_of_mvPowerSeries_ringHom` and
`ker_le_of_minimal_mvPowerSeries_ringHom` were PROVEN the same day; and
every statement they replace is now PROVEN here.
...
`ℤ_ℓ` when `k = 𝔽_ℓ`), and the clauses pin `Λ` up to isomorphism.

CLAUSE ADDED 2026-07-25: `IsPrecomplete (𝔪_Λ) Λ`. The surjectivity
stratum `surjective_of_mvPowerSeries_ringHom` is FALSE without it (see
its docstring for the counterexample `Λ = ℤ_(ℓ)`, `R = ℤ_ℓ`), and it
costs this leaf nothing: mathlib's `WittVector.isAdicCompleteIdealSpanP`
is exactly `IsAdicComplete (span {(ℓ : 𝕎 k)}) (𝕎 k)`, which is the
maximal ideal by the clause below.

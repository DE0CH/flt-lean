...
-- (A second copy of `det_pushforwardFrame`, with the same statement and
-- an equivalent proof, stood here until 2026-07-26; two owners had added
-- the lemma independently, above and here, and the duplicate made the
-- WHOLE MODULE fail to elaborate with `has already been declared` — a
-- hard error that emits no `declaration uses 'sorry'` warning and so is
-- invisible to every frontier scan. Deleted; the copy above serves both
-- the fibre-product argument and the descent below.)

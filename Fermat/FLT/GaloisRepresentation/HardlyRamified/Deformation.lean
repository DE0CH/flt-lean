...
four were narrowed 2026-07-25: Mazur representability was first reduced
to the representation-level classifying datum rather than its `charFrob`
shadow, and then CUT along Schlessinger's architecture into the
Artinian-level representability leaf and the pro-finite limit leaf; the
Carayol leaf receives the Chebotarev density step as a hypothesis; and
the finiteness leaf is stated as `𝔪`-primarity of `(ℓ)` — the
trace-shadow glue, the Schlessinger assembly, the density step and the

...
Intended proof. (1) The subgroup `M⁰ ⊆ M` of points `φ` with
`φ (1 ⊗ e₀) = 1` — the geometric points of the connected component of
the identity — is `Γ ℚ₃ᵥ`-stable because `e₀` is `𝒪ᵥ`-rational. (2)
`hconn` (equivalently `displacement_point_apply_idempotent_eq_one`,
ibid., PROVEN) says the inertia displacement `(σ ∘ φ) ⋆ φ⁻¹` of ANY
point lands in `M⁰`; that is the third bullet once `M⁰` is identified
with the `w₁`-line. (3) `M⁰` is an
...
`g ∈ Γ ℚ`. Writing `d g = f (ρ g w₀) − f w₀`:

* `E g − a g ∈ 𝔪` for every `g`: the two triangularisations along `w₁`
  and along `w₀` agree modulo `𝔪V` because `w₁ − w₀ ∈ 𝔪V` (and `ρ g`
  and the scalars preserve `𝔪V`), and `w₁` is residually nonzero, so
  `mem_maximalIdeal_of_smul_mem_smul_top` extracts the scalar.
* `f w₁ ∈ 𝔪ⁿ⁺¹`. At an element `g₀` with `ω g₀ ≠ 1` — one exists by
...
theorem omega_defect_vanishes_on_cyclotomicKernel_of_connectedEtale
  ...
  -- spanning a `Γ ℚ`-stable line modulo `𝔪ⁿ⁺²V`, with diagonal entry `E`
  obtain ⟨w₁, hw₁, ⟨E, hE⟩, -⟩ :=
  ...
  -- the two triangularisations agree residually
  have hEa : ∀ g : Γ ℚ, E g - a g ∈ IsLocalRing.maximalIdeal R := by
    intro g
    ...
    have hexp : (E g - a g) • w₁ =
      ...
      abel
    rw [hexp]
    refine Submodule.sub_mem _ (Submodule.add_mem _ (ha g) ?_) (hmle (hE g))
  ...
  -- off the cyclotomic kernel the diagonal entry `E` is `≡ -1`, so `E - 1`
  -- is a unit: `2` is invertible because `3 ∈ 𝔪`
  obtain ⟨g₀, hg₀⟩ := exists_cyclotomicCharacterModL_three_ne_one
  have hunit : IsUnit (E g₀ - 1) := by
    have hnot : E g₀ - 1 ∉ IsLocalRing.maximalIdeal R := by
      intro hmem
      have h1 : a g₀ + 1 ∈ IsLocalRing.maximalIdeal R := (hres g₀).2 hg₀
      have h2 : (2 : R) ∈ IsLocalRing.maximalIdeal R := by
        have h3 : ((E g₀ - a g₀) - (E g₀ - 1)) + (a g₀ + 1) = (2 : R) := by ring
        rw [← h3]
        exact Ideal.add_mem _ (Ideal.sub_mem _ (hEa g₀) hmem) h1
  ...
  have hfw₁ : f w₁ ∈ IsLocalRing.maximalIdeal R ^ (n + 1) := by
    have hz : f (ρ g₀ w₁ - E g₀ • w₁) ∈
        IsLocalRing.maximalIdeal R ^ (n + 2) :=
      linearMap_apply_mem_of_mem_smul_top f (hE g₀)
    have hkey : (E g₀ - 1) * f w₁ ∈ IsLocalRing.maximalIdeal R ^ (n + 1) := by
      have hexp : (E g₀ - 1) * f w₁ =
          (f (ρ g₀ w₁) - f w₁) - f (ρ g₀ w₁ - E g₀ • w₁) := by
        rw [map_sub, map_smul, smul_eq_mul]
        ring
      rw [hexp]
      exact Ideal.sub_mem _ (hf g₀ w₁) (Ideal.pow_le_pow_right (by omega) hz)
    obtain ⟨u, hu⟩ := hunit
    have heq : f w₁ = (↑u⁻¹ : R) * ((E g₀ - 1) * f w₁) := by
      rw [← hu, ← mul_assoc, u.inv_mul, one_mul]
    rw [heq]
    exact Ideal.mul_mem_left _ _ hkey
  -- on the cyclotomic kernel the diagonal entry is `≡ 1`
  have hEσ : E σ' - 1 ∈ IsLocalRing.maximalIdeal R := by
    ...
    rw [h2]
    exact Ideal.add_mem _ (hEa σ') h1
  ...
  have hz : f (ρ σ' w₁ - E σ' • w₁) ∈ IsLocalRing.maximalIdeal R ^ (n + 2) :=
    linearMap_apply_mem_of_mem_smul_top f (hE σ')

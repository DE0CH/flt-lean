...
/-! ## DELETED 2026-08-01: the orphaned Raynaud `3`-power cluster (18 declarations, 2247 lines)

Everything that stood here was **DEAD CODE** — proven or sorried material that no
declaration in the tree reached — and it is deleted rather than repaired.  It was
recovered by `flt-lean-256`, which had been dispatched at the `sorry` inside it.

WHAT WAS HERE.  Thirteen helper theorems, then the three-theorem cluster

    exists_uniform_pow_localInertia_smul_connected_of_threeTorsion_trivial   (SORRY LEAF)
    exists_uniform_pow_localInertia_smul_connected_of_threeTorsion_uniform   (proven over it)
    exists_uniform_pow_localInertia_smul_connected_of_hopf_package           (proven over that)

and two further helpers (`eq_of_unipotent_of_pow_coprime_three`,
`pow_sub_one_apply_mem_maximalIdeal_pow_smul_top`).

WHY IT IS DEAD, measured rather than assumed.  The cluster's consumer was
`exists_ordinary_line_of_flat_hopf_package`, which was deleted with the refuted
Raynaud `e = 1` cone in `0c5c678c` (and in `dc6836b9` before it); the name occurs
**nowhere in the tree** today, on `main` or on `merger`.  So the top of the cluster,
`..._of_hopf_package`, had a transitive consumer closure of exactly ONE declaration —
itself — while the file's surviving Raynaud route (`connected_locus_smul_of_hopf_package`
and `connected_locus_le_line_of_hopf_package`, immediately below) reaches
`fermat_last_theorem` through 20 487 declarations.  The two routes are independent;
`0c5c678c`'s own commit message records that `exists_connectedEtale_line_of_hopf_package`
"never consumed them".

THIS DELETION WAS ALREADY COMPUTED ONCE.  `flt-lean-172` (`1cec200b`) removed the
`..._uniform` theorem as free-floating.  `0c5c678c` declined that half in order to
preserve `flt-lean-293`'s proof of it — but a proof of a theorem nothing consumes is
exactly free-floating code, so the island survived and went on drawing dispatches.
The whole 18-declaration set is the FIXPOINT of "no surviving consumer", computed on
the comment-stripped in-file usage graph and cross-checked against every other file
under `Fermat/`; the compiler then confirmed it, the file elaborating with no new error.

RECOVERY.  `git show fe5131ca:Fermat/FLT/GaloisRepresentation/HardlyRamified/Threeadic.lean`,
lines 2180 to 4426.  Two things in there are worth reading before anyone re-derives them:

* `le_span_singleton_sup_smul_pow_of_displacement_surjective` carried a counterexample
  (`N = A(1,0) + A(0,3)` over `ZMod 9`) showing that the naive Nakayama step everyone
  reaches for is FALSE.  It is clean, it was never poisoned by the refutation, and it is
  the one item here whose loss CLAUDE.md warns about by name.
* The deleted leaf's docstring carried a full route analysis of Raynaud's dévissage
  ("`3`-torsion of multiplicative type implies multiplicative type") together with a
  refutation of every one-element form of its hypothesis.  The mathematics is still
  owed by the two surviving leaves below; the analysis is still accurate; only the
  cluster's framing was an artefact of the deleted route.

DO NOT RESTORE THIS BLOCK to close a frontier count.  The statements are not refuted,
but nothing consumes them, and re-adding them re-creates the phantom.  If the Raynaud
dévissage is ever wanted in this vocabulary again, state it where a live consumer needs
it — today that is the two leaves below.
-/
set_option backward.isDefEq.respectTransparency false in
set_option synthInstance.maxHeartbeats 1000000 in
set_option maxHeartbeats 2000000 in

...
/-! ## RESTORED release 40 (2026-08-03): the Raynaud `3`-power cluster (18 declarations)

Deleted in `62fb25fe` (2026-08-01) on a measured audit — at that time the cluster's
top theorem `exists_uniform_pow_localInertia_smul_connected_of_hopf_package` had a
transitive consumer closure of exactly one declaration, itself.  A PARALLEL branch,
cut before that deletion landed, rewrote `connected_locus_le_line_of_hopf_package`
(below) to CONSUME the cluster — its docstring says "It is now consumed", and its
proof also cites the helpers `mem_smul_top_of_one_tmul_quotient_eq_zero`,
`eq_of_unipotent_of_pow_coprime_three` from this block.  That proof's own closure
reaches `fermat_last_theorem`, so on the merged tree the deletion audit is inverted:
the block is load-bearing, and the deletion tombstone's own restoration condition —
"state it where a live consumer needs it" — is met in exactly this vocabulary.
Restored verbatim from `fe5131ca` lines 2180–4426 (byte-identical to the deletion's
parent); the file's preamble above is byte-identical to what the block elaborated
against.  The full deletion audit, worth reading, is in `62fb25fe`'s message and in
the tombstone it left (recoverable with `git show b11bdfe5:<this file>`).

This re-opens the SORRY leaf
`exists_uniform_pow_localInertia_smul_connected_of_threeTorsion_trivial` — no longer
a phantom: it now has real consumers.  It is queued for a prover this release.
-/
set_option synthInstance.maxHeartbeats 1000000 in
...
below. This is the RAYNAUD DÉVISSAGE half; it carries NO residual
input.)

Setting: `G` a finite flat `𝒪₃ᵥ ≅ ℤ₃`-Hopf order with étale generic
fibre, `e₀` its connected counit idempotent, and `fG` a
`Γ ℚ₃ᵥ`-equivariant bijection from the geometric points of the generic
fibre onto `N`. Write `N⁰` for the CONNECTED LOCUS — the `z : N` whose
point has value `1` on `e₀`.

Hypothesis `hstab₃` is the ONE-DIMENSIONALITY input, in the shape
`OortTate.exists_muType_coordinate`'s `hstab` takes: local inertia
moves every `3`-TORSION connected vector to a MULTIPLE OF ITSELF. The
conclusion upgrades that pointwise, `3`-torsion statement to a UNIFORM
power on the whole of `N⁰`: one `m : ℕ` with `ρ'(σ) z = m • z` for
EVERY connected `z`, of whatever `3`-power order.

ROUTE (Raynaud, Bull. SMF 102 (1974), 3.3.2–3.3.6; Oort–Tate; Tate,
*Finite flat group schemes* §4).

1. `hstab₃` puts every `3`-torsion connected point in the situation of
   the PROVEN
   `OortTate.connected_cyclic_point_smul_eq_conv_pow_cyclotomicCharacter`
   (`Fermat/FLT/GroupScheme/ConnectedEtale.lean`), whose two halves are
   `inertia_character_trivial_or_cyclotomic` (Raynaud's dichotomy at
   `e = 1 < p − 1 = 2`) and `not_inertia_character_trivial_of_connected`
   (connectedness kills the unramified branch). So `σ • φ = φ ^ n` for
   every `n ≡ χ_cyc(σ) mod 3`, i.e. `𝒢⁰[3]` is of MULTIPLICATIVE type.
2. DÉVISSAGE up the `3`-power filtration: a finite flat group scheme
   over `ℤ₃` whose `3`-torsion is multiplicative is itself
...
Cartan, on which no vector is moved to a multiple of itself. So that
`G` fails `hstab₃` and is excluded, which is exactly the point of the
hypothesis.
...
conclusions are twist-blind. The conclusion is a VALUE-level identity in
`N`; no element of `G`, no coordinate and no `Γ`-wide rationality is
...
**The check that would refute it**: exhibit a finite flat `ℤ₃`-group
scheme with étale generic fibre whose `3`-torsion connected points are
inertia-power-stable but on which inertia does NOT act by a uniform
power — i.e. a connected finite flat group scheme over `ℤ₃` with
multiplicative `3`-torsion that is not itself of multiplicative type.
Raynaud at `e = 1 < p − 1` says there is none. -/
theorem exists_uniform_pow_localInertia_smul_connected_of_hopf_package
    {A : Type*} [CommRing A] [TopologicalSpace A]
    {N : Type*} [AddCommGroup N] [Module A N]
    (ρ' : GaloisRep ℚ A N)
    (G : Type) [CommRing G] [HopfAlgebra 𝒪₃ᵥ G] [Module.Flat 𝒪₃ᵥ G]
    ...
    (fG : Additive (ℚ₃ᵥ ⊗[𝒪₃ᵥ] G →ₐ[ℚ₃ᵥ] ℚ₃ᵥᵃˡᵍ) →+[Γ ℚ₃ᵥ]
      ((ρ'.toLocal 𝔭₃).Space))
    (hfG : Function.Bijective fG)
    (hstab₃ : ∀ τ ∈ localInertiaGroup 𝔭₃, ∀ z : N, (3 : ℕ) • z = 0 →
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm z))
          ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1 →
      ∃ k : ℕ, (ρ'.toLocal 𝔭₃) τ z = k • z)
    (σ : Γ ℚ₃ᵥ) (hσ : σ ∈ localInertiaGroup 𝔭₃) :
    ∃ m : ℕ, ∀ z : N,
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm z))
          ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1 →
...
arithmetic input whatever).

If `u` is an `A`-endomorphism of `M`, `z : M` is killed by `3`, some
`u ^ K` with `3 ∤ K` fixes `z`, and `(u − 1) ^ m` kills `z` for some `m`,
then `u z = z`.

PROOF, and it deliberately avoids characteristic-`3` Frobenius identities
in `End A M` (which is not an `𝔽₃`-algebra). The two-step case is the
whole content: if `(u − 1) ² y = 0` then `w := (u − 1) y` is `u`-fixed, so
`u ^ j y = y + j • w` by induction, and `u ^ K y = y` gives `K • w = 0`;
also `3 • w = 0` since `3 • y = 0`. So `addOrderOf w` divides
`gcd 3 K = 1`, i.e. `w = 0`. The general case peels the exponent one step
at a time: `(u − 1) ^ (j + 2) z = 0` says `(u − 1) ² ((u − 1) ^ j z) = 0`,
and `(u − 1) ^ j z` is still `3`-torsion and still fixed by `u ^ K`
(because `u` commutes with `u − 1`), so the two-step case lowers the
exponent to `j + 1`.

This is where the TAMENESS of the connected `3`-torsion is converted into
a fixed-point statement, and it is the only place `3 ∤ K` is used. -/
theorem eq_of_unipotent_of_pow_coprime_three {A : Type*} [CommRing A]
    {M : Type*} [AddCommGroup M] [Module A M]
    (u : Module.End A M) (z : M) (h3 : (3 : ℕ) • z = 0)
    (K : ℕ) (hK : ¬ (3 ∣ K)) (hKz : (u ^ K) z = z)
    (m : ℕ) (hnil : ((u - 1) ^ m) z = 0) :
    u z = z := by
  classical
  -- STEP 1. the two-step case: `(u − 1)² y = 0` already forces `(u − 1) y = 0`
  have key : ∀ y : M, (3 : ℕ) • y = 0 → (u ^ K) y = y →
      (((u - 1) ^ 2 : Module.End A M)) y = 0 → (u - 1) y = 0 := by
    intro y hy3 hyK hy2
    set w : M := (u - 1) y with hw
    have hu2 : (u - 1) w = 0 := by
      have h : (((u - 1) ^ 2 : Module.End A M)) y = (u - 1) ((u - 1) y) := by
    ...
    have huw : u w = w := by
      ...
      exact h
    have huy : u y = y + w := by
      rw [hw, LinearMap.sub_apply, Module.End.one_apply]
      abel
    have hpow : ∀ j : ℕ, (u ^ j) y = y + j • w := by
      intro j
      induction j with
      | zero => simp
      | succ j ih =>
          have h1 : (u ^ (j + 1)) y = u ((u ^ j) y) := by
            rw [pow_succ', Module.End.mul_apply]
          rw [h1, ih, map_add, map_nsmul, huw, huy, succ_nsmul]
          abel
    have hKw : (K : ℕ) • w = 0 := by
      have h := hpow K
      rw [hyK] at h
      simpa using h.symm
    have h3w : (3 : ℕ) • w = 0 := by
      rw [hw, ← map_nsmul, hy3, map_zero]
    have hdvd : addOrderOf w ∣ Nat.gcd 3 K :=
      Nat.dvd_gcd (addOrderOf_dvd_of_nsmul_eq_zero h3w)
        (addOrderOf_dvd_of_nsmul_eq_zero hKw)
    have hgcd : Nat.gcd 3 K = 1 :=
      (Nat.Prime.coprime_iff_not_dvd Nat.prime_three).mpr hK
    rw [hgcd, Nat.dvd_one] at hdvd
    exact AddMonoid.addOrderOf_eq_one_iff.mp hdvd
  -- STEP 2. peel the unipotency exponent down to `1`
  have hcomm : ∀ j : ℕ, Commute (u ^ K) ((u - 1) ^ j) := fun j =>
    (((Commute.refl u).sub_right (Commute.one_right u)).pow_pow K j)
  have main : ∀ j : ℕ, ((u - 1) ^ (j + 1)) z = 0 → (u - 1) z = 0 := by
    intro j
    induction j with
    | zero => intro h; simpa using h
    | succ j ih =>
        intro h
        have hy2 : (((u - 1) ^ 2 : Module.End A M))
            (((u - 1) ^ j : Module.End A M) z) = 0 := by
          have h1 : (((u - 1) ^ 2 * (u - 1) ^ j : Module.End A M)) z = 0 := by
            rw [← pow_add]
            have h2 : 2 + j = j + 1 + 1 := by ring
            rw [h2]
            exact h
          rwa [Module.End.mul_apply] at h1
        have hy3 : (3 : ℕ) • (((u - 1) ^ j : Module.End A M) z) = 0 := by
          rw [← map_nsmul, h3, map_zero]
        have hyK : (u ^ K) (((u - 1) ^ j : Module.End A M) z)
            = ((u - 1) ^ j : Module.End A M) z := by
          have h1 : ((u ^ K * (u - 1) ^ j : Module.End A M)) z
              = ((u - 1) ^ j * u ^ K : Module.End A M) z := by
            rw [(hcomm j).eq]
          rw [Module.End.mul_apply, Module.End.mul_apply, hKz] at h1
          exact h1
        have hstep := key _ hy3 hyK hy2
        refine ih ?_
        rw [pow_succ', Module.End.mul_apply]
        exact hstep
  match m, hnil with
  | 0, hnil =>
      have hz : z = 0 := by simpa using hnil
      rw [hz, map_zero]
  | (j + 1), hnil =>
      have h := main j hnil
      rw [LinearMap.sub_apply, Module.End.one_apply, sub_eq_zero] at h
...
set_option synthInstance.maxHeartbeats 1000000 in
set_option maxHeartbeats 2000000 in
/-- **RESIDUAL UNIPOTENCE along the kernel of `ω`** (helper, PROVEN
2026-07-28): for `g` in the kernel of the mod-`3` cyclotomic character,
`(ρ g − 1) ^ (2 k)` maps `V` into `𝔪ᵏ V`.

This is the only place the RESIDUAL PACKAGE (`hV`, `hρ`, `kk`, `π`, `v₀`)
is spent in the one-dimensionality leaf below, and it is exactly the
`3`-DISTINGUISHED ORDINARY input in its usable form: `hπequiv` makes `ρ̄`
an extension of the TRIVIAL character by a character `a`, `hV` with `hρ`
forces `det ρ̄ = ω` hence `a = ω`
(`residual_twist_eq_cyclotomicCharacterModL` above), so `ω g = 1` gives
`a g ≡ 1 mod 𝔪` and `ρ̄ g` is UNIPOTENT — upper triangular with both
diagonal entries `1` in the adapted basis
(`exists_residual_adapted_basis` / `exists_residual_matrix_entries`
above). Hence `(ρ g − 1) ² V ⊆ 𝔪 V`, and `R`-linearity iterates that to
`(ρ g − 1) ^ (2 k) V ⊆ 𝔪ᵏ V`.

NOTE the quantifier is over ALL of `Γ ℚ` and this is not a widening of an
inertia-only statement: residual triangularity along `ker π` holds for the
whole Galois group by `hπequiv`, which is itself stated over `Γ ℚ`. The
inertia restriction enters only in the consumer, through tameness. -/
theorem pow_sub_one_apply_mem_maximalIdeal_pow_smul_top
    {R : Type u} [CommRing R]
    [Algebra ℤ_[3] R] [Module.Finite ℤ_[3] R]
    [Module.Free ℤ_[3] R] [TopologicalSpace R] [IsTopologicalRing R]
    [IsLocalRing R] [IsModuleTopology ℤ_[3] R]
    ...
    (hπequiv : ∀ g : Γ ℚ, ∀ w : kk ⊗[R] V,
      π ((ρ.baseChange kk) g w) = π w)
    (v₀ : V) (hv₀ : π ((1 : kk) ⊗ₜ[R] v₀) ≠ 0)
    (g : Γ ℚ) (hgω : cyclotomicCharacterModL 3 g = 1) (k : ℕ) (x : V) :
    ((ρ g - 1) ^ (2 * k)) x ∈
      (IsLocalRing.maximalIdeal R ^ k) • (⊤ : Submodule R V) := by
  classical
  obtain ⟨b, hb0π, hb0ne, hb1⟩ :=
    exists_residual_adapted_basis V hV kk hsurj π hπsurj v₀ hv₀
  obtain ⟨a, c, hac⟩ :=
    exists_residual_matrix_entries hV kk hsurj π hπsurj hπequiv (b 0) v₀ hb0π hb0ne
  have ha1 : a g - 1 ∈ IsLocalRing.maximalIdeal R :=
    (residual_twist_eq_cyclotomicCharacterModL V hV hρ kk hsurj π hπsurj hπequiv
      v₀ hv₀ (b 0) hb0π hb0ne a (fun h => (hac h).1) g).1 hgω
  set T : Module.End R V := ρ g - 1 with hT
  have hTb0 : T (b 0) ∈ (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    have h1 : T (b 0) = (ρ g (b 0) - a g • b 0) + (a g - 1) • b 0 := by
      rw [hT, LinearMap.sub_apply, Module.End.one_apply, sub_smul, one_smul]
      abel
    rw [h1]
    exact Submodule.add_mem _ (hac g).1 (Submodule.smul_mem_smul ha1 trivial)
  have hTb1 : T (b 1) - c g • b 0 ∈
      (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    have h1 : T (b 1) - c g • b 0 = ρ g v₀ - (v₀ + c g • b 0) := by
      rw [hT, LinearMap.sub_apply, Module.End.one_apply, hb1]
      abel
    rw [h1]
    exact (hac g).2
  have hTT : ∀ y : V, ((T ^ 2 : Module.End R V)) y = T (T y) := fun y => by
    rw [pow_two, Module.End.mul_apply]
  have hD2b : ∀ i : Fin 2,
      ((T ^ 2 : Module.End R V)) (b i) ∈
        (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    intro i
    fin_cases i
    · show ((T ^ 2 : Module.End R V)) (b 0) ∈ _
      rw [hTT]
      exact apply_mem_smul_top T hTb0
    · show ((T ^ 2 : Module.End R V)) (b 1) ∈ _
      rw [hTT]
      have h2 : T (b 1) = (T (b 1) - c g • b 0) + c g • b 0 := by abel
      rw [h2, map_add, map_smul]
      exact Submodule.add_mem _ (apply_mem_smul_top T hTb1)
        (Submodule.smul_mem _ _ hTb0)
  have hD2 : ∀ y : V,
      ((T ^ 2 : Module.End R V)) y ∈
        (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    intro y
    have hsum : y = ∑ i, b.repr y i • b i := (b.sum_repr y).symm
    rw [hsum, map_sum]
    refine Submodule.sum_mem _ fun i _ => ?_
    rw [map_smul]
    exact Submodule.smul_mem _ _ (hD2b i)
  have hstep : ∀ (J : Ideal R) (y : V), y ∈ J • (⊤ : Submodule R V) →
      ((T ^ 2 : Module.End R V)) y ∈
        (J * IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    intro J y hy
    refine Submodule.smul_induction_on hy (fun r hr w _ => ?_) (fun p q hp hq => ?_)
    · rw [map_smul]
      refine Submodule.smul_induction_on (hD2 w) (fun s hs w' _ => ?_)
        (fun p q hp hq => ?_)
      · rw [smul_smul]
        exact Submodule.smul_mem_smul (Ideal.mul_mem_mul hr hs) trivial
      · rw [smul_add]
  ...
  | zero =>
      intro x
      rw [Nat.mul_zero, pow_zero, pow_zero, Ideal.one_eq_top, Submodule.top_smul,
        Module.End.one_apply]
      exact Submodule.mem_top
  | succ k ih =>
      intro x
      have hk : 2 * (k + 1) = 2 + 2 * k := by ring
      rw [hk, pow_add T 2 (2 * k), Module.End.mul_apply,
        pow_succ (IsLocalRing.maximalIdeal R) k]
      exact hstep _ _ (ih x)
...
set_option synthInstance.maxHeartbeats 1000000 in
set_option maxHeartbeats 2000000 in
/-- **THE `3`-TORSION OF THE CONNECTED PART IS OF MULTIPLICATIVE TYPE**
— Raynaud/Oort–Tate at `e = 1 < p − 1 = 2`, in the ONE shape the rest of
this file consumes.

**SORRY LEAF, cut 2026-08-02 (flt-lean-98)**, out of
`connected_locus_le_line_of_hopf_package` just below, which is now
PROVEN over it (see that theorem's `# HOW IT IS PROVEN` section). It is
the LAST finite-flat input of the rank half, and it is the `hstab₃`
argument of `exists_uniform_pow_localInertia_smul_connected_of_hopf_package`
above — a block that had NO code consumer anywhere in `Fermat/` until
this cut, and is wired in by it.

STATEMENT. `M := (R ⧸ 𝔪ⁿ⁺²) ⊗[R] V` is the space of the congruence
quotient of `ρ`, identified `Γ ℚ₃ᵥ`-equivariantly by `fG` with the
geometric points of the finite flat `𝒪₃ᵥ`-group scheme `Spec G`, and a
point is CONNECTED when it takes the value `1` at the connected counit
idempotent `e₀`. This leaf says local inertia at `3` moves every
`3`-TORSION connected vector to a `ℕ`-MULTIPLE OF ITSELF.

WHY IT IS TRUE. `𝒢⁰[3]`, the `3`-torsion of the connected part, is of
MULTIPLICATIVE TYPE, and on a group of multiplicative type local inertia
acts through the cyclotomic character — by ONE scalar, so a fortiori it
stabilises every line. Multiplicativity is Raynaud's classification at
`e = 1 < p − 1 = 2` (`𝒪₃ᵥ ≅ ℤ₃` is absolutely unramified, and `p = 3`),
combined with the ORDINARITY of the residual representation that this
leaf's hypotheses force: `hπequiv` makes `π` a `Γ ℚ`-invariant
surjection, so `ker π` is a `Γ ℚ`-stable `kk`-LINE with TRIVIAL quotient
character, and `det ρ = χ_cyc` (part of `IsHardlyRamified`) then forces
the sub-character to be `ω`, which is RAMIFIED at `3`. So the flat model
of `ρ̄` is ordinary: its connected part is the `ω`-line, of multiplicative
type, and its étale quotient is the unramified line.

WHY THE HYPOTHESES MAY NOT BE DROPPED — and this is the exact content of
the leaf, so the audit is short and sharp. Without residual reducibility
the statement is **FALSE**: for `E ⧸ ℚ` with good SUPERSINGULAR reduction
at `3` and `G = 𝒪(E[3])`, the model is CONNECTED (so `e₀ = 1` and the
connected locus is everything) and tame inertia acts on `E[3]` through
the level-`2` fundamental characters of `𝔽₉ˣ` (Serre, Invent. Math. **15**
(1972), §1.11 prop. 12) — a nonsplit Cartan, which stabilises NO line, so
no `k` exists. That witness is the one recorded in the FALSITY AUDIT of
`exists_inertia_scalar_on_connected_locus_of_hopf_package` below and in
the `WHY `hstab₃` MAY NOT BE DROPPED` paragraph of
`exists_uniform_pow_localInertia_smul_connected_of_hopf_package` above.
It CANNOT be instantiated here: `hπequiv` makes `ρ̄` residually REDUCIBLE,
hence not `I₃`-simple, whereas the supersingular obstruction needs `E[3]`
irreducible over `I₃`. So `hπequiv`, `hρ` and `hV` are all load-bearing
FOR TRUTH, not decoration, and a prover may not weaken them away.

`hw₀π`/`hw₀ne`/`hv₀`/`hsurj` pin `w₀` as a residual generator of `ker π`
and `v₀` as a lift of the quotient; they are what make "ordinary" a
statement about THIS `ρ` rather than about some model of it. `hmin₀` and
`habs₀` are the minimality and comultiplication-absorption clauses that
`Bialgebra.exists_connected_counit_idempotent` produces, and they are
what make `e₀` the connected component of the IDENTITY rather than an
arbitrary idempotent; `mul_eq_zero_or_mul_eq_of_minimal`
(`Fermat/FLT/GroupScheme/ConnectedEtale.lean`) converts `hmin₀` into the
primitivity form `hprim₀` that the Oort–Tate block wants, and
`Bialgebra.comulAlgHom_apply` converts `habs₀` into `hcomul₀`.
...
character is `χ_cyc · ψ` with `ψ` an UNRAMIFIED twist and no single power
works, so the inertia-only form is the true one and is twist-blind. The
`∀ τ` is likewise load-bearing and may NOT be specialised to the `σ` the
consumer happens to use — see the FALSITY AUDIT of
`exists_uniform_pow_localInertia_smul_connected_of_threeTorsion_trivial`
above, where exactly that specialisation is refuted. The conclusion is a
VALUE-level identity in `M`: no element of `G`, no coordinate and no
`Γ`-wide rationality is produced, so the leaf is on the true side of this
development's `𝒪ᵥ`-descent rule and blind to the `p − 1` unramified twists
`μ₃ ⊗ ψ` that killed `exists_muType_closure`.

WHERE TO GET IT. This is precisely the `hstab` hypothesis of the PROVEN
`OortTate.exists_muType_coordinate` and of
`OortTate.connected_cyclic_point_smul_eq_conv_pow_cyclotomicCharacter`
(both in `Fermat/FLT/GroupScheme/ConnectedEtale.lean`, both sorry-free),
so closing it hands the whole Oort–Tate cyclotomic node to this file with
nothing further owed. `HardlyRamified/Family.lean` carries the same
content coefficient-free and intrinsically on the corner
(`isMultiplicativeType_corner_of_inertiaLevelOneFlag`, over the still-open
`isMultiplicativeType_of_connected_of_inertiaLevelOneFlag`), and
`Threeadic` CANNOT import `Family` (`Family.lean:16` is
`public import Fermat.FLT.Modularity.Interface`, and `Interface.lean`
imports this file, so the edge is a CYCLE) — so a relocation would have to
be an extraction DOWNWARD into a module both files import.
`Fermat/FLT/Mathlib/RingTheory/HopfAlgebra/Diagonalizable.lean` wires
`HopfAlgebra.IsMultiplicativeType`, `HopfAlgebra.cornerIdeal`,
`HopfAlgebra.isMultiplicativeType_of_isShortExact` and
`HopfAlgebra.etale_of_isShortExact` in with NO cycle (its transitive
closure is nine modules, all under `Fermat/FLT/Mathlib/RingTheory/`, none
of them downstream of this file); add it in the same commit that first
uses it.

Raynaud, *Schémas en groupes de type `(p, …, p)`*, Bull. SMF **102**
(1974), 3.3.2–3.3.6; Oort–Tate, *Group schemes of prime order*, Ann. Sci.
ÉNS **3** (1970); Tate, *Finite flat group schemes*, §4, in
Cornell–Silverman–Stevens; Serre, Invent. Math. **15** (1972), §1.11. -/
theorem connected_threeTorsion_inertia_stable_of_hopf_package
    {R : Type u} [CommRing R]
    [Algebra ℤ_[3] R] [Module.Finite ℤ_[3] R]
    [Module.Free ℤ_[3] R] [TopologicalSpace R] [IsTopologicalRing R]
    [IsLocalRing R] [IsModuleTopology ℤ_[3] R]
...
set_option synthInstance.maxHeartbeats 1000000 in
set_option maxHeartbeats 2000000 in
/-- **RAYNAUD LEAF (b): THE CONNECTED LOCUS HAS RANK AT MOST ONE, AND
ANY RESIDUALLY NONZERO CONNECTED VECTOR GENERATES IT.**

**PROVEN 2026-08-02 (flt-lean-98)**, over the single new leaf
`connected_threeTorsion_inertia_stable_of_hopf_package` immediately
above, together with its already-open sibling
`connected_locus_smul_of_hopf_package` and the already-open
`exists_uniform_pow_localInertia_smul_connected_of_threeTorsion_trivial`
(reached through the PROVEN
`exists_uniform_pow_localInertia_smul_connected_of_hopf_package`). It
was a SORRY LEAF from 2026-07-31 (flt-lean-124); the direct-sorry count
of this file is unchanged at `3` — one leaf closed, one opened — and what
changed is stated under `# WHAT THE CUT BOUGHT` below.
...
(`1 ⊗ w₁ ≠ 0` in `kk ⊗[R] V`, i.e. `w₁ ∉ 𝔪V`), every `x ∈ M⁰` is an
`R`-multiple of `w₁` modulo `𝔪ⁿ⁺²V`.

WHY IT IS TRUE. `M⁰` is the connected part of a finite flat model of
`ρ ⧸ 𝔪ⁿ⁺²`, and the whole point of the residual re-basing (see the
`# WHY THIS STATEMENT IS TRUE` section of
`exists_connectedEtale_line_of_hopf_package` below) is that `M⁰` is FREE
OF RANK ONE over `R ⧸ 𝔪ⁿ⁺²`:

* residually, `hπequiv`/`hπsurj` make `ker π` a `Γ ℚ`-stable `kk`-LINE
  of `kk ⊗[R] V` with trivial quotient character, and `det ρ = χ_cyc`
  (part of `IsHardlyRamified`) forces the sub-character to be `ω`, which
  is RAMIFIED at `3`; so the flat model of `ρ̄` is ordinary with
  connected part exactly the `kk`-line `ker π = kk · (1 ⊗ w₀)`, of
  `kk`-dimension `1`;
* `V` is free of rank `2` over `R`, so `kk ⊗[R] V` has `kk`-dimension
  `2` for EVERY coefficient ring — the dimension bound is FREE at the
  residual level, which is precisely why the route was re-based there;
* lifting to `𝔪ⁿ⁺²` is Wiles's dévissage (*Modular elliptic curves and
  Fermat's Last Theorem*, ch. 1 prop. 1.1) over Raynaud's uniqueness of
  prolongations at `e = 1 < p − 1 = 2`: the `𝔪`-adic filtration of
  `V ⧸ 𝔪ⁿ⁺²V` becomes a filtration of the flat model by finite flat
  subgroup schemes with residual graded pieces, each of whose connected
  parts is of multiplicative type by the previous item, and an extension
  of multiplicative by multiplicative is multiplicative
  (`HopfAlgebra.isMultiplicativeType_of_isShortExact`, PROVEN in
  `Fermat/FLT/Mathlib/RingTheory/HopfAlgebra/ShortExact.lean` over
  `HopfAlgebra.etale_of_isShortExact` and `IsShortExact.cartierDual`,
  with no classification and no `e < p − 1` spent). So `M⁰` is cyclic of
  the same length as `R ⧸ 𝔪ⁿ⁺²`, i.e. free of rank one.

Given that, the conclusion is Nakayama in its safe form: write
`M⁰ = (R ⧸ 𝔪ⁿ⁺²) · w`; then `w₁ = r₁ • w` with `r₁ ∉ 𝔪` (else
`w₁ ∈ 𝔪V`, against `hw₁ne`), so `r₁` is a unit and `w₁` generates too.
That last step is written out in the deleted proof (recoverable, see
below) and needs no flat input; what this leaf owes is the freeness.

**A WARNING THE DELETED PROOF RECORDED, worth keeping.** The naive
Nakayama argument — "`M⁰` has residually one-dimensional image and
contains a residually nonzero vector, hence is generated by it" — is
FALSE. Residual one-dimensionality controls `N ⧸ (N ∩ 𝔪V)`, whereas
Nakayama needs `N ⧸ 𝔪N`, and the two differ: over `A = ℤ ⧸ 9` with
`V = A²` the submodule `N = A · (1,0) + A · (0,3)` has residual image
the line `kk · (1,0)` and contains the residually nonzero `(1,0)`, yet
`(0,3)` is not an `A`-multiple of `(1,0)`. So freeness of `M⁰` really
does have to come from the flat theory; it cannot be talked out of the
residual picture. `le_span_singleton_sup_smul_pow_of_displacement_surjective` is the
pure-algebra lemma that packages the correct substitute — surjectivity of
a displacement operator on `M⁰` — and **it is PRESENT AND PROVEN in this
file** (the paragraph that stood here until 2026-08-02 said it had been
deleted with the refuted cone; that was false, it survived because it
never touched the refuted leaf). It is what the proof below runs on.

FAITHFULNESS. Like its sibling, the conclusion is a VALUE-level
statement over `𝒪ᵥ` and asks for no element of `G` and no `Γ`-rational
coordinate, so the `𝒪ᵥ`-descent rule of `CLAUDE.md` is satisfied and the
unramified twists `μ₃ ⊗ ψ` are invisible to it. Note also that `hw₁ne`
is not decoration: without it the statement is FALSE at `w₁ = 0` as soon
as `M⁰ ≠ 𝔪ⁿ⁺²V`.

# HOW IT IS PROVEN (2026-08-02)

The route is the displacement one the warning above points at, and every
step below the new leaf is already in this file:

1. take `σ ∈ I₃` with `ω σ ≠ 1`
   (`exists_localInertia_cyclotomicCharacterModL_three_ne_one`) and let
   `g₀` be its image in `Γ ℚ`; the residual matrix entries
   (`exists_residual_matrix_entries`) plus the determinant identification
   (`residual_twist_eq_cyclotomicCharacterModL`) make `a g₀ − 1` a UNIT —
   the one place `p = 3` is spent, since `3 ∈ 𝔪` turns `a g₀ ≡ −1` into
   `a g₀ − 1 ≡ 1`;
2. the new leaf feeds
   `exists_uniform_pow_localInertia_smul_connected_of_hopf_package`
   above, giving ONE `m : ℕ` with `ρ'(σ) z = m • z` for EVERY connected
   `z`. Pulled back to `V` through `mem_smul_top_of_one_tmul_quotient_eq_zero`
   this reads `ρ g₀ y ≡ (m : R) • y mod 𝔪ⁿ⁺²V` for connected `y`;
3. `m` is IDENTIFIED with `a g₀` modulo `𝔪` by testing (2) on the
   inertia displacement `u := ρ g₀ w₀ − w₀`, which is connected for free
   (`inertia_displacement_apply_connected_idempotent_eq_one`) and is
   residually `(a g₀ − 1) • w₀`; since `a g₀ − 1` is a unit and `w₀` is
   residually nonzero, `(m : R) − 1` is a UNIT too;
4. that unit does BOTH remaining jobs. It makes the displacement
   operator `D y = ρ g₀ y − y` SURJECTIVE on the connected locus
   (`y := ((m : R) − 1)⁻¹ • z`, connected by the sibling leaf), and it
   forces every connected `z` into `ker π` residually — apply `π` to
   `ρ g₀ z ≡ (m : R) • z` and use `hπequiv`, so `((m : kk) − 1) π(1 ⊗ z) = 0`
   with `(m : kk) − 1 ≠ 0`. A finrank count identifies `ker π` with the
   `w₀`-line, which is the `hNp` hypothesis;
5. `le_span_singleton_sup_smul_pow_of_displacement_surjective` above then
   gives `M⁰ ≤ R ∙ (D w₀) ⊔ 𝔪ⁿ⁺²V`, and the Nakayama endgame transfers
   the generator from `D w₀` to the given `w₁`: `w₁ = c₁ • D w₀` modulo
   `𝔪ⁿ⁺²V`, and `c₁ ∉ 𝔪` because otherwise `w₁ ∈ 𝔪V`, against `hw₁ne`.

The connected locus is assembled into an honest `Submodule R V` on the
way: closure under addition is the comultiplication-absorption block
(`convMul_apply_one_of_comul_absorbs` against `hcomul₀`), `0` is
connected because it is the displacement of `0`, and scalars are the
sibling leaf.

# WHAT THE CUT BOUGHT

The count is unchanged (`3 → 3` direct sorries in this file), so it must
be judged by what is LEFT. Three things changed.

* The residue is now a statement about `3`-TORSION only: no `n`, no
  congruence level, no dévissage, no `w₁`, no Nakayama. It is exactly the
  `hstab` hypothesis of the PROVEN `OortTate.exists_muType_coordinate`,
  i.e. Oort–Tate/Raynaud at `e = 1 < p − 1` and nothing else.
* Wiles's dévissage up the `𝔪`-adic filtration — the expensive half of
  the route this docstring used to prescribe — is GONE from the frontier.
  It is replaced by step (2)'s uniform power, which
  `exists_uniform_pow_localInertia_smul_connected_of_hopf_package` already
  derives from the `3`-torsion statement.
* `exists_uniform_pow_localInertia_smul_connected_of_{threeTorsion_trivial,
  threeTorsion_uniform,hopf_package}` had **no code consumer anywhere in
  `Fermat/`** before this cut — a proven block anchored on an open leaf,
  i.e. free-floating, which this project forbids. It is now consumed.

WHY THE 2026-07-29 REFUTATION DOES NOT REACH IT. Proven in this file
until 2026-07-31 over `connected_locus_cyclic_of_hopf_package`, hence
over the REFUTED
`eq_one_of_smul_eq_mul_localInertia_connected_threeTorsion`, so the old
proof is void. The witness of that refutation (`E = 37a1`, supersingular
at `3`; a CONNECTED order-`27` object, socle of dimension `3`, wild
inertia unipotent) cannot arise here: `hπequiv` makes `ρ̄` residually
REDUCIBLE, hence not `I₃`-simple, and the supersingular obstruction
needs `E[3]` irreducible over `I₃`. The residual socle here has
`kk`-dimension `2`, exactly the range in which the refuted leaf's own
route was sound. Recover the void proof with

    git show dc6836b9^:Fermat/FLT/GaloisRepresentation/HardlyRamified/Threeadic.lean

at `connected_locus_le_line_of_hopf_package` (line `7754` there), using
`git log -m -S` rather than `git log -S`, since the deletion happened
inside what became a merge.

WHERE TO GET IT: see the closing section of
...
Fermat's Last Theorem*, ch. 1 prop. 1.1; Tate, *Finite flat group
schemes*, §4, in Cornell–Silverman–Stevens; Oort–Tate, *Group schemes of
prime order*, Ann. Sci. ÉNS **3** (1970). -/
theorem connected_locus_le_line_of_hopf_package
  ...
  -- primitivity and comultiplication absorption, the forms the block below wants
  have hprim₀ : ∀ y : G, IsIdempotentElem y → y * e₀ = 0 ∨ y * e₀ = e₀ :=
    fun y hy => mul_eq_zero_or_mul_eq_of_minimal he₀ hε₀ hmin₀ y hy
  have hcomul₀ : Coalgebra.comul (R := 𝒪₃ᵥ) e₀ * (e₀ ⊗ₜ[𝒪₃ᵥ] e₀) =
      e₀ ⊗ₜ[𝒪₃ᵥ] e₀ := by
    rwa [Bialgebra.comulAlgHom_apply] at habs₀
  -- the uniform inertia power on the connected locus
  obtain ⟨m, hm⟩ := exists_uniform_pow_localInertia_smul_connected_of_hopf_package
    ...
    hprim₀ hcomul₀ fG hfG
    (connected_threeTorsion_inertia_stable_of_hopf_package V hV hρ kk hsurj π
      hπsurj hπequiv v₀ hv₀ w₀ hw₀π hw₀ne n G fG hfG e₀ he₀ hε₀ hmin₀ habs₀)
    σ hσ
  -- transfer the uniform power down to `V`: for a CONNECTED `y`, the inertia
  -- displacement is multiplication by `m` modulo `𝔪ⁿ⁺²V`
  have hmV : ∀ y : V,
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm
      ...
      ρ g₀ y - (m : R) • y ∈
        (IsLocalRing.maximalIdeal R ^ (n + 2)) • (⊤ : Submodule R V) := by
    intro y hy
    have h1 := hm ((1 : R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2))) ⊗ₜ[R] y) hy
    rw [GaloisRep.toLocal_apply, GaloisRep.baseChange_tmul, ← hg₀] at h1
    refine mem_smul_top_of_one_tmul_quotient_eq_zero _ ?_
    rw [TensorProduct.tmul_sub, h1, TensorProduct.tmul_smul,
      Nat.cast_smul_eq_nsmul, sub_self]
  -- the inertia displacement of `w₀`: connected, and residually `(a g₀ − 1) • w₀`
  have huconn : (Additive.toMul ((Equiv.ofBijective fG hfG).symm
    ...
    exact h
  have hures : (ρ g₀ w₀ - w₀) - (a g₀ - 1) • w₀ ∈
      (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    have h1 : (ρ g₀ w₀ - w₀) - (a g₀ - 1) • w₀ = ρ g₀ w₀ - a g₀ • w₀ := by
      rw [sub_smul, one_smul]; abel
    rw [h1]; exact ha g₀
  have hple : (IsLocalRing.maximalIdeal R ^ (n + 2)) • (⊤ : Submodule R V) ≤
      (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) :=
    Submodule.smul_mono (Ideal.pow_le_self (by omega)) le_rfl
  have hkey : ((a g₀ - 1) * (a g₀ - (m : R))) • w₀ ∈
      (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    have A1 : ρ g₀ (ρ g₀ w₀ - w₀) - (m : R) • (ρ g₀ w₀ - w₀) ∈
        (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) :=
      hple (hmV _ huconn)
    have A2 : ρ g₀ (ρ g₀ w₀ - w₀) - (a g₀ - 1) • (ρ g₀ w₀) ∈
        (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
      have h2 : ρ g₀ (ρ g₀ w₀ - w₀) - (a g₀ - 1) • (ρ g₀ w₀)
          = (ρ g₀ : V →ₗ[R] V) ((ρ g₀ w₀ - w₀) - (a g₀ - 1) • w₀) := by
        simp only [map_sub, map_smul]
      rw [h2]
      exact apply_mem_smul_top _ hures
    have A3 : (a g₀ - 1) • (ρ g₀ w₀ - a g₀ • w₀) ∈
        (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) :=
      Submodule.smul_mem _ _ (ha g₀)
    have A4 : (m : R) • ((ρ g₀ w₀ - w₀) - (a g₀ - 1) • w₀) ∈
        (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) :=
      Submodule.smul_mem _ _ hures
    have hid : ((a g₀ - 1) * (a g₀ - (m : R))) • w₀ =
        (ρ g₀ (ρ g₀ w₀ - w₀) - (m : R) • (ρ g₀ w₀ - w₀))
        - (ρ g₀ (ρ g₀ w₀ - w₀) - (a g₀ - 1) • (ρ g₀ w₀))
        - (a g₀ - 1) • (ρ g₀ w₀ - a g₀ • w₀)
        + (m : R) • ((ρ g₀ w₀ - w₀) - (a g₀ - 1) • w₀) := by
      module
    rw [hid]
    exact Submodule.add_mem _ (Submodule.sub_mem _ (Submodule.sub_mem _ A1 A2) A3) A4
  -- hence `m ≡ a g₀ mod 𝔪`, so `(m : R) − 1` is a UNIT
  have hunitm : IsUnit ((m : R) - 1) := by
    have hmem := mem_maximalIdeal_of_smul_mem_smul_top kk hsurj hw₀ne hkey
    have hne1 : a g₀ - 1 ∉ IsLocalRing.maximalIdeal R :=
      IsLocalRing.notMem_maximalIdeal.mpr hunit
    have hma : a g₀ - (m : R) ∈ IsLocalRing.maximalIdeal R := by
      rcases (IsLocalRing.maximalIdeal.isMaximal R).isPrime.mem_or_mem hmem with h | h
      · exact absurd h hne1
      · exact h
    refine IsLocalRing.notMem_maximalIdeal.mp fun hmm => hne1 ?_
    have : a g₀ - 1 = ((m : R) - 1) + (a g₀ - (m : R)) := by ring
    rw [this]
    exact Submodule.add_mem _ hmm hma
  -- `ker π` is exactly the residual `w₀`-line
  haveI hfinkk : Module.Finite kk (kk ⊗[R] V) :=
    Module.Finite.of_basis ((Module.Free.chooseBasis R V).baseChange kk)
  have hfr : Module.finrank kk (kk ⊗[R] V) = 2 :=
    Module.finrank_eq_of_rank_eq (by rw [Module.rank_baseChange, hV]; simp)
  have hker1 : Module.finrank kk (LinearMap.ker π) = 1 := by
    have h := LinearMap.finrank_range_add_finrank_ker π
    rw [LinearMap.range_eq_top.mpr hπsurj, finrank_top, Module.finrank_self, hfr] at h
    omega
  ...
  -- a connected vector is killed by `π` residually, because inertia scales it
  -- by `m` and `π` is inertia-invariant while `(m : R) − 1` is a unit
  have hNpi : ∀ z : V,
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm
      ...
      π ((1 : kk) ⊗ₜ[R] z) = 0 := by
    intro z hz
    have h1 : (1 : kk) ⊗ₜ[R] (ρ g₀ z - (m : R) • z) = 0 :=
      one_tmul_eq_zero_of_mem_maximalIdeal_smul_top kk hsurj (hple (hmV z hz))
    rw [TensorProduct.tmul_sub, sub_eq_zero, TensorProduct.tmul_smul] at h1
    have h2 : (ρ.baseChange kk) g₀ ((1 : kk) ⊗ₜ[R] z) =
        (m : R) • ((1 : kk) ⊗ₜ[R] z) := by
      rw [GaloisRep.baseChange_tmul]; exact h1
    have h3 := hπequiv g₀ ((1 : kk) ⊗ₜ[R] z)
    rw [h2, ← algebraMap_smul kk ((m : R)) ((1 : kk) ⊗ₜ[R] z), map_smul,
      smul_eq_mul] at h3
    have hun : IsUnit (algebraMap R kk ((m : R) - 1)) := hunitm.map _
    have hne0 : algebraMap R kk (m : R) - 1 ≠ 0 := by
      have := hun.ne_zero
      rwa [map_sub, map_one] at this
    have h4 : (algebraMap R kk (m : R) - 1) * π ((1 : kk) ⊗ₜ[R] z) = 0 := by
      rw [sub_mul, one_mul, h3, sub_self]
    rcases mul_eq_zero.mp h4 with h | h
    · exact absurd h hne0
    · exact h
  -- the connected locus is closed under addition (comultiplication absorption)
  have hfs : ∀ t : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2))) ⊗[R] V, fG ((Equiv.ofBijective fG hfG).symm t) = t :=
    fun t => (Equiv.ofBijective fG hfG).apply_symm_apply t
  have hgs_add : ∀ t s : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2))) ⊗[R] V,
      (Equiv.ofBijective fG hfG).symm (t + s) =
        (Equiv.ofBijective fG hfG).symm t + (Equiv.ofBijective fG hfG).symm s := by
    intro t s
    apply (Equiv.ofBijective fG hfG).injective
    show fG ((Equiv.ofBijective fG hfG).symm (t + s)) =
      fG ((Equiv.ofBijective fG hfG).symm t + (Equiv.ofBijective fG hfG).symm s)
    rw [map_add fG, hfs, hfs, hfs]
  have hPadd : ∀ t s : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2))) ⊗[R] V,
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm t))
        ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1 →
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm s))
        ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1 →
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm (t + s)))
        ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1 := by
    intro t s ht hs
    have ht' : (AlgHom.liftEquiv 𝒪₃ᵥ ℚ₃ᵥ G ℚ₃ᵥᵃˡᵍ).symm
        (Additive.toMul ((Equiv.ofBijective fG hfG).symm t)) e₀ = 1 := by
      rw [AlgHom.liftEquiv_symm_apply]; exact ht
    have hs' : (AlgHom.liftEquiv 𝒪₃ᵥ ℚ₃ᵥ G ℚ₃ᵥᵃˡᵍ).symm
        (Additive.toMul ((Equiv.ofBijective fG hfG).symm s)) e₀ = 1 := by
      rw [AlgHom.liftEquiv_symm_apply]; exact hs
    rw [hgs_add, toMul_add, ← AlgHom.liftEquiv_symm_apply,
      vendored_mul_eq_convMul, liftEquiv_symm_convMul]
    exact convMul_apply_one_of_comul_absorbs e₀ hcomul₀ _ _ ht' hs'
  -- `0` is connected: it is the inertia displacement of `0`
  have hzeroconn : (Additive.toMul ((Equiv.ofBijective fG hfG).symm
      ((1 : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2)))) ⊗ₜ[R] (0 : V)))) ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1 := by
    have h := inertia_displacement_apply_connected_idempotent_eq_one
      (ρ.baseChange (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2)))) G e₀ he₀ hε₀ fG hfG σ hσ (0 : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2))) ⊗[R] V)
    rw [map_zero, sub_zero] at h
    rw [TensorProduct.tmul_zero]
    exact h
  -- the inertia displacement operator, kept OPAQUE (only its values matter)
  obtain ⟨D, hDapp⟩ : ∃ D : V →ₗ[R] V, ∀ y : V, D y = ρ g₀ y - y :=
    ⟨(ρ g₀ : V →ₗ[R] V) - LinearMap.id, fun y => by simp⟩
  -- the connected locus as an `R`-submodule of `V`
  set Nsub : Submodule R V :=
    { carrier := {y : V | (Additive.toMul ((Equiv.ofBijective fG hfG).symm
        ((1 : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2)))) ⊗ₜ[R] y))) ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1}
      add_mem' := by
        intro y z hy hz
        have h := hPadd _ _ hy hz
        rwa [← TensorProduct.tmul_add] at h
      zero_mem' := hzeroconn
      smul_mem' := fun r y hy =>
        connected_locus_smul_of_hopf_package ρ n G fG hfG e₀ he₀ hε₀ hmin₀
          habs₀ r y hy } with hNsub
  have hmemN : ∀ y : V, y ∈ Nsub ↔
      (Additive.toMul ((Equiv.ofBijective fG hfG).symm
        ((1 : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2)))) ⊗ₜ[R] y))) ((1 : ℚ₃ᵥ) ⊗ₜ[𝒪₃ᵥ] e₀) = 1 := fun _ => Iff.rfl
  -- every inertia displacement is connected
  have hDN : ∀ y : V, D y ∈ Nsub := by
    intro y
    rw [hmemN, hDapp]
    have h := inertia_displacement_apply_connected_idempotent_eq_one
      (ρ.baseChange (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2)))) G e₀ he₀ hε₀ fG hfG σ hσ ((1 : (R ⧸ (IsLocalRing.maximalIdeal R ^ (n + 2)))) ⊗ₜ[R] y)
    rw [GaloisRep.toLocal_apply, GaloisRep.baseChange_tmul,
      ← TensorProduct.tmul_sub, ← hg₀] at h
    exact h
  -- the connected locus is residually inside the `w₀`-line
  have hNp : Nsub ≤ Submodule.span R {w₀} ⊔
      (IsLocalRing.maximalIdeal R) • (⊤ : Submodule R V) := by
    intro z hz
    have hz0 := hNpi z ((hmemN z).1 hz)
    have hmem : (1 : kk) ⊗ₜ[R] z ∈ Submodule.span kk {(1 : kk) ⊗ₜ[R] w₀} := by
      rw [hspan]; exact hz0
    obtain ⟨μ, hμ⟩ := Submodule.mem_span_singleton.mp hmem
    obtain ⟨r, hr⟩ := hsurj μ
    have hzero : (1 : kk) ⊗ₜ[R] (z - r • w₀) = 0 := by
      rw [TensorProduct.tmul_sub, TensorProduct.tmul_smul,
        ← algebraMap_smul kk r ((1 : kk) ⊗ₜ[R] w₀), hr, hμ, sub_self]
    have hmm := mem_maximalIdeal_smul_top_of_one_tmul_eq_zero kk hsurj hzero
    have hzz : z = r • w₀ + (z - r • w₀) := by abel
    rw [hzz]
    exact Submodule.add_mem _
      (Submodule.mem_sup_left (Submodule.mem_span_singleton.mpr ⟨r, rfl⟩))
      (Submodule.mem_sup_right hmm)
  -- the displacement operator is SURJECTIVE on the connected locus
  have hDsurj : ∀ z ∈ Nsub, ∃ y ∈ Nsub,
      z - D y ∈
        (IsLocalRing.maximalIdeal R ^ (n + 2)) • (⊤ : Submodule R V) := by
    intro z hz
    obtain ⟨v, hv⟩ := hunitm.exists_right_inv
    refine ⟨v • z, Nsub.smul_mem v hz, ?_⟩
    have h1 := hmV (v • z) ((hmemN _).1 (Nsub.smul_mem v hz))
    have hmv : (m : R) * v = 1 + v := by linear_combination hv
    have h2 : z - D (v • z) = -(ρ g₀ (v • z) - (m : R) • (v • z)) := by
      rw [hDapp, smul_smul, hmv]
      module
    rw [h2]
    exact Submodule.neg_mem _ h1
  -- CYCLICITY: the connected locus is generated by the displacement of `w₀`
  have hline := le_span_singleton_sup_smul_pow_of_displacement_surjective
    (IsLocalRing.maximalIdeal R) (n + 2) Nsub D w₀ hDN hNp hDsurj
  -- NAKAYAMA: a residually nonzero connected `w₁` generates the same line
  obtain ⟨s₁, hs₁, q, hq, hweq⟩ := Submodule.mem_sup.1 (hline ((hmemN w₁).2 hw₁conn))
  obtain ⟨c₁, hc₁⟩ := Submodule.mem_span_singleton.1 hs₁
  have hc₁unit : IsUnit c₁ := by
    refine IsLocalRing.notMem_maximalIdeal.mp fun hc₁m => ?_
    refine hw₁ne ?_
    refine one_tmul_eq_zero_of_mem_maximalIdeal_smul_top kk hsurj ?_
    rw [← hweq, ← hc₁]
    exact Submodule.add_mem _
      (Submodule.smul_mem_smul hc₁m Submodule.mem_top) (hple hq)
  obtain ⟨d, hd⟩ := hc₁unit.exists_right_inv
  obtain ⟨s₀, hs₀, p, hp, hxeq⟩ := Submodule.mem_sup.1 (hline ((hmemN x).2 hx))
  obtain ⟨r₀, hr₀⟩ := Submodule.mem_span_singleton.1 hs₀
  refine ⟨r₀ * d, ?_⟩
  have hfin : x - (r₀ * d) • w₁ = p - (r₀ * d) • q := by
    rw [← hxeq, ← hweq, ← hr₀, ← hc₁]
    have hdc : (r₀ * d) * c₁ = r₀ := by
      rw [mul_assoc, mul_comm d c₁, hd, mul_one]
    rw [smul_add, smul_smul, hdc]
    abel
  rw [hfin]
  exact Submodule.sub_mem _ hp (Submodule.smul_mem _ _ hq)
...
AGAIN since 2026-07-31 (flt-lean-124)**, over the two Raynaud leaves
`connected_locus_smul_of_hopf_package` and
`connected_locus_le_line_of_hopf_package` above.

# STATUS: WHAT HAPPENED HERE, AND WHY THE CUT MOVED DOWN

Between 2026-07-25 and 2026-07-30 this theorem was PROVEN over a tower
of `35` declarations occupying lines `2119`–`8419` of the pre-cut file,
whose foundation was the arbitrary-Hopf-order Raynaud leaf
`eq_one_of_smul_eq_mul_localInertia_connected_threeTorsion`. That leaf
was REFUTED on 2026-07-29 (witness below) and the refutation was
independently re-verified on 2026-07-30, so a FALSE `sorry` with `39`
live consumers sat in the tree for two days with `three_adic` resting on
it. Commit `dc6836b9` performed the prescribed deletion: the tower went,
and this theorem — TRUE, and vouched for below — became the single leaf.

**This commit reduces that leaf.** The refutation poisons the PROOFS in
the deleted tower, not their STATEMENTS, and only `18` of the `35`
deleted declarations actually reached the false leaf. Walking the
dependency graph of the pre-cut file (`dc6836b9^`) and marking every
declaration that transitively cites
`eq_one_of_smul_eq_mul_localInertia_connected_threeTorsion` shows the
poison enters at
`smul_eq_of_pow_three_smul_eq_localInertia_connected_threeTorsion` and
that the chain from there to here splits cleanly. So the four
declarations restored above this one —
`exists_connected_line_of_hopf_package`,
`exists_localInertia_moves_connected_point_of_hopf_package`,
`exists_connected_line_generator_of_hopf_package`, and this theorem's
own assembly — are PROVEN again with their original, previously verified
proofs, and the finite-flat debt is now carried by exactly two crisp
leaves:

* `connected_locus_smul_of_hopf_package` — Raynaud full faithfulness at
...
argument, which spends NO flat theory — return to the tree. That is the
trade `CLAUDE.md` prescribes ("fewer OPEN leaves after, not fewer leaves
created" is the tie-breaker between RIVAL CUTS of one node; here the two
statements are strictly smaller than the one they replace, and the
mathematics left open is strictly less). Both new statements are
coefficient-free finite-flat facts that match `Family.lean`'s
`isMultiplicativeType_corner_of_inertiaLevelOneFlag` cone directly,
which is what the eventual extraction wants; the old single leaf did
not, because it bundled the Raynaud content together with the residual
bookkeeping that is now proven above.

`exists_localInertia_cyclotomicCharacterModL_three_ne_one`, listed as
...
FIRST STEP FOR WHOEVER PROVES THE TWO LEAVES (this paragraph was written
for the single leaf that used to sit here; it applies verbatim to
`connected_locus_smul_of_hopf_package` and
`connected_locus_le_line_of_hopf_package` above, which is where the
finite-flat debt now lives). The multiplicative-type layer is
...
**Five of the `35` are back as of 2026-07-31** and are struck from the
list below by this paragraph rather than by editing it, so that the
original inventory stays readable:
`exists_localInertia_cyclotomicCharacterModL_three_ne_one` (PROVEN, at
the top of this file), `exists_connected_line_of_hopf_package` (PROVEN,
over the two new leaves), `connected_locus_smul_of_hopf_package` and
`connected_locus_le_line_of_hopf_package` (STATEMENTS restored as the
two `sorry` leaves; their old proofs are void and were NOT restored),
`exists_localInertia_moves_connected_point_of_hopf_package` and
`exists_connected_line_generator_of_hopf_package` (both PROVEN, original
proofs).

The remaining `29` are still gone. Of those, `16` are PROVEN and do NOT
depend on the refuted leaf — `one_tmul_quotient_eq_zero_of_mem_smul_top`,
...
`exists_uniform_pow_localInertia_smul_connected_of_threeTorsion_uniform`
(itself a leaf), `exists_uniform_pow_localInertia_smul_connected_of_hopf_package`,
`eq_of_unipotent_of_pow_coprime_three`,
`pow_sub_one_apply_mem_maximalIdeal_pow_smul_top` — so they are free for
the taking if a route wants them, and in particular
`le_span_singleton_sup_smul_pow_of_displacement_surjective` is the
pure-algebra packaging of the correct (non-naive) Nakayama step that
`connected_locus_le_line_of_hopf_package` will want. They were not
restored here only because nothing consumes them yet and free-floating
code is not allowed.
...
section at the head of this docstring. The two audits quoted above stand
as written; what changed is that the refutation was found to poison only
`18` of the `35` deleted declarations, so the chain from
`connected_locus_smul_of_hopf_package` /
`connected_locus_le_line_of_hopf_package` up to here could be restored
with its original proofs. The mechanical check behind that claim: build
the citation graph of the pre-cut file `dc6836b9^` restricted to the
`35` deleted names, then take the transitive closure of
`eq_one_of_smul_eq_mul_localInertia_connected_threeTorsion`'s consumers.
`17` names lie outside it; `exists_localInertia_moves_connected_point_of_`
`hopf_package` is one of them and is what the restored generator
argument needs, together with
`exists_localInertia_cyclotomicCharacterModL_three_ne_one`, which had
already returned to the top of this file on `merger`. -/

...
public import Fermat.FLT.DedekindDomain.FiniteAdeleRing.TensorPi
public import Fermat.FLT.HaarMeasure.HaarChar.Ring
public import Fermat.FLT.Hacks.RightActionInstances
public import Fermat.FLT.Mathlib.LinearAlgebra.Dimension.Constructions
public import Fermat.FLT.Mathlib.NumberTheory.NumberField.AdeleRing
public import Fermat.FLT.Mathlib.NumberTheory.NumberField.FiniteAdeleRing
public import Fermat.FLT.Mathlib.NumberTheory.NumberField.InfiniteAdeleRing
...
def DinfTensorPiEquivPiTensorAux :
  ...
  map_smul' x y := by
    ...
    -- VENDORING REPAIR (2026-07-27, pin drift `81a5d2` → `a3364fa`). The original
    -- proof compared the two `algebraMap ℝ _ x` terms at the level of the PRODUCT
    -- `∀ i, D ⊗[K] i.Completion`. That no longer works: under the surrounding
    -- `backward.isDefEq.respectTransparency false`, `InfinitePlace K` unfolds to
    -- `{w // ∃ φ, place φ = w}`, the goal stops being type-correct at `instances`
    -- transparency, and `Algebra.smul_def` silently fails to fire on the right-hand
    -- `x • E y` — leaving a goal in which `h₂`'s pattern does not occur at all.
    -- Going pointwise FIRST keeps every `smul` at a single component, where the
    -- `Algebra ℝ (D ⊗[K] vi.Completion)` instance is the honest `Algebra.compHom`
    -- one and the product-level instance is never mentioned.
    funext vi
    show tensorPiEquivPiTensor K D InfinitePlace.Completion (x • y) vi
      = x • (tensorPiEquivPiTensor K D InfinitePlace.Completion y vi)
    simp only [Algebra.smul_def]
    rw [tensorPi_equiv_piTensor_map_mul]
    show (tensorPiEquivPiTensor K D InfinitePlace.Completion
      ((algebraMap ℝ (Dinf K D)) x)) vi * _ = _
    congr 1
    have h₁ : (algebraMap ℝ (Dinf K D)) x
        = 1 ⊗ₜ[K] (algebraMap ℝ (InfiniteAdeleRing K) x) := rfl
    rw [h₁, tensorPi_equiv_piTensor_apply]
    -- `tensorPi_equiv_piTensor_apply` produces an un-beta-reduced `(fun i => …) vi`,
    -- which `rw` cannot see through; `simp only` beta-reduces first.
    simp only [algebraMap_completion]
    rfl
}

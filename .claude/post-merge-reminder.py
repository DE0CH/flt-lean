...
PROMPT = (
    ...
    "CURRENT POOL STATE FIRST. If it reads `reclaiming`, mark it "
    "`suspended`, NOT `free` (Deyao, 2026-07-25): `reclaiming` means "
    "'claimed, but retire this slot when its task finishes' -- it is how the "
    "fleet is being drained from 62 workers down to ~30, because the user "
    ...
    "lengthen every verification instead of adding throughput. Only if the "
    "state reads `claimed` do you mark it back to `free`. Either way REMOVE "
    "ITS LINE "

...
PROMPT = (
    ...
    "TO `staging`, NOT `main`** (Deyao, 2026-07-25). `main` is the last commit "
    "known to BUILD; `staging` is the integration queue. You merge into staging "
    "and NEVER build -- a full cone build costs hours and would make integration "
    "the fleet's bottleneck. The staging worker (a fixed worktree outside the "
    "pool, dispatched with the `{{FLT_STAGING}}` sentinel) snapshots staging onto "
    "`staging-worker`, builds it, fixes what is red, and promotes to main only on "
    "a GREEN build. So: resolve conflicts here with your best effort, and let the "
    "worker catch what only a compiler can. Workers fast-forward to `staging` at "
    "dispatch, NOT main -- a leaf a completion just created exists only on staging "
    "until promotion, so dispatching against main would send an agent to a tree "
    "where its target does not exist; (5) **`git push`** "

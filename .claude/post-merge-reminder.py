...
PROMPT = (
    ...
    "worker catch what only a compiler can. Workers still branch off `main`, so "
    "they always start from a tree that builds; (5) **`git push`** "
    ...
    "main at dispatch, so unpushed work is both unreplicated AND invisible to "

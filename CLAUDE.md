...
## Doctrine library: `doctrine/` — read on demand, never auto-loaded

The fleet's accumulated lessons — dated post-mortems, Lean/mathlib traps,
merge-damage classes, audit failure modes, release-window lore — live in
`doctrine/`, ONE FILE PER LESSON, slug-named after its heading. They are
deliberately NOT in this file: on 2026-08-03 CLAUDE.md had grown to 2.35 MB
(~600k tokens) and every merger session died at the context ceiling within
minutes of spawning. This file stays small so sessions start with room to
work. Do not undo the split.

- Discovery is by grep, not by index (an index would rot):
  `grep -rln '<concept, 2-3 spellings>' doctrine/` then Read the hits.
  Run it before pricing work off any absence claim, before writing any
  "missing from the pin" verdict, and whenever a task prompt cites doctrine.
- NEW lessons go in a NEW file `doctrine/<slug>.md` opening with a `## `
  heading — never appended here. Every line added to CLAUDE.md is paid for
  by every future session of every agent.
- MERGER POLICY for branches cut before 2026-08-03: they still carry
  CLAUDE.md section additions. Resolve every such hunk by landing the added
  section verbatim as `doctrine/<slug>.md` and keeping CLAUDE.md unchanged.
  A CLAUDE.md conflict is never resolved by growing this file.

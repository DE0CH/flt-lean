...
Here `P` was "`γ₂ ∘ γ = c(γ)·γ₂` with `c(γ)³ = 1`" (proven from `η`'s and `E₄`'s `S`- and
`T`-transformations plus `SpecialLinearGroup.SL2Z_generators`, ~50 lines) and `Q` was Weber's
`γ₂(τ₀) ∈ ℚ`, which still needs Shimura reciprocity.

...
**Refinement (Deyao, same day): the tool is trusted, but HOW automation calls it is NOT.** A slow/odd-looking tool run falls in the automation class until the invocation is audited (arguments, batching, flags) — e.g. a per-module build loop instead of one multi-target `lake build` would serialize a parallel tool. Verify the call site BEFORE attributing behavior to the tool.
...
**Addendum (Deyao, 2026-07-23 — "it doesn't exist then how are you telling me about it????"):** never describe designed/instructed/planned things in the present tense as if they exist. Every report states VERIFIED state first (what is on disk / running right now), and labels everything else explicitly as pending, instructed, or planned. An instruction sent to an agent is not a fact about the repo.

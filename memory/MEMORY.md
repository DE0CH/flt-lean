...
- [Carry the conductor, not the localization](flt-carry-the-conductor-not-the-localization.md) — "z ∈ R_p for every p" formalises as an ideal of DENOMINATORS in the base ring; one plumbing lemma instead of Subalgebra iInf, and the hypotheses (noetherian, local, dimension) all drop
- [The consumer supplies the missing stability](flt-consumer-supplies-the-missing-stability.md) — a wall on "closed under localization" is often free for the class the consumer actually holds; state the theorem over the closure hypothesis and discharge it there (regular local ⟹ normal is blocked, smooth stalks are not)

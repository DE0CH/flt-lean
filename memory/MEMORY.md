...
- [Identification pinned geometrically, not modularly](flt-identification-pinned-geometrically-not-modularly.md) — a datum's fibre identification pinned only by an open immersion silently blocks every leaf that composes a moduli pin across the two fibres; add a FIELD charged to the theorem that CHOOSES the model, never a leaf over the datum

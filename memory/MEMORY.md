...
- [Absent from the pin, present in another formulation](theorem-absent-from-pin-may-be-present-in-another-formulation.md) — a correct "Mathlib lacks X" audit hid that HyperellipticJacobian.lean already proves `deg (div g) = 0` over abstract places; grep docstrings for the INFORMAL STATEMENT and the CITATION, not identifiers

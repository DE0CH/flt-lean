...
- [A refactor has TWO bases](flt-refactor-has-two-bases.md) — the green base you can verify on and the merger's base you must land on; main was 868 commits and 60k X0 lines behind merger while merger's X0 was red, so ship the TRANSFORM, not the diff
- [Cycle verdicts expire](flt-cycle-verdict-expires.md) — "no route, genuine import cycle" measured while the leaves were open describes their INTENDED proofs; the Mazur tail's NeronModel cycle is gone on merger and the real blocker is 11 declarations in X1.lean

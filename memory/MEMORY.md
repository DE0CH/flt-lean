...
- [Ask the CAS where the cusps sit](flt-ask-the-cas-where-the-cusps-sit.md) — "Y_0(N) is the affine model" is FALSE at 37 (one cusp affine, one at infinity); get the model AND its distinguished points in one query, then cut the explicit chart as U
- [A monic quadratic needs no Gauss lemma](flt-monic-quadratic-needs-no-gauss.md) — mathlib's degree-≤3 root criterion is stated over a DOMAIN, so `y² = f(x)` needs no fraction field; and "f is not a square" is one evaluation with a negative value, not five coefficients

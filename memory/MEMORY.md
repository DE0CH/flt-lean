...
- [RationalMap.domain is the glue](flt-rationalmap-domain-is-the-glue.md) — "spread out, then glue with the open it extends" is `RationalMap.domain`/`toPartialMap`, one mathlib definition, not a hand-rolled cover; and `@Struct.mk` beats brace notation when instance fields sit between data fields

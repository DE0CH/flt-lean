...
- [Limit machinery vs greedy recursion](flt-limit-machinery-vs-greedy-recursion.md) — an "inverse limit of nonempty FINITE sets" route is a greedy recursion if the level-wise EXTENSION always succeeds; also: a small extension's completeness comes from its TARGET (Artin–Rees), and state regularity algebraically, not as `Continuous`
- [A boilerplate line is not an anchor](flt-boilerplate-line-is-not-an-anchor.md) — a scripted replace on `omit … in` hit 9 lines where 3 were meant, silently editing six proven lemmas; assert count==1 and require `0 deletions` on an insert-only change

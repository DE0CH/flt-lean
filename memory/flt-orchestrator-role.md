...
metadata: 
  ...
  modified: 2026-07-24T22:31:19.776Z
---
...
**Verification gates are NOT the driver's job either (Deyao, 2026-07-24):**
"you shouldn't be working though, you should be queueing task and handling
merge conflicts as we said before." Running composition/insurance diagnostics
on main counts as working — don't. Each agent verifies its own branch with the
blocking report MCP before reporting, and that IS the gate. The driver's whole
job in steady state is: pop the queue, write/queue successor tasks, MERGE and
resolve conflicts, duplicate-scan, free slots, push. If a merge is
hand-resolved and genuinely needs re-verification, dispatch that as a task.

#!/bin/bash
# release 34 merge driver.  Same shape as release 29's, plus:
...
#   * per-branch log files under /tmp/r34/logs so a 341-branch run is auditable.
export PATH="$HOME/.elan/bin:$PATH"
cd ~/flt-staging || exit 1
T=tools/merge
mkdir -p /tmp/r34/logs
for b in "$@"; do
  L=/tmp/r34/logs/$b.log
  echo "########## $b" | tee "$L"
  ...
  git add -A
  git commit --no-edit --allow-empty -m "release 34: merge $b" >>"$L" 2>&1
